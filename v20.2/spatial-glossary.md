...
<span class="version-tag">New in v20.2</span>: CockroachDB has support for efficiently storing and querying spatial data.
...
- _Bounding box_: Given a set of points, a bounding box is the smallest rectangle that encloses all of the points in the set. Due to edge cases in how geographic points are mapped to [cartographic projections](#cartographic-projection), a bounding box for a given set of points may be larger than a human looking at the points might think are needed.

<a name="spheroid"></a>

- _Spheroid_: A spheroid is essentially a "slightly squished" sphere. Spheroids are used to represent almost-but-not-quite spherical objects. For example, a spheroid is used to represent the Earth in the [World Geodetic System](#wgs84) standard.

<a name="cartographic-projection"></a>

- _Cartographic projection_: A cartographic projection, or map projection, is the process by which we represent data that is 3-dimensional (or higher) on a 2-dimensional surface. Practically speaking, this is usually related to how we might display 3-dimensional shapes represented in a database by the [`GEOGRAPHY` data type](#geography) on the surface of a map, which is a flat plane. For more information, see the GIS Lounge article [What is a Map Projection?](https://www.gislounge.com/map-projection/) by Caitlin Dempsey.
...
- _OpenGIS_: The Open Geospatial Consortium (a.k.a. [OGC](#ogc)) was formerly known known as the "Open GIS Consortium". The organization is still referred to colloquially as "OpenGIS" in many places online.

<a name="ogc"></a>

- _OGC_: The [Open Geospatial Consortium](https://www.ogc.org) is a consortium of businesses, government agencies, universities, etc. It was formerly known as the "Open GIS Consortium". See also: [OpenGIS](#opengis).
...
- _Esri_: A company providing "location intelligence" services. Developers of spatial and [GIS](#gis) software, including the popular [ArcGIS](#arcgis) package. For more information about Esri, see <https://www.esri.com>.
...
- _DE-9IM_: The [Dimensionally Extended nine-Intersection Model (DE-9IM)](https://en.wikipedia.org/wiki/DE-9IM) defines a method that uses a 3x3 matrix to determine whether two shapes (1) touch along a boundary, (2), intersect (overlap), or (3) are equal to each other - that is, they are the same shape that covers the same area. This notation is used by the `ST_Relate` built-in function. Notably, almost all other spatial predicate functions can be logically implemented using this model. In practice, most are not, and `ST_Relate` is reserved for advanced use cases.
...
- `GEOMETRY`: Used to represent shapes relative to 2- or 3-dimensional plane geometry (or 4-dimensional if the "time" dimension is added to track the motion of objects).

<a name="geography"></a>

- `GEOGRAPHY`: Used to represent shapes relative to locations on the earth's surface. Depending on their size, these shapes need to take into account the curvature of the Earth, which is not a perfect sphere, but a [spheroid](#spheroid), a.k.a. "a sphere with a bulge".
...
- _WKB_: The "Well Known Binary" data format is a convenient machine-readable notation for representing [spatial objects](#spatial-objects). For efficiency, an application may choose to use this data format, but humans may prefer to read [WKT](#wkt). This format is defined by the [OGC](#ogc).
...
- _GeometryCollection_: A geometry collection is a "box" or "bag" in which to group 1 or more of of the other types of objects defined above into a collection: namely, points, linestrings, or polygons. In the particular case of SQL, it provides a way of referring to a group of spatial objects as one "thing" so that you can operate on it/them more conveniently, using various SQL functions.
