...
To change the bucket size of an existing hash-sharded primary key index, use an [`ALTER PRIMARY KEY`](alter-primary-key.html) statement with a [`USING HASH WITH BUCKET_COUNT = n_buckets` clause](sql-grammar.html#opt_hash_sharded) that specifies the new bucket size. Note that when you use an `ALTER PRIMARY KEY` statement that only changes the bucket size of a hash-sharded primary key index, the old primary key (and the old bucket size) are permanently discarded.
