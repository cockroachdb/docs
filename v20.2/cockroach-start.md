...
`--accept-sql-without-tls` | This experimental flag allows you to connect to the cluster using a SQL user's password without [validating the nodes' certificates](authentication.html#client-authentication). When connecting to the cluster using a [connection string](connection-parameters.html), you can omit the `sslmode` parameter from the string. When connecting using the built-in SQL client, [use the `--insecure` flag with the `cockroach sql` statement](cockroach-sql.html#client-connection).
