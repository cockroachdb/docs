...
To verify that intents may be causing an issue, visit DB Console → Advanced Debug → Custom Chart and create a chart for the metric `intentcount`. This will show the number of intents present over time. The following query can also be used to get intent counts by range:
