...
### Slow (hung) backups and other operations due to write intent buildup

Transactions do not always clean up their [write intents](architecture/transaction-layer#write-intents) (newly written values) when they are committed or rolled back, due to known bugs. Garbage collection is also rather slow to react to them. This can cause the amount of unresolved write intents to build up over time. While this isn't necessarily a problem in itself, some operations do not handle large amounts of intents well. In particular, backups and other operations that touch large numbers of values may become very slow and appear to hang.

To verify that intents may be causing an issue, visit the DB Console and create a custom chart for the metric `intentcount`. This will show the number of intents present over time. The following query can also be used to get intent counts by range:
...
* Do a high-priority scan of the table, which will resolve intents as it runs. Note that this may abort any conflicting transactions that are currently running. If the table has indexes, these can be cleaned by changing `<table>` into `<table>@<index>`. The key ranges in the intent query above will output numeric table identifiers that can be entered as `[<tableid>]` instead of the table name.
...
* Manually enqueue the range for garbage collection. Go into the DB Console → Advanced Debug → Enqueue Range. Select "Queue: gc", enter the range ID as output by the intent query above, check "SkipShouldQueue", and press "Submit". The operation will succeed on the leaseholder node and error on the others -- this is expected.

The progress and effect of the cleanup can be monitored via the custom DB Console chart described above.
