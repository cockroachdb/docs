...
6. [Enable an {{ site.data.products.enterprise }} license](licensing-faqs.html#obtain-a-license).
    {{site.data.alerts.callout_info}} You need the {{ site.data.products.enterprise }} license if you want to use the GSSAPI feature. However, if you only want to test that the GSSAPI setup is working, you do not need to enable an {{ site.data.products.enterprise }} license. {{site.data.alerts.end}}
...
4. If you specified an {{ site.data.products.enterprise }} license earlier, you should now have a Postgres shell in CockroachDB, indicating that the GSSAPI authentication was successful. If you did not specify an {{ site.data.products.enterprise }} license, you'll see a message like this: `psql: ERROR:  use of GSS authentication requires an {{ site.data.products.enterprise }} license.` If you see this message, GSSAPI authentication is set up correctly.
