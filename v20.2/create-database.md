...
`CONNECTION LIMIT` | <span class="version-tag">New in v20.2:</span> Supported for compatibility with PostgreSQL. By default, CockroachDB does not limit connections, and `CONNECTION LIMIT = -1`. This clause is a no-op for the default value (`-1`), and all other values return an error.
