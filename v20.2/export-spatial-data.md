...
SQL type information is lost in the conversion to CSV, such that the `tornadoes.sql` file output by the `ogr2ogr` command above thinks every non-geometry field is a [`VARCHAR`](string.html). After loading this data into another database (including another CockroachDB instance), you will have to run [`ALTER TYPE`](alter-type.html) statements to restore the data's SQL types.
{{site.data.alerts.end}}
