...
summary: Learn how to use CockroachDB from a simple Go application with the upper/db data access tool.
toc: true
twitter: false
---
...
This tutorial shows you how build a simple Go application with CockroachDB and the [upper/db](https://upper.io/) data access tool.
...
Issue the following statements to create the `accounts` table in the `bank` database:
...
Change to the directory where you cloned the repo and get the dependencies with `go init mod`:

{% include copy-clipboard.html %}
~~~ shell
$ go init mod hello-world-go-upperdb
~~~
