...
The example below returns `false` due to the additional criterion for `ST_Contains` that at least one point in the interior of _B_ (the line below) lies in the interior of _A_ (the triangle). A query against these shapes with [`ST_Covers`](st_covers.html) will return `true`.
