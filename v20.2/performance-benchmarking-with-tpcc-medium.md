...
<img src="{{ 'images/v20.2/tpcc13k.png' | relative_url }}" alt="TPC-C 13,000" style="border:1px solid #eee;max-width:100%" />
...
2. On the VM with the `workload` binary, briefly run TPC-C to set up partitioning. Bump the file descriptor limits with `ulimit` to the high value shown below to make sure there are plenty of descriptors, since the workload generators create a lot of database connections.
