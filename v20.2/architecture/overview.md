...
This guide details how CockroachDB is built, but does not explain how to architect an application using CockroachDB. For more information about how to develop applications that use CockroachDB, check out our [Developer Guide](../developer-guide-overview.html).
...
- Make life easier for humans. This means being low-touch and highly automated for operators and simple to reason about for [developers](../developer-guide-overview.html).
...
With the confluence of these features, we hope that CockroachDB helps you build global, scalable, resilient applications.
...
Once the CockroachDB cluster is initialized, developers interact with CockroachDB through a [PostgreSQL-compatible](../postgresql-compatibility.html) SQL API. Thanks to the symmetrical behavior of all nodes in a cluster, you can send [SQL requests](sql-layer.html) to any of them; this makes CockroachDB easy to integrate with load balancers.
