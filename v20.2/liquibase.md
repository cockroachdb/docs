...
We have tested Liquibase enough to claim **beta-level** support. If you encounter problems, please [open an issue](https://github.com/cockroachdb/cockroach/issues/new) with details to help us make progress toward full support.
...
1. [Install CockroachDB](install-cockroachdb.html), and [start a secure cluster](secure-a-cluster.html). When starting your cluster, make sure that you generate cluster certificates, create the `bank` database, and create the `max` user with password `roach`.
1. Download and install a Java Development Kit. Liquibase supports Java 8+. In this tutorial, we use AdoptOpenJDK 11.
...
1. Append the full path of the `liquibase` binary (now located in the `liquibase-3.10.2-bin` folder) to your machine's `PATH` environment variable:
    ...
    {% include copy-clipboard.html %}
    ~~~ shell
    $ echo "export PATH=$PATH:/path-to-download/liquibase-3.10.2-bin" >> ~/.bash_profile
    ~~~
...
Download the JDBC driver from [the PostgreSQL website](https://jdbc.postgresql.org/download.html), and place the driver in the directory of your choice.
...
When you [started a secure CockroachDB cluster](secure-a-cluster.html), you should have created a user `max` with password `roach`. You should have also given this user the [`admin` role](authorization.html#admin-role), which grants all privileges to all databases on the cluster. In this tutorial, Liquibase runs schema changes as the `max` user.
...
1. Create the SQL file specified by the first changeset:
...
{{site.data.alerts.callout_info}}
You can also set Liquibase properties on-the-fly with the `liquibase` command-line tool.
{{site.data.alerts.end}}

1. Create a `liquibase.properties` file:
...
1. Add the following property definitions to the file:
    ...
    ~~~ yml
    changeLogFile: /full-path/changelog-main.xml
    driver: org.postgresql.Driver
    classpath: /full-path/postgresql-x.x.jar
    url: jdbc:postgresql://localhost:26257/bank?ssl=true&sslmode=require&sslrootcert=/full-path/certs/ca.crt&sslkey=/full-ath/certs/client.max.key.pk8&sslcert=/full-path/certs/client.max.crt
    username: max
    password: roach
    ~~~
      - For `changeLogFile`, specify the full path of the `changelog-main.xml` file.
      - For `classpath`, specify the full path of [the JDBC driver](#step-2-download-the-postgresql-jdbc-driver).
...
To run Liquibase from the command line, execute the following command from the directory containing your `liquibase.properties` file:
...
-----+--------+-----------------------------------------+----------------------------------+---------------+----------+------------------------------------+--------------------------------------------------------------------------------------------------------+----------+------+-----------+----------+--------+----------------
  1  | root   | /path/liquibase-demo/changelog-main.xml | 2020-07-30 11:16:46.507829+00:00 |             1 | EXECUTED | 8:567321cdb0100cbe76731a7ed414674b | sqlFile                                                                                                |          | NULL | 3.10.2    | crdb     | NULL   | 6122206453
  2  | max    | /path/liquibase-demo/changelog-main.xml | 2020-07-30 11:16:46.554696+00:00 |             2 | EXECUTED | 8:c2945f2a445cf60b4b203e1a91d14a89 | insert tableName=account; insert tableName=account; insert tableName=account; insert tableName=account |          | NULL | 3.10.2    | crdb     | NULL   | 6122206453
(2 rows)
~~~

You can also query the `account` table directly to see the latest changes reflected in the table format:
...
-----+--------+-----------------------------------------+----------------------------------+---------------+----------+------------------------------------+--------------------------------------------------------------------------------------------------------+----------+------+-----------+----------+--------+----------------
  1  | root   | /path/liquibase-demo/changelog-main.xml | 2020-07-30 11:16:46.507829+00:00 |             1 | EXECUTED | 8:567321cdb0100cbe76731a7ed414674b | sqlFile                                                                                                |          | NULL | 3.10.2    | crdb     | NULL   | 6122206453
  2  | max    | /path/liquibase-demo/changelog-main.xml | 2020-07-30 11:16:46.554696+00:00 |             2 | EXECUTED | 8:c2945f2a445cf60b4b203e1a91d14a89 | insert tableName=account; insert tableName=account; insert tableName=account; insert tableName=account |          | NULL | 3.10.2    | crdb     | NULL   | 6122206453
  3  | max    | /path/liquibase-demo/changelog-main.xml | 2020-07-30 11:19:36.450805+00:00 |             3 | EXECUTED | 8:7b76f0ae200b1ae1d9f0c0f78979348b | sqlFile                                                                                                |          | NULL | 3.10.2    | crdb     | NULL   | 6122376070
  4  | max    | /path/liquibase-demo/changelog-main.xml | 2020-07-30 11:19:36.84275+00:00  |             4 | EXECUTED | 8:facd6f3627d02ffe05eeb69c06b3b90f | sqlFile                                                                                                |          | NULL | 3.10.2    | crdb     | NULL   | 6122376070
(4 rows)
~~~
