...
A MultiPoint can be created from SQL by calling an aggregate function such as `ST_Collect` or `ST_Union` on a column that contains [Point](point.html) geometries.  In the example below, we will build a MultiPoint from several Points.
...
Next, build a MultiPoint from the individual [Points](point.html) using `ST_Union`, and check the output with `ST_GeometryType` to verify that it is indeed a MultiPoint:

{% include copy-clipboard.html %}
~~~ sql
SELECT st_geometrytype(st_union(geom)) AS output FROM tmp_points;
~~~
