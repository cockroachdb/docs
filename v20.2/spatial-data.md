...
- Geographic objects, which are also made up of points, lines, polygons, etc., in 2-dimensional space. They are projected onto the surface of a sphere and are represented in SQL by the `GEOGRAPHY` data type. (Technically, they are projected onto a spheroid: "a sphere with a bulge"). This projection means that paths between geographic objects are not straight lines; they are curves, and so the distances between objects are calculated using [great circle math](https://en.wikipedia.org/wiki/Great-circle_distance).
...
We recommend using `GEOMETRY` as much as possible for best performance, and converting to the `GEOGRAPHY` type only where needed by your application.

In order to avoid full table scans, make sure to add indexes to any columns that are frequently accessed. You can check which queries are using which indexes using the [`EXPLAIN`](explain.html) statement. For more information about general query tuning (including index usage), see [Make queries fast](make-queries-fast.html).

The syntax for adding an (inverted) index to a geometry column is:

{% include copy-clipboard.html %}
~~~ sql
CREATE INDEX tornado_geom_idx ON tornadoes USING GIN (geom);
~~~

For more information about how inverted indexes work, see [Inverted indexes](inverted-indexes.html).

Because CockroachDB is a scale-out, multi-node database, our spatial indexing strategy is based on a [space-filling curve](https://en.wikipedia.org/wiki/Space-filling_curve) design (also known as "divide the space"), rather than the [R-Tree](https://en.wikipedia.org/wiki/R-tree) data structure used by some other spatial databases (also known as "divide the objects"). Other databases that use a "divide the space" strategy include Microsoft SQL Server and MongoDB.

We are still working on optimizing our performance for some common use cases. In most cases we expect to be within 2x of the performance of PostGIS for supported queries.
...
Connect to the server from a [SQL client](cockroach-sql.html) installed on your local machine:
...
Follow the steps below to load the SQL for the NYC data used in the [Introduction to PostGIS](https://postgis.net/workshops/postgis-intro/) tutorial.  CockroachDB can work with the tutorial up to Chapter 20.  The exception is that we do not support GML and KML data.

##### Step 1. Load the NYC data

Clone the data set and change to the directory where the SQL files are:
...
{% include copy-clipboard.html %}
~~~ shell
cd otan-scripts/geospatial/geospatial_sql
~~~
...
{% include copy-clipboard.html %}
~~~ shell
cat *sql |cockroach sql --insecure --host=localhost --port=26257
~~~
