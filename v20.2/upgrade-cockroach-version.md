...
{{site.data.alerts.callout_danger}}
Before upgrading to v20.2, you must have fewer than 100 jobs in a non-terminal state (i.e., any state other than `succeeded`, `failed`, or `canceled`). Otherwise, the v20.2 node will hang and never successfully start. Note that `SCHEMA CHANGE GC` jobs are counted even though they are passive.

This will be fixed in a later v20.2 [testing release](releases/#testing-releases). For more context, see [Known Limitations in CockroachDB v20.2](known-limitations.html#upgrading-to-v20-2-with-more-than-100-non-terminal-jobs).
{{site.data.alerts.end}}
