...
| `server.oidc_authentication.scopes` | 'profile email'
...
1. Add the callback URL to the list of **Authorized redirect URIs**. On a local cluster, this will be `https://localhost:8080/oidc/callback`. You will later set `server.oidc_authentication.redirect_url` to the same value. 
...
1. Specify the following values to obtain an OIDC identifier that will be mapped to a SQL user.
	...
	Request the `email` scope from the Access Token:
