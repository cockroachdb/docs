...
- [interpreted literals](#interpreted-literals): these define arbitrary values with an explicit
  type, for example, `INTERVAL '3 days'`; and
...
- [hexadecimal-encoded strings](#hexadecimal-encoded-string-literals)
...
For example, both `x'636174'` and `X'636174'` are equivalent to `'cat'`.
...
A byte array literal uses the same syntax as [string literals containing character escapes](#string-literals-with-character-escapes),
...
- byte array literals always have data type `BYTES`, whereas the data
  type of a string literal depends on context; and
