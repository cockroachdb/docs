...
Once you're done with your test cluster, open a new terminal and stop the first node:
...
Now stop the second and third nodes. Again, you can do this in the same terminal or in a new terminal.
...
{{site.data.alerts.callout_info}}The <code>cockroach quit</code> command for node 3 will take longer to respond (about a minute) and will eventually force kill the node. This is because, with only 1 of 3 nodes left, a majority of replicas are not available, and so the cluster is no longer operational.{{site.data.alerts.end}}
