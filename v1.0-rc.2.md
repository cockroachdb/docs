...
- [Diagnostics reporting](diagnostics-reporting.html) now includes anonymized query statistics. [#15491](https://github.com/cockroachdb/cockroach/pull/15491)

### SQL Language Changes

- Restrictions on schema changes in transactions with other statements have been relaxed. It is now possible to insert into a table in the same transaction that created it, provided there are no [`FOREIGN KEY`](foreign-key.html) or [`INTERLEAVE`](interleave-in-parent.html) clauses. [#15511](https://github.com/cockroachdb/cockroach/pull/15511)
...
- If the new flag `--listening-url-file` is passed to [`cockroach start`](start-a-node.html), the server will write its SQL connection URL to the specified file on successful startup, in addition to printing it to `stdout`. This is particularly helpful in identifying the node's port when an unused port is assigned automatically (`--port=0`).[#15468](https://github.com/cockroachdb/cockroach/pull/15468)
- The [`cockroach sql`](use-the-built-in-sql-client.html) shell no longer buffers all results in memory before printing them. [#15277](https://github.com/cockroachdb/cockroach/pull/15277)
- The log file is now flushed more reliably when a node exits. [#15531](https://github.com/cockroachdb/cockroach/pull/15531)
- The new flag `--max-offset` on the [`cockroach start`](start-a-node.html) command defines the maximum allowed clock offset for the cluster and replaces the environment variable `COCKROACH_MAX_OFFSET`. Note that the `--max-offset` value must be the same on all nodes in the cluster. In order to change it, every node in the cluster must be stopped and restarted with the new value. [#15568](https://github.com/cockroachdb/cockroach/pull/15568)
- The `--raft-tick-interval` flag has been removed from the [`cockroach start`](start-a-node.html) command. [#15547](https://github.com/cockroachdb/cockroach/pull/15547)
...
- Fixed a crash when a transaction that uses [`SAVEPOINT cockroach_restart`](savepoint.html) does something other than `ROLLBACK TO SAVEPOINT` after an error. [#15563](https://github.com/cockroachdb/cockroach/pull/15563)
- Fixed [`RESTORE`](restore.html) for interleaved tables. [#15564](https://github.com/cockroachdb/cockroach/pull/15564)
...
- Previously, [`ALTER TABLE ADD COLUMN`](add-column.html) with a `DEFAULT unique_rowid()` specification could create many duplicate values. This has been corrected. [#15628](https://github.com/cockroachdb/cockroach/pull/15628)
Adding a new non-unique column with a `DEFAULT unique_rowid()` specification no longer generates duplicate values.
- Fixed a consistency error in [`BACKUP`](backup.html). [#15642](https://github.com/cockroachdb/cockroach/pull/15642)
...
- [`RESTORE`](restore.html) performs its pre-splitting less aggressively. [#15548](https://github.com/cockroachdb/cockroach/pull/15548)
...
- Added an FAQ on different [ways to log SQL queries](sql-faqs.html). [#1359](https://github.com/cockroachdb/docs/pull/1359)
- Updated [local deployment tutorials](start-a-local-cluster.html) to have nodes listen only on `localhost`. This ensures these tutorials work even on machines whose hostnames aren’t resolvable. [#1358](https://github.com/cockroachdb/docs/pull/1358)
