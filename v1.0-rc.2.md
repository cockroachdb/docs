...
- Adding a new column via [`ALTER TABLE ADD COLUMN`](add-column.html) with a `DEFAULT unique_rowid()` specification no longer generates duplicate values, except in cases where a node's clock jumps backwards during the back fill. [#15628](https://github.com/cockroachdb/cockroach/pull/15628)
