...
- This release uses more open file descriptors than older releases. It is recommended to set the process's file descriptor limit to at least 10000.
...
- The `ILIKE` operator for case-insensitive matching is now supported. [#7635](https://github.com/cockroachdb/cockroach/pull/7635)
- Four new operators for regular expression matches are now supported: `~` (regex match), `!~` (negated regex match), `~*` (case-insensitive regex match), `!~*` (negated case-insensitive regex matcH). [#7686](https://github.com/cockroachdb/cockroach/pull/7686)
...
- The `--http-addr` flag can be used to control the network interface used for the web UI. [#7475](https://github.com/cockroachdb/cockroach/pull/7475)
- `ALTER TABLE ADD COLUMN` can now be used to create column families. [#7711](https://github.com/cockroachdb/cockroach/pull/7711)
- CockroachDB can now be built on FreeBSD. [#7545](https://github.com/cockroachdb/cockroach/pull/7545)
...
- Column families are now used by default, improving performance of all tables created by this release. [#7623](https://github.com/cockroachdb/cockroach/pull/7623)
...
- The command-line SQL client is now smarter about semicolons and `\` commands inside string literals. [#7510](https://github.com/cockroachdb/cockroach/pull/7510)
...
- The `cockroach quit` command now waits for the server to stop before returning. [#7603](https://github.com/cockroachdb/cockroach/pull/7603)
...
- The `TIMESTAMP WITH TIME ZONE` type is now reported correctly in the network protocol. [#7642](https://github.com/cockroachdb/cockroach/pull/7642)
- Constraint names are now required to be unique. [#7629](https://github.com/cockroachdb/cockroach/pull/7629)
...
This release includes 131 merged PRs by 24 authors. We would like to
