...
In this lab, we'll create a schema that leverages interleaved tables, which can help improve performance of read operations by placing data on the same range (and therefor the same node).
...
Because interleaved tables work on hierarchical data, we're going to create a three tables, which would be common for an ecommerce site:

- `customers`
- `orders`, whose "parent" is `customers`
- `packages`, whose "parent" is `packages`
...
- [Monitoring with the Admin UI](admin-ui.md)
