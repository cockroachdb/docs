...
This lab has you work with SQL directly, so you should open the built-in SQL client:

~~~ shell
$ cockroach sql --certs-dir=certs
~~~

CockroachDB comes with a single default `system` database, which contains CockroachDB metadata and is read-only. To create a new database, use [`CREATE DATABASE`](/stable/create-database.html) followed by a database name:
