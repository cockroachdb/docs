...
1. Download the Docker Compose file that defines a 6-node CockroachDB cluster spread across 3 separate networks:
    ...
    ~~~ shell
    $ curl -O {{site.url}}/docs/training/resources/docker-compose.yaml
    ~~~
    </div>
    ...
    {% include copy-clipboard.html %}
    ~~~ shell
    $ wget {{site.url}}/docs/training/resources/docker-compose.yaml
    ~~~
    </div>
...
3. Check whether the "Suspect" nodes consider themselves live by hitting their
   `/_admin/v1/health` endpoints:
