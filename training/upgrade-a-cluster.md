...
5. If you're running with a process manager, have the node rejoin the cluster by starting it.
    ...
    `[other flags]` includes any flags you [use to a start node](/stable/start-a-node.html), such as it `--host`.

6. Verify the node has rejoined the cluster through its output to `stdout` or through the [admin UI](/stable/admin-ui-access-and-navigate.html).
