...
- [`CHECK`](check.html) constraints can now be added with the [`ALTER TABLE ADD CHECK`](alter-table.html) and `ALTER TABLE VALIDATE` statements. [#9127](https://github.com/cockroachdb/cockroach/pull/9127) [#9152](https://github.com/cockroachdb/cockroach/pull/9152)
- The [`ALTER TABLE DROP CONSTRAINT`](alter-table.html) statement can now drop [`CHECK`](check.html) and [`FOREIGN KEY`](foreign-key.html) constraints. [#8747](https://github.com/cockroachdb/cockroach/pull/8747)
...
- The [`cockroach gen example-data`](generate-cockroachdb-resources.html) command can now be used to generate SQL for example databases. [#9231](https://github.com/cockroachdb/cockroach/pull/9231), [#9475](https://github.com/cockroachdb/cockroach/pull/9475)
...
- Added docs on the [`cockroach gen`](generate-cockroachdb-resources.html) command, which can be used to generate `man` pages, a `bash` autocompletion script, and example SQL data suitable to populate test databases. [#755](https://github.com/cockroachdb/docs/pull/755)
