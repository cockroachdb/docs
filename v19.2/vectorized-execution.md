...
You can turn vectorized execution on or off for all operations with the `vectorize` [session variable](set-vars.html). The following options are supported:
...
For performance tuning, you can change the minimum number of rows required to use the vectorized engine to execute a query plan with the `vectorize_row_count_threshold` [session variable](set-vars.html). This variable is ignored if `vectorize=experimental_on`.
