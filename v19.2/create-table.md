...
+------------+------------+------------+--------------+-------------+-----------+---------+----------+
  users      | primary    |   false    |            1 | rowid       | ASC       |  false  |  false
(1 row)
~~~

### Create a table (primary key defined)

In this example, we create the `users` table, but with some column [constraints](constraints.html). One column is the [`PRIMARY KEY`](primary-key.html), another is given the [`UNIQUE` constraint](unique.html), and another has both a `UNIQUE` constraint and a [`CHECK`](check.html) constraint that limits the length of the string. The `PRIMARY KEY` and column with the `UNIQUE` constraint are automatically indexed.
...
~~~
  column_name | data_type | is_nullable | column_default | generation_expression |                indices                | is_hidden
+-------------+-----------+-------------+----------------+-----------------------+---------------------------------------+-----------+
  id          | UUID      |    false    | NULL           |                       | {primary,users_name_key,users_dl_key} |   false
  city        | STRING    |    true     | NULL           |                       | {}                                    |   false
  name        | STRING    |    true     | NULL           |                       | {users_name_key}                      |   false
  address     | STRING    |    true     | NULL           |                       | {}                                    |   false
  credit_card | STRING    |    true     | NULL           |                       | {}                                    |   false
...
users      | users_name_key |   false    |            1 | name        | ASC       |  false  |  false
users      | users_name_key |   false    |            2 | id          | ASC       |  false  |   true
...
In this example, we create a secondary and inverted indexes during table creation. Secondary indexes allow efficient access to data with keys other than the primary key. [Inverted indexes](inverted-indexes.html) allow efficient access to the schemaless data in a [`JSONB`](jsonb.html) column.
