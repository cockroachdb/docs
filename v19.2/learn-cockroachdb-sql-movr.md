...
This tutorial walks you through some of the most essential CockroachDB SQL statements, using the `movr` dataset. For a complete list of supported SQL statements and related details, see [SQL Statements](sql-statements.html). For more information about the `movr` dataset and the MovR example application, see [MovR: A Global Ride-sharing App](movr.html).
...
+------------+------------+------------+--------------+-------------+-----------+---------+----------+
  ...
  drivers    | name_idx   |    true    |            1 | name        | ASC       |  false  |  false
  drivers    | name_idx   |    true    |            2 | city        | ASC       |  false  |   true
  drivers    | name_idx   |    true    |            3 | id          | ASC       |  false  |   true
(5 rows)
~~~
...
+---------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+----------------------------------------------+
  ...
  accept_gas_hundred              | Listen much get art popular.                                                                                                                                                                                     | {"type": "percent_discount", "value": "10%"}
  according_share_door            | Region difference letter now huge next any. Nothing hotel gas election hospital hope give. Capital can address look. Window off beyond success couple PM as hair. Side who understand indeed future system vote. | {"type": "percent_discount", "value": "10%"}
  administration_challenge_just   | Federal marriage best send. Wide he while another. Mention agency already thank. Officer cup drop when fall wind question. Kitchen two arm serious difference billion party throughout.                          | {"type": "percent_discount", "value": "10%"}
  administration_market_important | Product join number more order. Bar choice floor difference. Forget between agree door painting list direction. Thing analysis expect war son else night crime. They opportunity upon may whatever.              | {"type": "percent_discount", "value": "10%"}
  administration_more_he          | Against either give game same. Paper arm bit yourself such good model weight. Building job worry go.                                                                                                             | {"type": "percent_discount", "value": "10%"}
  admit_certain_against           | On law discuss long garden time. Much actually down might choice cold up audience.                                                                                                                               | {"type": "percent_discount", "value": "10%"}
(10 rows)
~~~
