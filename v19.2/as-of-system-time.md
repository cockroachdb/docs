...
`experimental_follower_read_timestamp()`| An [experimental](experimental-feature-lifecycle.html) [function](functions-and-operators.html) that runs your queries at a time as close as possible to the present time while remaining safe for [follower reads](follower-reads.html#what-are-follower-reads).
