...
The `cockroach start-single-node` command supports the following [general-use](#general), [networking](#networking), [security](#security), and [logging](#logging) flags. All flags must be specified each time the node is started, as they will not be remembered.
...
5. Update the default [replication zone](configure-replication-zones.html) to enable 3-way replication:
