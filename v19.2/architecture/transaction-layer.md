...
1. The transaction record's state is `STAGING`, and its list of pending writes (i.e., `InFlightWrites`) have all succeeded (i.e., achieved consensus across the cluster). Any observer of this transaction can verify that its writes have replicated. Transactions in this state are *implicitly committed*.
