...
`opt_partition_clause`  | An optional `PARTITION BY` clause. See [Define Table Partitions](partitioning.html) for details.
`opt_sort_clause` | An optional `ORDER BY` clause. See [Ordering Query Results](query-order.html) for details.
`opt_frame_clause`  | An optional frame clause, which contains a frame boundary or `EXCLUDE` clause.
...
----------|-------------
`window_name` | The name of the new window frame.
`opt_existing_window_name` | An optional name of an existing window frame, defined in a separate window definition.
`opt_partition_clause`  | An optional `PARTITION BY` clause. See [Define Table Partitions](partitioning.html) for details.
`opt_sort_clause` | An optional `ORDER BY` clause. See [Ordering Query Results](query-order.html) for details.
`frame_bound` | An optional frame boundary.<br>Valid boundaries include `UNBOUNDED PRECEDING`, `<offset> PRECEDING`, `UNBOUNDED FOLLOWING`, `<offset> FOLLOWING`, and `CURRENT ROW`.
