...
summary: Learn how to benchmark CockroachDB against TPC-C 10k.
toc: true
toc_not_nested: true
---
...
1. [Create 16 VM instances](https://cloud.google.com/compute/docs/instances/create-start-instance), 15 for CockroachDB nodes and 1 for the TPC-C workload.
...
3. Run the [`cockroach start`](start-a-node.html) command:
    ...
    Each node will start with a [locality](start-a-node.html#locality) that includes an artificial "rack number" (e.g., `--locality=rack=0`). Use 5 racks for 15 nodes so that 3 nodes will be assigned to each rack.
...
2. Download the `workload` binary and make it executable:
...
3. Import the TPC-C dataset:
    ...
    This will load 800GB of data for 10,000 "warehouses". This can take around **TODO** hours to complete.
...
1. On the VM with the `workload` binary, briefly run TPC-C to set up partitioning:
...
2. Now that the dataset is loaded, re-enable 3-way replication:
...
Once up-replication and partitioning has finished, on the VM with the `workload` binary, run TPC-C for 30 minutes:
