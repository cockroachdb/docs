...
A partitioned backup is specified by a list of URIs, each of which has a `COCKROACH_LOCALITY` URL parameter whose single value is either `default` or a URL-encoded locality query string parameter such as `region=us-east`. At least one `COCKROACH_LOCALITY` must be the `default`.
