...
3. The ordering of the data source for an [`INSERT`](insert.html)
   ...
   `LIMIT` is preserved, to determine [which rows are processed, but not their order](#Ordering-of-rows-in-DML-statements).
4. The ordering indicated for an [`UPDATE`](update.html) or [`DELETE`](delete.html)
   ...
   [which rows are processed, but not their order](#Ordering-of-rows-in-DML-statements).
...
When using `ORDER BY` with a [`INSERT`](insert.html),
[`UPSERT`](upsert.html), [`UPDATE`](update.html) or
[`DELETE`](delete.html) (DML statement), the `ORDER BY` clause is
...
which rows of the input are used to insert/update/delete the table
...
determined by its [primary and secondary indexes](indexes.html). These
are determined outside and prior to the execution of DML statements.

To order the result of the `RETURNING` clause, see the next section.
