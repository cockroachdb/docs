...
1. On one of the VMs with the custom TPC-C `workload` binary, briefly run the workload to set up partitioning:
    ...
    {% include copy-clipboard.html %}
    ~~~ shell
    $ ulimit -n 10000 && ./workload run tpcc \
    --partitions 27 \
    --warehouses 100000 \
    --duration 1m \
    --ramp 1ms \
...
--ramp 1m \
--duration 30m \
--histograms $now.histogram.ndjson \
$(cat addrs)
~~~
...
1. Collect the results from each VM with `workload`:
    ...
    {% include copy-clipboard.html %}
    ~~~ shell
    $ scp <username>@<workload instance 1 address>:$now.histogram.ndjson .
    ~~~

    {% include copy-clipboard.html %}
    ~~~ shell
    $ scp <username>@<workload instance 2 address>:$now.histogram.ndjson .
    ~~~

    {% include copy-clipboard.html %}
    ~~~ shell
    $ scp <username>@<workload instance 3 address>:$now.histogram.ndjson .
    ~~~

    {% include copy-clipboard.html %}
    ~~~ shell
    $ scp <username>@<workload instance 4 address>:$now.histogram.ndjson .
    ~~~

    {% include copy-clipboard.html %}
    ~~~ shell
    $ scp <username>@<workload instance 5 address>:$now.histogram.ndjson .
    ~~~

2. Create a `merge_tpcc_histograms.go` program with the following code:
...
3. Run the program to analyze the results:
    ...
    {% include copy-clipboard.html %}
    ~~~ shell
    $ go run merge_tpcc_histograms.go \
    --warehouses 100000 \
    *.histogram.ndjson
    ~~~
