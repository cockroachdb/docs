---
title: Verify Constraint Conformance
...
- See if any of your cluster's data placement constraints are being violated, and whether the cluster is recovering from those violations.
...
- [`system.replication_constraint_stats`](#system-replication_constraint_stats) shows a list of violations to any data placement requirements you've configured, along with information about whether the cluster is actively recovering from those violations.
...
- [`system.reports_meta`](#system-reports_meta) lists the IDs and times when the reports ran that generated the data in the `system.replication_*` tables.
- [`crdb_internal.zones`](#crdb_internal-zones) can be used with the tables above to figure out the database and table names where the non-conforming or at-risk data is located.

To configure how often the conformance reports run, adjust the XXX cluster setting.
...
{{site.data.alerts.callout_info}}
The `system.replication_*` tables listed above are only populated in clusters where the zone configurations have been changed from the defaults.
{{site.data.alerts.end}}
...
| total_ranges            | [`INT8`](int.html) | Total [ranges](architecture/overview.html#architecture-range) in the cluster.          |
| unavailable_ranges      | [`INT8`](int.html) | Unavailable ranges.                                                                    |
| under_replicated_ranges | [`INT8`](int.html) | [Under-replicated](admin-ui-replication-dashboard.html#under-replicated-ranges) ranges. |
| over_replicated_ranges  | [`INT8`](int.html) | Over-replicated ranges.                                                                |

### system.replication_critical_localities

The `system.replication_critical_localities` table shows which localities in your cluster have data that is at risk in the event of node failures in your cluster.

For an example using this table, see [Find out which databases and tables have at-risk ranges](#find-out-which-databases-and-tables-have-at-risk-ranges).
...
| zone_id        | [`INT8`](int.html)      | The ID of the [replication zone](configure-zone.html).                                                                  |
| subzone_id     | [`INT8`](int.html)      | The ID of the subzone (i.e., [partition](partition-by.html)).                                                           |
| locality       | [`STRING`](string.html) | The human-readable name of the critical [locality](configure-replication-zones.html#zone-config-node-locality).         |
| report_id      | [`INT8`](int.html)      | The ID of the [report](#system-reports_meta) that generated this data.                                                  |
...
The `system.replication_constraint_stats` table lists violations to any data placement requirements you've configured, along with information about whether the cluster is actively recovering from those violations.
...
| report_id        | [`INT8`](int.html)            | The ID of the [report](#system-reports_meta) that generated this data.                                  |
| violation_start  | [`TIMESTAMP`](timestamp.html) | The time when the violation occurred, in UTC.                                                           |
...
| Column name | Data type                     | Description                                                                                      |
|-------------+-------------------------------+--------------------------------------------------------------------------------------------------|
| id          | [`INT8`](int.html)            | The ID of the report run that generated the data available in the `system.replication_*` tables. |
| generated   | [`TIMESTAMP`](timestamp.html) | When the report was generated, in UTC.                                                           |

### crdb_internal.zones

The `crdb_internal.zones` table is useful for:

- Matching up data returned from the various conformance reports with the human-readable names of the databases and tables, indexes, and partitions where that data lives.
...
| raw_config_yaml     | [`STRING`](string.html) | The YAML you used to [configure this replication zone](configure-replication-zones.html).                            |
| raw_config_sql      | [`STRING`](string.html) | The SQL you used to [configure this replication zone](configure-replication-zones.html).                             |
| raw_config_protobuf | [`BYTES`](bytes.html)   | The protobuf representation of the YAML used to [configure this replication zone](configure-replication-zones.html). |
| full_config_yaml    | [`STRING`](string.html) | The same as `raw_config_yaml`, with default values such as `gc.ttlseconds` and `num_replicas` filled in.             |
| full_config_sql     | [`STRING`](string.html) | The same as `raw_config_sql`, with default values such as `gc.ttlseconds` and `num_replicas` filled in.              |
