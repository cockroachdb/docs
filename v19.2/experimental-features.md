...
For more information about how vectorized query execution works, see the blog post [Vectorizing the merge joiner in CockroachDB](https://www.cockroachlabs.com/blog/vectorizing-the-merge-joiner-in-cockroachdb/).

{{site.data.alerts.callout_danger}}
The vectorized execution engine does not yet monitor memory usage or spill to disk for large joins, so there is a risk that queries that perform large joins or other memory-intensive operations could cause the node to run out of memory and crash.
{{site.data.alerts.end}}
