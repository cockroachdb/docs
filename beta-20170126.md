...
- The hex string syntax `X'0d0a'` now produces [byte values](sql-constants.html#hexadecimal-encoded-byte-array-literals) instead of string values, so it can accept non-UTF-8 input. [#13094](https://github.com/cockroachdb/cockroach/pull/13094)
...
- The location of replicas, both when first added and when rebalanced to maintain cluster equilibrium, is now influenced by [`--locality`](start-a-node.html) settings at the node-level and more [expressive zone configuration constraints](configure-replication-zones.html#replication-constraints). [#13119](https://github.com/cockroachdb/cockroach/pull/13119)
- Old log files are now garbage collected automatically. [#13083](https://github.com/cockroachdb/cockroach/pull/13083)

### SQL Language Changes

- The `information_schema.statistics` table and the output of [`SHOW INDEXES`](show-index.html) now include columns that are added to the index implicitly (the primary key is an implicit part of secondary indexes). These columns are marked by a new `Implicit` column. [#12801](https://github.com/cockroachdb/cockroach/pull/12801)
...
- The `sum()` [function](functions-and-operators.html) now works with [`INTEGER`](int.html) values. [#12938](https://github.com/cockroachdb/cockroach/pull/12938)
- The `format_type()` nd `col_description()` [functions](functions-and-operators.html) are now supported. [#12967](https://github.com/cockroachdb/cockroach/pull/12967) [#12978](https://github.com/cockroachdb/cockroach/pull/12978)
...
- [`SHOW COLUMNS`](show-columns.html) now includes a summary of indices that the column is a part of. [#12907](https://github.com/cockroachdb/cockroach/pull/12907)

### Command-Line Interface Changes

- The [`cockroach dump`](back-up-and-restore-data.html) command can now dump multiple tables at the same time. [#12759](https://github.com/cockroachdb/cockroach/pull/12759)
- The [`cockroach dump`](back-up-and-restore-data.html) has a new flag, `--dump-mode`, which can be used to dump the schema or the data for a table separately. [#12759](https://github.com/cockroachdb/cockroach/pull/12759)
...
- [`UPDATE`](update.html) statements using a secondary index no longer cause server crashes during schema changes. [#12794](https://github.com/cockroachdb/cockroach/pull/12794)
...
- Added documentation on [SQL Value Expressions](sql-expressions.html), [Table Expressions](table-expressions.html), and [Constants](sql-constants.html). [#1008](https://github.com/cockroachdb/docs/pull/1008)
- Added documentation on [String Collation](collate.html). [#974](https://github.com/cockroachdb/docs/pull/974)
- Update the [`cockroach dump`](back-up-and-restore-data.html) documentation to cover dumping multiple tables as well as choosing whether to dump a table's schema, data, or both. [#1010](https://github.com/cockroachdb/docs/pull/1010)
- Updated the [`cockroach zone`](configure-replication-zones.html) documentation to explain how node-level locality settings and zone configuration constraints influence the location of replicas, and added [scenario-based examples](configure-replication-zones.html#scenario-based-examples). [#1027](https://github.com/cockroachdb/docs/pull/1027)
- Updated cluster topology guidance in [Recommended Production Settings](recommended-production-settings.html#cluster-topology). [#1027](https://github.com/cockroachdb/docs/pull/1027)
