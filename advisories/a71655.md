...
CockroachDB versions 19.2, 20.1, 20.2, and 21.1 are affected by this bug. Versions 19.2 and 20.1 are no longer eligible for [maintenance support](../releases/release-support-policy.html), so there will be no patches for these versions.
...
The fix will be included in upcoming patches to 20.2 and 21.1. See [Mitigation](#mitigation) for the steps to avoid the issue until patches are released.
