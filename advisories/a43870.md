...
more users to the `admin` role using the GRANT statement. Users are
encouraged to upgrade and use GRANT. For deployments using previous
versions, a workaround is available in the Mitigation section below.

Additionally, CockroachDB 19.2.3 and v20.1 include a way to create an
...
with servers/clusters running versions v2.1, 19.1 and v19.2 without
...
via an INSERT statement, for example:

~~~sql
INSERT INTO system.role_members(role, member) VALUES ('admin', 'someuser');
~~~
