...
In all versions of CockroachDB from v21.2.0 to v21.2.10 as well as v22.1.0, executing a prepared [`SELECT`](../{{site.versions["stable"]}}/select-clause.html) query with a casted placeholder value in a constant filter could produce incorrect results if the casted type did not match the filtered column type.
...
Users of CockroachDB are encouraged to upgrade to a maintenance version with the fix applied: v21.2.11 and v22.1.1.
