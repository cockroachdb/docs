...
In 20.2, the implementation of TRUNCATE changed to keep the table ID stable and instead create new indexes within that table, dropping the existing indexes. This code was buggy when it came to indexes which were not yet public. The upshot is that indexes which were not fully backfilled, but with the old data, would be made public instead of dropped. This results in a public index with data that does not match the primary index. It’s a bad corruption bug. The root cause is #62842.
