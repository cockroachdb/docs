...
Range deletions (as caused by certain [table drops](../drop-table.html), [table truncates](../truncate.html), and certain rebalancing operations during [node decommissioning](../remove-nodes.html) or failover), when occurring concurrently with replica [garbage collection](../architecture/storage-layer.html#garbage-collection), can yield permanent data corruption or loss in rare cases.
...
This bug has been fixed in versions v19.1.9, v19.2.7, and v20.1.0. All users are encouraged to [upgrade](../upgrade-cockroach-version.html).
