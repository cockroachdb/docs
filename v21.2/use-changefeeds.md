...
The resolved timestamp acts as a checkpoint confirming that all change events up to its timestamp have been emitted.

The resolved timestamp can be used to remove [duplicates](#duplicates) from change notifications. In some cases, it may be preferable to silence duplicate messages (e.g. when a client cannot accept duplicate messages). The resolved timestamp provides a way of identifying which messages can be cleaned up.

Furthermore, resolved timestamps can be used to enforce global ordering between messages by buffering records in between the timestamp closures. In this workflow, the resolved timestamp is used to see every row that changed at a certain time. For example, resolved timestamp notifications sent to each Kafka partition can be used to provide strong ordering and global consistency.
