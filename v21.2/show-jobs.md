...
`for_schedules_clause` |  The schedule you want to view jobs for. You can view jobs for a specific schedule (`FOR SCHEDULE id`) or view jobs for multiple schedules by nesting a [`SELECT` clause](select-clause.html) in the statement (`FOR SCHEDULES <select_clause>`). See the [examples](#show-jobs-for-a-schedule).
...
`last_run` | The last execution time of the job.
`next_run` | The next execution time of the job based on exponential-backoff delay.
`execution_errors` | An array of strings that describe errors encountered during execution of the job.
