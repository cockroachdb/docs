...
* <span class="version-tag">New in v21.2:</span> Enable the `kv.rangefeed.catchup_scan_iterator_optimization.enabled` setting to have [rangefeeds](stream-data-out-of-cockroachdb-using-changefeeds.html#enable-rangefeeds) use time-bound iterators for catch-up scans when possible. Catch-up scans are run for each rangefeed request. This setting improves the overall performance of changefeeds during some [range-split operations](architecture/distribution-layer.html#range-splits). Note that the [`WITH diff` option](#diff-opt) cannot be used with this setting. [Tracking GitHub Issue](https://github.com/cockroachdb/cockroach/pull/69191)
