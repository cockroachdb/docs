...
### Pebble
 ...
 CockroachDB uses [Pebble](../cockroach-start.html#storage-engine)––an embedded key-value store with a RocksDB-compatible API developed by Cockroach Labs––to read and write data to disk.
...
SSTs are an on-disk representation of sorted lists of key-value pairs. Conceptually, they look something like this:
...
The levels of the LSM are organized from L0 to L6. L0 is the top-most level. L6 is the bottom-most level. New data is ingested into L0 (e.g. using [`INSERT`](../insert.html) or [`IMPORT`](../import.html)) and then merged down into lower levels over time.
...
If the compaction process falls behind the amount of data being ingested, and there is more data stored at a higher level of the tree than the level below, the LSM shape can become inverted.
...
_Read amplification_ happens when the LSM is inverted, and reads need to start in higher levels and "look down" through a lot of SSTs to read a key's correct (freshest) value. Specifically, when the storage engine needs to read from multiple SST files in order to service a single logical read, this state is known as read amplification. Read amplification can be especially bad if a large [`IMPORT`](../import.html) is overloading the cluster (due to insufficient CPU and/or IOPS) and the storage engine has to consult many small SSTs in L0 to determine the most up-to-date value of the keys being read (e.g., using a [`SELECT`](../select-clause.html)).
