...
- <span class="version-tag">New in v21.2</span>: [Identity columns](#identity-columns) ([sequence](create-sequence.html) columns)
...
<span class="version-tag">New in v21.2</span>: *Identity columns* are columns that are populated with values in a [sequence](create-sequence.html). CockroachDB populates identity columns with the result of the `nextval()` [built-in function](functions-and-operators.html).
...
Note the following limitation of identity columns:

- `GENERATED ALWAYS AS IDENTITY`/`GENERATED BY DEFAULT AS IDENTITY` is only supported in [`ALTER TABLE ... ADD COLUMN`](add-column.html) statements when the table being altered is empty, as [CockroachDB does not support back-filling sequential column data](known-limitations.html#adding-a-column-with-sequence-based-default-values). For more information, see the [Tracking GitHub Issue](https://github.com/cockroachdb/cockroach/issues/42508).

For an example of an identity column, see [Create a table with an identity column](#add-an-identity-column).
...
<span class="version-tag">New in v21.2</span>: Identity columns define a sequence from which to populate a column when a new row is inserted.
...
{% include copy-clipboard.html %}
~~~ sql
> INSERT INTO bank (balance) VALUES ('0'), ('0'), ('0');
~~~

{% include copy-clipboard.html %}
~~~ sql
> SELECT id, balance, numerical FROM bank;
~~~

~~~
                   id                  | balance | numerical
---------------------------------------+---------+------------
  9bd0addd-c128-4c2f-b037-c87c25aedd24 |       0 |         1
  d964b043-72fd-4108-ae6e-1f9db1ea1f31 |       0 |         0
  f3558efe-947d-4c6a-b27b-8e1a4aff5371 |       0 |         2
(3 rows)
~~~

When a new row is added to the table, CockroachDB populates the `numerical` column with the result of the `nextval()` [built-in function](functions-and-operators.html).
...
{% include copy-clipboard.html %}
~~~ sql
> UPDATE bank SET numerical = 500 WHERE id = '9bd0addd-c128-4c2f-b037-c87c25aedd24';
~~~
...
~~~
                   id                  | balance | numerical
---------------------------------------+---------+------------
  9bd0addd-c128-4c2f-b037-c87c25aedd24 |       0 |       500
  d964b043-72fd-4108-ae6e-1f9db1ea1f31 |       0 |         0
  f3558efe-947d-4c6a-b27b-8e1a4aff5371 |       0 |         2
(3 rows)
~~~

If the column were to follow the `ALWAYS` rule instead, then the sequence values in the column could not be overwritten.
