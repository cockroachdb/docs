...
When selecting a deployment environment for a CockroachDB cluster, we recommend that you select a cloud provider such as Google Cloud Platform (GCP) or Amazon Web Services (AWS), which automatically provide industry-standard encryption for cloud storage resources. You can learn more about [GCP persistent disk encryption](https://cloud.google.com/compute/docs/disks#pd_encryption) or [AWS Elastic Block Storage](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/EBSEncryption.html). {{ site.data.products.db }} clusters, which can be deployed in either GCP or AWS, automatically receive this protection.
...
{{ site.data.products.db }} clusters benefit from, we rely on the cloud provider's encryption-at-rest, and do not enable CockroachDB's [Encryption at Rest Enterprise feature](../enterprise-licensing.html) (see below).

In the context of {{ site.data.products.core }}, customers are responsible for managing their own encryption at rest. We recommend that you deploy clusters in environments such as GCP and AWS includes the encryption at rest services they offer by default.
