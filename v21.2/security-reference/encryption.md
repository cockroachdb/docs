...
Encryption is performed in the [storage layer](../storage-layer.html) and configured per store.
...
CockroachDB does not currently force re-encryption of older files but instead relies on normal [storage engine](../storage-layer.html) churn to slowly rewrite all data with the desired encryption.
...
See [making encrypted backups]({{ page.version.version }}/backups/encrypted-backup-description.html).
