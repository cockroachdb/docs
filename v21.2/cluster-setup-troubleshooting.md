...
A _disk stall_ is any disk operation that does not terminate in a reasonable amount of time.  This usually manifests as write-related system calls such as [`fsync(2)`](https://man7.org/linux/man-pages/man2/fdatasync.2.html) (a.k.a. `fdatasync`) appearing to "hang" or take a lot longer than expected. (FIXME: how long is "longer than expected"?)  The mitigation in almost all cases is to [restart the node](cockroach-start.html) with the stalled disk.
...
- Writes on one node come to a halt.  In rare cases, a node may be able to perform liveness checks (which involve writing to disk) even though it cannot write other data to disk due to one or more slow/stalled calls to `fsync`. Because the node is passing its liveness checks, it is able to hang onto its leases even though it can't make progress on the ranges for which it's the leaseholder.  This wedged node has a ripple effect on the rest of the cluster such that all processing of the ranges whose leaseholders are on that node basically grinds to a halt.  The solution when this occurs is to [restart the node](cockroach-start.html).
...
- Every 10 seconds, the CockroachDB storage engine writes to the tail of the [_write-ahead log_](https://en.wikipedia.org/wiki/Write-ahead_logging), or _WAL_.  If that data is not synced (via `fsync`) within 10 seconds, the log message `disk stall detected: unable to write to %s within %s %s warning log entry` is written to the [storage logs](logging.html#storage).  This duration is configurable with the `COCKROACH_ENGINE_MAX_SYNC_DURATION` environment variable.
