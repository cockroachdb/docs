...
- **Existing tables** can have the following constraints added:
  ...
  - `DEFAULT` values and `NOT NULL` constraints can be added through [`ALTER TABLE...ALTER COLUMN`](alter-column.html#set-or-change-a-default-value). For example, this statement adds the [`DEFAULT` value contraint](default-value.html) to `baz.bool`:
