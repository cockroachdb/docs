...
Since each node is responsible for backing up the ranges for which it is the [leaseholder](architecture/replication-layer.html#leases), the locality of the leaseholder node determines where the backup files will be placed in a locality-aware backup. The leaseholder node writes files to the backup storage location with a locality that matches its own localities (with an overall preference for more specific values in the locality hierarchy). If there is no match, the `default` locality is used.
...
This backup can be restored by running:
...
See [Show a node's locality](#show-a-nodes-locality) for guidance on identifying the locality of a node to pass in a backup query.
