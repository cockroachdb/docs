...
1. Select whether the network can connect to the cluster's **DB Console to monitor the cluster**, **CockroachDB Client to access databases**, or both.
    ...
    The DB Console is where you can observe your cluster's health and performance. For more information, see [DB Console Overview](../{{site.versions["cloud"]}}/ui-overview.html).
...
1. From the **Database** dropdown, select the database you want to connect to.
    ...
    The default database is `defaultdb`. For more information, see [Default databases](../{{site.versions["cloud"]}}/show-databases.html#preloaded-databases).
...
To connect to your cluster with the [built-in SQL client](../{{site.versions["cloud"]}}/cockroach-sql.html):
...
1. Copy the [`cockroach sql`](../{{site.versions["cloud"]}}/cockroach-sql.html) command and connection string provided in the Console, which will be used in the next step (and to connect to your cluster in the future):
...
To connect to your cluster with a [CockroachDB-compatible tool](../{{site.versions["cloud"]}}/third-party-database-tools.html), use the connection parameters provided in the Console.
  </section>
