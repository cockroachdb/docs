...
You can set the cluster name, cloud infrastructure provider, region, and [resource limits](../{{site.versions["stable"]}}/glossary.html#resource-limits) as command options. The following command is equivalent to the previous command that uses the default values.
...
To set [resource limits](../{{site.versions["stable"]}}/glossary.html#resource-limits) for a {{ site.data.products.serverless }} cluster, or create a {{ site.data.products.dedicated }} cluster, you must [add a credit card](billing-management.html) to your organization.
