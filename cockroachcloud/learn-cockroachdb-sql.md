...
This tutorial walks you through some of the most essential CockroachDB SQL statements. For a complete list of supported SQL statements and related details, see [SQL Statements](../stable/sql-statements.html).
...
To create a new database, connect with your initial "admin" user and use [`CREATE DATABASE`](../stable/create-database.html) followed by a database name:
...
To see all databases, use the [`SHOW DATABASES`](../stable/show-databases.html) statement or the `\l` [shell command](../stable/cockroach-sql.html#commands):
...
Table and column names must follow [these rules](../stable/keywords-and-identifiers.html#identifiers). Also, when you do not explicitly define a [primary key](../stable/primary-key.html), CockroachDB will automatically add a hidden `rowid` column as the primary key.
...
To show all of the columns from a table, use the [`SHOW COLUMNS FROM <table>`](../stable/show-columns.html) statement or the `\d <table>` [shell command](../stable/cockroach-sql.html#commands):
...
To see all tables in the active database, use the [`SHOW TABLES`](../stable/show-tables.html) statement or the `\dt` [shell command](../stable/cockroach-sql.html#commands):
...
To create an index for non-unique columns, use [`CREATE INDEX`](../stable/create-index.html) followed by an optional index name and an `ON` clause identifying the table and column(s) to index.  For each column, you can choose whether to sort ascending (`ASC`) or descending (`DESC`).
...
To query a table, use [`SELECT`](../stable/select-clause.html) followed by a comma-separated list of the columns to be returned and the table from which to retrieve the data:
