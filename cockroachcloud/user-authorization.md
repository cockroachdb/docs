...
To create a new user, use the [`CREATE USER ... WITH PASSWORD`](../{{site.versions["cloud"]}}/create-user.html) statement:
...
Be sure to create a password for each new user. Without a password, a user cannot connect to the cluster or access the DB Console. To add or change a password for a user, use the [`ALTER USER`](../{{site.versions["cloud"]}}/alter-user.html) statement.
...
<section class="filter-content" markdown="1" data-scope="client">
To list all the users in your cluster, use the [`SHOW USERS`](../{{site.versions["cloud"]}}/show-users.html) statement:
...
<section class="filter-content" markdown="1" data-scope="client">
To change a user's password, use the [`ALTER USER`](../{{site.versions["cloud"]}}/alter-user.html) statement:
...
<section class="filter-content" markdown="1" data-scope="client">
To remove a user, use the [`DROP USER`](../{{site.versions["cloud"]}}/drop-user.html) statement:
...
Access to the data in your cluster is controlled by [privileges](../{{site.versions["cloud"]}}/security-reference/authorization.html#managing-privileges). When a user connects to a database, either via the CockroachDB SQL client or a PostgreSQL driver or ORM, CockroachDB checks the user's privileges for each statement executed. If the user does not have sufficient privileges for a statement, CockroachDB returns an error.

To grant a user privileges for specific databases and tables in your cluster, use the [`GRANT`](../{{site.versions["cloud"]}}/grant.html) statement. For example, to assign a user all privileges for all tables in a database:
...
For more details, see [Privileges](../{{site.versions["cloud"]}}/security-reference/authorization.html#managing-privileges) and [`GRANT`](../{{site.versions["cloud"]}}/grant.html).
...
To show privileges granted to a user, use the [`SHOW GRANTS`](../{{site.versions["cloud"]}}/show-grants.html) statement:
...
To revoke privileges from a user, use the [`REVOKE`](../{{site.versions["cloud"]}}/revoke.html) statement:
...
- To create a role, use the [`CREATE ROLE`](../{{site.versions["cloud"]}}/create-role.html) statement:
...
- To grant privileges to a role, use the [`GRANT <privilege>`](../{{site.versions["cloud"]}}/grant.html) statement:
...
- To add a user (or another role) to a role, use the [`GRANT <role>`](../{{site.versions["cloud"]}}/grant.html) statement:
...
- To revoke privileges from a role, use the [`REVOKE <privilege>`](../{{site.versions["cloud"]}}/revoke.html) statement:
...
- To remove a user (or another role) from a role, use the [`REVOKE <role>`](../{{site.versions["cloud"]}}/revoke.html) statement:
...
- To list all roles in your cluster, use the [`SHOW ROLES`](../{{site.versions["cloud"]}}/show-roles.html) statement:
...
- To remove a role, use the [`DROP ROLE`](../{{site.versions["cloud"]}}/drop-role.html) statement:
