...
The [{{ site.data.products.dedicated }} integration for Datadog](https://docs.datadoghq.com/integrations/cockroachdb_dedicated/) enables data collection and alerting on a [subset of CockroachDB metrics](#available-metrics) available at the [Prometheus endpoint](../{{site.versions["cloud"]}}/monitoring-and-alerting.html#prometheus-endpoint), using the Datadog platform.
...
The [available metrics](#available-metrics) are drawn directly from the CockroachDB [Prometheus endpoint](../{{site.versions["cloud"]}}/monitoring-and-alerting.html#prometheus-endpoint) and are intended for use as building blocks for your own charts.
...
Select **Threshold Alert** as the detection method, which configures an alert that is sent when a supported metric reaches a given threshold. For descriptions of some useful CockroachDB alerts, see [Monitoring and Alerting](../{{site.versions["cloud"]}}/monitoring-and-alerting.html#events-to-alert-on).
