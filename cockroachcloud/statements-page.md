...
Use this page to identify SQL statements that you may want to [troubleshoot](../{{site.versions["stable"]}}/query-behavior-troubleshooting.html). This might include statements that are experiencing high latencies, multiple [retries](../{{site.versions["stable"]}}/transactions.html#transaction-retries), or execution failures. You can optionally create and retrieve [diagnostics](#diagnostics) for these statements.
...
By default, the Statements page displays all SQL statements executed within a one-hour time interval. The display is cleared at the end of each interval. You can change the interval with the [`diagnostics.reporting.interval`](../{{site.versions["stable"]}}/cluster-settings.html#settings) cluster setting.
...
- Statements with this fingerprint were executed by more than one [`application_name`](../{{site.versions["stable"]}}/show-vars.html#supported-variables).
...
- **App** displays the name specified by the [`application_name`](../{{site.versions["stable"]}}/show-vars.html#supported-variables) session setting.
...
- **Vectorized execution?** indicates whether the execution used the [vectorized execution engine](../{{site.versions["stable"]}}/vectorized-execution.html).
- **Used cost-based optimizer?** indicates whether the execution used the [cost-based optimizer](../{{site.versions["stable"]}}/cost-based-optimizer.html).
...
- **Retries** is the cumulative number of [retries](../{{site.versions["stable"]}}/transactions.html#transaction-retries) of statements with this fingerprint within the [specified time interval](#time-interval).
...
The information collected in the bundle can be used to diagnose problematic SQL statements, such as [slow queries](../{{site.versions["stable"]}}/query-behavior-troubleshooting.html#query-is-always-slow). We recommend that you share the diagnostics bundle with our [support team](../{{site.versions["stable"]}}/support-resources.html), which can help you interpret the results. You can also import `trace-jaeger.json` into Jaeger to [visualize the statement traces](../{{site.versions["stable"]}}/query-behavior-troubleshooting.html#visualize-statement-traces-in-jaeger).
