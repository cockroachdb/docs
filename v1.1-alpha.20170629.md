...
- If the server's own hostname cannot be resolved, report an error at startup instead of starting a node that will be unable to participate in a cluster. Local test clusters may need to pass `--host=localhost` to avoid this error. [#16177](https://github.com/cockroachdb/cockroach/pull/16177)
- The statement `EXPLAIN (TRACE)` has been removed, because it was surprising that the query would actually be executed. It has been replaced by `SHOW TRACE FOR ...`, so that the `EXPLAIN` commands can remain read-only. [#16524](https://github.com/cockroachdb/cockroach/pull/16524)
...
- New statement `SHOW SESSION TRACE` can be used to view trace data collected with the `SET TRACE=on|off` statement introduced in a previous alpha. [#16542](https://github.com/cockroachdb/cockroach/pull/16542)
- KV operations can be traced by using `SHOW KV TRACE FOR ...` or `SET trace=on,kv; ...; SHOW SESSION KV TRACE` [#16622](https://github.com/cockroachdb/cockroach/pull/16622)
...
- The `cockroach sql` command now tracks the current database and will restore it when a connection must be recreated. [#16589](https://github.com/cockroachdb/cockroach/pull/16589)
...
- `SHOW {QUERIES,SESSIONS}` no longer produces invalid UTF-8 when truncating strings. [#16602](https://github.com/cockroachdb/cockroach/pull/16602)
