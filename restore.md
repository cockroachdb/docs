...
You can restore entire tables (which automatically includes their indexes) or [views](views.html) from a backup.
...
{{site.data.alerts.callout_info}}<code>RESTORE</code> <em>does not</em> support row-level restores.{{site.data.alerts.end}}

### Object Relationships

All objects with relationships must be restored at the same time.

Relationships include:

- Tables with foreign key constraints and the tables they reference (although this relationship can instead be [removed during the restore](#skipmissingforeignkeys), letting you restore only one of the tables)
...
However, the target database must have not have tables or views with the same name as the restored table or view. If its name is being used, you can:

- [`DROP TABLE`](drop-table.html) or [`DROP VIEW`](drop-view.html) and then restore it.
...
However, if you backed up `system.users`, you can [restore users and their passwords](#restoring-users-from-systemusers-backup).
...
The process initiated by `RESTORE` has been designed to minimally impact performance by distributing work to all nodes in your cluster.
...
By default, all tables with Foreign Key relationships must be restored together. However, using the [`skip_missing_foreign_keys`](#skipmissingforeignkeys) option you can remove the Foreign Key constraint from the table and then restore it, which lets you restore a single table from the Foreign Key relationship.
...
If you backed up `system.users` (which stores your cluster's usernames and their hashed passwords), you must restore it into a new database because you cannot drop the `system.users` table.

After it's restored into a new database, you can write its data to the cluster's existing `system.users` table.
