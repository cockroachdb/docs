...
To handle errors in transactions, you should listen for the following types of errors:
...
- **Ambiguous Errors**: Errors with the code `XX000`, which indicate that the state of the transaction is ambiguous, i.e., you cannot assume it either committed or failed. How you handle these errors depends on how you want to resolve the ambiguity.
...
To improve the performance of transactions, CockroachDB includes a set of statements that let you retry transactions that fail due to contention errors. Retrying transactions has the benefit of increasing their priority each time they're retried, increasing their likelihood to succeed.
