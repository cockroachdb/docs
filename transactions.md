...
| [`ROLLBACK cockroach_restart`](rollback-transaction.html) | Handle [retryable errors](l#error-handling) by rolling back a transaction's changes and increasing its priority. |
...
- **Retryable Errors**: Errors with the code `40001` or string `retry transaction`, which indicates the transaction failed because another concurrent or recent transaction accessed the same values. To handle these errors, you should [retry the transaction](#client-side-transaction-retries).
- **Non-retryable Errors**: All other errors, which indicate that there was a problem with a statement in the transaction. To handle these errors, you can either issue a `COMMIT` or `ROLLBACK` statement to return the database to its previous state. If you want to attempt the same set of statements again, you must begin a completely new transaction.
...
In these cases, CockroachDB assumes there is nothing conditional about these values, so it can continue to retry the transaction with the values it originally received.

If any of the statement's values are conditional (i.e., they change depending on how long the transaction takes to succeed), we recommend instead writing your transaction to use [client-side intervention](#client-side-intervention).
...
- *Recommended*: Use the `SAVEPOINT cockroach_restart` functions to create retryable transactions. Retryable transaction can improve performance because their priority's increased each time they are retried, making them more likely to succeed the longer they're in your system. 
...
- [`ROLLBACK TO cockroach_restart`](rollback-transaction.html#retry-a-transaction) is used when your application detects `40001` / `retry transaction` errors. It provides you a chance to "retry" the transaction by rolling the database's state back to the beginning of the transaction and increasing the transaction's priority. 
  ...
  After issuing `ROLLBACK TO cockroach_restart`, you must issue any statements you want the transaction to contain. Typically, this means recalculating and reissuing a similar set of statements to the previous attempt.
