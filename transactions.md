...
{{site.data.alerts.callout_success}}For Golang users, client-side retries are built into the <code>crdb</code> package of the CockroachDB Go client and happen automatically. For more information, see the <strong>Go</strong> tab of <a href="build-a-test-app.html#step-4-execute-transactions-from-a-client">Build a Test App: Execute transaction from a client</a>.{{site.data.alerts.end}}
