...
`opt_index_visible` | An optional annotation that indicates whether the index is visible to the [cost-based optimizer](cost-based-optimizer.html#control-whether-the-optimzer-uses-indexes-marked-as-not-visible). For an example, see [Set an index to be not visible](alter-index.html#set-an-index-to-be-not-visible).
