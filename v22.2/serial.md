...
----------------------|------------------
 ...
 `unordered_rowid`    | `SERIAL` implies `DEFAULT unordered_unique_rowid()`. The real data type is always `INT8`, regardless of the `default_int_size` [session variable](set-vars.html) or `sql.defaults.default_int_size` [cluster setting](cluster-settings.html).
