...
`opt_index_visible` | An optional annotation that indicates whether the index should be visible to the [cost-based optimizer](cost-based-optimizer.html#control-whether-the-optimzer-uses-indexes-marked-as-not-visible) (i.e. it won't be used in queries unless specifically selected with a hint). For an example, see [Set an index to be not visible](alter-index.html#set-an-index-to-be-not-visible).
