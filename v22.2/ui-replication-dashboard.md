...
- In the node view, the graph shows the rate of `ReplicaUnavailableError` events logged per second since the `cockroach` process started, for the selected node.

- In the cluster view, the graph shows the rate of `ReplicaUnavailableError` events logged per second since the `cockroach` process started, for each node in the cluster.

Metric | Description
-------|------------
`{node}` | The rate of `ReplicaUnavailableError` events that have occurred per second on that node since the `cockroach` process started.
