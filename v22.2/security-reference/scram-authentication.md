...
SCRAM authentication imposes additional computational load on your application servers, which need to compute the client proof for each authentication. Cockroach Labs recommends that you:

- Allocate additional CPU resources to systems running client applications where SCRAM authentication is enabled. 
- Migrate your user population and workloads in batches when migrating to SCRAM authentication.
- Test configuration changes against workloads that reflect the usage patterns you expect the application to handle.

Additional changes may be required to your application, such as limiting the number of connections in your application's connection pool or limiting the number of concurrent transactions that your client allows. Refer to [Troubleshoot SQL client application problems](../error-handling-and-troubleshooting.html#troubleshoot-sql-client-application-problems).
