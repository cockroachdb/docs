...
<a name="store-partition-rate"></a> `partition-rate` | A mapping of a store name to a bandwidth limit, expressed in bytes per second. A value of `0` (the default) represents unlimited bandwidth. For example: <br /><br />`--store=partition-rate=disk-name=/mnt/ssd01=200`<br /><br />This value overrides the [`kv.store.admission.provisioned_bandwidth` cluster setting](cluster-settings.html#settings).
