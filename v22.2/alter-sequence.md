...
`CYCLE`/`NO CYCLE` | The sequence will wrap around when the sequence value hits the maximum or minimum value. `CYCLE` is not implemented. If `NO CYCLE` is set, the sequence will not wrap.
