...
- `ZONE` (default). Databases configured to survive zone failures have 3 [voting replicas](overview.html#architecture-replica) for every range, all in the [home region](multiregion-overview.html#table-localities). To support low-latency reads from other regions, one [non-voting replica](replication-layer.html#non-voting-replicas) is placed in each non-home region.
