...
For CockroachDB v22.2.0 on ARM Macs, [spatial features](spatial-features.html) are disabled due to an issue with macOS code signing for the [GEOS](https://libgeos.org/) libraries. Users needing spatial features on an ARM Mac may instead [use Rosetta](https://developer.apple.com/documentation/virtualization/running_intel_binaries_in_linux_vms_with_rosetta) to [run the Intel binary](#install-binary) or use the [Docker image](#use-docker) distribution. This issue is expected to be resolved in an upcoming 22.2 patch release. See the [GitHub tracking issue](https://github.com/cockroachdb/cockroach/issues/93161) for more information.
