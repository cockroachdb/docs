...
| `ORDER BY sortby_list` | Sort retrieved rows in the order specified by `sortby_list`. You can optionally specify `ASC` or `DESC` order for each column. See the page on [controlling row order](query-order.html) |
