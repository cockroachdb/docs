...
[`TABLE`](selection-clauses.html#table-clause).
...
`select_stmt` | A [statement-like query](relational-expressions.html#statement-like-queries) to use as [subquery](#subqueries-as-table-expressions).
...
complex table expressions from more simple parts.
...
`(... subquery ...)` | A [statement-like query](relational-expressions.html#statement-like-queries) used as [subquery](#subqueries-as-table-expressions). | `(SELECT * FROM customers c)`
...
{{site.data.alerts.callout_info}}<code>WITH ORDINALITY</code> necessarily prevents some optimizations of the surrounding query. Use it sparingly if performance is a concern, and always check the output of <a href="explain.html">EXPLAIN</a> in case of doubt. {{site.data.alerts.end}}
...
Any [statement-like
query](relational-expressions.html#statement-like-queries) enclosed
...
{{site.data.alerts.callout_info}}To use other statements that produce data in a table expression, for example `SHOW`, use the <a href="#using-the-output-of-other-statements">square bracket notation</a>.{{site.data.alerts.end}}
...
parentheses</a>, which is restricted to <a href="relational-expressions.html#statement-like-queries">Statement-like Queries</a>. It was introduced to enable use of any <a href="explain.html#explainable-statements">explainable statement</a> as subquery, including <code>SHOW</code> and other non-query statements.{{site.data.alerts.end}}
...
[`TABLE`](selection-clauses.html#table-clause) variants of [selection
clauses](selection-clauses.html), and thus can appear everywhere where
...
For more options to compose query results, see [Relational Expressions](relational-expressions.html).
