...
- [Statement-like Queries](#statement-like-queries), which can occur:
  - At the top-level of a query, where they appear like standalone [SQL statements](sql-statements.html). 
...
----------|------------
`common_table_expr` | Docs coming soon.
...
For example, the construct [`SELECT * FROM accounts`](select-clause.html) is a selection clause. It is also a valid statement-like query, and thus can be used as a standalone statement by appending a semicolon:
