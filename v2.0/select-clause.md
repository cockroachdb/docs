...
When used as a standalone statement, the simple `SELECT` clause is
also called "the `SELECT` statement". However, it is also a
[selection clause](selection-clauses.html) that can be combined
with other constructs to form more complex [relational
expressions](relational-expressions.html).
...
| `DISTINCT ON ( a_expr [, ...] )` | `DISTINCT ON` followed by a list of [scalar expressions](scalar-expressions.html) within parentheses. See [Eliminate Duplicate Rows].
| `target_elem` | A [scalar expression](scalar-expressions.html) to use to compute a column in each result row, or `*` to automatically retrieve all columns from the `FROM` clause.<br><br>If `target_elem` contains an [aggregate function](functions-and-operators.html#aggregate-functions), a `GROUP BY` clause can be used to further control the aggregation. |
...
{{site.data.alerts.callout_info}}Using two or more table expressions in the <code>FROM</code> sub-clause,
separated with a comma, is equivalent to a <a href="joins.html"><code>CROSS JOIN</code> expression</a>.{{site.data.alerts.end}}
...
See [Relational Expressions](relational-expressions.html) for
...
`OFFSET` to form a [statement-like query](relational-expressions.html#statement-like-queries) or
...
- Using [set operations](set-operations.html) to combine rows
