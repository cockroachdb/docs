...
-----------------|------------------
Yes | Start each node with `--host` set to its private address and do not specify `--advertise-host`. In this case, other nodes will use the private address advertised, and load balancers/clients in the private network must use it as well.
...
### Multiple networks interconnected through NAT
