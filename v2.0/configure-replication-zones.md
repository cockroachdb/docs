...
`--url`, `--host`, `--port`, `--user`, `--database`, `--certs-dir`, `--insecure`,  | See [Client Connection Parameters](connection-parameters.html) for details. Currently, only the `root` user can configure replication zones and the `--database` flag is not effective.
