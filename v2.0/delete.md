...
{{site.data.alerts.callout_danger}}<code>CASCADE</code> does not list objects it drops or updates, so it should be used cautiously. If you delete a row that is referenced by a <a href="foreign-key.html">foreign key</a> and <code>ON DELETE CASCADE</code> is set, all of the dependent rows will also be deleted.{{site.data.alerts.end}}
