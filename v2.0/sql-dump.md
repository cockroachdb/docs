...
`--url`, `--host`, `--port`, `--user`, `--password`, `--database`, `--certs-dir`, `--insecure`,  | See [Client Connection Parameters](connection-parameters.html) for details.

{{site.data.alerts.callout_info}}The user specified with `--user` must
have the `SELECT` privilege on the target
