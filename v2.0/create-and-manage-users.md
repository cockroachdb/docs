...
`--url`, `--host`, `--port`, `--user`, `--certs-dir`, `--insecure`,  | See [Client Connection Parameters](connection-parameters.html) for details. Currently, only the `root` user can create users.
