...
Aliases are primarily used with `JOIN` ([which is not supported yet](https://github.com/cockroachdb/cockroach/issues/2970)), but this example demonstrates that the `AS` clause lets you alias a table and then refer to its columns using `<table>.<column>` notation.

~~~ sql
DELETE FROM account_details AS accounts WHERE accounts.account_id=7 RETURNING *;
~~~ 
~~~ 
+------------+---------+--------------+
| account_id | balance | account_type |
