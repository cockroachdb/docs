...
|`AS col_label`<br><em>use with RETURNING a_expr</em>| Change the label of the column named in `a_expr` to `col_label`<br><br>You can also change column labels with an `identifer`, but must observe [these rules](keywords-and-identifiers.html#rules-for-all-identifiers).|
