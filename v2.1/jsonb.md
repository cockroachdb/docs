...
To organize your `JSONB` field values, use the GROUP BY and ORDER BY clauses with the `->>` operator. For example, organise the `first_name` values from the table you created in the [first example](#create-a-table-with-a-jsonb-column):
...
The `->>` operator returns a `STRING` value and uses string comparison rules to order the data. If you want numeric ordering, cast the resulting data to a float.
