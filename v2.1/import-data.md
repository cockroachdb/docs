...
The following options are required:
 ...
 Option            | Description
-------------------+--------------------------------------------------------------------------------------------------------------------------------------
 `--inserts`       | Required until we develop support for the Postgres [`COPY`](https://www.postgresql.org/docs/current/static/sql-copy.html) statement.
 `--no-owner`      | Required until we develop support for Postgres' object ownership statements.
 `--no-privileges` | Required until we develop support for Postgres' access privilege statements.
...
Finally, we can confirm that despite needing to edit the `gender` column's data type in the Postgres dump file, the column was populated correctly during the import:
