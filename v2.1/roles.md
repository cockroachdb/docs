...
For the purpose of this example, we need an [enterprise license](enterprise-licensing.html) and one CockroachDB node running in insecure mode:
...
4. [Create a role](create-role.html) and then [list all roles](show-roles.html) in our database:
...
5. Grant privileges to the `system_ops` role we created:
...
7. To test the privileges we just added to the `system_ops` role, use `\q` or `ctrl-d` to exit the interactive shell, and then open the shell again as the `maxroach` user (who is a member of the `system_ops` role):
...
9. As the `maxroach` user, try to drop the table:
    ...
    We cannot drop the table because our current user (`maxroach`) is a member of the `system_ops` role, which doesn't have `DROP` privileges.
...
11. Let's switch back to the `root` user to test more of the SQL statements related to roles. Use `\q` or `ctrl-d` to exit the interactive shell, and then open the shell again as the `root` user:
