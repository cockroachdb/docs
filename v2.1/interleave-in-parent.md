...
> CREATE TABLE IF NOT EXISTS grandchild (
    ...
    parent_id INT,
    grandparent_id INT,
    FOREIGN KEY (grandparent_id, parent_id) REFERENCES child (parent_id, id) ON DELETE CASCADE ON UPDATE CASCADE,
    PRIMARY KEY (grandparent_id, parent_id, id)
  )
  INTERLEAVE IN PARENT child (grandparent_id, parent_id);
~~~
