...
<span class="version-tag">New in v2.1:</span> Fast deletes are available for interleaved tables that use [`ON DELETE CASCADE`](add-constraint.html#add-the-foreign-key-constraint-with-cascade).  Deleting from such tables will use an optimized code path and run much faster, as long as the following conditions are met:
...
For an example showing how to create a table that meets these criteria, see [Interleaved fast path deletes](#interleaved-fast-path-deletes) below.
...
This example shows how to create interleaved tables that enable our SQL engine to use a code path optimized to run much faster when deleting from these tables.  For more information about the criteria to enable fast deletes on these tables, see [fast path deletes](#fast-path-deletes) above.
...
The following statements will delete both tables very quickly, using the optimization described above:
...
The following statement will delete some rows from the `parent` table, also very quickly:
