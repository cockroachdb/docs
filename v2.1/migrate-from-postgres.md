...
The import will fail if the dump file contains functions, triggers, or type definitions.  In addition to calling [`pg_dump`][pgdump] with the flags specified below, you may need to edit the dump file to remove functions and data types.
...
To dump the entire database, run the [`pg_dump`][pgdump] command shown below.  Note that no additional arguments to [`pg_dump`][pgdump] are supported (e.g., `--data-only`, `--create`, etc.), only those shown here.

{% include copy-clipboard.html %}
~~~ shell
$ pg_dump --no-privileges --disable-triggers employees > /tmp/employees-full.sql
~~~

The following options to [`pg_dump`][pgdump] are required in this case:

| Option                 | Description                                                                                                                                                                 |
|------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `--disable-triggers`   | Required since CockroachDB does not support [Postgres triggers](https://www.postgresql.org/docs/current/static/plpgsql-trigger.html).                                       |
| `--no-privileges` 	 | Required if you want to import a full database dump.  CockroachDB does not fully support PostgreSQL's access privilege statements such as `GRANT` or `ACCESS DEFAULT PRIVILEGES`. |
...
To dump the `employees` table from a Postgres database also named `employees`, run the [`pg_dump`][pgdump] command shown below.  You can import this table using the instructions in [Import a table from a table dump](#import-a-table-from-a-table-dump) below.  Note that no additional arguments to [`pg_dump`][pgdump] are supported (e.g., `--data-only`, `--create`, etc.), only those shown here.

{% include copy-clipboard.html %}
~~~ shell
$ pg_dump --disable-triggers -t employees  employees > /tmp/employees.sql
~~~
...
{% include copy-clipboard.html %}
~~~ sql
> IMPORT PGDUMP 'https://s3-us-west-1.amazonaws.com/cockroachdb-movr/datasets/employees-db/pg_dump/employees-full.sql';
~~~
...
> USE employees;
> IMPORT TABLE employees FROM PGDUMP 'https://s3-us-west-1.amazonaws.com/cockroachdb-movr/datasets/employees-db/pg_dump/employees-full.sql';
~~~
...
> CREATE DATABASE IF NOT EXISTS employees;
> USE employees;
> IMPORT PGDUMP 'https://s3-us-west-1.amazonaws.com/cockroachdb-movr/datasets/employees-db/pg_dump/employees.sql';
~~~
...
> IMPORT TABLE employees (
  ...
  PGDUMP DATA ('https://s3-us-west-1.amazonaws.com/cockroachdb-movr/datasets/employees-db/pg_dump/employees.sql');
~~~
