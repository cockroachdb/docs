...
In addition to the case described above, where the leaseholder for a range is responsible for serving reads, there are cases in which non-leaseholder replicas can serve reads for data stored on a range, provided certain criteria are met. Such cases are called _follower reads_.

Follower reads are [consistent](../strong-consistency.html) reads at historical timestamps from follower replicas. They make the non-leaseholder replicas in a range suitable sources for historical reads for [`AS OF SYSTEM TIME`](../as-of-system-time.html) queries with timestamps suitably far in the past (10-20 seconds).  This can increase read throughput by allowing every replica to serve read requests.  For geo-distributed clusters, this can translate to significant latency improvements.

For more information about follower reads and how they work, see [Follower Reads](../follower-reads.html).
