...
---------------|-------------|---------------|-----------------------------------------------
 ...
 `distsql` | <span class="version-tag">New in v2.1:</span> The query distribution mode for the session. By default, compatible queries are distributed and all other queries are run through [DistSQL](../architecture/sql-layer.html#distsql).  | `auto` | Yes
