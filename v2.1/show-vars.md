...
---------------|-------------|---------------|-----------------------------------------------
 ...
 `distsql` | <span class="version-tag">New in v2.1:</span> The query distribution mode for the session. By default, all queries that should be distributed (determined by a heuristic) are distributed, and all other queries are run through the gateway node.  | `auto` | Yes
