...
{{site.data.alerts.callout_info}}Historical data is available only
within the garbage collection window, which is determined by the
<code>ttlseconds</code> field in the <a
href="configure-replication-zones.html">replication zone
configuration</a>.{{site.data.alerts.end}}
...
- In [`BACKUP`](backup.html), after the parameters to `TO` sub-clause.
...
timestamps. It is the entire query that runs at the specified time in
the past.
...
CockroachDB will allow `AS OF SYSTEM TIME` in sub-queries under the
