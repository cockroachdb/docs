...
A common/traditional approach for generating unique IDs is one of the following:

- Monotonically increase `INT` IDs by using transactions with roundtrip `SELECT`s.
- Use [`SERIAL`](serial.html) as pseudo-type for a column to generate random unique IDs.
