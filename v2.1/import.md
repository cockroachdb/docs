...
The tabular data to import must be valid [CSV files](https://tools.ietf.org/html/rfc4180), with the caveat that the comma [delimiter](#delimiter) can be set to another single character. Files must be UTF-8 encoded. If the delimiter, a double quote (`"`), newline (`\n`), or carriage return (`\r`) appears in a field, the field must be surrounded by double quotes. Double quotes appearing in a field must be escaped with a backslash (`\"`).
