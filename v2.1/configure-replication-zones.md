...
Use the [`SHOW ZONE CONFIGURATIONS`](show-zone-configurations.html) statement to view details about existing replication zones.

## Manage replication zones

Use the [`CONFIGURE ZONE`](configure-zone.html) statement to add, modify, reset, and remove replication zones.
...
More examples are also on the [`CONFIGURE ZONE`](configure-zone.html) and [`SHOW ZONE CONFIGURATIONS`](show-zone-configurations.html) pages.
...
4. Configure a replication zone for the table that must be replicated more strictly:
    ...
    {% include copy-clipboard.html %}
    ~~~ sql
    > ALTER TABLE CONFIGURE ZONE USING num_replicas = 5, constraints = '[+ssd]'
    ~~~
