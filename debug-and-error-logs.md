...
When nodes process [`cockroach` commands](cockroach-commands.html), they produce a stream of messages about the command's activities. Each message's body describes the activity, and its envelope contains metadata such as the message's severity level.
