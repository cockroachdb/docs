// adapted from https://github.com/ascorbic/slash-edge/blob/main/lib/add-slash.ts
...
export default async function handler(request: Request, context: Context) {
  ...
  for (const suffix of ['/index.html', '.html']) {
    if (pathname.includes(suffix)) {
      // Replace the suffix with a slash
      return Response.redirect(`${request.url.replace(suffix, "/")}`, 301)
    }
  }
  // Add a slash, unless there's a file extension or a query string
  if (!pathname.endsWith('/') && !pathname.split('/').pop()?.includes('.') && !pathname.split('/').pop()?.includes('?')) {
