...
In these situations, you will need to use something we call an _include file_. An include file is a separate Markdown file (stored in `_includes/some/shared-file.md`) where you will write content to be shared across multiple pages.
...
Sometimes we need to include files that are maintained in other places than the `cockroachdb/docs` repo but referenced in our docs. The `remote_include` tag is used for this. We most often use this tag for:

- SQL diagrams, which are maintained in the `cockroachdb/cockroach` repo
- Code samples, which are maintained in various repos

For SQL diagrams, we remotely include the entire (HTML) file like so:
...
For code samples, we usually want to show only part of a larger file to highlight a specific technique, or due to length considerations.

To accomplish this, the `remote_include` tag has a feature whereby you can pass arguements (usually named `START {text}` and `END {text}` by convention) that tell it to pull in the text of the remote file between `START {text}` and `END {text}`.

For example, in the file `movr-flask-application.md` (which becomes the page [Develop a Global Web Application](https://www.cockroachlabs.com/docs/v22.1/movr-flask-application.html)), the following `remote_include` appears:
...
If you browse to the `dbinit.sql` file linked above, you will find the following SQL code block that uses those start and end tags:
...
- Include files with a paragraph followed by a code block may not render correctly in all cases in the files they are included from due to a possible limitation in our [Markdown](#markdown) renderer.
