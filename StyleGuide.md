...
*You can document a known limitation too early (usually in the event that the limitation is a bug that engineering finds the time to fix during the stability period leading up to a GA release).*
...
Each known limitation should be documented on the [Known Limitations](https://www.cockroachlabs.com/docs/stable/known-limitations.html) page.

If the limitation is related to a feature documented elsewhere on our docs site, the page that documents that feature should also document the limitation, under a dedicated "Known limitations" header. In this case, you usually want to create a file in `_includes/vX.X/known-limitations` and just include the file in both places.
...
Feature doesn't do this thing because it just doesn't do it. To get around this, just do this other thing. For example, instead of `do this thing`, use `do this other thing`.
