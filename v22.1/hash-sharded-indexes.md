...
In general, the number of buckets present in a hash-sharded index has a direct proportion to the write performance and an inverse proportion to the performance of reading sequential data or ranges of data.
...
You can specify a different `bucket_count` on a hash-sharded index to optimize either write performance or sequential read performance on a table:
