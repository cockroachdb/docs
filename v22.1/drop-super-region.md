...
Note that you cannot [drop a region](drop-region.html) that is part of a super region until you first [drop the super region](drop-super-region.html). For example, using the "usa" super region that was added in [`ADD SUPER REGION`](add-super-region.html#add-a-super-region-to-a-database):
