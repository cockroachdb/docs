...
<img src="{{ 'images/v22.1/raw-status-endpoints.png' | relative_url }}" alt="Raw Status Endpoints" style="border:1px solid #eee;max-width:100%" />
...
While the active monitoring methods described above provide useful insight into the overall health and performance of a cluster, it is also essential to configure alerting rules that promptly send notifications when your CockroachDB instance experiences events that require investigation or intervention. This section identifies the most important events that you might want to create alerting rules for, and provides pre-defined rules definitions for these events appropriate for use with Prometheus' Alertmanager.

### Alertmanager

If you have configured [Prometheus](monitor-cockroachdb-with-prometheus.html) to monitor your CockroachDB instance, you can also configure Prometheus' Alertmanager to detect [important events](#events-to-alert-on) and alert you when they occur by using alerting rule definitions.

#### Prometheus alerting rules endpoint

<span class="version-tag">New in v22.1:</span> Every node of a CockroachDB cluster exports an alerting rules template at `http://<host>:<http-port>/api/v2/rules`. These rule definitions are formatted for easy integration with Prometheus' Alertmanager, and require minimal editing.
...
{{site.data.alerts.callout_info}}
Currently, not all events listed have corresponding alert rule definitions available from the the `api/v2/rules` endpoint. Events not yet available in this manner are either defined in the <a href="https://github.com/cockroachdb/cockroach/blob/master/monitoring/rules/alerts.rules.yml">pre-defined alerting rules</a> instead, or are forthcoming. For more details, see [Monitor CockroachDB with Prometheus](monitor-cockroachdb-with-prometheus.html).
{{site.data.alerts.end}}
...
- **Rule definition:** A rule definition for this alert is forthcoming.
...
- **Rule:** Send an alert when the number of ranges with less-than-quorum replicas online is non-zero for too long.
...
- **Rule:** Send an alert when total node storage capacity falls below a configurable threshold.

- **How to detect:** Calculate this using the `capacity` metric in the node's `_status/vars` output.

- **Rule definition:** Use the `node:capacity` recording rule from your cluster's [`api/v2/rules` metrics endpoint](#alertmanager).

#### Cluster capacity

- **Rule:** Send an alert when total cluster storage capacity falls below a configurable threshold.

- **How to detect:** Calculate this using the `capacity` metric in the node's `_status/vars` output.

- **Rule definition:** Use the `cluster:capacity` recording rule from your cluster's [`api/v2/rules` metrics endpoint](#alertmanager).

#### Node capacity available

- **Rule:** Send an alert when available node storage capacity falls below a configurable threshold.

- **How to detect:** Calculate this using the `capacity` metric in the node's `_status/vars` output.

- **Rule definition:** Use the `node:capacity_available` recording rule from your cluster's [`api/v2/rules` metrics endpoint](#alertmanager).

#### Cluster capacity available

- **Rule:** Send an alert when available cluster storage capacity falls below a configurable threshold.

- **How to detect:** Calculate this using the `capacity` metric in the node's `_status/vars` output.

- **Rule definition:** Use the `cluster:capacity_available` recording rule from your cluster's [`api/v2/rules` metrics endpoint](#alertmanager).

#### Capacity available ratio

- **Rule:** Send an alert when the ratio of available storage capacity to total storage capacity exceeds exceeds a configurable threshold.

- **How to detect:** Calculate this by dividing the `capacity_avilable` metric by the `capacity` metric in the node's `_status/vars` output.

- **Rule definition:** Use the `capacity_available:ratio` recording rule from your cluster's [`api/v2/rules` metrics endpoint](#alertmanager).

#### Node capacity available ratio

- **Rule:** Send an alert when the ratio of available node storage capacity to total node storage capacity exceeds exceeds a configurable threshold.

- **How to detect:** Calculate this by dividing the `capacity_avilable` metric by the `capacity` metric in the node's `_status/vars` output.

- **Rule definition:** Use the `node:capacity_available:ratio` recording rule from your cluster's [`api/v2/rules` metrics endpoint](#alertmanager).

#### Cluster capacity available ratio

- **Rule:** Send an alert when the ratio of available cluster storage capacity to total cluster storage capacity exceeds exceeds a configurable threshold.

- **How to detect:** Calculate this by dividing the `capacity_avilable` metric by the `capacity` metric in the node's `_status/vars` output.

- **Rule definition:** Use the `cluster:capacity_available:ratio` recording rule from your cluster's [`api/v2/rules` metrics endpoint](#alertmanager).
