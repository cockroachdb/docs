...
Certain `IMPORT TABLE` statements that defined the table schema inline are **not** supported in v22.1+.
...
To import data into a new table, use [`CREATE TABLE`](create-table.html) followed by [`IMPORT INTO`](import-into.html). `IMPORT INTO` supports `CSV`/`TSV`, `AVRO`, and `DELIMITED DATA`.  

For an example, read [Import into a new table from a CSV file](import-into.html#import-into-a-new-table-from-a-csv-file).
...
-  `IMPORT` supports [computed columns](computed-columns.html) for Postgres dump files only.
