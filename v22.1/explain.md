...
`GIST`             | Generate a sequence of bytes representing the flattened tree of operators and various operator specific metadata. See [`GIST` option](#gist-option).
...
A _gist_ is a sequence of bytes representing the flattened tree of operators and various operator specific metadata. You can use the `GIST` option to generate a gist of a plan.

For example, a query like `EXPLAIN SELECT * FROM abc UNION SELECT * FROM abc ORDER BY b,a` produces the following plan:
...
To generate the gist of this plan, you run `EXPLAIN(GIST) SELECT * FROM abc UNION SELECT * FROM abc ORDER BY b,a`, and it returns the the gist `AgFuAgAHAAAAEQFuAgAHAAAAERANAAYGAA==`. To convert a gist back into a plan, run `SELECT * from crdb_internal.decode_plan_gist('gist');`.
