...
Flag | Description
-----|------------
`--auto-enable-rangefeeds` | Override the default behavior of `cockroach demo`, which has rangefeeds enabled on startup. Use `--auto-enable-rangefeeds=false` to disable rangefeeds — this will improve performance. To run a [changefeed](create-and-configure-changefeeds.html) on a demo cluster, you must enable rangefeeds. See [Enable rangefeeds](use-changefeeds.html#enable-rangefeeds) for more detail. <br><br>**Default:** `true`
