...
- **CMEK keys** give you an additional layer of protection for your cluster's store key. When you enable [Customer Managed Encryption Keys (CMEK)](cmek.md), you provide a symmetric key that is created and managed by your organization, stored in your cloud tenant or Hashicorp Vault Secrets Manager. This key is used to encrypt the store key. Data can be read from or written to the cluster only when the CMEK key is available to the cluster's nodes. To temporarily or permanently prevent access to a cluster's data, you can disable or revoke access to the CMEK key. To reinstate access, you can restore access to the CMEK key.
