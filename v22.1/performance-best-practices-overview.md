...
Transactions that operate on the same range but _different index keys_ are limited by the overall hardware capacity of [the range lease holder](architecture/overview.html#architecture-leaseholder) node. These are referred to as _hot spots_.
