...
<a name="protect-pause"></a>`protect_data_from_gc_on_pause` | N/A |  When a [changefeed is paused](pause-job.html), ensure that the data needed to [resume the changefeed](resume-job.html) is not garbage collected. `protect_data_from_gc_on_pause` and `on_error=pause` will **not** provide protection from garbage collection if the [garbage collection TTL](configure-replication-zones.html#gc-ttlseconds) has already passed. <br><br>**Note:** If you use this option, changefeeds left paused can prevent garbage collection for long periods of time. <br><br>See [Garbage collection and changefeeds](use-changefeeds.html#garbage-collection-and-changefeeds) for more detail on protecting changefeed data.
