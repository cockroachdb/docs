...
`end_time` | [Timestamp](as-of-system-time.html#parameters) | <span class="version-tag">New in v22.1:</span> Indicate the time when a changefeed should stop. Provide a future timestamp to `end_time` in number of nanoseconds since the [Unix epoch](https://en.wikipedia.org/wiki/Unix_time). For example, `end_time="1654117200000000"`. At the specified time, the changefeed will stop running and return a `successful` status.
