...
`CREATEROLE`/`NOCREATEROLE` |  Allow or disallow a new role to [create](create-role.html), [alter](alter-role.html), and [drop](drop-role.html) other non-`admin` roles. <br><br>By default, the role option is set to `NOCREATEROLE` for all non-`admin` roles.
