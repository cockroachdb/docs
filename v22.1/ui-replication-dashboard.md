...
The **Snapshots Data Received** graph shows the amount of data received in bytes by each node via [Raft snapshot transfers](architecture/replication-layer.html#snapshots).

On hovering over the graph, the value for the following metric is displayed:

Metric | Description
-------|------------
`<node>` | The amount of snapshot data received in bytes per node.
