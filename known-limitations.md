...
The `unique_rowid()` [function](functions-and-operators.html) returns values JavaScript cannot exactly represent with numbers. For example, a `235191684988928001` value returned by `unique_rowid()` is represented as `235191684988928000` in JavaScript. Notice that the last digit is different. In such cases, if the JavaScript application temporarily stores the value `235191684988928001`, it is actually stored as `235191684988928000` and queries attempting to use the value will not receive the correct results.

As a workaround when using JavaScript (or any another language without support for 64-bit integer types), cast the integer to a string server-side before inspecting or using it, for example:
