...
`regions` | The [`ALTER DATABASE`](alter-database.html) statement(s) used to configure the database with [multi-region zone configurations](multiregion-overview.html#database-regions), if they exist. If the database is single-region, `NULL` will show. 
