...
- Security-related command-line arguments have been changed. Instead of using multiple flags, certificates and keys are now loaded from a single [`--certs-dir`](create-security-certificates.html#certificate-directory) flag, which defaults to `~/.cockroach-certs` but can be set to a custom location. The individual flags still work in this release but will be removed in the next one. Also note that the `--insecure` flag is now **required** for insecure mode. [#14703](https://github.com/cockroachdb/cockroach/pull/14703)
...
- The `^` operator now means `pow()` instead of bitwise xor (bitwise xor uses the `#` operator; this is consistent with PostgreSQL). Any tables with this operator in a [`DEFAULT`](default-value.html) or [`CHECK`](check.html) expression will need to be updated. [#14882](https://github.com/cockroachdb/cockroach/pull/14882)
...
- Type errors in [`CHECK`](check.html) constraints are now detected earlier. [#14984](https://github.com/cockroachdb/cockroach/pull/14984)
...
- Bounds are now checked when converting [`FLOAT`](float.html) or [`DECIMAL`](decimal.html) values to [`INTEGER`](integer.html). [#14967](https://github.com/cockroachdb/cockroach/pull/14967)
...
- The [`cockroach cert`](create-security-certificates.html) commands have been updated to reflect the new `--certs-dir` convention. [#14703](https://github.com/cockroachdb/cockroach/pull/14703)
...
- The [`cockroach quit`](stop-a-node.html) command now has a hard timeout of 1 minute if the cluster has lost quorum. [#14708](https://github.com/cockroachdb/cockroach/pull/14708)
...
- Expanded the [`TRUNCATE`](truncate.html) documentation to cover using `CASCADE` to truncate dependent tables. [#1297](https://github.com/cockroachdb/docs/pull/1297)
- Minor improvements to the [`ROLLBACK`](rollback-transaction.html) documentation. [#1296](https://github.com/cockroachdb/docs/pull/1296)
