...
<div id="os-tabs" class="clearfix">
    ...
    <a href="https://binaries.cockroachdb.com/cockroach-beta-20170420.windows-6.2-amd64.exe.tgz"><button id="windows" data-eventcategory="windows-binary-release-notes">Windows</button></a>
</div>

### Backwards-Incompatible Changes

- Security-related command-line arguments have been changed. Instead of individual flags, certificates and keys are now loaded from `--certs-dir` (defaults to `~/.cockroach-certs`, or settable with `COCKORACH_CERTS_DIR` environment variable). The individual flags still work in this release but will be removed in the next one. The `--insecure` flag is now **required** for insecure mode. [#14703](https://github.com/cockroachdb/cockroach/pull/14703)
...
- The `^` operator now means `pow()` instead of bitwise xor (bitwise xor uses the `#` operator; this is consistent with PostgreSQL). Any tables with this operator in a `DEFAULT` or `CHECK` expression will need to be updated. [#14882](https://github.com/cockroachdb/cockroach/pull/14882)
...
- CockroachDB's build system has been rewritten to improve portability. It can no longer be installed with `go get` alone; use of the `Makefile` is now required. GNU Make, CMake, and XZ Utils must also be installed to build from source. As a result, CockroachDB can now be built on a larger variety of systems, including experimental Windows support. [#14840](https://github.com/cockroachdb/cockroach/pull/14840)

### SQL Language Changes

- Type errors in `CHECK` constraints are now detected earlier. [#14984](https://github.com/cockroachdb/cockroach/pull/14984)
...
- Bounds are now checked when converting `FLOAT` or `DECIMAL` values to `INTEGER`. [#14967](https://github.com/cockroachdb/cockroach/pull/14967)
...
- New arguments `--log-file-max-size` (default 10 MiB) and `--log-dir-max-size` (default 100 MiB) can be used to control log rotation and deletion. `--log-file-verbosity` (default `INFO`) can be used to control the verbosity of the log file. [#14841](https://github.com/cockroachdb/cockroach/pull/14841)
...
- `cockroach quit` now has a hard timeout of 1 minute if the cluster has lost quorum. [#14708](https://github.com/cockroachdb/cockroach/pull/14708)
