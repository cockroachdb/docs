...
| `AS OF SYSTEM TIME timestamp` | Back up data as it existed as of `timestamp`. {% include aost-v1.0.md %} However, the `timestamp` must be more recent than your cluster's last garbage collection (which defaults to occur every 24 hours, but is [configurable per table](configure-replication-zones.html#replication-zone-format)). |
