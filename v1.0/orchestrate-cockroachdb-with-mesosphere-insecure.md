...
1. Discover the endpoints for your CockroachDB cluster:
    ...
    In general, the `.mesos` endpoints will only work from within the same DC/OS cluster. From outside the cluster, you can either use the direct IPs or set up a proxy service that acts as a frontend to your CockroachDB instance. For development and testing purposes, you can use [DC/OS Tunnel](https://dcos.io/docs/1.10/developing-services/tunnel/) to access services from outside the cluster, but this option is not suitable for production use. See [monitor the cluster](#step-4-monitor-the-cluster) below for more details.
...
To access the cluster's [Admin UI](admin-ui-overview.html), you can [use a DC/OS tunnel to run an HTTP proxy](https://dcos.io/docs/1.10/developing-services/tunnel/#using-dc-os-tunnel-to-run-a-http-proxy):
...
2. Start the DC/OS tunnel:

    ~~~ shell
    $ sudo dcos tunnel http
    ~~~

3. Discover the http endpoints for your CockroachDB cluster:

    {% include copy-clipboard.html %}
    ~~~ shell
    $ dcos cockroachdb endpoints http
    ~~~
    ...
    ~~~
    {
      ...
      "dns": [
        "cockroachdb-0-node-init.cockroachdb.autoip.dcos.thisdcos.directory:8123",
        "cockroachdb-1-node-join.cockroachdb.autoip.dcos.thisdcos.directory:8123",
        "cockroachdb-2-node-join.cockroachdb.autoip.dcos.thisdcos.directory:8123"
      ],
      "vip": "http.cockroachdb.l4lb.thisdcos.directory:80"
    }
    ~~~

3. In a browser, go to the `vip` address with `.mydcos.directory` appended before the `:80` port, e.g., `http.cockroachdb.l4lb.thisdcos.directory.mydcos.directory:80`.
...
The Scheduler process will then restart with the new configuration and will validate any detected changes. To check that nodes were successfully added to the cluster, [SSH to the DC/OS master node](https://dcos.io/docs/1.10/administering-clusters/sshcluster/):
...
The `cockroachdb` DC/OS service provides an easy way use CockroachDB's open source [`cockroach dump`](sql-dump.html) command to backup data on a per-database basis to an S3 bucket and to restore data from such a backup. Note that using datastores other than S3 is not yet supported.

{{site.data.alerts.callout_success}}If you need to backup to/restore from datasources other than S3, or you have a very large database and need <a href="backup.html">faster backups</a>, <a href="backup.html#incremental-backups">incremental backups</a>, or a <a href="restore.html">faster, distributed restore process</a>, consider contacting Cockroach Labs about an <a href="https://www.cockroachlabs.com/pricing/">enterprise license</a>.{{site.data.alerts.end}}
