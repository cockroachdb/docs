...
Each node of a CockroachDB cluster requires at a mimimum:

- 2 CPU cores
- 2 GB of RAM

If performance is a priority, also consider the following:

- Use SSDs over HHDs.
- Since CockroachDB balances data approximately evenly across nodes, use more nodes to increase the work that can be handled in parallel and to reduce the amount of data to recover in case of node failure.
- Use larger nodes to increase processing power and, on cloud platforms, to increase the chance of getting a dedicated machine.
