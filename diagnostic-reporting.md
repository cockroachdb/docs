...
{{site.data.alerts.callout_info}}No actual table data is shared, just schema data, and each name and other string value is scrubbed and replaced with an underscore.{{site.data.alerts.end}}
...
To turn diagnostic reporting off once a cluster is running, [use the built-in SQL client](use-the-built-in-sql-client.html) to execute the following `SET` statement, which switches the `diagnostics.reporting.enabled` [global configuration](global-configurations.html) to `false`:
