...
- [`INTEGER`](integer.html) and [`DECIMAL`](decimal.html) types can now be mixed in arithmetic expressions without casts. [#7756](https://github.com/cockroachdb/cockroach/pull/7756)
...
- Fixed a panic when single-statement [transactions](transaction.html) were aborted after an automatic retry. [#8010](https://github.com/cockroachdb/cockroach/pull/8010)
...
- [SQL statements](sql-statements.html), [data types](data-types.html), and [data definition](data-definition.html) topics are now available at-a-glance in the sidenav. [#483](https://github.com/cockroachdb/docs/pull/483)
- Docs for the [`cockroach dump`](back-up-and-restore-data.html) command are now available. [#472](https://github.com/cockroachdb/docs/pull/472)
- [Recommended Production Settings](recommended-production-settings.html) has been updated to clarify how CockroachDB allocates file descriptors when the limit is under the recommended amounts.[#480](https://github.com/cockroachdb/docs/pull/480)
- Docs for the [`INTERVAL`](interval.html) data type now cover all supported formats: Golang, Traditional Postgres, and ISO 8601. [#462](https://github.com/cockroachdb/docs/pull/462)
