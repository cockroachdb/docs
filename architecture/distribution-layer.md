...
The Transaction Layer of CockroachDB's architecture provides a unified view of your cluster's data.
...
To make all data in your cluster accessible from any node, CockroachDB uses a monolithic sorted map, which is a sorted key-value range that describes all of the data in your cluster, as well as its location.
...
To address data, the location of all ranges in your cluster are stored in a two-level index at the beginning of your key-space, known as meta ranges, where the first level (`meta1`) addresses the second, and the second (`meta2`) addresses data in the cluster. Importantly, every node has information on where to locate the `meta1` range (known as its Range Descriptor, detailed below), and the range is never split.

This meta range structure lets us address up to 4EB of user data: we can address 2^(18 + 18) = 2^36 ranges; each range addresses 2^26 B, and altogether we address 2^(36+26) B = 2^62 B = 4EB.
...
Like all other data in your cluster, meta ranges are structured as KV pairs, as well. Both meta ranges have a similar structure:

~~~
metaX/successorKey -> Lease HolderAddress, [list of other nodes containing data]
~~~
...
`Lease HolderAddress` | The range primarily responsible for reads and writes. The Replication Layer contains more information about [Leases](replication-layer.html#leases).
...
1. `meta1` contains the address for the nodes containing the `meta2` ranges.
...
Key-Value data, which represents the data in your tables uses the following structure:
...
- The addresses of nodes containing replicas of the range, with its Lease Holder (which is responsible for its reads and writes) in the first position. This determines the `meta2` range's key's values.

Whenever there are membership changes to a range's Raft group (discussed in more detail in the [Replication Layer](replication-layer.html#membership-changes-rebalance-repair)), its Lease Holder, or there are range splits, the range updates its Range Descriptor and then propagates the change to the `meta2` range.
...
Learn how CockroachDB presents a copies data and ensures consistency in the [Replication Layer](replication-layer.html).
