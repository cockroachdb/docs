...
CockroachDB was designed to create the open-source database our developers would want to use; one that is both scalable and consistent. Developers often have questions how we've achieved this, and this document sets out to detail the inner-workings of the `cockroach` process as a means of explanation.

However, you definitely do not need to understand the underlying architecture to use CockroachDB. These documents merely give serious users and database enthusiasts a high-level framework to explain what's happening under the hood.

### Using this Document

While these documents are broken out into sections detailing each layer of CockroachDB, they can be easier to understand if you read through the layers sequentially (i.e., starting with this overview and then proceeding to the SQL Layer).

If you're looking for a high-level understanding of CockroachDB, you can simply read the **Overview** section of each layer. For more technical detail––for example, if you're interested in [contributing to the project](https://github.com/cockroachdb/cockroach#contributing)––you should read the **Components** sections, as well.

And a caveat: these documents detail how CockroachDB is built, but do not explain how *you* should architect an application using CockroachDB. For help with your own application's architecture using CockroachDB, check out our [user docs page](https://cockroachlabs.com/docs/stable).
...
The following documents describe the function each layer performs, but most ignores the details of other layers. This description is true to the experience of the layers themselves, which treat the other layers as mostly black-box APIs. There are interactions that occur between layers which *are not* clearly articulated and require an understanding of each layer's function to understand the entire process.

Level | Layer Name | Purpose
------|------------|--------
1 | [SQL](sql-layer.html) | Translate client SQL queries to KV operations.
2 | [Transactional](transactional-layer.html) | Allow atomic changes to multiple KV entries
3 | [Distribution](distribution-layer.html) | Present replicated KV ranges as a single entity
4 | [Replication](replication-layer.html) | Consistently and synchronously replicate KV ranges across many nodes. This layer also enables consistent reads via leases.
5 | [Storage](storage-layer.html) | Write and read KV data on disk.
