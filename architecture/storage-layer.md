...
The Storage Layer of CockroachDB's architecture reads and writes data from disk.
...
CockroachDB stores its key-values on disk using RocksDB, which is treated primarily as a black-box API. Internally, each store contains three instance of RocksDB:
...
CockroachDB regularly garbage collects MVCC values to reduce the size of data stored on disk. To do this, we compact old MVCC values when there is a newer MVCC value with a timestamp that's older than the garbage collection period. By default, the garbage collection period is 24 hours, but it can be set on a per-table basis through [Replication Zones](configure-replication-zones.html).
...
Now that you've learned about our architecture, [start a local cluster]((/stable/install-cockroachdb.html)) and start [building an app with CockroachDB](/stable/build-an-app-with-cockroachdb.html/).
