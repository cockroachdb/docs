...
1. Start the [built-in SQL client](use-the-built-in-sql-client.html) in one-off interactive pod, using the `cockroachdb-public` hostname to access the CockroachDB cluster::
...
4. When you're done with the SQL shell, press **CTRL + D** to exit and delete the temporary pod.
...
Verify that a fourth pod/node was added successfully: 
