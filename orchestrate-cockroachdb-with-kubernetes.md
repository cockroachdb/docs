...
Databases are stateful applications, and so running them on Kubernetes requires some of its more complex features, which are still being actively developed and thus only available in alpha versions of Kubernetes. There are easier ways to run CockroachDB for testing purposes on Kubernetes, but the method presented here is destined to become a production deployment once Kubernetes matures sufficiently.

CockroachDB will run as a PetSet - roughly speaking, a replication set whose instances are distinguishable and always bound back to the same persistent storage on restart.
...
Normally, on-disk files in a pod are ephemeral. If a pod crashes and Kubernetes starts a new one, the on-disk files are lost. To prevent that from happening, you need to mount local temporary directories as [persistent volumes](http://kubernetes.io/docs/user-guide/persistent-volumes/) that will endure for as long as the Kubernetes cluster is running. You also need create [persistent volume claims](http://kubernetes.io/docs/user-guide/persistent-volumes/#persistentvolumeclaims); when pods are created later (one per node), each pod will request a persistent volume claim to "claim" durable storage for its node.
