...
- [Stop](stop-a-node.html) and [restart](start-a-node.html) problematic nodes with the `--logtostderr` flag, which prints logs to your terminal instead of writing them to a file, so you can see all of your cluster's as they occur.
