...
`--ca-cert` | The path to the [CA certificate](create-security-certificates.html). This flag is required to start a node in secure more. 
`--cert` | The path to the [node certificate](create-security-certificates.html). This flag is required to start a node in secure more.
`--host` | The address to listen on for internal and client communication. The node also advertises itself to other nodes using this address.<br><br>When running without `--insecure` and without cert flags (an insecure local cluster), this defaults to `localhost` and cannot be changed. When running with `--insecure` but without cert flags (an insecure distributed cluster) or without `--insecure` but with cert flags (a secure cluster), this can be an external address.
`--http-port` | The port to listen on for HTTP requests from the Admin UI. <br><br>**Default:** 8080
`--insecure` | If set, the cluster is insecure and listens on all interfaces. To run an insecure local cluster that listens only on `localhost`, leave this flag out. To run a secure cluster, leave this flag out and set the `--ca-cert`, `--cert`, and `-key` flags. 
...
`--key` | The path to the [node key](create-security-certificates.html) protecting the node certificate. This flag is required to start a node in secure more. 
