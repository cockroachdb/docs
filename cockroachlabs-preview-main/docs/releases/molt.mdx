...
This page has details about each release of the following [MOLT (Migrate Off Legacy Technology) tools](/docs/molt/migration-overview):

- `molt`: [MOLT Fetch](/docs/molt/molt-fetch) and [MOLT Verify](/docs/molt/molt-verify)
- `replicator`: [MOLT Replicator](/docs/molt/molt-replicator)
...
- Deprecated `data-load-and-replication`, `replication-only`, and `failback` modes in Fetch. Replication should now be performed separately with Replicator. Refer to the [MOLT Replicator](/docs/molt/molt-replicator) documentation.
...
- MOLT Fetch replication modes are deprecated in favor of a separate replication workflow using `replicator`. For details, refer to [MOLT Replicator](/docs/molt/molt-replicator).
...
- Added support for assume role authentication during [data export](/docs/molt/molt-fetch#data-export-phase) with MOLT Fetch.
...
- Added exponential backoff to `replicator` for retryable errors when applying mutations to the target database. This reduces load on the target database and prevents exhausting retries prematurely. The new [replication flags](/docs/molt/replicator-flags) `--retryInitialBackoff`, `--retryMaxBackoff`, and `--retryMultiplier` control backoff behavior. The new `--maxRetries` flag configures the maximum number of retries. To retain the previous "immediate retry" behavior, set `--retryMaxBackoff 1ns --retryInitialBackoff 1ns --retryMultiplier 1`.
...
- MOLT Fetch now supports [sharding](/docs/molt/molt-fetch#table-sharding) of primary keys of any data type on PostgreSQL 11+ sources. This can be enabled with the [`--use-stats-based-sharding`](/docs/molt/molt-fetch#global-flags) flag.
- Added the [`--ignore-replication-check`](/docs/molt/molt-fetch#global-flags) flag to allow data loads with planned downtime and no replication setup. The `--pglogical-ignore-wal-check` flag has been removed.
- Added the `--enableParallelApplies` [replication flag](/docs/molt/molt-replicator#flags) to enable parallel application of independent table groups during replication. By default, applies are synchronous. When enabled, this increases throughput at the cost of increased target pool and memory usage.
...
- MOLT Fetch [failback](/docs/molt/migrate-failback) now reliably creates changefeeds with a sorted list of table names so that create changefeed operations can be properly deduplicated.
...
- Fixed a bug in [`--direct-copy` mode](/docs/molt/molt-fetch#direct-copy) that occurred when [`--case-sensitive`](/docs/molt/molt-fetch#global-flags) was set to `false` (default). Previously, the `COPY` query could use incorrect column names in some cases during data transfer, causing errors. The query now uses the correct column names.
...
- MOLT Fetch users can now set [`--table-concurrency`](/docs/molt/molt-fetch#global-flags) and [`--export-concurrency`](/docs/molt/molt-fetch#global-flags) to values greater than `1` for MySQL sources.
- MOLT Fetch now supports case-insensitive comparison of table and column names by default. Previously, case-sensitive comparisons could result in `no matching table on target` errors. To disable case-sensitive comparisons explicitly, set [`--case-sensitive=false`](/docs/molt/molt-fetch#global-flags). If `=` is **not** included (e.g., `--case-sensitive false` ), this is interpreted as `--case-sensitive` (i.e., `--case-sensitive=true` ).

### February 5, 2025

`molt` 1.2.2 is [available](#installation).

- Added an [`--import-region`](/docs/molt/molt-fetch#global-flags) flag that is used to set the `AWS_REGION` query parameter explicitly in the [`s3` URL](/docs/molt/molt-fetch#bucket-path).
- Fixed the [`truncate-if-exists`](/docs/molt/molt-fetch#target-table-handling) schema mode for cases where there are uppercase table or schema names.
- Fixed an issue with unsigned `BIGINT` values overflowing in replication.
- Added a `--schemaRefresh` [replication flag](/docs/molt/molt-replicator#flags) that is used to configure the schema watcher refresh delay in the replication phase. Previously, the refresh delay was set to a constant value of 1 minute. Set the flag as follows: `--replicator-flags "--schemaRefresh {value}"`.

### December 13, 2024

`molt` 1.2.1 is [available](#installation).

- MOLT Fetch users now can use [`--assume-role`](/docs/molt/molt-fetch#global-flags) to specify a service account for assume role authentication to cloud storage. `--assume-role` must be used with `--use-implicit-auth`, or the flag will be ignored.
...
- MySQL columns of type `BIGINT UNSIGNED` or `SERIAL` are now auto-mapped to [`DECIMAL`](/docs/v25.4/decimal) type in CockroachDB. MySQL regular `BIGINT` types are mapped to [`INT`](/docs/v25.4/int) type in CockroachDB.
- The `pglogical` replication workflow was modified in order to enforce safer and simpler defaults for the [`data-load`](/docs/molt/molt-fetch#fetch-mode), `data-load-and-replication`, and `replication-only` workflows for PostgreSQL sources. Fetch now ensures that the publication is created before the slot, and that `replication-only` defaults to using publications and slots created either in previous Fetch runs or manually.
...
- For a MySQL source in `replication-only` mode, the [`--stagingSchema` replicator flag](/docs/molt/replicator-flags#staging-schema) can now be used to resume streaming replication after being interrupted. Otherwise, the [`--defaultGTIDSet` replicator flag](/docs/molt/replicator-flags#default-gtid-set) is used to start initial replication after a previous Fetch run in [`data-load`](/docs/molt/molt-fetch#fetch-mode) mode, or as an override to the current replication stream.
...
- The [`--pprof-list-addr` flag](/docs/molt/molt-fetch#global-flags), which specifies the address of the `pprof` endpoint, is now configurable. The default value is `'127.0.0.1:3031'`.
- [Fetch modes](/docs/molt/molt-fetch#fetch-mode) involving replication now state that MySQL 8.0 and later are supported for replication between MySQL and CockroachDB.
- [Partitioned tables](/docs/molt/molt-fetch#transformations) can now be moved to CockroachDB using [`IMPORT INTO`](/docs/v25.4/import-into).
- Improved logging for the [Fetch](/docs/molt/molt-fetch) schema check phases under the `trace` logging level, which is set with [`--logging trace`](/docs/molt/molt-fetch#global-flags).
- Added a [sample Grafana dashboard](https://molt.cockroachdb.com/molt/cli/grafana_dashboard.json) for monitoring MOLT tasks.
- Fetch now logs the name of the staging database in the target CockroachDB cluster used to store metadata for [replication modes](/docs/molt/molt-fetch#fetch-mode).
- String [primary keys](/docs/v25.4/primary-key) that use `C` [collations](/docs/v25.4/collate) on PostgreSQL can now be compared to the default `en_US.utf8` on CockroachDB.
...
- When a [Fetch transformation rule](/docs/molt/molt-fetch#transformations) is used to rename a table or map partitioned tables, a script in the format `partitionTableScript.{timestamp}.ts` is now automatically generated to ensure that [replication](/docs/molt/molt-fetch#fetch-mode) works properly if enabled.

### August 19, 2024

`molt` 1.1.6 is [available](#installation).

- Fixed a Fetch bug where [`--table-exclusion-filter`](/docs/molt/molt-fetch#schema-and-table-selection) was ignored when `--table-filter` and `--schema-filter` were set to the default ( `'.*'` ).
...
- Fetch can now be run in an export-only mode by specifying [`--mode export-only`](/docs/molt/molt-fetch#fetch-mode). This will export all the data in `csv` or `csv.gz` format to the specified cloud or local store.
- Fetch can now be run in an import-only mode by specifying [`--mode import-only`](/docs/molt/molt-fetch#fetch-mode). This will load all data in the specified cloud or local store into the target CockroachDB database, effectively skipping the export data phase.
...
- Partitioned tables can now be mapped to renamed tables on the target database, using the Fetch [transformations framework](/docs/molt/molt-fetch#transformations).
...
- Fixed an issue where connections were not being closed immediately after sharding was completed. This could lead to errors if the [maximum number of connections](/docs/molt/molt-fetch#best-practices) was set to a low value.
- Fetch users can now exclude specific tables from migration using the [`--table-exclusion-filter` flag](/docs/molt/molt-fetch#global-flags).
...
- Fetch now supports column exclusions and computed column mappings via a new [transformations framework](/docs/molt/molt-fetch#transformations).
- The new Fetch [`--transformations-file`](/docs/molt/molt-fetch#global-flags) flag specifies a JSON file for schema/table/column transformations, which has validation utilities built in.

### July 10, 2024

`molt` 1.1.1 is [available](#installation).

- Fixed a bug that led to incorrect list continuation file behavior if a trailing slash was provided in [`--bucket-path`](/docs/molt/molt-fetch#global-flags).
...
- Added a [`--use-implicit-auth`](/docs/molt/molt-fetch#global-flags) flag that determines whether [implicit authentication](/docs/v25.4/cloud-storage-authentication) is used for cloud storage import URIs.
...
- Renamed the `--table-splits` flag to [`--concurrency-per-table`](/docs/molt/molt-fetch#global-flags), which is more descriptive.
- Increased the default value of [`--import-batch-size`](/docs/molt/molt-fetch#global-flags) to `1000`. This leads to better performance on the target post-migration. Each individual import job will take longer, since more data is now imported in each batch, but the sum total of all jobs should take the same (or less) time.

### May 29, 2024

`molt` 0.3.0 is [available](#installation).

- Added an [`--import-batch-size`](/docs/molt/molt-fetch#global-flags) flag, which configures the number of files to be imported in each `IMPORT` job.
...
- [`VARBIT`](/docs/v25.4/bit) &lt;&gt; [`BOOL`](/docs/v25.4/bool) conversion is now allowed for Fetch and Verify. The bit array is first converted to `UINT64`. A resulting `1` or `0` is converted to `true` or `false` accordingly. If the `UINT64` is another value, an error is emitted.
...
- When using MySQL as a source, [`--table-concurrency`](/docs/molt/molt-fetch#global-flags) and [`--export-concurrency`](/docs/molt/molt-fetch#global-flags) are strictly set to `1`.
- Fixed a bug involving history retention for [`DECIMAL`](/docs/v25.4/decimal) values.

### May 3, 2024

`molt` 0.2.0 is [available](#installation).

- Fetch now supports CockroachDB [multi-region tables](/docs/v25.4/multiregion-overview).
- Fetch now supports continuous replication for PostgreSQL and MySQL source databases via the [`--ongoing-replication`](/docs/molt/molt-fetch#global-flags) flag. When Fetch finishes the initial data load phase, it will start the replicator process as a subprocess, which runs indefinitely until the user ends the process with a `SIGTERM` ( `ctrl-c` ).
- Replicator flags for ( [PostgreSQL](https://github.com/cockroachdb/replicator/wiki/PGLogical#postgresql-logical-replication) and [MySQL](https://github.com/cockroachdb/replicator/wiki/MYLogical#mysqlmariadb-replication) ) are now supported, allowing users to further configure the [`--ongoing-replication`](/docs/molt/molt-fetch#global-flags) mode for their use case.
- Added the [`--type-map-file`](/docs/molt/molt-fetch#global-flags) flag, which enables custom type mapping for schema creation.
