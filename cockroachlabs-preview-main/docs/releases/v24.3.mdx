...
CockroachDB v24.3 [(LTS)](/docs/releases/release-support-policy#support-types) is a required [Regular release](/docs/releases#major-versions).
...
On this page, you can read about changes and find downloads for all production and testing releases of CockroachDB v24.3 [(LTS)](/docs/releases/release-support-policy#support-phases)
...
- After downloading a supported CockroachDB binary, learn how to [install CockroachDB](/docs/v24.3/install-cockroachdb) or [upgrade your cluster](/docs/v24.3/upgrade-cockroach-version).
...
- CockroachDB v24.3.12 and subsequent v24.3 releases are eligible for [long term support (LTS)](/docs/releases/release-support-policy#support-types).
...
Before [upgrading to CockroachDB v24.3](/docs/v24.3/upgrade-cockroach-version), be sure to review the following backward-incompatible changes, as well as [key cluster setting changes](#v24-3-0-cluster-settings), and adjust your deployment as necessary.

If you plan to upgrade to v24.3 directly from v24.1 and skip v24.2, be sure to also review the [v24.2 release notes](/docs/releases/v24.2) for backward-incompatible changes from v24.1.

- Upgrading to v24.3 is blocked if no [license](/docs/v24.3/licensing-faqs) is installed, or if a trial/free license is installed with telemetry disabled. [#130576](https://github.com/cockroachdb/cockroach/pull/130576)
...
- A cluster must have an [Enterprise license](/docs/v24.3/licensing-faqs#set-a-license) or a [trial license](/docs/v24.3/licensing-faqs#obtain-a-license) set before an upgrade to v24.3 can be finalized.
- New clusters that are initialized for the first time on v24.3, and clusters that are upgraded to v24.3 will now have a [zone config](/docs/v24.3/configure-replication-zones) defined for the `timeseries` range if it does not already exist, which specifies the value for `gc.ttlseconds`, but inherits all other attributes from the zone config for the `default` range.

#### Key Cluster Setting Changes

Changes to [cluster settings](/docs/v24.3/cluster-settings) should be reviewed prior to upgrading. New default cluster setting values will be used unless you have manually set a value for a setting. This can be confirmed by running the SQL statement `SELECT * FROM system.settings` to view the non-default settings.
...
- `goschedstats.always_use_short_sample_period.enabled`: when set to `true`, helps to prevent unnecessary queueing due to CPU [admission control](/docs/v24.3/admission-control) by forcing `1ms` sampling of runnable queue lengths. The default value is `false`. [#133585](https://github.com/cockroachdb/cockroach/pull/133585)
- `kv.range.range_size_hard_cap`: allows you to limit how large a [range](/docs/v24.3/architecture/overview#architecture-range) can grow before [backpressure](/docs/v24.3/common-errors#split-failed-while-applying-backpressure-are-rows-updated-in-a-tight-loop) is applied. This can help to mitigate against a situation where a range cannot be split, such as when a range is comprised of a single key due to an issue with the schema or workload pattern, or a bug in client application code. The default is `8 GiB`, 16 times the default maximum range size. If you have changed the maximum range size, you may need to adjust this cluster setting or reduce the range size. [#129450](https://github.com/cockroachdb/cockroach/pull/129450)
- `kvadmission.flow_controller.token_reset_epoch`: can be used to refill replication [admission control](/docs/v24.3/admission-control) v2 tokens. This setting is marked as `reserved`, as it is not supported for tuning, by default. Use it only after consultation with your account team. [#133294](https://github.com/cockroachdb/cockroach/pull/133294)
- `kvadmission.store.snapshot_ingest_bandwidth_control.enabled`: enables a new [Admission Control](/docs/v24.3/admission-control) integration for pacing snapshot ingest traffic based on disk bandwidth. It requires provisioned bandwidth to be set for the store, or the cluster through the setting `kvadmission.store.provisioned_bandwidth`, for it to take effect. [#131243](https://github.com/cockroachdb/cockroach/pull/131243)
- Settings have been added which control the refresh behavior for the cached data in the Databases page of the [DB Console](/docs/v24.3/ui-overview):
...
- `server.jwt_authentication.client.timeout`: the HTTP client timeout for external calls made during [JWT authentication](/docs/v24.3/sso-sql). [#127145](https://github.com/cockroachdb/cockroach/pull/127145)
- Partial [statistics](/docs/v24.3/cost-based-optimizer#table-statistics) can now be automatically collected at the extremes of indexes when a certain fraction and minimum number of rows are stale (by default 5% and 100%, respectively). These can be configured with new [table storage parameters](/docs/v24.3/alter-table#set-and-reset-storage-parameters) and [cluster settings](/docs/v24.3/cluster-settings):
...
- `sql.stats.histogram_buckets.include_most_common_values.enabled`: controls whether common values are included in [histogram collection](/docs/v24.3/cost-based-optimizer#control-histogram-collection) for use by the [optimizer](/docs/v24.3/cost-based-optimizer). When enabled, histogram buckets will represent the most common sampled values as upper bounds. [#129378](https://github.com/cockroachdb/cockroach/pull/129378)
...
- `sql.txn.repeatable_read_isolation.enabled`: defaults to `false`. When set to `true`, the following statements configure transactions to run under `REPEATABLE READ` isolation, rather than being automatically interpreted as [`SERIALIZABLE`](/docs/v24.3/demo-serializable):
...
- Cluster settings for configuring [rate limiting for traffic to cloud storage](/docs/v24.3/backup#cluster-settings-for-cloud-storage) are now public.
...
- The settings [`enterprise.license`](/docs/v24.3/cluster-settings#setting-enterprise-license) and [`diagnostics.reporting.enabled`](/docs/v24.3/cluster-settings#setting-diagnostics-reporting-enabled) now have additional validation. To disable diagnostics reporting, the cluster must also have a license that is not an [Enterprise Trial or Enterprise Free license](/docs/v24.3/licensing-faqs#types-of-licenses). Additionally, to set one of these licenses, the cluster must already be submitting diagnostics information. [#131097](https://github.com/cockroachdb/cockroach/pull/131097) [#132257](https://github.com/cockroachdb/cockroach/pull/132257)
...
- The description of the setting [`changefeed.sink_io_workers`](/docs/v24.3/cluster-settings#setting-changefeed-sink-io-workers) has been updated to reflect all of the [sinks](/docs/v24.3/changefeed-sinks) that support the setting: the batching versions of webhook, pubsub, and kafka sinks that are enabled by `changefeed.new_<sink type>_sink_enabled`. [#129946](https://github.com/cockroachdb/cockroach/pull/129946)

#### Deprecations

The following deprecations are announced in v24.3. If you plan to upgrade to v24.3 directly from v24.1 and skip v24.2, be sure to also review the [v24.2 release notes](/docs/releases/v24.2) for deprecations.

- The session variable [`enforce_home_region_follower_reads_enabled`](/docs/v24.3/session-variables#enforce-home-region-follower-reads-enabled) is now deprecated, and will be removed in a future release. The related session variable [`enforce_home_region`](/docs/v24.3/session-variables#enforce-home-region) is **not** deprecated. [#129024](https://github.com/cockroachdb/cockroach/pull/129024)

#### Known limitations

For information about new and unresolved limitations in CockroachDB v24.3, with suggested workarounds where applicable, refer to [Known Limitations](/docs/v24.3/known-limitations).
...
- [Physical Cluster Replication](/docs/v24.3/physical-cluster-replication-overview) reader catalogs now bypass AOST timestamps using the `bypass_pcr_reader_catalog_aost` session variable, which can be used to modify cluster settings within the reader. [#133876](https://github.com/cockroachdb/cockroach/pull/133876)

### Operational changes

- Added a timer for inner [changefeed sink](/docs/v24.3/changefeed-sinks) client flushes. [#133288](https://github.com/cockroachdb/cockroach/pull/133288)
...
- The new cluster setting `kvadmission.flow_controller.token_reset_epoch` can be used to refill replication [admission control](/docs/v24.3/admission-control) v2 tokens. This is an advanced setting. Use it only after consultation with your account team. [#133294](https://github.com/cockroachdb/cockroach/pull/133294)
...
- In [Database](/docs/v24.3/ui-databases-page) pages, the **Refresh** tooltip now includes details about the progress of cache updates and when the job started. [#133351](https://github.com/cockroachdb/cockroach/pull/133351)

### Bug fixes

- Fixed a bug where [changefeed sink](/docs/v24.3/changefeed-sinks) ) timers were not correctly registered with the metric system. [#133288](https://github.com/cockroachdb/cockroach/pull/133288)
...
- Fixed a panic that could occur while running a `CREATE TABLE AS` statement that included a [sequence](/docs/v24.3/create-sequence) with an invalid function overload. [#133870](https://github.com/cockroachdb/cockroach/pull/133870)
...
- If a table is the destination of a logical data replication stream, then only schema change statements that are deemed safe are allowed on the table. Safe statements are those that do not result in a rebuild of the primary [index](/docs/v24.3/indexes) and do not create an index on a virtual [computed column](/docs/v24.3/computed-columns). [#133266](https://github.com/cockroachdb/cockroach/pull/133266)

### Operational changes

- The two new metrics `sql.crud_query.count` and `sql.crud_query.started.count` measure the number of [`INSERT`](/docs/v24.3/insert) / [`UPDATE`](/docs/v24.3/update) / [`DELETE`](/docs/v24.3/delete) / [`SELECT`](/docs/v24.3/selection-queries) queries executed and started respectively. [#133198](https://github.com/cockroachdb/cockroach/pull/133198)
- When creating a logical data replication stream, any [user-defined types](/docs/v24.3/create-type) in the source and destination are now checked for equivalency. This allows for creating a stream that handles user-defined types without needing to use the `WITH SKIP SCHEMA CHECK` option as long as the stream uses `mode = immediate`. [#133274](https://github.com/cockroachdb/cockroach/pull/133274)
- Logical data replication streams that reference tables with [user-defined types](/docs/v24.3/create-type) can now be created with the `mode = immediate` option. [#133295](https://github.com/cockroachdb/cockroach/pull/133295)

### DB Console changes

- The **SQL Statements** graph on the [**Overview**](/docs/v24.3/ui-overview-dashboard) and [**SQL**](/docs/v24.3/ui-sql-dashboard) dashboard pages in DB Console has been renamed [**SQL Queries Per Second**](/docs/v24.3/ui-overview-dashboard#sql-queries-per-second) and now shows **Total Queries** as a general Queries Per Second (QPS) metric. [#133198](https://github.com/cockroachdb/cockroach/pull/133198)
- Due to the inaccuracy of the **Range Count** column on the [**Databases** page](/docs/v24.3/ui-databases-page) and the cost incurred to fetch the correct range count for every database in a cluster, this data will no longer be visible. This data is still available via a [`SHOW RANGES`](/docs/v24.3/show-ranges) query. [#133267](https://github.com/cockroachdb/cockroach/pull/133267)

### Bug fixes

- Users with the [`admin` role](/docs/v24.3/security-reference/authorization#admin-role) can now run [`ALTER DEFAULT PRIVILEGES FOR target_role...`](/docs/v24.3/alter-default-privileges) on any `target_role`. Previously, this could result in a privilege error, which is incorrect as `admin` s are allowed to perform any operation. [#133072](https://github.com/cockroachdb/cockroach/pull/133072)
- [`REASSIGN OWNED BY current_owner_role...`](/docs/v24.3/reassign-owned) will now transfer ownership of the `public` schema. Previously, it would always skip over the `public` schema even if it was owned by the `current_owner_role`. [#133072](https://github.com/cockroachdb/cockroach/pull/133072)
...
- The cluster setting [`diagnostics.reporting.enabled`](/docs/v24.3/cluster-settings#setting-diagnostics-reporting-enabled) is now ignored if the cluster has a [Enterprise Trial or Enterprise Free license](/docs/v24.3/licensing-faqs#types-of-licenses), or if the reporting job is unable to load any license at all. [#132257](https://github.com/cockroachdb/cockroach/pull/132257)

### Enterprise edition changes

- This change ensures authorization with LDAP only works when the `ldapgrouplistfilter` option is present in the [HBA configuration](/docs/v24.3/security-reference/authentication#hba-configuration-syntax), otherwise authentication will proceed with the provided LDAP auth method options in the HBA configuration. This change is to ensure external authorization with LDAP is opt-in rather than enabled by default. [#132235](https://github.com/cockroachdb/cockroach/pull/132235)
- Added a [changefeed sink](/docs/v24.3/changefeed-sinks) error metric `changefeed.sink_errors`, and expanded reporting of the internal retries metric `changefeed.internal_retry_message_count` to all sinks that perform internal retries. [#132092](https://github.com/cockroachdb/cockroach/pull/132092)

### SQL language changes

- Implemented `DROP TRIGGER` statements. The `CASCADE` option for dropping a [trigger](/docs/v24.3/sql-feature-support) is not supported. [#128540](https://github.com/cockroachdb/cockroach/pull/128540)
- Added support for `CREATE TRIGGER`. The `OR REPLACE` syntax is not supported. Also, [triggers](/docs/v24.3/sql-feature-support) cannot be executed, so creation is a no-op. [#128540](https://github.com/cockroachdb/cockroach/pull/128540)
- [`REGIONAL BY ROW`](/docs/v24.3/table-localities#regional-by-row-tables) and [`PARTITION ALL BY`](/docs/v24.3/partitioning) tables can now be inserted into under [non-`SERIALIZABLE` isolation levels](/docs/v24.3/read-committed) as long as there is no `ON CONFLICT` clause in the statement. Also, [`REGIONAL BY ROW`](/docs/v24.3/table-localities#regional-by-row-tables) and [`PARTITION ALL BY`](/docs/v24.3/partitioning) tables can now be updated under non- `SERIALIZABLE` isolation levels. [#129837](https://github.com/cockroachdb/cockroach/pull/129837)
- Attempting to add [foreign keys](/docs/v24.3/foreign-key) referencing a table with [row-level TTL](/docs/v24.3/row-level-ttl) enabled will generate a notice informing the user about potential impact on the row-level TTL deletion job. Similarly, a notice is generated while attempting to enable row-level TTL on a table that has inbound foreign key references. [#127935](https://github.com/cockroachdb/cockroach/pull/127935)
- It is now possible to assign to an element of a composite typed variable in [PL/pgSQL](/docs/v24.3/plpgsql). For example, given a variable `foo` with two integer elements `x` and `y`, the following assignment statement is allowed: `foo.x:= 100;`. [#132628](https://github.com/cockroachdb/cockroach/pull/132628)
- [Backup](/docs/v24.3/backup) and [restore](/docs/v24.3/restore) now work for tables with triggers. When the [`skip_missing_udfs` option](/docs/v24.3/restore#skip-missing-udfs) is applied, triggers with missing trigger functions are removed from the table. [#128555](https://github.com/cockroachdb/cockroach/pull/128555)
- `UPSERT and INSERT... ON CONFLICT` statements are now supported on [`REGIONAL BY ROW`](/docs/v24.3/table-localities#regional-by-row-tables) tables under [`READ COMMITTED` isolation](/docs/v24.3/read-committed). [#132768](https://github.com/cockroachdb/cockroach/pull/132768)
...
- Added support for [PL/pgSQL](/docs/v24.3/plpgsql) integer `FOR` loops, which iterate over a range of integer values. [#130211](https://github.com/cockroachdb/cockroach/pull/130211)

### Operational changes

- [Admission Control](/docs/v24.3/admission-control) now has an integration for pacing snapshot ingest traffic based on disk bandwidth. `kvadmission.store.snapshot_ingest_bandwidth_control.enabled` is used to turn on this integration. It requires provisioned bandwidth to be set for the store (or cluster through the [cluster setting](/docs/v24.3/cluster-settings) ) for it to take effect. [#131243](https://github.com/cockroachdb/cockroach/pull/131243)
- Added validation to check whether [audit logging](/docs/v24.3/configure-logs) and [buffering configurations](/docs/v24.3/configure-logs#log-buffering-for-network-sinks) are both present in the [file log sink](/docs/v24.3/configure-logs#configure-log-sinks). Audit logging and buffering configuration should not both exist in the file log sink. [#132742](https://github.com/cockroachdb/cockroach/pull/132742)
- Updated the [file log sink](/docs/v24.3/configure-logs#configure-log-sinks) validation message. This would give clear indication to the user about the expected valid configuration. [#132899](https://github.com/cockroachdb/cockroach/pull/132899)

### DB Console changes

- The value of the automatic [statistics](/docs/v24.3/cost-based-optimizer#table-statistics) collection cluster setting [`sql.stats.automatic_collection.enabled`](/docs/v24.3/cluster-settings#setting-sql-stats-automatic-collection-enabled) is now in the top right corner of the [**Databases**](/docs/v24.3/ui-databases-page) overview page. [#132269](https://github.com/cockroachdb/cockroach/pull/132269)
- In the new [**Databases**](/docs/v24.3/ui-databases-page) and [**Tables**](/docs/v24.3/ui-databases-page#tables-list-tab) pages, when cached data is being refreshed, the refresh button will be disabled and its tooltip text will display, `Data is currently refreshing`. [#132462](https://github.com/cockroachdb/cockroach/pull/132462)

### Bug fixes

- Addressed a rare bug that could prevent [backups](/docs/v24.3/backup) taken during a [`DROP COLUMN`](/docs/v24.3/alter-table#drop-column) operation with a [sequence](/docs/v24.3/create-sequence) owner from [restoring](/docs/v24.3/restore) with the error: `rewriting descriptor ids: missing rewrite for <id> in SequenceOwner...`. [#132202](https://github.com/cockroachdb/cockroach/pull/132202)
- Fixed a bug existing since before v23.1 that could lead to incorrect results in rare cases. The bug requires a [join](/docs/v24.3/joins) between two tables with an equality between columns with equivalent, but not identical [types](/docs/v24.3/data-types) (e.g., `OID` and `REGCLASS` ). In addition, the join must lookup into an [index](/docs/v24.3/indexes) that includes a [computed column](/docs/v24.3/computed-columns) that references one of the equivalent columns. [#126345](https://github.com/cockroachdb/cockroach/pull/126345)
- Fixed a bug existing since before v23.1 that could lead to incorrect results in rare cases. The bug requires a lookup [join](/docs/v24.3/joins) into a table with a computed [index](/docs/v24.3/indexes) column, where the [computed column](/docs/v24.3/computed-columns) expression is composite sensitive. A composite sensitive expression can compare differently if supplied non-identical but equivalent input values (e.g., `2.0::DECIMAL` versus `2.00::DECIMAL` ). [#126345](https://github.com/cockroachdb/cockroach/pull/126345)
- Fixed a bug that caused quotes around the name of a routine to be dropped when it was called within another [routine](/docs/v24.3/user-defined-functions). This could prevent the correct routine from being resolved if the nested routine name was case-sensitive. The bug has existed since v24.1 when nested routines were introduced. [#131643](https://github.com/cockroachdb/cockroach/pull/131643)
- Fixed a bug where the [SQL shell](/docs/v24.3/cockroach-sql) would print out the previous error message when executing the `quit` command. [#130736](https://github.com/cockroachdb/cockroach/pull/130736)
- Fixed a bug where a [span statistics](/docs/v24.3/show-ranges#span-statistics) request on a mixed-version cluster resulted in a null pointer exception. [#132349](https://github.com/cockroachdb/cockroach/pull/132349)
- Fixed an issue where [changefeeds](/docs/v24.3/change-data-capture-overview) would fail to update [protected timestamp records](/docs/v24.3/protect-changefeed-data) in the face of [retryable errors](/docs/v24.3/monitor-and-debug-changefeeds#changefeed-retry-errors). [#132712](https://github.com/cockroachdb/cockroach/pull/132712)
- The [`franz-go`](https://github.com/twmb/franz-go) library has been updated to fix a potential deadlock on [changefeed](/docs/v24.3/change-data-capture-overview) restarts. [#132761](https://github.com/cockroachdb/cockroach/pull/132761)
- Fixed a bug that in rare cases could cause incorrect evaluation of [scalar expressions](/docs/v24.3/scalar-expressions) involving `NULL` values. [#132261](https://github.com/cockroachdb/cockroach/pull/132261)
- Fixed a bug in the query [optimizer](/docs/v24.3/cost-based-optimizer) that in rare cases could cause CockroachDB nodes to crash. The bug could occur when a query contains a filter in the form `col IN (elem0, elem1,..., elemN)` only when `N` is very large, (e.g., 1.6+ million), and when `col` exists in a [hash-sharded index](/docs/v24.3/hash-sharded-indexes), or exists a table with an indexed, [computed column](/docs/v24.3/computed-columns) dependent on `col`. [#132701](https://github.com/cockroachdb/cockroach/pull/132701)
- The `proretset` column of the [`pg_catalog.pg_proc`](/docs/v24.3/pg-catalog) table is now properly set to `true` for set-returning built-in functions. [#132853](https://github.com/cockroachdb/cockroach/pull/132853)
- Fixed an error that could be caused by using an [`AS OF SYSTEM TIME`](/docs/v24.3/as-of-system-time) expression that references a user-defined (or unknown) type name. These kinds of expressions are invalid, but previously the error was not handled properly. Now, a correct error message is returned. [#132348](https://github.com/cockroachdb/cockroach/pull/132348)
...
- The parameters for an [HBA config entry](/docs/v24.3/security-reference/authentication#hba-configuration-syntax) for LDAP are now validated when the entry is created or amended, in addition to the validation that happens during an authentication attempt. [#132086](https://github.com/cockroachdb/cockroach/pull/132086)
- Added automatic cleanup and validation for [default privileges](/docs/v24.3/security-reference/authorization#default-privileges) that reference dropped roles after a major-version upgrade to v24.3. [#131782](https://github.com/cockroachdb/cockroach/pull/131782)
...
- You can now [authenticate to the DB console API](/docs/v24.3/ui-overview#authentication) by supplying a Java Web Token (JWT) as a Bearer token in the Authorization header. [#130779](https://github.com/cockroachdb/cockroach/pull/130779)

### SQL language changes

- To view comments on a type, you can use the new [`SHOW TYPES WITH COMMENT`](/docs/v24.3/show-types) command. Comments can be added using [`COMMENT ON`](/docs/v24.3/comment-on). [#131183](https://github.com/cockroachdb/cockroach/pull/131183)
- You can create or alter a [user-defined function (UDF)](/docs/v24.3/user-defined-functions) or [stored procedure (SP)](/docs/v24.3/stored-procedures) with `[EXTERNAL] SECURITY DEFINER` instead of the default `[EXTERNAL] SECURITY INVOKER`. With `SECURITY DEFINER`, the privileges of the owner are checked when the UDF or SP is executed, rather than the privileges of the executor. The `EXTERNAL` keyword is optional and exists for SQL language conformity. [#129720](https://github.com/cockroachdb/cockroach/pull/129720)

### Operational changes

- The following new [metrics](/docs/v24.3/metrics) show details about [replication](https://www.cockroachlabs.com/docs/v24.2/architecture/replication-layer) flow control send queue when the [cluster setting](/docs/v24.3/cluster-settings) `kvadmission.flow_control.enabled` is set to `true` and the cluster setting `kvadmission.flow_control.mode` is set to `apply_to_all`.
...
- The following [metrics](/docs/v24.3/metrics) have been renamed:
...
- The `_status/ranges/` endpoint on DB Console [Advanced debug pages](/docs/v24.3/ui-debug-pages) is now enabled for non-system virtual clusters, where it returns the ranges only for the tenant you are logged into. For the system virtual cluster, the `_status/ranges/` endpoint continues to return ranges for the specified node across all virtual clusters. [#131100](https://github.com/cockroachdb/cockroach/pull/131100)

### DB Console changes

- Improved performance in the **Databases**, **Tables View**, and **Table Details** sections of the [**Databases page**](/docs/v24.3/ui-databases-page) [#131769](https://github.com/cockroachdb/cockroach/pull/131769)
...
- Fixed an error that could happen if an [aggregate function](/docs/v24.3/functions-and-operators#aggregate-functions) was used as the value in a `SET` command. [#131891](https://github.com/cockroachdb/cockroach/pull/131891)
- Fixed a rare bug introduced in v22.2 in which an update of a [primary key](/docs/v24.3/primary-key) column could fail to update the primary index if it is also the only column in a separate column family. [#131869](https://github.com/cockroachdb/cockroach/pull/131869)
- Fixed a rare bug where dropping a column of `FLOAT4`, `FLOAT8`, `DECIMAL`, `JSON`, `ARRAY`, or collate `STRING` type stored in a single [column family](/docs/v24.3/column-families) could prevent subsequent reading of the table if the column family was not the first column family. [#131967](https://github.com/cockroachdb/cockroach/pull/131967)
- Fixed an `unimplemented` internal error that could occur when ordering by a [`VECTOR`](/docs/v24.3/vector) column. [#131703](https://github.com/cockroachdb/cockroach/pull/131703)
...
- Error handling during periodic table history polling has been improved when the `schema_locked` [table parameter](/docs/v24.3/with-storage-parameter#table-parameters) is not used. [#131951](https://github.com/cockroachdb/cockroach/pull/131951)
...
- URLs in the [`CREATE CHANGEFEED`](/docs/v24.3/create-changefeed) and [`CREATE SCHEDULE FOR CHANGEFEED`](/docs/v24.3/create-schedule-for-changefeed) SQL statements are now sanitized of any secrets before being written to unredacted [logs](/docs/v24.3/logging). [#126970](https://github.com/cockroachdb/cockroach/pull/126970)
- The LDAP [cluster settings](/docs/v24.3/cluster-settings) `server.ldap_authentication.client.tls_certificate` and `server.ldap_authentication.client.tls_key` did not have callbacks installed to reload the settings value for LDAP authManager. This change fixes this by adding the necessary callbacks. [#131151](https://github.com/cockroachdb/cockroach/pull/131151)
- [Cluster settings](/docs/v24.3/cluster-settings) for [host-based authentication](/docs/v24.3/security-reference/authentication#authentication-configuration) configuration ( [`server.host_based_authentication.configuration`](/docs/v24.3/cluster-settings#setting-server-host-based-authentication-configuration) ) and identity map configuration ( [`server.identity_map.configuration`](/docs/v24.3/cluster-settings) ) need to be redacted as they can be configured to contain LDAP bind usernames, passwords, and mapping of external identities to SQL users that are sensitive. These cluster settings can be configured for redaction via the `server.redact_sensitive_settings.enabled` cluster setting. [#131150](https://github.com/cockroachdb/cockroach/pull/131150)
- Added support for configuring authorization using LDAP. During login, the list of groups that a user belongs to are fetched from the LDAP server. These groups are mapped to [SQL roles](/docs/v24.3/create-role) by extracting the common name (CN) from the group. After authenticating the user, the login flow grants these roles to the user, and revokes any other roles that are not returned by the LDAP server. The groups given by the LDAP server are treated as the sole source of truth for role memberships, so any roles that were manually granted to the user will not remain in place. [#131043](https://github.com/cockroachdb/cockroach/pull/131043)
- Previously, the [host-based authentication](/docs/v24.3/security-reference/authentication#authentication-configuration) (HBA) configuration cluster setting [`server.host_based_authentication.configuration`](/docs/v24.3/cluster-settings#setting-server-host-based-authentication-configuration) was unable to handle double quotes in authentication method option values. For example, for the following entry:
...
- CockroachDB will now avoid [logging](/docs/v24.3/logging) unnecessary stack traces while executing [scheduled jobs](/docs/v24.3/show-jobs). [#129846](https://github.com/cockroachdb/cockroach/pull/129846)
- Upgrading to 24.3 is blocked if no [license](/docs/v24.3/licensing-faqs) is installed, or if a trial/free license is installed with telemetry disabled. [#130576](https://github.com/cockroachdb/cockroach/pull/130576)
...
- Added a `CompressionLevel` field to the changefeed [`kafka_sink_config`](/docs/v24.3/changefeed-sinks#kafka-sink-configuration) option. [Changefeeds](/docs/v24.3/change-data-capture-overview) will use this compression level when emitting events to a [Kafka sink](/docs/v24.3/changefeed-sinks#kafka). The possible values depend on a compression codec. The `CompressionLevel` field optimizes for faster or stronger level of [compression](/docs/v24.3/changefeed-sinks#kafka-sink-configuration). [#125456](https://github.com/cockroachdb/cockroach/pull/125456)
- The updated version of the [CockroachDB changefeed Kafka sink implementation](/docs/v24.3/changefeed-sinks#kafka) now supports specifying compression levels. [#127827](https://github.com/cockroachdb/cockroach/pull/127827)
- Introduced the cluster setting [`server.jwt_authentication.client.timeout`](/docs/v24.3/cluster-settings) to capture the HTTP client timeout for external calls made during [JWT authentication](/docs/v24.3/sso-sql). [#127145](https://github.com/cockroachdb/cockroach/pull/127145)
- The JWT authentication [cluster settings](/docs/v24.3/cluster-settings) have been made `public`. [#128170](https://github.com/cockroachdb/cockroach/pull/128170)
...
- Added changefeed support for the [`mvcc_timestamp`](/docs/v24.3/create-changefeed#mvcc-timestamp) option when the changefeed is emitting in [`avro`](/docs/v24.3/changefeed-messages#avro) format. If both options are specified, the Avro schema includes an `mvcc_timestamp` metadata field and emits the row's [MVCC timestamp](/docs/v24.3/architecture/storage-layer#mvcc) with the row data. [#129840](https://github.com/cockroachdb/cockroach/pull/129840)
- Updated the cluster setting [`changefeed.sink_io_workers`](/docs/v24.3/cluster-settings#setting-changefeed-sink-io-workers) with all the [sinks](/docs/v24.3/changefeed-sinks) that support the setting. [#129946](https://github.com/cockroachdb/cockroach/pull/129946)
- Added a LDAP authentication method to complement password-based login for the [DB Console](/docs/v24.3/ui-overview) if HBA configuration has an entry for LDAP for the user attempting login, along with other matching criteria (like the requests originating IP address) for authentication to the DB Console. [#130418](https://github.com/cockroachdb/cockroach/pull/130418)
- Added timers around key parts of the [changefeed](/docs/v24.3/change-data-capture-overview) pipeline to help debug feeds experiencing issues. The `changefeed.stage.<stage>.latency` metrics now emit latency histograms for each stage. The metric respects the [changefeed `scope` label](/docs/v24.3/monitor-and-debug-changefeeds#using-changefeed-metrics-labels) for debugging specific feeds. [#128794](https://github.com/cockroachdb/cockroach/pull/128794)
- For [enterprise changefeeds](/docs/v24.3/how-does-a-changefeed-work), [events](/docs/v24.3/eventlog) `changefeed_failed` and `create_changefeed` now include a `JobId` field. [#131396](https://github.com/cockroachdb/cockroach/pull/131396)
- The new [metric](/docs/v24.3/metrics) `seconds_until_license_expiry` allows you to monitor the status of a cluster's Enterprise license. [#129052](https://github.com/cockroachdb/cockroach/pull/129052).
- Added the `changefeed.total_ranges` metric, which [monitors](/docs/v24.3/monitor-and-debug-changefeeds) the number of [ranges](/docs/v24.3/architecture/overview#architecture-range) that are watched by [changefeed aggregators](/docs/v24.3/how-does-a-changefeed-work). It shares the same polling interval as [`changefeed.lagging_ranges`](/docs/v24.3/advanced-changefeed-configuration#lagging-ranges), which is controlled by the existing `lagging_ranges_polling_interval` option. [#130897](https://github.com/cockroachdb/cockroach/pull/130897)

### SQL language changes

- Added a session setting, [`optimizer_use_merged_partial_statistics`](/docs/v24.3/session-variables) which defaults to `false`. When set to `true`, it enables usage of existing partial [statistics](/docs/v24.3/cost-based-optimizer#table-statistics) merged with full statistics when [optimizing](/docs/v24.3/cost-based-optimizer) a query. [#126948](https://github.com/cockroachdb/cockroach/pull/126948)
- The [`enable_create_stats_using_extremes`](/docs/v24.3/session-variables) session setting is now `true` by default. Partial statistics at extremes can be collected using the [`CREATE STATISTICS <stat_name> ON <column_name> FROM <table_name> USING EXTREMES`](/docs/v24.3/create-statistics) syntax. [#127850](https://github.com/cockroachdb/cockroach/pull/127850)
- Added [`SHOW SCHEMAS WITH COMMENT`](/docs/v24.3/show-schemas) and `SHOW SCHEMAS FROM database_name WITH COMMENT` functionality similar to [`SHOW TABLES`](/docs/v24.3/show-tables) and [`SHOW DATABASES`](/docs/v24.3/show-databases). [#127816](https://github.com/cockroachdb/cockroach/pull/127816)
- The [`deadlock_timeout` session variable](/docs/v24.3/session-variables) is now supported. The configuration can be used to specify the time to wait on a lock before pushing the lock holder for deadlock detection. It can be set at session granularity. [#128506](https://github.com/cockroachdb/cockroach/pull/128506)
- Partial statistics at extremes can now be collected on all valid columns of a table using the [`CREATE STATISTICS <stat_name>` `FROM <table_name> USING EXTREMES`](/docs/v24.3/create-statistics) syntax, without an `ON <col_name>` clause. Valid columns are all single column prefixes of a forward [index](/docs/v24.3/indexes) excluding partial, sharded, and implicitly partitioned indexes. [#127836](https://github.com/cockroachdb/cockroach/pull/127836)
- Partial [statistics](/docs/v24.3/cost-based-optimizer#table-statistics) can now be automatically collected at the extremes of indexes when a certain fraction and minimum number of rows are stale (by default 5% and 100 respectively). These can be configured with new [table storage parameters](/docs/v24.3/alter-table#set-and-reset-storage-parameters) and [cluster settings](/docs/v24.3/cluster-settings), and the feature is disabled by default. The new cluster settings and table parameters are:
...
- The session variable [`enforce_home_region_follower_reads_enabled`](/docs/v24.3/session-variables#enforce-home-region-follower-reads-enabled) is now deprecated, and will be removed in a future release. The related session variable [`enforce_home_region`](/docs/v24.3/session-variables#enforce-home-region) is **not** deprecated. [#129024](https://github.com/cockroachdb/cockroach/pull/129024)
- Added a new [cluster setting](/docs/v24.3/cluster-settings) to control whether most common values are collected as part of [histogram collection](/docs/v24.3/cost-based-optimizer#control-histogram-collection) for use by the [optimizer](/docs/v24.3/cost-based-optimizer). The setting is called `sql.stats.histogram_buckets.include_most_common_values.enabled`. When enabled, the histogram collection logic will ensure that the most common sampled values are represented as histogram bucket upper bounds. Since histograms in CockroachDB track the number of elements equal to the upper bound in addition to the number of elements less, this allows the optimizer to identify the most common values in the histogram and better estimate the rows processed by a query plan. To set the number of most common values to include in a histogram, a second setting `sql.stats.histogram_buckets.max_fraction_most_common_values` was added. Currently, the default is `0.1`, or `10%` of the number of buckets. With a 200 bucket histogram, by default, at most 20 buckets may be adjusted to include a most common value as the upper bound. [#129378](https://github.com/cockroachdb/cockroach/pull/129378)
- Added a new column to [`crdb_internal.table_spans`](/docs/v24.3/crdb-internal#tables) to indicate whether a table is [dropped](/docs/v24.3/drop-table). Rows for dropped tables will be removed once they are [garbage collected](/docs/v24.3/architecture/storage-layer#garbage-collection). [#128788](https://github.com/cockroachdb/cockroach/pull/128788)
- Added the [cluster setting](/docs/v24.3/cluster-settings) `sql.txn.repeatable_read_isolation.enabled`, which defaults to`false`. When set to `true`, the following statements will configure transactions to run under `REPEATABLE READ` isolation, rather than being automatically interpreted as [`SERIALIZABLE`](/docs/v24.3/demo-serializable):
    - `BEGIN TRANSACTION ISOLATION LEVEL REPEATABLE READ`
    - `SET TRANSACTION ISOLATION LEVEL REPEATABLE READ`
    - `SET default_transaction_isolation = 'repeatable read'`
    - `SET SESSION CHARACTERISTICS AS TRANSACTION ISOLATION LEVEL REPEATABLE READ`

        This setting was added since [`REPEATABLE READ` transactions](/docs/v24.3/transactions#comparison-to-ansi-sql-isolation-levels) is a [preview](/docs/v24.3/cockroachdb-feature-availability) feature, so usage of it is opt-in for v24.3. In a future CockroachDB major version, this setting will change to default to `true`. [#130089](https://github.com/cockroachdb/cockroach/pull/130089)
- Previously, [`SHOW CHANGEFEED JOBS`](/docs/v24.3/show-jobs#show-changefeed-jobs) showed the changefeed jobs for the last 14 days by default. Now, it uses the same age filter for [`SHOW JOBS`](/docs/v24.3/show-jobs), which shows jobs from the last 12 hours by default. [#127584](https://github.com/cockroachdb/cockroach/pull/127584)
- Set the default for session variable [`large_full_scan_rows`](/docs/v24.3/session-variables#large-full-scan-rows) to `0`. This means that by default, [`disallow_full_table_scans`](/docs/v24.3/session-variables#disallow-full-table-scans) will disallow **all** [full table scans](/docs/v24.3/show-full-table-scans), even full scans on very small tables. If `large_full_scan_rows` is set &gt; 0, `disallow_full_table_scans` will allow full scans estimated to read fewer than `large_full_scan_rows`. [#131040](https://github.com/cockroachdb/cockroach/pull/131040)
- It is now possible to create [PL/pgSQL](/docs/v24.3/plpgsql) trigger functions, which can be executed by a trigger in response to table mutation events. Note that this patch does not add support for triggers, only trigger functions. [#126734](https://github.com/cockroachdb/cockroach/pull/126734)
- Cluster settings [`enterprise.license`](/docs/v24.3/cluster-settings#setting-enterprise-license) and [`diagnostics.reporting.enabled`](/docs/v24.3/cluster-settings#setting-diagnostics-reporting-enabled) now have additional validation. [#131097](https://github.com/cockroachdb/cockroach/pull/131097)
- The [`SHOW SESSIONS`](/docs/v24.3/show-sessions) command was changed to include an `authentication_method` column in the result. This column will show the method used to authenticate the session, for example, `password`, `cert`, `LDAP`, etc. [#131625](https://github.com/cockroachdb/cockroach/pull/131625)

### Operational changes

- [Events](/docs/v24.3/eventlog) `DiskSlownessDetected` and `DiskSlownessCleared` are now logged when disk slowness is detected and cleared on a store. [#127025](https://github.com/cockroachdb/cockroach/pull/127025)
- Several [cluster settings](/docs/v24.3/cluster-settings) allow you to configure rate-limiting traffic to cloud storage over various protocols. These settings begin with `cloudstorage`. [#127207](https://github.com/cockroachdb/cockroach/pull/127207)
- The new [cluster setting](/docs/v24.3/cluster-settings) `kv.range.range_size_hard_cap` allows you to limit how large a [range](/docs/v24.3/architecture/overview#architecture-range) can grow before [backpressure](/docs/v24.3/common-errors#split-failed-while-applying-backpressure-are-rows-updated-in-a-tight-loop) is applied. This can help to mitigate against a situation where a range cannot be split, such as when a range is comprised of a single key due to an issue with the schema or workload pattern or a bug in client application code. The default is 8 GiB, which is 16 times the default max range size. If you have changed the max range size, you may need to adjust this cluster setting or reduce the range size. [#129450](https://github.com/cockroachdb/cockroach/pull/129450)
- The following `kvflowcontrol` [metrics](/docs/v24.3/metrics) have been renamed. After a cluster is finalized on v24.3, old and new metrics will be populated. The previous metrics under `kvasdmission.flow_controller` will be removed.
...
- The new `ranges.decommissioning` [metric](/docs/v24.3/metrics) shows the number of ranges with a replica on a [decommissioning](/docs/v24.3/node-shutdown) node. [#130117](https://github.com/cockroachdb/cockroach/pull/130117)
- New [cluster settings](/docs/v24.3/cluster-settings) have been added which control the refresh behavior for the cached data in the **Databases** page of the [DB Console](/docs/v24.3/ui-overview):
    - `obs.tablemetadatacache.data_valid_duration`: the duration for which the data in `system.table_metadata` is considered valid before a cache reset will occur. Default: 20 minutes.
    - `obs.tablemetadatacache.automatic_updates.enabled`: whether to automatically update the cache according the validity interval. Default: `false`.

        [#130198](https://github.com/cockroachdb/cockroach/pull/130198)
- New gauge [metrics](/docs/v24.3/metrics) `security.certificate.expiration.{cert-type}` and `security.certificate.ttl.{cert-type}` show the expiration and TTL for a certificate. [#130110](https://github.com/cockroachdb/cockroach/pull/130110)
- To set the [logging format](/docs/v24.3/log-formats) for `stderr`, you can now set the `format` field to any valid format, rather than only `crdb-v2-tty`. [#131529](https://github.com/cockroachdb/cockroach/pull/131529)
- The following new [metrics](/docs/v24.3/metrics) show connection latency for each SQL authentication method:
...
- Verbose logging of slow [Pebble](/docs/v24.3/architecture/storage-layer#pebble) reads can no longer be enabled via the shorthand flag `--vmodule=pebble_logger_and_tracer=2`, where `pebble_logger_and_tracer` contains the CockroachDB implementation of the logger needed by Pebble. Instead, you must list the Pebble files that contain the log statements. For example `--vmodule=reader=2,table=2`. [#127066](https://github.com/cockroachdb/cockroach/pull/127066)
- The lowest [admission control](/docs/v24.3/admission-control) priority for the storage layer has been renamed from `ttl-low-pri` to `bulk-low-pri`. [#129564](https://github.com/cockroachdb/cockroach/pull/129564)
- New clusters will now have a [zone configuration](/docs/v24.3/show-zone-configurations) defined for the `timeseries` range, which specifies `gc.ttlseconds` and inherits all other attributes from the zone config of the `default` range. This zone config will also be added to a cluster that is [upgraded](/docs/v24.3/upgrade-cockroach-version) to v24.3 if it does not already have a zone config defined.[#128032](https://github.com/cockroachdb/cockroach/pull/128032)

### Command-line changes

- [`cockroach debug tsdump`](/docs/v24.3/cockroach-debug-tsdump) now includes all the available resolutions in the time range supplied by the user. [#127186](https://github.com/cockroachdb/cockroach/pull/127186)
- Added the flag `--tenant-name-scope` to the [`cert create-client`](/docs/v24.3/cockroach-cert#create-the-certificate-and-key-pair-for-a-client) command. This allows users to generate tenant-scoped [client certificates](/docs/v24.3/authentication#client-authentication) using tenant names in addition to tenant IDs. [#129216](https://github.com/cockroachdb/cockroach/pull/129216)

### DB Console changes

- If a [range](/docs/v24.3/architecture/overview#architecture-range) is larger than twice the max range size, it will now display in the [**Problem Ranges** page](/docs/v24.3/ui-debug-pages) in the DB Console. [#129001](https://github.com/cockroachdb/cockroach/pull/129001)
- Updated some metric charts on the [Overview](/docs/v24.3/ui-overview-dashboard) and [Replication](/docs/v24.3/ui-replication-dashboard) dashboards to omit verbose details in the legends for easier browsing. [#129149](https://github.com/cockroachdb/cockroach/pull/129149)
...
- The grants table in the [DB Details](/docs/v24.3/ui-databases-page#databases) page will now show the database level grants. For example, when clicking a database in the databases list. Previously, it showed grants per table in the database. [#131250](https://github.com/cockroachdb/cockroach/pull/131250)
...
- The [DB Console](/docs/v24.3/ui-overview) will reflect any throttling behavior from the cluster due to an expired license or missing telemetry data. Enterprise licenses are not affected. [#131326](https://github.com/cockroachdb/cockroach/pull/131326)
...
- The [**Databases** pages](/docs/v24.3/ui-databases-page) in the DB console have been updated to read cached metadata about database and table storage statistics. The cache update time is now displayed in the top right-hand corner of the database and tables list pages. Users may trigger a cache refresh with the **refresh** icon next to the last updated time. The cache will also update automatically when users visit a **Databases** page and the cache is older than or equal to 20 minutes. [#131463](https://github.com/cockroachdb/cockroach/pull/131463)

### Bug fixes

- Fixed a bug where CockroachDB could incorrectly evaluate an `IS NOT NULL` [filter](/docs/v24.3/select-clause#filter-rows) if it was applied to non- `NULL` tuples that had `NULL` elements (like `(1, NULL)` or `(NULL, NULL)` ). The bug was present since v20.2. [#126901](https://github.com/cockroachdb/cockroach/pull/126901)
- Fixed a bug related to displaying the names of composite types in the [`SHOW CREATE TABLES`](/docs/v24.3/show-create) command. The names are now shown as two-part names, which disambiguates the output and makes it more portable to other databases. [#127158](https://github.com/cockroachdb/cockroach/pull/127158)
- The `CONCAT()` [built-in function](/docs/v24.3/functions-and-operators) now accepts arguments of any data type. [#127098](https://github.com/cockroachdb/cockroach/pull/127098)
- Fixed a bug that prevented merged [statistics](/docs/v24.3/cost-based-optimizer#table-statistics) from being created after injecting statistics or recreating [statement bundles](/docs/v24.3/explain-analyze). This would occur when the injected statistics or statement bundle contained related full and partial statistics. [#127252](https://github.com/cockroachdb/cockroach/pull/127252)
- Fixed a bug where CockroachDB could encounter spurious `(error encountered after some results were delivered)` `ERROR: context canceled` errors in rare cases when evaluating some queries. The bug was present since v22.2. The conditions that triggered the bug were queries that:
    ...
    - Had a [`LIMIT`](/docs/v24.3/limit-offset).
    - Have at least two [`UNION`](/docs/v24.3/selection-queries#union-combine-two-queries) clauses.
    - Have some lookup or index [joins](/docs/v24.3/joins) in the `UNION` branches. [#127076](https://github.com/cockroachdb/cockroach/pull/127076)
- Updated the restore [job](/docs/v24.3/show-jobs) description from `RESTORE... FROM` to `RESTORE FROM {backup} IN {collectionURI}` to reflect the new [`RESTORE`](/docs/v24.3/restore) syntax. [#127970](https://github.com/cockroachdb/cockroach/pull/127970)
- Fixed a bug that could cause a `CASE` statement with multiple [subqueries](/docs/v24.3/subqueries) to produces the side effects of one of the subqueries even if that subquery shouldn't have been evaluated. [#120327](https://github.com/cockroachdb/cockroach/pull/120327)
- Changed the [schema changer](/docs/v24.3/online-schema-changes) ’s merge process so that it can detect [contention errors](/docs/v24.3/transaction-retry-error-reference) and automatically retry with a smaller batch size. This makes the merge process more likely to succeed without needing to manually tune settings. [#128201](https://github.com/cockroachdb/cockroach/pull/128201)
- [`SHOW CREATE ALL TYPES`](/docs/v24.3/show-create) now shows corresponding type comments in its output. [#128084](https://github.com/cockroachdb/cockroach/pull/128084)
- Enforce the [`statement_timeout` session setting](/docs/v24.3/session-variables) when waiting for [jobs](/docs/v24.3/show-jobs) after a [schema change](/docs/v24.3/online-schema-changes) in an [implicit transaction](/docs/v24.3/transactions#individual-statements). [#128474](https://github.com/cockroachdb/cockroach/pull/128474)
- Fixed a bug where certain dropdowns in the [DB Console](/docs/v24.3/ui-overview) appeared to be empty (with no options to select from) for users of the Safari browser. [#128996](https://github.com/cockroachdb/cockroach/pull/128996)
- Fixed a bug that would cause the [`hlc_to_timestamp` function](/docs/v24.3/functions-and-operators) to return an incorrect [timestamp](/docs/v24.3/timestamp) for some input [decimals](/docs/v24.3/decimal). [#129153](https://github.com/cockroachdb/cockroach/pull/129153)
- Fixed a memory leak where [statement insight](/docs/v24.3/ui-statements-page) objects could leak if the session was closed without the [transaction](/docs/v24.3/transactions) finishing. [#128400](https://github.com/cockroachdb/cockroach/pull/128400)
- Fixed a bug in the public preview [WAL failover](/docs/v24.3/cockroach-start#write-ahead-log-wal-failover) feature that could prevent a node from starting if it crashed during a failover. [#129331](https://github.com/cockroachdb/cockroach/pull/129331)
...
- Fixed a spurious error log from the [replication queue](/docs/v24.3/ui-queues-dashboard#replication-queue) involving the text `" needs lease, not adding"`. [#129351](https://github.com/cockroachdb/cockroach/pull/129351)
- Using more than one [`DECLARE`](/docs/v24.3/plpgsql#structure) statement in the definition of a [user-defined function](/docs/v24.3/user-defined-functions) now correctly declares additional variables. [#129951](https://github.com/cockroachdb/cockroach/pull/129951)
- Fixed a bug in which some [`SELECT FOR UPDATE`](/docs/v24.3/select-for-update) or [`SELECT FOR SHARE`](/docs/v24.3/select-for-update) queries using `NOWAIT` could still block on locked rows when using the `optimizer_use_lock_op_for_serializable` [session setting](/docs/v24.3/session-variables) under [serializable](/docs/v24.3/demo-serializable) isolation. This bug was introduced with `optimizer_use_lock_op_for_serializable` in v23.2.0. [#130103](https://github.com/cockroachdb/cockroach/pull/130103)
- Fixed a bug in the [upgrade](/docs/v24.3/upgrade-cockroach-version) pre-condition for repairing descriptor corruption that could lead to finalization being stuck. [#130064](https://github.com/cockroachdb/cockroach/pull/130064)
- Fixed a bug that caused the optimizer to plan unnecessary post-query uniqueness checks during [`INSERT`](/docs/v24.3/insert), [`UPSERT`](/docs/v24.3/upsert), and [`UPDATE`](/docs/v24.3/update) statements on tables with partial, unique, [hash-sharded indexes](/docs/v24.3/hash-sharded-indexes). These unnecessary checks added overhead to execution of these statements, and caused the statements to error when executed under [`READ COMMITTED` isolation](/docs/v24.3/read-committed). [#130366](https://github.com/cockroachdb/cockroach/pull/130366)
- Fixed a bug that caused incorrect evaluation of `CASE`, [`COALESCE`](/docs/v24.3/functions-and-operators#conditional-and-function-like-operators), and [`IF`](/docs/v24.3/functions-and-operators#conditional-and-function-like-operators) expressions with branches producing fixed-width string-like types, such as [`CHAR`](/docs/v24.3/string). In addition, the `BPCHAR` type no longer incorrectly imposes a length limit of `1`. [#129007](https://github.com/cockroachdb/cockroach/pull/129007)
- Fixed a bug where [zone configuration](/docs/v24.3/configure-replication-zones) changes issued by the [declarative schema changer](/docs/v24.3/online-schema-changes) were not blocked if a table had the `schema_locked` [storage parameter](/docs/v24.3/with-storage-parameter) set. [#130670](https://github.com/cockroachdb/cockroach/pull/130670)
- Fixed a bug that could prevent a [`CHANGEFEED`](/docs/v24.3/changefeed-examples) from being able to resume after being paused for a prolonged period of time. [#130622](https://github.com/cockroachdb/cockroach/pull/130622)
- Fixed a bug where if a client connection was attempting a [schema change](/docs/v24.3/online-schema-changes) while the same [schema objects](/docs/v24.3/schema-design-overview) were being dropped, it was possible for the connection to be incorrectly dropped. [#130928](https://github.com/cockroachdb/cockroach/pull/130928)
- Fixed a bug introduced in v23.1 that could cause incorrect results when:
    1. The query contained a [correlated subquery](/docs/v24.3/subqueries#correlated-subqueries).
    2. The correlated subquery had a [`GROUP BY`](/docs/v24.3/select-clause#group-by-an-alias) or [`DISTINCT`](/docs/v24.3/performance-best-practices-overview#avoid-select-distinct-for-large-tables) operator with an outer-column reference in its input.
    3. The correlated subquery was in the input of a [`SELECT`](/docs/v24.3/select-clause) or [`JOIN`](/docs/v24.3/joins) operator.
...
- Fixed a bug which could cause errors with the message `"internal error: Non-nullable column..."` when executing statements under [`READ COMMITTED`](/docs/v24.3/read-committed) isolation that involved tables with `NOT NULL` [virtual columns](/docs/v24.3/computed-columns#create-a-virtual-computed-column-using-jsonb-data). [#130725](https://github.com/cockroachdb/cockroach/pull/130725)
...
- Fixed a bug that could cause incorrect evaluation of [scalar expressions](/docs/v24.3/scalar-expressions) involving `NULL` values in rare cases. [#128123](https://github.com/cockroachdb/cockroach/pull/128123)
- [`SHOW CREATE ALL SCHEMAS`](/docs/v24.3/show-create) now shows corresponding schema comments in its output. [#130164](https://github.com/cockroachdb/cockroach/pull/130164)
- Fixed a bug, introduced in v23.2.0, where creating a new [incremental schedule](/docs/v24.3/take-full-and-incremental-backups#incremental-backups) (using [`ALTER BACKUP SCHEDULE`](/docs/v24.3/alter-backup-schedule) ) on a [full backup schedule](/docs/v24.3/take-full-and-incremental-backups#full-backups) created on an older version would fail. [#131231](https://github.com/cockroachdb/cockroach/pull/131231)
- Fixed a bug that could cause an internal error if a table with an implicit ( `rowid` ) [primary key](/docs/v24.3/primary-key) was locked from within a [subquery](/docs/v24.3/subqueries) like `SELECT * FROM (SELECT * FROM foo WHERE x = 2) FOR UPDATE;`. The error could occur either under [`READ COMMITTED`](/docs/v24.3/read-committed) isolation, or with the `optimizer_use_lock_op_for_serializable` [session setting](/docs/v24.3/session-variables) enabled. [#129768](https://github.com/cockroachdb/cockroach/pull/129768)
- Fixed a bug where [jobs](/docs/v24.3/show-jobs) created in a session with non-zero session [timezone offsets](/docs/v24.3/set-vars#set-time-zone) could hang before starting, or report incorrect creation times when viewed in [`SHOW JOBS`](/docs/v24.3/show-jobs) and the [DB Console](/docs/v24.3/ui-overview). [#123632](https://github.com/cockroachdb/cockroach/pull/123632)
- Fixed a bug which could result in [changefeeds using CDC queries](/docs/v24.3/create-schedule-for-changefeed#create-a-scheduled-changefeed-with-cdc-queries) failing due to a system table being [garbage collected](/docs/v24.3/architecture/storage-layer#garbage-collection). [#131027](https://github.com/cockroachdb/cockroach/pull/131027)
- [`ALTER COLUMN TYPE`](/docs/v24.3/alter-table#convert-to-a-different-data-type) now errors out when there is a [partial index](/docs/v24.3/partial-indexes) that is dependent on the column being altered. [#131590](https://github.com/cockroachdb/cockroach/pull/131590)
...
- [Raft log](/docs/v24.3/architecture/replication-layer#raft-logs) sync callback handling is now parallelized, which can improve write-heavy workload performance on large, single-store nodes. [#126523](https://github.com/cockroachdb/cockroach/pull/126523)
- [Planning time](/docs/v24.3/cost-based-optimizer) for complex queries has been reduced. [#128049](https://github.com/cockroachdb/cockroach/pull/128049)
- Reduced the [write-amplification](/docs/v24.3/architecture/storage-layer#write-amplification) impact of [rebalances](/docs/v24.3/architecture/replication-layer) by splitting snapshot SSTable files into smaller ones before ingesting them into [Pebble](/docs/v24.3/architecture/storage-layer#pebble). [#127997](https://github.com/cockroachdb/cockroach/pull/127997)
- Improved the performance of [job-system](/docs/v24.3/show-jobs) related queries. [#123848](https://github.com/cockroachdb/cockroach/pull/123848)
- The [query optimizer](/docs/v24.3/cost-based-optimizer) now plans limited, [partial-index scans](/docs/v24.3/partial-indexes) in more cases. [#129901](https://github.com/cockroachdb/cockroach/pull/129901)
- The initialization of the execution engine for a query is now more efficient when the [query plan](/docs/v24.3/cost-based-optimizer) contains [aggregate functions](/docs/v24.3/functions-and-operators#aggregate-functions). [#130834](https://github.com/cockroachdb/cockroach/pull/130834)
- Enabled multi-level [compactions](/docs/v24.3/architecture/storage-layer#compaction) that moderately reduce [write amplification](/docs/v24.3/architecture/storage-layer#write-amplification) within the [storage engine](/docs/v24.3/architecture/storage-layer). [#131378](https://github.com/cockroachdb/cockroach/pull/131378)
- Increased the per-vCPU concurrency limits for KV operations. Specifically, increased the `kv.dist_sender.concurrency_limit` [cluster setting](/docs/v24.3/cluster-settings) to 384/vCPU (up from 64/vCPU) and `kv.streamer.concurrency_limit` to 96/vCPU (up from 8/vCPU). [#131226](https://github.com/cockroachdb/cockroach/pull/131226)
- The [optimizer](/docs/v24.3/cost-based-optimizer) now plans more efficient [lookup joins](/docs/v24.3/joins#lookup-joins) in some cases. [#131383](https://github.com/cockroachdb/cockroach/pull/131383)
