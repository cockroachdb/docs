...
old_description: "If a CREATE MATERIALIZED VIEW statement fails, all objects referenced in its SELECT query will be unusable."
---
...
The problem can occur on any cluster running versions [v21.2.0](/docs/releases/v21.2#v21-2-0) to [v21.2.12](/docs/releases/v21.2#v21-2-12) or [v22.1.0](/docs/releases/v22.1#v22-1-0). If a [`CREATE MATERIALIZED VIEW`](https://www.cockroachlabs.com/docs/v22.1/views#materialized-views) statement fails, all the tables, views, sequences and types referenced in its [`SELECT`](https://www.cockroachlabs.com/docs/v22.1/selection-queries) query will become unusable. Any statement or query referencing these objects will trigger an error matching the following regular expression:
...
Users of CockroachDB versions v21.2.0 to v21.2.12 or v22.1.0 are encouraged to upgrade to [v21.2.13](/docs/releases/v21.2#v21-2-13) and [v22.1.1](/docs/releases/v22.1#v22-1-1) or a later version. Upgrading to a later maintenance release will prevent this problem from appearing, but it will not fix the problem if it is already present.
