...
old_description: "In rare cases, some bulk write operations could succeed without ingesting all expected data."
---
...
Other bulk write operations are not affected. Restore is unaffected because it never uses the rare code path affected by this bug. Index backfill [schema changes](https://www.cockroachlabs.com/docs/v25.1/online-schema-changes) avoid this problem because of post-ingestion index validation, which fails the index creation if it finds any missing data.
...
Users on affected versions can determine whether they may have encountered the problem by running the script [`detect_144650.sh`](/docs/advisories/detect_144650.sh) to detect any bulk ingestion jobs that improperly succeeded. If the script detects a potential issue, users should [contact support](https://support.cockroachlabs.com/) and work through the appropriate [manual validation](#manual-validation) steps.

For details, refer to [Detection via logs](#detection-via-logs) and [Manual validation](#manual-validation).

### Detection via logs

[`detect_144650.sh`](/docs/advisories/detect_144650.sh) requires the following dependencies:

- [ripgrep](https://github.com/BurntSushi/ripgrep#installation)
- [DuckDB](https://duckdb.org/docs/installation)

Run `detect_144650.sh` from within a decompressed [`debug.zip`](https://www.cockroachlabs.com/docs/v25.1/cockroach-debug-zip) directory or by appending the path to the directory. For example:
...
1. If the output of the [`IMPORT` statement](https://www.cockroachlabs.com/docs/v25.1/import-into) showed the expected number of rows, then the import of the primary index was unaffected, but secondary indexes may be corrupted. Cockroach Labs suggests [dropping](https://www.cockroachlabs.com/docs/v25.1/drop-index) and [recreating](https://www.cockroachlabs.com/docs/v25.1/create-index) secondary indexes.
2. After a successful `IMPORT` job, there should be a [`RecoveryEvent`](https://www.cockroachlabs.com/docs/v25.1/eventlog#recovery_event) logged to the [`TELEMETRY` logging channel](https://www.cockroachlabs.com/docs/v25.1/logging#telemetry) with `"RecoveryType": "import_job"`. If `NumRows` from this event shows the expected number of rows, then the import of the primary index was unaffected, but secondary indexes may be corrupted. Cockroach Labs suggests [dropping](https://www.cockroachlabs.com/docs/v25.1/drop-index) and [recreating](https://www.cockroachlabs.com/docs/v25.1/create-index) secondary indexes.
3. If the preceding two steps do not show the expected number of rows, or cannot be performed due to the time elapsed since the `IMPORT` job, then use [`RESTORE... AS OF SYSTEM TIME`](https://www.cockroachlabs.com/docs/v25.1/restore#restore-with-as-of-system-time) to reconstruct the table as it was immediately after the import finished. Run a `SELECT count(*)` query to count the number of rows in the restored table. (If the table contained any rows before the `IMPORT` statement was run, then use a second `RESTORE... AS OF SYSTEM TIME` to reconstruct the table as it was immediately before the `IMPORT` started, and subtract the `SELECT count(*)` on this table from the first count.)
    - If this count shows the expected number of rows, then the import of the primary index was unaffected, but secondary indexes may be corrupted. As in the preceding cases, Cockroach Labs suggests [dropping](https://www.cockroachlabs.com/docs/v25.1/drop-index) and [recreating](https://www.cockroachlabs.com/docs/v25.1/create-index) secondary indexes.
    - If this count does not show the expected number of rows, this indicates a discrepancy affecting the primary index. After [upgrading to a version with the fix](#mitigation), run the `IMPORT` a second time. Compare the two imported datasets with a [`JOIN`](https://www.cockroachlabs.com/docs/v25.1/joins), and manually add any missing rows.

#### MOLT Fetch

By default, MOLT Fetch uses [`IMPORT INTO`](https://www.cockroachlabs.com/docs/v25.1/import-into) to load data into CockroachDB, and can therefore be affected by this issue. [As recommended in the migration documentation](/docs/molt/migrate-load-replicate#stop-replication-and-verify-data), a run of [MOLT Fetch](/docs/molt/molt-fetch) should be followed by a run of [MOLT Verify](/docs/molt/molt-verify) to ensure that all data on the target side matches the data on the source side.
...
- If you did not run Verify after Fetch, analyze the exported files that exist in your configured [Fetch data path](/docs/molt/molt-fetch#data-path) to determine the expected number of rows. Then follow steps 1-3 in the [`IMPORT`](#import) section.

#### Physical Cluster Replication

After upgrading both the primary and standby clusters to a [version with the fix](#mitigation), run a [point-in-time fingerprint on both clusters](https://www.cockroachlabs.com/docs/v25.1/physical-cluster-replication-monitoring#data-verification) to verify that the data on the primary and standby clusters are the same at a certain [replicated time](https://www.cockroachlabs.com/docs/v25.1/physical-cluster-replication-technical-overview#failover-and-promotion-process).

If the fingerprints do not have the same value, then [upgrade to a version with the fix](#mitigation), cancel your PCR stream by [dropping the replicating virtual cluster](https://www.cockroachlabs.com/docs/v25.1/drop-virtual-cluster), and [restart the PCR stream](https://www.cockroachlabs.com/docs/v25.1/set-up-physical-cluster-replication). Cockroach Labs encourages users to [contact support](https://support.cockroachlabs.com/) to assist with these steps if a fingerprint mismatch is found.

#### `CREATE TABLE... AS` (CTAS)

[`CREATE TABLE... AS`](https://www.cockroachlabs.com/docs/v25.1/create-table-as) (CTAS) operations can be validated similarly to [`IMPORT`](#import) jobs. The number of rows in the created table indicates whether the CTAS operation was successful.

To check whether a CTAS operation was affected, compare the number of rows in the created table with the output of the CTAS query. If you cannot perform this comparison due to the time elapsed since the CTAS operation, then either re-run the CTAS query with [`AS OF SYSTEM TIME`](https://www.cockroachlabs.com/docs/v25.1/as-of-system-time) or use [`RESTORE... AS OF SYSTEM TIME`](https://www.cockroachlabs.com/docs/v25.1/restore#restore-with-as-of-system-time) to reconstruct the table as it was immediately after the CTAS operation finished.

If you find a discrepancy, then [upgrade to a version with the fix](#mitigation), [drop the affected table](https://www.cockroachlabs.com/docs/v25.1/drop-table), and re-run the CTAS operation.

#### Materialized view refreshes

[Materialized view refreshes](https://www.cockroachlabs.com/docs/v25.1/refresh) can be validated similarly to [CTAS operations](#create-table-as-ctas). To check whether a `REFRESH` was affected, compare the number of rows in the materialized view with the source data.

If you find a discrepancy, then [upgrade to a version with the fix](#mitigation) and [refresh the materialized view](https://www.cockroachlabs.com/docs/v25.1/refresh) to ensure it has the most up-to-date data.
