...
old_description: "A query with ORDER BY and LIMIT clauses could return incorrect results if it scanned a multi-column index containing the ORDER BY columns, and a prefix of th..."
---
...
In CockroachDB [v22.1.0](/docs/releases/v22.1#v22-1-0) to [v22.1.8](/docs/releases/v22.1#v22-1-8), a query with `ORDER BY` and `LIMIT` clauses could return incorrect results if it scanned a multi-column index containing the `ORDER BY` columns, and a prefix of the index columns was held fixed to two or more constant values by the query filter or schema. Examples of schema elements that could constrain the index columns include:
...
The fix has been applied to the maintenance release of CockroachDB [v22.1.9](/docs/releases/v22.1#v22-1-9).

This public issue is tracked by [#88993](https://github.com/cockroachdb/cockroach/pull/88993).

## Mitigation

Users of CockroachDB [v22.1.0](/docs/releases/v22.1#v22-1-0) to [v22.1.8](/docs/releases/v22.1#v22-1-8) are encouraged to upgrade to [v22.1.9](/docs/releases/v22.1#v22-1-9) or a later version. To determine whether your queries may be affected by this issue on v22.1.0 to v22.1.8, examine the query plans of any queries with an `ORDER BY` and `LIMIT` clause or aggregation with min or max by using an `EXPLAIN (OPT)` statement. In particular, you should examine plans for queries that have a multi-column index containing the `ORDER BY`, `min`, or `max` columns and for which a prefix of the index columns are constrained to two or more constant values by one of the following:
