...
old_description: "Non-admin SQL users with an authenticated HTTP session could download statement diagnostic bundles given a bundle URL from the DB Console or SQL shell with a..."
---
...
A potential data leak/privilege escalation of statement diagnostic bundles by non-admin users was discovered. We expose the ability to download a statement diagnostic bundle over HTTP via a URL provided either by the [DB Console](https://www.cockroachlabs.com/docs/v22.2/ui-statements-page#activate-diagnostics-collection-and-download-bundles) or the [SQL shell](https://www.cockroachlabs.com/docs/v22.2/explain-analyze#debug-option). This URL is accessible to any logged-in user, but should be accessible only to [admin users](https://www.cockroachlabs.com/docs/v22.2/security-reference/authorization#admin-role). This bundle contains unredacted query execution data including query text and the values of variables used within.
