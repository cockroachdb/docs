...
old_description: "Queries reading a table that uses at least one explicit column family in its definition and was restored from a backup using version v22.2.7 to v22.2.13 or v..."
---
...
Queries reading a table that uses at least one explicit column family in its definition and was [restored](/docs/stable/backup-and-restore-overview) from a backup using the following versions may return errors or incorrect results:
