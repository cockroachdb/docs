...
old_description: "A race condition between MVCC garbage collection and range splits in CockroachDB can, under specific and unlikely circumstances, result in the deletion of li..."
---
...
- **Time-series with late-arriving data**: Tables keyed by timestamp or (entity_id, timestamp) where old data is aged out and new data occasionally backfills into older timestamp ranges. The backfilled data must arrive after the GC threshold (4 hours by default).
...
On older versions, secondary indexes can be used to confirm the presence of a suspected lost write. For example, if you suspect that a row in my_table with primary key pk=1 and secondary index key a=2 may have been affected:
...
<Frame>
  ![MVCC versions diagram](/images/common/a162085-mvcc-diagram.png)
</Frame>
