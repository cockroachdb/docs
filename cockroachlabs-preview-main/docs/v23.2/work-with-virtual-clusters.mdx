...
<InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v23.2: Enable [cluster virtualization](/docs/v23.2/cluster-virtualization-overview) in your CockroachDB cluster to set up a [**physical cluster replication (PCR)**](/docs/v23.2/physical-cluster-replication-overview) stream. This page is a guide to working with virtual clusters.

## Connect to a virtual cluster

This section shows how to use [SQL clients](#sql-clients) or the [DB Console](#db-console) to connect to a virtual cluster.

When [PCR](/docs/v23.2/physical-cluster-replication-overview) is enabled, the virtual cluster is named `application`.

### SQL clients

This section shows how to connect using [`cockroach sql`](/docs/v23.2/cockroach-sql) when cluster virtualization is enabled.

Unless you specify which virtual cluster to connect to, when you connect using a SQL client, you are logged into the default virtual cluster. When [PCR](/docs/v23.2/physical-cluster-replication-overview) is enabled, the default virtual cluster is named `application`.
...
This section shows how to connect using the [DB Console](/docs/v23.2/ui-overview) when cluster virtualization is enabled.

Unless you specify which virtual cluster to connect to, when you connect using the DB Console, you are logged into the default virtual cluster. When [PCR](/docs/v23.2/physical-cluster-replication-overview) is enabled, the default virtual cluster is named `application`.
...
To [grant](/docs/v23.2/grant) access to the system virtual cluster, you must connect to the system virtual cluster as a user with the `admin` role, then grant either of the following to the SQL user:

- The `admin` [role](/docs/v23.2/security-reference/authorization#admin-role) grants the ability to read and modify system tables and cluster settings on any virtual cluster, including the system virtual cluster.
- The `VIEWSYSTEMDATA` [system privilege](/docs/v23.2/security-reference/authorization#supported-privileges) grants the ability to read system tables and cluster settings on any virtual cluster, including the system virtual cluster.
...
When [cluster virtualization](/docs/v23.2/cluster-virtualization-overview) is enabled, each [cluster setting](/docs/v23.2/cluster-settings) is scoped to either a virtual cluster or the system virtual cluster.
