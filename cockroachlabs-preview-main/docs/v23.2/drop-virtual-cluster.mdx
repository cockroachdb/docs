...
<InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v23.2: The `DROP VIRTUAL CLUSTER` statement removes virtual clusters in order to restart a **physical cluster replication (PCR)** stream. Virtual clusters are used only as part of the [PCR](/docs/v23.2/physical-cluster-replication-overview) workflow.
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>IF EXISTS</code></td><td>Drop a virtual cluster if it exists. If it does not exist, do not return an error.</td></tr><tr><td><code>virtual&#95;cluster&#95;spec</code></td><td>The name of the virtual cluster.</td></tr><tr><td><code>IMMEDIATE</code></td><td>Drop a virtual cluster immediately instead of waiting for garbage collection (<a href="/docs/v23.2/configure-replication-zones#gc-ttlseconds">GC TTL</a>).</td></tr></tbody></table>
...
Next, restart the PCR stream with the [`CREATE VIRTUAL CLUSTER`](/docs/v23.2/create-virtual-cluster) syntax:
