...
CockroachDB automatically splits ranges of data in [the key-value store](/docs/v23.2/architecture/storage-layer) based on [the size of the range](/docs/v23.2/architecture/distribution-layer#range-splits) and on [the load streaming to the range](/docs/v23.2/load-based-splitting). To split a range based on load, the system looks for a point in the range that evenly divides incoming traffic. If the range is indexed on a column of data that is sequential in nature (e.g., [an ordered sequence](/docs/v23.2/sql-faqs#what-are-the-differences-between-uuid-sequences-and-unique_rowid) or a series of increasing, non-repeating [`TIMESTAMP`s](/docs/v23.2/timestamp)), then all incoming writes to the range will be the last (or first) item in the index and appended to the end of the range. As a result, the system cannot find a point in the range that evenly divides the traffic, and the range cannot benefit from load-based splitting, creating a [hot spot](/docs/v23.2/performance-best-practices-overview#hot-spots) on the single range.
...
Hash-sharded indexes contain a [virtual computed column](/docs/v23.2/computed-columns#virtual-computed-columns), known as a shard column. CockroachDB uses this shard column, as opposed to the sequential column in the index, to control the distribution of values across the index. The shard column is hidden by default but can be seen with [`SHOW COLUMNS`](/docs/v23.2/show-columns).

<Note>
In v21.2 and earlier, hash-sharded indexes create a physical `STORED` [computed column](/docs/v23.2/computed-columns) instead of a virtual computed column. If you are using a hash-sharded index that was created in v21.2 or earlier, the `STORED` column still exists in your database. When dropping a hash-sharded index that has created a physical shard column, you must include the [`CASCADE`](/docs/v23.2/drop-index#remove-an-index-and-dependent-objects-with-cascade) clause to drop the shard column. Doing so will require a rewrite of the table.
</Note>
...
<Note>
Hash-sharded indexes created in v22.1 and later will not [backfill](/docs/v23.2/changefeed-messages#schema-changes-with-column-backfill), as the shard column isn't stored. Hash-sharded indexes created prior to v22.1 will backfill if `schema_change_policy` is set to `backfill`, as they use a stored column. If you don't want CockroachDB to backfill hash-sharded indexes you created prior to v22.1, drop them and recreate them.
</Note>

### Shard count

When creating a hash-sharded index, CockroachDB creates a specified number of shards (buckets) within the cluster based on the value of the `sql.defaults.default_hash_sharded_index_bucket_count` [cluster setting](/docs/v23.2/cluster-settings). You can also specify a different `bucket_count` by passing in an optional storage parameter. See the example below.
...
- The table is partitioned implicitly with [`REGIONAL BY ROW`](/docs/v23.2/table-localities#regional-by-row-tables), and the `crdb_region` column is not part of the columns in the hash-sharded index.
