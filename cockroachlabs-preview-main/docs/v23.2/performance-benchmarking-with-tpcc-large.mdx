...
This page shows you how to reproduce [CockroachDB TPC-C performance benchmarking results](/docs/v23.2/performance#scale). Across all scales, CockroachDB can process tpmC (new order transactions per minute) at near maximum efficiency. Start by choosing the scale you're interested in:

<LinkedPageTabs
  currentPath="/docs/v23.2/performance-benchmarking-with-tpcc-large"
  items={[
    {
      title: "Local",
      href: "/docs/v23.2/performance-benchmarking-with-tpcc-local",
    },
    {
      title: "Local (Multi-Region)",
      href: "/docs/v23.2/performance-benchmarking-with-tpcc-local-multiregion",
    },
    {
      title: "Small",
      href: "/docs/v23.2/performance-benchmarking-with-tpcc-small",
    },
    {
      title: "Medium",
      href: "/docs/v23.2/performance-benchmarking-with-tpcc-medium",
    },
    {
      title: "Large",
      href: "/docs/v23.2/performance-benchmarking-with-tpcc-large",
    },
  ]}
/>
...
2. Visit [Releases](/docs/releases?filters=windows) to download and CockroachDB for Linux. Select the architecture of the VM, either Intel or ARM. Releases are rolled out gradually, so the latest version may not yet be available.
3. Extract the binary you downloaded, then optionally copy it into a location in your `PATH`. If you choose to copy it into a system directory, you may need to use `sudo`.
4. Start CockroachDB using the [`cockroach start`](/docs/v23.2/cockroach-start) command:
    ...
    Each node will start with a [locality](/docs/v23.2/cockroach-start#locality) that includes an artificial "rack number" (e.g., `--locality=rack=0`). Use 81 racks for 81 nodes so that 1 node will be assigned to each rack.
5. Repeat these steps for the other 80 VMs for CockroachDB nodes. Each time, be sure to:
    - Adjust the `--advertise-addr` flag.
    - Set the [`--locality`](/docs/v23.2/cockroach-start#locality) flag to the appropriate "rack number".
6. On any of the VMs with the `cockroach` binary, run the one-time [`cockroach init`](/docs/v23.2/cockroach-init) command to join the first nodes into a cluster:
...
2. Launch the [built-in SQL shell](/docs/v23.2/cockroach-sql):

    ```shell
    $ cockroach sql --insecure --host=<address of any node>
    ```
3. Adjust some [cluster settings](/docs/v23.2/cluster-settings):
    ...
    ```
4. Change the default [GC TTL](/docs/v23.2/configure-replication-zones#gc-ttlseconds) to the following value:
...
2. Download the [CockroachDB archive](https://binaries.cockroachdb.com/cockroach-v23.2.29.linux-amd64.tgz) for Linux, extract the binary, and copy it into the `PATH`:
...
3. Import the TPC-C dataset:
    ...
    You can monitor progress on the **Jobs** screen of the DB Console. Open the [DB Console](/docs/v23.2/ui-overview) by pointing a browser to the address in the `admin` field in the standard output of any node on startup.

## Step 5. Partition the database

1. [Partition your database](/docs/v23.2/partitioning) to divide all of the TPC-C tables and indexes into 81 partitions, one per rack, and then use [zone configurations](/docs/v23.2/configure-replication-zones) to pin those partitions to a particular rack.
2. Wait for up-replication and partitioning to finish. You will know when they have finished because both the number of *lease transfers* and *snapshots* will go down to `0` and stay there. This will likely take 10s of minutes.
    - To monitor the number of lease transfers, open the [DB Console](/docs/v23.2/ui-overview), select the **Replication** dashboard, hover over the **Range Operations** graph, and check the **Lease Transfers** data point.
    - To check the number of snapshots, open the [DB Console](/docs/v23.2/ui-overview), select the **Replication** dashboard, and hover over the **Snapshots** graph.

        <Frame>
          ![TPC-C 140k replication and partitioning dashboards](/images/v23.2/tpcc-large-replication-dashboard.png)
        </Frame>
...
- [Performance Overview](/docs/v23.2/performance)
- Hardware
