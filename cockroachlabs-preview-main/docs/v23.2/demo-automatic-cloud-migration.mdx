...
CockroachDB's flexible [replication controls](/docs/v23.2/configure-replication-zones) make it trivially easy to run a single CockroachDB cluster across cloud platforms and to migrate data from one cloud to another without any service interruption. This page guides you through a local simulation of the process.
...
- Install the latest version of [CockroachDB](/docs/v23.2/install-cockroachdb).
...
If you've already [started a local cluster](/docs/v23.2/start-a-local-cluster), the commands for starting nodes should be familiar to you. The new flag to note is [`--locality`](/docs/v23.2/configure-replication-zones#descriptive-attributes-assigned-to-nodes), which accepts key-value pairs that describe the topography of a node. In this case, you're using the flag to specify that the first 3 nodes are running on cloud 1.
...
In a new terminal, use the [`cockroach init`](/docs/v23.2/cockroach-init) command to perform a one-time initialization of the cluster:
...
In a new terminal, run the [`cockroach gen haproxy`](/docs/v23.2/cockroach-gen) command, specifying the port of any node:
...
So your cluster is replicating across two simulated clouds. But let's say that after experimentation, you're happy with cloud vendor 2, and you decide that you'd like to move everything there. Can you do that without interruption to your live client traffic? Yes, and it's as simple as running a single command to add a [hard constraint](/docs/v23.2/configure-replication-zones#replication-constraints) that all replicas must be on nodes with `--locality=cloud=2`.

In a new terminal, [edit the default replication zone](/docs/v23.2/alter-range#configure-zone):
...
<Frame>
  ![DB Console](/images/v23.2/ui_replicas_migration3.png)
</Frame>
...
- [Even Replication Across Datacenters](/docs/v23.2/configure-replication-zones#even-replication-across-availability-zones)
- [Multiple Applications Writing to Different Databases](/docs/v23.2/configure-replication-zones#multiple-applications-writing-to-different-databases)
- [Stricter Replication for a Table and Its Indexes](/docs/v23.2/configure-replication-zones#stricter-replication-for-a-table-and-its-secondary-indexes)
- [Tweaking the Replication of System Ranges](/docs/v23.2/configure-replication-zones#tweaking-the-replication-of-system-ranges)
