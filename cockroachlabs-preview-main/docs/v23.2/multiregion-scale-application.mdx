...
Before reading this page, review [Multi-Region Capabilities Overview](/docs/v23.2/multiregion-overview).
...
Use an [`ALTER DATABASE... SET PRIMARY REGION`](/docs/v23.2/alter-database#set-primary-region) statement to set the database's [primary region](/docs/v23.2/multiregion-overview#database-regions) to a region in which the cluster is deployed. This region must have been specified as a [regional locality](/docs/v23.2/cockroach-start#locality) at cluster startup.

Setting the primary region before adding new regional nodes to the cluster prevents CockroachDB from [rebalancing row replications](/docs/v23.2/architecture/replication-layer#leaseholder-rebalancing) across all regions each time a node is added in a new region.

<Note>
Executing `ALTER` statements performs a [schema migration](/docs/v23.2/online-schema-changes) on the cluster. If you are using a schema migration tool, you will need to execute these statements as raw SQL, as the [multi-region SQL syntax](/docs/v23.2/multiregion-overview) is specific to CockroachDB.

Here are some simple tutorials on executing schema migrations against CockroachDB clusters:

- [Migrate CockroachDB Schemas with Liquibase](/docs/v23.2/liquibase)
- [Migrate CockroachDB Schemas with Flyway](/docs/v23.2/flyway)
- [Migrate CockroachDB Schemas with Alembic](/docs/v23.2/alembic)
- [Execute SQL statements from a file](/docs/v23.2/cockroach-sql#execute-sql-statements-from-a-file) and [Change and Remove Objects in a Database Schema](/docs/v23.2/schema-design-update)
</Note>
...
- For orchestrated deployments, see [Orchestrate CockroachDB Across Multiple Kubernetes Clusters](/docs/v23.2/orchestrate-cockroachdb-with-kubernetes-multi-cluster).
- For manual deployments, see [`cockroach start`](/docs/v23.2/cockroach-start) and [Manual Deployment](/docs/v23.2/manual-deployment).

<Note>
For orchestrated and manual deployments, you must specify a [regional locality](/docs/v23.2/cockroach-start#locality) for each node at startup. These regional localities are represented as [cluster regions](/docs/v23.2/multiregion-overview#cluster-regions) in the cluster.
</Note>

### Step 3. Scale the database schema

Use an [`ALTER DATABASE... ADD REGIONS`](/docs/v23.2/alter-database#add-region) statement to add the new regions to your database. Only cluster regions (i.e., regional localities specified at cluster startup) can be added as [database regions](/docs/v23.2/multiregion-overview#database-regions).

After you add new regions to the database schema, you can optionally configure the [survival goals](/docs/v23.2/multiregion-overview#survival-goals) and [table localities](/docs/v23.2/multiregion-overview#table-locality) of the multi-region database:

- Add [`ALTER DATABASE... SURVIVE... FAILURE`](/docs/v23.2/alter-database#survive-zone-region-failure) statements to set your database's [survival goals](/docs/v23.2/multiregion-overview#survival-goals).
- Add [`ALTER TABLE... SET LOCALITY`](/docs/v23.2/alter-table#set-locality) statements to set [table localities](/docs/v23.2/multiregion-overview#table-locality) for each table.
...
For most table localities, including the default locality `LOCALITY REGIONAL BY TABLE IN PRIMARY REGION`, *you do not need to update your application code after migrating your database schema for multi-region*. CockroachDB automatically optimizes queries against multi-region databases, based on the regional locality of the node executing the query, and on the multi-region configuration of the database. For more details, see [Regional Tables](/docs/v23.2/regional-tables#regional-by-row-tables). For an extended example, see [Develop and Deploy a Global Application: Create a Multi-Region Database Schema](/docs/v23.2/movr-flask-database).

However, there are some scenarios in which you might need to update the SQL operations in your application. For example:

- If a table has a `REGIONAL BY ROW AS <custom_region_column>` table locality, and you want to explicitly insert regional values into a table, as shown in [Low Latency Reads and Writes in a Multi-Region Cluster](/docs/v23.2/demo-low-latency-multi-region-deployment#configure-regional-by-row-tables).
- If a table has a `REGIONAL BY ROW` locality, and you want to update the `crdb_region` value of existing rows in the table based on some other column value, as shown in [Set the table locality to `REGIONAL BY ROW`](/docs/v23.2/alter-table#set-the-table-locality-to-regional-by-row).
- If a table has a `REGIONAL BY ROW` locality, and you want to filter a [selection query](/docs/v23.2/select-clause#filter-rows) based on the `crdb_region` value.

In all of these scenarios, statements reference the column that tracks the region for each row in a `REGIONAL BY ROW` locality. This column can be a custom column of the built-in `ENUM` type `crdb_internal_region`, or it can be the default, hidden [`crdb_region` column](/docs/v23.2/alter-table#crdb_region).
...
- Verify that the region-tracking column is visible to the ORM.
    ...
    To make a hidden column visible, use an [`ALTER TABLE... ALTER COLUMN... SET VISIBLE` statement](/docs/v23.2/alter-table#alter-column). By default, the `crdb_region` column created by CockroachDB is hidden.
