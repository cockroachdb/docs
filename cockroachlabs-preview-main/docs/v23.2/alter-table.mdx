...
import { AlterTable3 } from "/snippets/syntax-diagrams/AlterTable3.jsx";
import { AlterTable4 } from "/snippets/syntax-diagrams/AlterTable4.jsx";

The `ALTER TABLE` [statement](/docs/v23.2/sql-statements) changes the definition of a table. For information on using `ALTER TABLE`, see the pages for its [subcommands](#subcommands).
...
<table><thead><tr><th>Subcommand</th><th>Description</th><th>Can combine with other subcommands?</th></tr></thead><tbody><tr><td><a href="#add-column"><code>ADD COLUMN</code></a></td><td>Add columns to tables.</td><td>Yes</td></tr><tr><td><a href="#add-constraint"><code>ADD CONSTRAINT</code></a></td><td>Add constraints to columns.</td><td>Yes</td></tr><tr><td><a href="#alter-column"><code>ALTER COLUMN</code></a></td><td>Change an existing column.</td><td>Yes</td></tr><tr><td><a href="#alter-primary-key"><code>ALTER PRIMARY KEY</code></a></td><td>Change the <a href="/docs/v23.2/primary-key">primary key</a> of a table.</td><td>Yes</td></tr><tr><td><a href="#configure-zone"><code>CONFIGURE ZONE</code></a></td><td><a href="/docs/v23.2/configure-replication-zones">Replication Controls</a> for a table.</td><td>No</td></tr><tr><td><a href="#drop-column"><code>DROP COLUMN</code></a></td><td>Remove columns from tables.</td><td>Yes</td></tr><tr><td><a href="#drop-constraint"><code>DROP CONSTRAINT</code></a></td><td>Remove constraints from columns.</td><td>Yes</td></tr><tr><td><a href="#experimental_audit"><code>EXPERIMENTAL&#95;AUDIT</code></a></td><td>Enable per-table audit logs, for security purposes.</td><td>Yes</td></tr><tr><td><a href="#owner-to"><code>OWNER TO</code></a></td><td>Change the owner of the table.</td><td>No</td></tr><tr><td><a href="#partition-by"><code>PARTITION BY</code></a></td><td>Partition, re-partition, or un-partition a table.</td><td>Yes</td></tr><tr><td><a href="#rename-column"><code>RENAME COLUMN</code></a></td><td>Change the names of columns.</td><td>Yes</td></tr><tr><td><a href="#rename-constraint"><code>RENAME CONSTRAINT</code></a></td><td>Change constraints columns.</td><td>Yes</td></tr><tr><td><a href="#rename-to"><code>RENAME TO</code></a></td><td>Change the names of tables.</td><td>No</td></tr><tr><td><a href="#reset-storage-parameter"><code>RESET &#123;storage parameter&#125;</code></a></td><td>Reset a storage parameter on a table to its default value.</td><td>Yes</td></tr><tr><td><a href="#set-storage-parameter"><code>SET &#123;storage parameter&#125;</code></a></td><td>Set a storage parameter on a table.</td><td>Yes</td></tr><tr><td><a href="#set-locality"><code>SET LOCALITY</code></a></td><td>Set the table locality for a table in a <a href="/docs/v23.2/multiregion-overview">multi-region database</a>.</td><td>No</td></tr><tr><td><a href="#set-schema"><code>SET SCHEMA</code></a></td><td>Change the <a href="/docs/v23.2/sql-name-resolution">schema</a> of a table.</td><td>No</td></tr><tr><td><a href="#scatter"><code>SCATTER</code></a></td><td>Makes a best-effort attempt to redistribute replicas and leaseholders for the ranges of a table or index. Note that it does not return an error even if replicas are not moved.</td><td>No</td></tr><tr><td><a href="#split-at"><code>SPLIT AT</code></a></td><td>Force a <a href="/docs/v23.2/architecture/distribution-layer#range-splits">range split</a> at the specified row in the table.</td><td>No</td></tr><tr><td><a href="#unsplit-at"><code>UNSPLIT AT</code></a></td><td>Remove a range split enforcement in the table.</td><td>No</td></tr><tr><td><a href="#validate-constraint"><code>VALIDATE CONSTRAINT</code></a></td><td>Check whether values in a column match a <a href="/docs/v23.2/constraints">constraint</a> on the column.</td><td>Yes</td></tr></tbody></table>
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>IF NOT EXISTS</code></td><td>Add a column only if a column of the same name does not already exist; if one does exist, do not return an error.</td></tr><tr><td><code>column&#95;name</code></td><td>The name of the column you want to add. The column name must follow these <a href="/docs/v23.2/keywords-and-identifiers#identifiers">identifier rules</a> and must be unique within the table but can have the same name as indexes or constraints.</td></tr><tr><td><code>typename</code></td><td>The <a href="/docs/v23.2/data-types">data type</a> of the new column.</td></tr><tr><td><code>col&#95;qualification</code></td><td>An optional list of <a href="/docs/v23.2/create-table#column-qualifications">column qualifications</a>.</td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

### `ADD CONSTRAINT`

Use `ALTER TABLE... ADD CONSTRAINT` to add the following [constraints](/docs/v23.2/constraints) to columns:

- [`UNIQUE`](#add-the-unique-constraint)
- [`CHECK`](#add-the-check-constraint)
- [`FOREIGN KEY`](#add-the-foreign-key-constraint-with-cascade)

To add a primary key constraint to a table, you should explicitly define the primary key at [table creation](/docs/v23.2/create-table). To replace an existing primary key, you can use `ADD CONSTRAINT... PRIMARY KEY`. For details, see [Changing primary keys with `ADD CONSTRAINT... PRIMARY KEY`](#changing-primary-keys-with-add-constraint-primary-key).

The [`DEFAULT`](/docs/v23.2/default-value) and [`NOT NULL`](/docs/v23.2/not-null) constraints are managed through [`ALTER COLUMN`](/docs/v23.2/alter-table#alter-column).
...
- No primary key was explicitly defined at [table creation](/docs/v23.2/create-table). In this case, the table is created with a default [primary key on `rowid`](/docs/v23.2/indexes#creation). Using `ADD CONSTRAINT... PRIMARY KEY` drops the default primary key and replaces it with a new primary key.
- A [`DROP CONSTRAINT`](/docs/v23.2/alter-table#drop-constraint) statement precedes the `ADD CONSTRAINT... PRIMARY KEY` statement, in the same transaction. For an example, see [Drop and add the primary key constraint](#drop-and-add-a-primary-key-constraint).
...
- Set, change, or drop a column's [`DEFAULT` constraint](/docs/v23.2/default-value).
- Set or drop a column's [`NOT NULL` constraint](/docs/v23.2/not-null).
- Set, change, or drop an [`ON UPDATE` expression](/docs/v23.2/create-table#on-update-expressions).
- Change a column's [data type](/docs/v23.2/data-types).
- Set the [visibility](#set-the-visibility-of-a-column) of a column.

<Note>
Support for altering column data types is [in preview](/docs/v23.2/cockroachdb-feature-availability), with certain limitations. For details, see [Altering column data types](#alter-column-data-types).
</Note>
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>column&#95;name</code></td><td>The name of the column to modify.</td></tr><tr><td><code>a&#95;expr</code></td><td>The new <a href="/docs/v23.2/default-value">default value</a> to set.</td></tr><tr><td><code>b&#95;expr</code></td><td>The <a href="/docs/v23.2/create-table#on-update-expressions"><code>ON UPDATE</code> expression</a> to set.</td></tr><tr><td><code>&#91;NOT&#93; VISIBLE</code></td><td>The visibility of a column when using <code>&#42;</code> in a <a href="/docs/v23.2/select-clause"><code>SELECT</code> clause</a>.</td></tr><tr><td><code>typename</code></td><td>The new <a href="/docs/v23.2/data-types">data type</a> you want to use.<br/>Support for altering column types is <a href="/docs/v23.2/cockroachdb-feature-availability">in preview</a>, with certain limitations. For details, see <a href="#alter-column-data-types">Alter column data types</a>.</td></tr><tr><td><code>USING a&#95;expr</code></td><td>How to compute a new column value from the old column value.</td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

#### Alter column data types

Support for altering column data types is [in preview](/docs/v23.2/cockroachdb-feature-availability), with certain limitations. To enable column type altering, set the `enable_experimental_alter_column_type_general` [session variable](/docs/v23.2/set-vars) to `true`.
...
- The column is part of an [index](/docs/v23.2/indexes).
- The column has [`CHECK` constraints](/docs/v23.2/check).
- The column owns a [sequence](/docs/v23.2/create-sequence).
- The column has a [`DEFAULT` expression](/docs/v23.2/default-value). This will result in an `ERROR:... column... cannot also have a DEFAULT expression` with `SQLSTATE: 42P16`.
- The `ALTER COLUMN TYPE` statement is part of a combined `ALTER TABLE` statement.
- The `ALTER COLUMN TYPE` statement is inside an [explicit transaction](/docs/v23.2/begin-transaction).
...
Use `ALTER TABLE... ALTER PRIMARY KEY` to change the [primary key](/docs/v23.2/primary-key) of a table.

Note the following:

- You cannot change the primary key of a table that is currently undergoing a primary key change, or any other [schema change](/docs/v23.2/online-schema-changes).
- `ALTER PRIMARY KEY` might need to rewrite multiple indexes, which can make it an expensive operation.
- When you change a primary key with `ALTER PRIMARY KEY`, the old primary key index becomes a [`UNIQUE`](/docs/v23.2/unique) secondary index. This helps optimize the performance of queries that still filter on the old primary key column.
- `ALTER PRIMARY KEY` does not alter the [partitions](/docs/v23.2/partitioning) on a table or its indexes, even if a partition is defined on [a column in the original primary key](/docs/v23.2/partitioning#partition-using-primary-key). If you alter the primary key of a partitioned table, you must update the table partition accordingly.
- The secondary index created by `ALTER PRIMARY KEY` will not be partitioned, even if a partition is defined on [a column in the original primary key](/docs/v23.2/partitioning#partition-using-primary-key). To ensure that the table is partitioned correctly, you must create a partition on the secondary index, or drop the secondary index.
- Any new primary key column set by `ALTER PRIMARY KEY` must have an existing [`NOT NULL` constraint](/docs/v23.2/not-null). To add a `NOT NULL` constraint to an existing column, use [`ALTER TABLE... ALTER COLUMN... SET NOT NULL`](#set-not-null-constraint).
...
The user must have the `CREATE` [privilege](/docs/v23.2/security-reference/authorization#managing-privileges) on a table to alter its primary key.

#### Parameters

<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>index&#95;params</code></td><td>The name of the column(s) that you want to use for the primary key. These columns replace the current primary key column(s).</td></tr><tr><td><code>USING HASH</code></td><td>Creates a <a href="/docs/v23.2/hash-sharded-indexes">hash-sharded index</a>.</td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

### `CONFIGURE ZONE`

`ALTER TABLE... CONFIGURE ZONE` is used to add, modify, reset, or remove replication zones for a table. To view details about existing replication zones, use [`SHOW ZONE CONFIGURATIONS`](/docs/v23.2/show-zone-configurations). For more information about replication zones, see [Replication Controls](/docs/v23.2/configure-replication-zones).
...
For instructions showing how to troubleshoot replication zones that may be misconfigured, see [Troubleshoot Replication Zone Configurations](/docs/v23.2/troubleshoot-replication-zones).

#### Required privileges

The user must be a member of the [`admin` role](/docs/v23.2/security-reference/authorization#admin-role) or have been granted [`CREATE`](/docs/v23.2/security-reference/authorization#supported-privileges) or [`ZONECONFIG`](/docs/v23.2/security-reference/authorization#supported-privileges) privileges. To configure [`system` objects](/docs/v23.2/configure-replication-zones#for-system-data), the user must be a member of the `admin` role.

#### Parameters

<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>variable</code></td><td>The name of the <a href="/docs/v23.2/configure-replication-zones#replication-zone-variables">replication zone variable</a> to change.</td></tr><tr><td><code>value</code></td><td>The value of the <a href="/docs/v23.2/configure-replication-zones#replication-zone-variables">replication zone variable</a> to change.</td></tr><tr><td><code>DISCARD</code></td><td>Remove a replication zone.</td></tr></tbody></table>
...
By default, `DROP COLUMN` drops any [indexes](/docs/v23.2/indexes) on the column being dropped, and any indexes that reference the column, including indexes with [`STORING` clauses](/docs/v23.2/create-index#store-columns) that reference the column.

For examples, see [Drop columns](#drop-columns).

#### Known limitations

- CockroachDB prevents a column from being dropped if it is referenced by a [partial index](/docs/v23.2/partial-indexes) predicate. To drop such a column, the partial indexes need to be dropped first using [`DROP INDEX`](/docs/v23.2/drop-index). See [tracking issue](https://github.com/cockroachdb/cockroach/issues/97813).
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>column&#95;name</code></td><td>The name of the column you want to drop.<br/><br/>When a column with a <a href="/docs/v23.2/check"><code>CHECK</code></a> constraint is dropped, the <code>CHECK</code> constraint is also dropped.</td></tr><tr><td><code>CASCADE</code></td><td>Drop the column even if objects (such as <a href="/docs/v23.2/views">views</a>) depend on it; drop the dependent objects, as well. <code>CASCADE</code> will drop a column with a foreign key constraint if it is the only column in the reference.<br/><br/><code>CASCADE</code> does not list the objects it drops, so should be used cautiously.<br/><br/><code>CASCADE</code> is not required to drop an indexed column, or a column that is referenced by an index. By default, <code>DROP COLUMN</code> drops any <a href="/docs/v23.2/indexes">indexes</a> on the column being dropped, and any indexes that reference the column, including <a href="/docs/v23.2/partial-indexes">partial indexes</a> with predicates that reference the column and indexes with <a href="/docs/v23.2/create-index#store-columns"><code>STORING</code> clauses</a> that reference the column.</td></tr><tr><td><code>RESTRICT</code></td><td><em>(Default)</em> Do not drop the column if any objects (such as <a href="/docs/v23.2/views">views</a>) depend on it.</td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

### `DROP CONSTRAINT`

Use `ALTER TABLE... DROP CONSTRAINT` to remove [`CHECK`](/docs/v23.2/check) and [`FOREIGN KEY`](/docs/v23.2/foreign-key) constraints from columns.

[`PRIMARY KEY`](/docs/v23.2/primary-key) constraints can be dropped with `DROP CONSTRAINT` if an [`ADD CONSTRAINT`](/docs/v23.2/alter-table#add-constraint) statement follows the `DROP CONSTRAINT` statement in the same transaction.

<Tip>
When you change a primary key with [`ALTER TABLE... ALTER PRIMARY KEY`](/docs/v23.2/alter-table#alter-primary-key), the old primary key index becomes a secondary index. If you do not want the old primary key to become a secondary index, use `DROP CONSTRAINT`/[`ADD CONSTRAINT`](/docs/v23.2/alter-table#add-constraint) to change the primary key.
</Tip>

<Note>
For information about removing other constraints, see [Constraints: Remove Constraints](/docs/v23.2/constraints#remove-constraints).
</Note>
...
`ALTER TABLE... EXPERIMENTAL_AUDIT` enables or disables the recording of SQL audit events to the [`SENSITIVE_ACCESS`](/docs/v23.2/logging#sensitive_access) logging channel for a table. The `SENSITIVE_ACCESS` log output is also called the *SQL audit log*. For details on using SQL audit logs, see [SQL Audit Logging](/docs/v23.2/sql-audit-logging).
...
CockroachDB stores audit log information in a way that ensures durability, but negatively impacts performance. As a result, we recommend using SQL audit logs for security purposes only. For more information, see [Performance considerations](/docs/v23.2/sql-audit-logging#performance-considerations).
...
To change the owner of a table, the user must be an `admin` user, or the current owner of the table and a member of the new owner [role](/docs/v23.2/security-reference/authorization#roles). The new owner role must also have the `CREATE` [privilege](/docs/v23.2/security-reference/authorization#managing-privileges) on the schema to which the table belongs.
...
<Note>
It is not possible to rename a column referenced by a view. For more details, see [View Dependencies](/docs/v23.2/views#view-dependencies).
</Note>
...
<Note>
It is not possible to rename a constraint for a column referenced by a view. For more details, see [View Dependencies](/docs/v23.2/views#view-dependencies).
</Note>
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>IF EXISTS</code></td><td>Rename the constraint only if a constraint of <code>current&#95;name</code> exists; if one does not exist, do not return an error.</td></tr><tr><td><code>constraint&#95;name</code></td><td>The current name of the constraint.</td></tr><tr><td><code>constraint&#95;new&#95;name</code></td><td>The new <a href="/docs/v23.2/sql-grammar#name"><code>name</code></a> you want to use for the constraint, which must be unique to its table and follow these <a href="/docs/v23.2/keywords-and-identifiers#identifiers">identifier rules</a>.</td></tr></tbody></table>
...
`ALTER TABLE... RENAME TO` cannot be used to move a table from one database to another. To change a table's database, use [`BACKUP`](/docs/v23.2/backup#back-up-a-table-or-view) and [`RESTORE`](/docs/v23.2/restore#restore-a-table).
</Note>

<Note>
It is not possible to rename a table referenced by a view. For more details, see [View Dependencies](/docs/v23.2/views#view-dependencies).
</Note>
...
The user must have the `DROP` [privilege](/docs/v23.2/security-reference/authorization#managing-privileges) on the table and the `CREATE` on the parent database. When moving a table from one database to another, the user must have the `CREATE` privilege on both the source and target databases.

#### Parameters

<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>table&#95;new&#95;name</code></td><td>The new name of the table, which must be unique within its database and follow these <a href="/docs/v23.2/keywords-and-identifiers#identifiers">identifier rules</a>. When the parent database is not set as the default, the name must be formatted as <code>database.name</code>.<br/><br/>The <a href="/docs/v23.2/upsert"><code>UPSERT</code></a> and <a href="/docs/v23.2/insert"><code>INSERT ON CONFLICT</code></a> statements use a temporary table called <code>excluded</code> to handle uniqueness conflicts during execution. It's therefore not recommended to use the name <code>excluded</code> for any of your tables.</td></tr></tbody></table>
...
<Note>
To reset a storage parameter on an existing index, you must drop and [recreate the index without the storage parameter](/docs/v23.2/with-storage-parameter).
</Note>
...
<Note>
To set a storage parameter on an existing index, you must drop and [recreate the index with the storage parameter](/docs/v23.2/with-storage-parameter).
</Note>
...
`ALTER TABLE.. SET LOCALITY` changes the [table locality](/docs/v23.2/multiregion-overview#table-locality) of a [table](/docs/v23.2/create-table) in a [multi-region database](/docs/v23.2/multiregion-overview).

While CockroachDB is processing an `ALTER TABLE.. SET LOCALITY` statement that enables or disables `REGIONAL BY ROW` on a table within a database, any [`ADD REGION`](/docs/v23.2/alter-database#add-region) and [`DROP REGION`](/docs/v23.2/alter-database#drop-region) statements on that database will fail.
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>locality</code></td><td>The <code>LOCALITY</code> clause, followed by the <a href="/docs/v23.2/multiregion-overview#table-locality">locality</a> to apply to this table. Allowed values:<br/><br/>- <a href="#regional-by-table"><code>REGIONAL BY TABLE</code></a> (default)<br/><br/>- <a href="#regional-by-row"><code>REGIONAL BY ROW</code></a><br/><br/>- <a href="#global"><code>GLOBAL</code></a></td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

For more information about which table locality is right for your use case, see [Table localities](/docs/v23.2/multiregion-overview#table-locality).

### `SET SCHEMA`

`ALTER TABLE... SET SCHEMA` changes the [schema](/docs/v23.2/sql-name-resolution) of a table.

<Note>
CockroachDB supports `SET SCHEMA` as an [alias for setting the `search_path` session variable](/docs/v23.2/set-vars#supported-variables).
</Note>
...
The user must have the `DROP` [privilege](/docs/v23.2/security-reference/authorization#managing-privileges) on the table, and the `CREATE` privilege on the schema.
...
`ALTER TABLE... SCATTER` runs a specified set of ranges for a table or index through the [replication layer](/docs/v23.2/architecture/replication-layer) queue. If many ranges have been created recently, the replication queue may transfer some leases to other replicas to balance load across the cluster.
...
The user must have the `INSERT` [privilege](/docs/v23.2/security-reference/authorization#managing-privileges) on the table or index.

#### Parameters

<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>table&#95;name</code></td><td>The name of the table that you want to scatter.</td></tr><tr><td><code>table&#95;index&#95;name</code></td><td>The name of the index that you want to scatter.</td></tr><tr><td><code>expr&#95;list</code></td><td>A list of <a href="/docs/v23.2/scalar-expressions">scalar expressions</a> in the form of the primary key of the table or the specified index.</td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

### `SPLIT AT`

`ALTER TABLE... SPLIT AT` forces a [range split](/docs/v23.2/architecture/distribution-layer#range-splits) at a specified row in the table.

CockroachDB breaks data into ranges. By default, CockroachDB attempts to keep ranges below [the default range size](/docs/v23.2/configure-replication-zones#range-max-bytes). To do this, the system will automatically [split a range](/docs/v23.2/architecture/distribution-layer#range-splits) if it grows larger than this limit. For most use cases, this automatic range splitting is sufficient, and you should never need to worry about when or where the system decides to split ranges.

However, there are reasons why you may want to perform manual splits on the ranges that store tables or indexes:

- When a table only consists of a single range, all writes and reads to the table will be served by that range's [leaseholder](/docs/v23.2/architecture/replication-layer#leases). If a table only holds a small amount of data but is serving a large amount of traffic, load distribution can become unbalanced and a [hot spot](/docs/v23.2/performance-best-practices-overview#hot-spots) can occur. Splitting the table's ranges manually can allow the load on the table to be more evenly distributed across multiple nodes. For tables consisting of more than a few ranges, load will naturally be distributed across multiple nodes and this will not be a concern.
- When a table is created, it will only consist of a single range. If you know that a new table will immediately receive significant write traffic, you may want to preemptively split the table based on the expected distribution of writes before applying the load. This can help avoid reduced workload performance that results when automatic splits are unable to keep up with write traffic and a [hot spot](/docs/v23.2/performance-best-practices-overview#hot-spots) occurs.

For examples, see [Split and unsplit tables](#split-and-unsplit-tables).

#### Required privileges

The user must have the `INSERT` [privilege](/docs/v23.2/security-reference/authorization#managing-privileges) on the table.

#### Parameters

<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>select&#95;stmt</code></td><td>A <a href="/docs/v23.2/selection-queries">selection query</a> that produces one or more rows at which to split the table.</td></tr><tr><td><code>a&#95;expr</code></td><td>The expiration of the split enforcement on the table. This can be a <a href="/docs/v23.2/decimal"><code>DECIMAL</code></a>, <a href="/docs/v23.2/interval"><code>INTERVAL</code></a>, <a href="/docs/v23.2/timestamp"><code>TIMESTAMP</code></a>, or <a href="/docs/v23.2/timestamp"><code>TIMESTAMPZ</code></a>.</td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

### `UNSPLIT AT`

`ALTER TABLE... UNSPLIT AT` removes a [split enforcement](#split-at) on a [range split](/docs/v23.2/architecture/distribution-layer#range-splits), at a specified row in the table.

Removing a split enforcement from a table or index ("unsplitting") allows CockroachDB to merge ranges as needed, to help improve your cluster's performance. For more information, see [Range Merges](/docs/v23.2/architecture/distribution-layer#range-merges).
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>select&#95;stmt</code></td><td>A <a href="/docs/v23.2/selection-queries">selection query</a> that produces one or more rows at which to unsplit a table.</td></tr><tr><td><code>ALL</code></td><td>Remove all split enforcements for a table.</td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

### `VALIDATE CONSTRAINT`

`ALTER TABLE... VALIDATE CONSTRAINT` checks whether values in a column match a [constraint](/docs/v23.2/constraints) on the column.
...
The following examples use the [`bank` demo database schema](/docs/v23.2/cockroach-demo#datasets).

To follow along, run [`cockroach demo bank`](/docs/v23.2/cockroach-demo) to start a temporary, in-memory cluster with the `bank` schema and dataset preloaded:
...
Moving frequently updated columns to their own [column family can increase performance](/docs/v23.2/column-families#default-behavior).

To move a column from one column family to another column family, create a temporary, [non-visible](/docs/v23.2/create-table#not-visible-property) [stored computed column](/docs/v23.2/computed-columns) in the target column family, then rename the columns. Once this succeeds, you can drop the original, now renamed column.
...
3. Drop the old column:
    ...
    <Note>
    You must set the [`sql_safe_updates` session variable](/docs/v23.2/set-vars#sql-safe-updates) to `false` to drop a column in a table that has data.
    </Note>

Moving a column to another column family executes writes to the underlying storage equal to two times the number of rows. For example, if the table has 10 million rows, there will be 20 million writes to the [storage layer](/docs/v23.2/architecture/storage-layer): 10 million writes when creating the temporary stored computed column, and 10 million writes when removing the original column.
...
Adding the [`UNIQUE` constraint](/docs/v23.2/unique) requires that all of a column's values be distinct from one another (except for `NULL` values).
...
Adding the [`CHECK` constraint](/docs/v23.2/check) requires that all of a column's values evaluate to `TRUE` for a Boolean expression.
...
- If a new column has a default value or is a [computed column](/docs/v23.2/computed-columns) that would have contained values that violate the new constraint.
</Note>
...
You can include a [foreign key action](/docs/v23.2/foreign-key#foreign-key-actions) to specify what happens when a foreign key is updated or deleted.
...
<Note>
By default, referenced columns must be in the same database as the referencing foreign key column. To enable cross-database foreign key references, set the `sql.cross_db_fks.enabled` [cluster setting](/docs/v23.2/cluster-settings) to `true`.
</Note>
...
1. View the details of the `promo_codes` table:
    ...
    (1 row)
    ```
2. Add a [`NOT NULL`](/docs/v23.2/not-null) constraint to the `creation_time` column with [`ALTER COLUMN`](#alter-column):
...
3. To issue the schema change atomically, use single statements as an implicit transaction. `DROP CONSTRAINT` and `ADD CONSTRAINT` can be combined in a single `ALTER TABLE` statement:
    ...
    <Note>
    You should **not** execute the schema change with multiple statements within an explicit transaction. Refer to [Schema changes within transactions](/docs/v23.2/online-schema-changes#schema-changes-within-transactions).
    </Note>
...
Using [`ALTER PRIMARY KEY`](/docs/v23.2/alter-table#alter-primary-key) would have created a `UNIQUE` secondary index called `promo_codes_code_key`. Instead, there is just one index for the primary key constraint.

#### Add a unique index to a `REGIONAL BY ROW` table

In [multi-region deployments](/docs/v23.2/multiregion-overview), most users should use [`REGIONAL BY ROW` tables](/docs/v23.2/table-localities#regional-by-row-tables) instead of explicit index [partitioning](/docs/v23.2/partitioning). When you add an index to a `REGIONAL BY ROW` table, it is automatically partitioned on the [`crdb_region` column](/docs/v23.2/alter-table#crdb_region). Explicit index partitioning is not required.

While CockroachDB process an [`ADD REGION`](/docs/v23.2/alter-database#add-region) or [`DROP REGION`](/docs/v23.2/alter-database#drop-region) statement on a particular database, creating or modifying an index will throw an error. Similarly, all [`ADD REGION`](/docs/v23.2/alter-database#add-region) and [`DROP REGION`](/docs/v23.2/alter-database#drop-region) statements will be blocked while an index is being modified on a `REGIONAL BY ROW` table within the same database.

This example assumes you have a simulated multi-region database running on your local machine following the steps described in [Low Latency Reads and Writes in a Multi-Region Cluster](/docs/v23.2/demo-low-latency-multi-region-deployment). It shows how a `UNIQUE` index is partitioned, but it's similar to how all indexes are partitioned on `REGIONAL BY ROW` tables.

To show how the automatic partitioning of indexes on `REGIONAL BY ROW` tables works, we will:

1. [Add a column](/docs/v23.2/alter-table#add-column) to the `users` table in the [MovR dataset](/docs/v23.2/movr).
2. Add a [`UNIQUE` constraint](/docs/v23.2/unique) to that column.
3. Verify that the index is automatically partitioned for better multi-region performance by using [`SHOW INDEXES`](/docs/v23.2/show-index) and [`SHOW PARTITIONS`](/docs/v23.2/show-partitions).
...
Next, issue the [`SHOW INDEXES`](/docs/v23.2/show-index) statement. You will see that [the implicit region column](#set-the-table-locality-to-regional-by-row) that was added when the table [was converted to regional by row](/docs/v23.2/demo-low-latency-multi-region-deployment#configure-regional-by-row-tables) is now indexed:
...
Next, issue the [`SHOW PARTITIONS`](/docs/v23.2/show-partitions) statement. The following output (which is edited for length) will verify that the unique index was automatically [partitioned](/docs/v23.2/partitioning) for you. It shows that the `user_email_unique` index is now partitioned by the database regions `europe-west1`, `us-east1`, and `us-west1`.
...
2. Thereafter, the [optimizer](/docs/v23.2/cost-based-optimizer) will automatically add a "uniqueness check" when necessary to any [`INSERT`](/docs/v23.2/insert), [`UPDATE`](/docs/v23.2/update), or [`UPSERT`](/docs/v23.2/upsert) statement affecting the columns in the unique constraint.
...
To auto-generate unique row identifiers in `REGIONAL BY ROW` tables, use the [`UUID`](/docs/v23.2/uuid) column with the `gen_random_uuid()` [function](/docs/v23.2/functions-and-operators#id-generation-functions) as the [default value](/docs/v23.2/default-value):
...
<Note>
When using `DEFAULT gen_random_uuid()` on columns in `REGIONAL BY ROW` tables, uniqueness checks on those columns are disabled by default for performance purposes. CockroachDB assumes uniqueness based on the way this column generates [`UUIDs`](/docs/v23.2/uuid#create-a-table-with-auto-generated-unique-row-ids). To enable this check, you can modify the `sql.optimizer.uniqueness_checks_for_gen_random_uuid.enabled` [cluster setting](/docs/v23.2/cluster-settings). Note that while there is virtually no chance of a [collision](https://wikipedia.org/wiki/Universally_unique_identifier#Collisions) occurring when enabling this setting, it is not truly zero.
</Note>

#### Using implicit vs. explicit index partitioning in `REGIONAL BY ROW` tables

In `REGIONAL BY ROW` tables, all indexes are partitioned on the region column (usually called [`crdb_region`](/docs/v23.2/alter-table#crdb_region)).

These indexes can either include or exclude the partitioning key (`crdb_region`) as the first column in the index definition:

- If `crdb_region` is included in the index definition, a [`UNIQUE` index](/docs/v23.2/unique) will enforce uniqueness on the set of columns, just like it would in a non-partitioned table.
...
In the latter case, the index alone cannot enforce uniqueness on columns that are not a prefix of the index columns, so any time rows are [inserted](/docs/v23.2/insert) or [updated](/docs/v23.2/update) in a `REGIONAL BY ROW` table that has an implicitly partitioned `UNIQUE` index, the [optimizer](/docs/v23.2/cost-based-optimizer) must add uniqueness checks.

Whether or not to explicitly include `crdb_region` in the index definition depends on the context:

- If you only need to enforce uniqueness at the region level, then including `crdb_region` in the `UNIQUE` index definition will enforce these semantics and allow you to get better performance on [`INSERT`](/docs/v23.2/insert) s, [`UPDATE`](/docs/v23.2/update) s, and [`UPSERT`](/docs/v23.2/upsert) s, since there will not be any added latency from uniqueness checks.
- If you need to enforce global uniqueness, you should not include `crdb_region` in the `UNIQUE` (or [`PRIMARY KEY`](/docs/v23.2/primary-key) ) index definition, and the database will automatically ensure that the constraint is enforced.
...
1. Start [`cockroach demo`](/docs/v23.2/cockroach-demo) as follows:
...
3. In the following statement, we add a new user with the required `id`, `email`, and `desk_id` columns. CockroachDB needs to do additional work to enforce global uniqueness for the `id` and `email` columns, which are implicitly partitioned. This additional work is in the form of "uniqueness checks" that the optimizer adds as part of mutation queries.
    ...
    The following `EXPLAIN` output shows that the optimizer has added two `constraint-check` post queries to check the uniqueness of the implicitly partitioned indexes `id` and `email`. There is no check needed for `desk_id` (really `(crdb_region, desk_id)`), since that constraint is automatically enforced by the explicitly partitioned index we added in the preceding [`CREATE TABLE`](/docs/v23.2/create-table) statement.
...
Setting the [`DEFAULT` value constraint](/docs/v23.2/default-value) inserts the value when data's written to the table without explicitly defining the value for the column. If the column already has a `DEFAULT` value set, you can use this statement to change it.
...
If the column has a defined [`DEFAULT` value](/docs/v23.2/default-value), you can remove the constraint, which means the column will no longer insert a value by default if one is not explicitly defined for the column.
...
To specify that the column cannot contain `NULL` values, set the [`NOT NULL` constraint](/docs/v23.2/not-null).
...
If the column has the [`NOT NULL` constraint](/docs/v23.2/not-null) applied to it, you can remove the constraint, which means the column becomes optional and can have `NULL` values written into it.
...
The `full_name` column is computed from the `first_name` and `last_name` columns without the need to define a [view](/docs/v23.2/views). You can view the column details with the [`SHOW COLUMNS`](/docs/v23.2/show-columns) statement:
...
To alter the formula for a computed column, you must [`DROP`](/docs/v23.2/alter-table#drop-column) and [`ADD`](/docs/v23.2/alter-table#add-column) the column back with the new definition. Take the following table for instance:
...
The [TPC-C](/docs/v23.2/performance-benchmarking-with-tpcc-small) database has a `customer` table with a column `c_credit_lim` of type `DECIMAL(10,2)`:
...
1. Set the `enable_experimental_alter_column_type_general` [session variable](/docs/v23.2/set-vars) to `true`:
...
The [TPC-C](/docs/v23.2/performance-benchmarking-with-tpcc-small) `customer` table contains a column `c_balance` of type `DECIMAL(12,2)`:
...
To specify that a column won't be returned when using `*` in a [`SELECT` clause](/docs/v23.2/select-clause), set the `NOT VISIBLE` property. You can set the `NOT VISIBLE` property only on individual columns.

For example, the `users` table of the [`movr` database](/docs/v23.2/movr) contains the `credit_card` column. If you don't want users to see that column when running `SELECT * FROM users;`, you can hide it as follows:
...
The primary key of this table is on the `name` column. This is a poor choice, as some users likely have the same name, and all primary keys enforce a `UNIQUE` constraint on row values of the primary key column. Per our [best practices](/docs/v23.2/performance-best-practices-overview#use-functions-to-generate-unique-ids), you should instead use a `UUID` for single-column primary keys, and populate the rows of the table with generated, unique values.
...
You cannot `DISCARD` any zone configurations on multi-region tables, indexes, or partitions if the [multi-region abstractions](/docs/v23.2/migrate-to-multiregion-sql#replication-zone-patterns-and-multi-region-sql-abstractions) created the zone configuration.
</Note>
...
If there is data in the table, the `sql_safe_updates` [session variable](/docs/v23.2/set-vars) must be set to `false`.
...
If the column has dependent objects, such as [views](/docs/v23.2/views), CockroachDB will not drop the column by default. However, if you want to be sure of the behavior you can include the `RESTRICT` clause.
...
Now, every access of customer data is logged to the `SENSITIVE_ACCESS` channel in a [`sensitive_table_access`](/docs/v23.2/eventlog#sensitive_table_access) event that looks like the following:
...
<Note>
The preceding example shows the default [`crdb-v2`](/docs/v23.2/log-formats#format-crdb-v2) log format. This can be changed to a different format (e.g., JSON). For details, see [Configure Logs](/docs/v23.2/configure-logs#file-logging-format).
</Note>

<Tip>
For descriptions of all SQL audit event types and their fields, see [Notable Event Types](/docs/v23.2/eventlog#sql-access-audit-events).
</Tip>

To turn on auditing for more than one table, issue a separate `ALTER` statement for each table.

<Tip>
For a more detailed example, see [SQL Audit Logging](/docs/v23.2/sql-audit-logging).
</Tip>
...
<Note>
If the user running the command is not an admin user, they must own the table and be a member of the new owning role. Also, the new owner role must also have the `CREATE` [privilege](/docs/v23.2/security-reference/authorization#managing-privileges) on the schema to which the table belongs.
</Note>
...
Some subcommands can be used in combination in a single [`ALTER TABLE`](/docs/v23.2/alter-table) statement. For example, let's say you create a `users` table with 2 columns, an `id` column for the primary key and a `name` column for each user's last name:
...
Then you decide you want distinct columns for each user's first name, last name, and full name, so you execute a single `ALTER TABLE` statement renaming `name` to `last_name`, adding `first_name`, and adding a [computed column](/docs/v23.2/computed-columns) called `name` that concatenates `first_name` and `last_name`:
...
In some situations, you may want to exclude a table's row data from a [backup](/docs/v23.2/backup). For example, you have a table that contains high-churn data that you would like to [garbage collect](/docs/v23.2/architecture/storage-layer#garbage-collection) more quickly than the [incremental backup](/docs/v23.2/take-full-and-incremental-backups#incremental-backups) schedule for the database or cluster holding the table. You can use the `exclude_data_from_backup = true` parameter with a [`CREATE TABLE`](/docs/v23.2/create-table#create-a-table-with-data-excluded-from-backup) or `ALTER TABLE` statement to mark a table's row data for exclusion from a backup.

For more detail and an example through the backup and [restore](/docs/v23.2/restore) process using this parameter, see [Take Full and Incremental Backups](/docs/v23.2/take-full-and-incremental-backups#exclude-a-tables-data-from-backups).
...
<Note>
[`RESTORE`](/docs/v23.2/restore) on [`REGIONAL BY TABLE`](#regional-by-table), [`REGIONAL BY ROW`](#regional-by-row), and [`GLOBAL`](#global) tables is supported with some limitations — see [Restoring to multi-region databases](/docs/v23.2/restore#restoring-to-multi-region-databases) for more detail.
</Note>
...
For more information about how this table locality works, see [Regional tables](/docs/v23.2/table-localities#regional-tables).

#### Set the table locality to `REGIONAL BY ROW`

<Note>
Before setting the locality to `REGIONAL BY ROW` on a table targeted by a changefeed, read the considerations in [Changefeeds on regional by row tables](/docs/v23.2/changefeeds-in-multi-region-deployments).
</Note>
...
Every row in a regional by row table has a column of type `crdb_internal_region` that represents the row's [home region](/docs/v23.2/multiregion-overview#table-localities). By default, this column is called `crdb_region` and is hidden. To see a row's home region, issue a statement like the following:

```sql
SELECT crdb_region, id FROM {table};
```

To update an existing row's home region, use an [`UPDATE`](/docs/v23.2/update) statement like the following:
...
- Let CockroachDB set the row's home region automatically. It will use the region of the [gateway node](/docs/v23.2/architecture/life-of-a-distributed-transaction#gateway) from which the row is inserted.
- Set the home region explicitly using an [`INSERT`](/docs/v23.2/insert) statement like the following:
...
For more information about how this table locality works, see [Regional by row tables](/docs/v23.2/table-localities#regional-by-row-tables).
...
In fact, you can specify any column definition you like for the `REGIONAL BY ROW AS` column, as long as the column is of type `crdb_internal_region` and is not nullable. For example, you could modify the [movr schema](/docs/v23.2/movr#the-movr-database) to have a region column generated as:
...
When auto-rehoming is enabled, the [home regions](#crdb_region) of rows in [`REGIONAL BY ROW`](#set-the-table-locality-to-regional-by-row) tables are automatically set to the gateway region of any [`UPDATE`](/docs/v23.2/update) or [`UPSERT`](/docs/v23.2/upsert) statements that write to those rows. This functionality is provided by adding an [`ON UPDATE` expression](/docs/v23.2/create-table#on-update-expressions) to the [`crdb_region`](#crdb_region) column of newly created regional by row tables.

To enable auto-rehoming using the [session setting](/docs/v23.2/set-vars), issue the following statement:
...
- The [`crdb_region`](#crdb_region) column from a [`REGIONAL BY ROW`](#set-the-table-locality-to-regional-by-row) table cannot be referenced as a [foreign key](/docs/v23.2/foreign-key) from another table.
...
1. Follow steps 1 and 2 from the [Low Latency Reads and Writes in a Multi-Region Cluster](/docs/v23.2/demo-low-latency-multi-region-deployment) tutorial. This will involve starting a [`cockroach demo`](/docs/v23.2/cockroach-demo) cluster in a terminal window (call it *terminal 1*).
2. From the [SQL client](/docs/v23.2/cockroach-sql) running in terminal 1, set the setting that enables auto-rehoming. You must issue this setting before creating the `REGIONAL BY ROW` tables that you want auto-rehomed.

    ```sql
    SET enable_auto_rehoming = on;
    ```
3. In a second terminal window (call it *terminal 2*), [finish the tutorial starting from step 3](/docs/v23.2/demo-low-latency-multi-region-deployment#step-3-load-and-run-movr) onward to finish loading the cluster with data and applying the multi-region SQL configuration.
4. Switch back to terminal 1, and check the gateway region of the node you are currently connected to:
    ...
    (1 row)
    ```
5. Open another terminal (call it *terminal 3*), and use [`cockroach sql`](/docs/v23.2/cockroach-sql) to connect to a node in a different region in the demo cluster:
    ...
    # To exit, type: \q.
    #
    # Server version: CockroachDB CCL v23.2.29 (x86_64-apple-darwin19, built 2026-02-19 00:00:00) (same version as client)
...
For more information about how this table locality works, see [Global tables](/docs/v23.2/table-localities#global-tables).
...
By default, [unqualified tables](/docs/v23.2/sql-name-resolution#lookup-with-unqualified-names) created in the database belong to the `public` schema:
...
The table's compound primary key is on the `city` and `dl` columns. Note that the table automatically generates an `id` and a `dl` value [using supported SQL functions](/docs/v23.2/functions-and-operators) if they are not provided.
...
You can specify the time at which a split enforcement expires by adding a `WITH EXPIRATION` clause to your `SPLIT` statement. Supported expiration values include [`DECIMAL`](/docs/v23.2/decimal), [`INTERVAL`](/docs/v23.2/interval), [`TIMESTAMP`](/docs/v23.2/timestamp), and [`TIMESTAMPZ`](/docs/v23.2/timestamp).
...
You can see the split's expiration date in the `split_enforced_until` column. The [`crdb_internal.ranges`](/docs/v23.2/crdb-internal) table also contains information about ranges in your CockroachDB cluster, including the `split_enforced_until` column.
...
<Note>
If present in a [`CREATE TABLE`](/docs/v23.2/create-table) statement, the table is considered validated because an empty table trivially meets its constraints.
</Note>
...
- [Foreign Key Constraint](/docs/v23.2/foreign-key)
- [Replication Controls](/docs/v23.2/configure-replication-zones)
- [SQL Audit Logging](/docs/v23.2/sql-audit-logging)
- [`CREATE TABLE`](/docs/v23.2/create-table)
- [`CREATE INDEX`](/docs/v23.2/create-index)
- [`ALTER INDEX`](/docs/v23.2/alter-index)
- [`ALTER PARTITION`](/docs/v23.2/alter-partition)
- [`SHOW JOBS`](/docs/v23.2/show-jobs)
- [`BACKUP`](/docs/v23.2/backup)
- [`RESTORE`](/docs/v23.2/restore)
- [`WITH` (storage parameter)](/docs/v23.2/with-storage-parameter)
- [Selection Queries](/docs/v23.2/selection-queries)
- [Distribution Layer](/docs/v23.2/architecture/distribution-layer)
- [Replication Layer](/docs/v23.2/architecture/replication-layer)
- [Online Schema Changes](/docs/v23.2/online-schema-changes)
