...
To view this dashboard, [access the DB Console](/docs/v23.2/ui-overview#db-console-access), click **Metrics** in the left-hand navigation, and select **Dashboard** &gt; **Networking**.

For additional information about node connectivity conditions, refer to the [Network page](/docs/v23.2/ui-network-latency-page).
...
Metric: [`sys.host.net.recv.bytes`](/docs/v23.2/essential-metrics-self-hosted#sys-host-net-recv-bytes) Bytes received on all network interfaces since the `cockroach` process started
...
Metric: [`sys.host.net.send.bytes`](/docs/v23.2/essential-metrics-self-hosted#sys-host-net-send-bytes) Bytes sent on all network interfaces since the `cockroach` process started
...
RPC heartbeat latency is the round-trip latency for recent successful outgoing heartbeats. It is the distribution of round-trip latencies with other nodes. This only reflects successful heartbeats and measures [gRPC](/docs/v23.2/architecture/distribution-layer#grpc) overhead as well as possible [head-of-line blocking](https://wikipedia.org/wiki/Head-of-line_blocking). Elevated values in this metric may hint at network issues or saturation or both, but they are not proof of them. [CPU overload](/docs/v23.2/cluster-setup-troubleshooting#cpu-issues) can similarly elevate this metric. To conclusively diagnose network issues, look at OS-level metrics such as packet loss and retransmits. Heartbeats are not very frequent (every 1 second), so they may not capture rare or short-lived degradations.
