...
A user-defined function (UDF) is a named function defined at the database level that can be called in queries and other contexts. CockroachDB supports invoking UDFs in `SELECT`, `FROM`, and `WHERE` clauses of [DML statements](/docs/v23.2/sql-statements#data-manipulation-statements).

Both [stored procedures](/docs/v23.2/stored-procedures) and user-defined functions are types of *routines*. However, they differ in the following ways:
...
- An argument has a *mode* and a *type*. CockroachDB supports the `IN` argument mode. The type can be a built-in type, [user-defined `ENUM`](/docs/v23.2/enum), or implicit record type. CockroachDB **does not** support default values for arguments.
- The return type can be a built-in [SQL type](/docs/v23.2/data-types), user-defined [`ENUM`](/docs/v23.2/enum), [`RECORD`](/docs/v23.2/create-function#create-a-function-that-returns-a-record-type), PL/pgSQL [`REFCURSOR`](/docs/v23.2/plpgsql#declare-cursor-variables) type, implicit record type, or `VOID`.
    - Preceding a type with `SETOF` indicates that a set, or multiple rows, may be returned. For an example, see [Create a function that returns a set of results](/docs/v23.2/create-function#create-a-function-that-returns-a-set-of-results).
    - `VOID` indicates that there is no return type and `NULL` will always be returned.
- The [volatility](/docs/v23.2/functions-and-operators#function-volatility) indicates whether the function has side effects. `VOLATILE` and `NOT LEAKPROOF` are the default.
    - Annotate a function with side effects with `VOLATILE`. This also prevents the [cost-based optimizer](/docs/v23.2/cost-based-optimizer) from pre-evaluating the function.
...
- `LANGUAGE` specifies the language of the function body. CockroachDB supports the languages `SQL` and [`PLpgSQL` (PL/pgSQL)](/docs/v23.2/plpgsql).
- The function body:
...
For more examples of UDF creation, see [`CREATE FUNCTION`](/docs/v23.2/create-function).
...
You invoke a UDF like a [built-in function](/docs/v23.2/functions-and-operators).
...
When possible, the [cost-based optimizer](/docs/v23.2/cost-based-optimizer) will improve a function's performance by inlining the UDF within the query plan. The UDF must have the following attributes:

- It is labeled as `IMMUTABLE`, `STABLE`, or `LEAKPROOF` (i.e., non- `VOLATILE` ).
- It has a single statement.
- It is not a [set-returning function](/docs/v23.2/create-function#create-a-function-that-returns-a-set-of-results).
- Its arguments are only variable or constant expressions.
- It is not a [record-returning function](/docs/v23.2/create-function#create-a-function-that-returns-a-record-type).
...
5. View the query plan when using `foo_s()` (the `STABLE` function) instead:
    ...
    The query takes only `4ms` to execute because the function is inlined and transformed to a [join](/docs/v23.2/joins) with an equality comparison `(a) = (b)`, which has much less overhead than invoking a function for each row scanned in table `a`.
...
- References to other user-defined functions.
    ...
    [Tracking GitHub issue](https://github.com/cockroachdb/cockroach/issues/93049)
- [DDL statements](/docs/v23.2/sql-statements#data-definition-statements) (e.g., `CREATE TABLE`, `CREATE INDEX`).
