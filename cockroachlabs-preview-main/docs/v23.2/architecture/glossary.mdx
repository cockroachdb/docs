...
The degree to which a transaction may be affected by other transactions running at the same time. CockroachDB provides the [`SERIALIZABLE`](https://wikipedia.org/wiki/Serializability) and `READ COMMITTED` isolation levels. For more information, see [Isolation levels](/docs/v23.2/transactions#isolation-levels).
...
A [state of conflict](/docs/v23.2/performance-best-practices-overview#transaction-contention) that occurs when:
...
- A transaction is [automatically retried](/docs/v23.2/transactions#automatic-retries) because it could not be placed into a [serializable ordering](/docs/v23.2/demo-serializable) among all of the currently executing transactions. This is also called a *serializability conflict*. If the automatic retry is not possible or fails, a [*transaction retry error*](/docs/v23.2/transaction-retry-error-reference) is emitted to the client, requiring the client application to [retry the transaction](/docs/v23.2/transaction-retry-error-reference#client-side-retry-handling).

Steps should be taken to [reduce transaction contention](/docs/v23.2/performance-best-practices-overview#reduce-transaction-contention) in the first place.
...
A CockroachDB cluster can be run in CockroachDB Cloud, within a customer [Organization](/docs/v23.2/architecture/glossary#organization), or can be self-hosted.
...
From a SQL perspective, a table and its secondary indexes initially map to a single range, where each key-value pair in the range represents a single row in the table (also called the *primary index* because the table is sorted by the primary key) or a single row in a secondary index. As soon as the size of a range reaches [the default range size](/docs/v23.2/configure-replication-zones#range-max-bytes), it is [split into two ranges](/docs/v23.2/architecture/distribution-layer#range-splits). This process continues for these new ranges as the table and its indexes continue growing.

### Replica

A copy of a range stored on a node. By default, there are three [replicas](/docs/v23.2/configure-replication-zones#num_replicas) of each range on different nodes.
...
The [consensus protocol](/docs/v23.2/architecture/replication-layer#raft) employed in CockroachDB that ensures that your data is safely stored on multiple nodes and that those nodes agree on the current state even if some of them are temporarily disconnected.
...
For more information on CockroachDB architecture, see [Architecture Overview](/docs/v23.2/architecture/overview#overview).

## CockroachDB deployment terms

#### Region

A logical identification of how nodes and data are clustered around [geographical locations](/docs/v23.2/multiregion-overview). A *cluster region* is the set of locations where cluster nodes are running. A *database region* is the subset of cluster regions database data should be restricted to.
...
#### [CockroachDB Self-Hosted](/docs/v23.2/start-a-local-cluster)
...
<Note>
This section is provided for reference purposes only. The inclusion of a term in this glossary does not imply that CockroachDB has support for any feature(s) related to that term. For more information about the specific spatial and GIS features supported by CockroachDB, see [Working with Spatial Data](/docs/v23.2/query-spatial-data).
</Note>
...
The Spatial Referencing System Identifier (a.k.a. SRID) is used to tell which spatial reference system will be used to interpret each spatial object. A [commonly used SRID is 4326](/docs/v23.2/srid-4326), which represents spatial data using longitude and latitude coordinates on the Earth's surface as defined in the [WGS84](#wgs84) standard.
...
Used to represent shapes relative to 2-, 3-, or higher-dimensional plane geometry. For more information about the spatial objects used to represent geometries, see: - [`POINT`](/docs/v23.2/point) - [`LINESTRING`](/docs/v23.2/linestring) - [`POLYGON`](/docs/v23.2/polygon) - [`MULTIPOINT`](/docs/v23.2/multipoint) - [`MULTILINESTRING`](/docs/v23.2/multilinestring) - [`MULTIPOLYGON`](/docs/v23.2/multipolygon) - [`GEOMETRYCOLLECTION`](/docs/v23.2/geometrycollection)
...
The "Well Known Text" data format is a convenient human-readable notation for representing [spatial objects](#spatial-objects). For example a 2-dimensional point object with x- and y-coordinates is represented in WKT as `POINT(123,456)`. This format is defined by the [OGC](#ogc). For more information, see the [Well Known Text](/docs/v23.2/well-known-text) documentation.

#### EWKT

The "Extended Well Known Text" data format extends [WKT](#wkt) by prepending an [SRID](#srid) to the shape's description. For more information, see the [Well Known Text](/docs/v23.2/well-known-text#ewkt) documentation.

#### WKB

The "Well Known Binary" data format is a convenient machine-readable binary representation for [spatial objects](#spatial-objects). For efficiency, an application may choose to use this data format, but humans may prefer to read [WKT](#wkt). This format is defined by the [OGC](#ogc). For more information, see [Well Known Binary](/docs/v23.2/well-known-binary).

#### EWKB

The "Extended Well Known Binary" data format extends [WKB](#wkb) by prepending [SRID](#srid) information to the shape's description. For more information, see [Well Known Binary](/docs/v23.2/well-known-binary#ewkb).
