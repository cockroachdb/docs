...
By default, CockroachDB automatically generates full statistics when tables are [created](/docs/v23.2/create-table) and after [schema changes](/docs/v23.2/online-schema-changes). Full statistics for a table are automatically refreshed when approximately 20% of its rows are updated.

<Tip>
You can manually collect *partial statistics* on a subset of table data without scanning the full table. Refer to [Create partial statistics using extremes](/docs/v23.2/create-statistics#create-partial-statistics-using-extremes).
</Tip>

A [background job](/docs/v23.2/create-statistics#view-statistics-jobs) automatically determines which columns to get statistics on. Specifically, the optimizer chooses:

- Columns that are part of the primary key or an index (in other words, all indexed columns).
- Up to 100 non-indexed columns.

By default, CockroachDB also automatically collects [multi-column statistics](/docs/v23.2/create-statistics#create-statistics-on-multiple-columns) on columns that prefix an index.
...
- After a successful [`IMPORT`](/docs/v23.2/import) or [`RESTORE`](/docs/v23.2/restore) into the table.
- After any schema change affecting the table.
- After each mutation operation ([`INSERT`](/docs/v23.2/insert), [`UPDATE`](/docs/v23.2/update), or [`DELETE`](/docs/v23.2/delete)), the probability of a refresh is calculated using a formula that takes the [cluster settings](/docs/v23.2/cluster-settings) shown in the following table as inputs. These settings define the target number of rows in a table that must be stale before statistics on that table are refreshed. Increasing either setting will reduce the frequency of refreshes. In particular, `min_stale_rows` impacts the frequency of refreshes for small tables, while `fraction_stale_rows` has more of an impact on larger tables.

    <table><thead><tr><th>Setting</th><th>Default Value</th><th>Details</th></tr></thead><tbody><tr><td><code>sql.stats.automatic&#95;collection.fraction&#95;stale&#95;rows</code></td><td>0.2</td><td>Target fraction of stale rows per table that will trigger a statistics refresh.</td></tr><tr><td><code>sql.stats.automatic&#95;collection.min&#95;stale&#95;rows</code></td><td>500</td><td>Target minimum number of stale rows per table that will trigger a statistics refresh.</td></tr></tbody></table>
...
Suppose the [clusters settings](/docs/v23.2/cluster-settings) `sql.stats.automatic_collection.fraction_stale_rows` and `sql.stats.automatic_collection.min_stale_rows` have the default values.2 and 500 as shown in the preceding table.
...
1. Set the `sql.stats.automatic_collection.enabled` cluster setting to `false`:
    ...
    ```
2. Use the [`SHOW STATISTICS`](/docs/v23.2/show-statistics) statement to view automatically generated statistics.
...
To learn how to manually generate statistics, see the [`CREATE STATISTICS` examples](/docs/v23.2/create-statistics#examples).
...
You can enable and disable automatic statistics collection for individual tables using the `sql_stats_automatic_collection_enabled` [table storage parameter](/docs/v23.2/with-storage-parameter#table-parameters). This table setting **takes precedence** over the `sql.stats.automatic_collection.enabled` [cluster setting](/docs/v23.2/cluster-settings) described in [Enable and disable automatic statistics collection for clusters](#enable-and-disable-automatic-statistics-collection-for-clusters).
...
Or by using [`ALTER TABLE... SET`](/docs/v23.2/alter-table#set-storage-parameter):
...
`ALTER TABLE accounts RESET (sql_stats_automatic_collection_enabled)` removes the table setting, in which case the `sql.stats.automatic_collection.enabled` [cluster setting](/docs/v23.2/cluster-settings) is in effect for the table.
...
CockroachDB deletes statistics on non-default columns according to the `sql.stats.non_default_columns.min_retention_period` [cluster setting](/docs/v23.2/cluster-settings), which defaults to 24 hours.
...
`ALTER TABLE accounts RESET (sql_stats_forecasts_enabled)` removes the table setting, in which case the `sql.stats.forecasts.enabled` [cluster setting](/docs/v23.2/cluster-settings) is in effect for the table.

For details on forecasted statistics, see [Display forecasted statistics](/docs/v23.2/show-statistics#display-forecasted-statistics).

### Control histogram collection

By default, the optimizer collects histograms for all index columns (specifically the first column in each index) during automatic statistics collection. If a single column statistic is explicitly requested using manual invocation of [`CREATE STATISTICS`](/docs/v23.2/create-statistics), a histogram will be collected, regardless of whether or not the column is part of an index.

<Note>
CockroachDB does not support:

- Histograms on [`ARRAY`-typed](/docs/v23.2/array) columns. As a result, statistics created on `ARRAY` -typed columns do not include histograms.
- Multi-column histograms.
</Note>

If you are an advanced user and need to disable histogram collection for troubleshooting or performance tuning reasons, change the [`sql.stats.histogram_collection.enabled` cluster setting](/docs/v23.2/cluster-settings) by running [`SET CLUSTER SETTING`](/docs/v23.2/set-cluster-setting) as follows:

```sql
SET CLUSTER SETTING sql.stats.histogram_collection.enabled = false;
```

When `sql.stats.histogram_collection.enabled` is set to `false`, histograms are never collected, either as part of automatic statistics collection or by manually invoking [`CREATE STATISTICS`](/docs/v23.2/create-statistics).
...
We recommend that you allow the optimizer to consider column size when costing plans. If you are an advanced user and need to disable using `avg_size` for troubleshooting or performance tuning reasons, set the [`cost_scans_with_default_col_size` session variable](/docs/v23.2/set-vars#cost-scans-with-default-col-size) to `true` with `SET cost_scans_with_default_col_size=true`.

## Control whether the optimizer creates a plan with a full scan

Even if you have [secondary indexes](/docs/v23.2/schema-design-indexes), the optimizer may determine that a full table scan will be faster. For example, if you add a secondary index to a table with a large number of rows and find that a statement plan is not using the secondary index, then it is likely that performing a full table scan using the primary key is faster than doing a secondary index scan plus an [index join](/docs/v23.2/indexes#example).

You can disable statement plans that perform full table scans with the [`disallow_full_table_scans` session variable](/docs/v23.2/set-vars#disallow-full-table-scans).
...
You can specify [whether an index is visible](/docs/v23.2/alter-index#not-visible) to the cost-based optimizer. By default, indexes are visible. If not visible, the index will not be used in queries unless it is specifically selected with an [index hint](/docs/v23.2/indexes#selection). This allows you to create an index and check for query plan changes without affecting production queries. For an example, refer to [Set an index to be not visible](/docs/v23.2/alter-index#set-an-index-to-be-not-visible).

You can also set an index as [partially visible](/docs/v23.2/alter-index#visibility) within a range of `0.0` to `1.0`, where `0.0` means not visible and `1.0` means visible. Any value between `0.0` and `1.0` means that an index is visible to the specified fraction of queries. For the purposes of [index recommendations](/docs/v23.2/explain#success-responses), partially visible indexes are treated as [not visible](/docs/v23.2/alter-index#not-visible). If a partially visible index can be used to improve a query plan, the optimizer will recommend making it fully visible. For an example, refer to [Set an index as partially visible](/docs/v23.2/alter-index#set-an-index-as-partially-visible).

<Note>
Indexes that are not visible are still used to enforce `UNIQUE` and `FOREIGN KEY` [constraints](/docs/v23.2/constraints). For more considerations, see [Index visibility considerations](/docs/v23.2/alter-index#not-visible).
</Note>

You can instruct the optimizer to use indexes marked as not visible with the [`optimizer_use_not_visible_indexes` session variable](/docs/v23.2/set-vars#optimizer-use-not-visible-indexes). By default, the variable is set to `off`.

## Locality optimized search in multi-region clusters

In [multi-region deployments](/docs/v23.2/multiregion-overview) with [regional by row tables](/docs/v23.2/table-localities#regional-by-row-tables), the optimizer, in concert with the [SQL engine](/docs/v23.2/architecture/sql-layer), may perform a *locality optimized search* to attempt to avoid high-latency, cross-region communication between nodes. If there is a possibility that the results of a query all live in local rows, the database will first search for rows in the gateway node's region. The search only continues in remote regions if rows in the local region did not satisfy the query. Examples of queries that can use locality optimized search include unique key lookups and queries with [`LIMIT`](/docs/v23.2/limit-offset) clauses.
...
<Note>
The asynchronous parallel lookup behavior does not occur if you [disable vectorized execution](/docs/v23.2/vectorized-execution#configure-vectorized-execution).
</Note>
...
Although the optimizer prefers to [read from rows in local regions](#locality-optimized-search-in-multi-region-clusters) when possible, by default, it does not guarantee that any query will not visit a remote region. This can occur if a query has no [home region](/docs/v23.2/multiregion-overview#table-localities) (for example, if it reads from different home regions in a [regional by row table](/docs/v23.2/table-localities#regional-by-row-tables)) or a query's home region differs from the [gateway](/docs/v23.2/architecture/life-of-a-distributed-transaction#gateway) region.

For some latency-sensitive applications, cross-region latency may not be acceptable. In these cases, set the [`enforce_home_region` session variable](/docs/v23.2/show-vars#enforce-home-region) to `on`. This configures the optimizer to return one of the following error types, and in some cases a suggested resolution, if a query cannot be run entirely in a single region:

- `Query has no home region`. The optimizer provides a hint on how to run the query in a single region.
- `Query is not running in its home region`. The optimizer provides a hint containing the home region of the query. The application should disconnect and then reconnect with a [connection string](/docs/v23.2/connection-parameters) specifying a node in the query's home region.

Only tables with `ZONE` [survivability](/docs/v23.2/multiregion-survival-goals#when-to-use-zone-vs-region-survival-goals) can be scanned without error when this setting is enabled.
...
The following statements can use the plan cache: [`SELECT`](/docs/v23.2/select-clause), [`INSERT`](/docs/v23.2/insert), [`UPDATE`](/docs/v23.2/update), [`UPSERT`](/docs/v23.2/upsert), and [`DELETE`](/docs/v23.2/delete).
...
- <InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v23.2.12: *Generic* query plans are generated and optimized once without considering specific placeholder values, and are **not** regenerated on subsequent executions, unless the plan becomes stale due to [schema changes](/docs/v23.2/online-schema-changes) or new [table statistics](#table-statistics) and must be re-optimized. This approach eliminates most of the query latency attributed to planning. For example:
    ...
    This feature is in [preview](/docs/v23.2/cockroachdb-feature-availability) and is subject to change.

    <Tip>
    Generic query plans will only benefit workloads that use prepared statements, which are issued via explicit `PREPARE` statements or by client libraries using the [PostgreSQL extended wire protocol](https://www.postgresql.org/docs/current/protocol-flow#PROTOCOL-FLOW-EXT-QUERY). Generic query plans are most beneficial for queries with high planning times, such as queries with many [joins](/docs/v23.2/joins). For more information on reducing planning time for such queries, refer to [Reduce planning time for queries with many joins](#reduce-planning-time-for-queries-with-many-joins).
    </Tip>

To change the type of plan that is cached, use the [`plan_cache_mode`](/docs/v23.2/session-variables#plan-cache-mode) session setting. This setting applies when a statement is executed, not when it is prepared. Statements are therefore not associated with a specific query plan type when they are prepared.
...
<Note>
Generic plans are always used for non-prepared statements that do not contain placeholders or [stable functions](/docs/v23.2/functions-and-operators#function-volatility), regardless of the `plan_cache_mode` setting.
</Note>
...
At the [database level](/docs/v23.2/alter-database#set-session-variable):

```sql
ALTER DATABASE db SET plan_cache_mode = auto;
```

At the [role level](/docs/v23.2/alter-role#set-default-session-variable-values-for-a-role):

```sql
ALTER ROLE db_user SET plan_cache_mode = auto;
```

To verify the plan type used by a query, check the [`EXPLAIN ANALYZE`](/docs/v23.2/explain-analyze) output for the query.
...
For a query involving multiple joins, the cost-based optimizer will explore additional [join orderings](/docs/v23.2/joins) in an attempt to find the lowest-cost execution plan, which can lead to significantly better performance in some cases.
...
To change this setting, which is controlled by the `reorder_joins_limit` [session variable](/docs/v23.2/set-vars), run the following statement:

```sql
SET reorder_joins_limit = 0;
```

To disable this feature, set the variable to `0`. You can configure the default `reorder_joins_limit` session setting with the [cluster setting](/docs/v23.2/cluster-settings) `sql.defaults.reorder_joins_limit`, which has a default value of `8`.
...
- To limit the size of the subtree that can be reordered, set the `reorder_joins_limit` [session variable](/docs/v23.2/set-vars) to a lower value, for example:
...
For a join hint example, see [Use the right join type](/docs/v23.2/apply-statement-performance-rules#rule-3-use-the-right-join-type).
...
- `INVERTED`: Forces an inverted join into the right side; the right side must be a table with a suitable [GIN index](/docs/v23.2/inverted-indexes). Note that `INVERTED` can only be used with `INNER` and `LEFT` joins.

    <Note>
    You cannot use inverted joins on [partial GIN indexes](/docs/v23.2/inverted-indexes#partial-gin-indexes).
    </Note>
...
The optimizer supports index hints to prevent or force a zigzag join. Apply the hints in the same way as other existing [index hints](/docs/v23.2/table-expressions#force-index-selection).
...
You can omit the `INNER INVERTED JOIN` statement by putting `v1.ext` on the left side of a `@>` join condition in a `WHERE` clause and using an [index hint](/docs/v23.2/table-expressions#force-index-selection) for the GIN index.
...
- [`JOIN` expressions](/docs/v23.2/joins)
