...
Specify the labels in `nodeSelector` in the Operator's custom resource, which is used to [deploy the cluster](/docs/v23.2/deploy-cockroachdb-with-kubernetes#initialize-the-cluster). If you specify multiple `nodeSelector` labels, the node must match all of them.
...
Specify node affinities in `affinity.nodeAffinity` in the Operator's custom resource, which is used to [deploy the cluster](/docs/v23.2/deploy-cockroachdb-with-kubernetes#initialize-the-cluster). If you specify multiple `matchExpressions` labels, the node must match all of them. If you specify multiple `values` for a label, the node can match any of the values.
...
Specify pod affinities and anti-affinities in `affinity.podAffinity` and `affinity.podAntiAffinity` in the Operator's custom resource, which is used to [deploy the cluster](/docs/v23.2/deploy-cockroachdb-with-kubernetes#initialize-the-cluster). If you specify multiple `matchExpressions` labels, the node must match all of them. If you specify multiple `values` for a label, the node can match any of the values.
...
3. Add the following rules to the Operator's custom resource, which is used to [deploy the cluster](/docs/v23.2/deploy-cockroachdb-with-kubernetes#initialize-the-cluster):
...
Specify pod tolerations in the `tolerations` object of the Operator's custom resource, which is used to [deploy the cluster](/docs/v23.2/deploy-cockroachdb-with-kubernetes#initialize-the-cluster).
...
A `NoSchedule` taint on a node prevents pods from being scheduled onto the node. The matching toleration allows a pod to be scheduled onto the node. A `NoSchedule` toleration is therefore best included before [deploying the cluster](/docs/v23.2/deploy-cockroachdb-with-kubernetes#initialize-the-cluster).
...
2. Add a taint to a running worker node:
    ...
    ```
3. Add a matching `tolerations` object to the Operator's custom resource, which was used to [deploy the cluster](/docs/v23.2/deploy-cockroachdb-with-kubernetes#initialize-the-cluster):
...
Specify pod topology spread constraints in the `topologySpreadConstraints` object of the Operator's custom resource, which is used to [deploy the cluster](/docs/v23.2/deploy-cockroachdb-with-kubernetes#initialize-the-cluster). If you specify multiple `topologySpreadConstraints` objects, the matching pods must satisfy all of the constraints.
...
Specify labels in `additionalLabels` and annotations in `additionalAnnotations` in the Operator's custom resource, which is used to [deploy the cluster](/docs/v23.2/deploy-cockroachdb-with-kubernetes#initialize-the-cluster):

```yaml
spec:
  additionalLabels:
    app.kubernetes.io/version: v23.2.29
