...
old_description: "How to apply SQL statement performance rules to optimize a query."
---

This tutorial shows how to apply [SQL statement performance rules](/docs/v23.2/make-queries-fast#sql-statement-performance-rules) to optimize a query against the [`movr` example dataset](/docs/v23.2/cockroach-demo#datasets).
...
First, study the schema so you understand the relationships between the tables. Run [`SHOW TABLES`](/docs/v23.2/show-tables):
...
This means that to get the information you want, you'll need to do a [join](/docs/v23.2/joins) on the `users` and `rides` tables.
...
As specified by your [`cockroach demo`](/docs/v23.2/cockroach-demo) command, the `users` table has 12,500 records, and the `rides` table has 125,000 records. Because it's so large, you want to avoid scanning the entire `rides` table in your query. In this case, you can avoid scanning `rides` using an index, as shown in the next section.
...
You can see why if you look at the output of [`EXPLAIN`](/docs/v23.2/explain):
...
The main problem is that you are doing full table scans on both the `users` and `rides` tables (see `spans: FULL SCAN`). This tells you that you do not have indexes on the columns in your `WHERE` clause, which is [an indexing best practice](/docs/v23.2/indexes#best-practices).
...
Before creating any more indexes, let's see what indexes already exist on the `rides` table by running [`SHOW INDEXES`](/docs/v23.2/show-index):
...
Because another performance best practice is to [create an index on the `WHERE` condition storing the join key](/docs/v23.2/sql-tuning-with-explain#solution-create-a-secondary-index-on-the-where-condition-storing-the-join-key), create an index on `start_time` that stores the join key `rider_id`:
...
To see what changed, look at the [`EXPLAIN`](/docs/v23.2/explain) output:
...
Out of the box, the [cost-based optimizer](/docs/v23.2/cost-based-optimizer) will select the right join type for your statement in the majority of cases. Therefore, you should only provide [join hints](/docs/v23.2/cost-based-optimizer#join-hints) in your query if you can **prove** to yourself through experimentation that the optimizer should be using a different [join type](/docs/v23.2/joins#join-algorithms) than it is selecting.
...
For example, you might think that a [lookup join](/docs/v23.2/joins#lookup-joins) could perform better in this instance, since one of the tables in the join is 10x smaller than the other.
