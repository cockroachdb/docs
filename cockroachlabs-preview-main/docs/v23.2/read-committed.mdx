...
<InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v23.2: `READ COMMITTED` is one of two [transaction isolation levels](https://wikipedia.org/wiki/Isolation_(database_systems)) supported on CockroachDB. By default, CockroachDB uses the [`SERIALIZABLE`](/docs/v23.2/demo-serializable) isolation level, which is the strongest [ANSI transaction isolation level](https://wikipedia.org/wiki/Isolation_(database_systems)#Isolation_levels).

`READ COMMITTED` isolation is appropriate in the following scenarios:

- Your application needs to maintain a high workload concurrency with minimal [transaction retries](/docs/v23.2/developer-basics#transaction-retries), and it can tolerate potential [concurrency anomalies](#concurrency-anomalies). Predictable query performance at high concurrency is more valuable than guaranteed transaction [serializability](/docs/v23.2/developer-basics#serializability-and-transaction-contention).
...
Whereas `SERIALIZABLE` isolation guarantees data correctness by placing transactions into a [serializable ordering](/docs/v23.2/demo-serializable), `READ COMMITTED` isolation permits some [concurrency anomalies](#concurrency-anomalies) in exchange for minimizing transaction aborts, [retries](/docs/v23.2/developer-basics#transaction-retries), and blocking. Compared to `SERIALIZABLE` transactions, `READ COMMITTED` transactions return fewer [serialization errors](/docs/v23.2/transaction-retry-error-reference) that require client-side handling. See [`READ COMMITTED` transaction behavior](#read-committed-transaction-behavior).
...
- The [`SET SESSION CHARACTERISTICS`](/docs/v23.2/set-vars#special-syntax-cases) statement, which applies to the current session:

    ```sql
    SET SESSION CHARACTERISTICS AS TRANSACTION ISOLATION LEVEL READ COMMITTED;
    ```
- The [`default_transaction_isolation`](/docs/v23.2/session-variables#default-transaction-isolation) session variable:
    ...
    ```
- The `default_transaction_isolation` session variable as a [connection parameter](/docs/v23.2/connection-parameters#supported-options-parameters) with [`cockroach sql`](/docs/v23.2/cockroach-sql):
...
- The [`BEGIN TRANSACTION ISOLATION LEVEL`](/docs/v23.2/begin-transaction#parameters) statement:

    ```sql
    BEGIN TRANSACTION ISOLATION LEVEL READ COMMITTED;
    ```
- The [`SET TRANSACTION ISOLATION LEVEL`](/docs/v23.2/set-transaction#parameters) statement, at the beginning of the transaction:

    ```sql
    BEGIN;
      SET TRANSACTION ISOLATION LEVEL READ COMMITTED;
    ```
- The [`transaction_isolation`](/docs/v23.2/session-variables#transaction-isolation) session variable, at the beginning of the transaction:
...
`READ COMMITTED` and `SERIALIZABLE` transactions both serve globally consistent ("non-stale") reads and [commit atomically](/docs/v23.2/developer-basics#how-transactions-work-in-cockroachdb). `READ COMMITTED` transactions have the following differences:

- Writes in concurrent `READ COMMITTED` transactions can interleave without aborting transactions, and a write can never block a non-locking read of the same row. This is because `READ COMMITTED` transactions are not required to be placed into a [serializable ordering](/docs/v23.2/demo-serializable).
- Whereas statements in `SERIALIZABLE` transactions see data that committed before the transaction began, statements in `READ COMMITTED` transactions see data that committed before each **statement** began. If rows are being updated by concurrent writes, reads in a `READ COMMITTED` transaction can [return different results](#non-repeatable-reads-and-phantom-reads).
    ...
    <Note>
    For details on how this is implemented, see [Read snapshots](/docs/v23.2/architecture/transaction-layer#read-snapshots).
    </Note>
...
- Transactions at all isolation levels are subject to [*lock contention*](/docs/v23.2/performance-best-practices-overview#transaction-contention), where a transaction attempts to lock a row that is already locked by a [write](/docs/v23.2/architecture/transaction-layer#write-intents) or [locking read](#locking-reads). In such cases, the later transaction is blocked until the earlier transaction commits or rolls back, thus releasing its lock on the row. Lock contention that produces a *deadlock* between two transactions will result in a transaction abort and a `40001` error ([`ABORT_REASON_ABORTED_RECORD_FOUND`](/docs/v23.2/transaction-retry-error-reference#abort_reason_aborted_record_found) or [`ABORT_REASON_PUSHER_ABORTED`](/docs/v23.2/transaction-retry-error-reference#abort_reason_pusher_aborted)) returned to the client.
- [Constraint](/docs/v23.2/constraints) violations will abort transactions at all isolation levels.
- In rare cases under `READ COMMITTED` isolation, a [`RETRY_WRITE_TOO_OLD`](/docs/v23.2/transaction-retry-error-reference#retry_write_too_old) or [`ReadWithinUncertaintyIntervalError`](/docs/v23.2/transaction-retry-error-reference#readwithinuncertaintyintervalerror) error can be returned to the client if a statement has already begun streaming a partial result set back to the client and cannot retry transparently. By default, the result set is buffered up to 16 KiB before overflowing and being streamed to the client. You can configure the result buffer size using the [`sql.defaults.results_buffer.size`](/docs/v23.2/cluster-settings#setting-sql-defaults-results-buffer-size) cluster setting.

### Concurrency anomalies

Statements in concurrent `READ COMMITTED` transactions can interleave with each other. This can create concurrency anomalies that are not permitted under `SERIALIZABLE` isolation, which places concurrent transactions into a [serializable ordering](/docs/v23.2/demo-serializable).

<Tip>
The behaviors described in this section assume the use of non-locking reads. You can prevent concurrency anomalies through the selective use of [locking reads](#locking-reads), which can also increase latency due to [lock contention](/docs/v23.2/performance-best-practices-overview#transaction-contention).
</Tip>
...
<Note>
Under `SERIALIZABLE` isolation, transaction `A` would have aborted with a [`RETRY_WRITE_TOO_OLD`](/docs/v23.2/transaction-retry-error-reference#retry_write_too_old) error, prompting the client to retry the transaction.
</Note>
...
The update in Session 2 appears to be "lost" because its result is overwritten by a concurrent transaction. It is **not** lost at the database level, and can be found using [`AS OF SYSTEM TIME`](/docs/v23.2/as-of-system-time) and a timestamp earlier than the commit in Session 1:
...
<Note>
For details on why this is allowed, see [Read refreshing](/docs/v23.2/architecture/transaction-layer#write-skew-tolerance).
</Note>
...
1. Open the SQL shell using [`cockroach demo`](/docs/v23.2/cockroach-demo).
...
In a new terminal (Session 2), open the SQL shell on your [`cockroach demo`](/docs/v23.2/cockroach-demo) cluster. Start a transaction:
...
[By design under `READ COMMITTED` isolation](/docs/v23.2/architecture/transaction-layer#write-skew-tolerance), CockroachDB allows the transaction to commit even though its previous read (the `SELECT` query) has changed due to the concurrent transaction in Session 2.
...
Check to make sure that another doctor is on call for `2023-12-05`. Use [`FOR UPDATE`](/docs/v23.2/select-for-update) to lock the rows so that only the current transaction can update them:
...
Check to make sure that another doctor is on call for `2023-12-05`. Use [`FOR SHARE`](/docs/v23.2/select-for-update) to lock the rows so that they cannot be updated by another transaction:
...
- [`SKIP LOCKED`](/docs/v23.2/select-for-update#wait-policies) requests do not check for [replicated locks](/docs/v23.2/architecture/transaction-layer#unreplicated-locks), which can be acquired by `READ COMMITTED` transactions.

The following affect the performance of `READ COMMITTED` transactions:

- Because locks acquired by [foreign key](/docs/v23.2/foreign-key) checks, [`SELECT FOR UPDATE`](/docs/v23.2/select-for-update), and [`SELECT FOR SHARE`](/docs/v23.2/select-for-update) are fully replicated under `READ COMMITTED` isolation, some queries experience a delay for Raft replication.
- [Foreign key](/docs/v23.2/foreign-key) checks are not performed in parallel under `READ COMMITTED` isolation.
- [`SELECT FOR UPDATE` and `SELECT FOR SHARE`](/docs/v23.2/select-for-update) statements are less optimized under `READ COMMITTED` isolation than under `SERIALIZABLE` isolation. Under `READ COMMITTED` isolation, `SELECT FOR UPDATE` and `SELECT FOR SHARE` usually perform an extra lookup join for every locked table when compared to the same queries under `SERIALIZABLE`. In addition, some optimization steps (such as de-correlation of correlated [subqueries](/docs/v23.2/subqueries) ) are not currently performed on these queries.
