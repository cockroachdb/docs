...
<InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v23.2: **Physical cluster replication (PCR)** automatically and continuously streams data from an active *primary* CockroachDB cluster to a passive *standby* cluster. Each cluster contains: a *system virtual cluster* and an application [virtual cluster](/docs/v23.2/cluster-virtualization-overview):
...
[Starting a physical replication](/docs/v23.2/set-up-physical-cluster-replication) stream consists of two jobs: one each on the standby and primary cluster:
...
2. The primary cluster chooses a timestamp at which to start the physical replication stream. Data on the primary is protected from [garbage collection](/docs/v23.2/architecture/storage-layer#garbage-collection) until it is replicated to the standby using a [protected timestamp](/docs/v23.2/architecture/storage-layer#protected-timestamps).
3. The primary cluster returns the timestamp and a [job ID](/docs/v23.2/show-jobs#response) for the replication job.
...
<Frame>
  ![Two virtualized clusters with system virtual cluster and application virtual cluster showing the directional stream.](/images/v23.2/physical-rep-to.png)
</Frame>
...
During the job, [rangefeeds](/docs/v23.2/create-and-configure-changefeeds#enable-rangefeeds) are periodically emitting resolved timestamps, which is the time where the ingested data is known to be consistent. Resolved timestamps provide a guarantee that there are no new writes from before that timestamp. This allows the standby cluster to move the [protected timestamp](/docs/v23.2/architecture/storage-layer#protected-timestamps) forward as the replicated timestamp advances. This information is sent to the primary cluster, which allows for [garbage collection](/docs/v23.2/architecture/storage-layer#garbage-collection) to continue as the replication stream on the standby cluster advances.

<Note>
If the primary cluster does not receive replicated time information from the standby after 3 days, it cancels the replication job. This ensures that an inactive replication job will not prevent garbage collection. The time at which the job is removed is configurable via the `stream_replication.job_liveness_timeout` [cluster setting](/docs/v23.2/cluster-settings).
</Note>
...
<Frame>
  ![Timeline showing how the cutover window is between the retained time and replicated time.](/images/v23.2/cutover-window.png)
</Frame>
...
For the [cutover process](/docs/v23.2/cutover-replication), the standby cluster waits until it has reached the specified cutover time, which can be in the [past](/docs/v23.2/cutover-replication#cut-over-to-a-point-in-time) (retained time), the [`LATEST`](/docs/v23.2/cutover-replication#cut-over-to-the-most-recent-replicated-time) timestamp, or in the [future](/docs/v23.2/cutover-replication#cut-over-to-a-point-in-time). Once that timestamp has been reached, the replication stream stops and any data in the standby cluster that is **above** the cutover time is removed. Depending on how much data the standby needs to revert, this can affect the duration of RTO (recovery time objective).
...
<Note>
For detail on cutting back to the primary cluster following a cutover, refer to [Cut back to the primary cluster](/docs/v23.2/cutover-replication#cut-back-to-the-primary-cluster).
</Note>
