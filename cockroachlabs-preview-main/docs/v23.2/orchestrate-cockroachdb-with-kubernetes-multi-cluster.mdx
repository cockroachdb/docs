...
<Tip>
To deploy CockroachDB in a single Kubernetes cluster instead, see [Kubernetes Single-Cluster Deployment](/docs/v23.2/deploy-cockroachdb-with-kubernetes). Also, for details about potential performance bottlenecks to be aware of when running CockroachDB in Kubernetes and guidance on how to optimize your deployment for better performance, see [CockroachDB Performance on Kubernetes](/docs/v23.2/kubernetes-performance).
</Tip>
...
4. If you haven't already, [install CockroachDB locally and add it to your `PATH`](/docs/v23.2/install-cockroachdb). The `cockroach` binary will be used to generate certificates.
...
5. Optionally, to optimize your deployment for better performance, review [CockroachDB Performance on Kubernetes](/docs/v23.2/kubernetes-performance) and make the desired modifications to the `cockroachdb-statefulset-secure.yaml` file.
...
1. On secure clusters, [certain pages of the DB Console](/docs/v23.2/ui-overview#db-console-access) can only be accessed by `admin` users.
    ...
    Get a shell into the pod with the `cockroach` binary created earlier and start the CockroachDB [built-in SQL client](/docs/v23.2/cockroach-sql):
...
6. In the UI, check the [**Node List**](/docs/v23.2/ui-cluster-overview-page#node-list) to verify that all nodes are running, open the [**Databases** page](/docs/v23.2/ui-databases-page) to verify that `bank` is listed, and open the [**Network Latency** page](/docs/v23.2/ui-network-latency-page) to see the performance of your CockroachDB cluster across 3 regions.
...
1. Verify that you can upgrade.
    ...
    Therefore, in order to upgrade to v23.2, you must be on a production release of v23.1.

    1. If you are upgrading to v23.2 from a production release earlier than v23.1, or from a testing release (alpha/beta), first [upgrade to a production release of v23.1](https://www.cockroachlabs.com/docs/v23.1/orchestrate-cockroachdb-with-kubernetes-multi-cluster#upgrade-the-cluster). Be sure to complete all the steps.
    2. Then return to this page and perform a second upgrade to v23.2.
    3. If you are upgrading from any production release of v23.1, or from any earlier v23.2 release, you do not have to go through intermediate releases; continue to step 2.
2. Verify the overall health of your cluster using the [DB Console](/docs/v23.2/ui-overview). On the **Overview**:
...
3. Review the backward-incompatible changes in v23.2 and deprecated features. If any affect your deployment, make the necessary changes before starting the rolling upgrade to v23.2.
4. Decide how the upgrade will be finalized.

    By default, after all nodes are running the new version, the upgrade process will be **auto-finalized**. This will enable certain features and performance improvements introduced in v23.2. After finalization, however, it will no longer be possible to perform a downgrade to v23.1. In the event of a catastrophic failure or corruption, the only option is to start a new cluster using the old binary and then restore from a [backup](/docs/v23.2/take-full-and-incremental-backups) created prior to the upgrade. For this reason, **we recommend disabling auto-finalization** so you can monitor the stability and performance of the upgraded cluster before finalizing the upgrade, but note that you will need to follow all of the subsequent directions, including the manual finalization in a later step.

    <Note>
    Finalization only applies when performing a major version upgrade (for example, from v23.1.x to v23.2). Patch version upgrades (for example, within the v23.2.x series) can always be downgraded.
    </Note>

    1. Get a shell into the pod with the `cockroach` binary created earlier and start the CockroachDB [built-in SQL client](/docs/v23.2/cockroach-sql):

        ```shell
        $ kubectl exec -it cockroachdb-client-secure --context <cluster-context> --namespace <cluster-namespace> -- ./cockroach sql --certs-dir=/cockroach-certs --host=cockroachdb-public
        ```
    2. Set the `cluster.preserve_downgrade_option` [cluster setting](/docs/v23.2/cluster-settings):
...
7. If you disabled auto-finalization earlier, monitor the stability and performance of your cluster until you are comfortable with the upgrade (generally at least a day).
    ...
    <Note>
    This is only possible when performing a major version upgrade (for example, from v23.1.x to v23.2). Patch version upgrades (for example, within the v23.2.x series) are auto-finalized.
    </Note>
...
4. The StatefulSet configuration includes a [`cockroach start`](/docs/v23.2/cockroach-start) command that creates the nodes on the Kubernetes pods.
...
2. Get a shell into the pod and start the CockroachDB [built-in SQL client](/docs/v23.2/cockroach-sql), again specifying the namespace and context of the Kubernetes cluster where the pod is running:
    ...
    ```
3. Run some basic [CockroachDB SQL statements](/docs/v23.2/learn-cockroachdb-sql):
    ...
    (1 row)
    ```
4. [Create a user with a password](/docs/v23.2/create-user#create-a-user-with-a-password):
...
5. Exit the SQL shell and pod:
    ...
    The pod will continue running indefinitely, so any time you need to reopen the built-in SQL client or run any other [`cockroach` client commands](/docs/v23.2/cockroach-commands) (e.g., `cockroach node`), repeat step 2 using the appropriate command.
...
- [Kubernetes Single-Cluster Deployment](/docs/v23.2/deploy-cockroachdb-with-kubernetes)
- [Kubernetes Performance Guide](/docs/v23.2/kubernetes-performance)
