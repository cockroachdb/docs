...
This page guides you through a series of simple database schema changes using the [Liquibase](https://www.liquibase.org/get-started/how-liquibase-works) command-line tool and the [CockroachDB SQL shell](/docs/v23.2/cockroach-sql).
...
1. [Install CockroachDB](/docs/v23.2/install-cockroachdb), and [start a secure cluster](/docs/v23.2/secure-a-cluster). When starting your cluster, make sure that you generate cluster certificates, create the `bank` database, and create the `max` user.
...
When you [started a secure CockroachDB cluster](/docs/v23.2/secure-a-cluster), you should have created a user `max`. You should have also given this user the [`admin` role](/docs/v23.2/security-reference/authorization#admin-role), which grants all privileges to all databases on the cluster. In this tutorial, Liquibase runs schema changes as the `max` user.

To authenticate connection requests to CockroachDB from the Liquibase client, you need to generate some certificates for `max`. Use [`cockroach cert`](/docs/v23.2/cockroach-cert) to generate the certificates:

```shell
$ cockroach cert create-client max --certs-dir=certs --ca-key=my-safe-directory/ca.key --also-generate-pkcs8-key
```

The [`--also-generate-pkcs8-key` flag](/docs/v23.2/cockroach-cert#flag-pkcs8) generates a key in [PKCS#8 format](https://tools.ietf.org/html/rfc5208), which is the standard key encoding format in Java. In this case, the generated PKCS8 key will be named `client.max.key.pk8`.
...
2. Add the following to the blank `changelog-main.xml` file:
    ...
    <Tip>
    CockroachDB [doesn't guarantee the atomicity of online schema changes in transactions with multiple statements](/docs/v23.2/online-schema-changes#schema-changes-within-transactions). To avoid running into issues with incomplete transactions, we recommend setting the [`runInTransaction` attribute](https://docs.liquibase.com/concepts/basic/changeset#available-attributes) to `"false"` on all changesets.
    </Tip>
3. In the same directory, create the SQL file specified by the first changeset:
    ...
    $ touch create.sql
    ```
4. Add the following [`CREATE TABLE`](/docs/v23.2/create-table) statement to the `create.sql` file:
...
2. Add the following SQL statement to `add_uuid.sql`:
    ...
    This statement adds [a new `unique_id` column](/docs/v23.2/alter-table#add-column) to the `accounts` table, with the default value as [a randomly-generated UUID](/docs/v23.2/performance-best-practices-overview#use-functions-to-generate-unique-ids).
...
5. Add the following SQL statement to `update_pk.sql`:
    ...
    This statement [alters the `accounts` primary key](/docs/v23.2/alter-table#alter-primary-key) to use the `unique_id` column.
...
CockroachDB has [doesn't guarantee the atomicity of schema changes within transactions with multiple statements](/docs/v23.2/online-schema-changes#schema-changes-within-transactions). If a schema change fails, automatic rollbacks can lead to unexpected results. To avoid running into issues with incomplete transactions, we recommend setting the `runInTransaction` attribute on each of your changesets to `"false"`, as demonstrated throughout this tutorial.
...
When multiple, concurrent transactions or statements are issued to a single CockroachDB cluster, [transaction contention](/docs/v23.2/performance-best-practices-overview#transaction-contention) can cause schema migrations to fail. In the event of transaction contention, CockroachDB returns a `40001 SQLSTATE` (i.e., a serialization failure).

Liquibase does not automatically retry transactions. To handle transaction failures, we recommend writing client-side transaction retry logic. For more information about client-side transaction retries in CockroachDB, see [Transaction Retries](/docs/v23.2/transactions#transaction-retries).

## Liquibase integrations

You can run Liquibase in the context of a Java application framework, like Spring Boot. For examples of using Liquibase for schema management in a Spring Boot application built on CockroachDB, see [Build a Spring App with CockroachDB and JDBC](/docs/v23.2/build-a-spring-app-with-cockroachdb-jdbc) and [Build a Spring App with CockroachDB and JPA](/docs/v23.2/build-a-spring-app-with-cockroachdb-jpa).
...
- CockroachDB version ( [`cockroach version`](/docs/v23.2/cockroach-version) )
- Liquibase version
- Operating system
...
- [Build a Spring App with CockroachDB and JDBC](/docs/v23.2/build-a-spring-app-with-cockroachdb-jdbc)
- [Build a Spring App with CockroachDB and JPA](/docs/v23.2/build-a-spring-app-with-cockroachdb-jpa)
