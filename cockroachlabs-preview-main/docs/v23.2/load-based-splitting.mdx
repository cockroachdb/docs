...
To optimize your cluster's performance, CockroachDB can split frequently accessed keys into smaller ranges. In conjunction with [load-based rebalancing](/docs/v23.2/architecture/replication-layer#load-based-replica-rebalancing), load-based splitting distributes load evenly across your cluster.

## Enable and disable load-based splitting

Load-based splitting is enabled by default. To enable and disable load-based splitting, set the `kv.range_split.by_load_enabled` [cluster setting](/docs/v23.2/cluster-settings). For example, to disable load-based splitting, execute:
...
To control the load-based splitting threshold, set the `kv.range_split.load_qps_threshold` [cluster setting](/docs/v23.2/cluster-settings) to the queries-per-second (QPS) at which you want to consider splitting a range (defaults to `2500`). For example:
...
CockroachDB creates a relatively even distribution of leaseholders throughout your cluster. ([Leaseholders](/docs/v23.2/architecture/replication-layer#leases) are a single replica of a range that both serve reads and coordinate write operations.)
...
This benefit is further amplified by [load-based rebalancing](/docs/v23.2/architecture/replication-layer#load-based-replica-rebalancing), which ensures that all nodes contain replicas with a roughly equal load. By evenly distributing load throughout your cluster, it's easier to prevent bottlenecks from arising, as well as simplifying hardware forecasting.

## Monitor load-based splitting

The [`INFO`](/docs/v23.2/logging#info) log message

```text
no split key found: ...
```

indicates that CockroachDB wants to [split the range](/docs/v23.2/architecture/distribution-layer#range-splits) due to load, but couldn’t find a key to split at.

Usually this log message can be ignored, unless it repeatedly shows up, which can indicate there is a load imbalance problem in the cluster. If there is a load imbalance problem, it could be because a [hot range](/docs/v23.2/ui-hot-ranges-page) cannot be split (because it's really a [hot key](/docs/v23.2/ui-hot-ranges-page#range-report)).

You can see how often a split key cannot be found over time by looking at the following [time-series metric](/docs/v23.2/monitoring-and-alerting#prometheus-endpoint):

- `kv.loadsplitter.nosplitkey`

This metric is directly related to the log message described above.

For more information about how to reduce hot spots (including hot ranges) on your cluster, see [Hot spots](/docs/v23.2/performance-best-practices-overview#hot-spots).

## See also

- [`SET CLUSTER SETTING`](/docs/v23.2/set-cluster-setting)
- [Load-based replica rebalancing](/docs/v23.2/architecture/replication-layer#load-based-replica-rebalancing)
