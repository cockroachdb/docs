...
In this tutorial, you will plan a vacation from New York City to the [Adirondack Mountains](https://visitadirondacks.com/about) in northern New York State to do some birdwatching while visiting local independent bookstores. In the process, you will explore several of CockroachDB's [spatial capabilities](/docs/v23.2/spatial-data-overview):
...
- Using various [built-in functions](/docs/v23.2/functions-and-operators#spatial-functions) for operating on spatial data.
- Creating [indexes](/docs/v23.2/spatial-indexes) on spatial data.
- Performing [joins](/docs/v23.2/joins) on spatial data, and using [`EXPLAIN`](/docs/v23.2/explain) to make sure indexes are effective.
...
The simplest way to get up and running is with [`cockroach demo`](/docs/v23.2/cockroach-demo), which starts a temporary, in-memory CockroachDB cluster and opens an interactive SQL shell:

```shell
$ cockroach demo
```

You will see a [SQL client prompt](/docs/v23.2/cockroach-sql) that looks like the one shown below. You will use this prompt for the rest of the tutorial.
...
1. Create a `tutorial` database, and use it.
    ...
    USE tutorial;
    ```
2. [`IMPORT`](/docs/v23.2/import) the parts of [the data set](#data-set-description) that live in the `tutorial` database.
...
3. Create a `birds` database, and use it.
    ...
    USE birds;
    ```
4. [`IMPORT`](/docs/v23.2/import) the parts of [the data set](#data-set-description) that live in the `birds` database.
...
2. Collect the resulting birdwatcher route geometries ( `routes.geom` ) into one geometry (a [MultiPoint](/docs/v23.2/multipoint) ).
3. Give the resulting table a name, `loon_sightings`, and query against it. In this case the query is rather simple: since the geometries have been collected into one in step 2 above, output the geometry as [GeoJSON](/docs/v23.2/geojson) so the result can be pasted into [https://geojson.io](https://geojson.io/) to generate a map of the sightings.
...
<Frame>
  ![Common Loon sightings in the years 2000-2019 in NY state](/images/v23.2/geospatial/tutorial/query-01.png)
</Frame>
...
2. Get the area of the [convex hull](/docs/v23.2/st_convexhull) of the resulting geometry.
3. Because the `birds.routes` data uses [SRID 4326](/docs/v23.2/srid-4326), the resulting area is measured in degrees, which can be converted to square miles by casting the data to a `GEOGRAPHY` type and dividing by 1609 (the number of meters in a mile) squared.
...
2. Query the result table of the CTE from step 1 to divide the number of sightings by the area of the loon's habitat (the convex hull). As in [#2](#2-what-is-the-total-area-of-loon-sightings), do some arithmetic to convert from the unit of degrees returned by [SRID 4326](/docs/v23.2/srid-4326) to square miles.
...
1. Build a CTE that returns the [convex hull](/docs/v23.2/st_convexhull) of Common Loon habitat.
2. Join the results of the above CTE with a query against [the `bookstores` table](#the-bookstores-and-bookstore_routes-tables) that checks whether a bookstore's location is [contained](/docs/v23.2/st_contains) by the loon habitat. Note that the query below [orders by](/docs/v23.2/order-by) the store geometries so that stores in the list are clustered by location. This ordering may be useful if you want to travel between nearby stores. For more information about how this ordering is calculated, see [How CockroachDB's spatial indexing works](/docs/v23.2/spatial-indexes#how-cockroachdbs-spatial-indexing-works).
...
<Note>
The final [`SELECT`](/docs/v23.2/selection-queries) in the query below is doing a join that will not benefit from [spatial indexing](/docs/v23.2/spatial-indexes), since both sides of the join are the results of [CTEs](/docs/v23.2/common-table-expressions), and are therefore not indexed.
</Note>
...
<Note>
The query below can also be written using an explicit `ST_DWithin`, which is an [index-accelerated function](/docs/v23.2/query-spatial-data#performance). CockroachDB optimizes `ST_Distance(...) < $some_value` to use `ST_DWithin` (see this query's [`EXPLAIN`](/docs/v23.2/explain) output for details).
</Note>
...
1. Build a CTE that returns the [convex hull](/docs/v23.2/st_convexhull) of Common Loon habitat.
2. Join the results of the above CTE with a query against [the `bookstores` table](#the-bookstores-and-bookstore_routes-tables) that checks whether a bookstore's location is [contained](/docs/v23.2/st_contains) by the loon habitat.
3. Collect the geometries that result from the step above into a single geometry, calculate its convex hull, and return the results as [GeoJSON](/docs/v23.2/geojson).
...
<Frame>
  ![Convex hull of bookstore locations within Common Loon habitat](/images/v23.2/geospatial/tutorial/query-09.png)
</Frame>
...
1. Build a CTE that returns the [convex hull](/docs/v23.2/st_convexhull) of Common Loon habitat.
2. Join the results of the above CTE with a query against [the `bookstores` table](#the-bookstores-and-bookstore_routes-tables) that checks whether a bookstore's location is [contained](/docs/v23.2/st_contains) by the loon habitat.
3. Get the area of the [convex hull](/docs/v23.2/st_convexhull) of the resulting geometry.
4. Collect the geometries that result from the step above into a single geometry, calculate its convex hull, and calculate the area of the hull. As in previous examples, note that because the `birds.routes` data uses [SRID 4326](/docs/v23.2/srid-4326), the resulting area is measured in degrees, which is converted to square miles by casting the data to a `GEOGRAPHY` type and dividing by 1609 (the number of meters in a mile) squared.
...
<Frame>
  ![What does the route from Mysteries on Main Street in Johnstown, NY to The Book Nook in Saranac Lake, NY look like?](/images/v23.2/geospatial/tutorial/query-12.png)
</Frame>
...
Unfortunately, this query is a bit slower than you would like: about 1.5 seconds on a single-node [`cockroach demo`](/docs/v23.2/cockroach-demo) cluster on a laptop. There are several reasons for this:

1. You haven't created any indexes at all yet. The query is likely to be doing full table scans, which you will need to hunt down with [`EXPLAIN`](/docs/v23.2/explain).
...
1. Add an index to the `roads.geom` column. Note that creating the index on `roads.geom` takes about a minute on a single-node [`cockroach demo`](/docs/v23.2/cockroach-demo) cluster, since the table is relatively large.

    ```sql
    CREATE INDEX ON roads USING GIST(geom);
    ```
2. Update the SRID of the `roads.geom` column from 0 to [4326](/docs/v23.2/srid-4326). This will take a few seconds.
...
To see why, look at the [`EXPLAIN`](/docs/v23.2/explain) output:
...
- The [spatial index](/docs/v23.2/spatial-indexes) on `roads.geom` is being used by the join between that column and the loon habitat geometry. This is the biggest reason for improved performance, since [the `roads` table](#the-roads-table) has ~225,000 rows.
...
2. There is a full table scan on the `bookstores` table. Since the predicate is [`ST_Contains`](/docs/v23.2/st_contains), you probably need to add a [spatial index](/docs/v23.2/spatial-indexes) on `bookstores.geom`.
...
- Learn more details about the [spatial features](/docs/v23.2/spatial-data-overview) supported by CockroachDB.
- Learn how to migrate spatial data from [shapefiles](/docs/v23.2/migrate-from-shapefiles), [GeoPackages](/docs/v23.2/migrate-from-geopackage), [GeoJSON](/docs/v23.2/migrate-from-geojson), or [OpenStreetMap](/docs/v23.2/migrate-from-openstreetmap) into CockroachDB.
- Learn how CockroachDB's [spatial indexes](/docs/v23.2/spatial-indexes) work.
...
Below is an entity-relationship diagram showing the `bookstores` and `bookstore_routes` tables (generated using [DBeaver](/docs/v23.2/dbeaver)):

<Frame>
  ![tutorial.bookstores and tutorial.bookstore_routes ER diagrams](/images/v23.2/geospatial/tutorial/er-bookstores.png)
</Frame>
...
<Frame>
  ![tutorial.roads ER diagrams](/images/v23.2/geospatial/tutorial/er-roads.png)
</Frame>
...
<Note>
The `roads` table was imported from a [shapefile](https://prd-tnm.s3.amazonaws.com/StagedProducts/Small-scale/data/Transportation/roadtrl010g.shp_nt00920.tar.gz) using the method described in [Migrate from Shapefiles](/docs/v23.2/migrate-from-shapefiles).
</Note>

#### The `birds` database

The `birds` database contains several tables that you will have to [join](/docs/v23.2/joins) together in most of the queries shown elsewhere on this page. The multi-table design reflects [the schema of the actual data set](https://www.sciencebase.gov/catalog/file/get/5ea04e9a82cefae35a129d65?f=__disk__b4%2F2f%2Fcf%2Fb42fcfe28a799db6e8c97200829ea1ebaccbf8ea&transform=1&allowOpen=true), which is split across files that map to these tables.
...
- `routes` is a list of ~130 prescribed locations that the birdwatchers helping with the survey visit each year. The `geom` associated with each route is a [Point](/docs/v23.2/point) marking the latitude and longitude of the route's starting point. For details, see the [schema](https://www.sciencebase.gov/catalog/file/get/5ea04e9a82cefae35a129d65?f=__disk__b4%2F2f%2Fcf%2Fb42fcfe28a799db6e8c97200829ea1ebaccbf8ea&transform=1&allowOpen=true) (search for the text "routes.csv").
- `observations` describes the ~85,000 times and places in which birds of various species were actually seen. The `bird_id` is a [foreign key](/docs/v23.2/foreign-key) to the ID in the `birds` table, and the `route_id` points to the ID of the `routes` table.

<Frame>
  ![birds.birds, birds.routes, and birds.observations ER diagrams](/images/v23.2/geospatial/tutorial/er-birds.png)
</Frame>

Each of these tables were populated using a script that parsed [the CSV files available for download](https://www.sciencebase.gov/catalog/item/52b1dfa8e4b0d9b325230cd9) and added the data using [`INSERT`](/docs/v23.2/insert) statements. For the `routes` table, once again the `ST_MakePoint` function was used to create a geometry from the lon/lat values in the CSV as follows:
