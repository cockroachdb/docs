...
A view is a stored and named [selection query](/docs/v23.2/selection-queries). By default, CockroachDB's views are **dematerialized**: they do not store the results of the underlying queries. Instead, the underlying query is executed anew every time the view is used.
...
Let's say you're using our [sample `startrek` database](/docs/v23.2/cockroach-gen#generate-example-data), which contains two tables, `episodes` and `quotes`.
...
To create a view, use the [`CREATE VIEW`](/docs/v23.2/create-view) statement:
...
<Note>
Any [selection query](/docs/v23.2/selection-queries) is valid as operand to `CREATE VIEW`, not just [simple `SELECT` clauses](/docs/v23.2/select-clause).
</Note>
...
To list just views, you can query the `views` table in the [Information Schema](/docs/v23.2/information-schema):
...
To query a view, target it with a [table expression](/docs/v23.2/table-expressions#table-and-view-names), for example using a [`SELECT` clause](/docs/v23.2/select-clause), just as you would with a stored table:
...
`SELECT`ing a view executes the view's stored `SELECT` statement, which returns the relevant data from the underlying table(s). To inspect the `SELECT` statement executed by the view, use the [`SHOW CREATE`](/docs/v23.2/show-create) statement:
...
You can also inspect the `SELECT` statement executed by a view by querying the `views` table in the [Information Schema](/docs/v23.2/information-schema):
...
A view depends on the objects targeted by its underlying query. Attempting to [rename an object](/docs/v23.2/alter-table#rename-to) referenced in a view's stored query therefore results in an error:
...
Likewise, attempting to [drop an object](/docs/v23.2/drop-table) referenced in a view's stored query results in an error:
...
When [dropping a table](/docs/v23.2/drop-table) or [dropping a view](/docs/v23.2/drop-view), you can use the `CASCADE` keyword to drop all dependent objects as well:
...
To rename a view, use the [`ALTER VIEW`](/docs/v23.2/alter-view) statement:
...
To replace a view, use [`CREATE OR REPLACE VIEW`](/docs/v23.2/create-view):
...
To remove a view, use the [`DROP VIEW`](/docs/v23.2/drop-view) statement:
...
When you [select](/docs/v23.2/selection-queries) from a materialized view, the stored query data that is returned might be out-of-date. This contrasts with a standard (i.e., "dematerialized") view, which runs its underlying query every time it is used, returning the latest results. In order to get the latest results from a materialized view, you must [refresh the view](/docs/v23.2/refresh), and then select from it.
...
Materialized views and standard views share similar syntax for [creating](/docs/v23.2/create-view), [showing](/docs/v23.2/show-tables), [renaming](/docs/v23.2/alter-view), and [dropping](/docs/v23.2/drop-view).

To create a materialized view, use a [`CREATE MATERIALIZED VIEW`](/docs/v23.2/create-view) statement.

For example, suppose that you have the [sample `bank` database](/docs/v23.2/cockroach-workload#bank-workload) loaded to a CockroachDB cluster, and populated with some workload values:
...
To show existing materialized views, use a [`SHOW TABLES`](/docs/v23.2/show-tables) statement:
...
To update the materialized view's results, use a [`REFRESH`](/docs/v23.2/refresh) statement:
...
To rename the materialized view, use [`ALTER MATERIALIZED VIEW`](/docs/v23.2/alter-view):
...
To remove the materialized view, use [`DROP MATERIALIZED VIEW`](/docs/v23.2/drop-view):
...
To speed up queries on materialized views, you can add an [index](/docs/v23.2/schema-design-indexes) to the view.

1. Create a materialized view of the [MovR](/docs/v23.2/movr) rides table where the revenue is less than &#36;20.00:
...
- CockroachDB cannot refresh materialized views inside [explicit transactions](/docs/v23.2/begin-transaction). Trying to refresh a materialized view inside an explicit transaction will result in an error.
...
CockroachDB supports session-scoped temporary views. Unlike persistent views, temporary views can only be accessed from the session in which they were created, and they are dropped at the end of the session. You can create temporary views on both persistent tables and [temporary tables](/docs/v23.2/temporary-tables).
...
<Note>
Temporary tables must be enabled in order to use temporary views. By default, temporary tables are disabled in CockroachDB. To enable temporary tables, set the `experimental_enable_temp_tables` [session variable](/docs/v23.2/set-vars) to `on`.
</Note>
...
- Temporary views can be created on both persistent tables and [temporary tables](/docs/v23.2/temporary-tables).
- When you create a view on a temporary table, the view automatically becomes temporary.

<Note>
Like [temporary tables](/docs/v23.2/temporary-tables), temporary views are not in the `public` schema. Instead, when you create the first temporary table, view, or sequence for a session, CockroachDB generates a single temporary schema (`pg_temp_<id>`) for all of the temporary objects in the current session for a database.
</Note>

### Usage

To create a temporary view, add [`TEMP`/`TEMPORARY`](/docs/v23.2/sql-grammar#opt_temp) to a [`CREATE VIEW`](/docs/v23.2/create-view) statement.
...
- [Simple `SELECT` Clauses](/docs/v23.2/select-clause)
