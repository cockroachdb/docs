...
- [`LIMIT` / `OFFSET`](/docs/v23.2/limit-offset) pagination (slow, not recommended)
- Keyset pagination ( **fast, recommended** )
- [Cursors](/docs/v23.2/cursors) (it depends, see [Differences between keyset pagination and cursors](#differences-between-keyset-pagination-and-cursors) )

## Keyset pagination

Keyset pagination (also known as the "seek method") is used to fetch a subset of records from a table quickly. It does this by restricting the set of records returned with a combination of `WHERE` and [`LIMIT`](/docs/v23.2/limit-offset) clauses. To get the next page, you check the value of the column in the `WHERE` clause against the last row returned in the previous page of results.
...
This is faster than using `LIMIT`/`OFFSET` because, instead of doing a full table scan up to the value of the `OFFSET`, a keyset pagination query looks at a fixed-size set of records for each iteration. This can be done quickly provided that the key used in the `WHERE` clause to implement the pagination is [indexed](/docs/v23.2/indexes#best-practices) and [unique](/docs/v23.2/unique). A [primary key](/docs/v23.2/primary-key) meets both of these criteria.
...
The query using `LIMIT`/`OFFSET` for pagination is almost 100 times slower. To see why, let's use [`EXPLAIN`](/docs/v23.2/explain).
...
Meanwhile, the keyset pagination queries are looking at a much smaller range of table spans, which is much faster (see `spans: [/300026 - ]` and `limit: 25` below). Because [there is an index on every column in the `WHERE` clause](/docs/v23.2/indexes#best-practices), these queries are doing an index lookup to jump to the start of the page of results, and then getting an additional 25 rows from there. This is much faster.
...
<Warning>
Using a sequential (i.e., non-[UUID](/docs/v23.2/uuid)) primary key creates hot spots in the database for write-heavy workloads, since concurrent [`INSERT`](/docs/v23.2/insert)s to the table will attempt to write to the same (or nearby) underlying [ranges](/docs/v23.2/architecture/overview#architecture-range). This can be mitigated by designing your schema with [multi-column primary keys which include a monotonically increasing column](/docs/v23.2/performance-best-practices-overview#use-multi-column-primary-keys).
</Warning>
