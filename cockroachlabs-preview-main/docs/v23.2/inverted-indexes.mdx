...
Generalized inverted indexes, or GIN indexes, store mappings from values within a container column (such as a [`JSONB`](/docs/v23.2/jsonb) document) to the row that holds that value. They are used to speed up containment searches, e.g., "show me all of the rows from this table which have a JSON column that contains the key-value pair `{"location":"NYC"}`". GIN indexes are commonly used in [document retrieval systems](https://wikipedia.org/wiki/Document_retrieval).

CockroachDB stores the contents of the following data types in GIN indexes:

- [`JSONB`](/docs/v23.2/jsonb)
- [`ARRAY`](/docs/v23.2/array)
- [Spatial data (`GEOMETRY` and `GEOGRAPHY` types)](/docs/v23.2/spatial-indexes)
- [`TSVECTOR` (for full-text search)](/docs/v23.2/tsvector)
- [`STRING` (using trigram indexes)](/docs/v23.2/trigram-indexes)

<Tip>
For a hands-on demonstration of using GIN indexes to improve query performance on a `JSONB` column, see the [JSON tutorial](/docs/v23.2/demo-json-support).
</Tip>

## How do GIN indexes work?

Standard [indexes](/docs/v23.2/indexes) work well for searches based on prefixes of sorted data. However, data types like [`JSONB`](/docs/v23.2/jsonb) or [arrays](/docs/v23.2/array) cannot be queried without a full table scan, since they do not adhere to ordinary value prefix comparison operators. `JSONB` in particular needs to be indexed in a more detailed way than what a standard index provides. This is where GIN indexes prove useful.
...
You can use GIN indexes to improve the performance of queries using [`JSONB`](/docs/v23.2/jsonb), [`ARRAY`](/docs/v23.2/array), [`TSVECTOR`](/docs/v23.2/tsvector) columns (for [full-text searches](/docs/v23.2/full-text-search)), or [`STRING`](/docs/v23.2/string) (for [fuzzy searches using trigrams](/docs/v23.2/trigram-indexes)). You can create them:

- Using the PostgreSQL-compatible syntax [`CREATE INDEX... USING GIN`](/docs/v23.2/create-index):

    ```sql
    CREATE INDEX {optional name} ON {table} USING GIN ({column});
    ```

    Also specify an opclass when [creating a trigram index](/docs/v23.2/trigram-indexes#creation):
...
- While creating the table, using the syntax [`CREATE INVERTED INDEX`](/docs/v23.2/create-table#create-a-table-with-secondary-and-gin-indexes):
...
To override CockroachDB's index selection, you can also force a query to use [a specific index](/docs/v23.2/table-expressions#force-index-selection) (also known as "index hinting") or use [an inverted join hint](/docs/v23.2/cost-based-optimizer#supported-join-algorithms).
...
- **is contained by**: [`<@`](/docs/v23.2/functions-and-operators#operators)
- **contains**: [`@>`](/docs/v23.2/functions-and-operators#operators)
- **equals**: [`=`](/docs/v23.2/functions-and-operators#operators). To use `=`, you must also reach into the JSON document with the [`->`](/docs/v23.2/functions-and-operators#supported-operations) operator. For example:
...
If you require comparisons using [`<`](/docs/v23.2/functions-and-operators#operators), [`<=`](/docs/v23.2/functions-and-operators#operators), etc., you can create an index on a [computed column](/docs/v23.2/computed-columns) using your JSON payload, and then create a standard index on that. To write a query where the value of `foo` is greater than three:
...
GIN indexes on [`ARRAY`](/docs/v23.2/array) columns support the following comparison operators:
...
For an example showing how to create a trigram index on a [`STRING`](/docs/v23.2/string) column, see [Trigram Indexes](/docs/v23.2/trigram-indexes#examples).

### Create a full-text index on a `TSVECTOR` column

For an example showing how to create a full-text index on a [`TSVECTOR`](/docs/v23.2/tsvector) column, see [Full-Text Search](/docs/v23.2/full-text-search#examples).
...
- [Trigram Indexes](/docs/v23.2/trigram-indexes)
- [`JSONB`](/docs/v23.2/jsonb)
- [Arrays](/docs/v23.2/array)
- [Spatial data (`GEOMETRY` and `GEOGRAPHY` types)](/docs/v23.2/spatial-indexes)
