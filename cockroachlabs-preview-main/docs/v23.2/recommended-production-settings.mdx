...
When planning your deployment, it's important to carefully review and choose the [topology patterns](/docs/v23.2/topology-patterns) that best meet your latency and resiliency requirements. This is especially crucial for multi-region deployments.
...
- To prevent unnecessary queuing in [admission control](/docs/v23.2/admission-control) CPU queues, set the `goschedstats.always_use_short_sample_period.enabled` [cluster setting](/docs/v23.2/cluster-settings#setting-goschedstats-always-use-short-sample-period-enabled) to `true` for any production cluster.
...
- A **larger number of smaller nodes** emphasizes resiliency across [failure scenarios](/docs/v23.2/disaster-recovery-planning).
    ...
    - Having more and smaller nodes allows [backup and restore jobs](/docs/v23.2/take-and-restore-encrypted-backups) to complete more quickly, since these jobs run in parallel and less data is hosted on each individual node.
...
- To optimize for the support of large numbers of tables, increase the amount of RAM. For more information, see [Quantity of tables and other schema objects](/docs/v23.2/schema-design-overview#quantity-of-tables-and-other-schema-objects). Supporting a large number of rows is a matter of [Storage](#storage).
...
- To help guard against [out-of-memory (OOM) crashes](/docs/v23.2/cluster-setup-troubleshooting#out-of-memory-oom-crash), consider tuning the cache and SQL memory for cluster nodes. Refer to the section [Cache and SQL memory size](#cache-and-sql-memory-size).
- Monitor [CPU](/docs/v23.2/common-issues-to-monitor#cpu-usage) and [memory](/docs/v23.2/common-issues-to-monitor#database-memory-usage) usage. Ensure that they remain within acceptable limits.

<Note>
Under-provisioning RAM results in reduced performance (due to reduced caching and increased spilling to disk), and in some cases can cause [OOM crashes](/docs/v23.2/cluster-setup-troubleshooting#out-of-memory-oom-crash). For more information, see [memory issues](/docs/v23.2/cluster-setup-troubleshooting#memory-issues).
</Note>
...
- Store CockroachDB [log files](/docs/v23.2/configure-logs#logging-directory) in a separate volume from the main data store so that logging is not impacted by I/O throttling.
...
- [Monitor your storage utilization](/docs/v23.2/common-issues-to-monitor#storage-capacity) and rate of growth, and take action to add capacity well before you hit the limit.
- CockroachDB will [automatically provision an emergency ballast file](/docs/v23.2/cluster-setup-troubleshooting#automatic-ballast-files) at [node startup](/docs/v23.2/cockroach-start). In the unlikely case that a node runs out of disk space and shuts down, you can delete the ballast file to free up enough space to be able to restart the node.
- Use [zone configs](/docs/v23.2/configure-replication-zones) to increase the replication factor from 3 (the default) to 5 (across at least 5 nodes).

    This is especially recommended if you are using local disks rather than a cloud provider's network-attached disks that are often replicated under the hood, because local disks have a greater risk of failure. You can do this for the [entire cluster](/docs/v23.2/configure-replication-zones#edit-the-default-replication-zone) or for specific [databases](/docs/v23.2/configure-replication-zones#create-a-replication-zone-for-a-database), [tables](/docs/v23.2/configure-replication-zones#create-a-replication-zone-for-a-table), or [rows](/docs/v23.2/configure-replication-zones#create-a-replication-zone-for-a-partition).
- Considerations for on-premises storage infrastructure:
    - **Distributed file systems** such as CephFS and GlusterFS should not be used for CockroachDB store volumes. CockroachDB already handles [data distribution](/docs/v23.2/architecture/distribution-layer), [replication](/docs/v23.2/architecture/replication-layer), and fault tolerance using [Raft](/docs/v23.2/architecture/replication-layer#raft). Adding a distributed file system underneath creates a second, uncoordinated layer that can cause duplicate replication, higher and more variable latency, and more complex failures.
...
<Note>
Under-provisioning storage leads to node crashes when the disks fill up. Once this has happened, it is difficult to recover from. To prevent your disks from filling up, provision enough storage for your workload, monitor your disk usage, and use a [ballast file](/docs/v23.2/cluster-setup-troubleshooting#automatic-ballast-files). For more information, see [capacity planning issues](/docs/v23.2/cluster-setup-troubleshooting#capacity-planning-issues) and [storage issues](/docs/v23.2/cluster-setup-troubleshooting#storage-issues).
</Note>
...
- [Monitor IOPS](/docs/v23.2/common-issues-to-monitor#disk-iops) using the DB Console and `iostat`. Ensure that they remain within acceptable values.
...
- Do not use LVM in the I/O path. Dynamically resizing CockroachDB store volumes can result in significant performance degradation. Using LVM snapshots in lieu of CockroachDB [backup and restore](/docs/v23.2/take-full-and-incremental-backups) is also not supported. Use [multiple stores per node](/docs/v23.2/cockroach-start#store) instead.

<Note>
Disk I/O especially affects [performance on write-heavy workloads](/docs/v23.2/architecture/reads-and-writes-overview#network-and-i-o-bottlenecks). For more information, see [capacity planning issues](/docs/v23.2/cluster-setup-troubleshooting#capacity-planning-issues) and [node liveness issues](/docs/v23.2/cluster-setup-troubleshooting#node-liveness-issues).
</Note>
...
These results were achieved using the ["bank" workload](/docs/v23.2/cockroach-workload#bank-workload) running on AWS using 6x c5d.4xlarge nodes, each with 5 TiB of gp2 EBS storage.
...
Therefore, to deploy CockroachDB in production, it is strongly recommended to [use TLS certificates to authenticate](/docs/v23.2/authentication) the identity of nodes and clients and to encrypt data in transit between nodes and clients. You can use either the built-in [`cockroach cert` commands](/docs/v23.2/cockroach-cert) or [`openssl` commands](/docs/v23.2/create-security-certificates-openssl) to generate security certificates for your deployment. Regardless of which option you choose, you'll need the following files:
...
- A separate certificate and key for each client and user you want to connect to your nodes, with the common name set to the username. The default user is `root`.
    ...
    Alternatively, CockroachDB supports [password authentication](/docs/v23.2/authentication#client-authentication), although we typically recommend using client certificates instead.

For more information, see the [Security Overview](/docs/v23.2/security-reference/security-overview).

## Networking

### Networking flags

When [starting a node](/docs/v23.2/cockroach-start), two main flags are used to control its network connections:
...
<table><thead><tr><th>Environment</th><th>Featured Approach</th></tr></thead><tbody><tr><td><a href="/docs/v23.2/deploy-cockroachdb-on-premises#step-6-set-up-load-balancing">On-Premises</a></td><td>Use HAProxy.</td></tr><tr><td><a href="/docs/v23.2/deploy-cockroachdb-on-aws#step-4-set-up-load-balancing">AWS</a></td><td>Use Amazon's managed load balancing service.</td></tr><tr><td><a href="/docs/v23.2/deploy-cockroachdb-on-microsoft-azure#step-4-set-up-load-balancing">Azure</a></td><td>Use Azure's managed load balancing service.</td></tr><tr><td><a href="/docs/v23.2/deploy-cockroachdb-on-digital-ocean#step-3-set-up-load-balancing">Digital Ocean</a></td><td>Use Digital Ocean's managed load balancing service.</td></tr><tr><td><a href="/docs/v23.2/deploy-cockroachdb-on-google-cloud-platform#step-4-set-up-load-balancing">GCP</a></td><td>Use GCP's managed TCP proxy load balancing service.</td></tr></tbody></table>
...
For guidance on sizing, validating, and using connection pools with CockroachDB, see [Use Connection Pools](/docs/v23.2/connection-pooling).
...
CockroachDB is purpose-built to be fault-tolerant and to recover automatically, but sometimes disasters happen. Having a [disaster recovery](/docs/v23.2/disaster-recovery-planning) plan enables you to recover quickly, while limiting the consequences.

Taking regular backups of your data in production is an operational best practice. You can create [full](/docs/v23.2/take-full-and-incremental-backups#full-backups) or [incremental](/docs/v23.2/take-full-and-incremental-backups#incremental-backups) backups of a cluster, database, or table. We recommend taking backups to [cloud storage](/docs/v23.2/use-cloud-storage) and enabling [object locking](/docs/v23.2/use-cloud-storage#immutable-storage) to protect the validity of your backups. CockroachDB supports Amazon S3, Azure Storage, and Google Cloud Storage for backups.

For details about available backup and restore types in CockroachDB, see [Backup and restore types](/docs/v23.2/backup-and-restore-overview#backup-and-restore-support).
...
CockroachDB manages its own memory caches, independently of the operating system. These are configured via the [`--cache`](/docs/v23.2/cockroach-start#flags) and [`--max-sql-memory`](/docs/v23.2/cockroach-start#flags) flags.
...
- Increasing a node's **SQL memory size** will increase the number of simultaneous client connections it allows, as well as the node's capacity for in-memory processing of rows when using `ORDER BY`, `GROUP BY`, `DISTINCT`, joins, and window functions.
    ...
    <Note>
    SQL memory size applies a limit globally to all sessions at any point in time. Certain disk-spilling operations also respect a memory limit that applies locally to a single operation within a single query. This limit is configured via a separate cluster setting. For details, see [Disk-spilling operations](/docs/v23.2/vectorized-execution#disk-spilling-operations).
    </Note>

    If a node runs out of its allocated SQL memory, a `memory budget exceeded` error occurs and the `cockroach` process may be at risk of crashing due to an out-of-memory (OOM) error. To mitigate this issue, refer to [&#96;memory budget exceeded](/docs/v23.2/common-errors#memory-budget-exceeded).

To manually increase a node's cache size and SQL memory size, start the node using the [`--cache`](/docs/v23.2/cockroach-start#flags) and [`--max-sql-memory`](/docs/v23.2/cockroach-start#flags) flags. As long as all machines are [provisioned with sufficient RAM](#memory), you can experiment with increasing each value up to `35%`.
...
To help guard against [OOM events](/docs/v23.2/cluster-setup-troubleshooting#out-of-memory-oom-crash), CockroachDB sets a soft memory limit using mechanisms in Go. Depending on your hardware and workload, you may not need to manually tune `--max-sql-memory`.
...
The [CockroachDB binary for Linux](/docs/v23.2/install-cockroachdb) depends on the following libraries:

<table><thead><tr><th>Library</th><th>Description</th></tr></thead><tbody><tr><td><a href="https://www.gnu.org/software/libc/libc"><code>glibc</code></a></td><td>The standard C library.<br/><br/>If you build CockroachDB from source, rather than use the official CockroachDB binary for Linux, you can use any C standard library, including MUSL, the C standard library used on Alpine.</td></tr><tr><td><code>libncurses</code></td><td>Required by the <a href="/docs/v23.2/cockroach-sql">built-in SQL shell</a>.</td></tr><tr><td><a href="https://www.iana.org/time-zones"><code>tzdata</code></a></td><td>Required by certain features of CockroachDB that use time zone data, for example, to support using location-based names as time zone identifiers. This library is sometimes called <code>tz</code> or <code>zoneinfo</code>.<br/><br/>The CockroachDB binary now includes Go's copy of the <code>tzdata</code> library. If CockroachDB cannot find the <code>tzdata</code> library externally, it will use this built-in copy.</td></tr></tbody></table>
...
- Use [SSDs instead of traditional HDDs](/docs/v23.2/kubernetes-performance#disk-type).
- Configure CPU and memory [resource requests and limits](/docs/v23.2/kubernetes-performance#resource-requests-and-limits).

For more information and additional customization suggestions, see our full detailed guide to [CockroachDB Performance on Kubernetes](/docs/v23.2/kubernetes-performance).

## Transaction retries

When several transactions try to modify the same underlying data concurrently, they may experience [contention](/docs/v23.2/performance-best-practices-overview#transaction-contention) that leads to [transaction retries](/docs/v23.2/transactions#transaction-retries). To avoid failures in production, your application should be engineered to handle transaction retries using [client-side retry handling](/docs/v23.2/transaction-retry-error-reference#client-side-retry-handling).
