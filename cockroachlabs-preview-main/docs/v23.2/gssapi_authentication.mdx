...
2. [Create certificates](/docs/v23.2/cockroach-cert) for inter-node and `root` user authentication:
...
5. Connect to CockroachDB as `root` using the `root` client certificate generated above:
    ...
    ```
6. [Enable an Enterprise license](/docs/v23.2/licensing-faqs#obtain-a-license).
...
7. Enable GSSAPI authentication:
    ...
    Setting the `server.host_based_authentication.configuration` [cluster setting](/docs/v23.2/cluster-settings) to this particular value makes it mandatory for all non-`root` users to authenticate using GSSAPI. The `root` user is always an exception and remains able to authenticate using a valid client cert or a user password.
