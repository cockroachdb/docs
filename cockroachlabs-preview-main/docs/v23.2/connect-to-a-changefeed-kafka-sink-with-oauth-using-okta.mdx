...
old_description: "Learn how to set up OAuth through Okta for a changefeed streaming to Kafka."
---

CockroachDB Enterprise [changefeeds](/docs/v23.2/change-data-capture-overview) can stream change data out to [Apache Kafka](https://kafka.apache.org/) using OAuth authentication.
...
- A CockroachDB cluster. You can use a CockroachDB Cloud or CockroachDB self-hosted cluster.
    - If you are using CockroachDB CockroachDB Cloud, refer to the [Quickstart with CockroachDB](/docs/cockroachcloud/quickstart) guide. For CockroachDB self-hosted clusters, refer to the [install](/docs/v23.2/install-cockroachdb) page.
...
3. Create a file in the `config` directory called `kafka_server_jaas.conf`:
    ...
    The [`OAuthBearerLoginModule`](https://kafka.apache.org/20/javadoc/org/apache/kafka/common/security/oauthbearer/OAuthBearerLoginModule) will ask the `OauthAuthenticateLoginCallbackHandler` (configured in the `server.properties` file) to return an `OAuthBearerToken`.
...
1. Clone the library:
    ...
    ```
2. Install [Apache Maven](https://maven.apache.org/what-is-maven), a tool for building Java-based projects:
...
Since this is a long URI to run, you can create an [external connection](/docs/v23.2/create-external-connection) to represent this URI:
