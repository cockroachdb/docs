...
old_description: "SQL users are required to pass encryption keys when reading from or writing to specific columns"
---

import { InlineImage } from "/snippets/InlineImage.jsx";

<InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v23.2: CockroachDB supports *column level encryption*. Using this feature, you can encrypt one or more of the columns in each row of a database table.

Column level encryption can be useful for compliance scenarios such as adhering to PCI.

Once a column is encrypted, SQL users are required to pass an encryption key when reading from or writing to that column. This prevents users without the encryption key from taking actions such as reading sensitive information (like *personally identifiable information* ([PII](https://en.wikipedia.org/wiki/Personal_data))) when accessing the tables.
...
The `encrypt` and `encrypt_iv` functions encrypt a column's data with a given key and cipher method. For more information, see [Cryptographic functions](/docs/v23.2/functions-and-operators#cryptographic-functions).
...
If you do not have a [license](/docs/v23.2/licensing-faqs), you will see an error message like the following if you try to use them:
...
The `decrypt` and `decrypt_iv` functions decrypt an encrypted column's data with a given key and cipher method. For more information, see [Cryptographic functions](/docs/v23.2/functions-and-operators#cryptographic-functions).
...
<Note>
The columns that will store the encrypted values must be of type [`BYTES`](/docs/v23.2/bytes) as shown below.
</Note>
