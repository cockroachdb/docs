...
import { CreateType2 } from "/snippets/syntax-diagrams/CreateType2.jsx";

The `CREATE TYPE` [statement](/docs/v23.2/sql-statements) creates a new type in a [database](/docs/v23.2/create-database). After the type is created, it can only be referenced from the database that contains the type.
...
<Note>
The `CREATE TYPE` statement performs a schema change. For more information about how online schema changes work in CockroachDB, see [Online Schema Changes](/docs/v23.2/online-schema-changes).
</Note>
...
To see the values accepted by the underlying [`ENUM`](/docs/v23.2/enum) data type, use [`SHOW ENUMS`](/docs/v23.2/show-enums):
...
- [`SHOW ENUMS`](/docs/v23.2/show-enums)
