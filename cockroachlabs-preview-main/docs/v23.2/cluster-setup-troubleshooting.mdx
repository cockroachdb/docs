...
Other services may be running on port 26257 or 8080 (CockroachDB's default `--listen-addr` port and `--http-addr` port respectively). You can either stop those services or start your node with different ports, specified in the [`--listen-addr` and `--http-addr` flags](/docs/v23.2/cockroach-start#networking).
...
Networking issues might prevent the node from communicating with itself on its hostname. You can control the hostname CockroachDB uses with the [`--listen-addr` flag](/docs/v23.2/cockroach-start#networking).
...
See [Why is my process hanging when I try to start it in the background?](/docs/v23.2/operational-faqs#why-is-my-process-hanging-when-i-try-to-start-nodes-with-the-background-flag)

## Cannot run SQL statements using built-in SQL client

If the CockroachDB node appeared to [start successfully](/docs/v23.2/start-a-local-cluster), in a separate terminal run:
...
- `connection refused` error, which indicates you have not included some flag that you used to start the node. We have additional troubleshooting steps for this error [here](/docs/v23.2/common-errors#connection-refused).
- The node crashed. To ascertain if the node crashed, run `ps | grep cockroach` to look for the `cockroach` process. If you cannot locate the `cockroach` process (i.e., it crashed), [file an issue](/docs/v23.2/file-an-issue), including the [logs from your node](/docs/v23.2/configure-logs#logging-directory) and any errors you received.

## Cannot run a multi-node CockroachDB cluster on the same machine

<Note>
Running multiple nodes on a single host is useful for testing CockroachDB, but it's not recommended for production deployments. To run a physically distributed cluster in production, see [Manual Deployment](/docs/v23.2/manual-deployment) or [Kubernetes Overview](/docs/v23.2/kubernetes-overview). Also be sure to review the [Production Checklist](/docs/v23.2/recommended-production-settings).
</Note>
...
The `kv.snapshot_rebalance.max_rate` [cluster setting](/docs/v23.2/cluster-settings#setting-kv-snapshot-rebalance-max-rate) sets the rate limit at which [snapshots](/docs/v23.2/architecture/replication-layer#snapshots) are sent to nodes. This setting can be temporarily increased to expedite replication during an outage or when scaling a cluster up or down.

However, if the setting is too high when nodes are added to the cluster, this can cause degraded performance and node crashes. We recommend **not** increasing this value by more than 2 times its [default value](/docs/v23.2/cluster-settings#setting-kv-snapshot-rebalance-max-rate) without explicit approval from Cockroach Labs.

**Explanation:** If `kv.snapshot_rebalance.max_rate` is set too high for the cluster during scaling, this can cause nodes to experience ingestions faster than [compactions](/docs/v23.2/architecture/storage-layer#compaction) can keep up, and result in an [inverted LSM](/docs/v23.2/architecture/storage-layer#inverted-lsms).

**Solution:** [Check LSM health](/docs/v23.2/common-issues-to-monitor#lsm-health). If compaction has fallen behind and caused an [inverted LSM](/docs/v23.2/architecture/storage-layer#inverted-lsms), throttle your workload concurrency to allow [compaction](/docs/v23.2/architecture/storage-layer#compaction) to catch up and restore a healthy LSM shape. The total number of workload connections across all connection pools **should not exceed 4 times the number of vCPUs** in the cluster by a large amount. If a node is severely impacted, you can [start a new node](/docs/v23.2/cockroach-start) and then [decommission the problematic node](/docs/v23.2/node-shutdown?filters=decommission#remove-nodes). By default, the [storage engine](/docs/v23.2/architecture/storage-layer) uses a compaction concurrency of 3. If you have sufficient IOPS and CPU headroom, you can consider increasing this setting via the `COCKROACH_COMPACTION_CONCURRENCY` environment variable. This may help to reshape the LSM more quickly in [inverted LSM](/docs/v23.2/architecture/storage-layer#inverted-lsms) scenarios; and it can lead to increased overall performance for some workloads. Cockroach Labs **strongly recommends** testing your workload against non-default values of this setting.

After [compaction](/docs/v23.2/architecture/storage-layer#compaction) has completed, lower `kv.snapshot_rebalance.max_rate` to its [default value](/docs/v23.2/cluster-settings#setting-kv-snapshot-rebalance-max-rate). As you add nodes to the cluster, slowly increase the value of the cluster setting, if desired. This will control the rate of new ingestions for newly added nodes. Meanwhile, monitor the cluster for unhealthy increases in [IOPS](/docs/v23.2/common-issues-to-monitor#disk-iops) and [CPU](/docs/v23.2/common-issues-to-monitor#cpu).

Outside of performing cluster maintenance, return `kv.snapshot_rebalance.max_rate` to its [default value](/docs/v23.2/cluster-settings#setting-kv-snapshot-rebalance-max-rate).
...
- Inaccessible hostnames on your nodes, which can be controlled with the `--listen-addr` and `--advertise-addr` flags on [`cockroach start`](/docs/v23.2/cockroach-start#networking)
...
The effect of a network partition depends on which nodes are partitioned, where the ranges are located, and to a large extent, whether [localities](/docs/v23.2/cockroach-start#locality) or [zone configurations](/docs/v23.2/configure-replication-zones) are defined. A partition that cuts off at least `(n-1)/2` replicas from a [range](/docs/v23.2/architecture/overview#architecture-range) will cause range unavailability which will cause some data unavailability. If there are no localities or other constraints on where replicas are placed, then a partition of any ([`num_replicas`](/docs/v23.2/configure-replication-zones#num_replicas) / 2) nodes will likely cause unavailability.

**Solution:**

To identify a network partition:

1. In the DB Console, access the [Network page](/docs/v23.2/ui-network-latency-page).
2. In the network matrix, check for nodes with [no connections](/docs/v23.2/ui-network-latency-page#no-connections). This indicates that a node cannot communicate with another node, and might indicate a network partition.
...
1. [Access the DB Console](/docs/v23.2/ui-overview#db-console-access).
...
1. For a period of up to 40 seconds, clients trying to connect with [username and password authentication](/docs/v23.2/authentication#client-authentication) cannot create new connections to any of the remaining nodes in the cluster.
...
- Since all system ranges are located [near the beginning of the keyspace](/docs/v23.2/architecture/distribution-layer#monolithic-sorted-map-structure), the system range containing the username/password info can sometimes be colocated with another system range that is used to determine [node liveness](#node-liveness-issues).
- If the username/password info and the node liveness record are stored together as described above, it can take extra time for the lease on this range to be transferred to another node. Normally, [lease transfers take a few seconds](/docs/v23.2/architecture/replication-layer#how-leases-are-transferred-from-a-dead-node), but in this case it may require multiple rounds of consensus to determine that the node in question is actually dead (the node liveness record check may be retried several times before failing).

For more information about how lease transfers work when a node dies, see [How leases are transferred from a dead node](/docs/v23.2/architecture/replication-layer#how-leases-are-transferred-from-a-dead-node).

The solution is to [use connection pooling](/docs/v23.2/connection-pooling).
...
[Restart](/docs/v23.2/node-shutdown) each node if:
...
- [What happens when node clocks are not properly synchronized](/docs/v23.2/operational-faqs#what-happens-when-node-clocks-are-not-properly-synchronized)
- [How can I tell how well node clocks are synchronized](/docs/v23.2/operational-faqs#how-can-i-tell-how-well-node-clocks-are-synchronized)
...
- Running storage at 100% utilization read/write causes poor service time and [node shutdown](/docs/v23.2/operational-faqs#what-happens-when-a-node-runs-out-of-disk-space).
- Running network at 100% utilization causes response between databases and client to be poor.

**Solution:** [Access the DB Console](/docs/v23.2/ui-overview#db-console-access) and navigate to **Metrics &gt; Hardware** dashboard to monitor the following metrics:
...
<Note>
For minimum provisioning guidelines, see [Basic hardware recommendations](/docs/v23.2/recommended-production-settings#basic-hardware-recommendations).
</Note>

Check for resources that are running out of capacity:

<table><thead><tr><th>Type</th><th>Time Series</th><th>What to look for</th></tr></thead><tbody><tr><td>RAM capacity</td><td>Memory Usage</td><td>Consistently more than 80%</td></tr><tr><td>CPU capacity</td><td>CPU Percent</td><td>Consistently less than 20% in idle (i.e., 80% busy)</td></tr><tr><td>Disk capacity</td><td>Available Disk Capacity</td><td>Consistently less than 20% of the <a href="/docs/v23.2/cockroach-start#store">store</a> size</td></tr><tr><td>Disk I/O</td><td>Disk Ops In Progress</td><td>Consistent double-digit values</td></tr><tr><td>Network capacity</td><td>Network Bytes Received<br/>Network Bytes Sent</td><td>Consistently more than 50% capacity for both</td></tr></tbody></table>
...
- [What happens when a node runs out of disk space?](/docs/v23.2/operational-faqs#what-happens-when-a-node-runs-out-of-disk-space)
- [Why is memory usage increasing despite lack of traffic?](/docs/v23.2/operational-faqs#why-is-memory-usage-increasing-despite-lack-of-traffic)
- [Why is disk usage increasing despite lack of writes?](/docs/v23.2/operational-faqs#why-is-disk-usage-increasing-despite-lack-of-writes)
- [Can I reduce the storage of timeseries data?](/docs/v23.2/operational-faqs#can-i-reduce-the-storage-of-time-series-data)

###### Automatic ballast files

CockroachDB automatically creates an emergency ballast file at [node startup](/docs/v23.2/cockroach-start). This feature is **on** by default. Note that the [`cockroach debug ballast`](/docs/v23.2/cockroach-debug-ballast) command is still available but deprecated.

The ballast file defaults to 1% of total disk capacity or 1 GiB, whichever is smaller. The size of the ballast file may be configured using [the `--store` flag to `cockroach start`](/docs/v23.2/cockroach-start#flags-store) with a [`ballast-size` field](/docs/v23.2/cockroach-start#fields-ballast-size); this field accepts the same value formats as the `size` field.
...
- Disk operations have slowed due to underprovisioned IOPS. Make sure you are deploying with our [recommended production settings for storage](/docs/v23.2/recommended-production-settings#storage) and [monitoring disk IOPS](/docs/v23.2/common-issues-to-monitor#disk-iops).
...
- Every 10 seconds, the CockroachDB storage engine checks the [*write-ahead log*](https://wikipedia.org/wiki/Write-ahead_logging), or *WAL*. If data has not been synced to disk (via `fsync`) within that interval, the log message `disk stall detected: unable to write to %s within %s %s warning log entry` is written to the [`STORAGE` logging channel](/docs/v23.2/logging#storage). If this state continues for 20 seconds or more (configurable with the `COCKROACH_ENGINE_MAX_SYNC_DURATION` environment variable), the `cockroach` process is terminated.
- Every time the storage engine writes to the main [`cockroach.log` file](/docs/v23.2/logging#dev), the engine waits 20 seconds for the write to succeed (configurable with the `COCKROACH_LOG_MAX_SYNC_DURATION` environment variable). If the write to the log fails, the `cockroach` process is terminated and the following message is written to `stderr` / `cockroach.log`, providing details regarding the type, size, and duration of the ongoing write:
    - `file write stall detected: %s`
- During [node liveness heartbeats](#node-liveness-issues), the [storage engine](/docs/v23.2/architecture/storage-layer) writes to disk as part of the node liveness heartbeat process.

#### Disk utilization is different across nodes in the cluster

This is expected behavior.

CockroachDB uses [load-based replica rebalancing](/docs/v23.2/architecture/replication-layer#load-based-replica-rebalancing) to spread [replicas](/docs/v23.2/architecture/overview#architecture-replica) across the nodes in a cluster.

The rebalancing criteria for load-based replica rebalancing do not include the percentage of disk utilized per node. Not all [ranges](/docs/v23.2/architecture/overview#architecture-range) are the same size. The proportions of larger and smaller ranges on each node balance each other out on average, so disk utilization differences across nodes should be relatively small.

However, in some cases a majority of the largest (or smallest) ranges are on one node, which will result in imbalanced utilization. Normally that shouldn't be a problem with [sufficiently provisioned storage](/docs/v23.2/recommended-production-settings#storage). If this imbalance is causing issues, please [contact Support](/docs/v23.2/support-resources) for guidance you on how to manually rebalance your cluster's disk usage.

Replica counts can also remain uneven when CockroachDB keeps replicas on a node to preserve survivability through locality diversity, or to satisfy [replica placement constraints](/docs/v23.2/configure-replication-zones#replication-constraints). For more information, refer to the [Replication Layer](/docs/v23.2/architecture/replication-layer#membership-changes-rebalance-repair) documentation.
...
Issues with CPU most commonly arise when there is insufficient CPU to support the scale of the workload. If the concurrency of your workload significantly exceeds your provisioned CPU, you will encounter a [degradation in SQL response time](/docs/v23.2/common-issues-to-monitor#service-latency). This is the most common symptom of CPU starvation.

Because [compaction](/docs/v23.2/architecture/storage-layer#compaction) requires significant CPU to run concurrent worker threads, a lack of CPU resources will eventually cause compaction to fall behind. This leads to [read amplification](/docs/v23.2/architecture/storage-layer#read-amplification) and inversion of the log-structured merge (LSM) trees on the [storage layer](/docs/v23.2/architecture/storage-layer).
...
- [Check for high CPU usage.](/docs/v23.2/common-issues-to-monitor#cpu-usage)
- [Check your workload concurrency](/docs/v23.2/common-issues-to-monitor#workload-concurrency) and compare it to your provisioned CPU.
    - To prevent issues with workload concurrency, [provision sufficient CPU](/docs/v23.2/recommended-production-settings#sizing) and use [connection pooling](/docs/v23.2/recommended-production-settings#connection-pooling) for the workload.
- [Check LSM health](/docs/v23.2/common-issues-to-monitor#lsm-health), which can be affected over time by CPU starvation.
...
A CockroachDB node will grow to consume all of the memory allocated for its `--cache`, [even if your cluster is idle](/docs/v23.2/operational-faqs#why-is-memory-usage-increasing-despite-lack-of-traffic). The default cache size is 25% of physical memory, which can be substantial, depending on your machine configuration. For more information, see [Cache and SQL memory size](/docs/v23.2/recommended-production-settings#cache-and-sql-memory-size).
...
- **CGo allocated memory**: Memory allocated by the C/C++ libraries linked into CockroachDB and primarily concerns the block caches for the [Pebble storage engine](/docs/v23.2/cockroach-start#storage-engine) ). This is the allocation specified with `--cache`. The size of CGo allocated memory is usually very close to the configured `--cache` size.
...
3. On hovering over the graph, the values for the following metrics are displayed:
    ...
    **Expected values for a healthy cluster**: Go Allocated will depend on workload but should not exceed [`--max-sql-memory`](/docs/v23.2/cockroach-start#flags) by more than 100%. CGo Allocated should not exceed the [`--cache`](/docs/v23.2/cockroach-start#flags) size and CGo Total should not exceed the [`--cache`](/docs/v23.2/cockroach-start#flags) size by more than 15%.

    If you observe values not within the expected range for a healthy cluster, [file an issue](/docs/v23.2/file-an-issue).
...
**Solution:** If you [observe nodes restarting after sudden crashes](/docs/v23.2/common-issues-to-monitor#node-process-restarts):

- [Confirm that the node restarts are caused by OOM crashes.](/docs/v23.2/common-issues-to-monitor#verify-oom-errors)
    - To prevent OOM crashes, [provision sufficient memory](/docs/v23.2/recommended-production-settings#memory). If all CockroachDB machines are provisioned and configured correctly, either run the CockroachDB process on another node with sufficient memory, or [reduce the memory allocated to CockroachDB](/docs/v23.2/recommended-production-settings#cache-and-sql-memory-size).
- [Check whether SQL queries may be responsible.](/docs/v23.2/common-issues-to-monitor#sql-memory-usage)

## Decommissioning issues

#### Decommissioning process hangs indefinitely

If the [decommissioning process](/docs/v23.2/node-shutdown?filters=decommission#remove-nodes) appears to be hung on a node, a message like the following will print to `stderr`:
...
**Explanation:** Before decommissioning a node, you need to make sure other nodes are available to take over the range replicas from the node. If no other nodes are available, the decommission process will hang indefinitely. For more information, see [Node Shutdown](/docs/v23.2/node-shutdown?filters=decommission#size-and-replication-factor).
...
- Under-replicated Ranges: When a cluster is first initialized, the few default starting ranges have a single replica. As more nodes become available, the cluster replicates these ranges to other nodes until the number of replicas for each range reaches the desired [replication factor](/docs/v23.2/configure-replication-zones#num_replicas) (3 by default). If a range has fewer replicas than the replication factor, the range is said to be "under-replicated". [Non-voting replicas](/docs/v23.2/architecture/replication-layer#non-voting-replicas), if configured, are not counted when calculating replication status.
...
3. Check for a network partition: Click the gear icon on the left-hand navigation bar to access the **Advanced Debugging** page. On the Advanced Debugging page, click **Network Latency**. In the **Latencies** table, check if any cells are marked as "X". If yes, it indicates that the nodes cannot communicate with those nodes, and might indicate a network partition. If there's no partition, and there's still no upreplication after 5 mins, then [file an issue](/docs/v23.2/file-an-issue).
...
1. [Access the DB Console](/docs/v23.2/ui-overview#db-console-access)
2. Click the gear icon on the left-hand navigation bar to access the **Advanced Debugging** page.
3. Click **Problem Ranges**.
4. In the **Connections** table, identify the node with the under-replicated/unavailable ranges and click the node ID in the Node column.
5. To view the **Range Report** for a range, click on the range number in the **Under-replicated (or slow)** table or **Unavailable** table.
6. On the Range Report page, scroll down to the **Simulated Allocator Output** section. The table contains an error message which explains the reason for the under-replicated range. Follow the guidance in the message to resolve the issue. If you need help understanding the error or the guidance, [file an issue](/docs/v23.2/file-an-issue). Please be sure to include the full Range Report and error message when you submit the issue.

#### Check for under-replicated or unavailable data

To see if any data is under-replicated or unavailable in your cluster, follow the steps described in [Critical nodes endpoint](/docs/v23.2/monitoring-and-alerting#critical-nodes-endpoint).

#### Check for replication zone constraint violations

To see if any of your cluster's [data placement constraints](/docs/v23.2/configure-replication-zones#replication-constraints) are being violated, follow the steps described in [Check for critical localities](#check-for-critical-localities).

#### Check for critical localities

To see which of your [localities](/docs/v23.2/cockroach-start#locality) (if any) are critical, follow the steps described in the [Critical nodes endpoint documentation](/docs/v23.2/monitoring-and-alerting#critical-nodes-endpoint). A locality is "critical" for a range if all of the nodes in that locality becoming [unreachable](#node-liveness-issues) would cause the range to become unavailable. In other words, the locality contains a majority of the range's replicas.
...
The [DB Console](/docs/v23.2/ui-overview) provides several ways to check for node liveness issues in your cluster:

- [Check node heartbeat latency](/docs/v23.2/common-issues-to-monitor#node-heartbeat-latency)
- [Check command commit latency](/docs/v23.2/common-issues-to-monitor#command-commit-latency)

<Note>
For more information about how node liveness works, see [Replication Layer](/docs/v23.2/architecture/replication-layer#epoch-based-leases-table-data).
</Note>

#### Impact of node failure is greater than 10 seconds

When the cluster needs to access a range on a leaseholder node that is dead, that range's [lease must be transferred to a healthy node](/docs/v23.2/architecture/replication-layer#how-leases-are-transferred-from-a-dead-node). In theory, this process should take no more than a few seconds for liveness expiration plus the cost of several network roundtrips.

In production, lease transfer upon node failure can take longer than expected. In v23.2, this is observed in the following scenarios:

- **The leaseholder node for the liveness range fails.** The liveness range is a system range that [stores the liveness record](/docs/v23.2/architecture/replication-layer#epoch-based-leases-table-data) for each node on the cluster. If a node fails and is also the leaseholder for the liveness range, operations cannot proceed until the liveness range is [transferred to a new leaseholder](/docs/v23.2/architecture/replication-layer#how-leases-are-transferred-from-a-dead-node) and the liveness record is made available to other nodes. This can cause momentary cluster unavailability.
- **Network or DNS issues cause connection issues between nodes.** If there is no live server for the IP address or DNS lookup, connection attempts to a node will not return an immediate error, but will hang [until timing out](/docs/v23.2/architecture/distribution-layer#grpc). This can cause unavailability and prevent a speedy movement of leases and recovery. CockroachDB avoids contacting unresponsive nodes or DNS during certain performance-critical operations, and the connection issue should generally resolve in 10-30 seconds. However, an attempt to contact an unresponsive node could still occur in other scenarios that are not yet addressed.
- **A node's disk stalls.** A [disk stall](#disk-stalls) on a node can cause write operations to stall indefinitely, also causes the node's heartbeats to fail since the storage engine cannot write to disk as part of the heartbeat, and may cause read requests to fail if they are waiting for a conflicting write to complete. Lease acquisition from this node can stall indefinitely until the node is shut down or recovered. Pebble detects most stalls and will terminate the `cockroach` process after 20 seconds, but there are gaps in its detection. In v22.1.2+ and v22.2+, each lease acquisition attempt on an unresponsive node [times out after a few seconds](/docs/v23.2/architecture/replication-layer#how-leases-are-transferred-from-a-dead-node). However, CockroachDB can still appear to stall as these timeouts are occurring.
...
**Solution:** If you are experiencing intermittent network or connectivity issues, first [shut down the affected nodes](/docs/v23.2/node-shutdown) temporarily so that nodes phasing in and out do not cause disruption.

If a node has become unresponsive without returning an error, [shut down the node](/docs/v23.2/node-shutdown) so that network requests immediately become hard errors rather than stalling.
...
If your cluster is in a partially-available state due to a recent node or network failure, the internal logging table `system.eventlog` might be unavailable. This can cause the logging of [notable events](/docs/v23.2/eventlog) (e.g., the execution of SQL statements) to the `system.eventlog` table to fail to complete, contributing to cluster unavailability. If this occurs, you can set the [cluster setting](/docs/v23.2/cluster-settings) `server.eventlog.enabled` to `false` to disable writing notable log events to this table, which may help to recover your cluster.

Even with `server.eventlog.enabled` set to `false`, notable log events are still sent to configured [log sinks](/docs/v23.2/configure-logs#configure-log-sinks) as usual.

## Something else?

If we do not have a solution here, you can try using our other [support resources](/docs/v23.2/support-resources), including:
