...
old_description: "Learn how metrics can differ between third-party monitoring tools that are integrated with CockroachDB and the DB Console of CockroachDB."
---

When using [Third-Party Monitoring Integrations](/docs/v23.2/third-party-monitoring-tools), such as the [metrics export feature](/docs/cockroachcloud/export-metrics), discrepancies may be seen when comparing those metrics charts to ones found on the [Metrics dashboards](/docs/v23.2/ui-overview#metrics) or [custom charts](/docs/v23.2/ui-custom-chart-debug-page) of the [DB Console](/docs/v23.2/ui-overview). This page explains why these different systems may yield different results.

## CockroachDB’s Timeseries Database

CockroachDB stores metrics data in its own internal timeseries database (TSDB). While CockroachDB exposes [point-in-time metrics](/docs/v23.2/monitor-cockroachdb-with-prometheus) via its [Prometheus endpoint](/docs/v23.2/monitoring-and-alerting#prometheus-endpoint), `/_status/vars`, it also periodically scrapes and writes this information to its own timeseries database storage. This data is scraped every 10 seconds and stored at that resolution. After a period of time determined by the [`timeseries.storage.resolution_10s.ttl` cluster setting](/docs/v23.2/cluster-settings#setting-timeseries-storage-resolution-10s-ttl), that 10 second resolution data is compacted into a 30 minute resolution. After another period of time determined by the [`timeseries.storage.resolution_30m.ttl cluster setting`](/docs/v23.2/cluster-settings#setting-timeseries-storage-resolution-30m-ttl), the 30 minute resolution data is deleted.
...
<table><thead><tr><th>time</th><th>t0</th><th>t1</th><th>t2</th><th>t3</th><th>t4</th><th>t5</th><th>t6</th></tr></thead><tbody><tr><td>TSDB scrapes every 10s</td><td>0</td><td>0</td><td>0</td><td>5</td><td>4</td><td>0</td><td>0</td></tr><tr><td>Datadog scrapes every 60s</td><td>0</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>0</td></tr></tbody></table>
