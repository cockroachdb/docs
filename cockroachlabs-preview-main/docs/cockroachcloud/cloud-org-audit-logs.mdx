...
CockroachDB Cloud captures audit logs when many types of events occur, such as when a cluster is created or when a user is added to or removed from an organization. Any user in an organization with a [service account](/docs/cockroachcloud/managing-access#manage-service-accounts) assigned the [Organization Admin role](/docs/cockroachcloud/authorization#organization-admin) can export these audit logs using the [`auditlogevents` endpoint](/docs/cockroachcloud/cloud-api#cloud-audit-logs) of the [Cloud API](/docs/cockroachcloud/cloud-api).

This page provides some examples of exporting CockroachDB Cloud organization audit logs. For details about each parameter and its defaults, refer to the API specification for the [`auditlogevents` endpoint](/docs/cockroachcloud/cloud-api#cloud-audit-logs).
...
- Learn more about the [Cloud API](/docs/cockroachcloud/cloud-api)
