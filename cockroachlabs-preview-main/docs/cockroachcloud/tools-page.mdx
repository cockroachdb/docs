...
To view the **Tools** page, select a cluster from the [**Clusters** page](/docs/cockroachcloud/cluster-management#view-clusters-page), and click **Tools** in the **Monitoring** section of the left side navigation.

## Monitor CockroachDB Cloud with Datadog

The [CockroachDB Cloud integration for Datadog](https://docs.datadoghq.com/integrations/cockroachdb_dedicated) enables data collection and alerting on a [subset of CockroachDB metrics](#available-metrics) available at the [Prometheus endpoint](/docs/v25.4/monitoring-and-alerting#prometheus-endpoint), using the Datadog platform.
...
- Use the [Cloud API](/docs/cockroachcloud/cloud-api), following the instructions on [Export Metrics From a CockroachDB Standard Cluster](/docs/cockroachcloud/export-metrics?filters=datadog-metrics-export) or [Export Metrics From a CockroachDB Advanced Cluster](/docs/cockroachcloud/export-metrics-advanced?filters=datadog-metrics-export).
...
The [available metrics](#available-metrics) are drawn directly from the CockroachDB [Prometheus endpoint](/docs/v25.4/monitoring-and-alerting#prometheus-endpoint) and are intended for use as building blocks for your own charts.

<Note>
Metric values and time-series graphs in Datadog are not guaranteed to match those in the [DB Console](#access-the-db-console), due to differences in how CockroachDB and Datadog calculate and display metrics. For more information, refer to [Differences in Metrics between Third-Party Monitoring Integrations and DB Console](/docs/v25.4/differences-in-metrics-between-third-party-monitoring-integrations-and-db-console).
</Note>
...
See [Metrics](/docs/v25.4/metrics) for the full list of metrics available in CockroachDB.
...
Select **Threshold Alert** as the detection method, which configures an alert that is sent when a supported metric reaches a given threshold. For descriptions of some useful CockroachDB alerts, see [Monitoring and Alerting](/docs/v25.4/monitoring-and-alerting#events-to-alert-on).
...
The CockroachDB Cloud integration for [Amazon CloudWatch](https://aws.amazon.com/cloudwatch) enables data collection and alerting on CockroachDB metrics available at the [Prometheus endpoint](/docs/v25.4/monitoring-and-alerting#prometheus-endpoint). It is only available with CockroachDB Standard and Advanced clusters hosted on AWS.
...
- Use the [Cloud API](/docs/cockroachcloud/cloud-api), following the instructions on [Export Metrics From a CockroachDB Standard Cluster](/docs/cockroachcloud/export-metrics) or [Export Metrics From a CockroachDB Advanced Cluster](/docs/cockroachcloud/export-metrics-advanced).
...
2. Fill in the **Role ARN**, **Target Region**, and **Log Group Name** fields with their corresponding values.
    - The **Role ARN** is in the format: `arn:aws:iam::{role_arn}:role/{role_name}`. To determine `{role_arn}` and `{role_name}`, follow the steps in the **Enable metrics export** section on [Export Metrics From a CockroachDB Standard Cluster](/docs/cockroachcloud/export-metrics#enable-metrics-export) or [Export Metrics From a CockroachDB Advanced Cluster](/docs/cockroachcloud/export-metrics-advanced#enable-metrics-export).
...
Metric values and time-series graphs in CloudWatch are not guaranteed to match those in the [DB Console](#access-the-db-console), due to differences in how CockroachDB and CloudWatch calculate and display metrics. For more information, refer to [Differences in Metrics between Third-Party Monitoring Integrations and DB Console](/docs/v25.4/differences-in-metrics-between-third-party-monitoring-integrations-and-db-console).
</Note>
...
2. Log in with your [SQL username](/docs/cockroachcloud/managing-access) and password.

<Note>
For details on creating additional users that can connect to the cluster and access the DB Console, see [User Management](/docs/cockroachcloud/managing-access).
</Note>

## Explore the DB Console

- For an overview of all the areas of the DB Console, see [DB Console Overview](/docs/v25.4/ui-overview).
- Be sure to check out the [**Node Map**](/docs/v25.4/ui-overview), which visualizes the geographic configuration of your cluster on a world map and provides real-time cluster metrics, with the ability to drill down to individual nodes. This Enterprise feature has been pre-configured and enabled for you.

<Note>
If you have a single-node cluster, you may see a warning that you have under-replicated ranges. This is expected because the default replication factor is set to 3, and you can only have one [replica](/docs/v25.4/architecture/overview#architecture-replica) per node. For more information about replication issues, see [Cluster Setup Troubleshooting](/docs/v25.4/cluster-setup-troubleshooting#db-console-shows-under-replicated-unavailable-ranges).
</Note>
