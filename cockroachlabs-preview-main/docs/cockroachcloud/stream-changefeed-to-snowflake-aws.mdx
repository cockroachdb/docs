...
This page demonstrates how to use a [changefeed](/docs/v25.4/create-changefeed) to stream row-level changes to [Snowflake](https://www.snowflake.com/), an online analytical processing (OLAP) database.
...
If you have not done so already, [create a cluster](/docs/cockroachcloud/create-your-cluster).

## Step 2. Connect to your cluster

Refer to [Connect to your cluster](/docs/cockroachcloud/connect-to-your-cluster) for detailed instructions on how to:
...
1. In your terminal, enter the `cockroach sql` command and connection string from [Step 2. Connect to your cluster](#step-2-connect-to-your-cluster) to start the [built-in SQL client](/docs/v25.4/cockroach-sql).
2. Enable [rangefeeds](/docs/v25.4/create-and-configure-changefeeds#enable-rangefeeds):
...
Back in the built-in SQL shell, [create a changefeed](/docs/v25.4/create-changefeed). Replace the placeholders with your AWS access key ID and AWS secret access key:
...
Refer to the [Cloud Storage Authentication](/docs/v26.1/cloud-storage-authentication) page for more detail on authenticating to Amazon S3 and other cloud providers.
...
You will receive the changefeed's job ID that you can use to [manage the changefeed](/docs/v25.4/create-and-configure-changefeeds#configure-a-changefeed) if needed.
...
2. Navigate back to the [S3 bucket](https://s3.console.aws.amazon.com/) to confirm that the data is now streaming to the bucket. A new date-based directory should display on the **Objects** tab.
    ...
    <Note>
    If your changefeed is running but data is not displaying in your S3 bucket, you might have to [debug your changefeed](/docs/v25.4/monitor-and-debug-changefeeds#debug-a-changefeed).
    </Note>
...
- Snowpipe is unaware of CockroachDB [resolved timestamps](/docs/v25.4/create-changefeed#resolved). This means CockroachDB transactions will not be loaded atomically and partial transactions can briefly be returned from Snowflake.
...
Refer to the [Create and Configure Changefeeds](/docs/v25.4/create-and-configure-changefeeds#known-limitations) page for more general changefeed known limitations.
