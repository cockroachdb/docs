...
Customer-Managed Encryption Keys (CMEK) allow you to protect data at rest in a CockroachDB Advanced [private cluster](/docs/cockroachcloud/private-clusters) using a cryptographic key that is entirely within your control, hosted in a supported cloud provider key-management system (KMS). This key is called the *CMEK key*.
...
To learn more, visit [Managing Customer-Managed Encryption Keys (CMEK) for CockroachDB Advanced](/docs/cockroachcloud/managing-cmek).
...
<Tip>
You can learn more about the [supported integrations between CockroachDB and HashiCorp Vault](/docs/v25.4/hashicorp-integration).
</Tip>
...
This article describes how CMEK works in CockroachDB Advanced clusters. To configure CMEK, see [Manage Customer-Managed Encryption Keys (CMEK) for CockroachDB Advanced](/docs/cockroachcloud/managing-cmek).
...
When you enable CMEK on a CockroachDB Advanced [private cluster](/docs/cockroachcloud/private-clusters), CockroachDB Cloud creates two kinds of encryption keys and begins to use them to protect newly-written data at rest. CockroachDB Cloud manages these encryption keys and propagates them to cluster nodes.
...
For more details about encryption in CockroachDB Cloud, see [Encryption At Rest](/docs/v25.4/encryption).
...
To learn more about rotating a CMEK key in CockroachDB Cloud, read [Rotate a CMEK key](/docs/cockroachcloud/managing-cmek#rotate-a-cmek-key).
...
CMEK can be enabled only on a private cluster on CockroachDB Advanced with advanced security features enabled. Advanced security features can be enabled only when the cluster is created. Refer to [Private Clusters](/docs/cockroachcloud/private-clusters) and [Enable advanced security features](/docs/cockroachcloud/create-an-advanced-cluster#step-6-configure-advanced-security-features).

</Accordion>

<Accordion title="Can we enable CMEK for an existing cluster that wasn't created as a private cluster?">

An existing cluster cannot be migrated to a private cluster. Instead, create a new CockroachDB Advanced cluster with advanced security features enabled, manually back up the cluster data to a secure location, then manually import the data to the new cluster. Refer to [Enable advanced security features](/docs/cockroachcloud/create-an-advanced-cluster#step-6-configure-advanced-security-features).
...
Yes, when you add a new region to a CMEK-enabled cluster, you must enable CMEK for that region. Refer to [Add a Region to a CMEK-enabled Cluster](/docs/cockroachcloud/managing-cmek#add-a-region-to-a-cmek-enabled-cluster).
...
Yes, the [managed backups](/docs/cockroachcloud/managed-backups) stored in CockroachDB Cloud infrastructure are also encrypted using the CMEK, using CoackroachDB’s [encrypted backup](/docs/v25.4/take-and-restore-encrypted-backups) capability. Internally, a backup data key is wrapped by the CMEK, and then the backup data key is used for encrypting the backup.
...
- [Managing Customer-Managed Encryption Keys (CMEK) for CockroachDB Advanced](/docs/cockroachcloud/managing-cmek)
- [Create Private Clusters](/docs/cockroachcloud/private-clusters)
- [Encryption At Rest](/docs/v25.4/encryption)
