...
- Managing [backups](/docs/v25.4/backup-and-restore-overview) as part of a disaster recovery plan
- Using [Change data capture (CDC) changefeeds](/docs/v25.4/change-data-capture-overview)
- [Exporting data](/docs/v25.4/export)
- [Exporting logs](/docs/cockroachcloud/export-logs)

By default, clusters can access external resources via the internet without restriction, and even [private clusters](/docs/cockroachcloud/private-clusters) can access their private network. This potentially leaves a cluster open to a *data exfiltration* scenario, wherein an attacker, often a [malicious insider](https://www.cisa.gov/defining-insider-threats), steals data by sending backups, changefeeds, data, or logs to a source that they control.

Operators of CockroachDB Advanced clusters can mitigate against this risk by using Egress Perimeter Controls, which enable Cluster Admins to restrict egress to a list of specified external destinations. This adds a strong layer of protection against malicious or accidental data exfiltration. Along with other measures such as [Private Clusters](/docs/cockroachcloud/private-clusters), Egress Perimeter Controls are an important component in an overall strategy for maximizing network security.

Further reading: [review how CockroachDB products differs in advanced security features](/docs/v25.4/security-reference/security-overview).
...
- Egress Perimeter Controls are supported on AWS, GCP, and Azure for the following deployment types:
    - CockroachDB Advanced clusters with [advanced security features](/docs/cockroachcloud/create-an-advanced-cluster#step-6-configure-advanced-security-features).
    - CockroachDB Advanced [Private Clusters](/docs/cockroachcloud/private-clusters).
- You need a service account with the [Cluster Admin](/docs/cockroachcloud/authorization#cluster-admin) role on clusters in your organization. You can provision service accounts and API keys in CockroachDB Cloud Console. Refer to [Service Accounts](/docs/cockroachcloud/managing-access#manage-service-accounts).
