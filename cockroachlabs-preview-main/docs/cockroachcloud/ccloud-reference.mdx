...
This page describes how to create and manage CockroachDB Cloud clusters. If you have not installed the `ccloud` tool or logged into CockroachDB Cloud, refer to [Get Started with the `ccloud` CLI](/docs/cockroachcloud/ccloud-get-started).
...
You can set the cluster name, cloud infrastructure provider, region, and [resource limits](/docs/v25.4/architecture/glossary#resource-limits) as command options. The following command is equivalent to the previous command that uses the default values.
...
Use the `ccloud cluster sql` command to start a CockroachDB SQL shell connection to the specified cluster using the [cluster ID](#list-clusters-in-an-organization). If you haven't created a [SQL user](/docs/v26.1/security-reference/authorization#sql-users) for the specified cluster, you will be prompted to create a new user and set the user password.
...
Use the `--sso` flag to connect to your cluster using [single sign-on (SSO) authentication](/docs/cockroachcloud/cloud-sso-sql), which will allow you to start a SQL shell without using a password.
...
By default, the `ccloud cluster sql` command will allow connections only from IP addresses in your cluster's [allowlist](/docs/cockroachcloud/network-authorization#ip-allowlisting). Use the `--skip-ip-check` flag to disable the client-side IP allowlist check:
...
Use the `ccloud cluster user create` command to create a new [SQL user](/docs/v26.1/security-reference/authorization#sql-users) by passing in the cluster name and the username. By default, newly created users are assigned to the `admin` role. An `admin` SQL user has full privileges for all databases and tables in your cluster. This user can also create additional users and grant them appropriate privileges.
...
To get the [connection URL](/docs/v25.4/connection-parameters#connect-using-a-url), use the `--connection-url` option.
...
The `ccloud cluster backup` commands lists backups and manages [backup configuration](/docs/cockroachcloud/backup-and-restore-overview#cockroachdb-cloud-backups) for a cluster.
...
Use the `ccloud cluster cmek get` command to view the [Customer-Managed Encryption Keys (CMEK)](/docs/cockroachcloud/cmek) configuration for a CockroachDB Advanced cluster.
...
The `ccloud cluster networking allowlist` commands create or manage an [IP allowlist](/docs/cockroachcloud/network-authorization#ip-allowlisting), which allows incoming network connections from the specified network IP range. Use the `--sql` flag to allow incoming CockroachDB SQL shell connections from the specified network. Use the `--ui` flag to allow access to the [DB Console](/docs/v26.1/ui-overview) from the specified network.
...
The `ccloud cluster networking client-ca-cert` commands [manage client CA certificates](/docs/cockroachcloud/client-certs-advanced) for a CockroachDB Advanced cluster. Client CA certificates allow clients to authenticate using TLS certificates signed by your own certificate authority.
...
The `ccloud cluster networking private-endpoint` commands manage [private endpoint connectivity](/docs/cockroachcloud/connect-to-an-advanced-cluster#establish-private-connectivity) for a CockroachDB Advanced cluster. Private endpoints provide private connectivity using AWS PrivateLink, GCP Private Service Connect, or Azure Private Link.
...
The `ccloud cluster networking egress-private-endpoint` commands manage [egress private endpoint connections](/docs/cockroachcloud/egress-private-endpoints) from a CockroachDB Advanced cluster. Egress private endpoints allow your cluster to connect to external services using private network connectivity.
...
The `ccloud replication` commands manage [physical cluster replication (PCR)](/docs/v25.4/physical-cluster-replication-overview) between CockroachDB Cloud clusters.
