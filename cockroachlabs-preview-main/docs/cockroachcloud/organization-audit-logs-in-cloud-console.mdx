...
1.  Navigate to the [CockroachDB Cloud Console](https://cockroachlabs.cloud/) and log in as a user with the [Organization Admin role](/docs/cockroachcloud/authorization#organization-admin).
...
-   **User email**: Select one or more email addresses from the list of [organization members](/docs/cockroachcloud/managing-access#manage-an-organizations-users) (optional).
...
-   **User**: Displays the following:
    ...
    -   [Service account name](/docs/cockroachcloud/managing-access#manage-service-accounts) if **Source** is `API`. _(Note: You cannot filter by service account name.)_
...
-   **Source**: Displays the following:
    ...
    -   `API` for actions executed via the [Cloud API](/docs/cockroachcloud/cloud-api).
...
-   [Export CockroachDB Cloud Organization Audit Logs](/docs/cockroachcloud/cloud-org-audit-logs)