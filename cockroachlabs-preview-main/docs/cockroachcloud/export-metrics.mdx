...
CockroachDB Standard users can use the [Cloud API](/docs/cockroachcloud/cloud-api) to configure metrics export to [Amazon CloudWatch](https://aws.amazon.com/cloudwatch), [Datadog](https://www.datadoghq.com/), or [Prometheus](https://prometheus.io/). Once the export is configured, metrics will flow from all [regions](/docs/cockroachcloud/regions) of your CockroachDB Standard cluster to your chosen cloud metrics sink.

<Tip>
CockroachDB Standard clusters use [Cloud Console](/docs/cockroachcloud/cluster-management#view-clusters-page) instead of [DB Console](/docs/v25.4/ui-overview), and DB Console is disabled. To export metrics from a CockroachDB self-hosted cluster, refer to [Monitoring and Alerting](/docs/v25.4/monitoring-and-alerting) instead of this page.
</Tip>
...
Exporting metrics to Amazon CloudWatch is only available on CockroachDB Standard clusters which are hosted on AWS. If your CockroachDB Standard cluster is hosted on GCP, you can [export metrics to Datadog](/docs/cockroachcloud/export-metrics?filters=datadog-metrics-export) or [Prometheus](/docs/cockroachcloud/export-metrics?filters=prometheus-metrics-export) instead.

<Note>
Enabling metrics export will send around 90 metrics per [region](/docs/cockroachcloud/regions) to Amazon CloudWatch. Review the [Amazon CloudWatch documentation](https://aws.amazon.com/cloudwatch/pricing) to gauge how this adds to your Amazon CloudWatch spend.
</Note>
...
3. Determine your cluster's cloud provider account ID. This command uses the third-party JSON parsing tool [`jq`](https://stedolan.github.io/jq/download) to isolate just the needed `aws_account_id` field:
    ...
    - `{secret_key}` is your CockroachDB Standard API key. See [API Access](/docs/cockroachcloud/managing-access) for more details.
...
8. Issue the following [Cloud API](/docs/cockroachcloud/cloud-api) command to enable metrics export for your CockroachDB Standard cluster:
    ...
    - `{secret_key}` is your CockroachDB Standard API key. See [API Access](/docs/cockroachcloud/managing-access) for instructions on generating this key.
...
9. Depending on the size of your cluster and how many regions it spans, the configuration may take a moment. You can monitor the ongoing status of the configuration using the following Cloud API command:
    ...
    Run the command periodically until the command returns a status of `ENABLED`, at which point the configuration is complete, and metrics will begin appearing in CloudWatch under the log group you created in step 1. Since the configuration is applied to the cluster [regions](/docs/cockroachcloud/regions) in a rolling fashion, you may see some metrics appear even before the `GET` command returns an `ENABLED` status.
...
4. Depending on the size of your cluster and how many regions it spans, the configuration may take a moment. You can monitor the ongoing status of the configuration using the following Cloud API command:
    ...
    Run the command periodically until the command returns a status of `ENABLED`, at which point the configuration is complete, and metrics will begin appearing in the Datadog interface. Since the configuration is applied to cluster [regions](/docs/cockroachcloud/regions) in a rolling fashion, you may see some metrics appear even before the `GET` command returns an `ENABLED` status.
...
2. Issue the following [Cloud API](/docs/cockroachcloud/cloud-api) command to enable metrics export for your CockroachDB Standard cluster:
...
3. Depending on the size of your cluster and how many regions it spans, the configuration may take a moment. You can monitor the ongoing status of the configuration using the following Cloud API command:
    ...
    Run the command periodically until the command returns a `status` of `ENABLED`, at which point the configuration across all [regions](/docs/cockroachcloud/regions) is complete. The response also includes `targets`, a map of scrape endpoints exposing metrics to regions. For example:
...
4. You can test a scrape endpoint by using the following Cloud API command:
    ...
    - `{cluster_id}` is your CockroachDB Standard cluster ID as determined in step 1, resembling `f78b7feb-b6cf-4396-9d7f-494982d7d81e`.
    - `{cluster_region}` is a region of your CockroachDB Standard cluster as shown in the `targets` of step 3, such as `us-east4`. You can also find your cluster’s region(s) on the [Cluster Overview page](/docs/cockroachcloud/cluster-overview-page).
    - `{secret_key}` is your CockroachDB Standard API key. See [API Access](/docs/cockroachcloud/managing-access) for instructions on generating this key.
