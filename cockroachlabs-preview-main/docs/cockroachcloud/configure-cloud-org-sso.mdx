...
This page describes how to enable [Cloud Organization SSO](/docs/cockroachcloud/cloud-org-sso) and manage your SSO configuration.
...
<Note>
After it is enabled, Cloud Organization SSO **cannot** be disabled. To configure Cloud Organization SSO to behave like [Basic SSO](/docs/cockroachcloud/cloud-org-sso#basic-sso), refer to [Emulate Basic SSO](#emulate-basic-sso).
</Note>
...
- Some members may need to be re-added to your organization:
    - All members of your CockroachDB Cloud organization who were using [Basic SSO](/docs/cockroachcloud/cloud-org-sso#basic-sso) rather than an email and password must sign in again to regain access to your organization. After signing in, members retain the same access they had before the migration.
    - Members who are also members of other organizations must be re-added to your organization. If they sign in using an authentication method with [autoprovisioning](#autoprovisioning) enabled, they are automatically added upon successful sign-in. Otherwise, they must be re-invited or [provisioned using SCIM](/docs/cockroachcloud/configure-scim-provisioning). If a re-invited member previously had the [Organization Admin](/docs/cockroachcloud/authorization#organization-admin) role, it must be assigned to them again.
...
If all of your administrators belong to multiple organizations, you can create a temporary user in your SSO provider or directly in CockroachDB Cloud. Assign the [**Organization Admin** role](/docs/cockroachcloud/authorization#organization-admin) to the temporary user, and use this temporary admin to enable Cloud Organization SSO. After migration, you should delete this temporary user or revoke the **Organization Admin** role.

## Enable Cloud Organization SSO

To enable Cloud Organization SSO:

1. Log in to [CockroachDB Cloud Console](https://cockroachlabs.cloud/) as an user with the [Organization Admin](/docs/cockroachcloud/authorization#organization-admin) role.
...
1. Log in to [CockroachDB Cloud Console](https://cockroachlabs.cloud/) as user with the [Organization Admin](/docs/cockroachcloud/authorization#organization-admin) role.
2. Go to **Organization** &gt; **Authentication**.
...
1. Log in to [CockroachDB Cloud Console](https://cockroachlabs.cloud/) as a user with the [Organization Admin](/docs/cockroachcloud/authorization#organization-admin) role.
2. Go to **Organization** &gt; **Authentication**.
3. To configure an authentication method, click its name.
4. At the top of the page, click **Edit**.
...
Autoprovisioning allows members to sign up for an account without waiting for an invitation. By default, autoprovisioning is disabled, and a member must exist in the SSO provider and must be [invited by a user with the **Organization Admin** role](/docs/cockroachcloud/managing-access#invite-team-members-to-an-organization) before they can create an account. When autoprovisioning is enabled, no invitation is required.
...
If a member's identity is removed from the SSO provider, they can no longer log in to CockroachDB Cloud, but their account is not automatically deprovisioned. If you require automatic deprovisioning or other centralized account automation features, refer to [SCIM Provisioning](/docs/cockroachcloud/configure-scim-provisioning).
...
12. In a separate browser, log in to [CockroachDB Cloud Console](https://cockroachlabs.cloud/) as a user with the [Organization Admin](/docs/cockroachcloud/authorization#organization-admin) role.
...
20. Click **Save**.
21. The authentication method has been added but is disabled. To enable it, toggle **Enable**.
22. (Optional) [Configure SCIM autoprovisioning](/docs/cockroachcloud/configure-scim-provisioning).
...
- [Cloud Organization SSO Frequently Asked Questions](/docs/cockroachcloud/cloud-org-sso#frequently-asked-questions-faq).
- [Configure SCIM Provisioning](/docs/cockroachcloud/configure-scim-provisioning)
