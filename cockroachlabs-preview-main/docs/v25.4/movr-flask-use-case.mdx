...
This page guides you through an example use-case for global application development and deployment. It is the first section of the [Develop and Deploy a Global Application](/docs/v25.4/movr#develop-and-deploy-a-global-application) tutorial.
...
<Note>
In the [example deployment](/docs/v25.4/movr-flask-deployment), the application and the database deployments are deployed on separate machines.
</Note>
...
Limiting latency improves the user experience, and it can also help you avoid problems with data integrity, like [transaction contention](/docs/v25.4/performance-best-practices-overview#transaction-contention).
...
- Deploy the application so that each instance of the application communicates with the closest database instance. This can be configured during [application deployment](/docs/v25.4/movr-flask-deployment).
- Design the application's persistence layer to limit the number of queries on data stored outside of the region closest to the application. You can use CockroachDB's [multi-region features](/docs/v25.4/multiregion-overview) to control where data is stored and from where it is accessed.
...
You will set up an external load balancer in [Deploy a Global Serverless Application](/docs/v25.4/movr-flask-deployment).

## Next steps

Next, learn more about [creating a multi-region database schema](/docs/v25.4/movr-flask-database).
