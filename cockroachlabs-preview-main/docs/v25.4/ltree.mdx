...
The `LTREE` [data type](/docs/v25.4/data-types) stores hierarchical tree-like structures as a *label path*, which is a sequence of dot-separated *labels*. Labels represent positions in a tree hierarchy. `LTREE` is useful for efficiently querying and managing hierarchical data without using recursive joins.
...
CockroachDB supports indexing `LTREE` columns. [Indexes](/docs/v25.4/indexes) on `LTREE` columns accelerate the following operations:
...
The following [built-in functions](/docs/v25.4/functions-and-operators#ltree-functions) are supported for `LTREE`:
...
For more details, refer to [Functions and Operators](/docs/v25.4/functions-and-operators).

## Supported casting and conversion

You can [cast](/docs/v25.4/data-types#data-type-conversions-and-casts) `LTREE` values to the following data type:
