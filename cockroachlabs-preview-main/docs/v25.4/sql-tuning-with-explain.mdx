...
old_description: "How to use EXPLAIN to identify and resolve SQL statement performance issues"
---

This tutorial presents the common causes for [slow SQL queries](/docs/v25.4/query-behavior-troubleshooting#identify-slow-queries) and describes how to use [`EXPLAIN`](/docs/v25.4/explain) to troubleshoot the issues in queries against the [`movr` example dataset](/docs/v25.4/cockroach-demo#datasets).

## Before you begin

Start the [MovR database](/docs/v25.4/movr) on a CockroachDB demo cluster with a larger data set.

```shell
cockroach demo movr --num-users 1250000
```

This command opens an interactive SQL shell to a temporary, in-memory cluster with the `movr` database preloaded and set as the [current database](/docs/v25.4/sql-name-resolution#current-database).
...
To understand why this query performs poorly, use [`EXPLAIN`](/docs/v25.4/explain):
...
To understand why the performance improved, use [`EXPLAIN`](/docs/v25.4/explain) to see the new query plan:
...
Because the `users` table is under [the maximum range size](/docs/v25.4/configure-replication-zones#range-max-bytes), the primary index and all secondary indexes are contained in a single range with a single leaseholder. If the table were bigger, however, the primary index and secondary index could reside in separate ranges, each with its own leaseholder. In this case, if the leaseholders were on different nodes, the query would require more network hops, further increasing latency.

### Solution: Filter by a secondary index storing additional columns

When you have a query that filters by a specific column but retrieves a subset of the table's total columns, you can improve performance by [storing](/docs/v25.4/indexes#storing-columns) those additional columns in the secondary index to prevent the query from needing to scan the primary index as well.
...
Secondary indexes are crucial when [joining](/docs/v25.4/joins) data from different tables as well.
...
To understand what's happening, use [`EXPLAIN`](/docs/v25.4/explain) to see the query plan:
...
To understand why performance improved, again use [`EXPLAIN`](/docs/v25.4/explain) to see the new query plan:
...
[Hash joins](/docs/v25.4/joins#hash-joins) are more expensive and require more memory than [lookup joins](/docs/v25.4/joins#lookup-joins). Hence the [cost-based optimizer](/docs/v25.4/cost-based-optimizer) uses a lookup join whenever possible.
