...
CockroachDB allows you to rotate [public key infrastructure (PKI)security certificates](/docs/v25.4/security-reference/transport-layer-security) without restarting nodes.

<Tip>
For an introduction to how security certificates work in a secure CockroachDB cluster, see [Create Security Certificates](/docs/v25.4/cockroach-cert).
</Tip>
...
For more background, see [Why CockroachDB creates a combined CA certificate](/docs/v25.4/rotate-certificates#why-cockroachdb-creates-a-combined-ca-certificate) and [Why rotate CA certificate in advance](/docs/v25.4/rotate-certificates#why-rotate-ca-certificates-in-advance).
...
2. Create a new CA certificate and key, using the `--overwrite` flag to overwrite the old CA certificate:
    ...
    This results in the [combined CA certificate](/docs/v25.4/rotate-certificates#why-cockroachdb-creates-a-combined-ca-certificate), `ca.crt`, which contains the new certificate followed by the old certificate.
