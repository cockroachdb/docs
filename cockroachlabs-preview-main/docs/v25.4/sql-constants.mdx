...
Bit array literals are acceptable both when values of types [`BIT`](/docs/v25.4/bit) or [`VARBIT`](/docs/v25.4/bit) (`BIT VARYING`) are expected.
...
Additionally, for compatibility with PostgreSQL, the notation `'string'::type` and `CAST('string' AS type)` is also recognized as an interpreted literal. These are special cases of [cast expressions](/docs/v25.4/scalar-expressions).

For more information about the allowable format of interpreted literals, refer to the "Syntax" section of the respective data types: [`DATE`](/docs/v25.4/date#syntax), [`INET`](/docs/v25.4/inet#syntax), [`INTERVAL`](/docs/v25.4/interval#syntax), [`TIME`](/docs/v25.4/time#syntax), [`TIMESTAMP`/`TIMESTAMPTZ`](/docs/v25.4/timestamp#syntax).
