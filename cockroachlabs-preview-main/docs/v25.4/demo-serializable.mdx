...
- In PostgreSQL, with the default [`READ COMMITTED`](/docs/v25.4/read-committed) isolation level, the [write skew](#write-skew) anomaly results in both doctors successfully booking leave and the hospital having no doctors on call for that particular shift.
...
<Frame>![Schema for serializable transaction tutorial](/images/v25.4/serializable_schema.png)</Frame>
...
Again, this anomaly is the result of PostgreSQL's default isolation level of [`READ COMMITTED`](/docs/v25.4/read-committed), but note that this would happen with any isolation level except `SERIALIZABLE` and some implementations of `REPEATABLE READ`:
...
1. If you haven't already, [install CockroachDB](/docs/v25.4/install-cockroachdb) locally.
2. Use the [`cockroach start-single-node`](/docs/v25.4/cockroach-start-single-node) command to start a one-node CockroachDB cluster in insecure mode:
...
3. In a new terminal window, open the [built-in SQL client](/docs/v25.4/cockroach-sql) and connect to `localhost`:
...
9. In the terminal for doctor 2, the application tries to commit the transaction:
   ...
   <Tip>
   For this kind of error, CockroachDB recommends a [client-side transaction retry loop](/docs/v25.4/transaction-retry-error-reference#client-side-retry-handling) that would transparently observe that the one doctor cannot take time off because the other doctor already succeeded in asking for it. You can find generic transaction retry functions for various languages in our [Build an App](/docs/v25.4/example-apps) tutorials.

   For more information about the error message for the `RETRY_SERIALIZABLE` error type, see the [Transaction Retry Error Reference](/docs/v25.4/transaction-retry-error-reference#retry_serializable).
...
- [Transactions Overview](/docs/v25.4/transactions)
