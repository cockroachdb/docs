...
In CockroachDB, the changefeed *envelope* is the structure of each [message](/docs/v25.4/changefeed-messages) emitted to the [sink](/docs/v25.4/changefeed-sinks). Changefeeds package *events*, triggered by an update to a row in a [watched table](/docs/v25.4/change-data-capture-overview#watched-table), into messages according to the configured envelope. By default, changefeed messages use the [`wrapped` envelope](#wrapped), which includes the primary key of the changed row and a top-level field indicating the value of the row after the change event.​
...
<Note>
You can also specify the *format* of changefeed messages, such as Avro. For more details, refer to [Message formats](/docs/v25.4/changefeed-messages#message-formats).
</Note>
...
<Note>
The values that the `envelope` option accepts are compatible with different [changefeed sinks](/docs/v25.4/changefeed-sinks), and the structure of the message will vary depending on the sink and [message format](/docs/v25.4/changefeed-messages#message-formats).
</Note>

### Audit changes in data

You can include both the previous and updated states of a row in the message envelope to support use cases like auditing or applying change-based logic in downstream systems. Use the [`diff`](#diff-option) option with [`CREATE CHANGEFEED`](/docs/v25.4/create-changefeed) to include the previous state of the row:
...
- `"c"` for [inserts](/docs/v25.4/insert).
- `"u"` for [updates](/docs/v25.4/update).
- `"d"` for [deletes](/docs/v25.4/delete).
...
Use the [`envelope=enriched, enriched_properties=schema`](#enriched-properties-option) options with [`CREATE CHANGEFEED`](/docs/v25.4/create-changefeed) to include the `"schema"` top-level field and the [schema fields and types](#schema):
...
Use the [`envelope=enriched, enriched_properties=source`](#enriched-properties-option) options with [`CREATE CHANGEFEED`](/docs/v25.4/create-changefeed) to include the [`"source"`](#source) top-level field that contains metadata for the origin cluster and the changefeed job:
...
A *full-fidelity* changefeed message envelope ensures complete information about every change event in the [watched tables](/docs/v25.4/change-data-capture-overview#watched-table)—including the before and after state of a row, timestamps, and rich metadata like source and schema information. This type of configured envelope allows downstream consumers to replay, audit, debug, or analyze changes with no loss of context.
...
Use the [`mvcc_timestamp`](#mvcc-timestamp-option), [`envelope=enriched, enriched_properties='source,schema'`](#enriched-properties-option), [`diff`](#diff-option), [`updated`](#updated-option), and [`key_in_value`](#key-in-value-option) (for Kafka) options with [`CREATE CHANGEFEED`](/docs/v25.4/create-changefeed) to create a full-fidelity envelope:
...
[`enriched`](#enriched-option) introduces additional metadata to the envelope, which is further configurable with the [`enriched_properties`](#enriched-properties-option) option. This envelope option is supported for [Kafka sinks](/docs/v25.4/changefeed-sinks#kafka), [webhook](/docs/v25.4/changefeed-sinks#webhook-sink), [Google Cloud Pub/Sub](/docs/v25.4/changefeed-sinks#google-cloud-pub-sub), and sinkless changefeeds.
...
`bare` removes the `after` key from the changefeed message and stores any metadata in a `crdb` field. When used with [`avro`](/docs/v25.4/changefeed-messages#avro) format, `record` will replace the `after` key.
...
[CDC queries](/docs/v25.4/cdc-queries) use [`envelope=bare`](/docs/v25.4/create-changefeed#envelope) message format by default. The `bare` message envelope places the output of the `SELECT` clause at the top level of the message instead of under an `"after"` key. When there is additional information that the changefeed is sending, such as [`updated`](/docs/v25.4/create-changefeed#updated) or [`resolved`](/docs/v25.4/create-changefeed#resolved) timestamps, the messages will include a `crdb` field containing this information.
...
A changefeed containing a `SELECT` clause with the [`topic_in_value`](/docs/v25.4/create-changefeed#topic-in-value) option specified:
...
`key_only` emits only the key and no value, which is faster if you only need to know the key of the changed row. This envelope option is only supported for [Kafka sinks](/docs/v25.4/changefeed-sinks#kafka) or sinkless changefeeds.
...
`row` emits the row without any additional metadata fields in the message. This envelope option is only supported for [Kafka sinks](/docs/v25.4/changefeed-sinks#kafka) or sinkless changefeeds. `row` does not support [`avro`](/docs/v25.4/changefeed-messages#avro) format—if you are using `avro`, refer to the [`bare`](#bare) envelope option.
...
CockroachDB provides multiple changefeed envelopes, each supported by different [sinks](/docs/v25.4/changefeed-sinks) and [use cases](#use-cases). For the [changefeed options](#option-reference) to enable each field, refer to the following descriptions:

### `"payload"`

The top-level `"payload"` field is present in envelopes for changefeeds emitting to a [webhook](/docs/v25.4/changefeed-sinks#webhook-sink) sink and contains all of the change event data. The messages emit as a batch with a `"payload"` wrapper around the change event data and a `"length"` field for the number of messages in the batch:
...
If you're using [CDC queries](/docs/v25.4/cdc-queries) to [filter only for the type of change operation](/docs/v25.4/cdc-queries#cdc-query-function-support), we recommend using the `envelope=enriched` option instead for this metadata.
...
- `"primary_keys"`: An array of [primary key](/docs/v25.4/primary-key) column name(s) for the changed row.
- `"schema_name"`: The schema name of the changed table.
- `"source_node_locality"`: The [locality](/docs/v25.4/cockroach-start#standard-output) of the node that emitted the changefeed messages, e.g., `"cloud=gce,region=us-east1,zone=us-east1-b"`.
...
- In sinkless changefeeds (those created with [`EXPERIMENTAL CHANGEFEED FOR`](/docs/v25.4/changefeed-for) or `CREATE CHANGEFEED... WITH sinkless` ), the output includes a `"table"` field for each row change, because a single sinkless feed could cover multiple tables.
- In the [`enriched`](#enriched) envelope, the [fully qualified table name](/docs/v25.4/sql-name-resolution) is typically part of the [`"source"`](#source) field (as `"database_name"`, `"schema_name"`, `"table_name"` sub-fields), rather than a separate top-level `"table"` field.

<Note>
When a changefeed targets a table with multiple column families, the family name is appended to the table name as part of the topic. Refer to [Tables with columns families in changefeeds](/docs/v25.4/changefeeds-on-tables-with-column-families#message-format) for guidance.
</Note>
...
Here `"__crdb__": {"key": [101]}` holds the primary key for the row, while the rest of the object are the row’s columns at the top level. Other metadata like [`"updated"`](#updated) timestamps or [`resolved`](#resolved-option) timestamps would also appear inside `__crdb__`, if present. This field is specific to the `bare` envelope or other cases (like custom [CDC queries](/docs/v25.4/cdc-queries) with `SELECT`) where metadata needs to be attached without interfering with the selected row data.
...
- For more details on the file naming format for cloud storage sinks, refer to [changefeed files](/docs/v25.4/create-changefeed#files).
