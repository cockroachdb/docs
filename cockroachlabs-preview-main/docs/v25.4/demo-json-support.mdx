...
This page guides you through a simple demonstration of how CockroachDB can store and query unstructured [`JSONB`](/docs/v25.4/jsonb) data from a third-party API, as well as how a [GIN index](/docs/v25.4/inverted-indexes) can optimize your queries.
...
For the purpose of this tutorial, you need only one CockroachDB node running in insecure mode, so use the [`cockroach start-single-node`](/docs/v25.4/cockroach-start-single-node) command in the foreground:
...
In a new terminal window, open the [built-in SQL shell](/docs/v25.4/cockroach-sql) and create a new SQL user, `maxroach`:
...
Then [grant privileges](/docs/v25.4/grant) to the `maxroach` user:
...
Download the [`json-sample.py`](https://raw.githubusercontent.com/cockroachdb/docs/master/_includes/v25.4/json/json-sample.py) file, or create the file yourself and copy the code into it:
...
Download the [`json-sample.go`](https://raw.githubusercontent.com/cockroachdb/docs/master/_includes/v25.4/json/json-sample.go) file, or create the file yourself and copy the code into it:
...
The query in the previous step took 103.748ms. To optimize the performance of queries that filter on the `JSONB` column, let's create a [GIN index](/docs/v25.4/inverted-indexes) on the column:
...
You may also want to learn more about the [`JSONB`](/docs/v25.4/jsonb) data type and [GIN indexes](/docs/v25.4/inverted-indexes).
