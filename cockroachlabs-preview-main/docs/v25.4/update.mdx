...
import { Update6 } from "/snippets/syntax-diagrams/Update6.jsx";

The `UPDATE` [statement](/docs/v25.4/sql-statements) updates rows in a table.

<Warning>
  If you update a row that contains a column referenced by a [foreign key constraint](/docs/v25.4/foreign-key) and has
  an [`ON UPDATE` action](/docs/v25.4/foreign-key#foreign-key-actions), all of the dependent rows will also be updated.
</Warning>

## Required privileges

The user must have the `SELECT` and `UPDATE` [privileges](/docs/v25.4/security-reference/authorization#managing-privileges) on the table.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        <a href="/docs/v25.4/functions-and-operators#aggregate-functions">aggregate function</a> you want to perform, or
        the <a href="/docs/v25.4/scalar-expressions">scalar expression</a> you want to use.
        <br />
        <br />
        To fill columns with their <a href="/docs/v25.4/default-value">default values</a>, use{" "}
    ...
    <tr>
      ...
      <td>
        A <a href="/docs/v25.4/selection-queries">selection query</a>. Each value must match the{" "}
        <a href="/docs/v25.4/data-types">data type</a> of its column on the left side of <code>=</code>.
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
        <code>a&#95;expr</code> must be a <a href="/docs/v25.4/scalar-expressions">scalar expression</a> that returns
    ...
    <tr>
      ...
      <td>
        An <code>ORDER BY</code> clause. See <a href="/docs/v25.4/order-by">Ordering Query Results</a> and{" "}
        <a href="/docs/v25.4/order-by#ordering-rows-in-dml-statements">Ordering of rows in DML statements</a> for more
        details.
      </td>
    </tr>
    <tr>
...
By using the [explicit index annotation](/docs/v25.4/table-expressions#force-index-selection) (also known as "index hinting"), you can override [CockroachDB's index selection](https://www.cockroachlabs.com/blog/index-selection-cockroachdb-2) and use a specific [index](/docs/v25.4/indexes) for updating rows of a named table.
...
To view how the index hint modifies the query plan that CockroachDB follows for updating rows, use an [`EXPLAIN (OPT)`](/docs/v25.4/explain#opt-option) statement. To see all indexes available on a table, use [`SHOW INDEXES`](/docs/v25.4/show-index).
...
For guidance and an example, see [Bulk-update Data](/docs/v25.4/bulk-update-data).
...
<Note>
  If the [`sql_safe_updates`](/docs/v25.4/cockroach-sql#allow-potentially-unsafe-sql-statements) session variable is set
...
You can use a [`SET`](/docs/v25.4/set-vars) statement to set session variables.
...
In this example, the `RETURNING` clause returns the `id` value of the row updated. The language-specific versions assume that you have installed the relevant [client drivers](/docs/v25.4/install-client-drivers).
...
Now suppose you want to update a couple rows in the table, based on their contents. You can use the [`EXPLAIN (OPT)`](/docs/v25.4/explain#opt-option) command to see how the [cost-based optimizer](/docs/v25.4/cost-based-optimizer) decides to perform the `UPDATE` statement:
...
To help you avoid failures arising from misbehaving applications that bloat the size of rows, you can specify the behavior when a row or individual [column family](/docs/v25.4/column-families) larger than a specified size is written to the database. Use the [cluster settings](/docs/v25.4/cluster-settings) `sql.guardrails.max_row_size_log` to discover large rows and `sql.guardrails.max_row_size_err` to reject large rows.

When you write a row that exceeds `sql.guardrails.max_row_size_log`:

- `INSERT`, `UPSERT`, `UPDATE`, `CREATE TABLE AS`, `CREATE INDEX`, `ALTER TABLE`, `ALTER INDEX`, `IMPORT`, or `RESTORE` statements will log a `LargeRow` to the [`SQL_PERF`](/docs/v25.4/logging#sql_perf) channel.
...
- `CREATE TABLE AS`, `CREATE INDEX`, `ALTER TABLE`, `ALTER INDEX`, `IMPORT`, and `RESTORE` statements will log a `LargeRowInternal` event to the [`SQL_INTERNAL_PERF`](/docs/v25.4/logging#sql_internal_perf) channel.
- `SELECT`, `DELETE`, `TRUNCATE`, and `DROP` are not affected.
...
- [Ordering of rows in DML statements](/docs/v25.4/order-by#ordering-rows-in-dml-statements)
