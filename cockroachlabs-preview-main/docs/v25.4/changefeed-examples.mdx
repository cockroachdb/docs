...
For a summary of changefeed features, refer to the [Change Data Capture Overview](/docs/v25.4/change-data-capture-overview) page.
...
Refer to the [Changefeed Sinks](/docs/v25.4/changefeed-sinks) page for more detail on forming sink URIs, available sink query parameters, and specifics on configuration.
...
Before you run the examples, verify that you have the `CHANGEFEED` privilege in order to create and manage changefeed jobs. Refer to [Required privileges](/docs/v25.4/create-changefeed#required-privileges) for more details.
...
1. Use the [`cockroach start-single-node`](/docs/v25.4/cockroach-start-single-node) command to start a single-node cluster:

    ```shell
    cockroach start-single-node --insecure --listen-addr=localhost
    ```
2. Download and extract the [Confluent platform](https://www.confluent.io/download) (which includes Kafka).
3. In a new terminal window, go to the extracted `confluent-<version>` directory and start Confluent:
...
5. As the `root` user, open the [built-in SQL client](/docs/v25.4/cockroach-sql):

    ```shell
    cockroach sql --insecure
    ```
6. Enable the `kv.rangefeed.enabled` [cluster setting](/docs/v25.4/cluster-settings):
...
9. Create a table and add data:
    ...
    ```
10. Create another table and add data:
...
12. In a new terminal, move into the extracted `confluent-<version>` directory and start watching the Kafka topics:
...
13. Back in the SQL client, insert more data:
    ...
    ```
14. Back in the terminal where you're watching the Kafka topics, the following output has appeared:

    ```text
    {"after": {"id": 3, "name": "Ernie"}}
    ```
15. When you are done, exit the SQL shell (`\q`).
16. To stop `cockroach`:
...
1. Use the [`cockroach start-single-node`](/docs/v25.4/cockroach-start-single-node) command to start a single-node cluster:
    ...
    cockroach start-single-node --insecure --listen-addr=localhost
    ```
2. In this example, you'll run CockroachDB's [Movr](/docs/v25.4/movr) application workload to set up some data for your changefeed.
    ...
    ```
3. As the `root` user, open the [built-in SQL client](/docs/v25.4/cockroach-sql):

    ```shell
    cockroach sql --insecure
    ```
4. Enable the `kv.rangefeed.enabled` [cluster setting](/docs/v25.4/cluster-settings):
...
5. To prepare the Confluent Cloud Kafka sink, sign up on the [Confluent Cloud trial page](https://www.confluent.io/confluent-cloud/tryfree).
6. On the **Create cluster** page, select the necessary cluster type. The **Basic** cluster type is sufficient to run this example. Select the configuration and regions. After you have confirmed payment and set a cluster name, **Launch** the cluster.
7. From the **Overview** page, click on **API Keys** in the navigation menu. Click **Create key** and select the scope for this API key. **Global access** is sufficient for this example. **Granular access** is more suitable for production. Copy or download the key and secret.
8. Create a topic in Confluent Cloud. Under **Topics** select **Add topic**. Add a topic name and define the number of partitions and then **Create**. CockroachDB defaults to using the table name as the topic name. If you would like to send messages to an alternate topic, you can specify the [`topic_name`](/docs/v25.4/create-changefeed#topic-name) parameter.
...
9. Construct the URI to connect your changefeed to the Confluent Cloud cluster. You will need:
    ...
    - (Optional) Any further parameters. Refer to [Query parameters](/docs/v25.4/create-changefeed#query-parameters).

        ```text
        'confluent-cloud://pkc-lzvrd.us-west4.gcp.confluent.cloud:9092?api_key={API key}&api_secret={url-encoded API secret}'
        ```
10. Back in the SQL shell, create a changefeed:
...
5. Next, you'll prepare your Pub/Sub sink.
    ...
    ```
6. Create the Pub/Sub [topic](/docs/v25.4/changefeed-sinks#pub-sub-topic-naming) to which your changefeed will emit messages:
    ...
    ```
7. With the topic and subscription set up, you can now download your Service Account credentials. Use the [`gcloud iam service-accounts keys create`](https://cloud.google.com/sdk/gcloud/reference/iam/service-accounts/keys/create) command to specify where to download the JSON credential file (`credentials.json`):
    ...
    Copy the output so that you can add it to your [`CREATE CHANGEFEED`](/docs/v25.4/create-changefeed) statement in the next step. When you create your changefeed, it is necessary that the credentials are base64 encoded before passing it in the URI.
8. Back in the SQL shell, create a changefeed that will emit messages to your Pub/Sub topic. Ensure that you have base64 encoded the entire credentials JSON object for your Service Account and then run:
    ...
    You can include the `region` parameter for your topic, or use the [WITH `unordered`](/docs/v25.4/create-changefeed#unordered) option for multi-region Pub/Sub. See the [Changefeed Sinks](/docs/v25.4/changefeed-sinks#google-cloud-pub-sub) page for more detail.
...
In this example, you'll set up a changefeed for a single-node cluster that is connected to an AWS S3 sink. The changefeed watches two tables. Note that you can set up changefeeds for any of [these cloud storage providers](/docs/v25.4/changefeed-sinks#cloud-storage-sink).
...
1. Use the [`cockroach start-single-node`](/docs/v25.4/cockroach-start-single-node) command to start a single-node cluster:
    ...
    $ cockroach start-single-node --insecure --listen-addr=localhost
    ```
2. As the `root` user, open the [built-in SQL client](/docs/v25.4/cockroach-sql):

    ```shell
    cockroach sql --insecure
    ```
3. Enable the `kv.rangefeed.enabled` [cluster setting](/docs/v25.4/cluster-settings):
...
6. Create a table and add data:
    ...
    > UPDATE office_dogs SET name = 'Petee H' WHERE id = 1;
    ```
7. Create another table and add data:
...
9. Monitor your changefeed on the [DB Console](http://localhost:8080/#/metrics/changefeeds/cluster). For more information, see [Changefeeds Dashboard](/docs/v25.4/ui-cdc-dashboard).
10. When you are done, exit the SQL shell (`\q`).
11. To stop `cockroach`:
...
5. To prepare the Azure Event Hubs sink, in the Portal, [create a resource group](https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/manage-resource-groups-portal#create-resource-groups).
6. Once you have a resource group, select **Event Hubs** in the portal navigation menu and then **+ Create** from the **Event Hubs** page tool bar to create a namespace. Complete the naming details for the namespace and select the resource group you created in the previous step. For **Pricing Tier**, ensure that you select **Standard** or **Premium** (the **Basic** tier does not support Apache Kafka workloads). Complete your configuration options, and then create the namespace.
7. Select **Go to resource** after the deployment is complete. On the namespace overview page, select **+ Event Hub** in the tool bar to create an event hub within this namespace. An Event Hub is equivalent to a Kafka topic. On the creation page, add a name and the configuration for partitions and message retention.

    <Note>
    You can manually create an Event Hub, or Event Hubs will create automatically when you start the changefeed.
    </Note>
8. To send changefeed messages to your Event Hub, you will need a `shared_access_key`. Find **Shared access policies** in the left-hand navigation. Click **+ Add** and provide a name for the policy as well as confirming the permissions. Once created, click on the policy name and then copy the **Primary key** — you will need this to create your changefeed.
9. Build the connection string to your Event Hub. You will need the following:
...
You can include the [`topic_name`](/docs/v25.4/create-changefeed#topic-name) or [`topic_prefix`](/docs/v25.4/create-changefeed#topic-prefix) parameters to create and name an Event Hub for emitted messages.

1. Create an [external connection](/docs/v25.4/create-external-connection) to store your connection string:
    ...
    ```
2. Create the changefeed to your Event Hub:
...
5. In a separate terminal window, set up your HTTP server. Clone the test repository:
    ...
    ```
6. Next make the script executable and then run the server (passing a specific port if preferred, otherwise it will default to `:3000`):
    ...
    ./server.sh <port>
    ```
7. Back in your SQL shell, run the following statement to create a changefeed that emits to your webhook sink:
    ...
    See the [options table](/docs/v25.4/create-changefeed#options) for more information on the options available for creating your changefeed to a webhook sink.
    ...
    For more detail on emitted changefeed messages, refer to the [Changefeed Messages](/docs/v25.4/changefeed-messages) page.
...
5. To prepare a Pulsar sink, refer to the [Apache Pulsar documentation](https://pulsar.apache.org/docs/next/getting-started-standalone) for setup guides to host Pulsar on a local cluster, Docker, or a Kubernetes cluster.
6. In a terminal window where your Pulsar sink is hosted, start the cluster:
...
7. Enter the SQL shell:

    ```shell
    cockroach sql --insecure
    ```
8. Create your changefeed:
    ...
    Changefeeds emitting to a Pulsar sink do not support external connections or a number of changefeed options. For a full list, refer to the [Changefeed Sinks](/docs/v25.4/changefeed-sinks#apache-pulsar) page.
9. In a different terminal window, start a [Pulsar consumer](https://pulsar.apache.org/docs/next/tutorials-produce-consume) to read messages from the changefeed. This example consumes messages from the `rides` topic:
...
5. Insert a row into the table:
    ...
    ```
6. Start the sinkless changefeed:
...
7. In a new terminal, add another row:
    ...
    ```
8. Back in the terminal where the changefeed is streaming, the following output has appeared:
...
In this example, you'll set up a sinkless changefeed for a single-node cluster that emits Avro records. CockroachDB's Avro binary encoding convention uses the [Confluent Schema Registry](https://docs.confluent.io/current/schema-registry/docs/serializer-formatter) to store Avro schemas.
...
1. Use the [`cockroach start-single-node`](/docs/v25.4/cockroach-start-single-node) command to start a single-node cluster:
    ...
    --background
    ```
2. Download and extract the [Confluent platform](https://www.confluent.io/download).
3. Move into the extracted `confluent-<version>` directory and start Confluent:
    ...
    Only `zookeeper`, `kafka`, and `schema-registry` are needed. To troubleshoot Confluent, see [their docs](https://docs.confluent.io/current/installation/installing_cp#zip-and-tar-archives) and the [Quick Start Guide](https://docs.confluent.io/platform/current/quickstart/ce-quickstart#ce-quickstart).
4. As the `root` user, open the [built-in SQL client](/docs/v25.4/cockroach-sql):
...
5. Enable the `kv.rangefeed.enabled` [cluster setting](/docs/v25.4/cluster-settings):
...
9. In a new terminal, add another row:
    ...
    ```
10. Back in the terminal where the changefeed is streaming, the output will appear:
...
For further information on sinkless changefeeds, refer to the [`CREATE CHANGEFEED`](/docs/v25.4/create-changefeed#create-a-sinkless-changefeed) page.
