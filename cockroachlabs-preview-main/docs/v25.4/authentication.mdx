...
- If you want to know how to create CockroachDB security certificates, see [Create Security Certificates](/docs/v25.4/cockroach-cert).

## Connecting to a CockroachDB cluster

Users may connect with CockroachDB self-hosted clusters in 2 main ways:

- SQL clients connections, including the CockroachDB CLI client and the [various supported drivers and ORMs](/docs/v25.4/install-client-drivers), connect directly to CockroachDB clusters using the [CockroachDB SQL interface](/docs/v25.4/sql-feature-support).
- A read-only monitoring service, which provides cluster and database details, and information useful for troubleshooting and performance tuning. Each CockroachDB node also acts as an HTTP server, providing both a [browser UI DB console](/docs/v25.4/ui-overview) and the [cluster API](/docs/v25.4/cluster-api), which provides much of the same information as the DB console, but as a rest API suitable for programmatic access.
...
- The hostname or address (IP address or DNS name) used to reach a node, either directly or through a load balancer, must be listed in the **Common Name** or **Subject Alternative Names** fields of the certificate:
    - The values specified in [`--listen-addr`](/docs/v25.4/cockroach-start#networking) and [`--advertise-addr`](/docs/v25.4/cockroach-start#networking) flags, or the node hostname and fully qualified hostname if not specified
...
Based on your security setup, you can use the [`cockroach cert` commands](/docs/v25.4/cockroach-cert), [`openssl` commands](/docs/v25.4/create-security-certificates-openssl), or a [custom CA](/docs/v25.4/create-security-certificates-custom-ca) to generate all the keys and certificates.
...
- Client authentication using TLS digital certificates, passwords, or [GSSAPI authentication](/docs/v25.4/gssapi_authentication) (for Enterprise users).
...
- **Password authentication without TLS**
    ...
    Enter password:
    ```
- [**Single sign-on authentication to DB Console**](/docs/v25.4/sso-db-console). CockroachDB supports OpenID Connect (OIDC) for authenticating users to the DB Console. <InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v25.4: You can also enable [automatic role synchronization](/docs/v25.4/oidc-authorization) based on group memberships from your identity provider.
- [**JWT authentication for SQL clients**](/docs/v25.4/sso-sql). CockroachDB supports JSON Web Token (JWT) authentication for SQL client connections. <InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v25.4: You can enable [automatic user provisioning](/docs/v25.4/sso-sql#configure-user-provisioning) and [automatic role synchronization](/docs/v25.4/jwt-authorization) based on group claims in JWT tokens.
- [**GSSAPI authentication**](/docs/v25.4/gssapi_authentication).

### Using `cockroach cert` or `openssl` commands

You can use the [`cockroach cert` commands](/docs/v25.4/cockroach-cert) or [`openssl` commands](/docs/v25.4/create-security-certificates-openssl) to create the CA certificate and key, and node and client certificates and keys.
...
See [Use Cloud Storage for Bulk Operations](/docs/v25.4/cloud-storage-authentication).
...
For details about when and how to change security certificates without restarting nodes, see [Rotate Security Certificates](/docs/v25.4/rotate-certificates).
...
To restrict your cluster to only allow connections using specific cipher suites, use the [`--tls-cipher-suites`](/docs/v25.4/cockroach-start#tls-cipher-suites) startup flag with the `cockroach start` command. This setting applies to all incoming SQL, RPC, and HTTP connections.
...
- [Local Deployment](/docs/v25.4/secure-a-cluster)
- [`cockroach` Commands Overview](/docs/v25.4/cockroach-commands)
- [Certificate-based authentication using multiple values from the X.509 Subject field](/docs/v25.4/certificate-based-authentication-using-the-x509-subject-field)
- [`ALTER ROLE... SUBJECT`](/docs/v25.4/alter-role#set-the-subject-role-option-for-certificate-based-authentication)
- [`CREATE ROLE... SUBJECT`](/docs/v25.4/create-role#set-the-subject-role-option-for-certificate-based-authentication)
- [`cockroach cert`](/docs/v25.4/cockroach-cert)
- [`cockroach auth-session`](/docs/v25.4/cockroach-auth-session)
- [GSSAPI Authentication](/docs/v25.4/gssapi_authentication)
- [Single Sign-on (SSO) for DB Console](/docs/v25.4/sso-db-console)
- [Cluster Single Sign-on (SSO) using JWTs](/docs/v25.4/sso-sql)
...
- [SQL Authentication](/docs/v25.4/security-reference/authentication)
- [Cloud Storage Authentication](/docs/v25.4/cloud-storage-authentication)
