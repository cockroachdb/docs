...
This page has instructions for updating existing rows of data in CockroachDB, using the following [SQL statements](/docs/v25.4/sql-statements):
...
- [Insert data](/docs/v25.4/insert-data) that you now want to update.
...
To update existing rows in a table, use an [`UPDATE` statement](/docs/v25.4/update) with a `WHERE` clause that filters on the columns that identify the rows that you want to update.
...
<Tip>
For detailed reference documentation on the `UPDATE` statement, including additional examples, see the [`UPDATE` syntax page](/docs/v25.4/update).
</Tip>
...
- To update a large number of rows (i.e., tens of thousands of rows or more), use a [batch-update loop](/docs/v25.4/bulk-update-data).
- When executing `UPDATE` statements from an application, make sure that you wrap the SQL-executing functions in [a retry loop that handles transaction errors](/docs/v25.4/query-behavior-troubleshooting#transaction-retry-errors) that can occur under [contention](/docs/v25.4/performance-best-practices-overview#transaction-contention).
...
To insert new rows into a table, and update rows that conflict with the primary key value(s) of the new rows, use an [`UPSERT` statement](/docs/v25.4/upsert).
...
<Tip>
For detailed reference documentation on the `UPSERT` statement, including additional examples, see the [`UPSERT` syntax page](/docs/v25.4/upsert).
</Tip>
...
- When executing `UPSERT` statements from an application, make sure that you wrap the SQL-executing functions in [a retry loop that handles transaction errors](/docs/v25.4/query-behavior-troubleshooting#transaction-retry-errors) that can occur under contention.
...
[Hash-sharded indexes](/docs/v25.4/hash-sharded-indexes) can be used for uniqueness checks.
...
- When executing `INSERT` statements from an application, make sure that you wrap the SQL-executing functions in [a retry loop that handles transaction errors](/docs/v25.4/query-behavior-troubleshooting#transaction-retry-errors) that can occur under contention.
- Follow the [performance best practices listed on the `INSERT`](/docs/v25.4/insert#performance-best-practices) page.
...
- [Bulk-update Data](/docs/v25.4/bulk-update-data)
- [`UPSERT`](/docs/v25.4/update)
- [`INSERT... ON CONFLICT`](/docs/v25.4/insert#on-conflict-clause)
...
- [Error Handling and Troubleshooting](/docs/v25.4/query-behavior-troubleshooting)
