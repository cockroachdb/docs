...
The `cockroach userfile upload` [command](/docs/v25.4/cockroach-commands) uploads a file to the [user-scoped file storage](/docs/v25.4/use-userfile-storage) using a SQL connection.
...
The user must have the `CREATE` [privilege](/docs/v25.4/security-reference/authorization#managing-privileges) on the target database. CockroachDB will proactively grant the user `GRANT`, `SELECT`, `INSERT`, `DROP`, `DELETE` on the metadata and file tables.
...
Then, you can use the file to [`IMPORT INTO`](/docs/v25.4/import-into) data.
...
To upload the contents of a directory to userfile storage, specify a source directory and destination. For example, to upload a [backup](/docs/v25.4/backup) directory to userfile storage:
