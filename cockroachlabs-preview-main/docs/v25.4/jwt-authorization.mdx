...
If you manage users through an identity provider (IdP) that supports JSON Web Tokens (JWT), you can configure CockroachDB to automatically assign [roles](/docs/v25.4/security-reference/authorization) to users based on group claims in their JWT tokens, simplifying access control.
...
5. In conjunction with [automatic user provisioning](/docs/v25.4/sso-sql#configure-user-provisioning) if enabled, users are created automatically during their first authentication and simultaneously receive specified role memberships.

## Prerequisites

- Enable [JWT Authentication](/docs/v25.4/sso-sql).
...
Create CockroachDB roles that match your IdP group names and grant appropriate privileges to each role. Remember that role names must comply with CockroachDB's [identifier requirements](/docs/v25.4/create-user#user-names).
...
3. Obtain a JWT token from your identity provider and connect to CockroachDB using the token. Refer to [Authenticate to your cluster](/docs/v25.4/sso-sql#authenticate-to-your-cluster).
...
Enable [`SESSION` logging](/docs/v25.4/logging#sessions) to preserve data that will help troubleshoot JWT authorization issues:
...
- [Security Reference: Authorization](/docs/v25.4/security-reference/authorization)
