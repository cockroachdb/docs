...
A *stored procedure* is a database object consisting of [PL/pgSQL](/docs/v25.4/plpgsql) or [SQL](/docs/v25.4/sql-statements) statements that can be issued with a single [`CALL`](/docs/v25.4/call) statement. This allows complex logic to be executed repeatedly within the database, which can improve performance and mitigate security risks.

Both stored procedures and [user-defined functions](/docs/v25.4/user-defined-functions) are types of *routines*. However, they differ in the following ways:
...
- Each parameter can be a supported [SQL data type](/docs/v25.4/data-types), [user-defined type](/docs/v25.4/create-type), or the PL/pgSQL `REFCURSOR` type, when [declaring PL/pgSQL cursor variables](/docs/v25.4/plpgsql#declare-cursor-variables).
- CockroachDB supports the `IN` (default), `OUT`, and `INOUT` modes for parameters. For an example, see [Create a procedure that uses `OUT` and `INOUT` parameters](/docs/v25.4/create-procedure#create-a-stored-procedure-that-uses-out-and-inout-parameters).
- `LANGUAGE` specifies the language of the function body. CockroachDB supports the languages [`SQL`](/docs/v25.4/sql-statements) and [`PLpgSQL`](/docs/v25.4/plpgsql).
- The procedure body:
    ...
    - Must conform to a [block structure](/docs/v25.4/plpgsql#structure) if written in PL/pgSQL.

For details, see [`CREATE PROCEDURE`](/docs/v25.4/create-procedure).
...
For more examples of stored procedure creation, see [`CREATE PROCEDURE`](/docs/v25.4/create-procedure#examples).
...
It uses the [PL/pgSQL](/docs/v25.4/plpgsql) [`WHILE`](/docs/v25.4/plpgsql#write-loops) syntax to iterate through the rows, [`RAISE`] to return notice and error messages, and `REFCURSOR` to define a cursor that fetches the next rows to be affected by the procedure.
...
[`CREATE PROCEDURE`](/docs/v25.4/create-procedure) defines a stored procedure called `delete_earliest_histories` with an `INT` and a `REFCURSOR` parameter.
...
`LANGUAGE` specifies [PL/pgSQL](/docs/v25.4/plpgsql) as the language for the stored procedure.

```sql
LANGUAGE PLpgSQL
```

`DECLARE` specifies the [PL/pgSQL variable definitions](/docs/v25.4/plpgsql#declare-a-variable) that are used in the procedure body.
...
`BEGIN` and `END` [group the PL/pgSQL statements](/docs/v25.4/plpgsql#structure) in the procedure body.
...
The following [`IF... THEN`](/docs/v25.4/plpgsql#write-conditional-statements) statement [raises an exception](/docs/v25.4/plpgsql#report-messages-and-handle-exceptions) if `vehicle_location_histories` has fewer rows than the number specified with `num_deletions`. If the exception is raised within an open transaction, the transaction will abort.
...
The following [`WHILE`](/docs/v25.4/plpgsql#write-loops) loop deletes rows iteratively from `vehicle_location_histories`, stopping when the number of loops reaches the `num_deletions` value.

The `DELETE... RETURNING... INTO` statement assigns column values from each deleted row into separate variables. For more information about assigning variables, see [Assign a result to a variable](/docs/v25.4/plpgsql#assign-a-result-to-a-variable).

Finally, the [`RAISE NOTICE`](/docs/v25.4/plpgsql#report-messages-and-handle-exceptions) statement reports these values for each deleted row.
...
The `OPEN` statement [opens a cursor](/docs/v25.4/plpgsql#open-and-use-cursors) for all remaining rows in `vehicle_location_histories`, sorted by timestamp. After calling the procedure in an open transaction, the cursor can be used to fetch rows from the table.
...
The following statement renames the [`delete_earliest_histories` example procedure](/docs/v25.4/stored-procedures#create-a-stored-procedure-using-pl-pgsql) to `delete_histories`:
