...
import { Cursors16 } from "/snippets/syntax-diagrams/Cursors16.jsx";
import { Cursors17 } from "/snippets/syntax-diagrams/Cursors17.jsx";
import { Cursors18 } from "/snippets/syntax-diagrams/Cursors18.jsx";
...
<Tip>
This document describes cursor usage within SQL transactions. For information about using cursors in PL/pgSQL functions and procedures, see [PL/pgSQL](/docs/v25.4/plpgsql#declare-cursor-variables).
</Tip>

Cursors differ from [keyset pagination](/docs/v25.4/pagination) and [`LIMIT`/`OFFSET`](/docs/v25.4/limit-offset) in that:

- Each cursor is a stateful SQL object that is referred to by a unique name.
- Each cursor requires holding open its own dedicated (read-only) [transaction](/docs/v25.4/transactions).
...
- [`DECLARE`](/docs/v25.4/sql-grammar#declare_cursor_stmt)
- [`WITH HOLD`](#use-a-holdable-cursor)
- [`FETCH`](/docs/v25.4/sql-grammar#fetch_cursor_stmt)
- [`CLOSE`](/docs/v25.4/sql-grammar#close_cursor_stmt)
...
These examples assume the presence of the [MovR data set](/docs/v25.4/movr).
...
CockroachDB implements SQL [cursor](/docs/v25.4/cursors) support with the following limitations:
...
- [`SELECT... FOR UPDATE`](/docs/v25.4/select-for-update) with a cursor is not supported. [#77103](https://github.com/cockroachdb/cockroach/issues/77103)
- Respect for [`SAVEPOINT`s](/docs/v25.4/savepoint) is not supported. Cursor definitions do not disappear properly if rolled back to a `SAVEPOINT` from before they were created. [#77104](https://github.com/cockroachdb/cockroach/issues/77104)
...
- [Keyset pagination](/docs/v25.4/pagination)
- [`LIMIT`/`OFFSET`](/docs/v25.4/limit-offset)
