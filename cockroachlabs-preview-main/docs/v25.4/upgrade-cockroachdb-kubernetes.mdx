...
This page shows how to upgrade a CockroachDB cluster that is [deployed on a Kubernetes cluster](/docs/v25.4/deploy-cockroachdb-with-kubernetes).

This page is for Kubernetes deployments that are not using the CockroachDB operator. For guidance specific to the CockroachDB operator, read [Upgrade a Cluster in Kubernetes with the CockroachDB operator](/docs/v25.4/upgrade-cockroachdb-operator).
...
1. Verify the overall health of your cluster using the [DB Console](/docs/v25.4/ui-cluster-overview-page):
    - Under **Node Status**, make sure all nodes that should be live are listed as such. If any nodes are unexpectedly listed as `SUSPECT` or `DEAD`, identify why the nodes are offline and either restart them or [decommission](/docs/v25.4/node-shutdown?filters=decommission#remove-nodes) them before beginning your upgrade. If there are `DEAD` and non-decommissioned nodes in your cluster, the upgrade cannot be finalized. If any node is not fully decommissioned, try the following:
        1. First, reissue the [decommission command](/docs/v25.4/node-shutdown?filters=decommission#decommission-the-node). The second command typically succeeds within a few minutes.
        2. If the second decommission command does not succeed, [recommission](/docs/v25.4/node-shutdown?filters=decommission#recommission-nodes) and then decommission it again. Before continuing the upgrade, the node must be marked as `decommissioned`.
    ...
    - In the **Metrics** dashboards, make sure [CPU](/docs/v25.4/common-issues-to-monitor#cpu-usage), [memory](/docs/v25.4/common-issues-to-monitor#database-memory-usage), and [storage](/docs/v25.4/common-issues-to-monitor#storage-capacity) capacity are within acceptable values for each node. Nodes must be able to tolerate some increase in case the new version uses more resources for your workload. If any of these metrics is above healthy limits, consider [adding nodes](/docs/v25.4/cockroach-start) to your cluster before beginning your upgrade.
2. Make sure your cluster is behind a [load balancer](/docs/v25.4/recommended-production-settings#load-balancing), or your clients are configured to talk to multiple nodes. If your application communicates with only a single node, stopping that node to upgrade its CockroachDB binary will cause your application to fail.
3. By default, the [storage engine](/docs/v25.4/architecture/storage-layer) uses a compaction concurrency of 3. If you have sufficient IOPS and CPU headroom, you can consider increasing this setting via the `COCKROACH_COMPACTION_CONCURRENCY` environment variable. This may help to reshape the LSM more quickly in [inverted LSM](/docs/v25.4/architecture/storage-layer#inverted-lsms) scenarios; and it can lead to increased overall performance for some workloads. Cockroach Labs **strongly recommends** testing your workload against non-default values of this setting.
4. CockroachDB is designed with high fault tolerance. However, taking regular backups of your data is an operational best practice for [disaster recovery](/docs/v25.4/disaster-recovery-planning) planning. Refer to [Restoring backups across versions](/docs/v25.4/restoring-backups-across-versions#support-for-restoring-backups-into-a-newer-version).
5. Review the [v25.4 Release Notes](/docs/releases/v25.4), as well as the release notes for any skipped major version. Pay careful attention to the sections for backward-incompatible changes, deprecations, changes to default cluster settings, and features that are not available until the upgrade is finalized.
...
1. Change the container image image in the custom resource:
    ...
    ```text
    image:
      name: cockroachdb/cockroach:v25.4.6
    ```
2. Apply the new settings to the cluster:
...
4. Verify that all pods have been upgraded:
    ...
    You can also check the CockroachDB version of each node in the [DB Console](/docs/v25.4/ui-cluster-overview-page#node-details).
...
1. Change the container image in the custom resource to use the previous major version:
    ...
    ```text
    image:
      name: cockroachdb/cockroach:v25.4
    ```
2. Apply the new settings to the cluster:
...
1. Change the container image in the StatefulSet:
    ...
    $ kubectl patch statefulset cockroachdb \
    --type='json' \
    -p='[{"op": "replace", "path": "/spec/template/spec/containers/0/image", "value":"cockroachdb/cockroach:v25.4.6"}]'
    ```
...
3. After the pod has been restarted with the new image, start the CockroachDB [built-in SQL client](/docs/v25.4/cockroach-sql):
...
8. Check the image of each pod to confirm that all have been upgraded:
    ...
    ```text
    cockroachdb-0   cockroachdb/cockroach:v25.4.6
    cockroachdb-1   cockroachdb/cockroach:v25.4.6
    cockroachdb-2   cockroachdb/cockroach:v25.4.6
    ...
    ```
...
1. Connect to the cluster using the SQL shell:
    ...
    --host=cockroachdb-public
    ```
2. Set the [cluster setting](/docs/v25.4/cluster-settings) `cluster.auto_upgrade.enabled` to `false`.
...
5. After the pod has been restarted with the new image, start the CockroachDB [built-in SQL client](/docs/v25.4/cockroach-sql):
...
10. Check the image of each pod to confirm that all have been upgraded:
    ...
    ```text
    my-release-cockroachdb-0    cockroachdb/cockroach:v25.4.6
    my-release-cockroachdb-1    cockroachdb/cockroach:v25.4.6
    my-release-cockroachdb-2    cockroachdb/cockroach:v25.4.6
    ...
    ```
...
1. Connect to the cluster using the SQL shell:
    ...
    --host=cockroachdb-public
    ```
2. Set the [cluster setting](/docs/v25.4/cluster-settings) `cluster.preserve_downgrade_option` to the cluster's current major version. The Helm chart does not yet support the [cluster setting](/docs/v25.4/cluster-settings) `cluster.auto_upgrade.enabled`.
