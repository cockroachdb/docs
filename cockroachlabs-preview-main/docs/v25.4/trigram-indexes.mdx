...
A _trigram index_ is a type of [inverted index](/docs/v25.4/inverted-indexes) created on a [`STRING`](/docs/v25.4/string) column. Trigram indexes are used to efficiently search for strings in large tables without providing an exact search term.
...
To display the trigrams within a string, use the `show_trgm()` [built-in function](/docs/v25.4/functions-and-operators#trigrams-functions):
...
To create a trigram index, use the [`CREATE INDEX`](/docs/v25.4/create-index) syntax that defines an [inverted index](/docs/v25.4/inverted-indexes), specifying a `STRING` column and the `gin_trgm_ops` or `gist_trgm_ops` opclass. It is necessary to specify an opclass in order to enable [trigram-based comparisons](#comparisons).
...
- **equality**: [`=`](/docs/v25.4/functions-and-operators#operators). Note that standard [`btree` secondary indexes](/docs/v25.4/create-index#parameters) may perform better than trigram indexes for equality searches.
- **pattern matching (case-sensitive)**: [`LIKE`](/docs/v25.4/functions-and-operators#operators)
- **pattern matching (case-insensitive)**: [`ILIKE`](/docs/v25.4/functions-and-operators#operators)
- **similarity matching**: [`%`](/docs/v25.4/functions-and-operators#operators). This operator returns `true` if the strings in the comparison have a similarity that meets or exceeds the threshold set by the `pg_trgm.similarity_threshold` [session variable](/docs/v25.4/show-vars#pg_trgm_similarity_threshold).
...
3. See how trigram matching performs without a trigram index. Retrieve the columns with values similar to `word`, using the `%` operator. Sort the results by the output of the `similarity()` [built-in function](/docs/v25.4/functions-and-operators#trigrams-functions):
   ...
   Values are not included in the results if their similarities do not meet the threshold set by [`pg_trgm.similarity_threshold`](/docs/v25.4/show-vars#pg_trgm_similarity_threshold), which defaults to `0.3`. For example:
   ...
   Notice that the fuzzy search took 30 milliseconds to execute. Without a trigram index, the statement performs a full scan, which you can verify using [`EXPLAIN`](/docs/v25.4/explain):
...
- `<<%` and `%>>` comparisons and acceleration.
- `<->`, `<<->`, `<->>`, `<<<->`, and `<->>>` comparisons.
- Acceleration on [regex string matching](/docs/v25.4/scalar-expressions#string-matching-using-posix-regular-expressions).
- `%` comparisons, `show_trgm`, and trigram index creation on [collated strings](/docs/v25.4/collate).
