...
[CockroachDB uses the Pebble storage engine](/docs/v25.4/cockroach-start#storage-engine). Pebble is inspired by RocksDB, but differs in that it:
...
CockroachDB uses [Pebble](/docs/v25.4/cockroach-start#storage-engine)––an embedded key-value store inspired by RocksDB, and developed by Cockroach Labs––to read and write data to disk.
...
<Frame>![Structure of an SST file](/images/v25.4/sst.png)</Frame>

SST files are immutable; they are never modified, even during the [compaction process](#compaction).

##### LSM levels

The levels of the LSM are organized from L0 to L6. L0 is the top-most level. L6 is the bottom-most level. New data is added into L0 (e.g., using [`INSERT`](/docs/v25.4/insert) or [`IMPORT INTO`](/docs/v25.4/import-into)) and then merged down into lower levels over time.
...
- To allow LSM-based storage engines like Pebble to support ingesting large amounts of data, such as when using the [`IMPORT INTO`](/docs/v25.4/import-into) statement.
- To allow for easier and more efficient flushes of [memtables](#memtable-and-write-ahead-log).

<Frame>![LSM tree with SST files](/images/v25.4/lsm-with-ssts.png)</Frame>
...
Read amplification can be especially bad if a large [`IMPORT INTO`](/docs/v25.4/import-into) is overloading the cluster (due to insufficient CPU and/or IOPS) and the storage engine has to consult many small SSTs in L0 to determine the most up-to-date value of the keys being read (e.g., using a [`SELECT`](/docs/v25.4/select-clause)).

A certain amount of read amplification is expected in a normally functioning CockroachDB cluster. For example, a read amplification factor less than 10 as shown in the [**Read Amplification** graph on the **Storage** dashboard](/docs/v25.4/ui-storage-dashboard#other-graphs) is considered healthy.
...
Value separation is especially beneficial for workloads with large values relative to key size (for example, [Raft log](/docs/v25.4/architecture/replication-layer#raft) entries). It reduces [write amplification](#write-amplification) by about 50%, at the cost of about 20% in [space amplification](/docs/v25.4/operational-faqs#space-amplification). In practice, value separation causes the storage engine to use far fewer [IOPS](/docs/v25.4/common-issues-to-monitor#disk-iops) and storage bandwidth overall, which are expensive, at the cost of an increase in storage capacity, which is much cheaper.

To enable value separation, set the following [cluster setting](/docs/v25.4/set-cluster-setting):
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        <a href="/docs/v25.4/architecture/storage-layer#value-separation">separated values</a> within the storage
        engine.
      </td>
      <td>
        Use this metric to track how many values (of key-value pairs) are being stored outside of the{" "}
        <a href="/docs/v25.4/architecture/storage-layer#log-structured-merge-trees">LSM</a> by the storage engine due to
        their large size.
      </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>
        ...
        <a href="/docs/v25.4/architecture/storage-layer#value-separation">stored separately in blob files</a> and
        referenced by a live <a href="/docs/v25.4/architecture/storage-layer#ssts">SSTable</a>.
      </td>
      <td>
    ...
    <tr>
      ...
      <td>
        The size of storage engine value bytes (pre-compression) that are{" "}
        <a href="/docs/v25.4/architecture/storage-layer#value-separation">stored separately in blob files</a> and not
        referenced by any live <a href="/docs/v25.4/architecture/storage-layer#ssts">SSTable</a>. These bytes are
        garbage that could be reclaimed by a <a href="/docs/v25.4/architecture/storage-layer#compaction">compaction</a>.
      </td>
...
<Tip>
  By default, the [storage engine](/docs/v25.4/architecture/storage-layer) uses a compaction concurrency of 3. If you
  ...
  LSM](/docs/v25.4/architecture/storage-layer#inverted-lsms) scenarios; and it can lead to increased overall performance
...
For instructions showing how to monitor your cluster's LSM health, see [LSM Health](/docs/v25.4/common-issues-to-monitor#lsm-health). To monitor your cluster's LSM L0 health, see [IO Overload](/docs/v25.4/ui-overload-dashboard#io-overload).
...
<Frame>![Relationship between memtable, WAL, and SSTs](/images/v25.4/memtable-wal-sst.png)</Frame>
...
CockroachDB regularly garbage collects MVCC values to reduce the size of data stored on disk. To do this, we compact old MVCC values when there is a newer MVCC value with a timestamp that's older than the garbage collection period. The garbage collection period can be set at the cluster, database, or table level by configuring the [`gc.ttlseconds` replication zone variable](/docs/v25.4/configure-replication-zones#gc-ttlseconds). For more information about replication zones, see [Replication Controls](/docs/v25.4/configure-replication-zones).
...
Now that you've learned about our architecture, [start up a CockroachDB Standard cluster](/docs/cockroachcloud/quickstart) or [local cluster](/docs/v25.4/install-cockroachdb) and start [building an app with CockroachDB](/docs/v25.4/example-apps).
