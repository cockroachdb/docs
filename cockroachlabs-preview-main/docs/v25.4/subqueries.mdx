...
SQL subqueries enable reuse of the results from a [selection query](/docs/v25.4/selection-queries) within another query.

CockroachDB supports two kinds of subqueries:

- **Relational** subqueries, which appear as operands in [selection queries](/docs/v25.4/selection-queries) and [table expressions](/docs/v25.4/table-expressions).
- **Scalar** subqueries, which appear as operands in a [scalar expression](/docs/v25.4/scalar-expressions).
...
This is true for subqueries defined using the [`(...)`](/docs/v25.4/table-expressions#use-a-subquery) notation and those defined using [`WITH`](/docs/v25.4/table-expressions#use-the-output-of-another-statement).
...
This query inserts 3 rows into `t`, even though the surrounding query only observes 1 row using [`LIMIT`](/docs/v25.4/limit-offset).
...
The [cost-based optimizer](/docs/v25.4/cost-based-optimizer) supports most [correlated subqueries](https://wikipedia.org/wiki/Correlated_subquery), with the exception of correlated subqueries that generate side effects inside a `CASE` statement.

### `LATERAL` subqueries

A `LATERAL` subquery is a correlated subquery that references another query or subquery in its `SELECT` statement, usually in the context of a [`LEFT` join](/docs/v25.4/joins#left-outer-joins) or an [`INNER` join](/docs/v25.4/joins#inner-joins). Unlike other correlated subqueries, `LATERAL` subqueries iterate through each row in the referenced query for each row in the inner subquery, like a [for loop](https://wikipedia.org/wiki/For_loop).
...
`LATERAL` subquery joins are especially useful when the join table includes a [computed column](/docs/v25.4/computed-columns).
...
- Statements containing multiple modification subqueries mutating the same row could cause corruption. These statements are disallowed by default, but you can enable multiple modification subqueries with one the following:
    - Set the `sql.multiple_modifications_of_table.enabled` [cluster setting](/docs/v25.4/cluster-settings) to `true`.
    - Use the `enable_multiple_modifications_of_table` [session variable](/docs/v25.4/set-vars).

        If multiple mutations inside the same statement affect different tables with [`FOREIGN KEY`](/docs/v25.4/foreign-key) relations and `ON CASCADE` clauses between them, the results will be different from what is expected in PostgreSQL. [#70731](https://github.com/cockroachdb/cockroach/issues/70731)
