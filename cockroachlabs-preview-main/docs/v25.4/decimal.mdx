...
The `DECIMAL` [data type](/docs/v25.4/data-types) stores exact, fixed-point numbers. This type is used when it is important to preserve exact precision, for example, with monetary data.
...
A constant value of type `DECIMAL` can be entered as a [numeric literal](/docs/v25.4/sql-constants#numeric-literals). For example: `1.414` or `-1234`.
...
The size of a `DECIMAL` value is variable, starting at 9 bytes. It's recommended to keep values under 64 kilobytes to ensure performance. Above that threshold, [write amplification](/docs/v25.4/architecture/storage-layer#write-amplification) and other considerations may cause significant performance degradation.
...
`DECIMAL` values can be [cast](/docs/v25.4/data-types#data-type-conversions-and-casts) to any of the following data types:
