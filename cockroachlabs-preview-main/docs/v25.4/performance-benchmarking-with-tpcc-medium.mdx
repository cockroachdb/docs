...
<LinkedPageTabs
  currentPath="/docs/v25.4/performance-benchmarking-with-tpcc-medium"
  items={[
    {
      title: "Local",
...
3. Run the [`cockroach start`](/docs/v25.4/cockroach-start) command:
    ...
    Each node will start with a [locality](/docs/v25.4/cockroach-start#locality) that includes an artificial "rack number" (e.g., `--locality=rack=0`). Use 5 racks for 15 nodes so that 3 nodes will be assigned to each rack.
...
Next, [partition your database](/docs/v25.4/partitioning) to divide all of the TPC-C tables and indexes into 5 partitions, one per rack, and then use [zone configurations](/docs/v25.4/configure-replication-zones) to pin those partitions to a particular rack.
