...
old_description: "A window function performs a calculation across a set of table rows that are somehow related to the current row."
---

import { WindowFunctions11 } from "/snippets/syntax-diagrams/WindowFunctions11.jsx";
import { WindowFunctions12 } from "/snippets/syntax-diagrams/WindowFunctions12.jsx";

CockroachDB supports the application of a function over a subset of the rows returned by a [selection query](/docs/v25.4/selection-queries). Such a function is known as a *window function*, and it allows you to compute values by operating on more than one row at a time. The subset of rows a window function operates on is known as a *window frame*.

For a complete list of supported window functions, see [Functions and Operators](/docs/v25.4/functions-and-operators#window-functions).

<Tip>
All [aggregate functions](/docs/v25.4/functions-and-operators#aggregate-functions) can also be used as [window functions](/docs/v25.4/functions-and-operators#window-functions). For more information, see the [Examples](#examples) below.
</Tip>

## Window definitions

Window frames are defined in [`OVER` clauses](/docs/v25.4/sql-grammar#over_clause) or [`WINDOW` clauses](/docs/v25.4/sql-grammar#window_clause).
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>window&#95;name</code></td><td>The name of the new window frame.</td></tr><tr><td><code>opt&#95;existing&#95;window&#95;name</code></td><td>An optional name of an existing window frame, defined in a different window definition.</td></tr><tr><td><code>opt&#95;partition&#95;clause</code></td><td>An optional <code>PARTITION BY</code> clause.</td></tr><tr><td><code>opt&#95;sort&#95;clause</code></td><td>An optional <code>ORDER BY</code> clause. See <a href="/docs/v25.4/order-by">Ordering Query Results</a> for details.</td></tr><tr><td><code>opt&#95;frame&#95;clause</code></td><td>An optional frame clause, which contains a frame boundary and/or an <code>EXCLUDE</code> clause.</td></tr></tbody></table>
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>window&#95;name</code></td><td>The name of the new window frame.</td></tr><tr><td><code>opt&#95;existing&#95;window&#95;name</code></td><td>An optional name of an existing window frame, defined in a different window definition.</td></tr><tr><td><code>opt&#95;partition&#95;clause</code></td><td>An optional <code>PARTITION BY</code> clause.</td></tr><tr><td><code>opt&#95;sort&#95;clause</code></td><td>An optional <code>ORDER BY</code> clause. See <a href="/docs/v25.4/order-by">Ordering Query Results</a> for details.</td></tr><tr><td><code>frame&#95;bound</code></td><td>An optional frame boundary.<br/>Valid start boundaries include <code>UNBOUNDED PRECEDING</code>, <code>&lt;offset&gt; PRECEDING</code>, and <code>CURRENT ROW</code>.<br/>Valid end boundaries include <code>UNBOUNDED FOLLOWING</code>, <code>&lt;offset&gt; FOLLOWING</code>, and <code>CURRENT ROW</code>.</td></tr><tr><td><code>opt&#95;frame&#95;exclusion</code></td><td>An optional frame <code>EXCLUDE</code> clause.<br/>Valid exclusions include <code>CURRENT ROW</code>, <code>GROUP</code>, <code>TIES</code>, and <code>NO OTHERS</code>.</td></tr></tbody></table>
...
1. Creating a "virtual table" using a [selection query](/docs/v25.4/selection-queries).
2. Splitting that table into window frames with [window definitions](#window-definitions). You can define window frames in an [`OVER` clause](/docs/v25.4/sql-grammar#over_clause), directly after the window function, or in a [`WINDOW` clause](/docs/v25.4/sql-grammar#window_clause), as a part of the selection query.
...
3. The window function `SUM(revenue) OVER (PARTITION BY city)` operates on several window frames in turn; each frame contains the `revenue` columns for a different city [partition](/docs/v25.4/partitioning) (Amsterdam, Boston, L.A., etc.).

<Frame>
  ![Window function diagram](/images/v25.4/window-functions.png)
</Frame>
...
Because of this, you should be aware of the behavior of any [aggregate function](/docs/v25.4/functions-and-operators#aggregate-functions) you use as a [window function](/docs/v25.4/functions-and-operators#window-functions). If you are not seeing results you expect from a window function, this behavior may explain why. You may need to specify the frame boundaries explicitly in [the window definition](#window-definitions).
...
- [Aggregate functions](/docs/v25.4/functions-and-operators#aggregate-functions)
- [Window Functions](/docs/v25.4/functions-and-operators#window-functions)
- [CockroachDB 2.0 Demo](https://www.youtube.com/watch?v=v2QK5VgLx6E)
