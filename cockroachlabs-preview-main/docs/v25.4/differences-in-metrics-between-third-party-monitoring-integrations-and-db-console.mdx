...
When using [Third-Party Monitoring Integrations](/docs/v25.4/third-party-monitoring-tools), such as the [metrics export feature](/docs/cockroachcloud/export-metrics), discrepancies may be seen when comparing those metrics charts to ones found on the [Metrics dashboards](/docs/v25.4/ui-overview#metrics) or [custom charts](/docs/v25.4/ui-custom-chart-debug-page) of the [DB Console](/docs/v25.4/ui-overview). This page explains why these different systems may yield different results.

## CockroachDB’s Timeseries Database

CockroachDB stores metrics data in its own internal timeseries database (TSDB). While CockroachDB exposes [point-in-time metrics](/docs/v25.4/monitor-cockroachdb-with-prometheus) via its [Prometheus endpoint](/docs/v25.4/prometheus-endpoint), it also periodically scrapes and writes this information to its own timeseries database storage. This data is scraped every 10 seconds and stored at that resolution. After a period of time determined by the [`timeseries.storage.resolution_10s.ttl` cluster setting](/docs/v25.4/cluster-settings#setting-timeseries-storage-resolution-10s-ttl), that 10 second resolution data is compacted into a 30 minute resolution. After another period of time determined by the [`timeseries.storage.resolution_30m.ttl cluster setting`](/docs/v25.4/cluster-settings#setting-timeseries-storage-resolution-30m-ttl), the 30 minute resolution data is deleted.
