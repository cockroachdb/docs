...
<Tip>
This tutorial explores the CockroachDB self-hosted integration with Prometheus. For the CockroachDB Advanced integration with Prometheus, refer to [Export Metrics From a CockroachDB Advanced Cluster](/docs/cockroachcloud/export-metrics-advanced?filters=prometheus-metrics-export) instead of this page.
</Tip>

## Before you begin

- Make sure you have already started a CockroachDB cluster, either [locally](/docs/v25.4/start-a-local-cluster) or in a [production environment](/docs/v25.4/manual-deployment).
...
2. Point your browser to `http://<hostname of machine running prometheus>:9090`, where you can use the Prometheus UI to query, aggregate, and graph CockroachDB time series metrics.
    - Prometheus auto-completes CockroachDB time series metrics for you, but if you want to see a full listing, with descriptions, point your browser to the [Prometheus endpoint](/docs/v25.4/prometheus-endpoint).
...
7. Now that Alertmanager is configured and running, you can optionally [import pre-existing rules or create your own](/docs/v25.4/monitoring-and-alerting#alertmanager) if you prefer.
