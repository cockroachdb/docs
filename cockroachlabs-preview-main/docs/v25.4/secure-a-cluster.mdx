...
<LinkedPageTabs
  currentPath="/docs/v25.4/secure-a-cluster"
  items={[
    {
      title: "Secure",
...
Once you've [installed CockroachDB](/docs/v25.4/install-cockroachdb), it's simple to run a secure multi-node cluster locally, using [TLS certificates](/docs/v25.4/cockroach-cert) to encrypt network communication.
...
You can use either [`cockroach cert`](/docs/v25.4/cockroach-cert) commands or [`openssl` commands](/docs/v25.4/create-security-certificates-openssl) to generate security certificates. This section features the `cockroach cert` commands.
...
1. Use the [`cockroach start`](/docs/v25.4/cockroach-start) command to start the first node:
...
4. Use the [`cockroach init`](/docs/v25.4/cockroach-init) command to perform a one-time initialization of the cluster, sending the request to any node on the `--join` list:
    ...
    At this point, each node also prints helpful [startup details](/docs/v25.4/cockroach-start#standard-output) to its log. For example, the following command retrieves node 1's startup details:
...
1. Run the [`cockroach sql`](/docs/v25.4/cockroach-sql) command against node 1:
...
5. Now [create a user with a password](/docs/v25.4/create-user#create-a-user-with-a-password), which you will need to [access the DB Console](#step-5-access-the-db-console):
...
1. On secure clusters, [certain pages of the DB Console](/docs/v25.4/ui-overview#db-console-access) can only be accessed by `admin` users.
    ...
    Run the [`cockroach sql`](/docs/v25.4/cockroach-sql) command against node 1:
...
6. On the [**Cluster Overview**](/docs/v25.4/ui-cluster-overview-page), notice that three nodes are live, with an identical replica count on each node:
...
2. Back on the **Cluster Overview** in the DB Console, you'll now see 5 nodes listed:
    ...
    At first, the replica count will be lower for nodes 4 and 5. Very soon, however, you'll see those numbers even out across all nodes, indicating that data is being [automatically rebalanced](/docs/v25.4/demo-replication-and-rebalancing) to utilize the additional capacity of the new nodes.
...
- [CockroachDB SQL client](/docs/v25.4/cockroach-sql)
