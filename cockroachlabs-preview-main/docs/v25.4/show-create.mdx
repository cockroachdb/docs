...
import { ShowCreate6 } from "/snippets/syntax-diagrams/ShowCreate6.jsx";

The `SHOW CREATE` [statement](/docs/v25.4/sql-statements) shows the `CREATE` statement for an existing [database](/docs/v25.4/create-database), [function](/docs/v25.4/create-function), [table](/docs/v25.4/create-table), [view](/docs/v25.4/create-view), or [sequence](/docs/v25.4/create-sequence).

## Required privileges

The user must have any [privilege](/docs/v25.4/security-reference/authorization#managing-privileges) on the target database, function, table, view, or sequence.
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>object&#95;name</code></td><td>The name of the database, function, table, view, or sequence for which to show the <code>CREATE</code> statement.</td></tr><tr><td><code>ALL TABLES</code></td><td>Show the <code>CREATE</code> statements for all tables, views, and sequences in the current database.<br/>This option is intended to provide the statements required to recreate the objects in the current database. As a result, <code>SHOW CREATE ALL TABLES</code> also returns the <a href="/docs/v25.4/alter-table"><code>ALTER</code> statements</a> that add, modify, and validate an object's <a href="/docs/v25.4/constraints">constraints</a>. The <code>ALTER</code> statements follow the <code>CREATE</code> statements to guarantee that all objects are added before their references.</td></tr><tr><td><code>ALL SCHEMAS</code></td><td>Show the <code>CREATE</code> statements for all <a href="/docs/v25.4/create-schema">schemas</a> in the current database.</td></tr><tr><td><code>ALL TYPES</code></td><td>Show the <code>CREATE</code> statements for all <a href="/docs/v25.4/create-type">types</a> in the current database.</td></tr></tbody></table>
...
<Note>
`SHOW CREATE TABLE` also lists any partitions and [zone configurations](/docs/v25.4/configure-replication-zones) defined on primary and secondary indexes of a table. If partitions are defined, but no zones are configured, the `SHOW CREATE TABLE` output includes a warning.
</Note>

### Show the `CREATE TABLE` statement for a table with a hidden column

If one or more columns is [`NOT VISIBLE`](/docs/v25.4/create-table#not-visible-property) within a table, `SHOW CREATE` will display the `NOT VISIBLE` flag after those columns.
...
If you [add a comment](/docs/v25.4/comment-on) on a table, `SHOW CREATE TABLE` will display the comment.
...
For more information, see [`COMMENT ON`](/docs/v25.4/comment-on).

### Show the `CREATE TABLE` statement for a table with a multi-region locality

Use the `SHOW CREATE TABLE` command to view [multi-region-defined](/docs/v25.4/multiregion-overview) table localities.

To add the first region to the database, or to set an already-added region as the primary region, use a [`SET PRIMARY REGION`](/docs/v25.4/alter-database#set-primary-region) statement:
...
All tables will be [`REGIONAL BY TABLE`](/docs/v25.4/alter-table#regional-by-table) in `us-east` by default. Configure the `users` table to be [`REGIONAL BY ROW`](/docs/v25.4/alter-table#regional-by-row) instead:
...
Note that this statement also returns the [`ALTER` statements](/docs/v25.4/alter-table) that add, modify, and validate an object's [constraints](/docs/v25.4/constraints).
...
Suppose that you have a multi-region cluster, and you want to return the `SHOW CREATE DATABASE` statement for a [multi-region database](/docs/v25.4/multiregion-overview).
...
In the SQL shell, [add regions to the database](/docs/v25.4/alter-database#add-region):
...
- [`CREATE TABLE`](/docs/v25.4/create-sequence)
