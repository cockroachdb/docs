...
You can create changefeeds on tables with more than one [column family](/docs/v25.4/column-families). Changefeeds will emit individual messages per column family on a table.
...
To target a table with multiple column families, set the [`split_column_families` option](/docs/v25.4/create-changefeed#split-column-families) when creating a changefeed:
...
<Note>
You can also use [sinkless changefeeds](/docs/v25.4/changefeeds-on-tables-with-column-families?filters=sinkless#create-a-sinkless-changefeed-on-a-table-with-column-families) on tables with column families by using the [`CREATE CHANGEFEED`](/docs/v25.4/create-changefeed) statement without a sink with `split_column_families` or the `FAMILY` keyword.
</Note>
...
The response will follow a typical [changefeed message format](/docs/v25.4/changefeed-messages), but with the family name appended to the table name with a `.`, in the format `table.family`:

```text
{"after":{"column":"value"},"key":[1],"topic":"table.family"}
```

For [cloud storage sinks](/docs/v25.4/changefeed-sinks#cloud-storage-sink), the filename will include the family name appended to the table name with a `+`, in the format `table+primary`.

[Avro](/docs/v25.4/changefeed-messages#avro) schema names will include the family name concatenated to the table name.
...
In this example, you'll set up changefeeds on two tables that have [column families](/docs/v25.4/column-families). You'll use a single-node cluster sending changes to a webhook sink for this example, but you can use any [changefeed sink](/docs/v25.4/changefeed-sinks) to work with tables that include column families.
...
13. To create a changefeed that emits messages for all column families in a table, use the [`split_column_families`](/docs/v25.4/create-changefeed#split-column-families) option:
    ...
    <Note>
    You can find details of your changefeed job using [`SHOW CHANGEFEED JOBS`](/docs/v25.4/show-jobs#show-changefeed-jobs). Changefeeds streaming to [Kafka](/docs/v25.4/changefeed-sinks#kafka) or [Google Cloud Pub/Sub](/docs/v25.4/changefeed-sinks#google-cloud-pub-sub) will populate the `topics` field in the `SHOW CHANGEFEED JOBS` output.
...
In this example, you'll set up a sinkless changefeed on two tables that have [column families](/docs/v25.4/column-families). You'll use a single-node cluster with the changefeed sending changes to the client.
...
11. To create a changefeed that emits messages for all column families in a table, use the [`split_column_families`](/docs/v25.4/changefeed-for#split-column-families) option:
...
- [`EXPERIMENTAL CHANGEFEED`](/docs/v25.4/changefeed-for)
- [`CREATE CHANGEFEED`](/docs/v25.4/create-changefeed)
- [Changefeed Sinks](/docs/v25.4/changefeed-sinks)
- [Changefeed Examples](/docs/v25.4/changefeed-examples)
