...
*Row-level Security* (RLS) is a security feature that allows organizations to restrict access to specific rows of data in a database based on user [roles](/docs/v25.4/security-reference/authorization#roles), [permissions](/docs/v25.4/security-reference/authorization#authorization-models), or other criteria.

Row-level security complements standard SQL privileges ([`GRANT`](/docs/v25.4/grant)/[`REVOKE`](/docs/v25.4/revoke)) by allowing administrators to define policies that determine precisely which rows users can view or modify within a specific table.
...
1. Choose which [schema objects](/docs/v25.4/schema-design-overview) need row-level security. You can re-use existing schemas, or create new ones and [insert data](/docs/v25.4/insert-data). ( [`CREATE TABLE`](/docs/v25.4/create-table), [`INSERT`](/docs/v25.4/insert) )
2. [Create roles](/docs/v25.4/create-role) &amp; [grant access](/docs/v25.4/grant) to schema objects by those roles. ( [`CREATE ROLE`](/docs/v25.4/create-role), [`GRANT`](/docs/v25.4/grant) )
3. Enable row-level security on the schema objects. ( [`ALTER TABLE... ENABLE ROW LEVEL SECURITY`](/docs/v25.4/alter-table#enable-row-level-security) )
4. Define row-level security policies on the schema objects which are assigned to specific roles. ( [`CREATE POLICY`](/docs/v25.4/create-policy) )
...
Policies function as filters or constraints applied automatically by CockroachDB during [query execution](/docs/v25.4/architecture/sql-layer#query-execution). They are based on boolean expressions evaluated in the context of the current [user](/docs/v25.4/security-reference/authorization#roles), [session properties](/docs/v25.4/show-sessions), and the row data itself.

When row-level security is enabled on a table:

1. Existing [SQL privileges](/docs/v25.4/security-reference/authorization#supported-privileges) still determine **if** a user can access the table at all (e.g., `SELECT`, `INSERT` ).
2. [Row-level security policies](/docs/v25.4/show-policies) determine **which rows** within the table are accessible or modifiable for specific commands.

Further details about RLS evaluation include:

- All [policies](/docs/v25.4/show-policies) apply to a specific set of [roles](/docs/v25.4/security-reference/authorization#roles). For a policy to be applicable, it must match at least one of the roles assigned to it. If the policy is associated with the `PUBLIC` role, it applies to all roles.
...
- The `USING` clause of [`CREATE POLICY`](/docs/v25.4/create-policy) filters rows during reads; the `WITH CHECK` clause validates writes, and defaults to `USING` if absent.

## Considerations

### Performance

Complex [policy expressions](/docs/v25.4/create-policy) evaluated per-row can impact query performance. To limit the performance impacts of row-level security, optimize your policy expressions and consider [indexing](/docs/v25.4/indexes) relevant columns.

### Security privileges

[Policy expressions](/docs/v25.4/create-policy) execute with the [privileges](/docs/v25.4/security-reference/authorization#supported-privileges) of the user invoking the query, unless functions marked [`SECURITY DEFINER`](/docs/v25.4/create-function#create-a-security-definer-function) are used.
...
- [Foreign keys](/docs/v25.4/foreign-key) (including cascades)
- [Primary key constraints](/docs/v25.4/primary-key)
- [Unique constraints](/docs/v25.4/unique)
- [`TRUNCATE`](/docs/v25.4/truncate)

### Change data capture (CDC)

[CDC](/docs/v25.4/change-data-capture-overview) messages that are emitted from a table will not be filtered using RLS policies. Furthermore, [CDC queries](/docs/v25.4/cdc-queries) are not supported on tables using RLS, and will fail with the error message: `CDC queries are not supported on tables with row-level security enabled`.

### Backup and restore

[Backup and restore](/docs/v25.4/backup-and-restore-overview) functionality does not take RLS policies into account.

### Logical data replication (LDR) and Physical cluster replication (PCR)

[Logical Data Replication (LDR)](/docs/v25.4/logical-data-replication-overview) and [Physical Cluster Replication (PCR)](/docs/v25.4/physical-cluster-replication-overview) do not take RLS policies into account.

LDR's [limitations with respect to schema changes](/docs/v25.4/manage-logical-data-replication#schema-changes) also apply to RLS, since RLS policies amount to a schema change.
...
When [views](/docs/v25.4/views) are accessed, RLS policies on any underlying [tables](/docs/v25.4/schema-design-table) are applied. [Policies](/docs/v25.4/create-policy) can only be defined directly on tables, not views.

Views will only show rows that the current [user](/docs/v25.4/security-reference/authorization#roles) has access to on the table that the view is referencing.

## Examples

### Create a policy

Refer to [`CREATE POLICY`](/docs/v25.4/create-policy).

### Alter a policy

Refer to [`ALTER POLICY`](/docs/v25.4/alter-policy).

### Drop a policy

Refer to [`DROP POLICY`](/docs/v25.4/drop-policy).
...
- [`ALTER TABLE... FORCE ROW LEVEL SECURITY`](/docs/v25.4/alter-table#force-row-level-security).

### RLS for data security (fine-grained access control)

In a fine-grained access control scenario, you will want to restrict access to specific rows within a table based on user [roles](/docs/v25.4/security-reference/authorization#roles), attributes, or relationships defined within the data itself. This goes beyond table-level [`GRANT`](/docs/v25.4/grant) permissions. Common examples include restricting access to salary information, personal data, or region-specific records.
...
Enable row-level security using the [`ALTER TABLE... ENABLE ROW LEVEL SECURITY`](/docs/v25.4/alter-table#enable-row-level-security) statement. Optionally, you may want to ensure that the table owner is also subject to RLS using [`ALTER TABLE... FORCE ROW LEVEL SECURITY`](/docs/v25.4/alter-table#force-row-level-security).
...
The following policy allows managers to view their direct reports' records. This requires a way to look up the manager's username. In this example, we use the `CURRENT_USER` special form of the [function with the same name](/docs/v25.4/functions-and-operators#special-syntax-forms).
...
Specifically, the [UUID](/docs/v25.4/uuid) following the period in `application_name` is the tenant ID. We will use the `current_setting()` function in our RLS policies to extract the ID.
...
For a demo showing how to combine row-level security with [Multi-region SQL](/docs/v25.4/multiregion-overview) to constrain access to specific rows based on a user's geographic region, play the following video:
...
- [`CREATE POLICY`](/docs/v25.4/create-policy)
