...
- A CockroachDB self-hosted cluster hosted on AWS. You can set up a cluster using [Deploy CockroachDB on AWS EC2](/docs/v25.4/deploy-cockroachdb-on-aws). You must create instances in the same VPC that the MSK cluster will use in order for the changefeed to authenticate successfully.
...
- The `CHANGEFEED` privilege in order to create and manage changefeed jobs. Refer to [Required privileges](/docs/v25.4/create-changefeed#required-privileges) for more details.
...
1. (Optional) On the EC2 instance running CockroachDB, run the [Movr](/docs/v25.4/movr) application workload to set up some data for your changefeed.
    ...
    ```
2. Start a SQL session. For details on the available flags, refer to the [`cockroach sql`](/docs/v25.4/cockroach-sql) page.
...
4. To connect the changefeed to the MSK cluster, the URI must contain the following parameters:
    ...
    - `sasl_aws_iam_session_name` set to a string that you specify to identify the session in AWS.
        ...
        You can either specify the Kafka URI in the `CREATE CHANGEFEED` statement directly. Or, create an [external connection](/docs/v25.4/create-external-connection) for the MSK URI.
        ...
        ```
5. Use the [`CREATE CHANGEFEED`](/docs/v25.4/create-changefeed) statement to start the changefeed using either the external connection (`external://`) or full `kafka://` URI:
    ...
    To view a changefeed job, use [`SHOW CHANGEFEED JOBS`](/docs/v25.4/show-jobs#show-changefeed-jobs).
...
- [Changefeed Sinks](/docs/v25.4/changefeed-sinks) page for details on parameters that sinks support.
- [Monitor and Debug Changefeeds](/docs/v25.4/monitor-and-debug-changefeeds) for details on monitoring the changefeed job.
