...
A column family is a group of columns in a table that are stored as a single key-value pair in the [underlying key-value store](/docs/v25.4/architecture/storage-layer). Column families reduce the number of keys stored in the key-value store, resulting in improved performance during [`INSERT`](/docs/v25.4/insert), [`UPDATE`](/docs/v25.4/update), and [`DELETE`](/docs/v25.4/delete) operations.
...
[Secondary indexes](/docs/v25.4/indexes) respect the column family definitions applied to tables. When you define a secondary index, CockroachDB breaks the secondary index key-value pairs into column families, according to the family and stored column configurations.
...
To manually assign a column family on [table creation](/docs/v25.4/create-table), use the `FAMILY` keyword.
...
When using the [`ALTER TABLE.. ADD COLUMN`](/docs/v25.4/alter-table#add-column) statement to add a column to a table, you can assign the column to a new or existing column family.
...
- When creating or updating a row, if the combined size of all values in a single [column family](/docs/v25.4/column-families) exceeds the [max range size](/docs/v25.4/configure-replication-zones#range-max-bytes) for the table, the operation may fail, or cluster performance may suffer. As a workaround, you can either [manually split a table's columns into multiple column families](/docs/v25.4/column-families#manual-override), or you can [create a table-specific zone configuration](/docs/v25.4/configure-replication-zones#create-a-replication-zone-for-a-table) with an increased max range size.
