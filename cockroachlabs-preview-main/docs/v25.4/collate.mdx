...
The `COLLATE` feature lets you sort [`STRING`](/docs/v25.4/string) values according to language- and country-specific rules, known as collations.
...
- **Column syntax**: `STRING COLLATE <collation>`. For example:
    ...
    <Note>
    You can also use any of the [aliases for `STRING`](/docs/v25.4/string#aliases).
    </Note>
...
You can use the `pg_collation_for` [built-in function](/docs/v25.4/functions-and-operators#string-and-byte-functions), or its alternative [syntax form](/docs/v25.4/functions-and-operators#special-syntax-forms) `COLLATION FOR`, to return the locale name of a collated string.
...
- Many string operations are not properly overloaded for [collated strings](/docs/v25.4/collate). For example, the `||` concatenation operator works with regular strings but returns an error with collated strings. [#10679](https://github.com/cockroachdb/cockroach/issues/10679)
