...
import { CreateStatistics11 } from "/snippets/syntax-diagrams/CreateStatistics11.jsx";
import { CreateStatistics12 } from "/snippets/syntax-diagrams/CreateStatistics12.jsx";

Use the `CREATE STATISTICS` [statement](/docs/v25.4/sql-statements) to [generate table statistics for the cost-based optimizer](/docs/v25.4/cost-based-optimizer#table-statistics) to use.

Once you [create a table](/docs/v25.4/create-table) and load data into it (e.g., [`INSERT`](/docs/v25.4/insert), [`IMPORT INTO`](/docs/v25.4/import-into)), table statistics can be generated. Table statistics help the cost-based optimizer determine the cardinality of the rows used in each query, which helps to predict more accurate costs.
...
For [PostgreSQL compatibility](/docs/v25.4/postgresql-compatibility), CockroachDB supports `ANALYZE` and `ANALYSE` as aliases for `CREATE STATISTICS`.
...
For more information about how the `AS OF SYSTEM TIME` clause works, including supported time formats, see [`AS OF SYSTEM TIME`](/docs/v25.4/as-of-system-time).

### Create partial statistics using extremes

CockroachDB supports [partial statistics](/docs/v25.4/cost-based-optimizer#partial-statistics), which collect statistics on a subset of table data to provide more up-to-date information without scanning the entire table.

To create [partial statistics](/docs/v25.4/cost-based-optimizer#partial-statistics) that collect statistics on the highest and lowest index values:
...
<InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v25.4: To create [partial statistics](/docs/v25.4/cost-based-optimizer#partial-statistics) on a specific column and values matching specific conditions, ensure the column is indexed:
...
For more information about the `DELETE` statement, see [`DELETE`](/docs/v25.4/delete).

### View statistics jobs

Every time the `CREATE STATISTICS` statement is executed, it starts a background job. This is true for queries issued by your application as well as queries issued for [automatically generated statistics](/docs/v25.4/cost-based-optimizer#table-statistics).

To view statistics jobs, there are two options:

1. Use [`SHOW JOBS`](/docs/v25.4/show-jobs) to see all statistics jobs that were created by user queries (i.e., someone entering `CREATE STATISTICS` at the SQL prompt or via application code):
    ...
    (5 rows)
    ```
2. Use `SHOW AUTOMATIC JOBS` to see statistics jobs that were created by [automatically generated statistics](/docs/v25.4/cost-based-optimizer#table-statistics):
...
The `ANALYZE` alias does not support specifying an [`AS OF SYSTEM TIME`](/docs/v25.4/as-of-system-time) timestamp. `ANALYZE` statements use `AS OF SYSTEM TIME '-0.001ms'` automatically. For more control over the statistics interval, use the `CREATE STATISTICS` syntax instead. [#96430](https://github.com/cockroachdb/cockroach/issues/96430)
