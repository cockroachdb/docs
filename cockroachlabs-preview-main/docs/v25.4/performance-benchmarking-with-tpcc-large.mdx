...
<LinkedPageTabs
  currentPath="/docs/v25.4/performance-benchmarking-with-tpcc-large"
  items={[
    {
      title: "Local",
...
3. Start CockroachDB using the [`cockroach start`](/docs/v25.4/cockroach-start) command:
    ...
    Each node will start with a [locality](/docs/v25.4/cockroach-start#locality) that includes an artificial "rack number" (e.g., `--locality=rack=0`). Use 81 racks for 81 nodes so that 1 node will be assigned to each rack.
4. Repeat these steps for the other 80 VMs for CockroachDB nodes. Each time, be sure to:
    - Adjust the `--advertise-addr` flag.
    - Set the [`--locality`](/docs/v25.4/cockroach-start#locality) flag to the appropriate "rack number".
5. On any of the VMs with the `cockroach` binary, run the one-time [`cockroach init`](/docs/v25.4/cockroach-init) command to join the first nodes into a cluster:
...
2. Launch the [built-in SQL shell](/docs/v25.4/cockroach-sql):

    ```shell
    $ cockroach sql --insecure --host=<address of any node>
    ```
3. Adjust some [cluster settings](/docs/v25.4/cluster-settings):
    ...
    ```
4. Change the default [GC TTL](/docs/v25.4/configure-replication-zones#gc-ttlseconds) to the following value:
...
3. Import the TPC-C dataset:
    ...
    You can monitor progress on the **Jobs** screen of the DB Console. Open the [DB Console](/docs/v25.4/ui-overview) by pointing a browser to the address in the `admin` field in the standard output of any node on startup.

## Step 5. Partition the database

1. [Partition your database](/docs/v25.4/partitioning) to divide all of the TPC-C tables and indexes into 81 partitions, one per rack, and then use [zone configurations](/docs/v25.4/configure-replication-zones) to pin those partitions to a particular rack.
2. Wait for up-replication and partitioning to finish. You will know when they have finished because both the number of *lease transfers* and *snapshots* will go down to `0` and stay there. This will likely take 10s of minutes.
    - To monitor the number of lease transfers, open the [DB Console](/docs/v25.4/ui-overview), select the **Replication** dashboard, hover over the **Range Operations** graph, and check the **Lease Transfers** data point.
    - To check the number of snapshots, open the [DB Console](/docs/v25.4/ui-overview), select the **Replication** dashboard, and hover over the **Snapshots** graph.

        <Frame>
          ![TPC-C 140k replication and partitioning dashboards](/images/v25.4/tpcc-large-replication-dashboard.png)
        </Frame>
...
- [Performance Overview](/docs/v25.4/performance)
- Hardware

    CockroachDB works well on commodity hardware in public cloud, private cloud, on-prem, and hybrid environments. For hardware recommendations, see our [Production Checklist](/docs/v25.4/recommended-production-settings#hardware).
- Performance tuning

    For guidance on tuning a real workload's performance, see [SQL Best Practices](/docs/v25.4/performance-best-practices-overview), and for guidance on techniques to minimize network latency in multi-region or global clusters, see [Multi-Region Capabilities Overview](/docs/v25.4/multiregion-overview).
