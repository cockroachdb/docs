...
A user-defined function (UDF) is a named function defined at the database level that can be called in queries and other contexts. CockroachDB supports invoking UDFs in `SELECT`, `FROM`, and `WHERE` clauses of [DML statements](/docs/v25.4/sql-statements#data-manipulation-statements).

Both [stored procedures](/docs/v25.4/stored-procedures) and user-defined functions are types of *routines*. However, they differ in the following ways:

- Functions return a value, and procedures do not return a value.
- Procedures must be invoked using a [`CALL`](/docs/v25.4/call) statement. Functions can be invoked in nearly any context, such as `SELECT`, `FROM`, and `WHERE` clauses, [`DEFAULT`](/docs/v25.4/default-value) expressions, and [computed column](/docs/v25.4/computed-columns) expressions.
- Functions have [volatility](/docs/v25.4/functions-and-operators#function-volatility) settings, and procedures do not.
...
- An argument has a *mode* and a *type*.
    - CockroachDB supports the `IN` (default), `OUT`, and `INOUT` argument modes. For an example, see [Create a function that uses `OUT` and `INOUT` parameters](/docs/v25.4/create-function#create-a-function-that-uses-out-and-inout-parameters).
    - The type can be a built-in type, [user-defined `ENUM`](/docs/v25.4/enum) or [composite](/docs/v25.4/create-type#create-a-composite-data-type) type, or implicit record type. A type can have a `DEFAULT` value.
- The return type can be a built-in [SQL type](/docs/v25.4/data-types), user-defined [`ENUM`](/docs/v25.4/enum) or [composite](/docs/v25.4/create-type#create-a-composite-data-type) type, [`RECORD`](/docs/v25.4/create-function#create-a-function-that-returns-a-record-type), [`TABLE`](/docs/v25.4/create-function#create-a-function-that-returns-a-table), PL/pgSQL [`REFCURSOR`](/docs/v25.4/plpgsql#declare-cursor-variables) type, implicit record type, [`TRIGGER`](/docs/v25.4/triggers#trigger-function), or `VOID`.
    - Preceding a type with `SETOF` indicates that a set, or multiple rows, may be returned. For an example, see [Create a function that returns a set of results](/docs/v25.4/create-function#create-a-function-that-returns-a-set-of-results).
    - `VOID` indicates that there is no return type and `NULL` will always be returned.
- The [volatility](/docs/v25.4/functions-and-operators#function-volatility) indicates whether the function has side effects. `VOLATILE` and `NOT LEAKPROOF` are the default.
    - Annotate a function with side effects with `VOLATILE`. This also prevents the [cost-based optimizer](/docs/v25.4/cost-based-optimizer) from pre-evaluating the function.
...
- `LANGUAGE` specifies the language of the function body. CockroachDB supports the languages `SQL` and [`PLpgSQL` (PL/pgSQL)](/docs/v25.4/plpgsql).
- The function body:
...
For more examples of UDF creation, see [`CREATE FUNCTION`](/docs/v25.4/create-function).
...
You invoke a UDF like a [built-in function](/docs/v25.4/functions-and-operators).
...
You can include UDF calls in a [view](/docs/v25.4/views) definition. The function is invoked each time the view is queried. A [materialized view](/docs/v25.4/views#materialized-views) will invoke the function only when the view is created or refreshed.

Refer to [Create a view that references routines](/docs/v25.4/create-view#create-a-view-that-references-routines).
...
When possible, the [cost-based optimizer](/docs/v25.4/cost-based-optimizer) will improve a function's performance by inlining the UDF within the query plan. The UDF must have the following attributes:

- It is labeled as `IMMUTABLE`, `STABLE`, or `LEAKPROOF` (i.e., non- `VOLATILE` ).
- It has a single statement.
- It is not a [set-returning function](/docs/v25.4/create-function#create-a-function-that-returns-a-set-of-results).
- Its arguments are only variable or constant expressions.
- It is not a [record-returning function](/docs/v25.4/create-function#create-a-function-that-returns-a-record-type).
...
5. View the query plan when using `foo_s()` (the `STABLE` function) instead:
    ...
    The query takes only `4ms` to execute because the function is inlined and transformed to a [join](/docs/v25.4/joins) with an equality comparison `(a) = (b)`, which has much less overhead than invoking a function for each row scanned in table `a`.
...
- Casting subqueries to [user-defined types](/docs/v25.4/create-type) in UDFs is not supported. [#108184](https://github.com/cockroachdb/cockroach/issues/108184)
...
- Routine parameters and return types cannot be declared using the `ANYENUM` polymorphic type, which is able to match any [`ENUM`](/docs/v25.4/enum) type. [123048](https://github.com/cockroachdb/cockroach/issues/123048)

Also refer to the [PL/pgSQL known limitations](/docs/v25.4/plpgsql#known-limitations).
