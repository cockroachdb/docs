...
import { CreateFunction4 } from "/snippets/syntax-diagrams/CreateFunction4.jsx";

The `CREATE FUNCTION` [statement](/docs/v24.3/sql-statements) creates a [user-defined function](/docs/v24.3/user-defined-functions).

<Note>
The `CREATE FUNCTION` statement performs a schema change. For more information about how online schema changes work in CockroachDB, see [Online Schema Changes](/docs/v24.3/online-schema-changes).
</Note>

## Required privileges

- To create a function, a user must have [`CREATE` privilege](/docs/v24.3/security-reference/authorization#supported-privileges) on the schema of the function. The user must also have privileges on all the objects referenced in the function body.
- To define a function with a [user-defined type](/docs/v24.3/create-type), a user must have `USAGE` privilege on the user-defined type.
...
A trigger function is a [function that is executed by a trigger](/docs/v24.3/triggers#trigger-function). A trigger function must return type `TRIGGER` and is written in [PL/pgSQL](/docs/v24.3/plpgsql).
...
The preceding example modifies a given `name` value and returns the `NEW` [trigger variable](/docs/v24.3/triggers#trigger-variables) because it is meant to be executed by a `BEFORE` trigger. For details, refer to [Triggers](/docs/v24.3/triggers).
...
1. Create two roles:
    ...
    CREATE ROLE invoker;
    ```
2. Grant a [`SELECT` privilege](/docs/v24.3/grant#supported-privileges) on the `user_promo_codes` table to the `owner` role.
...
4. Create a simple `SECURITY DEFINER` function that reads the contents of `user_promo_codes`.
    ...
    CREATE OR REPLACE FUNCTION get_codes() 
      ...
      $$;
    ```
5. Grant the [`EXECUTE` privilege](/docs/v24.3/grant#supported-privileges) on the `get_codes` function to the `invoker` role.
