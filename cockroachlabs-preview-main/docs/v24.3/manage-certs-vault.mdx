...
old_description: "Use Google Cloud Platform Certificate Authority Service to manage PKI certificates"
---

This tutorial guides the user through implementing [public key infrastructure (PKI)](https://www.cockroachlabs.com/docs/v23.1/security-reference/transport-layer-security) for a CockroachDB self-hosted cluster deployed in Google Cloud Platform (GCP), using [Vault PKI Secrets Engine](https://www.cockroachlabs.com/docs/v23.1/hashicorp-integration).
...
- Review [Transport Layer Security (TLS) and Public Key Infrastructure (PKI)](https://www.cockroachlabs.com/docs/v23.1/security-reference/transport-layer-security).
- Vault:
...
- To enforce *short validity durations for all issued certificates*, rather than employing the more heavy-weight option of [Online Certificate Status Protocol (OCSP)](https://www.cockroachlabs.com/docs/v23.1/manage-certs-revoke-ocsp).
...
CockroachDB OCSP but not CRLs. To learn more, read [Using Online Certificate Status Protocol (OCSP) with CockroachDB](https://www.cockroachlabs.com/docs/v23.1/manage-certs-revoke-ocsp).
...
<table><thead><tr><th>persona</th><th>tasks</th><th>Vault permissions</th><th>GCP Permissions</th></tr></thead><tbody><tr><td><code>ca-admin</code></td><td>- Manage CA lifecycle<br/><br/>- Issue node certificates<br/><br/>- Issue client certificates</td><td>- Admin</td><td>- CA jumpbox access<br/><br/>- Node SSH access<br/><br/>- Client SSH access</td></tr><tr><td><code>node-operator</code></td><td>- Install, configure and run CockroachDB and dependencies on nodes</td><td></td><td>- Node SSH access</td></tr><tr><td><code>client-operator</code></td><td>- Install, configure and run CockroachDB and dependencies on clients</td><td></td><td>- Client SSH access</td></tr></tbody></table>
...
3. Create node instances
    ...
    Create three compute instances to use as CockroachDB nodes. Follow the guidelines described in [Deploy CockroachDB on Google Cloud Platform](https://www.cockroachlabs.com/docs/v23.1/deploy-cockroachdb-on-google-cloud-platform#step-2-create-instances), and additionally, specify the `node-operator` service account as the managing service account.
...
3. Create a secrets directory on the jumpbox.
    ...
    ```
4. [Install Vault](https://learn.hashicorp.com/tutorials/vault/getting-started-install) on the jumpbox, following the [instructions for **Ubuntu/Debian** Linux](https://www.vaultproject.io/downloads).
...
### Provision each node's credentials and [trust store](https://www.cockroachlabs.com/docs/v23.1/security-reference/transport-layer-security#trust-store)
...
function install_roach_on_node {
    gcloud compute ssh $1 --command 'if [[ ! -e cockroach-v23.1.30.linux-amd64 ]];
    then
        echo "roach not installed; installing roach"
        sudo curl https://binaries.cockroachdb.com/cockroach-v23.1.30.linux-amd64.tgz | tar -xz
        sudo cp -i cockroach-v23.1.30.linux-amd64/cockroach /usr/local/bin/
        sudo mkdir -p /usr/local/lib/cockroach
        sudo cp -i cockroach-v23.1.30.linux-amd64/lib/libgeos.so /usr/local/lib/cockroach/
        sudo cp -i cockroach-v23.1.30.linux-amd64/lib/libgeos_c.so /usr/local/lib/cockroach/
    else
