...
import { WindowFunctions7 } from "/snippets/syntax-diagrams/WindowFunctions7.jsx";
import { WindowFunctions8 } from "/snippets/syntax-diagrams/WindowFunctions8.jsx";

CockroachDB supports the application of a function over a subset of the rows returned by a [selection query](/docs/v24.3/selection-queries). Such a function is known as a *window function*, and it allows you to compute values by operating on more than one row at a time. The subset of rows a window function operates on is known as a *window frame*.

For a complete list of supported window functions, see [Functions and Operators](/docs/v24.3/functions-and-operators#window-functions).

<Tip>
All [aggregate functions](/docs/v24.3/functions-and-operators#aggregate-functions) can also be used as [window functions](/docs/v24.3/functions-and-operators#window-functions). For more information, see the [Examples](#examples) below.
</Tip>

## Window definitions

Window frames are defined in [`OVER` clauses](/docs/v24.3/sql-grammar#over_clause) or [`WINDOW` clauses](/docs/v24.3/sql-grammar#window_clause).
...
1. Creating a "virtual table" using a [selection query](/docs/v24.3/selection-queries).
2. Splitting that table into window frames with [window definitions](#window-definitions). You can define window frames in an [`OVER` clause](/docs/v24.3/sql-grammar#over_clause), directly after the window function, or in a [`WINDOW` clause](/docs/v24.3/sql-grammar#window_clause), as a part of the selection query.
...
3. The window function `SUM(revenue) OVER (PARTITION BY city)` operates on several window frames in turn; each frame contains the `revenue` columns for a different city [partition](/docs/v24.3/partitioning) (Amsterdam, Boston, L.A., etc.).

<Frame>
  ![Window function diagram](/images/v24.3/window-functions.png)
</Frame>
...
- [Aggregate functions](/docs/v24.3/functions-and-operators#aggregate-functions)
- [Window Functions](/docs/v24.3/functions-and-operators#window-functions)
