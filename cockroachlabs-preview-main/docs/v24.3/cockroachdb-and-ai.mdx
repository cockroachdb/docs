...
- Operational workflows encoded as [Agent Skills for CockroachDB](/docs/v24.3/agent-skills).
...
- Access to the Cockroach Labs public documentation via the [CockroachDB Docs MCP server](/docs/v24.3/docs-mcp-integration).

The following sections provide an overview of these tools.

### CockroachDB Cloud MCP server

The [CockroachDB Cloud MCP server](/docs/cockroachcloud/connect-to-the-cockroachdb-cloud-mcp-server) is a managed endpoint in CockroachDB Cloud that exposes a set of tools for inspecting and querying your clusters from your AI tools. These tools let your AI assistants list [databases](/docs/v24.3/show-databases) and [tables](/docs/v24.3/show-tables), describe [schemas](/docs/v24.3/show-schemas) and [indexes](/docs/v24.3/show-index), inspect cluster health and running queries, and run read-only SQL and [`EXPLAIN`](/docs/v24.3/explain) statements. When explicitly enabled, they can also [create databases](/docs/v24.3/create-database), [create tables](/docs/v24.3/create-table), and [insert rows](/docs/v24.3/insert).
...
[Agent Skills for CockroachDB](/docs/v24.3/agent-skills) are small, structured capabilities that encode CockroachDB operational expertise in a machine-executable format. These skills live in the public [cockroachdb-skills](https://github.com/cockroachlabs/cockroachdb-skills) repository and follow the [Agent Skills Specification](https://agentskills.io/specification), with defined inputs, outputs, and safety guardrails.
...
The [CockroachDB Docs MCP server](/docs/v24.3/docs-mcp-integration) exposes the published CockroachDB documentation to your MCP-compatible tools over HTTP. After you add the server configuration to your client, your AI assistant can answer questions using the official documentation without leaving your editor. For example, you could use your AI tool to ask the following:

```text
How do table statistics get refreshed in CockroachDB?
```

An AI tool using the documentation MCP server can provide a response to this prompt informed by documentation pages such as [Cost-Based Optimizer](/docs/v24.3/cost-based-optimizer) and [`CREATE STATISTICS`](/docs/v24.3/create-statistics).
...
CockroachDB has a [`VECTOR` data type](/docs/v24.3/vector) for storing fixed-length floating-point embeddings and supports similarity operators such as L2 distance (`<->`), inner product (`<#>`), and cosine distance (`<=>`).

You can store vector embeddings, relational data, and [JSON](/docs/v24.3/jsonb) metadata in the same table and query them together. For example:
...
CockroachDB's [transactional model](/docs/v24.3/transactions) provides a foundation for storing agent state, execution history, and coordination metadata. [Serializable isolation](/docs/v24.3/demo-serializable) ensures that state transitions occur correctly even when multiple agents or processes attempt concurrent updates. The database's [high availability](/docs/v24.3/frequently-asked-questions#how-does-cockroachdb-survive-failures) design allows agents to continue operating during node or region failures without requiring external coordination services.
...
- [Multi-region deployments](/docs/v24.3/multiregion-overview) place data closer to users and can enforce [data locality](/docs/v24.3/regional-tables) or residency requirements while maintaining strong consistency.

CockroachDB provides [serializable transactions](/docs/v24.3/demo-serializable) by default, ensuring that all data operations, including vector similarity searches, maintain consistency across tables storing embeddings, relational records, and agent state.
