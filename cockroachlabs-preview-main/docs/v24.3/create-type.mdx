...
import { CreateType4 } from "/snippets/syntax-diagrams/CreateType4.jsx";

The `CREATE TYPE` [statement](/docs/v24.3/sql-statements) creates a new type in a [database](/docs/v24.3/create-database). After the type is created, it can only be referenced from the database that contains the type.
...
<Note>
The `CREATE TYPE` statement performs a schema change. For more information about how online schema changes work in CockroachDB, see [Online Schema Changes](/docs/v24.3/online-schema-changes).
</Note>
...
To see the values accepted by the underlying [`ENUM`](/docs/v24.3/enum) data type, use [`SHOW ENUMS`](/docs/v24.3/show-enums):
