...
A user-defined function (UDF) is a named function defined at the database level that can be called in queries and other contexts. CockroachDB supports invoking UDFs in `SELECT`, `FROM`, and `WHERE` clauses of [DML statements](/docs/v24.3/sql-statements#data-manipulation-statements).

Both [stored procedures](/docs/v24.3/stored-procedures) and user-defined functions are types of *routines*. However, they differ in the following ways:
...
- An argument has a *mode* and a *type*.
    - CockroachDB supports the `IN` (default), `OUT`, and `INOUT` argument modes. For an example, see [Create a function that uses `OUT` and `INOUT` parameters](/docs/v24.3/create-function#create-a-function-that-uses-out-and-inout-parameters).
    - The type can be a built-in type, [user-defined `ENUM`](/docs/v24.3/enum) or [composite](/docs/v24.3/create-type#create-a-composite-data-type) type, or implicit record type. A type can have a `DEFAULT` value.
- The return type can be a built-in [SQL type](/docs/v24.3/data-types), user-defined [`ENUM`](/docs/v24.3/enum) or [composite](/docs/v24.3/create-type#create-a-composite-data-type) type, [`RECORD`](/docs/v24.3/create-function#create-a-function-that-returns-a-record-type), PL/pgSQL [`REFCURSOR`](/docs/v24.3/plpgsql#declare-cursor-variables) type, implicit record type, [`TRIGGER`](/docs/v24.3/triggers#trigger-function), or `VOID`.
    - Preceding a type with `SETOF` indicates that a set, or multiple rows, may be returned. For an example, see [Create a function that returns a set of results](/docs/v24.3/create-function#create-a-function-that-returns-a-set-of-results).
    - `VOID` indicates that there is no return type and `NULL` will always be returned.
- The [volatility](/docs/v24.3/functions-and-operators#function-volatility) indicates whether the function has side effects. `VOLATILE` and `NOT LEAKPROOF` are the default.
    - Annotate a function with side effects with `VOLATILE`. This also prevents the [cost-based optimizer](/docs/v24.3/cost-based-optimizer) from pre-evaluating the function.
...
- `LANGUAGE` specifies the language of the function body. CockroachDB supports the languages `SQL` and [`PLpgSQL` (PL/pgSQL)](/docs/v24.3/plpgsql).
- The function body:
...
For more examples of UDF creation, see [`CREATE FUNCTION`](/docs/v24.3/create-function).
...
You invoke a UDF like a [built-in function](/docs/v24.3/functions-and-operators).
...
It uses the [PL/pgSQL](/docs/v24.3/plpgsql) [`LOOP`](/docs/v24.3/plpgsql#write-loops) syntax to iterate through a simple calculation, and [`RAISE EXCEPTION`](/docs/v24.3/plpgsql#report-messages-and-handle-exceptions) to return an error message if the specified `n` is negative.
...
When possible, the [cost-based optimizer](/docs/v24.3/cost-based-optimizer) will improve a function's performance by inlining the UDF within the query plan. The UDF must have the following attributes:

- It is labeled as `IMMUTABLE`, `STABLE`, or `LEAKPROOF` (i.e., non- `VOLATILE` ).
- It has a single statement.
- It is not a [set-returning function](/docs/v24.3/create-function#create-a-function-that-returns-a-set-of-results).
- Its arguments are only variable or constant expressions.
- It is not a [record-returning function](/docs/v24.3/create-function#create-a-function-that-returns-a-record-type).
...
5. View the query plan when using `foo_s()` (the `STABLE` function) instead:
    ...
    The query takes only `4ms` to execute because the function is inlined and transformed to a [join](/docs/v24.3/joins) with an equality comparison `(a) = (b)`, which has much less overhead than invoking a function for each row scanned in table `a`.
...
- [`ALTER FUNCTION`](/docs/v24.3/alter-function)
- [`DROP FUNCTION`](/docs/v24.3/drop-function)
