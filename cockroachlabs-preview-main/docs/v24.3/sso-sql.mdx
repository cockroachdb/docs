...
CockroachDB clusters allow users to authenticate with Single Sign-on (SSO), both to the [DB Console](/docs/v24.3/ui-overview), and for SQL client access.
...
<Note>
  Cluster single sign-on for the DB Console is supported on CockroachDB
  [Enterprise](/docs/v24.3/licensing-faqs#types-of-licenses) and Advanced clusters. CockroachDB Standard and Basic
  clusters do not support cluster single sign-on and do not have access to the DB Console. However, both CockroachDB
  Standard and CockroachDB Basic clusters can use [Cluster Single Sign-on (SSO) to authenticate to the `ccloud`
  command-line interface and to the CockroachDB Cloud Console](/docs/cockroachcloud/cloud-sso-sql).
</Note>
...
- You must have your cluster pre-configured for OIDC/SSO authentication for DB Console. Use the [Single Sign-on (SSO) for DB Console](/docs/v24.3/sso-db-console) guide to set this up.
- SQL users/credentials:
  - You must have the ability to update your cluster settings, which can be achieved in several ways. Refer to [`SET CLUSTER SETTING`: Required permissions](/docs/v24.3/set-cluster-setting#required-privileges).
  - A SQL user that corresponds with your external identity must be pre-provisioned on the cluster. To provision such users, you must have access to the [`admin` role](/docs/v24.3/security-reference/authorization#admin-role).

## Configure your cluster for SSO

### Cluster Settings

You must configure the [cluster settings](/docs/v24.3/cluster-settings) in the following table to enable JWT authentication to your cluster. Refer to the [Update your cluster settings](#update-your-cluster-settings) section to configure your cluster settings.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        <a href="/docs/v24.3/sso-db-console">Single Sign-on (SSO) for DB Console</a>.
      </td>
    </tr>
    <tr>
      <td>
...
<Note>
  [Single Sign-on (SSO) for DB Console](/docs/v24.3/sso-db-console) and cluster SSO cannot be configured with different
  IdPs.
</Note>
...
3. Add your IdP's issuer URL to your cluster's list of accepted token issuers. If you have already configured [Single Sign-on (SSO) for DB Console](/docs/v24.3/sso-db-console#cluster-settings), the issuer URL must match the value of `server.oidc_authentication.provider_url`.

   ```sql
   SET CLUSTER SETTING server.jwt_authentication.issuers.configuration = 'https://cockroachlabs.cloud';
   ```

4. Configure your cluster's audience, which is your cluster's ID (or JSON array of IDs) in your IdP. The audience is a mechanism for limiting the clients that can obtain JWTs from your IdP. If you have already configured [Single Sign-on (SSO) for DB Console](/docs/v24.3/sso-db-console#cluster-settings), the issuer URL must match the value of `server.oidc_authentication.client_id`.
...
<Tip>
  This example uses [`cockroach sql`](/docs/v24.3/cockroach-sql), but you can use any SQL client that supports
...
- Learn more about [Authentication](/docs/v24.3/authentication) in CockroachDB.
