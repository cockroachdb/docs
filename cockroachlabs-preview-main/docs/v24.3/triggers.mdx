...
A *trigger* executes a function when one or more specified SQL operations is performed on a table. The executed function is called a [*trigger function*](#trigger-function) and is written in [PL/pgSQL](/docs/v24.3/plpgsql).
...
A trigger consists of a trigger name, table name associated with the trigger, SQL operations and other conditions that activate the trigger, and a trigger function name with optional arguments. A trigger is defined with [`CREATE TRIGGER`](/docs/v24.3/create-trigger) and has the following overall structure:
...
- The [trigger function](#trigger-function), written in [PL/pgSQL](/docs/v24.3/plpgsql), is executed each time the trigger activates. A comma-separated list of constant string arguments can be included.

### Trigger conditions

A trigger activates when one or more SQL statements is issued on a table. The statement can be [`INSERT`](/docs/v24.3/insert), [`DELETE`](/docs/v24.3/delete), or [`UPDATE`](/docs/v24.3/update).
...
<Note>
`INSERT` and `UPDATE` triggers activate when [`UPSERT`](/docs/v24.3/upsert) statements insert or update rows, respectively. However, `UPSERT` cannot be specified in a [`CREATE TRIGGER`](/docs/v24.3/create-trigger) statement.

`UPDATE` triggers activate when the [`ON CONFLICT`](/docs/v24.3/insert#on-conflict-clause) clause of an `INSERT` updates rows.
</Note>
...
<Note>
Due to a [known limitation](/docs/v24.3/known-limitations#limitations-for-composite-types), `OLD` and `NEW` must be wrapped in parentheses when accessing column names.
</Note>
...
A trigger executes a [function](/docs/v24.3/user-defined-functions) called a trigger function. A trigger function is defined with [`CREATE FUNCTION`](/docs/v24.3/create-function#create-a-trigger-function) and has the following requirements:

- The function must return type `TRIGGER`.
- The function must be declared without arguments.
- The function must be written in [PL/pgSQL](/docs/v24.3/plpgsql).
...
1. Run [`cockroach demo`](/docs/v24.3/cockroach-demo) to start a temporary, in-memory cluster with the [`movr`](/docs/v24.3/movr) sample dataset preloaded:
...
2. Create a table that stores audit records. Each record includes the table that was affected, the SQL operation that was performed on the table, the old and new table rows, and the timestamp when the change was made:
    ...
    ```
3. Create a [trigger function](/docs/v24.3/create-function#create-a-trigger-function) that inserts the corresponding values into the `audit_log` table:
...
2. Create a `product_sales_summary` table that stores summary records. Each record includes the total number of orders and the total value of sales for each product:
    ...
    ```
3. Create a [trigger function](/docs/v24.3/create-function#create-a-trigger-function) that updates existing summary records, or inserts a new summary record, to reflect each order that is placed:
...
1. Create a sample table of employees and their wages:
    ...
    ```
2. Create a [trigger function](/docs/v24.3/create-function#create-a-trigger-function) that checks whether a new wage is below the minimum:
    ...
    The function prints the wage that is initially assigned to the employee. If the new wage is below minimum, the function [raises an exception](/docs/v24.3/plpgsql#report-messages-and-handle-exceptions) to abort the SQL operation that changes the wage. Otherwise, it returns the `NEW` row resulting from the SQL operation.
...
- [`CREATE TRIGGER`](/docs/v24.3/create-trigger)
- [`DROP TRIGGER`](/docs/v24.3/drop-trigger)
- [User-defined functions](/docs/v24.3/user-defined-functions)
- [`CREATE FUNCTION`](/docs/v24.3/create-function)
