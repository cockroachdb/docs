...
1. [`CSV`](/docs/v24.3/migrate-from-csv) or [`DELIMITED DATA`](/docs/v24.3/import-into) (both have about the same import performance).
2. [`AVRO`](/docs/v24.3/migrate-from-avro).

We recommend formatting your import files as `CSV` or `AVRO`. These formats can be processed in parallel by all the nodes in the cluster, which increases performance. To import in these formats, use [`IMPORT INTO`](/docs/v24.3/import-into).
...
When importing data into a table with [foreign keys](/docs/v24.3/foreign-key), temporarily remove the foreign keys, and add them after the initial migration with an [`ALTER TABLE` statement](/docs/v24.3/alter-table#add-the-foreign-key-constraint-with-cascade).

### Data type sizes

Above a certain size, many data types such as [`STRING`](/docs/v24.3/string)s, [`DECIMAL`](/docs/v24.3/decimal)s, [`ARRAY`](/docs/v24.3/array), [`BYTES`](/docs/v24.3/bytes), and [`JSONB`](/docs/v24.3/jsonb) may run into performance issues due to [write amplification](/docs/v24.3/architecture/storage-layer#write-amplification). See each data type's documentation for its recommended size limits.
...
For example, if you have a 3-node cluster, split your data into at least 3 files, create your table schema, and [import into that table](/docs/v24.3/import-into):
...
You should split the files so that the first file contains rows where the [primary key](/docs/v24.3/primary-key) begins with the letters "a" to "i," the second file "j" to "r," and the third file "s" to "z". No file should contain duplicate rows, and within each file the data is sorted alphabetically by the primary key.
...
During migration, all of the features of [`IMPORT INTO`](/docs/v24.3/import-into) that interact with external file storage assume that every node has the exact same view of that storage. In other words, in order to import from a file, every node needs to have the same access to that file.
...
- [Migrate from Oracle](/docs/v24.3/migrate-from-oracle)
...
- [Migrate from Avro](/docs/v24.3/migrate-from-avro)
