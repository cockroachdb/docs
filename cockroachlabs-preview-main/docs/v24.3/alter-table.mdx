...
import { AlterTable7 } from "/snippets/syntax-diagrams/AlterTable7.jsx";
import { AlterTable8 } from "/snippets/syntax-diagrams/AlterTable8.jsx";

The `ALTER TABLE` [statement](/docs/v24.3/sql-statements) changes the definition of a table. For information on using `ALTER TABLE`, see the pages for its [subcommands](#subcommands).
...
- No primary key was explicitly defined at [table creation](/docs/v24.3/create-table). In this case, the table is created with a default [primary key on `rowid`](/docs/v24.3/indexes#creation). Using `ADD CONSTRAINT... PRIMARY KEY` drops the default primary key and replaces it with a new primary key.
- A [`DROP CONSTRAINT`](/docs/v24.3/alter-table#drop-constraint) statement precedes the `ADD CONSTRAINT... PRIMARY KEY` statement, in the same transaction. For an example, see [Drop and add the primary key constraint](#drop-and-add-a-primary-key-constraint).
...
- Set, change, or drop a column's [`DEFAULT` constraint](/docs/v24.3/default-value).
- Set or drop a column's [`NOT NULL` constraint](/docs/v24.3/not-null).
- Set, change, or drop an [`ON UPDATE` expression](/docs/v24.3/create-table#on-update-expressions).
- Change a column's [data type](/docs/v24.3/data-types).
- Set the [visibility](#set-the-visibility-of-a-column) of a column.

<Note>
Support for altering column data types is [in preview](/docs/v24.3/cockroachdb-feature-availability), with certain limitations. For details, see [Altering column data types](#alter-column-data-types).
</Note>
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>column&#95;name</code></td><td>The name of the column to modify.</td></tr><tr><td><code>a&#95;expr</code></td><td>The new <a href="/docs/v24.3/default-value">default value</a> to set.</td></tr><tr><td><code>b&#95;expr</code></td><td>The <a href="/docs/v24.3/create-table#on-update-expressions"><code>ON UPDATE</code> expression</a> to set.</td></tr><tr><td><code>&#91;NOT&#93; VISIBLE</code></td><td>The visibility of a column when using <code>&#42;</code> in a <a href="/docs/v24.3/select-clause"><code>SELECT</code> clause</a>.</td></tr><tr><td><code>typename</code></td><td>The new <a href="/docs/v24.3/data-types">data type</a> you want to use.<br/>Support for altering column types is <a href="/docs/v24.3/cockroachdb-feature-availability">in preview</a>, with certain limitations. For details, see <a href="#alter-column-data-types">Alter column data types</a>.</td></tr><tr><td><code>USING a&#95;expr</code></td><td>How to compute a new column value from the old column value.</td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

#### Alter column data types

Support for altering column data types is [in preview](/docs/v24.3/cockroachdb-feature-availability), with certain limitations. To enable column type altering, set the `enable_experimental_alter_column_type_general` [session variable](/docs/v24.3/set-vars) to `true`.
...
Use `ALTER TABLE... ALTER PRIMARY KEY` to change the [primary key](/docs/v24.3/primary-key) of a table.

Note the following:

- You cannot change the primary key of a table that is currently undergoing a primary key change, or any other [schema change](/docs/v24.3/online-schema-changes).
- `ALTER PRIMARY KEY` might need to rewrite multiple indexes, which can make it an expensive operation.
- When you change a primary key with `ALTER PRIMARY KEY`, the old primary key index becomes a [`UNIQUE`](/docs/v24.3/unique) secondary index. This helps optimize the performance of queries that still filter on the old primary key column.
- `ALTER PRIMARY KEY` does not alter the [partitions](/docs/v24.3/partitioning) on a table or its indexes, even if a partition is defined on [a column in the original primary key](/docs/v24.3/partitioning#partition-using-primary-key). If you alter the primary key of a partitioned table, you must update the table partition accordingly.
- The secondary index created by `ALTER PRIMARY KEY` will not be partitioned, even if a partition is defined on [a column in the original primary key](/docs/v24.3/partitioning#partition-using-primary-key). To ensure that the table is partitioned correctly, you must create a partition on the secondary index, or drop the secondary index.
- Any new primary key column set by `ALTER PRIMARY KEY` must have an existing [`NOT NULL` constraint](/docs/v24.3/not-null). To add a `NOT NULL` constraint to an existing column, use [`ALTER TABLE... ALTER COLUMN... SET NOT NULL`](#set-not-null-constraint).
...
The user must have the `CREATE` [privilege](/docs/v24.3/security-reference/authorization#managing-privileges) on a table to alter its primary key.

#### Parameters

<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>index&#95;params</code></td><td>The name of the column(s) that you want to use for the primary key. These columns replace the current primary key column(s).</td></tr><tr><td><code>USING HASH</code></td><td>Creates a <a href="/docs/v24.3/hash-sharded-indexes">hash-sharded index</a>.</td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

### `CONFIGURE ZONE`

`ALTER TABLE... CONFIGURE ZONE` is used to add, modify, reset, or remove replication zones for a table. To view details about existing replication zones, use [`SHOW ZONE CONFIGURATIONS`](/docs/v24.3/show-zone-configurations). For more information about replication zones, see [Replication Controls](/docs/v24.3/configure-replication-zones).
...
By default, `DROP COLUMN` drops any [indexes](/docs/v24.3/indexes) on the column being dropped, and any indexes that reference the column, including indexes with [`STORING` clauses](/docs/v24.3/create-index#store-columns) that reference the column.
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>column&#95;name</code></td><td>The name of the column you want to drop.<br/><br/>When a column with a <a href="/docs/v24.3/check"><code>CHECK</code></a> constraint is dropped, the <code>CHECK</code> constraint is also dropped.</td></tr><tr><td><code>CASCADE</code></td><td>Drop the column even if objects (such as <a href="/docs/v24.3/views">views</a>) depend on it; drop the dependent objects, as well. <code>CASCADE</code> will drop a column with a foreign key constraint if it is the only column in the reference.<br/><br/><code>CASCADE</code> does not list the objects it drops, so should be used cautiously.<br/><br/><code>CASCADE</code> is not required to drop an indexed column, or a column that is referenced by an index. By default, <code>DROP COLUMN</code> drops any <a href="/docs/v24.3/indexes">indexes</a> on the column being dropped, and any indexes that reference the column, including <a href="/docs/v24.3/partial-indexes">partial indexes</a> with predicates that reference the column and indexes with <a href="/docs/v24.3/create-index#store-columns"><code>STORING</code> clauses</a> that reference the column.</td></tr><tr><td><code>RESTRICT</code></td><td><em>(Default)</em> Do not drop the column if any objects (such as <a href="/docs/v24.3/views">views</a>) depend on it.</td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

### `DROP CONSTRAINT`

Use `ALTER TABLE... DROP CONSTRAINT` to remove [`CHECK`](/docs/v24.3/check) and [`FOREIGN KEY`](/docs/v24.3/foreign-key) constraints from columns.

[`PRIMARY KEY`](/docs/v24.3/primary-key) constraints can be dropped with `DROP CONSTRAINT` if an [`ADD CONSTRAINT`](/docs/v24.3/alter-table#add-constraint) statement follows the `DROP CONSTRAINT` statement in the same transaction.

<Tip>
When you change a primary key with [`ALTER TABLE... ALTER PRIMARY KEY`](/docs/v24.3/alter-table#alter-primary-key), the old primary key index becomes a secondary index. If you do not want the old primary key to become a secondary index, use `DROP CONSTRAINT`/[`ADD CONSTRAINT`](/docs/v24.3/alter-table#add-constraint) to change the primary key.
</Tip>

<Note>
For information about removing other constraints, see [Constraints: Remove Constraints](/docs/v24.3/constraints#remove-constraints).
</Note>
...
`ALTER TABLE... EXPERIMENTAL_AUDIT` enables or disables the recording of SQL audit events to the [`SENSITIVE_ACCESS`](/docs/v24.3/logging#sensitive_access) logging channel for a table. The `SENSITIVE_ACCESS` log output is also called the *SQL audit log*. For details on using SQL audit logs, see [SQL Audit Logging](/docs/v24.3/sql-audit-logging).
...
CockroachDB stores audit log information in a way that ensures durability, but negatively impacts performance. As a result, we recommend using SQL audit logs for security purposes only. For more information, see [Performance considerations](/docs/v24.3/sql-audit-logging#performance-considerations).
...
To change the owner of a table, the user must be an `admin` user, or the current owner of the table and a member of the new owner [role](/docs/v24.3/security-reference/authorization#roles). The new owner role must also have the `CREATE` [privilege](/docs/v24.3/security-reference/authorization#managing-privileges) on the schema to which the table belongs.
...
<Note>
It is not possible to rename a column referenced by a view. For more details, see [View Dependencies](/docs/v24.3/views#view-dependencies).
</Note>
...
<Note>
It is not possible to rename a constraint for a column referenced by a view. For more details, see [View Dependencies](/docs/v24.3/views#view-dependencies).
</Note>
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>IF EXISTS</code></td><td>Rename the constraint only if a constraint of <code>current&#95;name</code> exists; if one does not exist, do not return an error.</td></tr><tr><td><code>constraint&#95;name</code></td><td>The current name of the constraint.</td></tr><tr><td><code>constraint&#95;new&#95;name</code></td><td>The new <a href="/docs/v24.3/sql-grammar#name"><code>name</code></a> you want to use for the constraint, which must be unique to its table and follow these <a href="/docs/v24.3/keywords-and-identifiers#identifiers">identifier rules</a>.</td></tr></tbody></table>
...
`ALTER TABLE... RENAME TO` cannot be used to move a table from one database to another. To change a table's database, use [`BACKUP`](/docs/v24.3/backup#back-up-a-table-or-view) and [`RESTORE`](/docs/v24.3/restore#restore-a-table).
</Note>

<Note>
It is not possible to rename a table referenced by a view. For more details, see [View Dependencies](/docs/v24.3/views#view-dependencies).
</Note>
...
The user must have the `DROP` [privilege](/docs/v24.3/security-reference/authorization#managing-privileges) on the table and the `CREATE` on the parent database. When moving a table from one database to another, the user must have the `CREATE` privilege on both the source and target databases.

#### Parameters

<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>table&#95;new&#95;name</code></td><td>The new name of the table, which must be unique within its database and follow these <a href="/docs/v24.3/keywords-and-identifiers#identifiers">identifier rules</a>. When the parent database is not set as the default, the name must be formatted as <code>database.name</code>.<br/><br/>The <a href="/docs/v24.3/upsert"><code>UPSERT</code></a> and <a href="/docs/v24.3/insert"><code>INSERT ON CONFLICT</code></a> statements use a temporary table called <code>excluded</code> to handle uniqueness conflicts during execution. It's therefore not recommended to use the name <code>excluded</code> for any of your tables.</td></tr></tbody></table>
...
<Note>
To reset a storage parameter on an existing index, you must drop and [recreate the index without the storage parameter](/docs/v24.3/with-storage-parameter).
</Note>
...
The user must be a member of the [`admin`](/docs/v24.3/security-reference/authorization#roles) or [owner](/docs/v24.3/security-reference/authorization#object-ownership) roles, or have the [`CREATE` privilege](/docs/v24.3/security-reference/authorization#supported-privileges) on the table.
...
<Note>
To set a storage parameter on an existing index, you must drop and [recreate the index with the storage parameter](/docs/v24.3/with-storage-parameter).
</Note>
...
<table><thead><tr><th>Parameter name</th><th>Description</th><th>Data type</th><th>Default value</th></tr></thead><tbody><tr><td><code>exclude&#95;data&#95;from&#95;backup</code></td><td>Exclude the data in this table from any future backups.</td><td>Boolean</td><td><code>false</code></td></tr><tr><td><code>schema&#95;locked</code></td><td>Disallow <a href="/docs/v24.3/online-schema-changes">schema changes</a> on this table. Enabling <code>schema&#95;locked</code> can help <a href="/docs/v24.3/create-changefeed#disallow-schema-changes-on-tables-to-improve-changefeed-performance">improve performance of changefeeds</a> running on this table.</td><td>Boolean</td><td><code>false</code></td></tr><tr><td><code>sql&#95;stats&#95;automatic&#95;collection&#95;enabled</code></td><td>Enable automatic collection of <a href="/docs/v24.3/cost-based-optimizer#full-statistics">full statistics</a> for this table.</td><td>Boolean</td><td><code>true</code></td></tr><tr><td><code>sql&#95;stats&#95;automatic&#95;collection&#95;min&#95;stale&#95;rows</code></td><td>Minimum number of stale rows in this table that will trigger a statistics refresh.</td><td>Integer</td><td>500</td></tr><tr><td><code>sql&#95;stats&#95;automatic&#95;collection&#95;fraction&#95;stale&#95;rows</code></td><td>Fraction of stale rows in this table that will trigger a statistics refresh.</td><td>Float</td><td>0.2</td></tr><tr><td><code>sql&#95;stats&#95;forecasts&#95;enabled</code></td><td>Enable <a href="/docs/v24.3/show-statistics#display-forecasted-statistics">forecasted statistics</a> collection for this table.</td><td>Boolean</td><td><code>true</code></td></tr></tbody></table>
...
For the list of storage parameters that affect how [Row-Level TTL](/docs/v24.3/row-level-ttl) works, see the list of [TTL storage parameters](/docs/v24.3/row-level-ttl#ttl-storage-parameters).

### `SET LOCALITY`

`ALTER TABLE.. SET LOCALITY` changes the [table locality](/docs/v24.3/multiregion-overview#table-locality) of a [table](/docs/v24.3/create-table) in a [multi-region database](/docs/v24.3/multiregion-overview).

While CockroachDB is processing an `ALTER TABLE.. SET LOCALITY` statement that enables or disables `REGIONAL BY ROW` on a table within a database, any [`ADD REGION`](/docs/v24.3/alter-database#add-region) and [`DROP REGION`](/docs/v24.3/alter-database#drop-region) statements on that database will fail.
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>locality</code></td><td>The <code>LOCALITY</code> clause, followed by the <a href="/docs/v24.3/multiregion-overview#table-locality">locality</a> to apply to this table. Allowed values:<br/><br/>- <a href="#regional-by-table"><code>REGIONAL BY TABLE</code></a> (default)<br/><br/>- <a href="#regional-by-row"><code>REGIONAL BY ROW</code></a><br/><br/>- <a href="#global"><code>GLOBAL</code></a></td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

For more information about which table locality is right for your use case, see [Table localities](/docs/v24.3/multiregion-overview#table-locality).

### `SET SCHEMA`

`ALTER TABLE... SET SCHEMA` changes the [schema](/docs/v24.3/sql-name-resolution) of a table.

<Note>
CockroachDB supports `SET SCHEMA` as an [alias for setting the `search_path` session variable](/docs/v24.3/set-vars#supported-variables).
</Note>
...
The user must have the `DROP` [privilege](/docs/v24.3/security-reference/authorization#managing-privileges) on the table, and the `CREATE` privilege on the schema.
...
`ALTER TABLE... SCATTER` runs a specified set of ranges for a table or index through the [replication layer](/docs/v24.3/architecture/replication-layer) queue. If many ranges have been created recently, the replication queue may transfer some leases to other replicas to balance load across the cluster.
...
`ALTER TABLE... SPLIT AT` forces a [range split](/docs/v24.3/architecture/distribution-layer#range-splits) at a specified row in the table.
...
The user must have the `INSERT` [privilege](/docs/v24.3/security-reference/authorization#managing-privileges) on the table.

#### Parameters

<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>select&#95;stmt</code></td><td>A <a href="/docs/v24.3/selection-queries">selection query</a> that produces one or more rows at which to split the table.</td></tr><tr><td><code>a&#95;expr</code></td><td>The expiration of the split enforcement on the table. This can be a <a href="/docs/v24.3/decimal"><code>DECIMAL</code></a>, <a href="/docs/v24.3/interval"><code>INTERVAL</code></a>, <a href="/docs/v24.3/timestamp"><code>TIMESTAMP</code></a>, or <a href="/docs/v24.3/timestamp"><code>TIMESTAMPZ</code></a>.</td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

### `UNSPLIT AT`

`ALTER TABLE... UNSPLIT AT` removes a [split enforcement](#split-at) on a [range split](/docs/v24.3/architecture/distribution-layer#range-splits), at a specified row in the table.
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>select&#95;stmt</code></td><td>A <a href="/docs/v24.3/selection-queries">selection query</a> that produces one or more rows at which to unsplit a table.</td></tr><tr><td><code>ALL</code></td><td>Remove all split enforcements for a table.</td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

### `VALIDATE CONSTRAINT`

`ALTER TABLE... VALIDATE CONSTRAINT` checks whether values in a column match a [constraint](/docs/v24.3/constraints) on the column.
...
The following examples use the [`bank` demo database schema](/docs/v24.3/cockroach-demo#datasets).

To follow along, run [`cockroach demo bank`](/docs/v24.3/cockroach-demo) to start a temporary, in-memory cluster with the `bank` schema and dataset preloaded:
...
Moving frequently updated columns to their own [column family can increase performance](/docs/v24.3/column-families#default-behavior).

To move a column from one column family to another column family, create a temporary, [non-visible](/docs/v24.3/create-table#not-visible-property) [stored computed column](/docs/v24.3/computed-columns) in the target column family, then rename the columns. Once this succeeds, you can drop the original, now renamed column.
...
3. Drop the old column:
    ...
    <Note>
    You must set the [`sql_safe_updates` session variable](/docs/v24.3/set-vars#sql-safe-updates) to `false` to drop a column in a table that has data.
    </Note>

Moving a column to another column family executes writes to the underlying storage equal to two times the number of rows. For example, if the table has 10 million rows, there will be 20 million writes to the [storage layer](/docs/v24.3/architecture/storage-layer): 10 million writes when creating the temporary stored computed column, and 10 million writes when removing the original column.
...
Adding the [`UNIQUE` constraint](/docs/v24.3/unique) requires that all of a column's values be distinct from one another (except for `NULL` values).
...
Adding the [`CHECK` constraint](/docs/v24.3/check) requires that all of a column's values evaluate to `TRUE` for a Boolean expression.
...
- If a new column has a default value or is a [computed column](/docs/v24.3/computed-columns) that would have contained values that violate the new constraint.
</Note>
...
You can include a [foreign key action](/docs/v24.3/foreign-key#foreign-key-actions) to specify what happens when a foreign key is updated or deleted.
...
<Note>
By default, referenced columns must be in the same database as the referencing foreign key column. To enable cross-database foreign key references, set the `sql.cross_db_fks.enabled` [cluster setting](/docs/v24.3/cluster-settings) to `true`.
</Note>
...
1. View the details of the `promo_codes` table:
    ...
    (1 row)
    ```
2. Add a [`NOT NULL`](/docs/v24.3/not-null) constraint to the `creation_time` column with [`ALTER COLUMN`](#alter-column):
...
3. To issue the schema change atomically, use single statements as an implicit transaction. `DROP CONSTRAINT` and `ADD CONSTRAINT` can be combined in a single `ALTER TABLE` statement:
    ...
    <Note>
    You should **not** execute the schema change with multiple statements within an explicit transaction. Refer to [Schema changes within transactions](/docs/v24.3/online-schema-changes#schema-changes-within-transactions).
    </Note>
...
Using [`ALTER PRIMARY KEY`](/docs/v24.3/alter-table#alter-primary-key) would have created a `UNIQUE` secondary index called `promo_codes_code_key`. Instead, there is just one index for the primary key constraint.

#### Add a unique index to a `REGIONAL BY ROW` table

In [multi-region deployments](/docs/v24.3/multiregion-overview), most users should use [`REGIONAL BY ROW` tables](/docs/v24.3/table-localities#regional-by-row-tables) instead of explicit index [partitioning](/docs/v24.3/partitioning). When you add an index to a `REGIONAL BY ROW` table, it is automatically partitioned on the [`crdb_region` column](/docs/v24.3/alter-table#crdb_region). Explicit index partitioning is not required.

While CockroachDB process an [`ADD REGION`](/docs/v24.3/alter-database#add-region) or [`DROP REGION`](/docs/v24.3/alter-database#drop-region) statement on a particular database, creating or modifying an index will throw an error. Similarly, all [`ADD REGION`](/docs/v24.3/alter-database#add-region) and [`DROP REGION`](/docs/v24.3/alter-database#drop-region) statements will be blocked while an index is being modified on a `REGIONAL BY ROW` table within the same database.

This example assumes you have a simulated multi-region database running on your local machine following the steps described in [Low Latency Reads and Writes in a Multi-Region Cluster](/docs/v24.3/demo-low-latency-multi-region-deployment). It shows how a `UNIQUE` index is partitioned, but it's similar to how all indexes are partitioned on `REGIONAL BY ROW` tables.

To show how the automatic partitioning of indexes on `REGIONAL BY ROW` tables works, we will:

1. [Add a column](/docs/v24.3/alter-table#add-column) to the `users` table in the [MovR dataset](/docs/v24.3/movr).
2. Add a [`UNIQUE` constraint](/docs/v24.3/unique) to that column.
3. Verify that the index is automatically partitioned for better multi-region performance by using [`SHOW INDEXES`](/docs/v24.3/show-index) and [`SHOW PARTITIONS`](/docs/v24.3/show-partitions).
...
Next, issue the [`SHOW INDEXES`](/docs/v24.3/show-index) statement. You will see that [the implicit region column](#set-the-table-locality-to-regional-by-row) that was added when the table [was converted to regional by row](/docs/v24.3/demo-low-latency-multi-region-deployment#configure-regional-by-row-tables) is now indexed:
...
Next, issue the [`SHOW PARTITIONS`](/docs/v24.3/show-partitions) statement. The following output (which is edited for length) will verify that the unique index was automatically [partitioned](/docs/v24.3/partitioning) for you. It shows that the `user_email_unique` index is now partitioned by the database regions `europe-west1`, `us-east1`, and `us-west1`.
...
2. Thereafter, the [optimizer](/docs/v24.3/cost-based-optimizer) will automatically add a "uniqueness check" when necessary to any [`INSERT`](/docs/v24.3/insert), [`UPDATE`](/docs/v24.3/update), or [`UPSERT`](/docs/v24.3/upsert) statement affecting the columns in the unique constraint.

Note that the [SQL engine](/docs/v24.3/architecture/sql-layer) will avoid sending requests to nodes in other regions when it can instead read a value from a unique column that is stored locally. This capability is known as [*locality optimized search*](/docs/v24.3/cost-based-optimizer#locality-optimized-search-in-multi-region-clusters).

#### Using `DEFAULT gen_random_uuid()` in `REGIONAL BY ROW` tables

To auto-generate unique row identifiers in `REGIONAL BY ROW` tables, use the [`UUID`](/docs/v24.3/uuid) column with the `gen_random_uuid()` [function](/docs/v24.3/functions-and-operators#id-generation-functions) as the [default value](/docs/v24.3/default-value):
...
<Note>
When using `DEFAULT gen_random_uuid()` on columns in `REGIONAL BY ROW` tables, uniqueness checks on those columns are disabled by default for performance purposes. CockroachDB assumes uniqueness based on the way this column generates [`UUIDs`](/docs/v24.3/uuid#create-a-table-with-auto-generated-unique-row-ids). To enable this check, you can modify the `sql.optimizer.uniqueness_checks_for_gen_random_uuid.enabled` [cluster setting](/docs/v24.3/cluster-settings). Note that while there is virtually no chance of a [collision](https://wikipedia.org/wiki/Universally_unique_identifier#Collisions) occurring when enabling this setting, it is not truly zero.
</Note>

#### Using implicit vs. explicit index partitioning in `REGIONAL BY ROW` tables

In `REGIONAL BY ROW` tables, all indexes are partitioned on the region column (usually called [`crdb_region`](/docs/v24.3/alter-table#crdb_region)).

These indexes can either include or exclude the partitioning key (`crdb_region`) as the first column in the index definition:

- If `crdb_region` is included in the index definition, a [`UNIQUE` index](/docs/v24.3/unique) will enforce uniqueness on the set of columns, just like it would in a non-partitioned table.
...
In the latter case, the index alone cannot enforce uniqueness on columns that are not a prefix of the index columns, so any time rows are [inserted](/docs/v24.3/insert) or [updated](/docs/v24.3/update) in a `REGIONAL BY ROW` table that has an implicitly partitioned `UNIQUE` index, the [optimizer](/docs/v24.3/cost-based-optimizer) must add uniqueness checks.

Whether or not to explicitly include `crdb_region` in the index definition depends on the context:

- If you only need to enforce uniqueness at the region level, then including `crdb_region` in the `UNIQUE` index definition will enforce these semantics and allow you to get better performance on [`INSERT`](/docs/v24.3/insert) s, [`UPDATE`](/docs/v24.3/update) s, and [`UPSERT`](/docs/v24.3/upsert) s, since there will not be any added latency from uniqueness checks.
- If you need to enforce global uniqueness, you should not include `crdb_region` in the `UNIQUE` (or [`PRIMARY KEY`](/docs/v24.3/primary-key) ) index definition, and the database will automatically ensure that the constraint is enforced.
...
1. Start [`cockroach demo`](/docs/v24.3/cockroach-demo) as follows:
...
3. In the following statement, we add a new user with the required `id`, `email`, and `desk_id` columns. CockroachDB needs to do additional work to enforce global uniqueness for the `id` and `email` columns, which are implicitly partitioned. This additional work is in the form of "uniqueness checks" that the optimizer adds as part of mutation queries.
    ...
    The following `EXPLAIN` output shows that the optimizer has added two `constraint-check` post queries to check the uniqueness of the implicitly partitioned indexes `id` and `email`. There is no check needed for `desk_id` (really `(crdb_region, desk_id)`), since that constraint is automatically enforced by the explicitly partitioned index we added in the preceding [`CREATE TABLE`](/docs/v24.3/create-table) statement.
...
Setting the [`DEFAULT` value constraint](/docs/v24.3/default-value) inserts the value when data's written to the table without explicitly defining the value for the column. If the column already has a `DEFAULT` value set, you can use this statement to change it.
...
If the column has a defined [`DEFAULT` value](/docs/v24.3/default-value), you can remove the constraint, which means the column will no longer insert a value by default if one is not explicitly defined for the column.
...
To specify that the column cannot contain `NULL` values, set the [`NOT NULL` constraint](/docs/v24.3/not-null).
...
If the column has the [`NOT NULL` constraint](/docs/v24.3/not-null) applied to it, you can remove the constraint, which means the column becomes optional and can have `NULL` values written into it.
...
The `full_name` column is computed from the `first_name` and `last_name` columns without the need to define a [view](/docs/v24.3/views). You can view the column details with the [`SHOW COLUMNS`](/docs/v24.3/show-columns) statement:
...
To alter the formula for a computed column, you must [`DROP`](/docs/v24.3/alter-table#drop-column) and [`ADD`](/docs/v24.3/alter-table#add-column) the column back with the new definition. Take the following table for instance:
...
The [TPC-C](/docs/v24.3/performance-benchmarking-with-tpcc-small) database has a `customer` table with a column `c_credit_lim` of type `DECIMAL(10,2)`:
...
1. Set the `enable_experimental_alter_column_type_general` [session variable](/docs/v24.3/set-vars) to `true`:
...
The [TPC-C](/docs/v24.3/performance-benchmarking-with-tpcc-small) `customer` table contains a column `c_balance` of type `DECIMAL(12,2)`:
...
To specify that a column won't be returned when using `*` in a [`SELECT` clause](/docs/v24.3/select-clause), set the `NOT VISIBLE` property. You can set the `NOT VISIBLE` property only on individual columns.

For example, the `users` table of the [`movr` database](/docs/v24.3/movr) contains the `credit_card` column. If you don't want users to see that column when running `SELECT * FROM users;`, you can hide it as follows:
...
The primary key of this table is on the `name` column. This is a poor choice, as some users likely have the same name, and all primary keys enforce a `UNIQUE` constraint on row values of the primary key column. Per our [best practices](/docs/v24.3/performance-best-practices-overview#use-functions-to-generate-unique-ids), you should instead use a [`UUID`](/docs/v24.3/uuid) for single-column primary keys, and populate the rows of the table with generated, unique values.
...
If there is data in the table, the `sql_safe_updates` [session variable](/docs/v24.3/set-vars) must be set to `false`.
...
If the column has dependent objects, such as [views](/docs/v24.3/views), CockroachDB will not drop the column by default. However, if you want to be sure of the behavior you can include the `RESTRICT` clause.
...
Now, every access of customer data is logged to the `SENSITIVE_ACCESS` channel in a [`sensitive_table_access`](/docs/v24.3/eventlog#sensitive_table_access) event that looks like the following:
...
<Note>
The preceding example shows the default [`crdb-v2`](/docs/v24.3/log-formats#format-crdb-v2) log format. This can be changed to a different format (e.g., JSON). For details, see [Configure Logs](/docs/v24.3/configure-logs#file-logging-format).
</Note>

<Tip>
For descriptions of all SQL audit event types and their fields, see [Notable Event Types](/docs/v24.3/eventlog#sql-access-audit-events).
</Tip>

To turn on auditing for more than one table, issue a separate `ALTER` statement for each table.

<Tip>
For a more detailed example, see [SQL Audit Logging](/docs/v24.3/sql-audit-logging).
</Tip>
...
<Note>
If the user running the command is not an admin user, they must own the table and be a member of the new owning role. Also, the new owner role must also have the `CREATE` [privilege](/docs/v24.3/security-reference/authorization#managing-privileges) on the schema to which the table belongs.
</Note>
...
Some subcommands can be used in combination in a single [`ALTER TABLE`](/docs/v24.3/alter-table) statement. For example, let's say you create a `users` table with 2 columns, an `id` column for the primary key and a `name` column for each user's last name:
...
Then you decide you want distinct columns for each user's first name, last name, and full name, so you execute a single `ALTER TABLE` statement renaming `name` to `last_name`, adding `first_name`, and adding a [computed column](/docs/v24.3/computed-columns) called `name` that concatenates `first_name` and `last_name`:
...
In some situations, you may want to exclude a table's row data from a [backup](/docs/v24.3/backup). For example, you have a table that contains high-churn data that you would like to [garbage collect](/docs/v24.3/architecture/storage-layer#garbage-collection) more quickly than the [incremental backup](/docs/v24.3/take-full-and-incremental-backups#incremental-backups) schedule for the database or cluster holding the table. You can use the `exclude_data_from_backup = true` parameter with a [`CREATE TABLE`](/docs/v24.3/create-table#create-a-table-with-data-excluded-from-backup) or `ALTER TABLE` statement to mark a table's row data for exclusion from a backup.

For more detail and an example through the backup and [restore](/docs/v24.3/restore) process using this parameter, see [Take Full and Incremental Backups](/docs/v24.3/take-full-and-incremental-backups#exclude-a-tables-data-from-backups).
...
<Note>
[`RESTORE`](/docs/v24.3/restore) on [`REGIONAL BY TABLE`](#regional-by-table), [`REGIONAL BY ROW`](#regional-by-row), and [`GLOBAL`](#global) tables is supported with some limitations — see [Restoring to multi-region databases](/docs/v24.3/restore#restoring-to-multi-region-databases) for more detail.
</Note>
...
For more information about how this table locality works, see [Regional tables](/docs/v24.3/table-localities#regional-tables).

#### Set the table locality to `REGIONAL BY ROW`

<Note>
Before setting the locality to `REGIONAL BY ROW` on a table targeted by a changefeed, read the considerations in [Changefeeds on regional by row tables](/docs/v24.3/changefeeds-in-multi-region-deployments).
</Note>
...
Every row in a regional by row table has a column of type `crdb_internal_region` that represents the row's [home region](/docs/v24.3/multiregion-overview#table-localities). By default, this column is called `crdb_region` and is hidden. To see a row's home region, issue a statement like the following:

```sql
SELECT crdb_region, id FROM {table};
```

To update an existing row's home region, use an [`UPDATE`](/docs/v24.3/update) statement like the following:
...
- Let CockroachDB set the row's home region automatically. It will use the region of the [gateway node](/docs/v24.3/architecture/life-of-a-distributed-transaction#gateway) from which the row is inserted.
- Set the home region explicitly using an [`INSERT`](/docs/v24.3/insert) statement like the following:
...
For more information about how this table locality works, see [Regional by row tables](/docs/v24.3/table-localities#regional-by-row-tables).
...
In fact, you can specify any column definition you like for the `REGIONAL BY ROW AS` column, as long as the column is of type `crdb_internal_region` and is not nullable. For example, you could modify the [movr schema](/docs/v24.3/movr#the-movr-database) to have a region column generated as:
...
When auto-rehoming is enabled, the [home regions](#crdb_region) of rows in [`REGIONAL BY ROW`](#set-the-table-locality-to-regional-by-row) tables are automatically set to the gateway region of any [`UPDATE`](/docs/v24.3/update) or [`UPSERT`](/docs/v24.3/upsert) statements that write to those rows. This functionality is provided by adding an [`ON UPDATE` expression](/docs/v24.3/create-table#on-update-expressions) to the [`crdb_region`](#crdb_region) column of newly created regional by row tables.

To enable auto-rehoming using the [session setting](/docs/v24.3/set-vars), issue the following statement:
...
- The [`crdb_region`](#crdb_region) column from a [`REGIONAL BY ROW`](#set-the-table-locality-to-regional-by-row) table cannot be referenced as a [foreign key](/docs/v24.3/foreign-key) from another table.
...
1. Follow steps 1 and 2 from the [Low Latency Reads and Writes in a Multi-Region Cluster](/docs/v24.3/demo-low-latency-multi-region-deployment) tutorial. This will involve starting a [`cockroach demo`](/docs/v24.3/cockroach-demo) cluster in a terminal window (call it *terminal 1*).
2. From the [SQL client](/docs/v24.3/cockroach-sql) running in terminal 1, set the setting that enables auto-rehoming. You must issue this setting before creating the `REGIONAL BY ROW` tables that you want auto-rehomed.

    ```sql
    SET enable_auto_rehoming = on;
    ```
3. In a second terminal window (call it *terminal 2*), [finish the tutorial starting from step 3](/docs/v24.3/demo-low-latency-multi-region-deployment#step-3-load-and-run-movr) onward to finish loading the cluster with data and applying the multi-region SQL configuration.
4. Switch back to terminal 1, and check the gateway region of the node you are currently connected to:
    ...
    (1 row)
    ```
5. Open another terminal (call it *terminal 3*), and use [`cockroach sql`](/docs/v24.3/cockroach-sql) to connect to a node in a different region in the demo cluster:
    ...
    # To exit, type: \q.
    #
    # Server version: CockroachDB CCL v24.3.29 (x86_64-apple-darwin19, built 2026-03-09 00:00:00) (same version as client)
...
For more information about how this table locality works, see [Global tables](/docs/v24.3/table-localities#global-tables).
...
By default, [unqualified tables](/docs/v24.3/sql-name-resolution#lookup-with-unqualified-names) created in the database belong to the `public` schema:
...
If the new schema does not already exist, [create it](/docs/v24.3/create-schema):
...
The table's compound primary key is on the `city` and `dl` columns. Note that the table automatically generates an `id` and a `dl` value [using supported SQL functions](/docs/v24.3/functions-and-operators) if they are not provided.
...
You can specify the time at which a split enforcement expires by adding a `WITH EXPIRATION` clause to your `SPLIT` statement. Supported expiration values include [`DECIMAL`](/docs/v24.3/decimal), [`INTERVAL`](/docs/v24.3/interval), [`TIMESTAMP`](/docs/v24.3/timestamp), and [`TIMESTAMPZ`](/docs/v24.3/timestamp).
...
<Note>
If present in a [`CREATE TABLE`](/docs/v24.3/create-table) statement, the table is considered validated because an empty table trivially meets its constraints.
</Note>
...
- [`WITH` (storage parameter)](/docs/v24.3/with-storage-parameter)
- [Selection Queries](/docs/v24.3/selection-queries)
- [Distribution Layer](/docs/v24.3/architecture/distribution-layer)
- [Replication Layer](/docs/v24.3/architecture/replication-layer)
- [Online Schema Changes](/docs/v24.3/online-schema-changes)
