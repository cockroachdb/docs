...
Generalized inverted indexes, or GIN indexes, store mappings from values within a container column (such as a [`JSONB`](/docs/v24.3/jsonb) document) to the row that holds that value. They are used to speed up containment searches, e.g., "show me all of the rows from this table which have a JSON column that contains the key-value pair `{"location":"NYC"}`". GIN indexes are commonly used in [document retrieval systems](https://wikipedia.org/wiki/Document_retrieval).

CockroachDB stores the contents of the following data types in GIN indexes:

- [`JSONB`](/docs/v24.3/jsonb)
- [`ARRAY`](/docs/v24.3/array)
- [Spatial data (`GEOMETRY` and `GEOGRAPHY` types)](/docs/v24.3/spatial-indexes)
- [`TSVECTOR` (for full-text search)](/docs/v24.3/tsvector)
- [`STRING` (using trigram indexes)](/docs/v24.3/trigram-indexes)

<Tip>
  For a hands-on demonstration of using GIN indexes to improve query performance on a `JSONB` column, see the [JSON
  tutorial](/docs/v24.3/demo-json-support).
</Tip>

## How do GIN indexes work?

Standard [indexes](/docs/v24.3/indexes) work well for searches based on prefixes of sorted data. However, data types like [`JSONB`](/docs/v24.3/jsonb) or [arrays](/docs/v24.3/array) cannot be queried without a full table scan, since they do not adhere to ordinary value prefix comparison operators. `JSONB` in particular needs to be indexed in a more detailed way than what a standard index provides. This is where GIN indexes prove useful.
...
You can use GIN indexes to improve the performance of queries using [`JSONB`](/docs/v24.3/jsonb), [`ARRAY`](/docs/v24.3/array), [`TSVECTOR`](/docs/v24.3/tsvector) columns (for [full-text searches](/docs/v24.3/full-text-search)), or [`STRING`](/docs/v24.3/string) (for [fuzzy searches using trigrams](/docs/v24.3/trigram-indexes)). You can create them:

- Using the PostgreSQL-compatible syntax [`CREATE INDEX... USING GIN`](/docs/v24.3/create-index):

  ```sql
  CREATE INDEX {optional name} ON {table} USING GIN ({column});
  ```

  Also specify an opclass when [creating a trigram index](/docs/v24.3/trigram-indexes#creation):
...
- While creating the table, using the syntax [`CREATE INVERTED INDEX`](/docs/v24.3/create-table#create-a-table-with-secondary-and-gin-indexes):
...
To override CockroachDB's index selection, you can use an index hint to force a query to use [a specific index](/docs/v24.3/table-expressions#force-index-selection), including [inverted indexes](/docs/v24.3/table-expressions#force-inverted-index-scan) and [partial GIN indexes](/docs/v24.3/table-expressions#force-partial-gin-index-scan); or use [an inverted join hint](/docs/v24.3/cost-based-optimizer#supported-join-algorithms).
...
- **is contained by**: [`<@`](/docs/v24.3/functions-and-operators#operators)
- **contains**: [`@>`](/docs/v24.3/functions-and-operators#operators)
- **equals**: [`=`](/docs/v24.3/functions-and-operators#operators). To use `=`, you must also reach into the JSON document with the [`->`](/docs/v24.3/functions-and-operators#supported-operations) operator. For example:
...
If you require comparisons using [`<`](/docs/v24.3/functions-and-operators#operators), [`<=`](/docs/v24.3/functions-and-operators#operators), etc., you can create an index on a [computed column](/docs/v24.3/computed-columns) using your JSON payload, and then create a standard index on that. To write a query where the value of `foo` is greater than three:
...
GIN indexes on [`ARRAY`](/docs/v24.3/array) columns support the following comparison operators:
...
For an example showing how to create a trigram index on a [`STRING`](/docs/v24.3/string) column, see [Trigram Indexes](/docs/v24.3/trigram-indexes#examples).

### Create a full-text index on a `TSVECTOR` column

For an example showing how to create a full-text index on a [`TSVECTOR`](/docs/v24.3/tsvector) column, see [Full-Text Search](/docs/v24.3/full-text-search#examples).
...
- [Trigram Indexes](/docs/v24.3/trigram-indexes)
- [`JSONB`](/docs/v24.3/jsonb)
- [Arrays](/docs/v24.3/array)
- [Spatial data (`GEOMETRY` and `GEOGRAPHY` types)](/docs/v24.3/spatial-indexes)
