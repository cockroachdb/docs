...
These settings override the defaults used when [deploying CockroachDB on Kubernetes](/docs/v24.3/deploy-cockroachdb-with-kubernetes).
...
<Note>
1 CPU in Kubernetes is equivalent to 1 vCPU or 1 hyperthread. For best practices on provisioning CPU and memory for CockroachDB, see the [Production Checklist](/docs/v24.3/recommended-production-settings#hardware).
</Note>

Specify CPU and memory values in `resources.requests` and `resources.limits` in the Operator's custom resource, which is used to [deploy the cluster](/docs/v24.3/deploy-cockroachdb-with-kubernetes#initialize-the-cluster):
...
Each CockroachDB node reserves a portion of its available memory for its cache and for storing temporary data for SQL queries. For more information on these settings, see the [Production Checklist](/docs/v24.3/recommended-production-settings#cache-and-sql-memory-size).
...
Specify `cache` and `maxSQLMemory` in the Operator's custom resource, which is used to [deploy the cluster](/docs/v24.3/deploy-cockroachdb-with-kubernetes#initialize-the-cluster):
...
<Note>
Specifying these values is equivalent to using the `--cache` and `--max-sql-memory` flags with [`cockroach start`](/docs/v24.3/cockroach-start#flags).
</Note>
...
The storage capacity of each volume is set in `pvc.spec.resources` in the Operator's custom resource, which is used to [deploy the cluster](/docs/v24.3/deploy-cockroachdb-with-kubernetes#initialize-the-cluster):
...
You should provision an appropriate amount of disk storage for your workload. For recommendations on this, see the [Production Checklist](/docs/v24.3/recommended-production-settings#storage).

### Expand disk size

If you discover that you need more capacity, you can expand the persistent volumes on a running cluster. Increasing disk size is often [beneficial for CockroachDB performance](/docs/v24.3/kubernetes-performance#disk-size).

Specify a new volume size in `resources.requests` and `resources.limits` in the Operator's custom resource, which is used to [deploy the cluster](/docs/v24.3/deploy-cockroachdb-with-kubernetes#initialize-the-cluster):
...
<table><thead><tr><th>Protocol</th><th>Default</th><th>Description</th><th>Custom Resource Field</th></tr></thead><tbody><tr><td>gRPC</td><td>26258</td><td>Used for node connections</td><td><code>grpcPort</code></td></tr><tr><td>HTTP</td><td>8080</td><td>Used to <a href="/docs/v24.3/ui-overview#db-console-access">access the DB Console</a></td><td><code>httpPort</code></td></tr><tr><td>SQL</td><td>26257</td><td>Used for SQL shell access</td><td><code>sqlPort</code></td></tr></tbody></table>

Specify alternate port numbers in the Operator's [custom resource](/docs/v24.3/deploy-cockroachdb-with-kubernetes#initialize-the-cluster) (for example, to match the default port `5432` on PostgreSQL):
...
Specify Ingress objects in `ingress.ui` (HTTP) or `ingress.sql` (SQL) in the Operator's custom resource, which is used to [deploy the cluster](/docs/v24.3/deploy-cockroachdb-with-kubernetes#initialize-the-cluster):
...
Specify CPU and memory values in `resources.requests` and `resources.limits` in the StatefulSet manifest you used to [deploy the cluster](/docs/v24.3/deploy-cockroachdb-with-kubernetes?filters=manual#configure-the-cluster):
...
The storage capacity of each volume is initially set in `volumeClaimTemplates.spec.resources` in the StatefulSet manifest you used to [deploy the cluster](/docs/v24.3/deploy-cockroachdb-with-kubernetes?filters=manual#configure-the-cluster):
...
<Note>
These steps assume you followed the tutorial [Deploy CockroachDB on Kubernetes](/docs/v24.3/deploy-cockroachdb-with-kubernetes?filters=manual).
</Note>
...
Specify CPU and memory values in `resources.requests` and `resources.limits` in the custom values file you created when [deploying the cluster](/docs/v24.3/deploy-cockroachdb-with-kubernetes?filters=helm#step-2-start-cockroachdb):
...
Specify `cache` and `maxSQLMemory` in the custom values file you created when [deploying the cluster](/docs/v24.3/deploy-cockroachdb-with-kubernetes?filters=helm#step-2-start-cockroachdb):
...
<Note>
These steps assume you followed the tutorial [Deploy CockroachDB on Kubernetes](/docs/v24.3/deploy-cockroachdb-with-kubernetes?filters=helm).
</Note>
