...
import { SelectClause5 } from "/snippets/syntax-diagrams/SelectClause5.jsx";

The simple `SELECT` clause is the main SQL syntax to read and process existing data.

When used as a stand-alone statement, the simple `SELECT` clause is also called "the `SELECT` statement". However, it is also a [selection clause](/docs/v24.3/selection-queries#selection-clauses) that can be combined with other constructs to form more complex [selection queries](/docs/v24.3/selection-queries).
...
<Tip>
  ...
  [functions](/docs/v24.3/functions-and-operators) like `SELECT current_timestamp();`.
</Tip>

## Required privileges

The user must have the `SELECT` [privilege](/docs/v24.3/security-reference/authorization#managing-privileges) on the tables used as operands.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <code>DISTINCT ON</code> followed by a list of <a href="/docs/v24.3/scalar-expressions">scalar expressions</a>{" "}
    ...
    <tr>
      ...
      <td>
        A <a href="/docs/v24.3/scalar-expressions">scalar expression</a> to compute a column in each result row.
        <br />
        <br />
        <code>&#42;</code> automatically retrieves all columns. If a column has the{" "}
        <a href="/docs/v24.3/create-table#not-visible-property">
          <code>NOT VISIBLE</code> property
        </a>
        , it is not returned with <code>&#42;</code>.<br />
        <br />
        If <code>target&#95;elem</code> contains an{" "}
        <a href="/docs/v24.3/functions-and-operators#aggregate-functions">aggregate function</a>, a{" "}
    ...
    <tr>
      ...
      <td>
        The <a href="/docs/v24.3/table-expressions">table expression</a> you want to retrieve data from.
        <br />
        <br />
        Using two or more table expressions in the <code>FROM</code> sub-clause, separated with a comma, is equivalent
        to a{" "}
        <a href="/docs/v24.3/joins">
    ...
    <tr>
      ...
      <td>
        ...
        <a href="/docs/v24.3/scalar-expressions">scalar expression</a> that returns Boolean values using columns (e.g.,{" "}
    ...
    <tr>
      ...
      <td>
        ...
        <br />
        <br />
        When an <a href="/docs/v24.3/functions-and-operators#aggregate-functions">aggregate function</a> follows{" "}
        ...
        <br />
        If a selected column is in a <a href="/docs/v24.3/subqueries">subquery</a>, and the column references a higher
    ...
    <tr>
      ...
      <td>
        ...
        a <a href="/docs/v24.3/scalar-expressions">scalar expression</a> that returns Boolean values using an aggregate
    ...
    <tr>
      ...
      <td>
        A list of <a href="/docs/v24.3/window-functions#window-definitions">window definitions</a>.
      </td>
    </tr>
  </tbody>
</table>
...
With `ON`, two rows are considered duplicates if they are equal only using the [scalar expressions](/docs/v24.3/scalar-expressions) listed with `ON`. When two rows are considered duplicates according to `DISTINCT ON`, the values from the first `FROM` row in the order specified by [`ORDER BY`](/docs/v24.3/order-by) are used to compute the remaining target expressions. If `ORDER BY` is not specified, CockroachDB will pick any one of the duplicate rows as first row, non-deterministically.
...
Columns without the [Primary Key](/docs/v24.3/primary-key) or [Unique](/docs/v24.3/unique) constraints can have multiple instances of the same value:
...
This _does not_ change the name of the column in the table. To do that, use [`RENAME COLUMN`](/docs/v24.3/alter-table#rename-column).

### Search for string values

Search for partial [string](/docs/v24.3/string) matches in columns using `LIKE`, which supports the following wildcard operators:
...
[Aggregate functions](/docs/v24.3/functions-and-operators#aggregate-functions) perform calculations on retrieved rows.
...
<Note>
  ...
  selected column is in a [subquery](/docs/v24.3/subqueries) that references a higher scope, a `GROUP BY` clause is not
  needed.
</Note>
...
Non-commutative aggregate functions are sensitive to the order in which the rows are processed in the surrounding `SELECT` clause. To specify the order in which input rows are processed, you can add an [`ORDER BY`](/docs/v24.3/order-by) clause within the function argument list.
...
By using the explicit index annotation, you can override [CockroachDB's index selection](https://www.cockroachlabs.com/blog/index-selection-cockroachdb-2) and use a specific [index](/docs/v24.3/indexes) when reading from a named table.

<Note>
  Index selection can impact [performance](/docs/v24.3/performance-best-practices-overview), but does not change the
  result of a query.
</Note>
...
Forcing a reverse scan can help with [performance tuning](/docs/v24.3/performance-best-practices-overview). To choose an index and its scan direction:
...
When a direction is specified, that scan direction is forced; otherwise the [cost-based optimizer](/docs/v24.3/cost-based-optimizer) is free to choose the direction it calculates will result in the best performance.

You can verify that the optimizer is choosing your desired scan direction using [`EXPLAIN (OPT)`](/docs/v24.3/explain#opt-option). For example, given the table
...
To force a scan of any [inverted index](/docs/v24.3/inverted-indexes) of the hinted table:
...
To force a [partial index scan](/docs/v24.3/partial-indexes), your statement must have a `WHERE` clause that implies the partial index filter.
...
To force a [partial GIN index](/docs/v24.3/inverted-indexes#partial-gin-indexes) scan, your statement must have a `WHERE` clause that:
...
<Tip>
  ...
  scans](/docs/v24.3/performance-best-practices-overview#prevent-the-optimizer-from-planning-full-scans).
</Tip>

### Select historical data (time-travel)

CockroachDB lets you find data as it was stored at a given point in time using `AS OF SYSTEM TIME` with various [supported formats](/docs/v24.3/as-of-system-time). This can be also advantageous for performance. For more details, see [`AS OF SYSTEM
TIME`](/docs/v24.3/as-of-system-time).

## Advanced uses of `SELECT` clauses

CockroachDB supports numerous ways to combine results from `SELECT` clauses together.

See [Selection Queries](/docs/v24.3/selection-queries) for details. A few examples follow.

### Sorting and limiting query results

To order the results of a `SELECT` clause or limit the number of rows in the result, you can combine it with `ORDER BY` or `LIMIT` / `OFFSET` to form a [selection query](/docs/v24.3/selection-queries) or [subquery](/docs/v24.3/table-expressions#use-a-subquery).

See [Ordering Query Results](/docs/v24.3/order-by) and [Limiting Query Results](/docs/v24.3/limit-offset) for more details.
...
<Note>
  ...
  values](/docs/v24.3/null-handling#nulls-and-sorting) first with `ASC` and last with `DESC`. This differs from
...
- Using [`JOIN` expressions](/docs/v24.3/joins) to combine rows according to conditions on specific columns.
- Using [set operations](/docs/v24.3/selection-queries#set-operations) to combine rows using inclusion/exclusion rules.
...
<Note>
  ...
  [`UPDATE`](/docs/v24.3/update) and [`UPSERT`](/docs/v24.3/upsert) statement execution. To turn off implicit `SELECT...
  FOR UPDATE` locking for `UPDATE` and `UPSERT` statements, set the `enable_implicit_select_for_update` [session
  variable](/docs/v24.3/set-vars) to `false`.
</Note>
...
- [Scalar Expressions](/docs/v24.3/scalar-expressions)
- [Selection Clauses](/docs/v24.3/selection-queries#selection-clauses)
- [`SELECT FOR UPDATE`](/docs/v24.3/select-for-update)
- [Set Operations](/docs/v24.3/selection-queries#set-operations)
