...
<LinkedPageTabs
  currentPath="/docs/v24.3/build-a-python-app-with-cockroachdb-sqlalchemy"
  items={[
    {
      title: "Use psycopg3",
      href: "/docs/v24.3/build-a-python-app-with-cockroachdb-psycopg3",
    },
    {
      title: "Use psycopg2",
      href: "/docs/v24.3/build-a-python-app-with-cockroachdb",
    },
    {
      title: "Use SQLAlchemy",
      href: "/docs/v24.3/build-a-python-app-with-cockroachdb-sqlalchemy",
    },
    {
      title: "Use Django",
      href: "/docs/v24.3/build-a-python-app-with-cockroachdb-django",
    },
    {
      title: "Use asyncpg",
      href: "/docs/v24.3/build-a-python-app-with-cockroachdb-asyncpg",
    },
  ]}
/>
...
2. To initialize the example database, use the [`cockroach sql`](/docs/v24.3/cockroach-sql) command to execute the SQL statements in the `dbinit.sql` file:
...
- It abstracts away the [client-side transaction retry logic](/docs/v24.3/transaction-retry-error-reference#client-side-retry-handling) from your application, which keeps your application code portable across different databases. For example, the sample code given on this page works identically when run against PostgreSQL (modulo changes to the prefix and port number in the connection string).

For more information about how transactions (and retries) work, see [Transactions](/docs/v24.3/transactions).
...
If you see an error message like `transaction is too large to complete; try splitting into pieces`, you are trying to commit too much data in a single transaction. As described in our [Cluster Settings](/docs/v24.3/cluster-settings) docs, the size limit for transactions is defined by the `kv.transaction.max_intents_bytes` setting, which defaults to 256 KiB. Although this setting can be changed by an admin, we strongly recommend against it in most cases.
...
If you are trying to get a large data set into CockroachDB all at once (a bulk import), avoid writing client-side code that uses an ORM and use the [`IMPORT INTO`](/docs/v24.3/import-into) statement instead. It is much faster and more efficient than making a series of [`INSERT`s](/docs/v24.3/insert) and [`UPDATE`s](/docs/v24.3/update) such as are generated by calls to [`session.bulk_save_objects()`](https://docs.sqlalchemy.org/orm/session_api?highlight=bulk_save_object#sqlalchemy.orm.session.Session.bulk_save_objects).
