...
To reference an object (e.g., a table) in a query, you can specify the [identifier](/docs/v24.3/keywords-and-identifiers#identifiers) that refers to a database, a schema, both, or neither. To resolve which object a query references, CockroachDB scans the [appropriate namespaces](#naming-hierarchy), following the rules in [How name resolution works](#how-name-resolution-works).
...
A CockroachDB cluster can store multiple databases. Each database can store multiple schemas, and each schema can store multiple [tables](/docs/v24.3/create-table), [views](/docs/v24.3/views), [sequences](/docs/v24.3/create-sequence), and [user-defined types](/docs/v24.3/enum).

When you first [start a cluster](/docs/v24.3/start-a-local-cluster), a number of [preloaded databases](/docs/v24.3/show-databases#preloaded-databases) and schemas are included, including the `defaultdb` database and the `public` schema. By default, objects (e.g., tables) are stored in the preloaded `public` schema, in the [current database](#current-database) (`defaultdb`, by default).

In addition to the `public` schema, CockroachDB supports a fixed set of [system catalog schemas](/docs/v24.3/system-catalogs), available in every database, that provide ancillary, non-stored data to client applications. For example, [`information_schema`](/docs/v24.3/information-schema) is provided for compatibility with the SQL standard, and [`pg_catalog`](/docs/v24.3/pg-catalog) and [`pg_extension`](/docs/v24.3/architecture/glossary#spatial-system-tables) are provided for compatibility with PostgreSQL.

To create a new database, use a [`CREATE DATABASE`](/docs/v24.3/create-database) statement. To create a new schema, use a [`CREATE SCHEMA`](/docs/v24.3/create-schema) statement. The list of all databases can be obtained with [`SHOW DATABASES`](/docs/v24.3/show-databases). The list of all schemas for a given database can be obtained with [`SHOW SCHEMAS`](/docs/v24.3/show-schemas). The list of all objects for a given schema can be obtained with other `SHOW` statements.

### Migrating namespaces from previous versions of CockroachDB

In CockroachDB versions &lt; v20.2, [user-defined schemas](/docs/v24.3/create-schema) are not supported, and all objects created in a given database use the `public` schema. To provide a multi-level structure for stored objects in earlier versions of CockroachDB, we have recommended using [database](/docs/v24.3/create-database) namespaces instead of schema namespaces.
...
If you are upgrading to v24.3, take any combination of the following actions after the upgrade is complete:

- [Create new schemas](/docs/v24.3/create-schema) in databases on your cluster. After the schemas are created, use [`ALTER TABLE... RENAME`](/docs/v24.3/alter-table#rename-to), [`ALTER SEQUENCE... RENAME`](/docs/v24.3/alter-sequence), [`ALTER TYPE... RENAME`](/docs/v24.3/alter-type), or [`ALTER VIEW... RENAME`](/docs/v24.3/alter-view) statements to move objects between databases as needed. To move objects between schemas, use [`ALTER TABLE... SET SCHEMA`](/docs/v24.3/alter-table#set-schema), [`ALTER SEQUENCE... SET SCHEMA`](/docs/v24.3/alter-sequence), or [`ALTER VIEW... SET SCHEMA`](/docs/v24.3/alter-view).
- If your cluster contains cross-database references (e.g., a cross-database foreign key reference, or a cross-database view reference), use the relevant [`ALTER TABLE`](/docs/v24.3/alter-table), [`ALTER SEQUENCE`](/docs/v24.3/alter-sequence), [`ALTER TYPE`](/docs/v24.3/alter-type), or [`ALTER VIEW`](/docs/v24.3/alter-view) statements to move any cross-referencing objects to the same database, but different schemas. Cross-database object references were allowed in earlier versions of CockroachDB to make database-object naming hierarchies more flexible for users. In v20.2, creating cross-database references are disabled for [foreign keys](/docs/v24.3/foreign-key), [views](/docs/v24.3/views), and [sequence ownership](/docs/v24.3/create-sequence). For details, see [tracking issue](https://github.com/cockroachdb/cockroach/issues/55791).
...
- You can view the current value of the `database` session variable with [`SHOW database`](/docs/v24.3/show-vars) and change it with [`SET database`](/docs/v24.3/set-vars).
- You can inspect the list of valid database names that can be specified in `database` with [`SHOW DATABASES`](/docs/v24.3/show-databases).
...
- You can set the current search path with [`SET search_path`](/docs/v24.3/set-vars) and inspected it with [`SHOW search_path`](/docs/v24.3/show-vars).
- You can inspect the list of valid schemas that can be listed in `search_path` with [`SHOW SCHEMAS`](/docs/v24.3/show-schemas).
...
CockroachDB supports the following ways to specify an index name for statements that require one (e.g., [`DROP INDEX`](/docs/v24.3/drop-index), [`ALTER INDEX... RENAME`](/docs/v24.3/alter-index), etc.):
...
Suppose that a client frequently accesses a stored table as well as a virtual table in the [Information Schema](/docs/v24.3/information-schema). Because `information_schema` is not in the search path by default, all queries that need to access it must mention it explicitly.
...
When resolving a partially qualified name with just one component prefix, CockroachDB will look up a schema with the given prefix name first, and only look up a database with that name if the schema lookup fails. This matters in the (likely uncommon) case where you wish your database to be called `information_schema`, `public`, `pg_catalog`, [`pg_extension`](/docs/v24.3/architecture/glossary#spatial-system-tables), or `crdb_internal`.
...
The [`CREATE TABLE`](/docs/v24.3/create-table) statement in this example uses a partially qualified name. Because the `public` prefix designates a valid schema in the current database, the full name of `mypublictable` becomes `mydb.public.mypublictable`. The table is created in database `mydb`.
...
- [Keywords and Identifiers](/docs/v24.3/keywords-and-identifiers)
