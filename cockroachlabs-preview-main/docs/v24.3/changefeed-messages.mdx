...
<Note>
  Changefeed types are not fully integrated with [user-defined composite types](/docs/v24.3/create-type). Running
  changefeeds with user-defined composite types is in
  [Preview](/docs/v24.3/cockroachdb-feature-availability#feature-availability-phases). Certain changefeed types do not
  support user-defined composite types. Refer to the change data capture [Known
  Limitations](/docs/v24.3/create-and-configure-changefeeds#known-limitations) for more detail.
</Note>

## Responses

By default, changefeed messages emitted to a [sink](/docs/v24.3/changefeed-sinks) contain keys and values of the watched table rows that have changed. The message will contain the following fields depending on the type of emitted change and the [options](/docs/v24.3/create-changefeed#options) you specified to create the changefeed:
...
- **Value**:
  - One of four possible top-level fields:
    ...
    - `updated`, which contains the [updated](/docs/v24.3/create-changefeed#updated) timestamp.
    ...
    - `before`, which contains the state of the row before an update. Changefeeds must use the [`diff` option](/docs/v24.3/create-changefeed#diff) with the default [`wrapped` envelope](#wrapped) to emit the `before` field. When a row did not previously have any data, the `before` field will emit `null`.
  - For [`INSERT`](/docs/v24.3/insert) and [`UPDATE`](/docs/v24.3/update), the current state of the row inserted or updated.
  - For [`DELETE`](/docs/v24.3/delete), `null`.

<Note>
  If you use the `envelope` option to alter the changefeed message fields, your messages may not contain one or more of
  the values noted in the preceding list. As an example, when emitting to a Kafka sink, you can limit messages to just
  the changed key value by using the `envelope` option set to [`key_only`](#key_only). For more detail, refer to
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        <code>INSERT INTO office&#95;dogs VALUES (1, 'Petee');</code>
      </td>
      <td>
        JSON: <code>&#91;1&#93; &#123;"after": &#123;"id": 1, "name": "Petee"&#125;&#125;</code> Avro:{" "}
        <code>
          &#123;"id":&#123;"long":1&#125;&#125;
          &#123;"after":&#123;"office&#95;dogs":&#123;"id":&#123;"long":1&#125;,"name":&#123;"string":"Petee"&#125;&#125;&#125;&#125;
        </code>
      </td>
    </tr>
    <tr>
      <td>
        <code>DELETE FROM office&#95;dogs WHERE name = 'Petee'</code>
      </td>
      <td>
        JSON: <code>&#91;1&#93; &#123;"after": null&#125;</code> Avro:{" "}
        <code>&#123;"id":&#123;"long":1&#125;&#125; &#123;"after":null&#125;</code>
      </td>
    </tr>
  </tbody>
</table>

When a changefeed targets a table with multiple column families, the family name is appended to the table name as part of the topic. Refer to [Tables with columns families in changefeeds](/docs/v24.3/changefeeds-on-tables-with-column-families#message-format) for guidance.

For [webhook sinks](/docs/v24.3/changefeed-sinks#webhook-sink), the response format arrives as a batch of changefeed messages with a `payload` and `length`.

```text
{"payload": [{"after" : {"a" : 1, "b" : "a"}, "key": [1], "topic": "foo"}, {"after": {"a": 1, "b": "b"}, "key": [1], "topic": "foo" }], "length":2}
```

[Webhook message batching](/docs/v24.3/changefeed-sinks#webhook-sink-configuration) is subject to the same key [ordering guarantee](#ordering-and-delivery-guarantees) as other sinks. Therefore, as messages are batched, you will not receive two batches at the same time with overlapping keys. You may receive a single batch containing multiple messages about one key, because ordering is maintained for a single key within its batch.

Refer to [changefeed files](/docs/v24.3/create-changefeed#files) for more detail on the file naming format for changefeeds that emit to a sink.

## Message envelopes

The _envelope_ defines the structure of a changefeed message. You can use the [`envelope`](/docs/v24.3/create-changefeed#envelope) option to manipulate the changefeed envelope. The values that the `envelope` option accepts are compatible with different [changefeed sinks](/docs/v24.3/changefeed-sinks), and the structure of the message will vary depending on the sink.

<Note>
  Changefeeds created with [`EXPERIMENTAL CHANGEFEED FOR`](/docs/v24.3/changefeed-for) or [`CREATE
  CHANGEFEED`](/docs/v24.3/create-changefeed) with no sink specified (sinkless changefeeds) produce messages without the
  envelope metadata fields of changefeeds emitting to sinks.
</Note>
...
- In CDC queries:
  ...
  - A changefeed containing a `SELECT` clause with the [`topic_in_value`](/docs/v24.3/create-changefeed#topic-in-value) option specified:
...
`key_only` emits only the key and no value, which is faster if you only need to know the key of the changed row. This envelope option is only supported for [Kafka sinks](/docs/v24.3/changefeed-sinks#kafka) or sinkless changefeeds.
...
- Kafka sink:
  ...
  <Note>
    It is necessary to set up a [Kafka consumer](https://docs.confluent.io/platform/current/clients/consumer) to display
    the key because the key is part of the metadata in Kafka messages, rather than in its own field. When you start a
    Kafka consumer, you can use `--property print.key=true` to have the key print in the changefeed message.
  </Note>
...
`row` emits the row without any additional metadata fields in the message. This envelope option is only supported for [Kafka sinks](/docs/v24.3/changefeed-sinks#kafka) or sinkless changefeeds. `row` does not support [`avro`](#avro) format—if you are using `avro`, refer to the [`bare`](#bare) envelope option.
...
<Note>Changefeeds do not support total message ordering or transactional ordering of messages.</Note>
...
3. Insert and update values in `employees`:
   ...
   <Note>
     In a [transaction](/docs/v24.3/transactions), if a row is modified more than once in the same transaction, the
     changefeed will only emit the last change.
   </Note>
...
4. The sink will receive messages of the inserted rows emitted per timestamp:
   ...
   <Note>
     Depending on the workload, you can use resolved timestamp notifications on every Kafka partition to provide strong
     ordering and global consistency guarantees by buffering records in between timestamp closures. Use the `resolved`
...
<Warning>
  The first time a message is delivered, it will be in the correct timestamp order, which follows the [per-key ordering
  guarantee](#per-key-ordering). However, when there are [duplicate messages](#duplicate-messages), the changefeed may
  **not** re-emit every row update. As a result, there may be gaps in a sequence of duplicate messages for a key.
</Warning>
...
<Note>
  When changes happen to a column that is part of a composite [key](/docs/v24.3/primary-key), the changefeed will
  produce a [delete message](/docs/v24.3/changefeed-messages#delete-messages) and then an insert message.
</Note>
...
When you create a changefeed with the [`resolved` option](/docs/v24.3/create-changefeed#resolved), the changefeed will emit resolved timestamp messages in a format dependent on the connected [sink](/docs/v24.3/changefeed-sinks). The resolved timestamp is the high-water mark that guarantees that no previously unseen rows with an [earlier update timestamp](#ordering-and-delivery-guarantees) will be emitted to the sink. That is, resolved timestamp messages do not emit until the changefeed job has reached a [checkpoint](/docs/v24.3/how-does-a-changefeed-work).

When you specify the `resolved` option at changefeed creation, the [job's coordinating node](/docs/v24.3/how-does-a-changefeed-work) will send the resolved timestamp to each endpoint at the sink. For example, each [Kafka](/docs/v24.3/changefeed-sinks#kafka) partition will receive a resolved timestamp message, or a [cloud storage sink](/docs/v24.3/changefeed-sinks#cloud-storage-sink) will receive a resolved timestamp file.
...
The changefeed job's coordinating node will emit resolved timestamp messages once the changefeed has reached a checkpoint. The frequency of the checkpoints determine how often the resolved timestamp messages emit to the sink. To configure how often the changefeed checkpoints, you can set the [`min_checkpoint_frequency`](/docs/v24.3/create-changefeed#min-checkpoint-frequency) option and [flush frequency](/docs/v24.3/changefeed-sinks) (if flushing is configurable for your sink).
...
<Frame>
  ![How checkpoints will re-emit messages when a changefeed pauses. The changefeed returns to the last checkpoint and
  potentially sends duplicate messages.](/images/v24.3/changefeed-duplicate-messages-emit.png)
</Frame>

### Changefeed encounters an error

By default, changefeeds treat errors as [retryable except for some specific terminal errors](/docs/v24.3/monitor-and-debug-changefeeds#changefeed-retry-errors). When a changefeed encounters a retryable or non-retryable error, the job will pause until a successful retry or you resume the job once the error is solved. This can cause duplicate messages at the sink as the changefeed returns to the last checkpoint.

We recommend monitoring for changefeed retry errors and failures. Refer to the [Monitor and Debug Changefeeds](/docs/v24.3/monitor-and-debug-changefeeds#recommended-changefeed-metrics-to-track) page.

<Note>
A sink's batching behavior can increase the number of duplicate messages. For example, if Kafka receives a batch of `N` messages and successfully saves `N-1` of them, the changefeed job only knows that the batch failed, not which message failed to commit. As a result, the changefeed job will resend the full batch of messages, which means all but one of the messages are duplicates. For Kafka sinks, reducing the batch size with [`kafka_sink_config`](/docs/v24.3/changefeed-sinks#kafka-sink-configuration) may help to reduce the number of duplicate messages at the sink.

Refer to the [Changefeed Sinks](/docs/v24.3/changefeed-sinks) page for details on sink batching configuration.

</Note>

### Node restarts

When a node restarts, the changefeed will emit duplicates since the last checkpoint. During a rolling restart of nodes, a changefeed can fall behind as it tries to catch up during each node restart. For example, as part of a rolling upgrade or cluster maintenance, a node may [drain](/docs/v24.3/node-shutdown) every 5 minutes and the changefeed job checkpoints every 5 minutes.

To prevent the changefeed from falling too far behind, [pause](/docs/v24.3/create-and-configure-changefeeds#configuring-all-changefeeds) changefeed jobs before performing rolling node restarts.
...
<Warning>
  Schema validation tools should ignore the `__crdb__` field. This is an internal CockroachDB schema type description
  that may change between CockroachDB versions.
</Warning>

### Schema changes with column backfill

When schema changes with column backfill (e.g., adding a column with a default, adding a [stored computed column](/docs/v24.3/computed-columns), adding a `NOT NULL` column, dropping a column) are made to watched rows, CockroachDB emits a copy of the table using the new schema.
...
Refer to the [`CREATE CHANGEFEED` option table](/docs/v24.3/create-changefeed#schema-change-events) for detail on the `schema_change_policy` option. You can also use the `schema_change_events` option to define the type of schema change event that triggers the behavior specified in `schema_change_policy`.

<Note>
  As of v22.1, changefeeds filter out [`VIRTUAL` computed columns](/docs/v24.3/computed-columns) from events by default.
  This is a [backward-incompatible change](/docs/releases/v22.1#v22-1-0-backward-incompatible-changes). To maintain the
  changefeed behavior in previous versions where [`NULL`](/docs/v24.3/null-handling) values are emitted for virtual
  computed columns, see the [`virtual_columns`](/docs/v24.3/create-changefeed#virtual-columns) option for more detail.
</Note>
...
To prevent changefeeds from emitting messages for any changes (e.g., `INSERT`, `UPDATE`) issued to watched tables during that session, set the `disable_changefeed_replication` [session variable](/docs/v24.3/session-variables) to `true`.
...
For details on syntax and examples, refer to the [Change Data Capture Queries](/docs/v24.3/cdc-queries) page.

## Message formats

By default, changefeeds emit messages in JSON format. You can use a different format by [creating a changefeed](/docs/v24.3/create-changefeed) with the [`format`](/docs/v24.3/create-changefeed#format) option and specifying one of the following:
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        <a href="/docs/v24.3/array">
          <code>ARRAY</code>
        </a>
      </td>
      <td>
        <a href="https://avro.apache.org/docs/1.8.1/spec#schema_complex">
          <code>ARRAY</code>
        </a>
      </td>
      <td></td>
    ...
    <tr>
      <td>
        <a href="/docs/v24.3/bit">
          <code>BIT</code>
        </a>
      </td>
      <td>
        Array of{" "}
        <a href="https://avro.apache.org/docs/1.8.1/spec#schema_primitive">
          <code>LONG</code>
        </a>
      </td>
      <td></td>
    ...
    <tr>
      ...
      <td>
        <a href="https://avro.apache.org/docs/1.8.1/spec#Date">
          <code>DATE</code>
        </a>
      </td>
    </tr>
    <tr>
      ...
      <td>
        <a href="https://avro.apache.org/docs/1.8.1/spec#Decimal">
          <code>DECIMAL</code>
        </a>
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        <a href="https://avro.apache.org/docs/1.8.1/spec#Time+%28microsecond+precision%29">
          <code>TIME-MICROS</code>
        </a>
      </td>
    </tr>
    <tr>
...
<Note>The `DECIMAL` type is a union between Avro `STRING` and Avro `DECIMAL` types.</Note>

### CSV

You can use the [`format=csv`](/docs/v24.3/create-changefeed#format) option to emit CSV format messages from your changefeed. However, there are the following limitations with this option:

- It **only** works in combination with the [`initial_scan = 'only'`](/docs/v24.3/create-changefeed#initial-scan) option.
- It does **not** work when used with the [`diff`](/docs/v24.3/create-changefeed#diff) or [`resolved`](/docs/v24.3/create-changefeed#resolved) options.
...
Changefeeds emit the same CSV format as [`EXPORT`](/docs/v24.3/export). In v22.1, changefeeds emitted CSV data that wrapped some values in single quotes, which were not wrapped when exporting data with the `EXPORT` statement.

See [Export Data with Changefeeds](/docs/v24.3/export-data-with-changefeeds) for detail on using changefeeds to export data from CockroachDB.
...
<InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v24.2: To distinguish between JSON `NULL` values and SQL `NULL` values in changefeed messages, you can use the [`encode_json_value_null_as_object` option](/docs/v24.3/create-changefeed#encode-json-value-null-as-object) with [`CREATE CHANGEFEED`](/docs/v24.3/create-changefeed). When you enable `encode_json_value_null_as_object`, JSON `NULL` values will emit as `{"__crdb_json_null__": true}`.

For example, the following `test` table has a primary key column and a [`JSONB`](/docs/v24.3/jsonb) column. The `INSERT` adds `NULL` values in each column. A changefeed without the option enabled will not distinguish between the SQL and JSON `NULL` values. With the `encode_json_value_null_as_object` option enabled, the changefeed emits the JSON `NULL` as `{"__crdb_json_null__": true}`:
...
When `encode_json_value_null_as_object` is enabled, if the changefeed encounters the literal value `{"__crdb_json_null__": true}` in JSON, it will have the same representation as a JSON `NULL` value and a warning will be printed to the [`DEV` logging channel](/docs/v24.3/logging#dev).
