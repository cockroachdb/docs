...
<LinkedPageTabs
  currentPath="/docs/v24.3/performance-benchmarking-with-tpcc-medium"
  items={[
    {
      title: "Local",
...
3. Run the [`cockroach start`](/docs/v24.3/cockroach-start) command:
    ...
    Each node will start with a [locality](/docs/v24.3/cockroach-start#locality) that includes an artificial "rack number" (e.g., `--locality=rack=0`). Use 5 racks for 15 nodes so that 3 nodes will be assigned to each rack.
...
Next, [partition your database](/docs/v24.3/partitioning) to divide all of the TPC-C tables and indexes into 5 partitions, one per rack, and then use [zone configurations](/docs/v24.3/configure-replication-zones) to pin those partitions to a particular rack.
