...
import { ShowTables4 } from "/snippets/syntax-diagrams/ShowTables4.jsx";

The `SHOW TABLES` [statement](/docs/v24.3/sql-statements) lists the schema, table name, table type, owner, and estimated row count for the tables or [views](/docs/v24.3/views) in a schema or database.

<Note>
While a table or view is being [dropped](/docs/v24.3/drop-table), `SHOW TABLES` will list the object with a `(dropped)` suffix.
</Note>
...
The `CONNECT` [privilege](/docs/v24.3/security-reference/authorization#managing-privileges) on the database of the concerned table is required to list it with SHOW TABLES.
...
When a `database_name` and `schema_name` are omitted, the tables of the [current schema](/docs/v24.3/sql-name-resolution#current-schema) in the [current database](/docs/v24.3/sql-name-resolution#current-database) are listed.

`SHOW TABLES` will attempt to find a schema with the specified name first. If that fails, it will try to find a database with that name instead, and list the tables of its `public` schema. For more details, see [Name Resolution](/docs/v24.3/sql-name-resolution).

## Performance

To optimize the performance of the `SHOW TABLES` statement, you can do the following:

- Disable table row-count estimation by setting the `sql.show_tables.estimated_row_count.enabled` [cluster setting](/docs/v24.3/cluster-settings) to `false` before executing a `SHOW TABLES` statement.
...
`SHOW TABLES` uses the [current schema](/docs/v24.3/sql-name-resolution#current-schema) `public` set by default in `search_path`:
...
Alternatively, within the built-in SQL shell, you can use the `\dt` [shell command](/docs/v24.3/cockroach-sql#commands):
...
You can use [`COMMENT ON`](/docs/v24.3/comment-on) to add comments on a table.
...
You can also view comments on a table with [`SHOW CREATE`](/docs/v24.3/show-create):
...
For [multi-region](/docs/v24.3/multiregion-overview) tables, you can display the locality of each table using the `SHOW TABLES` command.

1. [Set the primary region](/docs/v24.3/alter-database#set-primary-region) on `movr` to `us-east`:

    ```sql
    > ALTER DATABASE movr SET PRIMARY REGION "us-east";
    ```

    All tables will be [`REGIONAL BY TABLE`](/docs/v24.3/alter-table#set-the-table-locality-to-regional-by-table) in the primary region by default.
2. Configure the `users` table to be [`REGIONAL BY ROW`](/docs/v24.3/alter-table#set-the-table-locality-to-regional-by-row):
    ...
    <Note>
    Locality information for tables is also available in the `locality` column within the [`crdb_internal.tables`](/docs/v24.3/crdb-internal) table.
    </Note>
