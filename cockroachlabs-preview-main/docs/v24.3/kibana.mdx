...
[Kibana](https://www.elastic.co/kibana) is a platform that visualizes data on the [Elastic Stack](https://www.elastic.co/elastic-stack). This page shows how to use the [CockroachDB module for Metricbeat](https://www.elastic.co/guide/en/beats/metricbeat/current/metricbeat-module-cockroachdb) to collect metrics exposed by your CockroachDB self-hosted cluster's [Prometheus endpoint](/docs/v24.3/monitoring-and-alerting#prometheus-endpoint) in Elasticsearch and how to visualize those metrics with Kibana.

<Tip>
To export metrics from a CockroachDB Cloud cluster, refer to [Export Metrics From a CockroachDB Advanced Cluster](/docs/cockroachcloud/export-metrics) instead of this page.
</Tip>
...
<Note>
This tutorial assumes that you have [started a secure CockroachDB cluster](/docs/v24.3/secure-a-cluster). [CockroachDB Cloud](/docs/cockroachcloud) does not expose a compatible monitoring endpoint.
</Note>
...
For example, if you used [`cockroach cert`](/docs/v24.3/cockroach-cert) to [secure your cluster](/docs/v24.3/secure-a-cluster#step-1-generate-certificates), the YAML should look like:
...
<Frame>
  ![CockroachDB dashboard selection for Metricbeat](/images/v24.3/kibana-crdb-dashboard-selection.png)
</Frame>

Click the dashboard title to open the dashboard, which presents metrics on replicas and query performance:

<Frame>
  ![CockroachDB Overview dashboard for Metricbeat](/images/v24.3/kibana-crdb-dashboard.png)
</Frame>
...
<Frame>
  ![CockroachDB Overview dashboard for Metricbeat with SQL metrics](/images/v24.3/kibana-crdb-dashboard-sql.png)
</Frame>
