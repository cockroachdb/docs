...
3. On the **General** tab, enter your database's connection string:
    ...
    <table><thead><tr><th>Field</th><th>Value</th></tr></thead><tbody><tr><td><strong>Host</strong></td><td>Your CockroachDB cluster's hostname</td></tr><tr><td><strong>Port</strong></td><td>Your CockroachDB cluster's port. By default, CockroachDB uses port <strong>26257</strong>.</td></tr><tr><td><strong>Database</strong></td><td>The database you want to connect to. Note that CockroachDB's notion of database differs from PostgreSQL's; you can see your cluster's databases through the <a href="/docs/v24.3/show-databases"><code>SHOW DATABASES</code></a> command.</td></tr><tr><td><strong>User</strong></td><td>The user to connect as. By default, you can use <strong>root</strong>.</td></tr><tr><td><strong>Password</strong></td><td>If your cluster uses password authentication, enter the password.</td></tr><tr><td><strong>Driver</strong></td><td>Select or install <strong>PostgreSQL</strong> using a version greater than or equal to 41.1. (Older drivers have not been tested.)</td></tr></tbody></table>
...
- If possible, a trace of the SQL statements sent to CockroachDB while the error is being reproduced using [SQL query logging](/docs/v24.3/logging-use-cases#sql_exec).
