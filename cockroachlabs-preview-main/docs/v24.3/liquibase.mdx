...
old_description: "Learn how to use Liquibase with a CockroachDB cluster."
---

This page guides you through a series of simple database schema changes using the [Liquibase](https://www.liquibase.org/get-started/how-liquibase-works) command-line tool and the [CockroachDB SQL shell](/docs/v24.3/cockroach-sql).

For detailed information about using Liquibase, see the [Liquibase documentation site](https://docs.liquibase.com/home).

## Before you begin

Before you begin the tutorial, do the following:

1. [Install CockroachDB](/docs/v24.3/install-cockroachdb), and [start a secure cluster](/docs/v24.3/secure-a-cluster). When starting your cluster, make sure that you generate cluster certificates, create the `bank` database, and create the `max` user.
2. Download and install a Java Development Kit. Liquibase supports JDK versions 8+. In this tutorial, we use [AdoptOpenJDK](https://adoptopenjdk.net/) 8, but you can follow along with any JDK version 8+.
...
<Tip>
If you are using Liquibase in the context of a separate Java application, we recommend that you use a dependency management tool, like [Maven](https://docs.liquibase.com/tools-integrations/maven/home), to download the driver.
</Tip>

## Step 3. Generate TLS certificates for the `max` user

When you [started a secure CockroachDB cluster](/docs/v24.3/secure-a-cluster), you should have created a user `max`. You should have also given this user the [`admin` role](/docs/v24.3/security-reference/authorization#admin-role), which grants all privileges to all databases on the cluster. In this tutorial, Liquibase runs schema changes as the `max` user.

To authenticate connection requests to CockroachDB from the Liquibase client, you need to generate some certificates for `max`. Use [`cockroach cert`](/docs/v24.3/cockroach-cert) to generate the certificates:

```shell
$ cockroach cert create-client max --certs-dir=certs --ca-key=my-safe-directory/ca.key --also-generate-pkcs8-key
```

The [`--also-generate-pkcs8-key` flag](/docs/v24.3/cockroach-cert#flag-pkcs8) generates a key in [PKCS#8 format](https://tools.ietf.org/html/rfc5208), which is the standard key encoding format in Java. In this case, the generated PKCS8 key will be named `client.max.key.pk8`.

## Step 4: Create a changelog

Liquibase uses [changelog files](https://docs.liquibase.com/concepts/basic/changelog) to manage database schema changes. Changelog files include a list of instructions, known as [changesets](https://docs.liquibase.com/concepts/basic/changeset), that are executed against the database in a specified order. Liquibase supports XML, YAML, and SQL formats for changelogs and changesets.

Let's define a changelog with the [XML format](https://docs.liquibase.com/concepts/basic/xml-format):
...
2. Add the following to the blank `changelog-main.xml` file:
    ...
    This first changeset uses [the `sqlFile` tag](https://docs.liquibase.com/change-types/community/sql-file), which tells Liquibase that an external `.sql` file contains some [SQL statements](https://docs.liquibase.com/concepts/basic/sql-format) to execute.

    <Tip>
    CockroachDB [doesn't guarantee the atomicity of online schema changes in transactions with multiple statements](/docs/v24.3/online-schema-changes#schema-changes-within-transactions). To avoid running into issues with incomplete transactions, we recommend setting the [`runInTransaction` attribute](https://docs.liquibase.com/concepts/basic/changeset#available-attributes) to `"false"` on all changesets.
    </Tip>
3. In the same directory, create the SQL file specified by the first changeset:
    ...
    $ touch create.sql
    ```
4. Add the following [`CREATE TABLE`](/docs/v24.3/create-table) statement to the `create.sql` file:
...
5. Now let's use the [XML format](https://docs.liquibase.com/concepts/basic/xml-format) to define the second changeset. Directly after the first `changeSet` element in `changelog-main.xml`, add the following:
    ...
    This second changeset uses the [Liquibase XML syntax](https://docs.liquibase.com/concepts/basic/xml-format) to specify a series of sequential `INSERT` statements that initialize the `account` table with some values.
...
<Tip>
When possible, we recommend limiting each changeset to a single statement, per the **one change per changeset** [Liquibase best practice](https://docs.liquibase.com/concepts/bestpractices). This is especially important for [online schema changes](/docs/v24.3/online-schema-changes). For more information, see [Liquibase and transactions](#liquibase-and-transactions).
</Tip>

## Step 5. Configure a Liquibase properties file

Liquibase properties are defined in a file named [`liquibase.properties`](https://docs.liquibase.com/workflows/liquibase-community/creating-config-properties). These properties define the database connection information.
...
When the changelog is first executed, Liquibase also creates a table called [`databasechangelog`](https://docs.liquibase.com/concepts/databasechangelog-table) in the database where it performs changes. This table's rows log all completed changesets.
...
Suppose that you want to change the primary key of the `accounts` table from a simple, incrementing [integer](/docs/v24.3/int) (in this case, `id`) to an auto-generated [UUID](/docs/v24.3/uuid), to follow some [CockroachDB best practices](/docs/v24.3/performance-best-practices-overview#unique-id-best-practices). You can make these changes to the schema by creating and executing an additional changeset:

1. Create a SQL file to add a new [UUID](/docs/v24.3/uuid)-typed column to the table:
    ...
    <Tip>
    Using [SQL files](https://docs.liquibase.com/concepts/basic/sql-format) to define statements can be helpful when you want to execute statements that use syntax specific to CockroachDB.
    </Tip>
2. Add the following SQL statement to `add_uuid.sql`:
    ...
    This statement adds [a new `unique_id` column](/docs/v24.3/alter-table#add-column) to the `accounts` table, with the default value as [a randomly-generated UUID](/docs/v24.3/performance-best-practices-overview#use-functions-to-generate-unique-ids).
...
5. Add the following SQL statement to `update_pk.sql`:
    ...
    This statement [alters the `accounts` primary key](/docs/v24.3/alter-table#alter-primary-key) to use the `unique_id` column.
...
CockroachDB has [doesn't guarantee the atomicity of schema changes within transactions with multiple statements](/docs/v24.3/online-schema-changes#schema-changes-within-transactions). If a schema change fails, automatic rollbacks can lead to unexpected results. To avoid running into issues with incomplete transactions, we recommend setting the `runInTransaction` attribute on each of your changesets to `"false"`, as demonstrated throughout this tutorial.
...
When multiple, concurrent transactions or statements are issued to a single CockroachDB cluster, [transaction contention](/docs/v24.3/performance-best-practices-overview#transaction-contention) can cause schema migrations to fail. In the event of transaction contention, CockroachDB returns a `40001 SQLSTATE` (i.e., a serialization failure).

Liquibase does not automatically retry transactions. To handle transaction failures, we recommend writing client-side transaction retry logic. For more information about client-side transaction retries in CockroachDB, see [Transaction Retries](/docs/v24.3/transactions#transaction-retries).

## Liquibase integrations

You can run Liquibase in the context of a Java application framework, like Spring Boot. For examples of using Liquibase for schema management in a Spring Boot application built on CockroachDB, see [Build a Spring App with CockroachDB and JDBC](/docs/v24.3/build-a-spring-app-with-cockroachdb-jdbc) and [Build a Spring App with CockroachDB and JPA](/docs/v24.3/build-a-spring-app-with-cockroachdb-jpa).

For documentation on running Liquibase with other tooling, see [the Liquibase documentation site](https://docs.liquibase.com/workflows/liquibase-community/running).
...
- CockroachDB version ( [`cockroach version`](/docs/v24.3/cockroach-version) )
- Liquibase version
- Operating system
...
- [Liquibase documentation](https://docs.liquibase.com/home)
...
- [Build a Spring App with CockroachDB and JDBC](/docs/v24.3/build-a-spring-app-with-cockroachdb-jdbc)
- [Build a Spring App with CockroachDB and JPA](/docs/v24.3/build-a-spring-app-with-cockroachdb-jpa)
