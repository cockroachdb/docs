...
The standby cluster's reader VC has its own system tables and [cluster settings](/docs/v24.3/cluster-settings). The reader VC replicates a subset of system tables, including **Users** and **Roles**, from the app VC, so that existing primary users can authenticate using the same [users and roles](/docs/v24.3/security-reference/authorization) as on the primary cluster's app VC. Other system tables and cluster settings are set to defaults in the reader VC. For more information, consult [Physical Cluster Replication Technical Overview](/docs/v24.3/physical-cluster-replication-technical-overview).
...
- You have already configured PCR between a *primary* cluster and a *standby* cluster. For information on configuring PCR, refer to [Set Up Physical Cluster Replication](/docs/v24.3/set-up-physical-cluster-replication).
...
To start a PCR stream that allows read access to the standby cluster, use the [`CREATE VIRTUAL CLUSTER... REPLICATION`](/docs/v24.3/create-virtual-cluster) statement with the `READ VIRTUAL CLUSTER` option:
...
To add read from standby capabilities to an existing PCR stream, use the [`ALTER VIRTUAL CLUSTER`](/docs/v24.3/alter-virtual-cluster) statement:
...
<Note>
The standby cluster's app VC must have a status of `replicating` before you can create your reader VC. Use the [`SHOW VIRTUAL CLUSTERS`](/docs/v24.3/show-virtual-cluster) command to check the status of the app VC.
</Note>
...
Once you have created a reader virtual cluster on the standby cluster, you can connect to it and run [read (`SELECT`) queries](/docs/v24.3/selection-queries). For example:
...
The results of queries on the standby cluster reflect the state of the primary cluster as of a historical time that approaches the [replicated time](/docs/v24.3/show-virtual-cluster#show-replication-status).
