...
import { Insert7 } from "/snippets/syntax-diagrams/Insert7.jsx";
import { Insert8 } from "/snippets/syntax-diagrams/Insert8.jsx";

The `INSERT` [statement](/docs/v24.3/sql-statements) inserts one or more rows into a table. In cases where inserted values conflict with uniqueness constraints, the `ON CONFLICT` clause can be used to update rather than insert rows.

## Required privileges

The user must have the `INSERT` [privilege](/docs/v24.3/security-reference/authorization#managing-privileges) on the table. To use `ON CONFLICT`, the user must also have the `SELECT` privilege on the table. To use `ON CONFLICT DO UPDATE`, the user must additionally have the `UPDATE` privilege on the table.
...
As an alternative to `INSERT... ON CONFLICT... DO UPDATE`, you can use the [`UPSERT`](/docs/v24.3/upsert) statement. For example, the following statements are equivalent:
...
However, `UPSERT` does not let you specify columns to infer a unique constraint as an arbiter. An arbiter is a [`UNIQUE`](/docs/v24.3/unique) constraint used to check for conflicts during execution of `INSERT... ON CONFLICT`. `UPSERT` always uses the primary key as the arbiter. You must therefore use `INSERT... ON CONFLICT... DO UPDATE` if your statement considers uniqueness for columns other than primary key columns. For an example, see [Upsert that fails (conflict on non-primary key)](/docs/v24.3/upsert#upsert-that-fails-conflict-on-non-primary-key).
...
- **Existing table**
    - Perform a [multi-row `INSERT`](#insert-multiple-rows-into-an-existing-table) in one statement in an [implicit transaction](/docs/v24.3/transactions#individual-statements).
    - Do not use large batches of 100,000 rows or more, which can lead to long-running transactions that result in [transaction retry errors](/docs/v24.3/transaction-retry-error-reference). If a multi-row `INSERT` results in an error code [`40001` with the message `"transaction deadline exceeded"`](/docs/v24.3/transaction-retry-error-reference#retry_commit_deadline_exceeded), Cockroach Labs recommends that you break up the `INSERT` into smaller batches.
    ...
    - You can also use the [`IMPORT INTO`](/docs/v24.3/import-into) statement to bulk-insert CSV data.
...
To check the [default values](/docs/v24.3/default-value) for columns in a table, use the [`SHOW CREATE TABLE`](/docs/v24.3/show-create) statement:
...
In this example, the `RETURNING` clause returns the `id` values of the rows inserted, which are generated server-side by the `gen_random_uuid()` function. The language-specific versions assume that you have installed the relevant [client drivers](/docs/v24.3/install-client-drivers).
...
If the input data for `INSERT ON CONFLICT` contains duplicate rows, you must use [`DISTINCT ON`](/docs/v24.3/select-clause#eliminate-duplicate-rows) to remove these duplicates.
