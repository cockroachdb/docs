...
- **Decommission a node** to permanently remove it from the cluster, such as when scaling down the cluster or to replace the node due to hardware failure. During decommission:
    ...
    - The node's data is moved off the node to other nodes. This [replica rebalancing](/docs/v24.3/architecture/glossary#replica) generates a large amount of node-to-node network traffic, so decommissioning a node is considered a heavyweight operation.
...
An operator [initiates the draining process](#drain-the-node-and-terminate-the-node-process) on the node. Draining a node disconnects clients after active queries are completed, and transfers any [range leases](/docs/v24.3/architecture/replication-layer#leases) and [Raft leaderships](/docs/v24.3/architecture/replication-layer#raft) to other nodes, but does not move replicas or data off of the node.
...
<Warning>
Do not terminate the `cockroach` process before all of the phases of draining are complete. Otherwise, you may experience latency spikes until the [leases](/docs/v24.3/architecture/glossary#leaseholder) that were on that node have transitioned to other nodes. It is safe to terminate the `cockroach` process only after a node has completed the drain process. This is especially important in a containerized system, to allow all TCP connections to terminate gracefully. If necessary, adjust the [`server.shutdown.initial_wait`](#server-shutdown-initial_wait) and the [termination grace period](/docs/v24.3/node-shutdown?filters=decommission#termination-grace-period) cluster settings and adjust your process manager or other deployment tooling to allow adequate time for the node to finish draining before it is terminated or restarted.
</Warning>

Node drain consists of the following consecutive phases:

1. **Unready phase:** The node's [`/health?ready=1` endpoint](/docs/v24.3/monitoring-and-alerting#health-ready-1) returns an HTTP `503 Service Unavailable` response code, which causes load balancers and connection managers to reroute traffic to other nodes. This phase completes when the [fixed duration set by `server.shutdown.initial_wait`](#server-shutdown-initial_wait) is reached.
...
3. **SQL drain phase:** All active transactions and statements for which the node is a [gateway](/docs/v24.3/architecture/life-of-a-distributed-transaction#gateway) are allowed to complete, and CockroachDB closes the SQL client connections immediately afterward. After this phase completes, CockroachDB closes all remaining SQL client connections to the node. This phase completes either when all transactions have been processed or the [maximum duration set by `server.shutdown.transactions.timeout`](#server-shutdown-transactions-timeout) is reached.
4. **DistSQL drain phase**: All [distributed statements](/docs/v24.3/architecture/sql-layer#distsql) initiated on other gateway nodes are allowed to complete, and DistSQL requests from other nodes are no longer accepted. This phase completes either when all transactions have been processed or the [maximum duration set by `server.shutdown.transactions.timeout`](#server-shutdown-transactions-timeout) is reached.
5. **Lease transfer phase:** The node's [`is_draining`](/docs/v24.3/cockroach-node#node-status) field is set to `true`, which removes the node as a candidate for replica rebalancing, lease transfers, and query planning. Any [range leases](/docs/v24.3/architecture/replication-layer#leases) or [Raft leaderships](/docs/v24.3/architecture/replication-layer#raft) must be transferred to other nodes. This phase completes when all range leases and Raft leaderships have been transferred.
...
- If the node was drained manually because an operator issued a `cockroach node drain` command:
    - <InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v24.2: If you pass the `--shutdown` flag to [`cockroach node drain`](/docs/v24.3/cockroach-node#flags), the `cockroach` process terminates automatically after draining completes.
...
Your [load balancer](/docs/v24.3/recommended-production-settings#load-balancing) should use the [`/health?ready=1` endpoint](/docs/v24.3/monitoring-and-alerting#health-ready-1) to actively monitor node health and direct SQL client connections away from draining nodes.
...
Change this setting **only** if you cannot tolerate connection errors during node drain and cannot configure the maximum lifetime of SQL client connections, which is usually configurable via a [connection pool](/docs/v24.3/connection-pooling#about-connection-pools). Depending on your requirements:
...
<Tip>
If there are still open transactions on the draining node when the server closes its connections, you will encounter errors. You may need to adjust your application server's [connection pool](/docs/v24.3/connection-pooling#about-connection-pools) settings.
</Tip>
...
In the ["lease transfer phase"](#draining) of node drain, the server attempts to transfer all [range leases](/docs/v24.3/architecture/replication-layer#leases) and [Raft leaderships](/docs/v24.3/architecture/replication-layer#raft) from the draining node. `server.shutdown.lease_transfer_iteration.timeout` sets the maximum duration of each iteration of this attempt (`5s` by default). Because this phase does not exit until all transfers are completed, changing this value affects only the frequency at which drain progress messages are printed.
...
After completing the maintenance work and [restarting the nodes](/docs/v24.3/cockroach-start), you would then change the setting back to its default:
...
- When using [Kubernetes](/docs/v24.3/kubernetes-overview) to orchestrate CockroachDB, refer to [Decommissioning and draining on Kubernetes](#decommissioning-and-draining-on-kubernetes).
...
- In general, we recommend setting the termination grace period to the sum of all `server.shutdown.*` settings. If a node requires more time than this to drain successfully, this may indicate a technical issue such as inadequate [cluster sizing](/docs/v24.3/recommended-production-settings#sizing).
...
<InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v24.2: If you passed the `--shutdown` flag to [`cockroach node drain`](/docs/v24.3/cockroach-node#flags), the `cockroach` process terminates automatically after draining completes. Otherwise, terminate the `cockroach` process.
...
- If a decommissioning node is forcibly terminated before decommission completes, [ranges will be under-replicated](/docs/v24.3/monitoring-and-alerting#critical-nodes-endpoint) and the cluster is at risk of [loss of quorum](/docs/v24.3/architecture/replication-layer#overview) if an additional node experiences an outage in the window before up-replication completes.
- If a draining or decommissioning node is forcibly terminated before the operation completes, it can corrupt log files and, in certain edge cases, can result in temporary data unavailability, latency spikes, [uncertainty errors](/docs/v24.3/transaction-retry-error-reference#readwithinuncertaintyintervalerror), [ambiguous commit errors](/docs/v24.3/common-errors#result-is-ambiguous), or query timeouts.
</Warning>
...
After you initiate a node shutdown or restart, the node's progress is regularly logged to the [default logging destination](/docs/v24.3/logging-overview#logging-destinations) until the operation is complete. The following sections provide additional ways to monitor the operation's progress.

### `OPS`

During node shutdown, progress messages are generated in the [`OPS` logging channel](/docs/v24.3/logging-overview#logging-channels). The frequency of these messages is configured with [`server.shutdown.lease_transfer_iteration.timeout`](#server-shutdown-lease_transfer_iteration-timeout). [By default](/docs/v24.3/configure-logs#default-logging-configuration), the `OPS` logs output to a `cockroach.log` file.
...
Draining status is reflected in the [`cockroach node status --decommission`](/docs/v24.3/cockroach-node) output:
...
To drain and shut down a node that was started in the foreground with [`cockroach start`](/docs/v24.3/cockroach-start):
...
1. Press `ctrl-c` in the terminal where the node is running.
    ...
    server drained and shutdown completed
    ```
2. Filter the logs for draining progress messages. [By default](/docs/v24.3/configure-logs#default-logging-configuration), the `OPS` logs output to a `cockroach.log` file:
...
3. Start the node to have it rejoin the cluster.
    ...
    Re-run the [`cockroach start`](/docs/v24.3/cockroach-start) command that you used to start the node initially. For example:
...
You can use [`cockroach node drain`](/docs/v24.3/cockroach-node) to drain a node separately from decommissioning the node or terminating the node process.
...
1. Run the `cockroach node drain` command, specifying the ID of the node to drain (and optionally a custom [drain timeout](#drain-timeout) to allow draining more time to complete). <InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v24.2: You can optionally pass the `--shutdown` flag to [`cockroach node drain`](/docs/v24.3/cockroach-node#flags) to automatically terminate the `cockroach` process after draining completes.
    ...
    ok
    ```
2. Filter the logs for shutdown progress messages. [By default](/docs/v24.3/configure-logs#default-logging-configuration), the `OPS` logs output to a `cockroach.log` file:
...
The node's [`is_decommissioning`](/docs/v24.3/cockroach-node#node-status) field is set to `true` and its `membership` status is set to `decommissioning`, which causes its replicas to be rebalanced to other nodes. If the rebalancing stalls during decommissioning, replicas that have yet to move are printed to the [SQL shell](/docs/v24.3/cockroach-sql) and written to the [`OPS` logging channel](/docs/v24.3/logging-overview#logging-channels). [By default](/docs/v24.3/configure-logs#default-logging-configuration), the `OPS` channel logs output to a `cockroach.log` file.

The node's [`/health?ready=1` endpoint](/docs/v24.3/monitoring-and-alerting#health-ready-1) continues to consider the node "ready" so that the node can function as a gateway to route SQL client connections to relevant data.
...
After draining and decommissioning are complete, an operator [terminates the node process](/docs/v24.3/node-shutdown?filters=decommission#terminate-the-node-process).

A node **process termination** stops the `cockroach` process on the node. The node will stop updating its liveness record.

A node that stays offline for the duration set by the `server.time_until_store_dead` [cluster setting](/docs/v24.3/cluster-settings) (5 minutes by default) is usually considered "dead" by the cluster. However, a decommissioned node retains **decommissioned** status.
...
Before decommissioning a node, make sure other nodes are available to take over the range replicas from the node. If fewer nodes are available than the replication factor, CockroachDB will automatically reduce the replication factor (for example, from 5 to 3) to try to allow the decommission to succeed. However, the replication factor will not be reduced lower than 3. If three nodes are not available, the decommissioning process will hang indefinitely until nodes are added or you update the [zone configurations](/docs/v24.3/configure-replication-zones#num_replicas) to use a replication factor of 1.
...
This can lead to disk utilization imbalance across nodes. **This is expected behavior**, since disk utilization per node is not one of the rebalancing criteria. For more information, see [Disk utilization is different across nodes in the cluster](/docs/v24.3/cluster-setup-troubleshooting#disk-utilization-is-different-across-nodes-in-the-cluster).
...
<Frame>
  ![Decommission Scenario 1](/images/v24.3/decommission-scenario1.1.png)
</Frame>
...
<Frame>
  ![Decommission Scenario 1](/images/v24.3/decommission-scenario1.2.png)
</Frame>
...
<Frame>
  ![Decommission Scenario 1](/images/v24.3/decommission-scenario1.3.png)
</Frame>
...
<Frame>
  ![Decommission Scenario 1](/images/v24.3/decommission-scenario2.1.png)
</Frame>
...
<Frame>
  ![Decommission Scenario 1](/images/v24.3/decommission-scenario2.2.png)
</Frame>
...
These checks can be skipped by [passing the flag `--checks=skip` to `cockroach node decommission`](/docs/v24.3/cockroach-node#decommission-checks).
...
Node decommission progress is reported in [`node_decommissioning`](/docs/v24.3/eventlog#node_decommissioning) and [`node_decommissioned`](/docs/v24.3/eventlog#node_decommissioned) events:
...
Draining and decommissioning statuses are reflected in the [`cockroach node status --decommission`](/docs/v24.3/cockroach-node) output:
...
- Before decommissioning nodes, verify that there are no [under-replicated or unavailable ranges](/docs/v24.3/ui-cluster-overview-page#cluster-overview-panel) on the cluster.
- Do **not** terminate the node process, delete the storage volume, or remove the VM before a `decommissioning` node has [changed its membership status](#status-change) to `decommissioned`. Prematurely terminating the process will prevent the node from rebalancing all of its range replicas onto other nodes gracefully, cause transient query errors in client applications, and leave the remaining ranges under-replicated and vulnerable to loss of [quorum](/docs/v24.3/architecture/replication-layer#overview) if another node goes down.
...
Open the **Cluster Overview** page of the DB Console and [note the node IDs](/docs/v24.3/ui-cluster-overview-page#node-details) of the nodes you want to decommission.
...
Run the [`cockroach node decommission`](/docs/v24.3/cockroach-node) command with the IDs of the nodes to decommission:
...
Once the nodes complete decommissioning, they will appear in the list of [**Recently Decommissioned Nodes**](/docs/v24.3/ui-cluster-overview-page#decommissioned-nodes) in the DB Console.
...
Alternatively, open the **Cluster Overview** page of the DB Console and check that the [node status](/docs/v24.3/ui-cluster-overview-page#node-status) of the node is `DEAD`.

#### Step 2. Decommission the dead node

Run the [`cockroach node decommission`](/docs/v24.3/cockroach-node) command against the address of any live node, specifying the ID of the dead node:
...
Once the node completes decommissioning, it will appear in the list of [**Recently Decommissioned Nodes**](/docs/v24.3/ui-cluster-overview-page#decommissioned-nodes) in the DB Console.
...
Run the [`cockroach node recommission`](/docs/v24.3/cockroach-node) command with the ID of the node to recommission:
...
On the **Cluster Overview** page of the DB Console, the [node status](/docs/v24.3/ui-cluster-overview-page#node-status) of the node should be `LIVE`. After a few minutes, you should see replicas rebalanced to the nodes.
...
Refer to [Termination grace period on Kubernetes](#termination-grace-period-on-kubernetes). For more details about managing CockroachDB on Kubernetes, refer to [Cluster upgrades](/docs/v24.3/upgrade-cockroachdb-kubernetes) and [Cluster scaling](/docs/v24.3/scale-cockroachdb-kubernetes).
...
- [Upgrade CockroachDB](/docs/v24.3/upgrade-cockroach-version)
- [`cockroach node`](/docs/v24.3/cockroach-node)
- [`cockroach start`](/docs/v24.3/cockroach-start)
- [Node status](/docs/v24.3/ui-cluster-overview-page#node-status)
- [Replication Layer](/docs/v24.3/architecture/replication-layer)
- [Decommissioning issues](/docs/v24.3/cluster-setup-troubleshooting#decommissioning-issues)
