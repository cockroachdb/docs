...
<InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v23.2: Enable [cluster virtualization](/docs/v24.3/cluster-virtualization-overview) in your CockroachDB cluster to set up a [**physical cluster replication (PCR)**](/docs/v24.3/physical-cluster-replication-overview) stream. This page is a guide to working with virtual clusters.

## Connect to a virtual cluster

This section shows how to use [SQL clients](#sql-clients) or the [DB Console](#db-console) to connect to a virtual cluster.

When [PCR](/docs/v24.3/physical-cluster-replication-overview) is enabled, the virtual cluster is named `main`.

### SQL clients

This section shows how to connect using [`cockroach sql`](/docs/v24.3/cockroach-sql) when cluster virtualization is enabled.

Unless you specify which virtual cluster to connect to, when you connect using a SQL client, you are logged into the default virtual cluster. When [PCR](/docs/v24.3/physical-cluster-replication-overview) is enabled, the default virtual cluster is named `main`.
...
This section shows how to connect using the [DB Console](/docs/v24.3/ui-overview) when cluster virtualization is enabled.

Unless you specify which virtual cluster to connect to, when you connect using the DB Console, you are logged into the default virtual cluster. When [PCR](/docs/v24.3/physical-cluster-replication-overview) is enabled, the default virtual cluster is named `main`. In order to view metrics on [replication lag](/docs/v24.3/physical-cluster-replication-technical-overview) for a PCR job, connect to the standby cluster's DB Console.
...
To [grant](/docs/v24.3/grant) access to the system virtual cluster, you must connect to the system virtual cluster as a user with the `admin` role, then grant either of the following to the SQL user:

- The `admin` [role](/docs/v24.3/security-reference/authorization#admin-role) grants the ability to read and modify system tables and cluster settings on any virtual cluster, including the system virtual cluster.
- The `VIEWSYSTEMDATA` [system privilege](/docs/v24.3/security-reference/authorization#supported-privileges) grants the ability to read system tables and cluster settings on any virtual cluster, including the system virtual cluster.
...
<Note>
Both the `auto_upgrade.enabled` and `preserve_downgrade_option` [cluster settings](/docs/v24.3/cluster-settings) are scoped to a virtual cluster. To prevent automatic finalization of an upgrade, you must do one of the following:

- Set `auto_upgrade.enabled` to `false` before beginning an upgrade. This setting persists after an upgrade. To finalize the upgrade manually, refer to [Upgrade a cluster](/docs/v24.3/upgrade-cockroach-version). This is the preferred way to disable auto-upgrade.
