...
- The schema change runs as a [background job](/docs/v24.3/show-jobs) without holding locks on the underlying table data.
- Your application's queries can run normally, with no effect on read/write latency. The schema is cached for performance.
- Your data is kept in a safe, [consistent](/docs/v24.3/frequently-asked-questions#how-is-cockroachdb-strongly-consistent) state throughout the entire schema change process.
...
1. You initiate a schema change by executing [`ALTER TABLE`](/docs/v24.3/alter-table), [`CREATE INDEX`](/docs/v24.3/create-index), [`TRUNCATE`](/docs/v24.3/truncate), etc.
2. The schema change engine converts the original schema to the new schema in discrete steps while ensuring that the underlying table data is always in a consistent state. These changes are executed as a [background job](/docs/v24.3/show-jobs), and can be [paused](/docs/v24.3/pause-job), [resumed](/docs/v24.3/resume-job), and [canceled](/docs/v24.3/cancel-job).
...
- The following statements:
    - [`ADD COLUMN`](/docs/v24.3/alter-table#add-column) when the statement also features `INDEX` or `UNIQUE`.
    - [`ALTER PRIMARY KEY`](/docs/v24.3/alter-table#alter-primary-key)
    - [`CREATE INDEX`](/docs/v24.3/create-index)
    - [`CREATE MATERIALIZED VIEW`](/docs/v24.3/views#materialized-views)
    - [`CREATE TABLE AS`](/docs/v24.3/create-table-as)
    - [`REFRESH`](/docs/v24.3/refresh)
    - [`SET LOCALITY`](/docs/v24.3/alter-table#set-locality) under one of the following conditions:
    - The locality changes from [`REGIONAL BY ROW`](/docs/v24.3/alter-table#regional-by-row) to something that is not `REGIONAL BY ROW`.
...
To estimate the size of the indexes in your table, use the [`SHOW RANGES`](/docs/v24.3/show-ranges) statement.
...
1. Start a 3 node [`cockroach demo`](/docs/v24.3/cockroach-demo) cluster with the MovR dataset.
...
Online schema changes that result in large backfill operations (for example, [`ALTER TABLE... ALTER COLUMN`](/docs/v24.3/alter-table#alter-column) statements) are computationally expensive, and can result in degraded performance. The [admission control system](/docs/v24.3/admission-control) will help keep high-priority operations running, but it's recommended to run backfill-heavy schema changes during times when the cluster is under relatively low loads.

### Schema changes in multi-region clusters

To reduce latency while making online schema changes, we recommend specifying a `lease_preference` [zone configuration](/docs/v24.3/configure-replication-zones) on the `system` database to a single region and running all subsequent schema changes from a node within that region. For example, if the majority of online schema changes come from machines that are geographically close to `us-east1`, run the following:
...
<Tip>
For more examples of schema change statements, see the [`ALTER TABLE`](/docs/v24.3/alter-table) subcommands.
</Tip>
...
However, you can run schema changes inside the same transaction as a [`CREATE TABLE`](/docs/v24.3/create-table) statement. For example:
...
Some schema changes can be used in combination in a single `ALTER TABLE` statement. For a list of commands that can be combined, see [`ALTER TABLE`](/docs/v24.3/alter-table). For a demonstration, see [Add and rename columns atomically](/docs/v24.3/alter-table#add-and-rename-columns-atomically).

### Show all schema change jobs

You can check on the status of the schema change jobs on your system at any time using the [`SHOW JOBS`](/docs/v24.3/show-jobs) statement:
...
All schema change jobs can be [paused](/docs/v24.3/pause-job), [resumed](/docs/v24.3/resume-job), and [canceled](/docs/v24.3/cancel-job).

### Enable automatic commit before running schema changes inside transactions

When the [`autocommit_before_ddl` session setting](/docs/v24.3/set-vars#autocommit-before-ddl) is set to `on`, any schema change statement that is sent during an [explicit transaction](/docs/v24.3/transactions) will cause the transaction to [commit](/docs/v24.3/commit-transaction) before executing the schema change.
...
- Use schema changes more easily under the [`READ COMMITTED` isolation level](/docs/v24.3/read-committed). The error message returned when running schema changes under `READ COMMITTED` isolation includes a hint to use this setting.

With `autocommit_before_ddl` enabled, [`COMMIT`](/docs/v24.3/commit-transaction), [`ROLLBACK`](/docs/v24.3/rollback-transaction), and other statements that normally return **errors** when used outside of an explicit transaction will instead return **warnings**. This behavior change is necessary because this setting can cause a transaction to end earlier than a client application may expect.

To enable this setting for the current [session](/docs/v24.3/show-sessions):

```sql
SET autocommit_before_ddl = on;
```

To enable it for [all users](/docs/v24.3/alter-role#set-default-session-variable-values-for-all-users):

```sql
ALTER ROLE ALL SET autocommit_before_ddl = on
```

You can also enable the setting [from your application's connection string](/docs/v24.3/connection-parameters#supported-options-parameters).
...
Prior to [garbage collection](/docs/v24.3/architecture/storage-layer#garbage-collection), it's possible to recover data that may have been lost prior to schema changes by using the [`AS OF SYSTEM TIME`](/docs/v24.3/as-of-system-time) parameter. However, this solution is limited in terms of time, and doesn't work beyond the designated garbage collection window.

For more long-term recovery solutions, consider taking either a [full or incremental backup](/docs/v24.3/take-full-and-incremental-backups) of your cluster.
...
- [`ALTER DATABASE`](/docs/v24.3/alter-database)
- [`ALTER INDEX`](/docs/v24.3/alter-index)
- [`ALTER RANGE`](/docs/v24.3/alter-range)
...
- [`DROP VIEW`](/docs/v24.3/drop-view)
