...
<Tip>
To deploy CockroachDB in a single Kubernetes cluster instead, see [Kubernetes Single-Cluster Deployment](/docs/v24.3/deploy-cockroachdb-with-kubernetes). Also, for details about potential performance bottlenecks to be aware of when running CockroachDB in Kubernetes and guidance on how to optimize your deployment for better performance, see [CockroachDB Performance on Kubernetes](/docs/v24.3/kubernetes-performance).
</Tip>
...
To deploy CockroachDB v24.3, Kubernetes 1.18 or higher is required. Cockroach Labs strongly recommends that you use a Kubernetes version that is [eligible for patch support by the Kubernetes project](https://kubernetes.io/releases).

#### Kubernetes Operator

- The CockroachDB Kubernetes Operator currently deploys clusters in a single region. For multi-region deployments using manual configs, see [Orchestrate CockroachDB Across Multiple Kubernetes Clusters](/docs/v24.3/orchestrate-cockroachdb-with-kubernetes-multi-cluster).
...
4. If you haven't already, [install CockroachDB locally and add it to your `PATH`](/docs/v24.3/install-cockroachdb). The `cockroach` binary will be used to generate certificates.
...
5. Optionally, to optimize your deployment for better performance, review [CockroachDB Performance on Kubernetes](/docs/v24.3/kubernetes-performance) and make the desired modifications to the `cockroachdb-statefulset-secure.yaml` file.
...
To access the cluster's [DB Console](/docs/v24.3/ui-overview):

1. On secure clusters, [certain pages of the DB Console](/docs/v24.3/ui-overview#db-console-access) can only be accessed by `admin` users.

    Get a shell into the pod with the `cockroach` binary created earlier and start the CockroachDB [built-in SQL client](/docs/v24.3/cockroach-sql):
...
6. In the UI, check the [**Node List**](/docs/v24.3/ui-cluster-overview-page#node-list) to verify that all nodes are running, open the [**Databases** page](/docs/v24.3/ui-databases-page) to verify that `bank` is listed, and open the [**Network Latency** page](/docs/v24.3/ui-network-latency-page) to see the performance of your CockroachDB cluster across 3 regions.
...
Each of your Kubernetes clusters contains 3 instances that can run CockroachDB pods. It's easy to scale a cluster to run more pods. To ensure that you do not have two CockroachDB pods on the same instance (as recommended in our [production best practices](/docs/v24.3/recommended-production-settings)), you need to first add a new instance and then edit your StatefulSet configuration to add another pod.
...
1. Verify that you can upgrade.
    ...
    Therefore, in order to upgrade to v24.3, you must be on a production release of v24.2.

    1. If you are upgrading to v24.3 from a production release earlier than v24.2, or from a testing release (alpha/beta), first [upgrade to a production release of v24.2](https://www.cockroachlabs.com/docs/v24.2/orchestrate-cockroachdb-with-kubernetes-multi-cluster#upgrade-the-cluster). Be sure to complete all the steps.
    2. Then return to this page and perform a second upgrade to v24.3.
    3. If you are upgrading from any production release of v24.2, or from any earlier v24.3 release, you do not have to go through intermediate releases; continue to step 2.
2. Verify the overall health of your cluster using the [DB Console](/docs/v24.3/ui-overview). On the **Overview**:
...
3. Review the backward-incompatible changes in v24.3 and deprecated features. If any affect your deployment, make the necessary changes before starting the rolling upgrade to v24.3.
4. Decide how the upgrade will be finalized.

    By default, after all nodes are running the new version, the upgrade process will be **auto-finalized**. This will enable certain features and performance improvements introduced in v24.3. After finalization, however, it will no longer be possible to perform a downgrade to v24.2. In the event of a catastrophic failure or corruption, the only option is to start a new cluster using the old binary and then restore from a [backup](/docs/v24.3/take-full-and-incremental-backups) created prior to the upgrade. For this reason, **we recommend disabling auto-finalization** so you can monitor the stability and performance of the upgraded cluster before finalizing the upgrade, but note that you will need to follow all of the subsequent directions, including the manual finalization in a later step.

    <Note>
    Finalization only applies when performing a major version upgrade (for example, from v24.2.x to v24.3). Patch version upgrades (for example, within the v24.3.x series) can always be downgraded.
    </Note>

    1. Get a shell into the pod with the `cockroach` binary created earlier and start the CockroachDB [built-in SQL client](/docs/v24.3/cockroach-sql):

        ```shell
        $ kubectl exec -it cockroachdb-client-secure --context <cluster-context> --namespace <cluster-namespace> -- ./cockroach sql --certs-dir=/cockroach-certs --host=cockroachdb-public
        ```
    2. Set the `cluster.preserve_downgrade_option` [cluster setting](/docs/v24.3/cluster-settings):
...
7. If you disabled auto-finalization earlier, monitor the stability and performance of your cluster until you are comfortable with the upgrade (generally at least a day).
    ...
    <Note>
    This is only possible when performing a major version upgrade (for example, from v24.2.x to v24.3). Patch version upgrades (for example, within the v24.3.x series) are auto-finalized.
    </Note>
...
<Note>
The below steps use [`cockroach cert` commands](/docs/v24.3/cockroach-cert) to quickly generate and sign the CockroachDB node and client certificates. Read our [Authentication](/docs/v24.3/authentication#using-digital-certificates-with-cockroachdb) docs to learn about other methods of signing certificates.
</Note>
...
4. The StatefulSet configuration includes a [`cockroach start`](/docs/v24.3/cockroach-start) command that creates the nodes on the Kubernetes pods.
...
2. Get a shell into the pod and start the CockroachDB [built-in SQL client](/docs/v24.3/cockroach-sql), again specifying the namespace and context of the Kubernetes cluster where the pod is running:
    ...
    ```
3. Run some basic [CockroachDB SQL statements](/docs/v24.3/learn-cockroachdb-sql):
    ...
    (1 row)
    ```
4. [Create a user with a password](/docs/v24.3/create-user#create-a-user-with-a-password):
...
5. Exit the SQL shell and pod:
    ...
    The pod will continue running indefinitely, so any time you need to reopen the built-in SQL client or run any other [`cockroach` client commands](/docs/v24.3/cockroach-commands) (e.g., `cockroach node`), repeat step 2 using the appropriate command.
...
- [Kubernetes Single-Cluster Deployment](/docs/v24.3/deploy-cockroachdb-with-kubernetes)
- [Kubernetes Performance Guide](/docs/v24.3/kubernetes-performance)
