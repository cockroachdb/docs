...
[PL/pgSQL](https://www.postgresql.org/docs/16/plpgsql-overview) is a procedural language that you can use within [user-defined functions](/docs/v24.3/user-defined-functions) and [stored procedures](/docs/v24.3/stored-procedures) in CockroachDB.

In contrast to [SQL statements](/docs/v24.3/sql-statements), which are issued one-by-one from the client to the database, PL/pgSQL statements are encapsulated in a [block structure](#structure) and executed on the database side, thus reducing network latency. PL/pgSQL enables more complex functionality than standard SQL, including [conditional statements](#write-conditional-statements), [loops](#write-loops), and [exception handling](#report-messages-and-handle-exceptions).
...
A function or procedure that uses PL/pgSQL must specify the `PLpgSQL` language within the [`CREATE FUNCTION`](/docs/v24.3/create-function) or [`CREATE PROCEDURE`](/docs/v24.3/create-procedure) statement:
...
- An optional `DECLARE` section that contains [variable declarations](#declare-a-variable) for all variables that are used within the block and are not defined as [`CREATE FUNCTION`](/docs/v24.3/create-function) or [`CREATE PROCEDURE`](/docs/v24.3/create-procedure) parameters.
- A [function](/docs/v24.3/user-defined-functions) or [procedure](/docs/v24.3/stored-procedures) body, consisting of statements enclosed by `BEGIN` and `END`.
...
- `data_type` can be a supported [SQL data type](/docs/v24.3/data-types), [user-defined type](/docs/v24.3/create-type), or the PL/pgSQL `REFCURSOR` type, when declaring [cursor](#declare-cursor-variables) variables.
...
Use the PL/pgSQL `INTO` clause to assign a result of a [`SELECT`](/docs/v24.3/select-clause) or mutation ([`INSERT`](/docs/v24.3/insert), [`UPDATE`](/docs/v24.3/update), [`DELETE`](/docs/v24.3/delete)) statement to a specified variable. The optional `STRICT` clause specifies that the statement must return exactly one row; otherwise, the function or procedure will error. This behavior can be enabled by default using the [`plpgsql_use_strict_into`](/docs/v24.3/session-variables#plpgsql-use-strict-into) session setting.
...
- `target` is an arbitrary variable name. This can be a list of comma-separated variables, or a single [composite variable](/docs/v24.3/create-type#create-a-composite-data-type).

For example, given a table `t` with `INT` column `col`:

The following [stored procedure](/docs/v24.3/stored-procedures) inserts a specified value `x` into the table, and the `INTO` clause assigns the [returned value](/docs/v24.3/insert#insert-and-return-values) to `i`.
...
The following [user-defined function](/docs/v24.3/user-defined-functions) uses the `max` [built-in function](/docs/v24.3/functions-and-operators#aggregate-functions) to find the maximum `col` value in table `t`, and assigns the result to `i`.
...
For an example, see [Create a stored procedure that uses conditional logic](/docs/v24.3/create-procedure#create-a-stored-procedure-that-uses-conditional-logic).
...
For an example, see [Create a stored procedure that uses a `WHILE` loop](/docs/v24.3/create-procedure#create-a-stored-procedure-that-uses-a-while-loop).
...
- When specified as a parameter in a [`CREATE PROCEDURE`](/docs/v24.3/create-procedure) statement, cursors can be accessed externally from the stored procedure.
...
For example, using the [`movr` dataset](/docs/v24.3/movr) loaded by [`cockroach demo`](/docs/v24.3/cockroach-demo):
...
The [`CALL`](/docs/v24.3/call) and `FETCH` statements have to be issued within the same transaction, or the cursor will not be found:
...
Use a `COMMIT` or `ROLLBACK` statement within a PL/pgSQL [stored procedure](/docs/v24.3/stored-procedures) to finish the current transaction and automatically start a new one.
...
- They must be used inside a PL/pgSQL [stored procedure](/docs/v24.3/stored-procedures).
- They cannot be inside a [block](#structure) with an [`EXCEPTION`](#write-exception-logic) clause.
- The procedure must be called directly in a [`CALL`](/docs/v24.3/call) SQL statement. It cannot be [called by another stored procedure](#call-a-procedure).
- The procedure must be called from an [implicit transaction](/docs/v24.3/transactions#individual-statements); i.e., the call cannot be enclosed by [`BEGIN`](/docs/v24.3/begin-transaction) and [`COMMIT`](/docs/v24.3/commit-transaction) SQL statements.
...
Use one or more optional `SET TRANSACTION` statements to set the [priority, isolation level, timestamp, or read-only status](/docs/v24.3/set-transaction#parameters) of a PL/pgSQL transaction. In PL/pgSQL, `SET TRANSACTION` statements **must** directly follow `COMMIT` or `ROLLBACK`, or another `SET TRANSACTION` statement; and must precede the other statements in the transaction.
...
Use a `CALL` statement to call a procedure from within a PL/pgSQL [function](/docs/v24.3/user-defined-functions) or [procedure](/docs/v24.3/stored-procedures).
...
A procedure with `OUT` parameters can only be called from a PL/pgSQL routine. For another example, see [Create a stored procedure that calls a procedure](/docs/v24.3/create-procedure#create-a-stored-procedure-that-calls-a-procedure).

## Examples

### Create a user-defined function using PL/pgSQL

The following [user-defined function](/docs/v24.3/user-defined-functions) returns the `n`th integer in the Fibonacci sequence.

It uses the PL/pgSQL [`LOOP`](/docs/v24.3/plpgsql#write-loops) syntax to iterate through a simple calculation, and [`RAISE EXCEPTION`](/docs/v24.3/plpgsql#report-messages-and-handle-exceptions) to return an error message if the specified `n` is negative.
...
The following [stored procedure](/docs/v24.3/user-defined-functions) removes a specified number of earliest rides in `vehicle_location_histories`.

It uses the PL/pgSQL [`WHILE`](/docs/v24.3/plpgsql#write-loops) syntax to iterate through the rows, [`RAISE`] to return notice and error messages, and `REFCURSOR` to define a cursor that fetches the next rows to be affected by the procedure.
...
For more details on this example, see the [Stored Procedures documentation](/docs/v24.3/stored-procedures#example-details).
