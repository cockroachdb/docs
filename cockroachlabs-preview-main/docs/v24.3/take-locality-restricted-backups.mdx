...
The `EXECUTION LOCALITY` option allows you to restrict the nodes that can execute a [backup](/docs/v24.3/backup) job by using a [locality filter](/docs/v24.3/cockroach-start#locality) when you create the backup. This will pin the [coordination of the backup job](/docs/v24.3/backup-architecture#job-creation-phase) and the nodes that [process the row data](/docs/v24.3/backup-architecture#export-phase) to the defined locality filter.

Pass the `WITH EXECUTION LOCALITY` option for [`RESTORE`](/docs/v24.3/restore) to restrict execution of the job to nodes with matching localities.
...
<Note>
CockroachDB also supports *locality-aware backups*, which allow you to partition and store backup data in a way that is optimized for locality. In general, when you run a locality-aware backup, nodes write backup data to the [cloud storage](/docs/v24.3/use-cloud-storage) bucket that is closest to the node locality configured at [node startup](/docs/v24.3/cockroach-start). Refer to [Take and Restore Locality-aware Backups](/docs/v24.3/take-and-restore-locality-aware-backups) for more detail.
</Note>

## Technical overview

For a technical overview of how a locality-restricted backup works, refer to [Job coordination using the `EXECUTION LOCALITY` option](/docs/v24.3/backup-architecture#job-coordination-using-the-execution-locality-option).
...
To specify the locality filter for the coordinating node, run `EXECUTION LOCALITY` with key-value pairs. The key-value pairs correspond to the [locality designations](/docs/v24.3/cockroach-start#locality) a node is configured to use when it starts.
...
- The backup or restore job may take slightly more time to start, because it must select the node that coordinates the backup or restore (the *coordinating node* ). Refer to [Job coordination using the `EXECUTION LOCALITY` option](/docs/v24.3/backup-architecture#job-coordination-using-the-execution-locality-option).
...
<Frame>
  ![Using execution locality when there is a network restriction between locality requirements](/images/v24.3/network-restriction.png)
</Frame>

To execute the backup only on nodes in the same region as the cloud storage location, you can specify [locality filters](/docs/v24.3/cockroach-start#locality) that a node must match to take part in the backup job's execution.
...
Refer to the [`BACKUP`](/docs/v24.3/backup) and [`RESTORE`](/docs/v24.3/restore) pages for further detail on other parameters and options.

### Create a non-primary region for backup jobs

Sometimes the execution of backup jobs can consume considerable resources when running [frequent full and incremental backups simultaneously](/docs/v24.3/create-schedule-for-backup). One approach to minimize the impact of running background jobs on foreground application traffic is to restrict the execution of backup jobs to a designated subset of nodes that are not also serving application traffic.

This diagram shows a CockroachDB cluster in four regions. The node used to run the backup job was configured with [non-voting replicas](/docs/v24.3/architecture/replication-layer#non-voting-replicas) to provide low-latency reads. The node in this region will complete the backup job coordination and data export to cloud storage.

<Frame>
  ![Using execution locality to create a non-primary region for backup jobs](/images/v24.3/background-work.png)
</Frame>

For details, refer to:

- The [`--locality`](/docs/v24.3/cockroach-start#locality) flag to specify the locality tiers that describe the location of a node.
- The `num_voters` and `voter_constraints` variables on the [Replication Controls](/docs/v24.3/configure-replication-zones#num_voters) page to configure non-voting replicas via zone configurations.
