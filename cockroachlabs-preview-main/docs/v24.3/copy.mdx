...
import { Copy4 } from "/snippets/syntax-diagrams/Copy4.jsx";

The `COPY... FROM` statement copies data from [`cockroach sql`](/docs/v24.3/cockroach-sql) or other [third party clients](/docs/v24.3/install-client-drivers) to tables in your cluster. The `COPY... TO` statement allows you to export a table or arbitrary query in a text or CSV format.
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>table&#95;name</code></td><td>The name of the table to which to copy data.</td></tr><tr><td><code>opt&#95;column&#95;list</code></td><td>The column name, or list of column names, to which to copy data.</td></tr><tr><td><code>WITH copy&#95;options</code></td><td>Optionally specify one or more <a href="#options">copy options</a>.</td></tr><tr><td><code>query</code></td><td>A <a href="/docs/v24.3/select-clause"><code>SELECT</code></a>, <a href="/docs/v24.3/insert"><code>INSERT</code></a>, <a href="/docs/v24.3/update"><code>UPDATE</code></a>, <a href="/docs/v24.3/upsert"><code>UPSERT</code></a>, or <a href="/docs/v24.3/delete"><code>DELETE</code></a> statement for which to copy results.</td></tr></tbody></table>
...
To run the examples, use [`cockroach demo`](/docs/v24.3/cockroach-demo) to start a temporary, in-memory cluster with the [`movr` database](/docs/v24.3/movr) preloaded.
...
1. Create a new table that you will load with CSV-formatted data:
    ...
    ```
2. Set the `bytea_output` [session variable](/docs/v24.3/set-vars#supported-variables) to specify that CockroachDB should ingest hex-encoded byte array data:
...
1. Start a temporary, in-memory cluster with the [`movr`](/docs/v24.3/movr) sample dataset preloaded:
