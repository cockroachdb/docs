...
To put files on your CockroachDB cluster without external servers, use `userfile`, a per-user file storage. To interact with `userfile`, use the following [commands](/docs/v24.3/cockroach-commands):
...
<Note>
A userfile uses storage space in the cluster, and is replicated with the rest of the cluster's data. We recommend using [`cockroach userfile upload`](/docs/v24.3/cockroach-userfile-upload) for quick uploads from your client (about 15MB or smaller).
</Note>
...
For more information, see [`cockroach userfile upload`](/docs/v24.3/cockroach-userfile-upload).
...
For more information, see [`cockroach userfile list`](/docs/v24.3/cockroach-userfile-list).
...
For more information, see [`cockroach userfile get`](/docs/v24.3/cockroach-userfile-get).
...
For more information, see [`cockroach userfile delete`](/docs/v24.3/cockroach-userfile-delete).
...
For more import options, see [`IMPORT INTO`](/docs/v24.3/import-into).

## Backup and restore with `userfile`

We recommend starting backups from a time at least 10 seconds in the past using [`AS OF SYSTEM TIME`](/docs/v24.3/as-of-system-time). Read our guidance in the [Performance](/docs/v24.3/backup#performance) section on the [`BACKUP`](/docs/v24.3/backup) page.
...
<Note>
When backing up from a cluster and restoring a database or table that is stored in your `userfile` space to a different cluster, you can run [`cockroach userfile get`](/docs/v24.3/cockroach-userfile-get) to download the backup files to a local machine and [`cockroach userfile upload -r <location/of/file> <userfile destination/of/file> --url {CONNECTION STRING}`](/docs/v24.3/cockroach-userfile-upload#upload-a-directory-recursively) to upload to the `userfile` of the restoring cluster.
</Note>
...
If you use `cockroach userfile delete {file}`, it will take as long as the [garbage collection](/docs/v24.3/configure-replication-zones#gc-ttlseconds) to be removed from disk.
