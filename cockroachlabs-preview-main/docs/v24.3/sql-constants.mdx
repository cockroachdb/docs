...
Bit array literals are acceptable both when values of types [`BIT`](/docs/v24.3/bit) or [`VARBIT`](/docs/v24.3/bit) (`BIT VARYING`) are expected.
...
Additionally, for compatibility with PostgreSQL, the notation `'string'::type` and `CAST('string' AS type)` is also recognized as an interpreted literal. These are special cases of [cast expressions](/docs/v24.3/scalar-expressions).

For more information about the allowable format of interpreted literals, refer to the "Syntax" section of the respective data types: [`DATE`](/docs/v24.3/date#syntax), [`INET`](/docs/v24.3/inet#syntax), [`INTERVAL`](/docs/v24.3/interval#syntax), [`TIME`](/docs/v24.3/time#syntax), [`TIMESTAMP`/`TIMESTAMPTZ`](/docs/v24.3/timestamp#syntax).
