...
This channel is special in that there are no constraints as to what may or may not be logged on it. Conversely, users in production deployments are invited to not collect `DEV` logs in centralized logging facilities, because they likely contain sensitive operational data. See [Configure logs](/docs/v24.3/configure-logs#dev-channel).
...
- [Cluster setting](/docs/v24.3/cluster-settings) changes
- [Zone configuration](/docs/v24.3/configure-replication-zones) changes
...
The `SESSIONS` channel is used to report client network activity when enabled via the `server.auth_log.sql_connections.enabled` and/or `server.auth_log.sql_sessions.enabled` [cluster setting](/docs/v24.3/cluster-settings):
...
- Data access audit events (when table audit is enabled via [ALTER TABLE... EXPERIMENTAL_AUDIT](/docs/v24.3/alter-table#experimental_audit) )
- Data access audit events (when role-based audit is enabled via [`sql.log.user_audit` cluster setting](/docs/v24.3/role-based-audit-logging#syntax-of-audit-settings) )
...
- Logical SQL statement executions (when enabled via the `sql.log.all_statements.enabled` [cluster setting](/docs/v24.3/cluster-settings) )
