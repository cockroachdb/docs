...
For a summary of changefeed features, refer to the [Change Data Capture Overview](/docs/v24.3/change-data-capture-overview) page.
...
Refer to the [Changefeed Sinks](/docs/v24.3/changefeed-sinks) page for more detail on forming sink URIs, available sink query parameters, and specifics on configuration.
...
Before you run the examples, verify that you have the `CHANGEFEED` privilege in order to create and manage changefeed jobs. Refer to [Required privileges](/docs/v24.3/create-changefeed#required-privileges) for more details.
...
1. Use the [`cockroach start-single-node`](/docs/v24.3/cockroach-start-single-node) command to start a single-node cluster:
...
5. As the `root` user, open the [built-in SQL client](/docs/v24.3/cockroach-sql):

    ```shell
    $ cockroach sql --insecure
    ```
6. Set your organization name and [Enterprise license](/docs/v24.3/licensing-faqs#types-of-licenses) key:
    ...
    ```
7. Enable the `kv.rangefeed.enabled` [cluster setting](/docs/v24.3/cluster-settings):
...
10. Create a table and add data:
    ...
    ```
11. Create another table and add data:
...
13. In a new terminal, move into the extracted `confluent-<version>` directory and start watching the Kafka topics:
...
14. Back in the SQL client, insert more data:
    ...
    ```
15. Back in the terminal where you're watching the Kafka topics, the following output has appeared:

    ```text
    {"after": {"id": 3, "name": "Ernie"}}
    ```
16. When you are done, exit the SQL shell (`\q`).
17. To stop `cockroach`:
...
1. If you do not already have one, [request a trial Enterprise license](/docs/v24.3/licensing-faqs#obtain-a-license).
2. Use the [`cockroach start-single-node`](/docs/v24.3/cockroach-start-single-node) command to start a single-node cluster:

    ```shell
    cockroach start-single-node --insecure --listen-addr=localhost
    ```
3. In this example, you'll run CockroachDB's [Movr](/docs/v24.3/movr) application workload to set up some data for your changefeed.
    ...
    ```
4. As the `root` user, open the [built-in SQL client](/docs/v24.3/cockroach-sql):

    ```shell
    $ cockroach sql --insecure
    ```
5. Set your organization name and [Enterprise license](/docs/v24.3/licensing-faqs#types-of-licenses) key:
    ...
    > SET CLUSTER SETTING enterprise.license = '<secret>';
    ```
6. Enable the `kv.rangefeed.enabled` [cluster setting](/docs/v24.3/cluster-settings):
...
11. Construct the URI to connect your changefeed to the Confluent Cloud cluster. You will need:
    ...
    - (Optional) Any further parameters. Refer to [Query parameters](/docs/v24.3/create-changefeed#query-parameters).
...
7. Next, you'll prepare your Pub/Sub sink.
    ...
    ```
8. Create the Pub/Sub [topic](/docs/v24.3/changefeed-sinks#pub-sub-topic-naming) to which your changefeed will emit messages:
...
9. With the topic and subscription set up, you can now download your Service Account credentials. Use the [`gcloud iam service-accounts keys create`](https://cloud.google.com/sdk/gcloud/reference/iam/service-accounts/keys/create) command to specify where to download the JSON credential file (`credentials.json`):
    ...
    Copy the output so that you can add it to your [`CREATE CHANGEFEED`](/docs/v24.3/create-changefeed) statement in the next step. When you create your changefeed, it is necessary that the credentials are base64 encoded before passing it in the URI.
10. Back in the SQL shell, create a changefeed that will emit messages to your Pub/Sub topic. Ensure that you have base64 encoded the entire credentials JSON object for your Service Account and then run:
    ...
    You can include the `region` parameter for your topic, or use the [WITH `unordered`](/docs/v24.3/create-changefeed#unordered) option for multi-region Pub/Sub. See the [Changefeed Sinks](/docs/v24.3/changefeed-sinks#google-cloud-pub-sub) page for more detail.
...
In this example, you'll set up a changefeed for a single-node cluster that is connected to an AWS S3 sink. The changefeed watches two tables. Note that you can set up changefeeds for any of [these cloud storage providers](/docs/v24.3/changefeed-sinks#cloud-storage-sink).
...
1. Use the [`cockroach start-single-node`](/docs/v24.3/cockroach-start-single-node) command to start a single-node cluster:
    ...
    $ cockroach start-single-node --insecure --listen-addr=localhost
    ```
2. As the `root` user, open the [built-in SQL client](/docs/v24.3/cockroach-sql):

    ```shell
    $ cockroach sql --insecure
    ```
3. Set your organization name and [Enterprise license](/docs/v24.3/licensing-faqs#types-of-licenses) key:
    ...
    > SET CLUSTER SETTING enterprise.license = '<secret>';
    ```
4. Enable the `kv.rangefeed.enabled` [cluster setting](/docs/v24.3/cluster-settings):
...
5. Create a database called `cdc_demo`:
...
7. Create a table and add data:
    ...
    > UPDATE office_dogs SET name = 'Petee H' WHERE id = 1;
    ```
8. Create another table and add data:
...
10. Monitor your changefeed on the [DB Console](http://localhost:8080/#/metrics/changefeeds/cluster). For more information, see [Changefeeds Dashboard](/docs/v24.3/ui-cdc-dashboard).
...
You can include the [`topic_name`](/docs/v24.3/create-changefeed#topic-name) or [`topic_prefix`](/docs/v24.3/create-changefeed#topic-prefix) parameters to create and name an Event Hub for emitted messages.

1. Create an [external connection](/docs/v24.3/create-external-connection) to store your connection string:
...
1. Use the [`cockroach start-single-node`](/docs/v24.3/cockroach-start-single-node) command to start a single-node cluster:
    ...
    $ cockroach start-single-node --insecure --listen-addr=localhost
    ```
2. In this example, you'll run CockroachDB's [Movr](/docs/v24.3/movr) application workload to set up some data for your changefeed.
    ...
    ```shell
     cockroach workload run movr --duration=1m "postgresql://root@127.0.0.1:26257?sslmode=disable"
    ```
3. As the `root` user, open the [built-in SQL client](/docs/v24.3/cockroach-sql):

    ```shell
    $ cockroach sql --insecure
    ```
4. Set your organization name and [Enterprise license](/docs/v24.3/licensing-faqs#types-of-licenses) key:
    ...
    > SET CLUSTER SETTING enterprise.license = '<secret>';
    ```
5. Enable the `kv.rangefeed.enabled` [cluster setting](/docs/v24.3/cluster-settings):
...
8. Back in your SQL shell, run the following statement to create a changefeed that emits to your webhook sink:
    ...
    See the [options table](/docs/v24.3/create-changefeed#options) for more information on the options available for creating your changefeed to a webhook sink.
    ...
    For more detail on emitted changefeed messages, refer to the [Changefeed Messages](/docs/v24.3/changefeed-messages) page.
...
10. Create your changefeed:
    ...
    Changefeeds emitting to a Pulsar sink do not support external connections or a number of changefeed options. For a full list, refer to the [Changefeed Sinks](/docs/v24.3/changefeed-sinks#apache-pulsar) page.
...
For further information on sinkless changefeeds, refer to the [`CREATE CHANGEFEED`](/docs/v24.3/create-changefeed#create-a-sinkless-changefeed) page.
