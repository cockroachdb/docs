...
The following diagram shows how WAL failover works at a high level. For more information about the WAL, memtables, and SSTables, refer to the [Architecture » Storage Layer documentation](/docs/v24.3/architecture/storage-layer).

<Frame>
  ![WAL failover overview diagram](/images/v24.3/wal-failover-overview.png)
</Frame>
...
[Install CockroachDB](/docs/v24.3/install-cockroachdb) on each node.

#### 3. Log configuration for WAL failover

If you are logging to a [file-based sink](/docs/v24.3/configure-logs#output-to-files), create a `logs.yaml` file. Later on, you will pass this file to `cockroach start` when starting each node in the cluster. Not doing so will negate the positive impact of enabling WAL failover, because writes to the diagnostic logs may block indefinitely during the disk stall, effectively stalling the node.
...
For more information about how to configure CockroachDB logs, refer to [Configure logs](/docs/v24.3/configure-logs).

#### 4. Start the multi-store cluster with WAL failover enabled

To enable WAL failover when you start the cluster, either pass the [`--wal-failover=among-stores` to `cockroach start`](/docs/v24.3/cockroach-start#enable-wal-failover)

*OR*

set [the environment variable `COCKROACH_WAL_FAILOVER=among-stores`](/docs/v24.3/cockroach-start#enable-wal-failover) before starting the cluster.
...
You must also [configure logs](/docs/v24.3/configure-logs) by passing in the `logs.yaml` file that you configured in [Step 3](#3-sidecar-configure-logs).

Replicate the shell commands below on each node using your preferred deployment model. Be sure to edit the values of the flags passed to [`cockroach start`](/docs/v24.3/cockroach-start) as needed for your environment.
...
To enable WAL failover when you start the cluster, either pass the [`--wal-failover=path={ path-to-my-side-disk-for-wal-failover }` to `cockroach start`](/docs/v24.3/cockroach-start#enable-wal-failover)

*OR*

set [the environment variable `COCKROACH_WAL_FAILOVER=path={ path-to-my-side-disk-for-wal-failover }`](/docs/v24.3/cockroach-start#enable-wal-failover) before starting the cluster.
...
Optionally, you can run a [workload](/docs/v24.3/cockroach-workload) at this point, if you want to see your workload continue during WAL failover. However, the presence of the workload does not have any bearing on the operation of WAL failover.

### 2. WAL failover during transient disk stalls

#### Cause transient disk stall

Before triggering a WAL failover, open the [DB Console](/docs/v24.3/ui-overview) so that you can observe WAL failover metrics.
...
In [DB Console's **Advanced Debug** page](/docs/v24.3/ui-custom-chart-debug-page), click on **Custom Time series chart**. On the custom chart page, add three charts: one for each of the preceding metrics.

Set the source of these metrics to be the node where you are running the disk stall/unstall script.

<Frame>
  ![WAL Failover Metrics Chart](/images/v24.3/wal-failover-metrics-chart.jpg)
</Frame>
...
You can confirm that the node is still live and available using [`cockroach node status`](/docs/v24.3/cockroach-node#node-status).
...
For [multiple stores](#multi-store-config), pass `--wal-failover=among-stores` to [`cockroach start`](/docs/v24.3/cockroach-start).

For a [side disk on a single-store config](#single-store-config), pass `--wal-failover={ path-to-my-side-disk-for-wal-failover }` to [`cockroach start`](/docs/v24.3/cockroach-start).
...
2. After starting the cluster, set it by changing the [`storage.max_sync_duration`](/docs/v24.3/cluster-settings#setting-storage-max-sync-duration) cluster setting. When WAL failover is enabled, [Cockroach Labs recommends using `40s` for this setting](#4-what-is-the-best-practice-storage-max_sync_duration). If WAL failover is not enabled, **do not change the default value of this setting**.
...
<Frame>
  ![how long WAL writes take during a disk stall with and without WAL failover enabled](/images/v24.3/wal-failover-behavior.png)
</Frame>
...
- Improves resiliency against transient [disk stalls](/docs/v24.3/cluster-setup-troubleshooting#disk-stalls).
- Minimizes the impact of disk stalls on write latency. Bounds [Raft log](/docs/v24.3/architecture/replication-layer#raft) latencies to about `100ms` in the event of a stall, which helps reduce the stall's impact on query tail latencies.
...
- For a [multi-store cluster](#multi-store-config), pass `--wal-failover=disabled` to [`cockroach start`](/docs/v24.3/cockroach-start#disable-wal-failover).
- For a [single-store cluster with a side disk](#single-store-config) for failover, pass `--wal-failover=disabled,prev_path={ path-to-failover-disk }` to [`cockroach start`](/docs/v24.3/cockroach-start#disable-wal-failover).

If you want to disable WAL failover on a running cluster, set the value of the [`storage.wal_failover.unhealthy_op_threshold`](/docs/v24.3/cluster-settings#setting-storage-wal-failover-unhealthy-op-threshold) cluster setting to a value greater than the [`storage.max_sync_duration`](/docs/v24.3/cluster-settings#setting-storage-max-sync-duration) cluster setting as follows:
...
To enable WAL failover at runtime on a [multi-store cluster](#multi-store-config), make sure to configure log buffering as described in [Enable WAL failover](/docs/v24.3/cockroach-start#enable-wal-failover).

Next, make sure the value of the [`storage.wal_failover.unhealthy_op_threshold`](/docs/v24.3/cluster-settings#setting-storage-wal-failover-unhealthy-op-threshold) cluster setting is a duration less than the [`storage.max_sync_duration`](/docs/v24.3/cluster-settings#setting-storage-max-sync-duration) cluster setting:

```sql
SET CLUSTER SETTING storage.wal_failover.unhealthy_op_threshold = '${value less than storage.max_sync_duration}'
```

The recommended value of the [`storage.wal_failover.unhealthy_op_threshold`](/docs/v24.3/cluster-settings#setting-storage-wal-failover-unhealthy-op-threshold) cluster setting is `100ms`.

</Accordion>

<Accordion title="8. Can I enable WAL failover during runtime for a single data store cluster?">

Yes: If the side disk for WAL failover was configured before the cluster was started, set the [`storage.wal_failover.unhealthy_op_threshold`](/docs/v24.3/cluster-settings#setting-storage-wal-failover-unhealthy-op-threshold) cluster setting to a value less than the [`storage.max_sync_duration`](/docs/v24.3/cluster-settings#setting-storage-max-sync-duration) cluster setting to enable WAL failover.

No: If the side disk for WAL failover was **not** pre-configured before the cluster was started, changing the value of the [`storage.wal_failover.unhealthy_op_threshold`](/docs/v24.3/cluster-settings#setting-storage-wal-failover-unhealthy-op-threshold) cluster setting to something less than the [`storage.max_sync_duration`](/docs/v24.3/cluster-settings#setting-storage-max-sync-duration) cluster setting **will not have any effect, and when the disk stalls, WAL failover will not trigger**. In other words, you cannot enable WAL failover at runtime for a single-store cluster if you did not [pre-configure a side disk](#single-store-config).

The recommended default value of the [`storage.wal_failover.unhealthy_op_threshold`](/docs/v24.3/cluster-settings#setting-storage-wal-failover-unhealthy-op-threshold) cluster setting is `100ms`.
...
Yes. To change [`storage.max_sync_duration`](/docs/v24.3/cluster-settings#setting-storage-max-sync-duration), issue a statement like the following:
...
No. When WAL failover is enabled, the WAL directory path is stored as an absolute path in the [store](/docs/v24.3/cockroach-start#store)'s data. It is not treated as a relative path. As a result, it is not sufficient to stop CockroachDB, move or rename that directory, and restart with a different `--wal-failover` path.

Instead, to change the WAL directory path, you must first [disable WAL failover](/docs/v24.3/cockroach-start#disable-wal-failover), [restart the node(s)](/docs/v24.3/node-shutdown#stop-and-restart-a-node), and then [re-enable WAL failover with the new path](/docs/v24.3/cockroach-start#enable-wal-failover).
...
- [`cockroach start` &gt; WAL Failover](/docs/v24.3/cockroach-start#write-ahead-log-wal-failover)
