...
- [Accessing the DB console on CockroachDB self-hosted clusters](/docs/v24.3/ui-overview).
...
A specific CockroachDB cluster's authentication behavior is configured by setting its `server.host_based_authentication.configuration` [cluster setting](/docs/v24.3/cluster-settings), using the [`SET CLUSTER SETTING` statement](/docs/v24.3/set-cluster-setting), which accepts a single text field that must be a correctly formatted HBA manifest. Inspect the current setting with [`SHOW CLUSTER SETTING`.](/docs/v24.3/show-cluster-setting)

## Supported authentication methods

<table><thead><tr><th>Authentication Method</th><th>CockroachDB Cloud</th><th>CockroachDB self-hosted</th><th>CockroachDB Enterprise</th></tr></thead><tbody><tr><td>password</td><td>✓</td><td>✓</td><td>✓</td></tr><tr><td>username/password combination</td><td>✓</td><td>✓</td><td>✓</td></tr><tr><td><a href="/docs/v24.3/security-reference/scram-authentication">SCRAM-SHA-256</a></td><td>✓</td><td>✓</td><td>✓</td></tr><tr><td><a href="/docs/v24.3/security-reference/transport-layer-security">certificate</a></td><td>✓</td><td>✓</td><td>✓</td></tr><tr><td>GSS</td><td></td><td></td><td>✓</td></tr></tbody></table>
...
6. Authentication **METHOD** by which specified user(s) may authenticate from specified addresses.
    ...
    - `scram-sha-256`: user may authenticate via [Salted Challenge-Response](/docs/v24.3/security-reference/scram-authentication)
    - `cert`: user may authenticate with a PKI certificate signed by a trusted certificate authority CA.
    - `cert-password`: user may authenticate with either a certificate or a password. Additionally, the server may use a [SCRAM](/docs/v24.3/security-reference/scram-authentication) exchange, if the cluster setting `server.user_login.cert_password_method.auto_scram_promotion.enabled` is set to `true`.
    - `cert-scram-sha-25`: user may authenticate with either a certificate or a [SCRAM](/docs/v24.3/security-reference/scram-authentication) exchange.
    - `gss`: user may authenticate with a GSSAPI token.
    - `ldap`: user may authenticate using [LDAP](/docs/v24.3/ldap-authentication) -compatible directory services, such as Microsoft Entra ID and Active Directory.
...
This is convenient for quick usage and experimentation, but is not suitable for clusters containing production data. It is a best practice to [configure SQL authentication for hardened CockroachDB cluster security](/docs/v24.3/security-reference/config-secure-hba).
