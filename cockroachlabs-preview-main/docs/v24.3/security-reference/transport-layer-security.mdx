...
Refer to [Manage PKI certificates for a CockroachDB deployment with HashiCorp Vault](/docs/v24.3/manage-certs-vault) for procedural information on administering and using client certificate authentication.
...
- Learn about [Manage PKI certificates for a CockroachDB deployment with HashiCorp Vault](/docs/v24.3/manage-certs-vault).
- Review our [breakdown of security features by offering](/docs/v24.3/security-reference/security-overview).
...
CockroachDB provides a number of SQL clients, including a [CLI](/docs/v24.3/cockroach-sql#start-a-sql-shell), and a number of [drivers and object-relational mapping (ORM) tools](/docs/v24.3/install-client-drivers). Regardless of which client you are using, how you are able to authenticate to a CockroachDB cluster depends on that cluster's [authentication configuration](/docs/v24.3/security-reference/authentication), specifically whether that configuration requires the user to authenticate with username/password combination, certificate or another method.
...
Because the server must still be TLS-authenticated, the client must know to trust the certificate authority that signed the public certificate identifying the server. The path to the CA's public certificate is passed as the `sslrootcert` parameter in a [database connection string](/docs/v24.3/connect-to-the-database), or by being placed in the directory specified by the `certs-dir` argument in a connection made with the [`cockroach sql`](/docs/v24.3/cockroach-sql) CLI command.
...
When using a non-TLS client authentication method, such as username/password or GSSAPI/Kerberos (Enterprise only), the server must still be TLS-authenticated. Therefore, the client must know to trust the certificate authority that signed the public certificate identifying the server. Therefore, the root CA certificate, called `ca.crt`, must be provided to client authentication attempts. This can be passed as the `sslrootcert` parameter in a [database connection string](/docs/v24.3/connect-to-the-database), or by being placed in the directory specified by the `certs-dir` argument in a connection made with the [`cockroach sql`](/docs/v24.3/cockroach-sql) CLI command.
...
CockroachDB can be [configured to check an OCSP responder](/docs/v24.3/manage-certs-revoke-ocsp).
...
See: [Managing security certificates with HashiCorp Vault](/docs/v24.3/manage-certs-vault)
