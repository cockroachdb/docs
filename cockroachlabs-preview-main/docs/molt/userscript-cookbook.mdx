...
Userscripts allow you to define custom schema and table transformations. When specified with the [`--userscript` flag](/docs/molt/replicator-flags#userscript), userscripts modify the data that [MOLT Replicator](/docs/molt/molt-replicator) migrates from a source database to CockroachDB.

This cookbook provides ready-to-use examples that demonstrate real-world uses of the [userscript API](/docs/molt/userscript-api). You can copy and paste them into your own code, and you can adapt them for your specific use cases.

Userscripts are comparable to MOLT Fetch's [transformations](/docs/molt/molt-fetch#transformations), which are used during the initial bulk load phase of a migration. When performing an [initial data load followed by live replication](/docs/molt/migrate-load-replicate), **apply equivalent transformations in both the Fetch command and Replicator userscript** to ensure data consistency. Below each example, you will see the equivalent way of carrying out that transformation using MOLT Fetch, if it's possible to do so.

## Before you begin

- Make sure that you understand the [purpose and usage of userscripts](/docs/molt/userscript-overview). Take a look at the [userscript API](/docs/molt/userscript-api). Understand [what you cannot do](/docs/molt/userscript-overview#unsupported-typescript-features) in a userscript.
- [Install MOLT Replicator](/docs/molt/molt-replicator#installation). The userscript API is accessible via the `replicator` library, which is already included in MOLT Replicator.
...
The following example shows how to use [`configureTargetSchema`](/docs/molt/userscript-api#configure-target-schema) to exclude a specific table from replication, while still replicating everything else in the same schema.

This could be useful when you have internal, staging, or audit tables that appear in the [changefeed](/docs/v25.4/change-data-capture-overview) but shouldn't be written to the target.
...
You can filter a single table using the MOLT Fetch [`--table-exclusion-filter`](/docs/molt/molt-fetch#schema-and-table-selection) flag.
...
The following example shows how to use [`configureTargetSchema`](/docs/molt/userscript-api#configure-target-schema) to exclude multiple tables from the replication process. This extends the example shown in [Filter a single table](#filter-a-single-table) to allow for multiple tables to be filtered instead of just one.
...
You can filter multiple tables using the MOLT Fetch [`--table-exclusion-filter`](/docs/molt/molt-fetch#schema-and-table-selection) flag.
...
The following example demonstrates how to use [`configureTargetSchema`](/docs/molt/userscript-api#configure-target-schema) to conditionally replicate rows.
...
You can selectively replicate data using the [`--filter-path`](/docs/molt/molt-fetch#selective-data-movement) flag, which accepts a path to a JSON file that specifies row-level filter expressions.
...
The following example shows how to use [`configureTargetSchema`](/docs/molt/userscript-api#configure-target-schema) to remove specific columns from replicated rows. For example, the source table may include internal metadata columns or values intended only for the source system. The following example removes a single column `qty` before writing the row to the target.
...
Filter columns using the [`--transformations-file`](/docs/molt/molt-fetch#transformations) flag, which accepts a path to a JSON file that specifies column exclusions.
...
The following example shows how you can use [`configureTargetSchema`](/docs/molt/userscript-api#configure-target-schema) to rename a table's columns on the target database. It demonstrates how you might handle column renaming in the case of both upserts and deletes.
...
MOLT Fetch does not have direct support for column renaming. You may need to rename the column on the target database after the [initial data load from MOLT Fetch](/docs/molt/migrate-load-replicate#start-fetch).

### Rename primary keys

This example shows how you can use [`configureTargetSchema`](/docs/molt/userscript-api#configure-target-schema) to rename a table's primary keys. Because of how the `onRowDelete` handler uses primary keys to identify rows, this scenario requires a different implementation than the [Rename columns](#rename-columns) example.
...
MOLT Fetch does not have direct support for primary key renaming. You may need to reconfigure the primary keys on the target database after the [initial data load from MOLT Fetch](/docs/molt/migrate-load-replicate#start-fetch).

### Route table partitions

The following example demonstrates how you can use [`configureTargetSchema`](/docs/molt/userscript-api#configure-target-schema) to distribute the rows of a single source table across multiple target tables based on partitioning rules.
...
This example shows how to use [`configureTargetSchema`](/docs/molt/userscript-api#configure-target-schema) to route data from a source table to a target table of a different name. This must be implemented in the schema-level handlers, as the row will need to be routed to the correct table-level handler using the correct target table name.
...
Rename tables using the [`--transformations-file`](/docs/molt/molt-fetch#transformations) flag, which accepts a path to a JSON file that specifies table mappings.
...
The following example shows how to use [`configureTargetSchema`](/docs/molt/userscript-api#configure-target-schema) to create computed columns on the target that do not exist on the source. In this example, we derive a `total` column, and add it to each replicated row on the target.
...
Creating computed columns is not supported by MOLT Fetch transforms. MOLT Fetch can only preserve computed columns that exist on the source. You may need to calculate this column for the target database table after the [initial data load from MOLT Fetch](/docs/molt/migrate-load-replicate#start-fetch).
...
- A `soft_delete_filter.json` file (to be included via the [`--filter-path`](/docs/molt/molt-fetch#selective-data-movement) flag).
- A `pii_removal_transform.json` file (to be included via the [`--transformations-file`](/docs/molt/molt-fetch#transformations) flag).
