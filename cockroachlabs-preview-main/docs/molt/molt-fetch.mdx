...
MOLT Fetch moves data from a source database into CockroachDB as part of a [database migration](/docs/molt/migration-overview).

MOLT Fetch uses [`IMPORT INTO`](/docs/v25.4/import-into) or [`COPY FROM`](/docs/v25.4/copy) to move the source data to cloud storage (Google Cloud Storage, Amazon S3, or Azure Blob Storage), a local file server, or local memory. Once the data is exported, MOLT Fetch loads the data into a target CockroachDB database. For details, refer to [Migration phases](#migration-phases).
...
`--target` specifies the [CockroachDB connection string](/docs/v25.4/connection-parameters#connect-using-a-url):
...
- `IMPORT INTO` achieves the highest throughput, but [requires taking the CockroachDB tables **offline**](/docs/v25.4/import-into#considerations) to achieve its import speed. Tables are taken back online once an [import job](/docs/v25.4/import-into#view-and-control-import-jobs) completes successfully. See [Best practices](#best-practices).
...
- `COPY FROM` enables your tables to remain online and accessible. However, it is slower than using [`IMPORT INTO`](/docs/v25.4/import-into).
...
- **Range-based sharding (default):** Tables are divided based on numerical ranges found in primary key values. Only tables with [`INT`](/docs/v25.4/int), [`FLOAT`](/docs/v25.4/float), or [`UUID`](/docs/v25.4/uuid) primary keys can use range-based sharding. Tables with other primary key data types export as a single shard.
...
Connect to a Google Cloud Storage bucket with [implicit authentication](/docs/v25.4/cloud-storage-authentication#google-cloud-storage-implicit) and [assume role](/docs/v25.4/cloud-storage-authentication#set-up-google-cloud-storage-assume-role):
...
Connect to an Azure Blob Storage container with [implicit authentication](/docs/v25.4/cloud-storage-authentication?filters=azure#azure-blob-storage-implicit-authentication):
...
`--local-path` instructs MOLT Fetch to write intermediate files to a path within a [local file server](/docs/v25.4/use-a-local-file-server). `local-path-listen-addr` specifies the address of the local file server. For example:
...
To filter tables during replication, use [MOLT Replicator](/docs/molt/molt-replicator) with [userscripts](/docs/molt/userscript-cookbook#filter-a-single-table).

### Selective data movement

Use `--filter-path` to specify the path to a JSON file that defines row-level filtering for data load. This enables you to move a subset of data in a table, rather than all data in the table. To apply row-level filters during replication, use [MOLT Replicator](/docs/molt/molt-replicator) with [userscripts](/docs/molt/userscript-cookbook#select-data-to-replicate).
...
To [truncate](/docs/v25.4/truncate) tables before loading the data, use `truncate-if-exists`:
...
- If the source table contains duplicate rows but the target has [`PRIMARY KEY`](/docs/v25.4/primary-key) or [`UNIQUE`](/docs/v25.4/unique) constraints, duplicate rows are deduplicated during import.
...
- PostgreSQL types are mapped to existing CockroachDB [types](/docs/v25.4/data-types) that have the same [`OID`](/docs/v25.4/oid).
...
- To override the default mappings for automatic schema creation, you can map source to target CockroachDB types explicitly. These are defined in the JSON file indicated by the `--type-map-file` flag. The allowable custom mappings are valid CockroachDB aliases, casts, and the following mappings specific to MOLT Fetch and [Verify](/docs/molt/molt-verify):
    - [`TIMESTAMP`](/docs/v25.4/timestamp) &lt;&gt; [`TIMESTAMPTZ`](/docs/v25.4/timestamp)
    - [`VARCHAR`](/docs/v25.4/string) &lt;&gt; [`UUID`](/docs/v25.4/uuid)
    - [`BOOL`](/docs/v25.4/bool) &lt;&gt; [`INT2`](/docs/v25.4/int)
    - [`VARBIT`](/docs/v25.4/bit) &lt;&gt; [`TEXT`](/docs/v25.4/string)
    - [`VARBIT`](/docs/v25.4/bit) &lt;&gt; [`BOOL`](/docs/v25.4/bool)
    - [`JSONB`](/docs/v25.4/jsonb) &lt;&gt; [`TEXT`](/docs/v25.4/string)
    - [`INET`](/docs/v25.4/inet) &lt;&gt; [`TEXT`](/docs/v25.4/string)
...
- `crdb_type` specifies the target CockroachDB [type](/docs/v25.4/data-types) to be mapped.

### Transformations

You can define transformation rules to be performed on the target database during the fetch task. These can be used to:

- Map [computed columns](/docs/v25.4/computed-columns) from source to target.
- Map [partitioned tables](/docs/v25.4/partitioning) to a single target table.
...
- `column_exclusion_opts`: Exclude columns or map them as computed columns.
    ...
    - `add_computed_def`: When `true`, map matching columns as [computed columns](/docs/v25.4/computed-columns) on target tables using [`ALTER TABLE... ADD COLUMN`](/docs/v25.4/alter-table#add-column) and the source column definition. All matching columns must be computed columns on the source.
...
[Example rule `1`](#transformation-rules-example) maps all source `age` columns to [computed columns](/docs/v25.4/computed-columns) on CockroachDB. This assumes that all matching `age` columns are defined as computed columns on the source:
...
- `table_rename_opts`: Rename tables on the target.
    - `value`: Target table name. For a single matching source table, renames it to this value. For multiple matches (n-to-1), consolidates matching [partitioned tables](/docs/v25.4/partitioning) with the same table definition into a single table with this name.
...
To view all active continuation tokens, issue a `molt fetch tokens list` command along with `--conn-string`, which specifies the [connection string](/docs/v25.4/connection-parameters#connect-using-a-url) for the target CockroachDB database. For example:
...
Use the `cdc_cursor` value as the checkpoint for MySQL or Oracle replication with [MOLT Replicator](/docs/molt/molt-replicator#replication-checkpoints).
...
For detailed steps, refer to [Bulk load migration](/docs/molt/migrate-bulk-load).
...
To perform an initial data load before setting up ongoing replication with [MOLT Replicator](/docs/molt/molt-replicator), run the `molt fetch` command without `--ignore-replication-check`. This captures replication checkpoints during the data load.
...
Use this `cdc_cursor` value when starting MOLT Replicator to ensure replication begins from the correct position. For detailed steps, refer to [Load and replicate](/docs/molt/migrate-load-replicate).
...
- To prevent connections from terminating prematurely during the [data export phase](#data-export-phase), set the following to high values on the source database:
    - **Maximum allowed number of connections.** MOLT Fetch can export data across multiple connections. The number of connections it will create is the number of shards ([`--export-concurrency`](#global-flags)) multiplied by the number of tables ([`--table-concurrency`](#global-flags)) being exported concurrently.
        ...
        <Note>
        With the default numerical range sharding, only tables with [primary key](/docs/v25.4/primary-key) types of [`INT`](/docs/v25.4/int), [`FLOAT`](/docs/v25.4/float), or [`UUID`](/docs/v25.4/uuid) can be sharded. PostgreSQL users can enable [`--use-stats-based-sharding`](#global-flags) to use statistics-based sharding for tables with primary keys of any data type. For details, refer to [Table sharding](#table-sharding).
        </Note>
...
- To optimize data load performance, drop all non-`PRIMARY KEY` [constraints](/docs/v25.4/alter-table#drop-constraint) and [indexes](/docs/v25.4/drop-index) on the target CockroachDB database before migrating:
    ...
    - [`NOT NULL`](/docs/v25.4/not-null) (you do not need to drop this constraint when using `drop-on-target-and-recreate` for [table handling](#target-table-handling) )

        <Warning>
        Do **not** drop [`PRIMARY KEY`](/docs/v25.4/primary-key) constraints.
        </Warning>

        You can recreate [constraints](/docs/v25.4/alter-table#add-constraint) and [indexes](/docs/v25.4/create-index) after loading the data.
- For PostgreSQL sources using [`--use-stats-based-sharding`](#global-flags), run [`ANALYZE`](/docs/v25.4/create-statistics) on source tables before migration to ensure optimal shard distribution. This is especially important for large tables where even distribution can significantly improve export performance.
...
- [Resume Replication](/docs/molt/migrate-resume-replication)
