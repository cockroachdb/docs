...
[Userscripts](/docs/molt/userscript-overview) allow you to define how rows are transformed, filtered, and routed before [MOLT Replicator](/docs/molt/molt-replicator) writes them to the target database.
...
The [userscript cookbook](/docs/molt/userscript-cookbook) includes example scenarios that further demonstrate how to use this API. Be sure to follow userscript [best practices](#best-practices).

## Access this API

To access the userscript API, [install MOLT Replicator **v1.3.0 or later**](/docs/molt/molt-replicator#installation). The userscript API is accessible via the `replicator` library, which you should import at the top of your TypeScript file: `import * as api from "replicator@v2";`. The `replicator` library is included in the MOLT Replicator binary itself, so you do not need to install any external packages in order to run userscripts.
...
`configureTargetSchema` registers schema-level handlers that run [before staging and table-level processing](/docs/molt/userscript-overview#how-it-works). Use this to transform, filter, or reroute rows broadly across a target database schema.
...
`onRowUpsert` is called when a row is inserted or updated on the source database. Because it's a handler for the [`configureTargetSchema`](#configure-target-schema) function, it is called [before staging and table-level processing](/docs/molt/userscript-overview#how-it-works).
...
For more information about renaming tables, refer to the [Rename tables](/docs/molt/userscript-cookbook#rename-tables) cookbook example.
</Note>
...
`onRowDelete` is called when a row is deleted on the source database. Because it's a handler for the [`configureTargetSchema`](#configure-target-schema) function, it is called [before staging and table-level processing](/docs/molt/userscript-overview#how-it-works).
...
For more information, refer to the [Rename tables](/docs/molt/userscript-cookbook#rename-tables) cookbook example.
</Note>
...
For more information, refer to the [Rename primary keys](/docs/molt/userscript-cookbook#rename-primary-keys) cookbook example.
</Note>
...
`configureTargetTables` registers table-level handlers that run [after rows are staged](/docs/molt/userscript-overview#how-it-works) and ready to be written to the target database. You can use this function to define transformations, filters, or column-level behaviors that are specific to certain tables.
...
- `tables`: A list of the (case-sensitive) names of the tables to write to in the **target** database. The handler functions will apply to all rows being routed to the tables in this list. Table names may be [fully qualified](/docs/v25.4/sql-name-resolution) (for example, `defaultdb.public.table1` ) or they may include only the name of the table (for example, `table1` ).
...
`onRowUpsert` is called when a row is inserted or updated on the source database. Because it's a handler for the [`configureTargetTables`](#configure-target-tables) function, it is called [after rows are staged](/docs/molt/userscript-overview#how-it-works) and ready to be written to the target database.
...
`onRowDelete` is called when a row is deleted on the source database. Because it's a handler for the [`configureTargetTables`](#configure-target-tables) function, it is called [after rows are staged](/docs/molt/userscript-overview#how-it-works) and ready to be written to the target database.
...
- A list of the [primary key](/docs/v25.4/primary-key) values that define the row to be deleted on the **target** database, as supplied by the schema-level handlers. This is only a list of the key values, not a mapping of key names to values.
- A [`Metadata`](#common-typescript-definitions) object containing the following (case-sensitive):
    - The `schema` name for the row in the **target** database.
    - The `table` name for the row in the **target** database.
...
`onWrite` is called after [`onRowUpsert`](#configure-target-tables-on-row-upsert) and [`onRowDelete`](#configure-target-tables-on-row-delete) for each of the passed-in rows. The `onWrite` handler lets you override [MOLT Replicator's default behavior](/docs/molt/molt-replicator#how-it-works) when rows are written to the target database. It is called right before the final commit, after all schema-level and table-level processing has completed. A [commit](/docs/v25.4/commit-transaction) will be scheduled once `onWrite` returns.
...
- Running [stored procedures](/docs/v25.4/stored-procedures) or custom SQL statements.
- Handling [retries](/docs/v25.4/transactions#transaction-retries) or [error](/docs/v25.4/common-errors) conditions manually.
...
[`onWrite`](#configure-target-tables-on-write) has a set of functions available to it that allows [transactional](/docs/v25.4/transactions) logic to be performed on the target database:
...
Use `api.getTX()` to run your own SQL statements within a single [transaction](/docs/v25.4/transactions). Use `api.write(rows)` to write the rows to the target database. Mix both approaches by handling some operations manually and passing others to `api.write(rows)`. Read detailed descriptions of these functions below.
...
The `getTX` function provides access to a managed [transaction](/docs/v25.4/transactions) on the target database that you can use inside your userscript. It allows you to perform direct SQL operations, such as reads, writes, or schema checks, within a single transactional context. This is especially useful when you need to enforce additional constraints, perform cross-table validation, or implement custom transactional logic during replication.
...
This function executes a SQL query within the current replication [transaction](/docs/v25.4/transactions). It returns an async iterable of result rows, allowing you to read data from the target database for validation or lookups.
...
This function runs a SQL statement (such as [`UPDATE`](/docs/v25.4/update), [`INSERT`](/docs/v25.4/insert), or [`DELETE`](/docs/v25.4/delete)) in the current [transaction](/docs/v25.4/transactions). It does not return results. Use it to modify data or to apply additional logic atomically with replication. Changes will not be [committed](/docs/v25.4/commit-transaction) until after [`onWrite`](#configure-target-tables-on-write) returns.
...
This function returns the [fully-qualified](/docs/v25.4/sql-name-resolution), quoted name of the target schema (for example, `db.public`). Use this to identify or reference the schema in SQL queries or metadata operations.
...
This function returns the [fully-qualified](/docs/v25.4/sql-name-resolution), quoted name of the target table (for example, `db.public.users`). Use this to reference the table in SQL queries or for metadata inspection.
...
The following example demonstrates how to use the [`getTX()`](#on-write-get-tx) and [`write()`](#on-write-write) API functions within [`onWrite`](#configure-target-tables-on-write) to access the [transaction](/docs/v25.4/transactions) context and execute custom SQL queries during replication. This is useful when you need to:
...
Userscripts include a built-in global `console` object that provides standard logging functions for debugging and observability. Messages written through `console` are captured by `replicator` and forwarded to its [structured logging system](/docs/molt/molt-replicator#logging), making them visible in logs, monitoring tools, or local output depending on your deployment.

Further observability can be accessed with the userscript [metrics](/docs/molt/userscript-metrics).
