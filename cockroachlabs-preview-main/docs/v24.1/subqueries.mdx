...
SQL subqueries enable reuse of the results from a [selection query](/docs/v24.1/selection-queries) within another query.

CockroachDB supports two kinds of subqueries:

- **Relational** subqueries, which appear as operands in [selection queries](/docs/v24.1/selection-queries) and [table expressions](/docs/v24.1/table-expressions).
- **Scalar** subqueries, which appear as operands in a [scalar expression](/docs/v24.1/scalar-expressions).
...
This is true for subqueries defined using the [`(...)`](/docs/v24.1/table-expressions#use-a-subquery) notation and those defined using [`WITH`](/docs/v24.1/table-expressions#use-the-output-of-another-statement).
...
This query inserts 3 rows into `t`, even though the surrounding query only observes 1 row using [`LIMIT`](/docs/v24.1/limit-offset).
...
The [cost-based optimizer](/docs/v24.1/cost-based-optimizer) supports most [correlated subqueries](https://wikipedia.org/wiki/Correlated_subquery), with the exception of correlated subqueries that generate side effects inside a `CASE` statement.

### `LATERAL` subqueries

A `LATERAL` subquery is a correlated subquery that references another query or subquery in its `SELECT` statement, usually in the context of a [`LEFT` join](/docs/v24.1/joins#left-outer-joins) or an [`INNER` join](/docs/v24.1/joins#inner-joins). Unlike other correlated subqueries, `LATERAL` subqueries iterate through each row in the referenced query for each row in the inner subquery, like a [for loop](https://wikipedia.org/wiki/For_loop).
...
`LATERAL` subquery joins are especially useful when the join table includes a [computed column](/docs/v24.1/computed-columns).
