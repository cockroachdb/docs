...
The SQL layer of CockroachDB's architecture exposes a SQL API to developers and converts high-level [SQL statements](/docs/v24.1/sql-statements) into low-level read and write requests to the underlying key-value store, which are passed to the [transaction Layer](/docs/v24.1/architecture/transaction-layer).
...
Once CockroachDB has been [deployed](/docs/cockroachcloud/quickstart), developers need only a [connection string](/docs/v24.1/connection-parameters) to the cluster, and they can start working with SQL statements.
...
Requests to the cluster arrive as SQL statements, but data is ultimately written to and read from the [storage layer](/docs/v24.1/architecture/storage-layer) as key-value (KV) pairs. To handle this, the SQL layer converts SQL statements into a plan of KV operations, which it then passes along to the [transaction layer](/docs/v24.1/architecture/transaction-layer).
...
- Converts SQL statements into low-level KV operations, which it sends as requests to the [transaction layer](/docs/v24.1/architecture/transaction-layer).

## Components

### Relational structure

Developers experience data stored in CockroachDB as a relational structure comprised of rows and columns. Sets of rows and columns are further organized into [tables](/docs/v24.1/show-tables). Collections of tables are then organized into [databases](/docs/v24.1/show-databases). A CockroachDB cluster can contain many databases.

CockroachDB provides typical relational features like [constraints](/docs/v24.1/constraints) (e.g., [foreign keys](/docs/v24.1/foreign-key)). These features mean that application developers can trust that the database will ensure consistent structuring of the application's data; data validation doesn't need to be built into the application logic separately.

### SQL API

CockroachDB implements most of the ANSI SQL standard to manifest its relational structure. For a complete list of the SQL features CockroachDB supports, see [SQL Feature Support](/docs/v24.1/sql-feature-support).

Importantly, through the SQL API developers have access to ACID-semantic [transactions](/docs/v24.1/transactions) like they would through any SQL database (using [`BEGIN`](/docs/v24.1/begin-transaction), [`COMMIT`](/docs/v24.1/commit-transaction), etc.).

### PostgreSQL wire protocol

SQL queries reach your cluster through the PostgreSQL wire protocol. This makes connecting your application to the cluster simple by supporting many PostgreSQL-compatible [drivers and ORMs](/docs/v24.1/install-client-drivers).
...
1. The AST is transformed into a high-level logical query plan. During this transformation, CockroachDB also performs [semantic analysis](https://wikipedia.org/wiki/Semantic_analysis_(compilers)), which includes operations like:
    ...
    - Finalizing which data types to use for intermediate results, e.g., when a query contains one or more [subqueries](/docs/v24.1/subqueries).
...
3. The logical plan is *optimized* using a [search algorithm](/docs/v24.1/cost-based-optimizer#how-is-cost-calculated) that evaluates many possible ways to execute a query and selects an execution plan with the least costs.

The result of the final step above is an optimized logical plan. To view the logical plan generated by the [cost-based optimizer](/docs/v24.1/cost-based-optimizer), use the [`EXPLAIN (OPT)`](/docs/v24.1/explain) statement.
...
More concretely, the physical planning phase transforms the optimized logical plan generated during [logical planning](#logical-planning) into a [directed acyclic graph](https://wikipedia.org/wiki/Directed_acyclic_graph) (DAG) of physical *SQL operators*. These operators can be viewed by running the [`EXPLAIN(DISTSQL)`](/docs/v24.1/explain) statement.

Because the [distribution layer](/docs/v24.1/architecture/distribution-layer) presents the abstraction of a single key space, the SQL layer can perform read and write operations for any range on any node. This allows the SQL operators to behave identically whether planned in gateway or distributed mode.
...
If [vectorized execution](/docs/v24.1/vectorized-execution) is enabled, the physical plan is sent to nodes to be processed by the vectorized execution engine.

Upon receiving the physical plan, the vectorized engine reads batches of table data [from disk](/docs/v24.1/architecture/storage-layer) and converts the data from row format to columnar format. These batches of column data are stored in memory so the engine can access them quickly during execution.
...
CockroachDB performs schema changes, such as the [addition of columns](/docs/v24.1/alter-table#add-column) or [secondary indexes](/docs/v24.1/create-index), using a protocol that allows tables to remain online (i.e., able to serve reads and writes) during the schema change. This protocol allows different nodes in the cluster to asynchronously transition to a new table schema at different times.
...
For more information, including examples and limitations, see [Online Schema Changes](/docs/v24.1/online-schema-changes).
...
Learn how CockroachDB handles concurrent requests in the [transaction layer](/docs/v24.1/architecture/transaction-layer).
