...
<Frame>
  ![range-lookup.png](/images/v24.1/range-lookup.png)
</Frame>
...
- Membership changes to a range's Raft group (discussed in more detail in the [Replication Layer](/docs/v24.1/architecture/replication-layer#membership-changes-rebalance-repair) )
- Range splits
- Range merges
...
By default, CockroachDB attempts to keep ranges/replicas at [the default range size](/docs/v24.1/configure-replication-zones#range-max-bytes). Once a range reaches that limit, we split it into two smaller ranges (composed of contiguous key spaces).
...
By default, CockroachDB automatically merges small ranges of data together to form fewer, larger ranges (up to [the default range size](/docs/v24.1/configure-replication-zones#range-max-bytes)). This can improve both query latency and cluster survivability.
...
Learn how CockroachDB copies data and ensures consistency in the [replication layer](/docs/v24.1/architecture/replication-layer).
