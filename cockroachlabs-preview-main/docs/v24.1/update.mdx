...
import { Update3 } from "/snippets/syntax-diagrams/Update3.jsx";

The `UPDATE` [statement](/docs/v24.1/sql-statements) updates rows in a table.

<Warning>
If you update a row that contains a column referenced by a [foreign key constraint](/docs/v24.1/foreign-key) and has an [`ON UPDATE` action](/docs/v24.1/foreign-key#foreign-key-actions), all of the dependent rows will also be updated.
</Warning>

## Required privileges

The user must have the `SELECT` and `UPDATE` [privileges](/docs/v24.1/security-reference/authorization#managing-privileges) on the table.
...
To view how the index hint modifies the query plan that CockroachDB follows for updating rows, use an [`EXPLAIN (OPT)`](/docs/v24.1/explain#opt-option) statement. To see all indexes available on a table, use [`SHOW INDEXES`](/docs/v24.1/show-index).
...
<Note>
If the [`sql_safe_updates`](/docs/v24.1/cockroach-sql#allow-potentially-unsafe-sql-statements) session variable is set to `true`, the client will prevent the update. `sql_safe_updates` is set to `true` by default.
</Note>
...
You can use a [`SET`](/docs/v24.1/set-vars) statement to set session variables.
...
In this example, the `RETURNING` clause returns the `id` value of the row updated. The language-specific versions assume that you have installed the relevant [client drivers](/docs/v24.1/install-client-drivers).
...
Now suppose you want to update a couple rows in the table, based on their contents. You can use the [`EXPLAIN (OPT)`](/docs/v24.1/explain#opt-option) command to see how the [cost-based optimizer](/docs/v24.1/cost-based-optimizer) decides to perform the `UPDATE` statement:
...
To help you avoid failures arising from misbehaving applications that bloat the size of rows, you can specify the behavior when a row or individual [column family](/docs/v24.1/column-families) larger than a specified size is written to the database. Use the [cluster settings](/docs/v24.1/cluster-settings) `sql.guardrails.max_row_size_log` to discover large rows and `sql.guardrails.max_row_size_err` to reject large rows.

When you write a row that exceeds `sql.guardrails.max_row_size_log`:

- `INSERT`, `UPSERT`, `UPDATE`, `CREATE TABLE AS`, `CREATE INDEX`, `ALTER TABLE`, `ALTER INDEX`, `IMPORT`, or `RESTORE` statements will log a `LargeRow` to the [`SQL_PERF`](/docs/v24.1/logging#sql_perf) channel.
...
- `CREATE TABLE AS`, `CREATE INDEX`, `ALTER TABLE`, `ALTER INDEX`, `IMPORT`, and `RESTORE` statements will log a `LargeRowInternal` event to the [`SQL_INTERNAL_PERF`](/docs/v24.1/logging#sql_internal_perf) channel.
- `SELECT`, `DELETE`, `TRUNCATE`, and `DROP` are not affected.
...
- [Ordering of rows in DML statements](/docs/v24.1/order-by#ordering-rows-in-dml-statements)
