...
CockroachDB clusters allow users to authenticate with Single Sign-on (SSO), both to the [DB Console](/docs/v24.1/ui-overview), and for SQL client access.
...
2. Add your IdP's formal `issuer` name (this must match the `iss` field in the JWT itself) to your cluster's list of accepted token issuers.
    ...
    This must match your cluster's configured value for `server.oidc_authentication.provider_url`. Refer to [Single Sign-on (SSO) for DB Console](/docs/v24.1/sso-db-console#cluster-settings). Issuers are expected to publish their configuration at `https://{ domain }/.well-known/openid-configuration`. For example:
...
3. `server.jwt_authentication.audience`
    ...
    The ID of your cluster as specified by the IdP, or a JSON array of such names. This must match `server.oidc_authentication.client_id`; refer to [Single Sign-on (SSO) for DB Console](/docs/v24.1/sso-db-console).
...
<Tip>
This example uses [`cockroach sql`](/docs/v24.1/cockroach-sql), but you can use any SQL client that supports sufficiently long passwords.
</Tip>
...
- Learn more about [Authentication](/docs/v24.1/authentication) in CockroachDB.
