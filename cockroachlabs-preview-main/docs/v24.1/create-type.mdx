...
import { CreateType3 } from "/snippets/syntax-diagrams/CreateType3.jsx";

The `CREATE TYPE` [statement](/docs/v24.1/sql-statements) creates a new type in a [database](/docs/v24.1/create-database). After the type is created, it can only be referenced from the database that contains the type.
...
<Note>
The `CREATE TYPE` statement performs a schema change. For more information about how online schema changes work in CockroachDB, see [Online Schema Changes](/docs/v24.1/online-schema-changes).
</Note>
...
To see the values accepted by the underlying [`ENUM`](/docs/v24.1/enum) data type, use [`SHOW ENUMS`](/docs/v24.1/show-enums):
