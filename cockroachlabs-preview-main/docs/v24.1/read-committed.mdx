...
`READ COMMITTED` is one of two [transaction isolation levels](https://wikipedia.org/wiki/Isolation_(database_systems)) supported on CockroachDB. By default, CockroachDB uses the [`SERIALIZABLE`](/docs/v24.1/demo-serializable) isolation level, which is the strongest [ANSI transaction isolation level](https://wikipedia.org/wiki/Isolation_(database_systems)#Isolation_levels).

`READ COMMITTED` isolation is appropriate in the following scenarios:

- Your application needs to maintain a high workload concurrency with minimal [transaction retries](/docs/v24.1/developer-basics#transaction-retries), and it can tolerate potential [concurrency anomalies](#concurrency-anomalies). Predictable query performance at high concurrency is more valuable than guaranteed transaction [serializability](/docs/v24.1/developer-basics#serializability-and-transaction-contention).
...
Whereas `SERIALIZABLE` isolation guarantees data correctness by placing transactions into a [serializable ordering](/docs/v24.1/demo-serializable), `READ COMMITTED` isolation permits some [concurrency anomalies](#concurrency-anomalies) in exchange for minimizing transaction aborts, [retries](/docs/v24.1/developer-basics#transaction-retries), and blocking. Compared to `SERIALIZABLE` transactions, `READ COMMITTED` transactions do **not** return [serialization errors](/docs/v24.1/transaction-retry-error-reference) that require client-side handling. See [`READ COMMITTED` transaction behavior](#read-committed-transaction-behavior).
...
By default, the `sql.txn.read_committed_isolation.enabled` [cluster setting](/docs/v24.1/cluster-settings) is `true`, enabling `READ COMMITTED` transactions. If the cluster setting is `false`, `READ COMMITTED` transactions will run as `SERIALIZABLE`.
...
- The [`SET SESSION CHARACTERISTICS`](/docs/v24.1/set-vars#special-syntax-cases) statement, which applies to the current session:

    ```sql
    SET SESSION CHARACTERISTICS AS TRANSACTION ISOLATION LEVEL READ COMMITTED;
    ```
- The [`default_transaction_isolation`](/docs/v24.1/session-variables#default-transaction-isolation) session variable:
    ...
    ```
- The `default_transaction_isolation` session variable as a [connection parameter](/docs/v24.1/connection-parameters#supported-options-parameters) with [`cockroach sql`](/docs/v24.1/cockroach-sql):
...
- The [`BEGIN TRANSACTION ISOLATION LEVEL`](/docs/v24.1/begin-transaction#parameters) statement:

    ```sql
    BEGIN TRANSACTION ISOLATION LEVEL READ COMMITTED;
    ```
- The [`SET TRANSACTION ISOLATION LEVEL`](/docs/v24.1/set-transaction#parameters) statement, at the beginning of the transaction:

    ```sql
    BEGIN;
      SET TRANSACTION ISOLATION LEVEL READ COMMITTED;
    ```
- The [`transaction_isolation`](/docs/v24.1/session-variables#transaction-isolation) session variable, at the beginning of the transaction:
...
`READ COMMITTED` and `SERIALIZABLE` transactions both serve globally consistent ("non-stale") reads and [commit atomically](/docs/v24.1/developer-basics#how-transactions-work-in-cockroachdb). `READ COMMITTED` transactions have the following differences:

- Writes in concurrent `READ COMMITTED` transactions can interleave without aborting transactions, and a write can never block a non-locking read of the same row. This is because `READ COMMITTED` transactions are not required to be placed into a [serializable ordering](/docs/v24.1/demo-serializable).
- Whereas statements in `SERIALIZABLE` transactions see data that committed before the transaction began, statements in `READ COMMITTED` transactions see data that committed before each **statement** began. If rows are being updated by concurrent writes, reads in a `READ COMMITTED` transaction can [return different results](#non-repeatable-reads-and-phantom-reads).
    ...
    <Note>
    For details on how this is implemented, see [Read snapshots](/docs/v24.1/architecture/transaction-layer#read-snapshots).
    </Note>
...
- When using `READ COMMITTED` isolation, you do **not** need to implement [client-side retries](/docs/v24.1/transaction-retry-error-reference#client-side-retry-handling) to handle [serialization errors](/docs/v24.1/transaction-retry-error-reference) under [transaction contention](/docs/v24.1/performance-best-practices-overview#transaction-contention). `READ COMMITTED` transactions never return [`RETRY_SERIALIZABLE`](/docs/v24.1/transaction-retry-error-reference#retry_serializable) errors, and will only return `40001` errors in limited cases, as described in the following points.

`READ COMMITTED` transactions can abort in certain scenarios:

- Transactions at all isolation levels are subject to [*lock contention*](/docs/v24.1/performance-best-practices-overview#transaction-contention), where a transaction attempts to lock a row that is already locked by a [write](/docs/v24.1/architecture/transaction-layer#write-intents) or [locking read](#locking-reads). In such cases, the later transaction is blocked until the earlier transaction commits or rolls back, thus releasing its lock on the row. Lock contention that produces a *deadlock* between two transactions will result in a transaction abort and a `40001` error ([`ABORT_REASON_ABORTED_RECORD_FOUND`](/docs/v24.1/transaction-retry-error-reference#abort_reason_aborted_record_found) or [`ABORT_REASON_PUSHER_ABORTED`](/docs/v24.1/transaction-retry-error-reference#abort_reason_pusher_aborted)) returned to the client.
- [Constraint](/docs/v24.1/constraints) violations will abort transactions at all isolation levels.
- In rare cases under `READ COMMITTED` isolation, a [`RETRY_WRITE_TOO_OLD`](/docs/v24.1/transaction-retry-error-reference#retry_write_too_old) or [`ReadWithinUncertaintyIntervalError`](/docs/v24.1/transaction-retry-error-reference#readwithinuncertaintyintervalerror) error can be returned to the client if a statement has already begun streaming a partial result set back to the client and cannot retry transparently. By default, the result set is buffered up to the value of the [`sql.defaults.results_buffer.size`](/docs/v24.1/cluster-settings#setting-sql-defaults-results-buffer-size) cluster setting before overflowing and being streamed to the client.

### Concurrency anomalies

Statements in concurrent `READ COMMITTED` transactions can interleave with each other. This can create concurrency anomalies that are not permitted under `SERIALIZABLE` isolation, which places concurrent transactions into a [serializable ordering](/docs/v24.1/demo-serializable).

<Tip>
The behaviors described in this section assume the use of non-locking reads. You can prevent concurrency anomalies through the selective use of [locking reads](#locking-reads), which can also increase latency due to [lock contention](/docs/v24.1/performance-best-practices-overview#transaction-contention).
</Tip>
...
<Note>
Under `SERIALIZABLE` isolation, transaction `A` would have aborted with a [`RETRY_WRITE_TOO_OLD`](/docs/v24.1/transaction-retry-error-reference#retry_write_too_old) error, prompting the client to retry the transaction.
</Note>
...
The update in Session 2 appears to be "lost" because its result is overwritten by a concurrent transaction. It is **not** lost at the database level, and can be found using [`AS OF SYSTEM TIME`](/docs/v24.1/as-of-system-time) and a timestamp earlier than the commit in Session 1:
...
<Note>
For details on why this is allowed, see [Read refreshing](/docs/v24.1/architecture/transaction-layer#write-skew-tolerance).
</Note>
...
Non-locking reads can allow intermediate writes to update rows before `READ COMMITTED` transactions commit, potentially creating [concurrency anomalies](#concurrency-anomalies). Locking reads prevent such anomalies, but increase the amount of lock contention that [may require intervention](/docs/v24.1/performance-recipes#waiting-transaction) if latency becomes too high. Note that locking reads do **not** prevent [phantom reads](#non-repeatable-reads-and-phantom-reads) that are caused by the insertion of new rows, since only existing rows can be locked.

<Note>
Locking reads are not effective for emulating `SERIALIZABLE` transactions, which can avoid locking reads because they always [retry or abort if reads are not current](/docs/v24.1/architecture/transaction-layer#read-refreshing). As a result, `READ COMMITTED` transactions that use locking reads will perform differently than `SERIALIZABLE` transactions at various levels of concurrency.
</Note>
...
1. Open the SQL shell using [`cockroach demo`](/docs/v24.1/cockroach-demo).
...
In a new terminal (Session 2), open the SQL shell on your [`cockroach demo`](/docs/v24.1/cockroach-demo) cluster. Start a transaction:
...
[By design under `READ COMMITTED` isolation](/docs/v24.1/architecture/transaction-layer#write-skew-tolerance), CockroachDB allows the transaction to commit even though its previous read (the `SELECT` query) has changed due to the concurrent transaction in Session 2.
...
Check to make sure that another doctor is on call for `2023-12-05`. Use [`FOR UPDATE`](/docs/v24.1/select-for-update) to lock the rows so that only the current transaction can update them:
...
Check to make sure that another doctor is on call for `2023-12-05`. Use [`FOR SHARE`](/docs/v24.1/select-for-update) to lock the rows so that they cannot be updated by another transaction:
...
- [Serializable Transactions](/docs/v24.1/demo-serializable)
