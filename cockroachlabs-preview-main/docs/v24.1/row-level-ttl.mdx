...
- Outbox pattern: When events are written to an outbox table and published to an external system like [Kafka](https://wikipedia.org/wiki/Apache_Kafka) using CockroachDB's [Change Data Capture (CDC)](/docs/v24.1/change-data-capture-overview) feature (also known as ["changefeeds"](#changefeeds)), those events must be deleted to prevent unbounded growth in the size of the outbox table.

## How it works

At a high level, Row-Level TTL works by:

- Issuing a [selection query](/docs/v24.1/selection-queries) at a [historical timestamp](/docs/v24.1/as-of-system-time), yielding a set of rows that are eligible for deletion (also known as "expired").
- Issuing batched [`DELETE`](/docs/v24.1/delete) statements for the expired rows.
- As part of the above process, deciding how many rows to [`SELECT`](/docs/v24.1/select-clause) and [`DELETE`](/docs/v24.1/delete) at once in each of the above queries.
- Running the SQL queries described above in parallel as [background jobs](/docs/v24.1/show-jobs).
- To minimize the performance impact on foreground application queries, the background deletion queries are rate limited; they are also submitted at a lower priority level using the [admission control system](/docs/v24.1/admission-control). When foreground traffic increases, CockroachDB will reduce the resources allocated to TTL deletes to handle the foreground traffic. When foreground traffic decreases, CockroachDB will increase the resources allocated to TTL deletes.
- Latency of row-level TTL queries is further reduced by using the elastic CPU limiter, which dynamically controls the total CPU percentage used by row-level TTL reads. The elastic CPU limiter can be disabled for row-level TTL queries by setting the `kvadmission.low_pri_read_elastic_control.enabled` and `sqladmission.low_pri_read_response_elastic_control.enabled` [cluster settings](/docs/v24.1/cluster-settings) to `false`.

The process above is conceptually similar to the process described by [Batch delete on an indexed column](/docs/v24.1/bulk-delete-data#batch-delete-on-an-indexed-column), except that Row-Level TTL is built into CockroachDB, so it saves you from having to write code to manage the process from your application and/or external job processing framework, including tuning the rate and performance of your background queries so they don't affect foreground application query performance.
...
- [Using `ttl_expiration_expression`](#using-ttl_expiration_expression) is useful for customizing expiration logic by providing an expression. For example, you could get the same behavior as `ttl_expire_after` by creating a [`TIMESTAMPTZ`](/docs/v24.1/timestamp) column with a default value and having the `ttl_expiration_expression` reference that column.
...
- If you add `ttl_expire_after` to an existing table, it **will cause a full table rewrite, which can affect performance**. Specifically, it will result in a [schema change](/docs/v24.1/online-schema-changes) that (1) creates a new [hidden column](/docs/v24.1/show-create#show-the-create-table-statement-for-a-table-with-a-hidden-column) `crdb_internal_expiration` for all rows, and (2) backfills the value of that new column to `now()` + `ttl_expire_after`.
- You cannot use `ttl_expire_after` with an existing [`TIMESTAMPTZ`](/docs/v24.1/timestamp) column.
- If you use `ttl_expiration_expression`, you can use an existing [`TIMESTAMPTZ`](/docs/v24.1/timestamp) column called e.g. `updated_at`.
</Tip>
...
To add custom expiration logic using `ttl_expiration_expression`, issue the following SQL statement that uses the [`ttl_expiration_expression`](#param-ttl-expiration-expression) parameter, which defines a [`TIMESTAMPTZ`](/docs/v24.1/timestamp) after which the row is considered expired:
...
To see the storage parameters, enter the [`SHOW CREATE TABLE`](/docs/v24.1/show-create) statement:
...
1. Creates a repeating [scheduled job](#view-scheduled-ttl-jobs) for the table and sets it to run once per day.
2. Adds a `NOT VISIBLE` column called `crdb_internal_expiration` of type [`TIMESTAMPTZ`](/docs/v24.1/timestamp) to represent the TTL.
3. Implicitly adds the `ttl` and `ttl_cron` [storage parameters](#ttl-storage-parameters).

To see the hidden column and the storage parameters, enter the [`SHOW CREATE TABLE`](/docs/v24.1/show-create) statement:
...
The settings that control the behavior of Row-Level TTL are provided using [storage parameters](/docs/v24.1/sql-grammar#opt_with_storage_parameter_list). These parameters can be set during table creation using [`CREATE TABLE`](#create-a-table-with-a-ttl_expiration_expression), added to an existing table using the [`ALTER TABLE`](#add-or-update-the-row-level-ttl-for-an-existing-table) statement, or [reset to default values](#reset-a-storage-parameter-to-its-default-value).

<table><thead><tr><th>Option</th><th>Description</th><th>Associated cluster setting</th></tr></thead><tbody><tr><td><code>ttl&#95;expiration&#95;expression</code><a></a></td><td><strong>Recommended</strong>. SQL expression that defines the TTL expiration. Must evaluate to a <a href="/docs/v24.1/timestamp"><code>TIMESTAMPTZ</code></a>. This and/or <a href="#param-ttl-expire-after"><code>ttl&#95;expire&#95;after</code></a> are required to enable TTL. This parameter is useful when you want to set the TTL for individual rows in the table. For an example, see <a href="#create-a-table-with-a-ttl_expiration_expression">Create a table with a <code>ttl&#95;expiration&#95;expression</code></a>.</td><td>N/A</td></tr><tr><td><code>ttl&#95;expire&#95;after</code><a></a></td><td>The <a href="/docs/v24.1/interval">interval</a> when a TTL will expire. This and/or <a href="#param-ttl-expiration-expression"><code>ttl&#95;expiration&#95;expression</code></a> are required to enable TTL. Minimum value: <code>'1 microsecond'</code>.</td><td>N/A</td></tr><tr><td><code>ttl</code><a></a></td><td>Signifies if a TTL is active. Automatically set.</td><td>N/A</td></tr><tr><td><code>ttl&#95;select&#95;batch&#95;size</code></td><td>How many rows to <a href="/docs/v24.1/select-clause">select</a> at one time during the row expiration check. Defaults to the value of the associated cluster setting if unset.</td><td><code>sql.ttl.default&#95;select&#95;batch&#95;size</code><br/>Default: 500. Minimum: 1.</td></tr><tr><td><code>ttl&#95;delete&#95;batch&#95;size</code><a></a></td><td>How many rows to <a href="/docs/v24.1/delete">delete</a> at a time. Defaults to the value of the associated cluster setting if unset.</td><td><code>sql.ttl.default&#95;delete&#95;batch&#95;size</code><br/>Default: 100. Minimum: 1.</td></tr><tr><td><code>ttl&#95;select&#95;rate&#95;limit</code></td><td>Maximum number of rows to be selected per second (rate limit). Defaults to the value of the associated cluster setting if unset.<br/><br/>Note: The rate limit is applied per node per table. The TTL job prefers to assign work to the leaseholder of each range, but this is not guaranteed. In some cases, non-leaseholder nodes may be assigned work.<br/><br/>As a result, the effective cluster-wide rate depends on how many nodes end up processing TTL work for the table.</td><td><code>sql.ttl.default&#95;select&#95;rate&#95;limit</code><br/>Default: 0. Minimum: 0 (unlimited).</td></tr><tr><td><code>ttl&#95;delete&#95;rate&#95;limit</code></td><td>Maximum number of rows to be deleted per second (rate limit). Defaults to the value of the associated cluster setting if unset.<br/><br/>Note: The rate limit is applied per node per table, with the same leaseholder preference and fallback behavior as <code>ttl&#95;select&#95;rate&#95;limit</code>.<br/><br/>The actual cluster-wide rate will vary depending on which nodes execute TTL work.</td><td><code>sql.ttl.default&#95;delete&#95;rate&#95;limit</code><br/>Default: 100. Minimum: 0 (unlimited).</td></tr><tr><td><code>ttl&#95;row&#95;stats&#95;poll&#95;interval</code></td><td>If set, counts rows and expired rows on the table to report as Prometheus metrics while the TTL job is running. Unset by default, meaning no stats are fetched and reported.</td><td>N/A</td></tr><tr><td><code>ttl&#95;pause</code><a></a></td><td>If set, stops the TTL job from executing.</td><td>N/A</td></tr><tr><td><code>ttl&#95;job&#95;cron</code><a></a></td><td>Frequency at which the TTL job runs, specified using <a href="https://cron.help/">CRON syntax</a>. Default: <code>'@daily'</code> (was <code>'@hourly'</code>).</td><td>N/A</td></tr><tr><td><code>ttl&#95;disable&#95;changefeed&#95;replication</code></td><td><InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v24.1: Disables <a href="/docs/v24.1/change-data-capture-overview">changefeed</a> replication for the deletes performed by the TTL job.</td><td><code>sql.ttl.changefeed&#95;replication.disabled</code></td></tr></tbody></table>
...
The table below lists the metrics you can use to monitor the effectiveness of your TTL settings. These metrics are visible on the [Advanced Debug Page](/docs/v24.1/ui-debug-pages), as well as at the `_status/vars` endpoint which can be scraped by [Prometheus](/docs/v24.1/monitor-cockroachdb-with-prometheus).
...
- Set the `server.child_metrics.enabled` [cluster setting](/docs/v24.1/cluster-settings) to `true`.
- Set the `ttl_label_metrics` storage parameter to `true`.

<Note>
For more information about the issues (including negative performance impacts) that can arise when you add cardinality, see the considerations listed in [Using changefeed metrics labels](/docs/v24.1/monitor-and-debug-changefeeds#using-changefeed-metrics-labels).
</Note>
...
- It must evaluate to a [`TIMESTAMPTZ`](/docs/v24.1/timestamp).
- It must not reference any columns outside the table to which it is applied.
- Any column it references cannot be [dropped](/docs/v24.1/alter-table#drop-column) or have its [type altered](/docs/v24.1/alter-type).
- Finally, if the [column is renamed](/docs/v24.1/alter-table#rename-column), the value of `ttl_expiration_expression` is automatically updated.
...
Use the SQL syntax shown below to create a new table with rows that expire after a 3 month [interval](/docs/v24.1/interval), execute a statement like the following:
...
To add or change the row-level TTL expiration for an existing table, use [`ALTER TABLE`](/docs/v24.1/alter-table) as shown in the following example. This example assumes you have an existing [`TIMESTAMPTZ`](/docs/v24.1/timestamp) or [`DATE`](/docs/v24.1/date) column you can use for the [`ttl_expiration_expression`](#param-ttl-expiration-expression).
...
You can use [`SHOW SCHEDULES`](/docs/v24.1/show-schedules) to view all TTL-related scheduled jobs by executing the following query:
...
You can use [`SHOW JOBS`](/docs/v24.1/show-jobs) to see any running TTL jobs by executing the following query:
...
<Tip>
You can also view running TTL jobs using the [Jobs page in the DB Console](/docs/v24.1/ui-jobs-page)
</Tip>

### View TTL storage parameters on a table

To view TTL storage parameters on a table, you can use [`SHOW CREATE TABLE`](/docs/v24.1/show-create):
...
Setting a TTL on a table controls when the rows therein are considered expired, but it only says that such rows *may* be deleted at any time after the expiration. To control how often the TTL deletion job runs, use the [`ttl_job_cron` storage parameter](#param-ttl-job-cron), which supports [CRON syntax](https://cron.help/). Cockroach Labs recommends setting `ttl_job_cron` to be equal to or longer than the [`gc.ttlseconds`](/docs/v24.1/configure-replication-zones#gc-ttlseconds) setting, which is the garbage collection interval for the cluster. The default value of `gc.ttlseconds` is 14400, or 4 hours. The CRON pattern for every four hours is `'0 */4 * * *'`.

To control the job interval at [`CREATE TABLE`](/docs/v24.1/create-table) time, add the storage parameter as shown below:
...
To update the TTL deletion job interval on a table that already has Row-Level TTL enabled, use [`ALTER TABLE`](/docs/v24.1/alter-table):
...
To reset a [TTL storage parameter](#ttl-storage-parameters) to its default value, use the [`ALTER TABLE`](/docs/v24.1/alter-table) statement:
...
To disable TTL jobs for the whole cluster, set the `sql.ttl.job.enabled` [cluster setting](/docs/v24.1/cluster-settings) to `false`:
...
To view the [cluster settings](/docs/v24.1/cluster-settings) that control how Row-Level TTL works, issue the following query:
...
Row-level TTL interacts with [changefeeds](/docs/v24.1/create-and-configure-changefeeds) in the following ways:

- When expired rows are deleted, a [changefeed delete message](/docs/v24.1/changefeed-messages#delete-messages) is emitted.
...
For guidance on how to filter changefeed messages to emit row-level TTL deletes only, refer to [Change Data Capture Queries](/docs/v24.1/cdc-queries#reference-ttl-in-a-cdc-query).

## Backup and restore

Row-level TTL interacts with [backup and restore](/docs/v24.1/backup-and-restore-overview) in the following ways:

- When you run a [`BACKUP`](/docs/v24.1/backup), all row-level TTL information associated with the tables being backed up (including TTL expiration times) is also backed up.
- When you [`RESTORE`](/docs/v24.1/restore) from a backup, all row-level TTL information associated with the tables being restored (including TTL expiration times) is also restored. Any expired rows in the restored tables are eligible to be [deleted](#when-are-rows-deleted) by the [TTL job](#how-it-works).

## Required Privileges

To add or update Row-Level TTL settings on a table, you must have one of the following:

- Membership to the [owner](/docs/v24.1/security-reference/authorization#object-ownership) role for the database where the table is located.
- The [`CREATE` or `ALTER` privilege](/docs/v24.1/security-reference/authorization#supported-privileges) on the database where the table is located.
...
- [Bulk-delete Data](/docs/v24.1/bulk-delete-data)
- [Delete data](/docs/v24.1/delete-data)
- [`DELETE`](/docs/v24.1/delete)
- [`SELECT` clause](/docs/v24.1/select-clause)
- [`AS OF SYSTEM TIME`](/docs/v24.1/as-of-system-time)
- [`TIMESTAMPTZ`](/docs/v24.1/timestamp)
- [`INTERVAL`](/docs/v24.1/interval)
- [SQL Performance Best Practices](/docs/v24.1/performance-best-practices-overview)
- [Developer Guide Overview](/docs/v24.1/developer-guide-overview)
