...
You can validate a backup of a [cluster](/docs/v24.1/backup#back-up-a-cluster), [database](/docs/v24.1/backup#back-up-a-database), or [table](/docs/v24.1/backup#back-up-a-table-or-view) backup with one of the following [`SHOW BACKUP`](/docs/v24.1/show-backup) or [`RESTORE`](/docs/v24.1/restore) statements. The three options result in increasing levels of backup validation:
...
3. Somewhat frequent ["full"](/docs/v24.1/restore) restores: Validates a backup completely.
...
Using `SHOW BACKUP` with the `check_files` option, you can check that all [SST and metadata files](/docs/v24.1/backup-architecture) that belong to a backup are present in the storage location.
...
To validate that a backup is restorable, you can run `RESTORE` with the `schema_only` option, which will complete a restore **without** restoring any rows. This process is significantly faster than running a [regular restore](/docs/v24.1/restore#examples) for the purposes of validation.
...
<Note>
Validation of full-cluster restores with `schema_only` must be run on an empty cluster in the same way as a complete [full-cluster restore](/docs/v24.1/restore#full-cluster). Once you have successfully validated the restore, you can destroy the test cluster.
</Note>
...
You can also use the [`new_db_name` option](/docs/v24.1/restore#rename-a-database-on-restore) to restore a database to a different name. For example, `new_db_name = test_movr`.
