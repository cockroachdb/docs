...
- Started a [secure CockroachDB self-hosted cluster](/docs/v24.1/secure-a-cluster).
...
<Frame>
  ![CockroachDB integration for Datadog installation](/images/v24.1/datadog-crdb-integration.png)
</Frame>
...
This enables metrics collection via our [Prometheus endpoint](/docs/v24.1/monitoring-and-alerting#prometheus-endpoint).
...
For example, if you used [`cockroach cert`](/docs/v24.1/cockroach-cert) to [secure your cluster](/docs/v24.1/secure-a-cluster#step-1-generate-certificates), the paths would look something like:
...
Optionally enable Datadog to collect [CockroachDB logs](/docs/v24.1/logging-overview) by adding the following block to `cockroachdb.d/conf.yaml`:
...
The `path` value specifies the [default](/docs/v24.1/configure-logs#default-logging-configuration) CockroachDB log file and location.

<Note>
You can configure both the CockroachDB [logging directory](/docs/v24.1/configure-logs#set-file-defaults) and [log files](/docs/v24.1/configure-logs#output-to-files).
</Note>
...
-------------------
  ...
  metadata:
    ...
    version.minor: 2
    version.patch: 0
    version.raw: v24.1.0-alpha.00000000-1724-gc5c74249f7
...
<Frame>
  ![CockroachDB Overview dashboard in Datadog Dashboard List](/images/v24.1/datadog-crdb-dashboard-list.png)
</Frame>

This sample dashboard presents metrics on cluster availability, query performance, and resource usage:

<Frame>
  ![CockroachDB Overview dashboard for Datadog](/images/v24.1/datadog-crdb-overview-dashboard.png)
</Frame>
...
To test the dashboard functionality, use [`cockroach workload`](/docs/v24.1/cockroach-workload) to run a sample workload on the cluster.
...
<Frame>
  ![CockroachDB Overview dashboard for Datadog with SQL metrics](/images/v24.1/datadog-crdb-workload-dashboard.png)
</Frame>
...
Select **Threshold Alert** as the detection method. You can use this option to configure an alert that is sent when a [supported metric](https://docs.datadoghq.com/integrations/cockroachdb?tab=host#data-collected) reaches a given threshold. For descriptions of some useful CockroachDB alerts, see [Monitoring and Alerting](/docs/v24.1/monitoring-and-alerting#events-to-alert-on).

The example alert below will trigger when [a node has less than 15% of storage capacity remaining](/docs/v24.1/monitoring-and-alerting#node-is-running-low-on-disk-space):

<Frame>
  ![CockroachDB Threshold Alert in Datadog](/images/v24.1/datadog-crdb-threshold-alert.png)
</Frame>

- `cockroachdb.capacity.available` is divided by `cockroachdb.capacity.total` to determine the fraction of available capacity on the node's [store](/docs/v24.1/architecture/storage-layer) (the directory on each node where CockroachDB reads and writes its data).
...
<Frame>
  ![CockroachDB Threshold Alert in Datadog](/images/v24.1/datadog-crdb-storage-alert.png)
</Frame>
