...
<LinkedPageTabs
  currentPath="/docs/v24.1/deploy-cockroachdb-on-aws"
  items={[
    {
      title: "Secure",
      href: "/docs/v24.1/deploy-cockroachdb-on-aws",
    },
    {
      title: "Insecure",
      href: "/docs/v24.1/deploy-cockroachdb-on-aws-insecure",
    },
  ]}
/>
...
- You must have [CockroachDB installed](/docs/v24.1/install-cockroachdb) locally. This is necessary for generating and managing your deployment's certificates.
...
- Carefully review the [Production Checklist](/docs/v24.1/recommended-production-settings), including supported hardware and software, and the recommended [Topology Patterns](/docs/v24.1/topology-patterns).
...
- Follow the [AWS IAM best practices](https://docs.aws.amazon.com/IAM/latest/UserGuide/best-practices) to harden the AWS environment. Use [roles](/docs/v24.1/security-reference/authorization#roles) to grant access to the deployment, following a [policy of least privilege](https://docs.aws.amazon.com/IAM/latest/UserGuide/best-practices#grant-least-privilege).
...
<Frame>
  ![Architecture diagram for a three-node CockroachDB cluster deployed on AWS](/images/v24.1/aws-architecture.png)
</Frame>
...
For more details, see [Hardware Recommendations](/docs/v24.1/recommended-production-settings#hardware) and [Cluster Topology](/docs/v24.1/recommended-production-settings#topology).
...
CockroachDB requires moderate levels of [clock synchronization](/docs/v24.1/recommended-production-settings#clock-synchronization) to preserve data consistency. For this reason, when a node detects that its clock is out of sync with at least half of the other nodes in the cluster by 80% of the maximum offset allowed (500ms by default), it spontaneously shuts down. This avoids the risk of consistency anomalies, but it's best to prevent clocks from drifting too far in the first place by running clock synchronization software on each node.
...
1. [Add AWS load balancing](https://docs.aws.amazon.com/elasticloadbalancing/latest/network/network-load-balancer-getting-started). Be sure to:
    ...
    - Configure health checks to use HTTP port **8080** and path `/health?ready=1`. This [health endpoint](/docs/v24.1/monitoring-and-alerting#health-ready-1) ensures that load balancers do not direct traffic to nodes that are live but not ready to receive requests.
...
<Note>
If you would prefer to use HAProxy instead of AWS's managed load balancing, see the [On-Premises](/docs/v24.1/deploy-cockroachdb-on-premises) tutorial for guidance.
</Note>

## Step 5. Generate certificates

You can use `cockroach cert` commands or [`openssl` commands](/docs/v24.1/create-security-certificates-openssl) to generate security certificates. This section features the `cockroach cert` commands.

Locally, you'll need to [create the following certificates and keys](/docs/v24.1/cockroach-cert):
...
1. [Install CockroachDB](/docs/v24.1/install-cockroachdb) on your local machine, if you haven't already.
...
11. Upload the CA certificate and client certificate and key to the machine where you will run a sample workload:
    ...
    In later steps, you'll also use the `root` user's certificate to run [`cockroach`](/docs/v24.1/cockroach-commands) client commands from your local machine. If you might also want to run `cockroach` client commands directly on a node (e.g., for local debugging), you'll need to copy the `root` user's certificate and key to that node as well.

<Note>
On accessing the DB Console in a later step, your browser will consider the CockroachDB-created certificate invalid and you’ll need to click through a warning message to get to the UI. You can avoid this issue by [using a certificate issued by a public CA](/docs/v24.1/create-security-certificates-custom-ca#accessing-the-db-console-for-a-secure-cluster).
</Note>
...
2. On the node, follow the instructions to [install CockroachDB](/docs/v24.1/install-cockroachdb).
...
6. Change the ownership of the `cockroach` directory to the user `cockroach`:
    ...
    ```
7. Download the [sample configuration template](https://raw.githubusercontent.com/cockroachdb/docs/main/src/current/_includes/v24.1/prod-deployment/securecockroachdb.service) and save the file in the `/etc/systemd/system/` directory:
...
8. In the sample configuration template, specify values for the following flags:
    ...
    For other flags not explicitly set, the command uses default values. For example, the node stores data in `--store=cockroach-data` and binds DB Console HTTP requests to `--http-addr=localhost:8080`. To set these options manually, see [Start a Node](/docs/v24.1/cockroach-start).
...
On your local machine, run the [`cockroach init`](/docs/v24.1/cockroach-init) command to complete the node startup process and have them join together as a cluster:

```shell
$ cockroach init --certs-dir=certs --host=<address of any node on --join list>
```

After running this command, each node prints helpful details to the [standard output](/docs/v24.1/cockroach-start#standard-output), such as the CockroachDB version, the URL for the DB Console, and the SQL URL for clients.
...
Use the [built-in SQL client](/docs/v24.1/cockroach-sql) locally as follows:
...
For comprehensive guidance on benchmarking CockroachDB with TPC-C, refer to [Performance Benchmarking](/docs/v24.1/performance-benchmarking-with-tpcc-local).
...
2. [Install CockroachDB for Linux](/docs/v24.1/install-cockroachdb).
3. Use the [`cockroach workload`](/docs/v24.1/cockroach-workload) command to load the initial schema and data, pointing it at the IP address of the load balancer:
...
5. To monitor the load generator's progress, open the [DB Console](/docs/v24.1/ui-overview) by pointing a browser to the address in the `admin` field in the standard output of any node on startup.
...
2. [Install CockroachDB for Linux](/docs/v24.1/install-cockroachdb).
3. Run the [`cockroach start`](/docs/v24.1/cockroach-start) command, passing the new node's address as the `--advertise-addr` flag and pointing `--join` to the three existing nodes (also include `--locality` if you set it earlier).
...
6. Change the ownership of the `cockroach` directory to the user `cockroach`:
    ...
    chown -R cockroach /var/lib/cockroach
    ```
7. Download the [sample configuration template](https://raw.githubusercontent.com/cockroachdb/docs/master/_includes/v24.1/prod-deployment/securecockroachdb.service):
...
1. [Implement your data model](/docs/v24.1/sql-statements).
2. [Create users](/docs/v24.1/create-user) and [grant them privileges](/docs/v24.1/grant).
3. [Connect your application](/docs/v24.1/install-client-drivers). Be sure to connect your application to the load balancer, not to a CockroachDB node.
4. [Take backups](/docs/v24.1/take-full-and-incremental-backups) of your data.
...
<Warning>
When running a cluster of 5 nodes or more, it's safest to [increase the replication factor for important internal data](/docs/v24.1/configure-replication-zones#create-a-replication-zone-for-a-system-range) to 5, even if you do not do so for user data. For the cluster as a whole to remain available, the ranges for this internal data must always retain a majority of their replicas.
</Warning>
