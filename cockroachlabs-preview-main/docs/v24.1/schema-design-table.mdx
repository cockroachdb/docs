...
This page provides best-practice guidance on creating tables, with some simple examples based on Cockroach Labs's fictional vehicle-sharing company, [MovR](/docs/v24.1/movr).

<Tip>
For detailed reference documentation on the `CREATE TABLE` statement, including additional examples, see the [`CREATE TABLE` syntax page](/docs/v24.1/create-table).
</Tip>
...
Tables are the [logical objects in a cluster](/docs/v24.1/schema-design-overview#database-schema-objects) that store data sent from your application's persistence layer. Tables organize records of data in rows and columns.

To create a table, use a [`CREATE TABLE` statement](/docs/v24.1/create-table), following the best practices that we have listed in the following sections:
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>&#123;schema&#95;name&#125;</code></td><td>The name of the user-defined schema.</td></tr><tr><td><code>&#123;table&#95;name&#125;</code></td><td>The name of the table.</td></tr><tr><td><code>&#123;elements&#125;</code></td><td>A comma-separated list of table elements, such as column definitions.</td></tr></tbody></table>
...
Suppose you want to create a table to store information about users of the [MovR](/docs/v24.1/movr) platform, and you want the SQL user `max` to manage that table.
...
This file will initialize the objects in the `max_schema` user-defined schema that you created in [Create a Schema](/docs/v24.1/schema-design-schema), starting with a `users` table.
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>&#123;column&#95;name&#125;</code></td><td>The name of the column.</td></tr><tr><td><code>&#123;DATA&#95;TYPE&#125;</code></td><td>The <a href="/docs/v24.1/data-types">data type</a> of the row values in the column.</td></tr><tr><td><code>&#123;column&#95;qualification&#125;</code></td><td>Some column qualification, such as a <a href="#add-additional-constraints">column-level constraint</a>, or a <a href="/docs/v24.1/computed-columns">computed column clause</a>.</td></tr></tbody></table>
...
- Review the supported column [data types](/docs/v24.1/data-types), and select the appropriate type for the data you plan to store in a column, following the best practices listed on the data type's reference page.
- Use column data types with a fixed size limit, or set a maximum size limit on column data types of variable size (e.g., [`VARBIT(n)`](/docs/v24.1/bit#size)). Values exceeding 1MB can lead to [write amplification](/docs/v24.1/architecture/storage-layer#write-amplification) and cause significant performance degradation.
...
All of the columns shown above use the [`STRING`](/docs/v24.1/string) data type, meaning that any value in any of the columns must be of the data type `STRING`.

CockroachDB supports a number of other column data types, including [`DECIMAL`](/docs/v24.1/decimal), [`INT`](/docs/v24.1/int), [`TIMESTAMP`](/docs/v24.1/timestamp), [`UUID`](/docs/v24.1/uuid), and [enumerated data types](/docs/v24.1/enum) and [spatial data types](/docs/v24.1/export-spatial-data). We recommend that you review the [supported types](/docs/v24.1/data-types), and create columns with data types that correspond to the types of data that you intend to persist to the cluster from your application.
...
- [`UUID`](/docs/v24.1/uuid), which we recommend for columns with [values that uniquely identify rows](https://wikipedia.org/wiki/Unique_key) (like an "id" column).
- [`TIMESTAMPTZ`](/docs/v24.1/timestamp), which we recommend for [timestamp values](https://wikipedia.org/wiki/Timestamp).
- [`BOOL`](/docs/v24.1/bool), which we recommend for columns that will only take one of two possible values.

The rest of the columns are `STRING`-typed.

Note that values in the `type` column will likely only be `STRING` values from a fixed list of values. Specifically, the vehicle type can only be one of the vehicle types supported by the MovR platform (e.g., a `bike`, a `scooter`, or a `skateboard`). For values like this, we recommend using a [user-defined, enumerated type](/docs/v24.1/enum).

To create a user-defined type, use a [`CREATE TYPE`](/docs/v24.1/create-type) statement. For example, above the `CREATE TABLE` statement for the `vehicles` table, add the following statements:
...
<Tip>
For detailed reference documentation on the `CREATE TYPE` statement, including additional examples, see the [`CREATE TYPE` syntax page](/docs/v24.1/create-type).
For detailed reference documentation on enumerated data types, including additional examples, see [`ENUM`](/docs/v24.1/enum).
</Tip>
...
Primary keys are defined in `CREATE TABLE` statements with the `PRIMARY KEY` [column constraint](/docs/v24.1/constraints). The `PRIMARY KEY` constraint requires that all the constrained column(s) contain only unique and non-`NULL` values.

When a table is created, CockroachDB creates an index (called the *primary index* and named `{tbl}_pkey`) on the column(s) constrained by the `PRIMARY KEY` constraint. CockroachDB uses this [index](/docs/v24.1/indexes) to find rows in a table more efficiently.
...
<Tip>
For detailed reference documentation on the `PRIMARY KEY` constraint, including additional examples, see the [`PRIMARY KEY` constraint page](/docs/v24.1/primary-key).
</Tip>
...
- Avoid defining primary keys over a single column of sequential data.
    ...
    Querying a table with a primary key on a single sequential column (e.g., an auto-incrementing [`INT`](/docs/v24.1/int) column, or a [`TIMESTAMP`](/docs/v24.1/timestamp) value) can result in single-range [hot spots](/docs/v24.1/performance-best-practices-overview#hot-spots) that negatively affect performance, or cause [transaction contention](/docs/v24.1/performance-best-practices-overview#transaction-contention).
...
- Define a primary key for every table.
    ...
    If you create a table without defining a primary key, CockroachDB will automatically create a primary key over a hidden, [`INT`](/docs/v24.1/int)-typed column named `rowid`. By default, sequential, unique identifiers are generated for each row in the `rowid` column with the [`unique_rowid()` function](/docs/v24.1/functions-and-operators#built-in-functions). The sequential nature of the `rowid` values can lead to a poor distribution of the data across a cluster, which can negatively affect performance. Furthermore, because you cannot meaningfully use the `rowid` column to filter table data, the primary key index on `rowid` does not offer any performance optimization. This means you will always have improved performance by defining a primary key for a table.

    To require an explicitly defined primary key for all tables created in your cluster, set the `sql.defaults.require_explicit_primary_keys.enabled` [cluster setting](/docs/v24.1/cluster-settings) to `true`.
- When possible, define primary key constraints over multiple columns (i.e., use [composite primary keys](https://wikipedia.org/wiki/Composite_key)).
    ...
    When defining composite primary keys, make sure the data in the first column of the primary key prefix is well-distributed across the nodes in the cluster. To improve the performance of [ordered queries](/docs/v24.1/order-by), you can add monotonically increasing primary key columns after the first column of the primary key prefix. For an example, see [below](#primary-key-examples).
- For single-column primary keys, use [`UUID`](/docs/v24.1/uuid)-typed columns with default values randomly-generated, using the `gen_random_uuid()` [SQL function](/docs/v24.1/functions-and-operators#id-generation-functions).
...
<Tip>
For detailed reference documentation for each supported constraint, see [the constraint's syntax page](/docs/v24.1/constraints).
</Tip>
...
When combined with [supported SQL functions](/docs/v24.1/functions-and-operators), default values can save resources in your application's persistence layer by offloading computation onto CockroachDB. For example, rather than using an application library to generate unique [`UUID`](/docs/v24.1/uuid) values, you can set a default value to be an automatically-generated `UUID` value with the `gen_random_uuid()` SQL function. Similarly, you could use a default value to populate a `TIMESTAMP` column with the current time of day, using the `now()` function.

For example, in the `vehicles` table definition in `max_init.sql`, you added a `DEFAULT gen_random_uuid()` clause to the `id` column definition. This set the default value to a generated [`UUID`](/docs/v24.1/uuid) value. Now, add a default value to the `creation_time` column:
...
<Tip>
For detailed reference documentation on the `DEFAULT` constraint, including additional examples, see [the `DEFAULT` syntax page](/docs/v24.1/default-value).
</Tip>
...
Suppose that you want to introduce promotional codes for users on the MovR platform, but you want the user promo code data to be managed by the `abbey` user that you created in [Create a Database](/docs/v24.1/schema-design-database).
...
This file will initialize the objects in the `abbey_schema` user-defined schema that you created in [Create a Schema](/docs/v24.1/schema-design-schema).
...
<Tip>
For detailed reference documentation on the `FOREIGN KEY` constraint, including additional examples, see [the `FOREIGN KEY` syntax page](/docs/v24.1/foreign-key).
</Tip>
...
<Tip>
For detailed reference documentation on the `UNIQUE` constraint, including additional examples, see [the `UNIQUE` syntax page](/docs/v24.1/unique).
</Tip>
...
<Tip>
For detailed reference documentation on the `NOT NULL` constraint, including additional examples, see [the `NOT NULL` syntax page](/docs/v24.1/not-null).
</Tip>
...
- Do not create tables as the `root` user. Instead, create tables as a [different user](/docs/v24.1/schema-design-overview#control-access-to-objects), with fewer privileges, following [authorization best practices](/docs/v24.1/security-reference/authorization#authorization-best-practices). The user that creates an object becomes that [object's owner](/docs/v24.1/security-reference/authorization#object-ownership).
...
After the statements have been executed, you can see the tables in the [CockroachDB SQL shell](/docs/v24.1/cockroach-sql#sql-shell).
...
To view the tables in the `max_schema` user-defined schema, issue a [`SHOW TABLES`](/docs/v24.1/show-tables) statement:
...
To see the individual `CREATE TABLE` statements for each table, use a [`SHOW CREATE`](/docs/v24.1/show-create) statement. For example, to see the `vehicles` `CREATE TABLE` statement:
...
After the statements have been executed, you can see the table in the [CockroachDB SQL shell](/docs/v24.1/cockroach-sql#sql-shell).
...
Note that none of the tables that you have created thus far have secondary indexes. For guidance on adding secondary indexes, see [Secondary Indexes](/docs/v24.1/schema-design-indexes).

## What's next?

- [Secondary Indexes](/docs/v24.1/schema-design-indexes)
- [Online Schema Changes](/docs/v24.1/online-schema-changes)

You might also be interested in the following pages:

- [Computed Columns](/docs/v24.1/computed-columns)
- [Column Families](/docs/v24.1/column-families)
- [`CREATE TABLE`](/docs/v24.1/create-table)
- [Data Types](/docs/v24.1/data-types)
- [`PRIMARY KEY`](/docs/v24.1/primary-key)
