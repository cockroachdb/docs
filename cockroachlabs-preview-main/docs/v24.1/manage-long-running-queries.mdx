...
Use the [`SHOW STATEMENTS`](/docs/v24.1/show-statements) statement to list details about currently active SQL queries, including each query's `start` timestamp:
...
Once you've identified a long-running query via [`SHOW STATEMENTS`](/docs/v24.1/show-statements), note the `query_id` and use it with the [`CANCEL QUERY`](/docs/v24.1/cancel-query) statement:
...
After cancelling a long-running query, use the [`EXPLAIN`](/docs/v24.1/explain) statement to examine it. It's possible that the query was slow because it performs a full-table scan. In these cases, you can likely improve the query's performance by [adding an index](/docs/v24.1/create-index).

<Tip>
For guidance on optimizing SQL performance, see [SQL Performance Best Practices](/docs/v24.1/performance-best-practices-overview).
</Tip>
...
- [Query Behavior Troubleshooting](/docs/v24.1/query-behavior-troubleshooting)
