...
old_description: "Configure SQL Authentication for Hardened Serverless Cluster Security"
---
...
This document describes the rationale for restricting database access to specific IP ranges as a security measure and then demonstrates the procedure using [authentication configuration](/docs/v24.1/security-reference/authentication) to achieve that aim.
...
Limiting allowed database connections to secure IP addresses reduces the risk that your cluster is compromised, because a potential attacker who acquires database credentials (e.g., username/password combinations or client [PKI certificates](/docs/v24.1/security-reference/transport-layer-security#certificates-signing-trust-and-authority)) cannot use those credentials without also gaining infrastructure access. Infrastructure access can and should be protected with multifactor authentication and restricted to appropriate parties using infrastructure-level IAM.

## Step 1: Provision and access your cluster

[Create your own free CockroachDB Basic cluster](/docs/cockroachcloud/create-a-basic-cluster).
...
You'll also need to download the cluster's [Certificate Authority certificate](/docs/v24.1/security-reference/transport-layer-security#certificates-signing-trust-and-authority), so that your client can authenticate the database server as it connects.
...
Returning to the SQL console, let's set our authentication configuration to limit access to the jumpbox. This configuration is accessed as a [cluster setting](/docs/v24.1/cluster-settings).
...
Finally, let's attempt the connection from the jumpbox. You'll need to use `scp` to transfer the cluster's root [CA certificate](/docs/v24.1/security-reference/transport-layer-security#certificates-signing-trust-and-authority) to the jumpbox, so that your client there can use it to authenticate the server. Then shell into the jumpbox with the `gcloud gcompute ssh` and run your connection command from inside the jumpbox.
...
Each user's permissions should then be precisely configured using CockroachDB's system of [access grants](/docs/v24.1/security-reference/authorization). Always keep in mind the [principle of least privilege](https://wikipedia.org/wiki/Principle_of_least_privilege), which is one of the golden rules of security!
