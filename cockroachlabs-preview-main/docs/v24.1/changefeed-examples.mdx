...
This page provides step-by-step examples for using Core and Enterprise changefeeds. Creating Enterprise changefeeds is available on CockroachDB Basic, Standard, Advanced, and with an [Enterprise license](/docs/v24.1/licensing-faqs#types-of-licenses) on CockroachDB self-hosted clusters. Basic changefeeds are available in all products.

For a summary of Core and Enterprise changefeed features, refer to the [Change Data Capture Overview](/docs/v24.1/change-data-capture-overview) page.
...
- <InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v24.1: [Azure Event Hubs](#create-a-changefeed-connected-to-an-azure-event-hubs-sink)
- <InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v24.1: [Apache Pulsar](#create-a-changefeed-connected-to-an-apache-pulsar-sink) (Preview)

Refer to the [Changefeed Sinks](/docs/v24.1/changefeed-sinks) page for more detail on forming sink URIs, available sink query parameters, and specifics on configuration.
...
Before you run the examples, verify that you have the `CHANGEFEED` privilege in order to create and manage changefeed jobs. Refer to [Required privileges](/docs/v24.1/create-changefeed#required-privileges) for more details.
...
1. If you do not already have one, [request a trial Enterprise license](/docs/v24.1/licensing-faqs#obtain-a-license).
2. Use the [`cockroach start-single-node`](/docs/v24.1/cockroach-start-single-node) command to start a single-node cluster:

    ```shell
    cockroach start-single-node --insecure --listen-addr=localhost
    ```
3. Download and extract the [Confluent platform](https://www.confluent.io/download) (which includes Kafka).
...
6. As the `root` user, open the [built-in SQL client](/docs/v24.1/cockroach-sql):

    ```shell
    $ cockroach sql --insecure
    ```
7. Set your organization name and [Enterprise license](/docs/v24.1/licensing-faqs#types-of-licenses) key:
    ...
    ```
8. Enable the `kv.rangefeed.enabled` [cluster setting](/docs/v24.1/cluster-settings):
...
2. Use the [`cockroach start-single-node`](/docs/v24.1/cockroach-start-single-node) command to start a single-node cluster:
    ...
    cockroach start-single-node --insecure --listen-addr=localhost
    ```
3. In this example, you'll run CockroachDB's [Movr](/docs/v24.1/movr) application workload to set up some data for your changefeed.
    ...
    ```
4. As the `root` user, open the [built-in SQL client](/docs/v24.1/cockroach-sql):

    ```shell
    $ cockroach sql --insecure
    ```
5. Set your organization name and [Enterprise license](/docs/v24.1/licensing-faqs#types-of-licenses) key:
    ...
    > SET CLUSTER SETTING enterprise.license = '<secret>';
    ```
6. Enable the `kv.rangefeed.enabled` [cluster setting](/docs/v24.1/cluster-settings):
...
11. Construct the URI to connect your changefeed to the Confluent Cloud cluster. You will need:
    ...
    - (Optional) Any further parameters. Refer to [Query parameters](/docs/v24.1/create-changefeed#query-parameters).
...
7. Next, you'll prepare your Pub/Sub sink.
    ...
    ```
8. Create the Pub/Sub [topic](/docs/v24.1/changefeed-sinks#pub-sub-topic-naming) to which your changefeed will emit messages:
...
9. With the topic and subscription set up, you can now download your Service Account credentials. Use the [`gcloud iam service-accounts keys create`](https://cloud.google.com/sdk/gcloud/reference/iam/service-accounts/keys/create) command to specify where to download the JSON credential file (`credentials.json`):
    ...
    Copy the output so that you can add it to your [`CREATE CHANGEFEED`](/docs/v24.1/create-changefeed) statement in the next step. When you create your changefeed, it is necessary that the credentials are base64 encoded before passing it in the URI.
10. Back in the SQL shell, create a changefeed that will emit messages to your Pub/Sub topic. Ensure that you have base64 encoded the entire credentials JSON object for your Service Account and then run:
    ...
    You can include the `region` parameter for your topic, or use the [WITH `unordered`](/docs/v24.1/create-changefeed#unordered) option for multi-region Pub/Sub. See the [Changefeed Sinks](/docs/v24.1/changefeed-sinks#google-cloud-pub-sub) page for more detail.
...
In this example, you'll set up a changefeed for a single-node cluster that is connected to an AWS S3 sink. The changefeed watches two tables. Note that you can set up changefeeds for any of [these cloud storage providers](/docs/v24.1/changefeed-sinks#cloud-storage-sink).
...
2. Use the [`cockroach start-single-node`](/docs/v24.1/cockroach-start-single-node) command to start a single-node cluster:
    ...
    $ cockroach start-single-node --insecure --listen-addr=localhost
    ```
3. As the `root` user, open the [built-in SQL client](/docs/v24.1/cockroach-sql):

    ```shell
    $ cockroach sql --insecure
    ```
4. Set your organization name and [Enterprise license](/docs/v24.1/licensing-faqs#types-of-licenses) key:
    ...
    > SET CLUSTER SETTING enterprise.license = '<secret>';
    ```
5. Enable the `kv.rangefeed.enabled` [cluster setting](/docs/v24.1/cluster-settings):
...
11. Monitor your changefeed on the [DB Console](http://localhost:8080/#/metrics/changefeeds/cluster). For more information, see [Changefeeds Dashboard](/docs/v24.1/ui-cdc-dashboard).
...
7. To prepare the Azure Event Hubs sink, in the Portal, [create a resource group](https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/manage-resource-groups-portal#create-resource-groups).
8. Once you have a resource group, select **Event Hubs** in the portal navigation menu and then **+ Create** from the **Event Hubs** page tool bar to create a namespace. Complete the naming details for the namespace and select the resource group you created in the previous step. For **Pricing Tier**, ensure that you select **Standard** or **Premium** (the **Basic** tier does not support Apache Kafka workloads). Complete your configuration options, and then create the namespace.
9. Select **Go to resource** after the deployment is complete. On the namespace overview page, select **+ Event Hub** in the tool bar to create an event hub within this namespace. An Event Hub is equivalent to a Kafka topic. On the creation page, add a name and the configuration for partitions and message retention.

    <Note>
    You can manually create an Event Hub, or Event Hubs will create automatically when you start the changefeed.
    </Note>
10. To send changefeed messages to your Event Hub, you will need a `shared_access_key`. Find **Shared access policies** in the left-hand navigation. Click **+ Add** and provide a name for the policy as well as confirming the permissions. Once created, click on the policy name and then copy the **Primary key** — you will need this to create your changefeed.
11. Build the connection string to your Event Hub. You will need the following:
...
You can include the [`topic_name`](/docs/v24.1/create-changefeed#topic-name) or [`topic_prefix`](/docs/v24.1/create-changefeed#topic-prefix) parameters to create and name an Event Hub for emitted messages.

1. Create an [external connection](/docs/v24.1/create-external-connection) to store your connection string:
...
9. Back in your SQL shell, run the following statement to create a changefeed that emits to your webhook sink:
    ...
    See the [options table](/docs/v24.1/create-changefeed#options) for more information on the options available for creating your changefeed to a webhook sink.
    ...
    For more detail on emitted changefeed messages, see [responses](/docs/v24.1/changefeed-messages#responses).
...
<InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v24.1: In this example, you'll set up a changefeed for a single-node cluster that is connected to an [Apache Pulsar](https://pulsar.apache.org/docs/next/getting-started-standalone) sink. The changefeed will watch a table and send messages to the sink.
...
7. To prepare a Pulsar sink, refer to the [Apache Pulsar documentation](https://pulsar.apache.org/docs/next/getting-started-standalone) for setup guides to host Pulsar on a local cluster, Docker, or a Kubernetes cluster.
8. In a terminal window where your Pulsar sink is hosted, start the cluster:
...
9. Enter the SQL shell:

    ```shell
    cockroach sql --insecure
    ```
10. Create your changefeed:
...
11. In a different terminal window, start a [Pulsar consumer](https://pulsar.apache.org/docs/next/tutorials-produce-consume) to read messages from the changefeed. This example consumes messages from the `rides` topic:
    ...
    For more detail on emitted changefeed messages, refer to [Responses](/docs/v24.1/changefeed-messages#responses).
...
1. In a terminal window, start `cockroach`:
    ...
    --background
    ```
2. As the `root` user, open the [built-in SQL client](/docs/v24.1/cockroach-sql):
...
1. Use the [`cockroach start-single-node`](/docs/v24.1/cockroach-start-single-node) command to start a single-node cluster:
...
For further information on basic changefeeds, see [`EXPERIMENTAL CHANGEFEED FOR`](/docs/v24.1/changefeed-for).
