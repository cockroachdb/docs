...
<LinkedPageTabs
  currentPath="/docs/v24.1/build-a-python-app-with-cockroachdb-django"
  items={[
    {
      title: "Use psycopg3",
...
For more information about configuration a Django connection to CockroachDB Standard, see [Connect to a CockroachDB Cluster](/docs/v24.1/connect-to-the-database?filters=python&filters=django).
...
3. Send a GET request to read from the `cockroach_example_customers` table:
    ...
    You can also query the table directly in the [SQL shell](/docs/v24.1/cockroach-sql) to see the changes:
