...
- If you want to know how to create CockroachDB security certificates, see [Create Security Certificates](/docs/v24.1/cockroach-cert).

## Connecting to a CockroachDB cluster

Users may connect with CockroachDB self-hosted clusters in 2 main ways:

- SQL clients connections, including the CockroachDB CLI client and the [various supported drivers and ORMs](/docs/v24.1/install-client-drivers), connect directly to CockroachDB clusters using the [CockroachDB SQL interface](/docs/v24.1/sql-feature-support).
- A read-only monitoring service, which provides cluster and database details, and information useful for troubleshooting and performance tuning. Each CockroachDB node also acts as an HTTP server, providing both a [browser UI DB console](/docs/v24.1/ui-overview) and the [cluster API](/docs/v24.1/cluster-api), which provides much of the same information as the DB console, but as a rest API suitable for programmatic access.
...
- The hostname or address (IP address or DNS name) used to reach a node, either directly or through a load balancer, must be listed in the **Common Name** or **Subject Alternative Names** fields of the certificate:
    - The values specified in [`--listen-addr`](/docs/v24.1/cockroach-start#networking) and [`--advertise-addr`](/docs/v24.1/cockroach-start#networking) flags, or the node hostname and fully qualified hostname if not specified
...
Based on your security setup, you can use the [`cockroach cert` commands](/docs/v24.1/cockroach-cert), [`openssl` commands](/docs/v24.1/create-security-certificates-openssl), or a [custom CA](/docs/v24.1/create-security-certificates-custom-ca) to generate all the keys and certificates.
...
- Client authentication using TLS digital certificates, passwords, or [GSSAPI authentication](/docs/v24.1/gssapi_authentication) (for Enterprise users).
...
- **Password authentication without TLS**
    ...
    Enter password:
    ```
- [**Single sign-on authentication to DB Console**](/docs/v24.1/sso-db-console).
- [**GSSAPI authentication**](/docs/v24.1/gssapi_authentication).

### Using `cockroach cert` or `openssl` commands

You can use the [`cockroach cert` commands](/docs/v24.1/cockroach-cert) or [`openssl` commands](/docs/v24.1/create-security-certificates-openssl) to create the CA certificate and key, and node and client certificates and keys.
...
See [Use Cloud Storage for Bulk Operations](/docs/v24.1/cloud-storage-authentication).
...
For details about when and how to change security certificates without restarting nodes, see [Rotate Security Certificates](/docs/v24.1/rotate-certificates).
...
- [Local Deployment](/docs/v24.1/secure-a-cluster)
- [`cockroach` Commands Overview](/docs/v24.1/cockroach-commands)
- [Certificate-based authentication using multiple values from the X.509 Subject field](/docs/v24.1/certificate-based-authentication-using-the-x509-subject-field)
- [`ALTER ROLE... SUBJECT`](/docs/v24.1/alter-role#set-the-subject-role-option-for-certificate-based-authentication)
- [`CREATE ROLE... SUBJECT`](/docs/v24.1/create-role#set-the-subject-role-option-for-certificate-based-authentication)
- [`cockroach cert`](/docs/v24.1/cockroach-cert)
- [`cockroach auth-session`](/docs/v24.1/cockroach-auth-session)
- [GSSAPI Authentication](/docs/v24.1/gssapi_authentication)
- [SQL Authentication](/docs/v24.1/security-reference/authentication)
- [Cloud Storage Authentication](/docs/v24.1/cloud-storage-authentication)
