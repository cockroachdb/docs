...
import { AlterIndex3 } from "/snippets/syntax-diagrams/AlterIndex3.jsx";

The `ALTER INDEX` [statement](/docs/v24.1/sql-statements) applies a [schema change](/docs/v24.1/online-schema-changes) to an index.
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>index&#95;name</code></td><td>The name of the <a href="/docs/v24.1/indexes">index</a> you want to change.</td></tr><tr><td><code>IF EXISTS</code></td><td>Alter the index only if an index <code>table&#95;index&#95;name</code> exists; if one does not exist, do not return an error.</td></tr></tbody></table>

Additional parameters are documented for the respective [subcommands](#subcommands).

## Subcommands

<table><thead><tr><th>Subcommand</th><th>Description</th></tr></thead><tbody><tr><td><a href="#configure-zone"><code>CONFIGURE ZONE</code></a></td><td><a href="/docs/v24.1/configure-replication-zones">Replication Controls</a> for an index.</td></tr><tr><td><a href="#partition-by"><code>PARTITION BY</code></a></td><td>Partition, re-partition, or un-partition an index.</td></tr><tr><td><a href="#rename-to"><code>RENAME TO</code></a></td><td>Change the name of an index.</td></tr><tr><td><a href="#scatter"><code>SCATTER</code></a></td><td>Make a best-effort attempt to redistribute replicas and leaseholders for the ranges of a table or index. Note that this statement does not return an error even if replicas are not moved.</td></tr><tr><td><a href="#split-at"><code>SPLIT AT</code></a></td><td>Force a <a href="/docs/v24.1/architecture/distribution-layer#range-splits">range split</a> at the specified row in the index.</td></tr><tr><td><a href="#unsplit-at"><code>UNSPLIT AT</code></a></td><td>Remove a range split enforcement in the index.</td></tr><tr><td><a href="#visibility"><code>VISIBILITY</code></a></td><td>Set the visibility of an index between a range of <code>0.0</code> and <code>1.0</code>.</td></tr><tr><td><a href="#not-visible"><code>&#91;NOT&#93; VISIBLE</code></a></td><td>Make an index visible or not visible to the <a href="/docs/v24.1/cost-based-optimizer#control-whether-the-optimizer-uses-an-index">cost-based optimizer</a>.</td></tr></tbody></table>

### `CONFIGURE ZONE`

`ALTER INDEX... CONFIGURE ZONE` is used to add, modify, reset, or remove replication zones for an index. To view details about existing replication zones, use [`SHOW ZONE CONFIGURATIONS`](/docs/v24.1/show-zone-configurations). For more information about replication zones, see [Replication Controls](/docs/v24.1/configure-replication-zones).
...
`ALTER INDEX... PARTITION BY` is used to partition, re-partition, or un-partition a secondary index. After defining partitions, [`CONFIGURE ZONE`](/docs/v24.1/alter-partition#create-a-replication-zone-for-a-partition) is used to control the replication and placement of partitions.

Similar to [indexes](/docs/v24.1/indexes), partitions can improve query performance by limiting the numbers of rows that a query must scan. In the case of [geo-partitioned data](/docs/v24.1/regional-tables), partitioning can limit a query scan to data in a specific region. For examples, see [Query partitions](/docs/v24.1/partitioning#query-partitions).

<Tip>
Most users should not need to use partitioning directly. Instead, they should use CockroachDB's built-in [multi-region capabilities](/docs/v24.1/multiregion-overview), which automatically handle geo-partitioning and other low-level details.
</Tip>

The [primary key required for partitioning](/docs/v24.1/partitioning#partition-using-primary-key) is different from the conventional primary key: The unique identifier in the primary key must be prefixed with all columns you want to partition and subpartition the table on, in the order in which you want to nest your subpartitions.

If the primary key in your existing table does not meet the requirements, you can change the primary key with [`ALTER TABLE... ALTER PRIMARY KEY`](/docs/v24.1/alter-table#alter-primary-key).
...
<Note>
It is not possible to rename an index referenced by a view. For more details, see [View Dependencies](/docs/v24.1/views#view-dependencies).
</Note>
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>index&#95;new&#95;name</code></td><td>The <a href="/docs/v24.1/sql-grammar#name"><code>name</code></a> you want to use for the index, which must be unique to its table and follow these <a href="/docs/v24.1/keywords-and-identifiers#identifiers">identifier rules</a>.</td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

### `SCATTER`

`ALTER INDEX... SCATTER` runs a specified set of ranges for a table or index through the [replication layer](/docs/v24.1/architecture/replication-layer) queue. If many ranges have been created recently, the replication queue may transfer some leases to other replicas to balance load across the cluster.
...
`ALTER INDEX... SPLIT AT` forces a [range split](/docs/v24.1/architecture/distribution-layer#range-splits) at a specified row in the index.
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>select&#95;stmt</code></td><td>A <a href="/docs/v24.1/selection-queries">selection query</a> that produces one or more rows at which to split the index.</td></tr><tr><td><code>a&#95;expr</code></td><td>The expiration of the split enforcement on the index. This can be a <a href="/docs/v24.1/decimal"><code>DECIMAL</code></a>, <a href="/docs/v24.1/interval"><code>INTERVAL</code></a>, <a href="/docs/v24.1/timestamp"><code>TIMESTAMP</code></a>, or <a href="/docs/v24.1/timestamp"><code>TIMESTAMPZ</code></a>.</td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

### `UNSPLIT AT`

`ALTER INDEX... UNSPLIT AT` removes a [split enforcement](#split-at) on a [range split](/docs/v24.1/architecture/distribution-layer#range-splits), at a specified row in the index.
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>select&#95;stmt</code></td><td>A <a href="/docs/v24.1/selection-queries">selection query</a> that produces one or more rows at which to unsplit an index.</td></tr><tr><td><code>ALL</code></td><td>Remove all split enforcements for an index.</td></tr></tbody></table>
...
- `VISIBILITY 0.0` means that an index is not visible to the [cost-based optimizer](/docs/v24.1/cost-based-optimizer#control-whether-the-optimizer-uses-an-index). This is equivalent to [`NOT VISIBLE`](#not-visible).
...
- Any value between `0.0` and `1.0` means that an index is visible to the specified fraction of queries. This is known as a *partially visible index*.
    ...
    <Note>
    For the purposes of [index recommendations](/docs/v24.1/explain#success-responses), partially visible indexes are treated as [not visible](/docs/v24.1/alter-index#not-visible). If a partially visible index can be used to improve a query plan, the [optimizer](/docs/v24.1/cost-based-optimizer) will recommend making it fully visible. For an example, refer to [Set an index as partially visible](/docs/v24.1/alter-index#set-an-index-as-partially-visible).
    </Note>

### `[NOT] VISIBLE`

`ALTER INDEX... VISIBLE` and `ALTER INDEX... NOT VISIBLE` determines whether the index is visible to the [cost-based optimizer](/docs/v24.1/cost-based-optimizer#control-whether-the-optimizer-uses-an-index).

By default, indexes are visible. If an index is `NOT VISIBLE`, queries will not read from the index unless it is specifically selected with an [index hint](/docs/v24.1/indexes#selection) or the property is overridden with the [`optimizer_use_not_visible_indexes` session variable](/docs/v24.1/set-vars#optimizer-use-not-visible-indexes). In order to keep `NOT VISIBLE` indexes up to date, queries will still write to the index as they insert and update data in the table.
...
- Queries may still read from `NOT VISIBLE`, [`UNIQUE`](/docs/v24.1/create-index#unique-indexes) indexes to enforce [`UNIQUE` constraints](/docs/v24.1/unique).
- Queries may still read from `NOT VISIBLE` indexes to perform foreign key cascades and enforce [`FOREIGN KEY` constraints](/docs/v24.1/foreign-key).
- When defining a [`UNIQUE` constraint](/docs/v24.1/unique), you cannot use the `NOT VISIBLE` syntax to make the corresponding index not visible. Instead, use `ALTER INDEX... NOT VISIBLE` after creating the `UNIQUE` constraint.
...
Add a new secondary [index](/docs/v24.1/indexes) to the `rides` table, on the `revenue` column:
...
For an example, see [`ALTER TABLE`](/docs/v24.1/alter-table#set-the-expiration-on-a-split-enforcement).

#### Unsplit an index

Add a new secondary [index](/docs/v24.1/indexes) to the `rides` table, on the `revenue` column, and then split the table ranges by secondary index values as described in [Split an index](#split-an-index).
...
The table is still split into ranges at `25.00`, `50.00`, and `75.00`, but the `split_enforced_until` column is now `NULL` for all ranges in the table. The split is no longer enforced, and CockroachDB can [merge the data](/docs/v24.1/architecture/distribution-layer#range-merges) in the table as needed.
