...
- [CockroachDB Integration with HashiCorp Vault](/docs/v24.1/hashicorp-integration)
...
- [Install the `cockroach` CLI](/docs/v24.1/install-cockroachdb).
...
If you want user/role management operations to finish more quickly, and do not care whether concurrent transactions will immediately see the side effects of those operations, set the [session variable](/docs/v24.1/set-vars) `allow_role_memberships_to_change_during_transaction` to `true`. To learn more, refer to [How to speed up user / role assignment](/docs/v24.1/hashicorp-integration#how-to-speed-up-user-role-management) and [`GRANT`](/docs/v24.1/grant).
