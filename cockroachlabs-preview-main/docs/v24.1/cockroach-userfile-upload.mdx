...
The `cockroach userfile upload` [command](/docs/v24.1/cockroach-commands) uploads a file to the [user-scoped file storage](/docs/v24.1/use-userfile-storage) using a SQL connection.
...
The user must have the `CREATE` [privilege](/docs/v24.1/security-reference/authorization#managing-privileges) on the target database. CockroachDB will proactively grant the user `GRANT`, `SELECT`, `INSERT`, `DROP`, `DELETE` on the metadata and file tables.
...
Then, you can use the file to [`IMPORT INTO`](/docs/v24.1/import-into) data.
...
To upload the contents of a directory to userfile storage, specify a source directory and destination. For example, to upload a [backup](/docs/v24.1/backup) directory to userfile storage:
...
- [`cockroach userfile list`](/docs/v24.1/cockroach-userfile-list)
- [`cockroach userfile delete`](/docs/v24.1/cockroach-userfile-delete)
- [`cockroach userfile get`](/docs/v24.1/cockroach-userfile-get)
