...
For instructions to reproduce the TPC-C results listed here, see [Performance Benchmarking with TPC-C](/docs/v24.1/performance-benchmarking-with-tpcc-large). If you fail to achieve similar results, there is likely a problem in either the hardware, workload, or test design.

<Tip>
This document is about CockroachDB performance on benchmarks. For guidance on tuning real workloads, see [SQL Best Practices](/docs/v24.1/performance-best-practices-overview), and for guidance on data location techniques to minimize network latency, see [Topology Patterns](/docs/v24.1/topology-patterns).
</Tip>
...
CockroachDB achieves this performance in [`SERIALIZABLE` isolation](/docs/v24.1/demo-serializable), the strongest isolation level in the SQL standard.
...
CockroachDB provides a number of important tuning practices for both single-region and multi-region deployments, including [secondary indexes](/docs/v24.1/indexes) and various [data topologies](/docs/v24.1/topology-patterns) to achieve low latency.
...
- TPC-C replication instructions
    ...
    For instructions showing how to replicate the TPC-C results described in this page, see [Performance Benchmarking with TPC-C](/docs/v24.1/performance-benchmarking-with-tpcc-large).
