...
import { CreateStatistics5 } from "/snippets/syntax-diagrams/CreateStatistics5.jsx";
import { CreateStatistics6 } from "/snippets/syntax-diagrams/CreateStatistics6.jsx";

Use the `CREATE STATISTICS` [statement](/docs/v24.1/sql-statements) to [generate table statistics for the cost-based optimizer](/docs/v24.1/cost-based-optimizer#table-statistics) to use.

Once you [create a table](/docs/v24.1/create-table) and load data into it (e.g., [`INSERT`](/docs/v24.1/insert), [`IMPORT INTO`](/docs/v24.1/import-into)), table statistics can be generated. Table statistics help the cost-based optimizer determine the cardinality of the rows used in each query, which helps to predict more accurate costs.
...
For [PostgreSQL compatibility](/docs/v24.1/postgresql-compatibility), CockroachDB supports `ANALYZE` and `ANALYSE` as aliases for `CREATE STATISTICS`.
...
For more information about how the `AS OF SYSTEM TIME` clause works, including supported time formats, see [`AS OF SYSTEM TIME`](/docs/v24.1/as-of-system-time).
...
For more information about the `DELETE` statement, see [`DELETE`](/docs/v24.1/delete).

### View statistics jobs

Every time the `CREATE STATISTICS` statement is executed, it starts a background job. This is true for queries issued by your application as well as queries issued for [automatically generated statistics](/docs/v24.1/cost-based-optimizer#table-statistics).

To view statistics jobs, there are two options:

1. Use [`SHOW JOBS`](/docs/v24.1/show-jobs) to see all statistics jobs that were created by user queries (i.e., someone entering `CREATE STATISTICS` at the SQL prompt or via application code):
    ...
    (5 rows)
    ```
2. Use `SHOW AUTOMATIC JOBS` to see statistics jobs that were created by [automatically generated statistics](/docs/v24.1/cost-based-optimizer#table-statistics):
...
- [Cost-Based Optimizer](/docs/v24.1/cost-based-optimizer)
