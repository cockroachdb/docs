...
CockroachDB is compatible with version 3.0 of the PostgreSQL wire protocol (pgwire) and works with the majority of PostgreSQL database tools such as [DBeaver](/docs/v24.1/dbeaver), [Intellij](/docs/v24.1/intellij-idea), and so on. Consult this link for a full list of supported [third-party database tools](/docs/v24.1/third-party-database-tools). CockroachDB also works with most [PostgreSQL drivers and ORMs](/docs/v24.1/example-apps).
...
<Note>
This document currently only covers unsupported SQL and how to rewrite SQL expressions. It does not discuss strategies for porting applications that use [SQL features CockroachDB does not currently support](/docs/v24.1/sql-feature-support).
</Note>

## Unsupported Features

The following PostgreSQL features are not supported in CockroachDB v24.1:
...
The following PostgreSQL features are partially supported in CockroachDB v24.1.

### Multiple active portals

CockroachDB v24.1 supports pgwire's multiple active portals as a [preview feature](/docs/v24.1/cockroachdb-feature-availability#features-in-preview). The feature is off by default, and can be enabled by setting the [session variable `multiple_active_portals_enabled`](/docs/v24.1/set-vars#multiple-active-portals-enabled) to `true`.
...
- Only read-only [`SELECT` queries](/docs/v24.1/selection-queries) without [subqueries](/docs/v24.1/subqueries) are supported.
- Postqueries (which are how CockroachDB executes [foreign key checks](/docs/v24.1/foreign-key), for example) are not supported - [cockroachdb/cockroach#96398](https://github.com/cockroachdb/cockroach/issues/96398)
- [Distributed SQL execution](/docs/v24.1/architecture/sql-layer#distsql) is not supported for multiple active portals; instead queries execute on the [gateway node](/docs/v24.1/architecture/life-of-a-distributed-transaction#gateway) only - [cockroachdb/cockroach#100822](https://github.com/cockroachdb/cockroach/issues/100822)
- Only the latest execution of a statement from a pausable portal is recorded by the [trace infrastructure](/docs/v24.1/show-trace) - [cockroachdb/cockroach#99404](https://github.com/cockroachdb/cockroach/issues/99404)
...
**Porting instructions:** Manually add a modulo to the second argument. Also note that CockroachDB's [`INT`](/docs/v24.1/int) type is always 64 bits. For example:
...
For more information, see [`SELECT FOR UPDATE`](/docs/v24.1/select-for-update).

### `CHECK` constraint validation for `INSERT ON CONFLICT`

CockroachDB validates [`CHECK`](/docs/v24.1/check) constraints on the results of [`INSERT ON CONFLICT`](/docs/v24.1/insert#on-conflict-clause) statements, preventing new or changed rows from violating the constraint. Unlike PostgreSQL, CockroachDB does not also validate `CHECK` constraints on the input rows of `INSERT ON CONFLICT` statements.
...
Click the following link to find a full list of [CockroachDB supported SQL Features](/docs/v24.1/sql-feature-support).
