...
- For CockroachDB clusters running v22.1 and earlier, enable the following [cluster settings](/docs/v24.1/cluster-settings):
...
- Ensure you have a secure, [publicly available](/docs/cockroachcloud/network-authorization) CockroachDB cluster running the latest **v24.1** [production release](/docs/releases/v24.1), and have created a [SQL user](/docs/v24.1/security-reference/authorization#sql-users).
...
6. Test that Extract and Replicat are working properly by adding values to `OGGADM1.testtable`:
    ...
    COMMIT;
    ```
7. [Connect to the target CockroachDB Standard cluster](/docs/v24.1/connect-to-the-database) and check that the data was delivered to `public.testtable`:
