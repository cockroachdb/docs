...
This page guides you through an example use-case for global application development and deployment. It is the first section of the [Develop and Deploy a Global Application](/docs/v24.1/movr#develop-and-deploy-a-global-application) tutorial.
...
<Note>
In the [example deployment](/docs/v24.1/movr-flask-deployment), the application and the database deployments are separate and not co-located on the same machine.
</Note>
...
Limiting latency improves the user experience, and it can also help you avoid problems with data integrity, like [transaction contention](/docs/v24.1/performance-best-practices-overview#transaction-contention).
...
- Deploy the application so that each instance of the application communicates with the closest database instance. This can be configured during [application deployment](/docs/v24.1/movr-flask-deployment).
- Design the application's persistence layer to limit the number of queries on data stored outside of the region closest to the application. You can use CockroachDB's [multi-region features](/docs/v24.1/multiregion-overview) to control where data is stored and from where it is accessed.
...
We cover setting up an external load balancer in [Deploy a Global, Serverless Application](/docs/v24.1/movr-flask-deployment).

## Next steps

You should now be ready to start [creating a multi-region database schema](/docs/v24.1/movr-flask-database).
