...
This page summarizes how `NULL` values are handled in CockroachDB SQL. Each topic is demonstrated via the [built-in SQL client](/docs/v24.1/cockroach-sql).
...
The [conditional operators](/docs/v24.1/scalar-expressions#conditional-expressions) (including `IF`, `COALESCE`, `IFNULL`) only evaluate some operands depending on the value of a condition operand, so their result is not always `NULL` depending on the given operands.
...
Aggregate [functions](/docs/v24.1/functions-and-operators) are those that operate on a set of rows and return a single value. The example data has been repeated here to make it easier to understand the results.
...
`NULL` values are considered as part of a `UNION` [set operation](/docs/v24.1/selection-queries#set-operations).
...
When [sorting a column](/docs/v24.1/order-by) containing `NULL` values, CockroachDB sorts `NULL` values first with `ASC` and last with `DESC`. This differs from PostgreSQL, which sorts `NULL` values last with `ASC` and first with `DESC`.

Use the `NULLS FIRST` and `NULLS LAST` options of the [`ORDER BY`](/docs/v24.1/order-by#parameters) to change where `NULL` values appear in the sort order.
...
<Note>
In CockroachDB v20.2 and earlier, for all values other than [`STRING`](/docs/v24.1/string), concatenation between a non-`NULL` value and a `NULL` value results in an [`ARRAY`](/docs/v24.1/array) of the non-`NULL` value's type. To return an `ARRAY` of a specific type from a `NULL` concatenation in CockroachDB v21.1 and later, [cast](/docs/v24.1/data-types#data-type-conversions-and-casts) the `NULL` value to an `ARRAY`.
</Note>
