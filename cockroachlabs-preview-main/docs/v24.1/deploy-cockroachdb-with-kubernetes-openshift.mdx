...
2. Enter "cockroach" in the search box. There are two tiles called **CockroachDB Operator**. Find the tile *without* the `Marketplace` label (which requires a subscription).
    ...
    <Frame>
      ![OpenShift OperatorHub](/images/v24.1/cockroachdb-operator-openshift.png)
    </Frame>
...
2. In the **CockroachDB Operator** tile, click **Create instance**.
    ...
    <Frame>
      ![OpenShift OperatorHub](/images/v24.1/cockroachdb-operator-instance-openshift.png)
    </Frame>
...
5. Navigate to **Workloads** &gt; **Pods** and observe the pods being created:
    ...
    <Frame>
      ![OpenShift OperatorHub](/images/v24.1/cockroachdb-operator-pods-openshift.png)
    </Frame>
...
1. Start the CockroachDB [built-in SQL client](/docs/v24.1/cockroach-sql) from the client pod:
...
To access the CockroachDB cluster's [DB Console](/docs/v24.1/ui-overview):
...
1. On secure clusters, [certain pages of the DB Console](/docs/v24.1/ui-overview#db-console-access) can only be accessed by `admin` users.
    ...
    Start the CockroachDB [built-in SQL client](/docs/v24.1/cockroach-sql) from the client pod:
...
To run a sample [CockroachDB workload](/docs/v24.1/cockroach-workload):
...
3. Select one of the CockroachDB pods on the **Pods** page and click **Logs**. This will reveal the JDBC URL that your application can use to connect to CockroachDB:
    ...
    <Frame>
      ![OpenShift OperatorHub](/images/v24.1/cockroachdb-operator-logs-openshift.png)
    </Frame>

## Step 8. Delete the cluster

<Note>
If you want to continue using this cluster, see the documentation on [configuring](/docs/v24.1/configure-cockroachdb-kubernetes), [scaling](/docs/v24.1/scale-cockroachdb-kubernetes), [monitoring](/docs/v24.1/monitor-cockroachdb-kubernetes), and [upgrading](/docs/v24.1/upgrade-cockroachdb-kubernetes) the cluster.
</Note>
...
1. Go to the **Installed Operators** page and find the cluster name of the CockroachDB cluster. Select **Delete CrdbCluster** from the menu.
    ...
    <Frame>
      ![OpenShift OperatorHub](/images/v24.1/cockroachdb-operator-delete-openshift.png)
    </Frame>
