...
You can create changefeeds on tables with more than one [column family](/docs/v24.1/column-families). Changefeeds will emit individual messages per column family on a table.
...
To target a table with multiple column families, set the [`split_column_families` option](/docs/v24.1/create-changefeed#split-column-families) when creating a changefeed:
...
<Note>
You can also use [basic changefeeds](/docs/v24.1/changefeeds-on-tables-with-column-families?filters=core#create-a-basic-changefeed-on-a-table-with-column-families) on tables with column families by using the [`EXPERIMENTAL CHANGEFEED FOR`](/docs/v24.1/changefeed-for) statement with `split_column_families` or the `FAMILY` keyword.
</Note>
...
The response will follow a typical [changefeed message format](/docs/v24.1/changefeed-messages#responses), but with the family name appended to the table name with a `.`, in the format `table.family`:

```text
{"after":{"column":"value"},"key":[1],"topic":"table.family"}
```

For [cloud storage sinks](/docs/v24.1/changefeed-sinks#cloud-storage-sink), the filename will include the family name appended to the table name with a `+`, in the format `table+primary`.

[Avro](/docs/v24.1/changefeed-messages#avro) schema names will include the family name concatenated to the table name.
...
In this example, you'll set up changefeeds on two tables that have [column families](/docs/v24.1/column-families). You'll use a single-node cluster sending changes to a webhook sink for this example, but you can use any [changefeed sink](/docs/v24.1/changefeed-sinks) to work with tables that include column families.
...
15. To create a changefeed that emits messages for all column families in a table, use the [`split_column_families`](/docs/v24.1/create-changefeed#split-column-families) option:
    ...
    <Note>
    You can find details of your changefeed job using [`SHOW CHANGEFEED JOBS`](/docs/v24.1/show-jobs#show-changefeed-jobs). Changefeeds streaming to [Kafka](/docs/v24.1/changefeed-sinks#kafka) or [Google Cloud Pub/Sub](/docs/v24.1/changefeed-sinks#google-cloud-pub-sub) will populate the `topics` field in the `SHOW CHANGEFEED JOBS` output.
...
In this example, you'll set up basic changefeeds on two tables that have [column families](/docs/v24.1/column-families). You'll use a single-node cluster with the basic changefeed sending changes to the client.
...
11. To create a changefeed that emits messages for all column families in a table, use the [`split_column_families`](/docs/v24.1/changefeed-for#split-column-families) option:
...
- [`EXPERIMENTAL CHANGEFEED`](/docs/v24.1/changefeed-for)
- [`CREATE CHANGEFEED`](/docs/v24.1/create-changefeed)
- [Changefeed Sinks](/docs/v24.1/changefeed-sinks)
- [Changefeed Examples](/docs/v24.1/changefeed-examples)
