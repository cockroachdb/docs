...
By default, CockroachDB automatically generates full statistics when tables are [created](/docs/v25.2/create-table) and after [schema changes](/docs/v25.2/online-schema-changes). Full statistics for a table are automatically refreshed when approximately 20% of its rows are updated.

A [background job](/docs/v25.2/create-statistics#view-statistics-jobs) automatically determines which columns to get statistics on. Specifically, the optimizer chooses:

- Columns that are part of the primary key or an index (in other words, all indexed columns).
- Up to 100 non-indexed columns.

By default, CockroachDB also automatically collects [multi-column statistics](/docs/v25.2/create-statistics#create-statistics-on-multiple-columns) on columns that prefix an index.

<InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v25.2: To control automatic collection of
full statistics, use the following settings:

- [`sql.stats.automatic_full_collection.enabled`](/docs/v25.2/cluster-settings#setting-sql-stats-automatic-full-collection-enabled): Cluster setting that enables automatic collection of full table statistics across all tables in the cluster.
- [`sql_stats_automatic_full_collection_enabled`](/docs/v25.2/with-storage-parameter#table-parameters): Table storage parameter that overrides the cluster setting when applied to a specific table.
...
Suppose the [cluster settings](/docs/v25.2/cluster-settings) `sql.stats.automatic_collection.fraction_stale_rows` and `sql.stats.automatic_collection.min_stale_rows` have the default values.2 and 500 as shown in the preceding table.
...
- Partial statistics created with `USING EXTREMES` and no `ON` clause are collected on all single-column prefixes of non-inverted indexes. Indexes that are [partial](/docs/v25.2/partial-indexes), [hash-sharded](/docs/v25.2/hash-sharded-indexes), or implicitly partitioned (such as in [`REGIONAL BY ROW` tables](/docs/v25.2/regional-tables#regional-by-row-tables) ) are excluded.
...
To control automatic collection of partial statistics, use the following [cluster settings](/docs/v25.2/cluster-settings) to configure behavior across all tables in the cluster:
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        <a href="/docs/v25.2/cluster-settings#setting-sql-stats-automatic-partial-collection-enabled">
    ...
    <tr>
      <td>
        <a href="/docs/v25.2/cluster-settings#setting-sql-stats-automatic-partial-collection-min-stale-rows">
    ...
    <tr>
      <td>
        <a href="/docs/v25.2/cluster-settings#setting-sql-stats-automatic-partial-collection-fraction-stale-rows">
...
Override cluster settings for specific tables using the following [table storage parameters](/docs/v25.2/with-storage-parameter#table-parameters):
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        <a href="/docs/v25.2/with-storage-parameter#table-parameters">
...
You can manually create partial statistics on the highest and lowest index values, when [`enable_create_stats_using_extremes`](/docs/v25.2/session-variables#enable-create-stats-using-extremes) session variable is enabled, using the `USING EXTREMES` clause: [`CREATE STATISTICS stats FROM table USING EXTREMES`](/docs/v25.2/create-statistics#create-partial-statistics-using-extremes).
...
2. Use the [`SHOW STATISTICS`](/docs/v25.2/show-statistics) statement to view automatically generated statistics.
...
To learn how to manually generate statistics, see the [`CREATE STATISTICS` examples](/docs/v25.2/create-statistics#examples).
...
You can enable and disable automatic [full](#full-statistics) and [partial](#partial-statistics) statistics collection for individual tables using the `sql_stats_automatic_collection_enabled` [storage parameter](/docs/v25.2/with-storage-parameter#table-parameters). This table setting **takes precedence** over the `sql.stats.automatic_collection.enabled` [cluster setting](/docs/v25.2/cluster-settings) described in [Enable and disable automatic statistics collection for clusters](#enable-and-disable-automatic-statistics-collection-for-clusters).
...
Or by using [`ALTER TABLE... SET`](/docs/v25.2/alter-table#set-storage-parameter):
...
`ALTER TABLE accounts RESET (sql_stats_automatic_collection_enabled)` removes the table setting, in which case the `sql.stats.automatic_collection.enabled` [cluster setting](/docs/v25.2/cluster-settings) is in effect for the table.
...
CockroachDB deletes statistics on non-default columns according to the `sql.stats.non_default_columns.min_retention_period` [cluster setting](/docs/v25.2/cluster-settings), which defaults to 24 hours.
...
`ALTER TABLE accounts RESET (sql_stats_forecasts_enabled)` removes the table setting, in which case the `sql.stats.forecasts.enabled` [cluster setting](/docs/v25.2/cluster-settings) is in effect for the table.

### Control histogram collection

By default, the optimizer collects histograms for all index columns (specifically the first column in each index) during automatic statistics collection. If a single column statistic is explicitly requested using manual invocation of [`CREATE STATISTICS`](/docs/v25.2/create-statistics), a histogram will be collected, regardless of whether or not the column is part of an index.

<Note>
CockroachDB does not support:

- Histograms on [`ARRAY`-typed](/docs/v25.2/array) columns. As a result, statistics created on `ARRAY` -typed columns do not include histograms.
- Multi-column histograms.
</Note>

If you are an advanced user and need to disable histogram collection for troubleshooting or performance tuning reasons, change the [`sql.stats.histogram_collection.enabled` cluster setting](/docs/v25.2/cluster-settings) by running [`SET CLUSTER SETTING`](/docs/v25.2/set-cluster-setting) as follows:

```sql
SET CLUSTER SETTING sql.stats.histogram_collection.enabled = false;
```

When `sql.stats.histogram_collection.enabled` is set to `false`, histograms are never collected, either as part of automatic statistics collection or by manually invoking [`CREATE STATISTICS`](/docs/v25.2/create-statistics).
...
We recommend that you allow the optimizer to consider column size when costing plans. If you are an advanced user and need to disable using `avg_size` for troubleshooting or performance tuning reasons, set the [`cost_scans_with_default_col_size` session variable](/docs/v25.2/set-vars#cost-scans-with-default-col-size) to `true` with `SET cost_scans_with_default_col_size=true`.

## Control whether the optimizer creates a plan with a full scan

Even if you have [secondary indexes](/docs/v25.2/schema-design-indexes), the optimizer may determine that a full table scan will be faster. For example, if you add a secondary index to a table with a large number of rows and find that a statement plan is not using the secondary index, then it is likely that performing a full table scan using the primary key is faster than doing a secondary index scan plus an [index join](/docs/v25.2/indexes#example).

You can disable statement plans that perform full table scans with the [`disallow_full_table_scans` session variable](/docs/v25.2/set-vars#disallow-full-table-scans).
...
You can specify [whether an index is visible](/docs/v25.2/alter-index#not-visible) to the cost-based optimizer. By default, indexes are visible. If not visible, the index will not be used in queries unless it is specifically selected with an [index hint](/docs/v25.2/indexes#selection). This allows you to create an index and check for query plan changes without affecting production queries. For an example, see [Set an index to be not visible](/docs/v25.2/alter-index#set-an-index-to-be-not-visible).

You can also set an index as [partially visible](/docs/v25.2/alter-index#visibility) within a range of `0.0` to `1.0`, where `0.0` means not visible and `1.0` means visible. Any value between `0.0` and `1.0` means that an index is visible to the specified fraction of queries. For the purposes of [index recommendations](/docs/v25.2/explain#success-responses), partially visible indexes are treated as [not visible](/docs/v25.2/alter-index#not-visible). If a partially visible index can be used to improve a query plan, the optimizer will recommend making it fully visible. For an example, refer to [Set an index as partially visible](/docs/v25.2/alter-index#set-an-index-as-partially-visible).

<Note>
  Indexes that are not visible are still used to enforce `UNIQUE` and `FOREIGN KEY`
  [constraints](/docs/v25.2/constraints). For more considerations, see [Index visibility
  considerations](/docs/v25.2/alter-index#not-visible).
</Note>

You can instruct the optimizer to use indexes marked as not visible with the [`optimizer_use_not_visible_indexes` session variable](/docs/v25.2/set-vars#optimizer-use-not-visible-indexes). By default, the variable is set to `off`.

## Locality optimized search in multi-region clusters

In [multi-region deployments](/docs/v25.2/multiregion-overview) with [regional by row tables](/docs/v25.2/table-localities#regional-by-row-tables), the optimizer, in concert with the [SQL engine](/docs/v25.2/architecture/sql-layer), may perform a _locality optimized search_ to attempt to avoid high-latency, cross-region communication between nodes. If there is a possibility that the results of a query all live in local rows, the database will first search for rows in the gateway node's region. The search only continues in remote regions if rows in the local region did not satisfy the query. Examples of queries that can use locality optimized search include unique key lookups and queries with [`LIMIT`](/docs/v25.2/limit-offset) clauses.
...
<Note>
  ...
  execution](/docs/v25.2/vectorized-execution#configure-vectorized-execution).
</Note>
...
Although the optimizer prefers to [read from rows in local regions](#locality-optimized-search-in-multi-region-clusters) when possible, by default, it does not guarantee that any query will not visit a remote region. This can occur if a query has no [home region](/docs/v25.2/multiregion-overview#table-localities) (for example, if it reads from different home regions in a [regional by row table](/docs/v25.2/table-localities#regional-by-row-tables)) or a query's home region differs from the [gateway](/docs/v25.2/architecture/life-of-a-distributed-transaction#gateway) region.

For some latency-sensitive applications, cross-region latency may not be acceptable. In these cases, set the [`enforce_home_region` session variable](/docs/v25.2/show-vars#enforce-home-region) to `on`. This configures the optimizer to return one of the following error types, and in some cases a suggested resolution, if a query cannot be run entirely in a single region:

- `Query has no home region`. The optimizer provides a hint on how to run the query in a single region.
- `Query is not running in its home region`. The optimizer provides a hint containing the home region of the query. The application should disconnect and then reconnect with a [connection string](/docs/v25.2/connection-parameters) specifying a node in the query's home region.

Only tables with `ZONE` [survivability](/docs/v25.2/multiregion-survival-goals#when-to-use-zone-vs-region-survival-goals) can be scanned without error when this setting is enabled.
...
The following statements can use the plan cache: [`SELECT`](/docs/v25.2/select-clause), [`INSERT`](/docs/v25.2/insert), [`UPDATE`](/docs/v25.2/update), [`UPSERT`](/docs/v25.2/upsert), and [`DELETE`](/docs/v25.2/delete).
...
- _Generic_ query plans are generated and optimized once without considering specific placeholder values, and are **not** regenerated on subsequent executions, unless the plan becomes stale due to [schema changes](/docs/v25.2/online-schema-changes) or new [table statistics](#table-statistics) and must be re-optimized. This approach eliminates most of the query latency attributed to planning. For example:
  ...
  <Tip>
    ...
    most beneficial for queries with high planning times, such as queries with many [joins](/docs/v25.2/joins). For more
...
To change the type of plan that is cached, use the [`plan_cache_mode`](/docs/v25.2/session-variables#plan-cache-mode) session setting. This setting applies when a statement is executed, not when it is prepared. Statements are therefore not associated with a specific query plan type when they are prepared.
...
<Note>
  ...
  functions](/docs/v25.2/functions-and-operators#function-volatility), regardless of the `plan_cache_mode` setting.
</Note>
...
At the [database level](/docs/v25.2/alter-database#set-session-variable):

```sql
ALTER DATABASE db SET plan_cache_mode = auto;
```

At the [role level](/docs/v25.2/alter-role#set-default-session-variable-values-for-a-role):

```sql
ALTER ROLE db_user SET plan_cache_mode = auto;
```

To verify the plan type used by a query, check the [`EXPLAIN ANALYZE`](/docs/v25.2/explain-analyze) output for the query.
...
For a query involving multiple joins, the cost-based optimizer will explore additional [join orderings](/docs/v25.2/joins) in an attempt to find the lowest-cost execution plan, which can lead to significantly better performance in some cases.
...
To change this setting, which is controlled by the `reorder_joins_limit` [session variable](/docs/v25.2/set-vars), run the following statement:

```sql
SET reorder_joins_limit = 0;
```

To disable this feature, set the variable to `0`. You can configure the default `reorder_joins_limit` session setting with the [cluster setting](/docs/v25.2/cluster-settings) `sql.defaults.reorder_joins_limit`, which has a default value of `8`.
...
- To limit the size of the subtree that can be reordered, set the `reorder_joins_limit` [session variable](/docs/v25.2/set-vars) to a lower value, for example:
...
For a join hint example, see [Use the right join type](/docs/v25.2/apply-statement-performance-rules#rule-3-use-the-right-join-type).
...
- `INVERTED`: Forces an inverted join into the right side; the right side must be a table with a suitable [GIN index](/docs/v25.2/inverted-indexes). `INVERTED` can be used only with `INNER` and `LEFT` joins.

  <Note>You cannot use inverted joins on [partial GIN indexes](/docs/v25.2/inverted-indexes#partial-gin-indexes).</Note>
...
<Note>
  ...
  `prefer_lookup_joins_for_fks` [session variable](/docs/v25.2/set-vars) to `on`.
</Note>
...
The optimizer supports index hints to prevent or force a zigzag join. Apply the hints in the same way as other existing [index hints](/docs/v25.2/table-expressions#force-index-selection).
...
You can omit the `INNER INVERTED JOIN` statement by putting `v1.ext` on the left side of a `@>` join condition in a `WHERE` clause and using an [index hint](/docs/v25.2/table-expressions#force-index-selection) for the GIN index.
