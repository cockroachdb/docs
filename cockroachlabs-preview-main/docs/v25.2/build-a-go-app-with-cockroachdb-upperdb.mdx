...
<LinkedPageTabs
  currentPath="/docs/v25.2/build-a-go-app-with-cockroachdb-upperdb"
  items={[
    {
      title: "Use pgx",
...
- Updates the rows in the `accounts` table within an explicit [transaction](/docs/v25.2/transactions).
...
Note that the sample code also includes a function that simulates a transaction error (`crdbForceRetry()`). Upper/db's CockroachDB adapter [automatically retries transactions](/docs/v25.2/transaction-retry-error-reference#client-side-retry-handling) when transaction errors are thrown. As a result, this function forces a transaction retry.

To run the code, copy the sample above, or [download it directly](https://raw.githubusercontent.com/cockroachdb/docs/master/_includes/v25.2/app/upperdb-basic-sample/main.go).
...
Copy the code or [download it directly](https://raw.githubusercontent.com/cockroachdb/docs/master/_includes/v25.2/app/insecure/upperdb-basic-sample/main.go).
