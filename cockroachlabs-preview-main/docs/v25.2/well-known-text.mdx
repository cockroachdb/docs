...
Shapes expressed in WKT can have an [SRID](/docs/v25.2/architecture/glossary) prepended to the shape and followed by a semicolon, in the form `SRID=123;TAG(...)`. The format is known as Extended Well Known Text (_EWKT_); it is the same as WKT, with an [SRID](/docs/v25.2/architecture/glossary#srid) representation prepended to the data structure.

For example, below is a polygon representing a geometry that uses [SRID 4326](/docs/v25.2/srid-4326), which is used to represent latitude and longitude coordinates on the Earth as defined in the [WGS84](/docs/v25.2/architecture/glossary#wgs84) standard:
