...
If you need a location to store files for the [`IMPORT INTO`](/docs/v25.2/import-into) process, but do not have access to (or cannot use) [cloud storage providers](/docs/v25.2/use-cloud-storage), you can run a local file server. You can then use this file server by leveraging support for our [HTTP Export Storage API](#http-export-storage-api).
...
<Note>
HTTP file servers are not supported as storage for [backups](/docs/v25.2/take-full-and-incremental-backups).
</Note>

## HTTP export storage API

A CockroachDB [`IMPORT INTO`](/docs/v25.2/import-into) process that requires reading or writing external files can use the HTTP Export Storage API by prefacing the address with `http`, e.g., `http://fileserver/mnt/cockroach-exports`.
...
The PHP language has an HTTP server built in. You can serve local files using the commands below. For more information about how to import these locally served files, see the documentation for the [`IMPORT INTO`](/docs/v25.2/import-into) statement.
...
The Python language has an HTTP server included in the standard library. You can serve local files using the commands below. For more information about how to import these locally served files, see the documentation for the [`IMPORT INTO`](/docs/v25.2/import-into) statement.
...
The Ruby language has an HTTP server included in the standard library. You can serve local files using the commands below. For more information about how to import these locally served files, see the documentation for the [`IMPORT INTO`](/docs/v25.2/import-into) statement.
