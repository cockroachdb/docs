...
import { Copy5 } from "/snippets/syntax-diagrams/Copy5.jsx";

The `COPY... FROM` statement copies data from [`cockroach sql`](/docs/v25.2/cockroach-sql) or other [third party clients](/docs/v25.2/install-client-drivers) to tables in your cluster. The `COPY... TO` statement allows you to export a table or arbitrary query in a text or CSV format.
...
To run the examples, use [`cockroach demo`](/docs/v25.2/cockroach-demo) to start a temporary, in-memory cluster with the [`movr` database](/docs/v25.2/movr) preloaded.
...
1. Create a new table that you will load with CSV-formatted data:
    ...
    ```
2. Set the `bytea_output` [session variable](/docs/v25.2/set-vars#supported-variables) to specify that CockroachDB should ingest hex-encoded byte array data:
...
1. Start a temporary, in-memory cluster with the [`movr`](/docs/v25.2/movr) sample dataset preloaded:
