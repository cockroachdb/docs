...
If you previously [authenticated with cockroach cert](/docs/v25.2/deploy-cockroachdb-with-cockroachdb-operator#initialize-the-cluster), follow these steps to rotate the certificates using the same CA:
...
2. Upload the new client certificate and key to the Kubernetes cluster as a **new** secret, renaming them to the filenames required by the CockroachDB operator:
    ...
    ```
3. Create a new certificate and key pair for your CockroachDB nodes, overwriting the previous certificate and key. Specify the namespace you used when [deploying the cluster](/docs/v25.2/deploy-cockroachdb-with-cockroachdb-operator#initialize-the-cluster). This example uses the `cockroach-ns` namespace:
...
4. Upload the new node certificate and key to the Kubernetes cluster as a **new** secret, renaming them to the filenames required by the CockroachDB operator:
    ...
    ```
5. Add `cockroachdb.tls.externalCertificates.certificates.nodeClientSecretName` and `cockroachdb.tls.externalCertificates.certificates.nodeSecretName` to the values file used to [deploy the cluster](/docs/v25.2/deploy-cockroachdb-with-cockroachdb-operator#initialize-the-cluster):
...
6. Check that the secrets were created on the cluster:
    ...
    <Note>
    Remember that `nodeSecretName` and `nodeClientSecretName` in the operator configuration must specify these secret names. For details, see the [deployment guide](/docs/v25.2/deploy-cockroachdb-with-cockroachdb-operator#initialize-the-cluster).
    </Note>
