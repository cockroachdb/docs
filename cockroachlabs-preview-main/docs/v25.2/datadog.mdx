...
- Started a [secure CockroachDB self-hosted cluster](/docs/v25.2/secure-a-cluster).
...
This enables metrics collection via our [Prometheus endpoint](/docs/v25.2/monitoring-and-alerting#prometheus-endpoint).
...
For example, if you used [`cockroach cert`](/docs/v25.2/cockroach-cert) to [secure your cluster](/docs/v25.2/secure-a-cluster#step-1-generate-certificates), the paths would look something like:
...
Optionally enable Datadog to collect [CockroachDB logs](/docs/v25.2/logging-overview) by adding the following block to `cockroachdb.d/conf.yaml`:
...
The `path` value specifies the [default](/docs/v25.2/configure-logs#default-logging-configuration) CockroachDB log file and location.

<Note>
You can configure both the CockroachDB [logging directory](/docs/v25.2/configure-logs#set-file-defaults) and [log files](/docs/v25.2/configure-logs#output-to-files).
</Note>
...
-------------------
  ...
  metadata:
    ...
    version.minor: 2
    version.patch: 0
    version.raw: v25.2.0-alpha.00000000-1724-gc5c74249f7
...
Select **Threshold Alert** as the detection method. You can use this option to configure an alert that is sent when a [supported metric](https://docs.datadoghq.com/integrations/cockroachdb?tab=host#data-collected) reaches a given threshold. For descriptions of some useful CockroachDB alerts, see [Monitoring and Alerting](/docs/v25.2/monitoring-and-alerting#events-to-alert-on).

The example alert below will trigger when [a node has less than 15% of storage capacity remaining](/docs/v25.2/monitoring-and-alerting#node-is-running-low-on-disk-space):

<Frame>
  ![CockroachDB Threshold Alert in Datadog](/images/v24.2/datadog-crdb-threshold-alert.png)
</Frame>

- `cockroachdb.capacity.available` is divided by `cockroachdb.capacity.total` to determine the fraction of available capacity on the node's [store](/docs/v25.2/architecture/storage-layer) (the directory on each node where CockroachDB reads and writes its data).
