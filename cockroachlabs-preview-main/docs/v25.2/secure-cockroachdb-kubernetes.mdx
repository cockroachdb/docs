...
<Note>
This article assumes you have already [deployed CockroachDB securely on a single Kubernetes cluster](/docs/v25.2/deploy-cockroachdb-with-kubernetes) using the Public operator or Helm. However, it's possible to configure these settings before starting CockroachDB on Kubernetes.
</Note>

By default, self-signed certificates are used when using the Public operator or Helm to securely [deploy CockroachDB on Kubernetes](/docs/v25.2/deploy-cockroachdb-with-kubernetes). However, the recommended approach is to use `cert-manager` for certificate management. For details, refer to [Deploy cert-manager for mTLS](/docs/v25.2/secure-cockroachdb-kubernetes?filters=helm#deploy-cert-manager-for-mtls).

This page is for Kubernetes deployments that are not using the CockroachDB operator. For guidance specific to the CockroachDB operator, read [Certificate Management with the CockroachDB operator](/docs/v25.2/secure-cockroachdb-operator).
...
To use your own certificate authority instead, add the following to the Public operator's custom resource **before** [initializing the cluster](/docs/v25.2/deploy-cockroachdb-with-kubernetes#initialize-the-cluster):
...
<Note>
This example uses [`cockroach cert` commands](/docs/v25.2/cockroach-cert) to generate and sign the CockroachDB node and client certificates. To learn more about the supported methods of signing certificates, refer to [Authentication](/docs/v25.2/authentication#using-digital-certificates-with-cockroachdb).
</Note>

<Note>
Complete the following steps **before** [initializing the cluster](/docs/v25.2/deploy-cockroachdb-with-kubernetes#initialize-the-cluster).
</Note>
...
4. Upload the client certificate and key to the Kubernetes cluster as a secret, renaming them to the filenames required by the Public operator:
    ...
    ```
5. Create the certificate and key pair for your CockroachDB nodes, specifying the namespace you used when [deploying the cluster](/docs/v25.2/deploy-cockroachdb-with-kubernetes#initialize-the-cluster). This example uses the Public operator's default namespace (`cockroach-operator-system`):
...
7. Check that the secrets were created on the cluster:
    ...
    ```
8. Add `nodeTLSSecret` and `clientTLSSecret` to the Public operator's [custom resource](/docs/v25.2/deploy-cockroachdb-with-kubernetes#initialize-the-cluster), specifying the generated secret names:
...
4. Upload the new node certificate and key to the Kubernetes cluster as a **new** secret, renaming them to the filenames required by the Public operator:
    ...
    ```
5. Add `nodeTLSSecret` and `clientTLSSecret` to the Public operator's [custom resource](/docs/v25.2/deploy-cockroachdb-with-kubernetes#initialize-the-cluster), specifying the new secret names:
...
6. Check that the secrets were created on the cluster:
    ...
    <Note>
    Remember that `nodeTLSSecret` and `clientTLSSecret` in the Public operator's [custom resource](/docs/v25.2/deploy-cockroachdb-with-kubernetes#initialize-the-cluster) must specify these secret names. For details, see [Use a custom CA](#use-a-custom-ca).
    </Note>
...
2. Generate an X.509 certificate, valid for 10 years. You will be prompted for the certificate field values.
    ...
    ```
3. Create the secret, making sure that [you are in the correct namespace](/docs/v25.2/deploy-cockroachdb-with-kubernetes#install-the-operator):
...
To use your own certificate authority instead, specify the following in the custom values file you created when [deploying the cluster](/docs/v25.2/deploy-cockroachdb-with-kubernetes?filters=helm#step-2-start-cockroachdb):
...
5. Create the certificate and key pair for your CockroachDB nodes:
    ...
    <Note>
    This example assumes that you followed our [deployment example](/docs/v25.2/deploy-cockroachdb-with-kubernetes?filters=helm), which uses `my-release` as the release name. If you used a different value, be sure to adjust the release name in this command.
    </Note>
...
7. Check that the secrets were created on the cluster:
    ...
    ```
8. Specify the following in the custom values file you created when [deploying the cluster](/docs/v25.2/deploy-cockroachdb-with-kubernetes?filters=helm#step-2-start-cockroachdb), using the generated secret names:
...
1. Upload the CA certificate that you previously created to the Kubernetes cluster as a secret:
    ...
    ```
2. Specify the following in the custom values file you created when [deploying the cluster](/docs/v25.2/deploy-cockroachdb-with-kubernetes?filters=helm#step-2-start-cockroachdb), using the generated secret name:
...
3. Customize the following `selfSigner` fields to set the frequency of certificate rotation. These should correspond to the durations of the CA, client, and node certificates.
    ...
    - `minimumCertDuration` specifies the minimum duration in hours for all certificates. This is to ensure that the client and node certificates are rotated within the duration of the CA certificate. This value must be less than:
        ...
        Certificate duration is configured when running [`cockroach cert`](/docs/v25.2/cockroach-cert#general). You can check the expiration dates of the `cockroach cert` certificates by running:
