...
A [statement diagnostics bundle](/docs/v25.2/ui-statements-page#diagnostics) contains statement traces in plaintext, JSON, and [Jaeger-compatible](/docs/v25.2/query-behavior-troubleshooting#visualize-statement-traces-in-jaeger) format.

## Usage overview

`SHOW TRACE FOR SESSION` returns [statement traces](#trace-description) for the most recently executed statements.

To start recording statement traces during a session, enable the `tracing` session variable via [`SET tracing = on;`](/docs/v25.2/set-vars#set-tracing). To stop recording statement traces during a session, disable the `tracing` session variable via [`SET tracing = off;`](/docs/v25.2/set-vars#set-tracing).

Recording statement traces during a session does not effect the logical execution of the statements. This means that errors encountered by statements during a recording are returned to clients. CockroachDB will [automatically retry](/docs/v25.2/transactions#automatic-retries) individual statements (considered implicit transactions) and multi-statement transactions sent as a single batch when [retry errors](/docs/v25.2/transactions#error-handling) are encountered due to [contention](/docs/v25.2/performance-best-practices-overview#transaction-contention). Also, clients will receive retry errors required to handle [client-side transaction retries](/docs/v25.2/transaction-retry-error-reference#client-side-retry-handling). As a result, traces of all transaction retries will be captured during a recording.
...
- [`SET (session settings)`](/docs/v25.2/set-vars)
