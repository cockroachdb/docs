...
[CockroachDB uses the Pebble storage engine](/docs/v25.2/cockroach-start#storage-engine). Pebble is inspired by RocksDB, but differs in that it:
...
CockroachDB uses [Pebble](/docs/v25.2/cockroach-start#storage-engine)––an embedded key-value store inspired by RocksDB, and developed by Cockroach Labs––to read and write data to disk.
...
<Frame>![Structure of an SST file](/images/v25.2/sst.png)</Frame>

SST files are immutable; they are never modified, even during the [compaction process](#compaction).

##### LSM levels

The levels of the LSM are organized from L0 to L6. L0 is the top-most level. L6 is the bottom-most level. New data is added into L0 (e.g., using [`INSERT`](/docs/v25.2/insert) or [`IMPORT INTO`](/docs/v25.2/import-into)) and then merged down into lower levels over time.
...
- To allow LSM-based storage engines like Pebble to support ingesting large amounts of data, such as when using the [`IMPORT INTO`](/docs/v25.2/import-into) statement.
- To allow for easier and more efficient flushes of [memtables](#memtable-and-write-ahead-log).

<Frame>![LSM tree with SST files](/images/v25.2/lsm-with-ssts.png)</Frame>
...
<Tip>
  By default, the [storage engine](/docs/v25.2/architecture/storage-layer) uses a compaction concurrency of 3. If you
  ...
  LSM](/docs/v25.2/architecture/storage-layer#inverted-lsms) scenarios; and it can lead to increased overall performance
...
For instructions showing how to monitor your cluster's LSM health, see [LSM Health](/docs/v25.2/common-issues-to-monitor#lsm-health). To monitor your cluster's LSM L0 health, see [IO Overload](/docs/v25.2/ui-overload-dashboard#io-overload).
...
<Frame>![Relationship between memtable, WAL, and SSTs](/images/v25.2/memtable-wal-sst.png)</Frame>
...
CockroachDB regularly garbage collects MVCC values to reduce the size of data stored on disk. To do this, we compact old MVCC values when there is a newer MVCC value with a timestamp that's older than the garbage collection period. The garbage collection period can be set at the cluster, database, or table level by configuring the [`gc.ttlseconds` replication zone variable](/docs/v25.2/configure-replication-zones#gc-ttlseconds). For more information about replication zones, see [Replication Controls](/docs/v25.2/configure-replication-zones).
...
Now that you've learned about our architecture, [start up a CockroachDB Standard cluster](/docs/cockroachcloud/quickstart) or [local cluster](/docs/v25.2/install-cockroachdb) and start [building an app with CockroachDB](/docs/v25.2/example-apps).
