...
[Starting a physical replication](/docs/v25.2/set-up-physical-cluster-replication) stream consists of two jobs: one each on the standby and primary cluster:
...
2. The primary cluster chooses a timestamp at which to start the physical replication stream. Data on the primary is protected from [garbage collection](/docs/v25.2/architecture/storage-layer#garbage-collection) until it is replicated to the standby using a [protected timestamp](/docs/v25.2/architecture/storage-layer#protected-timestamps).
3. The primary cluster returns the timestamp and a [job ID](/docs/v25.2/show-jobs#response) for the replication job.
...
During the job, [rangefeeds](/docs/v25.2/create-and-configure-changefeeds#enable-rangefeeds) are periodically emitting resolved timestamps, which is the time where the ingested data is known to be consistent. Resolved timestamps provide a guarantee that there are no new writes from before that timestamp. This allows the standby cluster to move the [protected timestamp](/docs/v25.2/architecture/storage-layer#protected-timestamps) forward as the replicated timestamp advances. This information is sent to the primary cluster, which allows for [garbage collection](/docs/v25.2/architecture/storage-layer#garbage-collection) to continue as the replication stream on the standby cluster advances.
...
<Frame>
  ![Timeline showing how the failover window is between the retained time and replicated time.](/images/v25.2/failover.svg)
</Frame>
...
For the [failover process](/docs/v25.2/failover-replication), the standby cluster waits until it has reached the specified failover time, which can be in the [past](/docs/v25.2/failover-replication#fail-over-to-a-point-in-time) (retained time), the [`LATEST`](/docs/v25.2/failover-replication#fail-over-to-the-most-recent-replicated-time) timestamp, or in the [future](/docs/v25.2/failover-replication#fail-over-to-a-point-in-time). Once that timestamp has been reached, the replication stream stops and any data in the standby cluster that is **above** the failover time is removed. Depending on how much data the standby needs to revert, this can affect the duration of RTO (recovery time objective).

<Note>
When a PCR stream is started with a `readonly` virtual cluster, the job will delete the `readonly` virtual cluster automatically if a failover is initiated with a [historical timestamp](/docs/v25.2/failover-replication#fail-over-to-a-point-in-time). If the failover is initiated with the [most recent replicated time](/docs/v25.2/failover-replication#fail-over-to-the-most-recent-replicated-time), the `readonly` virtual cluster will remain on the standby cluster.
</Note>
...
For details on failing back to the primary cluster following a failover, refer to [Fail back to the primary cluster](/docs/v25.2/failover-replication#failback).

### Multi-region behavior and best practices

You can use PCR to replicate between clusters with different [cluster regions](/docs/v25.2/multiregion-overview#cluster-regions), [database regions](/docs/v25.2/multiregion-overview#database-regions), and [table localities](/docs/v25.2/table-localities). Mismatched regions and localities do not impact the [failover process](/docs/v25.2/failover-replication) or ability to access clusters after failover, but they do impact [leaseholders](/docs/v25.2/architecture/glossary#leaseholder) and locality-dependent settings.
