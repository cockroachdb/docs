...
To deploy CockroachDB in a single Kubernetes cluster instead, see [Kubernetes Single-Cluster Deployment](/docs/v25.2/deploy-cockroachdb-with-kubernetes). Also, for details about potential performance bottlenecks to be aware of when running CockroachDB in Kubernetes and guidance on how to optimize your deployment for better performance, see [CockroachDB Performance on Kubernetes](/docs/v25.2/kubernetes-performance).
...
4. If you haven't already, [install CockroachDB locally and add it to your `PATH`](/docs/v25.2/install-cockroachdb). The `cockroach` binary will be used to generate certificates.
...
5. Optionally, to optimize your deployment for better performance, review [CockroachDB Performance on Kubernetes](/docs/v25.2/kubernetes-performance) and make the desired modifications to the `cockroachdb-statefulset-secure.yaml` file.
...
1. On secure clusters, [certain pages of the DB Console](/docs/v25.2/ui-overview#db-console-access) can only be accessed by `admin` users.

    Get a shell into the pod with the `cockroach` binary created earlier and start the CockroachDB [built-in SQL client](/docs/v25.2/cockroach-sql):
...
6. In the UI, check the [**Node List**](/docs/v25.2/ui-cluster-overview-page#node-list) to verify that all nodes are running, open the [**Databases** page](/docs/v25.2/ui-databases-page) to verify that `bank` is listed, and open the [**Network Latency** page](/docs/v25.2/ui-network-latency-page) to see the performance of your CockroachDB cluster across 3 regions.
...
Each of your Kubernetes clusters contains 3 instances that can run CockroachDB pods. It's easy to scale a cluster to run more pods. To ensure that you do not have two CockroachDB pods on the same instance (as recommended in our [production best practices](/docs/v25.2/recommended-production-settings)), you need to first add a new instance and then edit your StatefulSet configuration to add another pod.
...
1. Verify that you can upgrade.
    ...
    Therefore, in order to upgrade to v25.2, you must be on a production release of v25.1.

    1. If you are upgrading to v25.2 from a production release earlier than v25.1, or from a testing release (alpha/beta), first [upgrade to a production release of v25.1](https://www.cockroachlabs.com/docs/v25.1/orchestrate-cockroachdb-with-kubernetes-multi-cluster#upgrade-the-cluster). Be sure to complete all the steps.
    2. Then return to this page and perform a second upgrade to v25.2.
    3. If you are upgrading from any production release of v25.1, or from any earlier v25.2 release, you do not have to go through intermediate releases; continue to step 2.
2. Verify the overall health of your cluster using the [DB Console](/docs/v25.2/ui-overview). On the **Overview**:
...
3. Review the backward-incompatible changes in v25.2 and deprecated features. If any affect your deployment, make the necessary changes before starting the rolling upgrade to v25.2.
4. Decide how the upgrade will be finalized.

    By default, after all nodes are running the new version, the upgrade process will be **auto-finalized**. This will enable certain features and performance improvements introduced in v25.2. After finalization, however, it will no longer be possible to perform a downgrade to v25.1. In the event of a catastrophic failure or corruption, the only option is to start a new cluster using the old binary and then restore from a [backup](/docs/v25.2/take-full-and-incremental-backups) created prior to the upgrade. For this reason, **we recommend disabling auto-finalization** so you can monitor the stability and performance of the upgraded cluster before finalizing the upgrade, but note that you will need to follow all of the subsequent directions, including the manual finalization in a later step.

    <Note>
    Finalization only applies when performing a major version upgrade (for example, from v25.1.x to v25.2). Patch version upgrades (for example, within the v25.2.x series) can always be downgraded.
    </Note>

    1. Get a shell into the pod with the `cockroach` binary created earlier and start the CockroachDB [built-in SQL client](/docs/v25.2/cockroach-sql):

        ```shell
        $ kubectl exec -it cockroachdb-client-secure --context <cluster-context> --namespace <cluster-namespace> -- ./cockroach sql --certs-dir=/cockroach-certs --host=cockroachdb-public
        ```
    2. Set the `cluster.preserve_downgrade_option` [cluster setting](/docs/v25.2/cluster-settings):
...
7. If you disabled auto-finalization earlier, monitor the stability and performance of your cluster until you are comfortable with the upgrade (generally at least a day).
    ...
    <Note>
    This is only possible when performing a major version upgrade (for example, from v25.1.x to v25.2). Patch version upgrades (for example, within the v25.2.x series) are auto-finalized.
    </Note>
...
<Note>
The below steps use [`cockroach cert` commands](/docs/v25.2/cockroach-cert) to quickly generate and sign the CockroachDB node and client certificates. Read our [Authentication](/docs/v25.2/authentication#using-digital-certificates-with-cockroachdb) docs to learn about other methods of signing certificates.
</Note>
...
4. The StatefulSet configuration includes a [`cockroach start`](/docs/v25.2/cockroach-start) command that creates the nodes on the Kubernetes pods.
...
2. Get a shell into the pod and start the CockroachDB [built-in SQL client](/docs/v25.2/cockroach-sql), again specifying the namespace and context of the Kubernetes cluster where the pod is running:
    ...
    ```
3. Run some basic [CockroachDB SQL statements](/docs/v25.2/learn-cockroachdb-sql):
    ...
    (1 row)
    ```
4. [Create a user with a password](/docs/v25.2/create-user#create-a-user-with-a-password):
...
5. Exit the SQL shell and pod:
    ...
    The pod will continue running indefinitely, so any time you need to reopen the built-in SQL client or run any other [`cockroach` client commands](/docs/v25.2/cockroach-commands) (e.g., `cockroach node`), repeat step 2 using the appropriate command.
...
- [Kubernetes Single-Cluster Deployment](/docs/v25.2/deploy-cockroachdb-with-kubernetes)
...
- [Performance Benchmarking](/docs/v25.2/performance-benchmarking-with-tpcc-small)
- [Performance Tuning](/docs/v25.2/performance-best-practices-overview)
