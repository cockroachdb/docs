...
<LinkedPageTabs
  currentPath="/docs/v25.2/performance-benchmarking-with-tpcc-large"
  items={[
    {
      title: "Local",
...
3. Start CockroachDB using the [`cockroach start`](/docs/v25.2/cockroach-start) command:
    ...
    Each node will start with a [locality](/docs/v25.2/cockroach-start#locality) that includes an artificial "rack number" (e.g., `--locality=rack=0`). Use 81 racks for 81 nodes so that 1 node will be assigned to each rack.
4. Repeat these steps for the other 80 VMs for CockroachDB nodes. Each time, be sure to:
    - Adjust the `--advertise-addr` flag.
    - Set the [`--locality`](/docs/v25.2/cockroach-start#locality) flag to the appropriate "rack number".
...
2. Launch the [built-in SQL shell](/docs/v25.2/cockroach-sql):

    ```shell
    $ cockroach sql --insecure --host=<address of any node>
    ```
3. Adjust some [cluster settings](/docs/v25.2/cluster-settings):
    ...
    ```
4. Change the default [GC TTL](/docs/v25.2/configure-replication-zones#gc-ttlseconds) to the following value:
...
1. [Partition your database](/docs/v25.2/partitioning) to divide all of the TPC-C tables and indexes into 81 partitions, one per rack, and then use [zone configurations](/docs/v25.2/configure-replication-zones) to pin those partitions to a particular rack.
2. Wait for up-replication and partitioning to finish. You will know when they have finished because both the number of *lease transfers* and *snapshots* will go down to `0` and stay there. This will likely take 10s of minutes.
    - To monitor the number of lease transfers, open the [DB Console](/docs/v25.2/ui-overview), select the **Replication** dashboard, hover over the **Range Operations** graph, and check the **Lease Transfers** data point.
    - To check the number of snapshots, open the [DB Console](/docs/v25.2/ui-overview), select the **Replication** dashboard, and hover over the **Snapshots** graph.
