...
Enable [cluster virtualization](/docs/v25.2/cluster-virtualization-overview) in your CockroachDB cluster to set up a [**physical cluster replication (PCR)**](/docs/v25.2/physical-cluster-replication-overview) stream. This page is a guide to working with virtual clusters.

## Connect to a virtual cluster

This section shows how to use [SQL clients](#sql-clients) or the [DB Console](#db-console) to connect to a virtual cluster.

When [PCR](/docs/v25.2/physical-cluster-replication-overview) is enabled, the virtual cluster is named `main`.

### SQL clients

This section shows how to connect using [`cockroach sql`](/docs/v25.2/cockroach-sql) when cluster virtualization is enabled.

Unless you specify which virtual cluster to connect to, when you connect using a SQL client, you are logged into the default virtual cluster. When [PCR](/docs/v25.2/physical-cluster-replication-overview) is enabled, the default virtual cluster is named `main`.
...
This section shows how to connect using the [DB Console](/docs/v25.2/ui-overview) when cluster virtualization is enabled.

Unless you specify which virtual cluster to connect to, when you connect using the DB Console, you are logged into the default virtual cluster. When [PCR](/docs/v25.2/physical-cluster-replication-overview) is enabled, the default virtual cluster is named `main`. In order to view metrics on [replication lag](/docs/v25.2/physical-cluster-replication-technical-overview) for a PCR job, connect to the standby cluster's DB Console.
...
To [grant](/docs/v25.2/grant) access to the system virtual cluster, you must connect to the system virtual cluster as a user with the `admin` role, then grant either of the following to the SQL user:

- The `admin` [role](/docs/v25.2/security-reference/authorization#admin-role) grants the ability to read and modify system tables and cluster settings on any virtual cluster, including the system virtual cluster.
- The `VIEWSYSTEMDATA` [system privilege](/docs/v25.2/security-reference/authorization#supported-privileges) grants the ability to read system tables and cluster settings on any virtual cluster, including the system virtual cluster.
...
When cluster virtualization is enabled, [backup](/docs/v25.2/backup) and [restore](/docs/v25.2/restore) commands are scoped to the virtual cluster by default.
...
2. [Back up the cluster](/docs/v25.2/backup). Only the virtual cluster's data and settings are included in the backup, and data and settings for other virtual clusters or for the system virtual cluster is omitted.
...
2. [Back up the cluster](/docs/v25.2/backup), and include the `INCLUDE_ALL_SECONDARY_TENANTS` flag in the `BACKUP` command. All virtual clusters and the system virtual cluster are included in the backup.
...
2. [Restore the cluster](/docs/v25.2/restore). Only the virtual cluster's data and settings are restored.
...
2. [Restore the cluster](/docs/v25.2/restore) from a backup that included the the `INCLUDE_ALL_SECONDARY_VIRTUAL_CLUSTERS` flag. All virtual clusters and the system virtual cluster are restored.

## Configure cluster settings

When [cluster virtualization](/docs/v25.2/cluster-virtualization-overview) is enabled, each [cluster setting](/docs/v25.2/cluster-settings) is scoped to either a virtual cluster or the system virtual cluster.
...
For more details, including the scope of each cluster setting, refer to [Cluster Setting Scopes with Cluster Virtualization enabled](/docs/v25.2/cluster-virtualization-setting-scopes).
...
<Note>
Both the `auto_upgrade.enabled` and `preserve_downgrade_option` [cluster settings](/docs/v25.2/cluster-settings) are scoped to a virtual cluster. To prevent automatic finalization of an upgrade, you must do one of the following:

- Set `auto_upgrade.enabled` to `false` before beginning an upgrade. This setting persists after an upgrade. To finalize the upgrade manually, refer to [Upgrade a cluster](/docs/v25.2/upgrade-cockroach-version). This is the preferred way to disable auto-upgrade.
...
- [Cluster Setting Scopes](/docs/v25.2/cluster-virtualization-setting-scopes)
- [Cluster Metric Scopes](/docs/v25.2/cluster-virtualization-metric-scopes)
- [Physical Cluster Replication](/docs/v25.2/physical-cluster-replication-overview)
