...
This page describes how to configure CockroachDB user authentication using LDAP. You can additionally configure CockroachDB to use the same directory service for user [authorization](/docs/v25.2/ldap-authorization) (role-based access control), which assigns CockroachDB roles to users based on their group memberships in the directory.
...
3. Authorize the user (optional)
    - If [LDAP authorization](/docs/v25.2/ldap-authorization) is also enabled, the cluster sends an additional request to retrieve the groups to which the user is assigned, using configurable criteria.
...
<Note>
LDAP authentication cannot be used for the `root` user or other [reserved identities](/docs/v25.2/security-reference/authorization#reserved-identities). You must manage credentials for `root` separately using password authentication to ensure continuous administrative access regardless of LDAP availability.
</Note>
...
To redact these two settings, refer to [Sensitive settings](/docs/v25.2/cluster-settings#sensitive-settings).

### Step 2: Configure Host-Based Authentication (HBA)

To enable LDAP, you will need to update the [host-based authentication (HBA)](/docs/v25.2/security-reference/authentication#authentication-configuration) configuration specified in the cluster setting `server.host_based_authentication.configuration`.
...
If you also intend to configure LDAP Authorization, you will need to include an additional LDAP parameter, `ldapgrouplistfilter`. For details, refer to [LDAP authorization](/docs/v25.2/ldap-authorization#configuration).
...
<Note>
As of v25.3, CockroachDB can automatically provision users during their first successful LDAP authentication. Refer to the [latest version](/docs/v26.1/ldap-authentication) of this page.
</Note>
...
SQL usernames must comply with CockroachDB's [username requirements](/docs/v25.2/create-user#user-names). Ensure that the values in the field you are using for `ldapsearchattribute` meet these requirements.
...
2. Produce a `.sql` file with a [`CREATE ROLE`](/docs/v25.2/create-role) statement per user, each on a separate line.
...
3. Run the SQL statements in the [file](/docs/v25.2/cockroach-sql#general):

    ```sql
    cockroach sql --file=create_users.sql --host=<servername> --port=<port> --user=<user> --database=<db> --certs-dir=path/to/certs
    ```

To update users on an ongoing basis, you could script the required [`CREATE ROLE`](/docs/v25.2/create-role), [`DROP ROLE`](/docs/v25.2/drop-role), or [`GRANT`](/docs/v25.2/grant) commands to be [executed](/docs/v25.2/cockroach-sql#general) as needed. For example:
...
Enable [`SESSION` logging](/docs/v25.2/logging#sessions) to preserve data that will help troubleshoot LDAP issues.
...
To view the logs, open `cockroach-session.log` from your [logging directory](/docs/v25.2/configure-logs#logging-directory).
