...
old_description: "Rotate the security certificates of a secure CockroachDB cluster by creating and reloading new certificates."
---

CockroachDB allows you to rotate [public key infrastructure (PKI)security certificates](/docs/v25.2/security-reference/transport-layer-security) without restarting nodes.

<Tip>
For an introduction to how security certificates work in a secure CockroachDB cluster, see [Create Security Certificates](/docs/v25.2/cockroach-cert).
</Tip>
...
For more background, see [Why CockroachDB creates a combined CA certificate](/docs/v25.2/rotate-certificates#why-cockroachdb-creates-a-combined-ca-certificate) and [Why rotate CA certificate in advance](/docs/v25.2/rotate-certificates#why-rotate-ca-certificates-in-advance).
...
2. Create a new CA certificate and key, using the `--overwrite` flag to overwrite the old CA certificate:
    ...
    This results in the [combined CA certificate](/docs/v25.2/rotate-certificates#why-cockroachdb-creates-a-combined-ca-certificate), `ca.crt`, which contains the new certificate followed by the old certificate.
...
3. Upload the new CA certificate to each node:
    ...
    ```
4. Upload the new CA certificate to each client using your preferred method.
5. On each node, reload the CA certificate without restarting the node by issuing a `SIGHUP` signal to the `cockroach` process:
...
6. Reload the CA certificate on each client.
    ...
    This step is application-specific and may require restarting the client.
7. Verify that certificate rotation was successful using the **Local Node Certificates** page in the DB Console: `https://<address of node with new certs>:8080/#/reports/certificates/local`.
...
8. Once you are confident that all nodes and clients have the new CA certificate, [rotate the node certificates](#rotate-node-certificates) and [rotate the client certificates](#rotate-client-certificates).
