...
import { Insert10 } from "/snippets/syntax-diagrams/Insert10.jsx";
import { Insert9 } from "/snippets/syntax-diagrams/Insert9.jsx";

The `INSERT` [statement](/docs/v25.2/sql-statements) inserts one or more rows into a table. In cases where inserted values conflict with uniqueness constraints, the `ON CONFLICT` clause can be used to update rather than insert rows.

## Required privileges

The user must have the `INSERT` [privilege](/docs/v25.2/security-reference/authorization#managing-privileges) on the table. To use `ON CONFLICT`, the user must also have the `SELECT` privilege on the table. To use `ON CONFLICT DO UPDATE`, the user must additionally have the `UPDATE` privilege on the table.
...
As an alternative to `INSERT... ON CONFLICT... DO UPDATE`, you can use the [`UPSERT`](/docs/v25.2/upsert) statement. For example, the following statements are equivalent:
...
- **Existing table**
    - Perform a [multi-row `INSERT`](#insert-multiple-rows-into-an-existing-table) in one statement in an [implicit transaction](/docs/v25.2/transactions#individual-statements).
    - Do not use large batches of 100,000 rows or more, which can lead to long-running transactions that result in [transaction retry errors](/docs/v25.2/transaction-retry-error-reference). If a multi-row `INSERT` results in an error code [`40001` with the message `"transaction deadline exceeded"`](/docs/v25.2/transaction-retry-error-reference#retry_commit_deadline_exceeded), Cockroach Labs recommends that you break up the `INSERT` into smaller batches.
    ...
    - You can also use the [`IMPORT INTO`](/docs/v25.2/import-into) statement to bulk-insert CSV data.
...
To check the [default values](/docs/v25.2/default-value) for columns in a table, use the [`SHOW CREATE TABLE`](/docs/v25.2/show-create) statement:
...
In this example, the `RETURNING` clause returns the `id` values of the rows inserted, which are generated server-side by the `gen_random_uuid()` function. The language-specific versions assume that you have installed the relevant [client drivers](/docs/v25.2/install-client-drivers).
...
If the input data for `INSERT ON CONFLICT` contains duplicate rows, you must use [`DISTINCT ON`](/docs/v25.2/select-clause#eliminate-duplicate-rows) to remove these duplicates.
