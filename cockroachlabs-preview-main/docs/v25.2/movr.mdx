...
- The `movr` dataset, which contains rows of data that populate tables in the `movr` database. The `movr` dataset is built into [`cockroach demo`](/docs/v25.2/cockroach-demo) and [`cockroach workload`](/docs/v25.2/cockroach-workload).
...
[`cockroach demo`](/docs/v25.2/cockroach-demo) opens a SQL shell to a temporary, in-memory cluster. To open a SQL shell to a demo cluster with the `movr` database preloaded and set as the [current database](/docs/v25.2/sql-name-resolution#current-database), use the following command:

```shell
$ cockroach demo movr
```

[`cockroach workload`](/docs/v25.2/cockroach-workload) loads sample datasets and workloads into running clusters. To load the `movr` database and some sample data into a running cluster, do the following:

1. Start a [secure](/docs/v25.2/secure-a-cluster) or [insecure](/docs/v25.2/start-a-local-cluster) local cluster.
...
3. Use [`cockroach sql`](/docs/v25.2/cockroach-sql) to open an interactive SQL shell and set `movr` as the [current database](/docs/v25.2/sql-name-resolution#current-database):
...
2. The user signs up for the service.
    ...
    <Note>
    Usually for [Universally Unique Identifier (UUID)](/docs/v25.2/uuid#create-a-table-with-auto-generated-unique-row-ids) you would need to generate it automatically but for the sake of this follow up we will use predetermined UUID to keep track of them in our examples.
    </Note>
...
For a tutorial on running MovR against a multi-region cluster, using two important multi-region [data topologies](/docs/v25.2/topology-patterns) to get very low latency reads and writes, see [Low Latency, Multi-Region Deployment](/docs/v25.2/demo-low-latency-multi-region-deployment).
...
1. [MovR: A Global Application Use-case](/docs/v25.2/movr-flask-use-case)
2. [Create a Multi-region Database Schema](/docs/v25.2/movr-flask-database)
3. [Set up a Virtual Environment for Developing Global Applications](/docs/v25.2/movr-flask-setup)
4. [Develop a Global Application](/docs/v25.2/movr-flask-application)
5. [Deploy a Global Application](/docs/v25.2/movr-flask-deployment)

## See also

- [Learn CockroachDB SQL](/docs/v25.2/learn-cockroachdb-sql)
- [Build an App with CockroachDB](/docs/v25.2/example-apps)
- [Features in Preview](/docs/v25.2/cockroachdb-feature-availability)
