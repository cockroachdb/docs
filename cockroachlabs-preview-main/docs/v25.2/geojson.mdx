...
GeoJSON can be used to represent the following spatial objects, which also have [Well Known Text (WKT)](/docs/v25.2/well-known-text) and [Well Known Binary (WKB)](/docs/v25.2/well-known-binary) representations:
...
<Note>
GeoJSON should only be used for spatial data that uses the [WGS84](/docs/v25.2/architecture/glossary) geographic spatial reference system. For more information, see [SRID 4326](/docs/v25.2/srid-4326).
</Note>

## Example

In the example below, we will convert a shape represented in [Well Known Text](/docs/v25.2/well-known-text) to GeoJSON using the `ST_AsGeoJSON` [function](/docs/v25.2/functions-and-operators#spatial-functions).
