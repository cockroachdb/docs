...
<Tip>
If your existing cluster was created using the Public operator, refer to the [Public operator migration guide](/docs/v25.2/migrate-cockroachdb-kubernetes-operator).
</Tip>
...
2. Create the `crdbnode` resource that corresponds to the StatefulSet pod you just scaled down. Each manifest is labeled with the pattern `crdbnode-X.yaml`, where `X` corresponds to a StatefulSet pod named `{STS_NAME}-X`. Note the pod that was scaled down and specify its manifest in a command like the following:
    ...
    ```
3. Wait for the new pod to become ready. If it doesn’t, [check the operator logs](/docs/v25.2/monitor-cockroachdb-operator#monitor-the-operator) for errors.
