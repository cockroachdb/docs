...
This page describes how to perform full-text searches using the provided [built-in functions](/docs/v25.2/functions-and-operators#full-text-search-functions).
...
- `LIKE` and `ILIKE` are only fast for prefix searches or when indexed with a [trigram index](/docs/v25.2/trigram-indexes).

<Tip>
  Depending on your use case, you may prefer to use [trigram indexes](/docs/v25.2/trigram-indexes) to do fuzzy string
...
To make a document searchable, convert it to the [`TSVECTOR`](/docs/v25.2/tsvector) data type. A `TSVECTOR` value consists of individual _lexemes_, which are normalized strings used for text matching. Each lexeme also includes a list of integer positions that indicate where the lexeme existed in the original document.

The `to_tsvector()` [built-in function](/docs/v25.2/functions-and-operators#full-text-search-functions) converts a string input into a `TSVECTOR` value:
...
A full-text search attempts to match a _query_ to a document. A full-text search query has the [`TSQUERY`](/docs/v25.2/tsquery) data type. Like `TSVECTOR`, a `TSQUERY` value consists of individual _lexemes_, which are normalized strings used for text matching. Lexemes in a `TSQUERY` are separated with any combination of `&` (AND), `|` (OR), `<->` (FOLLOWED BY), or `!` (NOT) operators.

- The `to_tsquery()` [built-in function](/docs/v25.2/functions-and-operators#full-text-search-functions) normalizes a `TSQUERY` input. The input must also be formatted as a `TSQUERY`, or the statement will error.
...
- The `plainto_tsquery()` [built-in function](/docs/v25.2/functions-and-operators#full-text-search-functions) converts a string input into a `TSQUERY` value, and separates the lexemes with `&` (AND):
...
- The `phraseto_tsquery()` [built-in function](/docs/v25.2/functions-and-operators#full-text-search-functions) converts a string input into a `TSQUERY` value, and separates the lexemes with `<->` (FOLLOWED BY):
...
The `ts_rank()` [built-in function](/docs/v25.2/functions-and-operators#full-text-search-functions) outputs a search rank based on the frequency of matching lexemes. In the following example, two lexemes match:
...
- **matching**: [`@@`](/docs/v25.2/functions-and-operators#operators). This operator is set between a `TSQUERY` and `TSVECTOR`, and returns `true` if the lexemes match. The `TSQUERY` and `TSVECTOR` can be specified in any order.
...
To create a full-text index, use the [`CREATE INDEX`](/docs/v25.2/create-index) syntax that defines an [inverted index](/docs/v25.2/inverted-indexes), specifying a `TSVECTOR` column.
...
You can specify a text search configuration as the first parameter when calling any of the [built-in functions](/docs/v25.2/functions-and-operators#full-text-search-functions) to [process a document](#process-a-document) or [form a query](#form-a-query). For example:
...
If you do not specify a configuration when calling the function, the value of the [`default_text_search_config`](/docs/v25.2/set-vars#default-text-search-config) session variable is used. This defaults to `english` and can be changed as follows:
...
Use the `plainto_tsquery()` [built-in function](/docs/v25.2/functions-and-operators#full-text-search-functions) to match text to a searchable document. This search is equivalent to the preceding example:
...
You can create an [expression index](/docs/v25.2/expression-indexes) on a `STRING` column, using [`to_tsvector()`](#process-a-document) to convert the value to `TSVECTOR`.
...
<Note>
  When using a [full-text search function](/docs/v25.2/functions-and-operators#full-text-search-functions) in an
...
You can create a full-text index on a [stored computed column](/docs/v25.2/computed-columns) that has a `TSVECTOR` data type.
...
<Note>
  When using a [full-text search function](/docs/v25.2/functions-and-operators#full-text-search-functions) in a stored
...
- [`TSVECTOR`](/docs/v25.2/tsvector)
- [`TSQUERY`](/docs/v25.2/tsquery)
- [Inverted indexes](/docs/v25.2/inverted-indexes)
