...
The `TIMESTAMP` and `TIMESTAMPTZ` [data types](/docs/v25.2/data-types) store a date and time pair in UTC.
...
The difference between these two variants is that `TIMESTAMPTZ` uses the client's session [time zone](/docs/v25.2/set-vars#set-time-zone), while the other simply does not. This behavior extends to functions like `now()` and `extract()` on `TIMESTAMPTZ` values.

<Note>
A time zone offset of `+00:00` is displayed for all [`TIME`](/docs/v25.2/time) and `TIMESTAMP` values, but is not stored in the database.
</Note>

You can use the [`timezone()`](/docs/v25.2/functions-and-operators#date-and-time-functions) and [`AT TIME ZONE`](/docs/v25.2/functions-and-operators#special-syntax-forms) functions to convert a `TIMESTAMPTZ` into a `TIMESTAMP` at a specified timezone, or to convert a `TIMESTAMP` into a `TIMESTAMPTZ` at a specified timezone.
...
We recommend always using the `TIMESTAMPTZ` variant because the `TIMESTAMP` variant can sometimes lead to unexpected behaviors when it ignores a session offset. However, we also recommend you avoid [setting a session time zone offset](/docs/v25.2/set-vars#set-time-zone) for your database.
...
You can express a constant value of type `TIMESTAMP`/`TIMESTAMPTZ` using an [interpreted literal](/docs/v25.2/sql-constants#interpreted-literals), or a string literal [annotated with](/docs/v25.2/scalar-expressions#explicitly-typed-expressions) type `TIMESTAMP`/`TIMESTAMPTZ` or [coerced to](/docs/v25.2/scalar-expressions#explicit-type-coercions) type `TIMESTAMP`/`TIMESTAMPTZ`. When it is unambiguous, a simple unannotated [string literal](/docs/v25.2/sql-constants#string-literals) is automatically interpreted as type `TIMESTAMP` or `TIMESTAMPTZ`.
...
By default, CockroachDB interprets truncated dates (e.g., `12-16-06`) as `MM-DD-YY`. To change the input string format of truncated dates, set the `datestyle` [session variable](/docs/v25.2/set-vars) or the `sql.defaults.datestyle` [cluster setting](/docs/v25.2/cluster-settings).
...
You can use an [`ALTER COLUMN... SET DATA TYPE`](/docs/v25.2/alter-table#alter-column) statement to change the precision level of a `TIMESTAMP`/`TIMESTAMPTZ`-typed column. If there is already a non-default precision level specified for the column, the precision level can only be changed to an equal or greater precision level. For an example, see [Create a table with a `TIMESTAMP`-typed column, with precision](#create-a-table-with-a-timestamp-typed-column-with-precision).

## Supported casting and conversion

`TIMESTAMP` values can be [cast](/docs/v25.2/data-types#data-type-conversions-and-casts) to any of the following data types:
...
To change the precision level of a column, you can use an [`ALTER COLUMN... SET DATA TYPE`](/docs/v25.2/alter-table#alter-column) statement:
...
- [`TIME`](/docs/v25.2/time)
- [`INT`](/docs/v25.2/int)
- [`DATE`](/docs/v25.2/date)
