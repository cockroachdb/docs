...
If you manage users through a service compatible with the Lightweight Directory Access Protocol (LDAP), such as Active Directory or Microsoft Entra ID, you can configure CockroachDB to automatically assign [roles](/docs/v25.2/security-reference/authorization) to users based on LDAP group memberships, simplifying access control.
...
- Enable [LDAP authentication](/docs/v25.2/ldap-authentication).
...
Add the `ldapgrouplistfilter` parameter to the HBA configuration that you enabled for [LDAP authentication](/docs/v25.2/ldap-authentication). The configuration will include two important LDAP filters:
...
Create CockroachDB roles that match your LDAP group names and grant appropriate privileges to each role. Remember that role names must comply with CockroachDB's [identifier requirements](/docs/v25.2/create-user#user-names).
...
2. When logged in as an admin to CockroachDB, create the matching test users:
    ...
    ```
3. Log in to CockroachDB as each test user (refer to [Connect to a cluster using LDAP](/docs/v25.2/ldap-authentication)#connect-to-a-cluster-using-ldap).
4. Using your admin credentials, log in to the CockroachDB SQL shell and run `SHOW ROLES;` to view and verify users and their role assignments.
