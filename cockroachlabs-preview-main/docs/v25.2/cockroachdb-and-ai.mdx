...
- Operational workflows encoded as [Agent Skills for CockroachDB](/docs/v25.2/agent-skills).
...
- Access to the Cockroach Labs public documentation via the [CockroachDB Docs MCP server](/docs/v25.2/docs-mcp-integration).

The following sections provide an overview of these tools.

### CockroachDB Cloud MCP server

The [CockroachDB Cloud MCP server](/docs/cockroachcloud/connect-to-the-cockroachdb-cloud-mcp-server) is a managed endpoint in CockroachDB Cloud that exposes a set of tools for inspecting and querying your clusters from your AI tools. These tools let your AI assistants list [databases](/docs/v25.2/show-databases) and [tables](/docs/v25.2/show-tables), describe [schemas](/docs/v25.2/show-schemas) and [indexes](/docs/v25.2/show-index), inspect cluster health and running queries, and run read-only SQL and [`EXPLAIN`](/docs/v25.2/explain) statements. When explicitly enabled, they can also [create databases](/docs/v25.2/create-database), [create tables](/docs/v25.2/create-table), and [insert rows](/docs/v25.2/insert).
...
[Agent Skills for CockroachDB](/docs/v25.2/agent-skills) are small, structured capabilities that encode CockroachDB operational expertise in a machine-executable format. These skills live in the public [cockroachdb-skills](https://github.com/cockroachlabs/cockroachdb-skills) repository and follow the [Agent Skills Specification](https://agentskills.io/specification), with defined inputs, outputs, and safety guardrails.
...
The [CockroachDB Docs MCP server](/docs/v25.2/docs-mcp-integration) exposes the published CockroachDB documentation to your MCP-compatible tools over HTTP. After you add the server configuration to your client, your AI assistant can answer questions using the official documentation without leaving your editor. For example, you could use your AI tool to ask the following:

```text
How do table statistics get refreshed in CockroachDB?
```

An AI tool using the documentation MCP server can provide a response to this prompt informed by documentation pages such as [Cost-Based Optimizer](/docs/v25.2/cost-based-optimizer) and [`CREATE STATISTICS`](/docs/v25.2/create-statistics).
...
CockroachDB has a [`VECTOR` data type](/docs/v25.2/vector) for storing fixed-length floating-point embeddings and supports similarity operators such as L2 distance (`<->`), inner product (`<#>`), and cosine distance (`<=>`). You can index vectors using [`CREATE VECTOR INDEX`](/docs/v25.2/create-index#create-vector-indexes) and combine them with other indexed columns.

You can store vector embeddings, relational data, and [JSON](/docs/v25.2/jsonb) metadata in the same table and query them together. For example:
...
CockroachDB's [transactional model](/docs/v25.2/transactions) provides a foundation for storing agent state, execution history, and coordination metadata. [Serializable isolation](/docs/v25.2/demo-serializable) ensures that state transitions occur correctly even when multiple agents or processes attempt concurrent updates. The database's [high availability](/docs/v25.2/frequently-asked-questions#how-does-cockroachdb-survive-failures) design allows agents to continue operating during node or region failures without requiring external coordination services.
...
- [Multi-region deployments](/docs/v25.2/multiregion-overview) place data closer to users and can enforce [data locality](/docs/v25.2/regional-tables) or residency requirements while maintaining strong consistency.

CockroachDB provides [serializable transactions](/docs/v25.2/demo-serializable) by default. Vector indexes participate in the same transaction and index maintenance model as other [secondary indexes](/docs/v25.2/indexes), so similarity search results remain aligned with the underlying data. This applies to tables storing embeddings, relational records, and agent state.
