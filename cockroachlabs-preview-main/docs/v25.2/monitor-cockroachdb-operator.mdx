...
old_description: "How to monitor a secure CockroachDB cluster deployed with the CockroachDB operator."
---
...
<Note>
If you're on Hosted GKE, before starting, make sure the email address associated with your Google Cloud account is part of the `cluster-admin` RBAC group, as shown in [Deploy CockroachDB with Kubernetes](/docs/v25.2/deploy-cockroachdb-with-cockroachdb-operator).
</Note>
...
6. Access the Prometheus UI locally and verify that CockroachDB is feeding data into Prometheus:
    ...
    3. To verify that each CockroachDB node is connected to Prometheus, go to **Status &gt; Targets**. The screen should look like this:
        ...
        <Frame>
          ![Prometheus targets](/images/v25.2/kubernetes-prometheus-targets.png)
        </Frame>
    4. To verify that data is being collected, go to **Graph**, enter the `sys_uptime` variable in the field, click **Execute**, and then click the **Graph** tab. The screen should like this:
        ...
        <Frame>
          ![Prometheus graph](/images/v25.2/kubernetes-prometheus-graph.png)
        </Frame>

        <Note>
        Prometheus auto-completes CockroachDB time series metrics for you, but if you want to see a full listing, with descriptions, port-forward as described in [Access the DB Console](/docs/v25.2/deploy-cockroachdb-with-cockroachdb-operator#step-4-access-the-db-console) and then point your browser to [http://localhost:8080/_status/vars](http://localhost:8080/_status/vars).
        </Note>
...
5. Verify that Alertmanager is running:
    ...
    2. Go to [http://localhost:9093](http://localhost:9093/) in your browser. The screen should look like this:
        ...
        <Frame>
          ![Alertmanager](/images/v25.2/kubernetes-alertmanager-home.png)
        </Frame>
6. Ensure that the Alertmanagers are visible to Prometheus by opening [http://localhost:9090/status](http://localhost:9090/status). The screen should look like this:
    ...
    <Frame>
      ![Alertmanager](/images/v25.2/kubernetes-prometheus-alertmanagers.png)
    </Frame>
...
8. Ensure that the rules are visible to Prometheus by opening [http://localhost:9090/rules](http://localhost:9090/rules). The screen should look like this:
    ...
    <Frame>
      ![Alertmanager](/images/v25.2/kubernetes-prometheus-alertrules.png)
    </Frame>
9. Verify that the `TestAlertManager` example alert is firing by opening [http://localhost:9090/alerts](http://localhost:9090/alerts). The screen should look like this:
    ...
    <Frame>
      ![Alertmanager](/images/v25.2/kubernetes-prometheus-alerts.png)
    </Frame>
...
You can use the operator to configure the CockroachDB logging system. This allows you to output logs to [configurable log sinks](/docs/v25.2/configure-logs#configure-log-sinks) such as file or network logging destinations.
...
The above configuration overrides the [default logging configuration](/docs/v25.2/configure-logs#default-logging-configuration) and saves debug-level logs (the `DEV` [log channel](/docs/v25.2/logging-overview#logging-channels)) to disk for troubleshooting.

The ConfigMap `name` must match the `cockroachdb.crdbCluster.loggingConfigMapName` object in the values file used to [deploy the cluster](/docs/v25.2/deploy-cockroachdb-with-cockroachdb-operator#initialize-the-cluster):
...
By default, the operator also modifies the [default logging configuration](/docs/v25.2/configure-logs#default-logging-configuration) with the following:
...
In this example, CockroachDB has already been deployed on a Kubernetes cluster. Override the [default logging configuration](/docs/v25.2/configure-logs#default-logging-configuration) to output [DEV](/docs/v25.2/logging#dev) logs to a `cockroach-dev.log` file.
