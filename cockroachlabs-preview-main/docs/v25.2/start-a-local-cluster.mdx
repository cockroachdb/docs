...
<LinkedPageTabs
  currentPath="/docs/v25.2/start-a-local-cluster"
  items={[
    {
      title: "Secure",
      href: "/docs/v25.2/secure-a-cluster",
    },
    {
      title: "Insecure",
      href: "/docs/v25.2/start-a-local-cluster",
    },
  ]}
/>

Once you've [installed CockroachDB](/docs/v25.2/install-cockroachdb), it's simple to run an insecure multi-node cluster locally.
...
- Make sure you have already [installed CockroachDB](/docs/v25.2/install-cockroachdb).
...
The store directory is `cockroach-data/` in the same directory as the `cockroach` command by default, or the location passed to the `--store` flag otherwise. For details about configuring the store location, refer to [cockroach start](/docs/v25.2/cockroach-start#store).
</Warning>

## Step 1. Start the cluster

This section shows how to start a cluster interactively. In production, operators usually use a process manager like `systemd` to start and manage the `cockroach` process on each node. Refer to [Deploy CockroachDB On-Premises](/docs/v25.2/deploy-cockroachdb-on-premises?filters=systemd).

1. Use the [`cockroach start`](/docs/v25.2/cockroach-start) command to start the `node1` in the foreground:
    ...
    *
    * Check out how to secure your cluster: https://www.cockroachlabs.com/docs/v25.2/secure-a-cluster.html
    *
    *
    ...
    *
    ```
2. Take a moment to understand the [flags](/docs/v25.2/cockroach-start#flags) you used:
...
4. Use the [`cockroach init`](/docs/v25.2/cockroach-init) command to perform a one-time initialization of the cluster, sending the request to any node on the `--join` list:
    ...
    At this point, each node also prints helpful [startup details](/docs/v25.2/cockroach-start#standard-output) to its log, and to `STDOUT` in the terminal window where the node was started. For example, the following command retrieves `node1`'s startup details:
...
1. In a new terminal, run the [cockroach sql](/docs/v25.2/cockroach-sql) command and connect to `node1`:
...
2. In the DB Console **Cluster Overview** page, confirm that the cluster now has five nodes.
    ...
    At first, the replica count will be lower for `node4` and `node5`. Very soon, however, you'll see those numbers even out across all nodes, indicating that data is being [automatically rebalanced](/docs/v25.2/demo-replication-and-rebalancing) to utilize the additional capacity of the new nodes.
