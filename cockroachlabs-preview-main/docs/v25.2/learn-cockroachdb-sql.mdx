...
old_description: "Learn some of the most essential CockroachDB SQL statements on a local cluster."
---
...
For a complete list of supported SQL statements and related details, see [SQL Statements](/docs/v25.2/sql-statements).

## Start CockroachDB

If you haven't already, [install CockroachDB](/docs/v25.2/install-cockroachdb).

Then run the [`cockroach demo`](/docs/v25.2/cockroach-demo) command:

```shell
$ cockroach demo
```

This starts a single-node, temporary cluster with the [`movr`](/docs/v25.2/movr) dataset pre-loaded.

## Show tables

To see all tables in the active database, use the [`SHOW TABLES`](/docs/v25.2/show-tables) statement or the `\dt` [shell command](/docs/v25.2/cockroach-sql#commands):
...
Table and column names must follow [these rules](/docs/v25.2/keywords-and-identifiers#identifiers). Also, when you do not explicitly define a [primary key](/docs/v25.2/primary-key), CockroachDB will automatically add a hidden `rowid` column as the primary key.
...
To show all of the columns from a table, use the [`SHOW COLUMNS FROM <table>`](/docs/v25.2/show-columns) statement or the `\d <table>` [shell command](/docs/v25.2/cockroach-sql#commands):
...
To insert a row into a table, use [`INSERT INTO`](/docs/v25.2/insert) followed by the table name and then the column values listed in the order in which the columns appear in the table:
...
[Default values](/docs/v25.2/default-value) are used when you leave specific columns out of your statement, or when you explicitly request default values. For example, both of the following statements create a row where the `name`, `dl`, and `address` entries each contain their default value, in this case `NULL`:
...
[Indexes](/docs/v25.2/indexes) help locate data without having to look through every row of a table. They're automatically created for the [primary key](/docs/v25.2/primary-key) of a table and any columns with a [`UNIQUE` constraint](/docs/v25.2/unique).

To create an index for non-unique columns, use [`CREATE INDEX`](/docs/v25.2/create-index) followed by an optional index name and an `ON` clause identifying the table and column(s) to index. For each column, you can choose whether to sort ascending (`ASC`) or descending (`DESC`).
...
To show the indexes on a table, use [`SHOW INDEX FROM`](/docs/v25.2/show-index) followed by the name of the table:
...
To query a table, use [`SELECT`](/docs/v25.2/select-clause) followed by a comma-separated list of the columns to be returned and the table from which to retrieve the data. You can also use the [`LIMIT`](/docs/v25.2/limit-offset) clause to restrict the number of rows retrieved:
...
To update rows in a table, use [`UPDATE`](/docs/v25.2/update) followed by the table name, a `SET` clause identifying the columns to update and their new values, and a `WHERE` clause identifying the rows to update:
...
To delete rows from a table, use [`DELETE FROM`](/docs/v25.2/delete) followed by the table name and a `WHERE` clause identifying the rows to delete:
...
When you no longer need a table, use [`DROP TABLE`](/docs/v25.2/drop-table) followed by the table name to remove the table and all its data:
...
- Explore all [SQL Statements](/docs/v25.2/sql-statements)
- [Use the built-in SQL client](/docs/v25.2/cockroach-sql) to execute statements from a shell or directly from the command line
- [Install the client driver](/docs/v25.2/install-client-drivers) for your preferred language and [build an app](/docs/v25.2/example-apps)
