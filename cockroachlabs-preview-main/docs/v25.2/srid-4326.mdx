...
Every geometric shape has a spatial reference system associated with it, and each such reference system has a Spatial Reference System ID ([SRID](/docs/v25.2/architecture/glossary#srid)). The SRID is used to tell which spatial reference system will be used to interpret each spatial object.

A common SRID in use is 4326, which represents spatial data using longitude and latitude coordinates on the Earth's surface as defined in the [WGS84](/docs/v25.2/architecture/glossary#wgs84) standard, which is also used for the [Global Positioning System (GPS)](https://wikipedia.org/wiki/Global_Positioning_System).
...
To add an SRID to a shape you are defining with [Well Known Text](/docs/v25.2/well-known-text), prepend the text with `SRID=N;` as shown below:
...
To add an SRID to a shape in SQL, use the `ST_SetSRID` [function](/docs/v25.2/functions-and-operators#spatial-functions):
...
<Warning>
  ...
  [`spatial_ref_sys`](/docs/v25.2/architecture/glossary#spatial_ref_sys) table. For more information, see [Getting SRID
  information](#getting-srid-information).
</Warning>

## Getting SRID information

You can get more detailed information about the SRIDs supported in CockroachDB from the [SQL client](/docs/v25.2/cockroach-sql) by querying the [`spatial_ref_sys`](/docs/v25.2/architecture/glossary#spatial_ref_sys) table.
