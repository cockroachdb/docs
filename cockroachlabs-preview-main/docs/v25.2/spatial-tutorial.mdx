---
title: "Spatial Data"
old_description: "Tutorial for working with spatial data in CockroachDB."
---

In this tutorial, you will plan a vacation from New York City to the [Adirondack Mountains](https://visitadirondacks.com/about) in northern New York State to do some birdwatching while visiting local independent bookstores. In the process, you will explore several of CockroachDB's [spatial capabilities](/docs/v25.2/spatial-data-overview):
...
- Using various [built-in functions](/docs/v25.2/functions-and-operators#spatial-functions) for operating on spatial data.
- Creating [indexes](/docs/v25.2/spatial-indexes) on spatial data.
- Performing [joins](/docs/v25.2/joins) on spatial data, and using [`EXPLAIN`](/docs/v25.2/explain) to make sure indexes are effective.
...
This tutorial can be accomplished in any CockroachDB cluster running [v20.2](/docs/releases/v20.2#v20-2-0) or later.

The simplest way to get up and running is with [`cockroach demo`](/docs/v25.2/cockroach-demo), which starts a temporary, in-memory CockroachDB cluster and opens an interactive SQL shell:

```shell
$ cockroach demo
```

You will see a [SQL client prompt](/docs/v25.2/cockroach-sql) that looks like the one shown below. You will use this prompt for the rest of the tutorial.
...
1. Create a `tutorial` database, and use it.
    ...
    USE tutorial;
    ```
2. In a separate terminal, download the parts of [the data set](#data-set-description) that live in the `tutorial` database:

    ```shell
    curl -o bookstores-and-roads.sql https://spatial-tutorial.s3.us-east-2.amazonaws.com/bookstores-and-roads-20210125.sql
    ```
3. Pipe the data set directly into [cockroach sql](/docs/v25.2/cockroach-sql), specifying the [connection string](/docs/v25.2/connection-parameters#connect-using-a-url) of your local cluster:
...
4. Back in the SQL shell, create a `birds` database, and use it.

    ```sql
    CREATE DATABASE birds;
    USE birds;
    ```
5. In a separate terminal, download the parts of [the data set](#data-set-description) that live in the `birds` database:

    ```shell
    curl -o birds.sql https://spatial-tutorial.s3.us-east-2.amazonaws.com/birds-20210125.sql
    ```
6. Pipe the data set directly into [cockroach sql](/docs/v25.2/cockroach-sql), specifying the [connection string](/docs/v25.2/connection-parameters#connect-using-a-url) of your local cluster:
    ...
    The SQL execution will take some time to complete.
7. Switch back to the `tutorial` database. All of the queries in this tutorial assume you are in the `tutorial` database.
...
Because of the structure of [the `birds` database](#the-birds-database), you will wrap the results of a [subquery](/docs/v25.2/subqueries) against the `birds` database in a [common table expression (CTE)](/docs/v25.2/common-table-expressions) to provide a shorthand name for referring to this data. This step will be necessary every time you want to get information about bird sightings. Therefore, the general pattern for many of these queries will be something like:
...
2. Collect the resulting birdwatcher route geometries ( `routes.geom` ) into one geometry (a [MultiPoint](/docs/v25.2/multipoint) ).
3. Give the resulting table a name, `loon_sightings`, and query against it. In this case the query is rather simple: since the geometries have been collected into one in step 2 above, output the geometry as [GeoJSON](/docs/v25.2/geojson) so the result can be pasted into [https://geojson.io](https://geojson.io/) to generate a map of the sightings.
...
Paste the result above into [https://geojson.io](https://geojson.io/) and you should see the following map, with gray markers for each loon sighting from the bird survey.

<Frame>
  ![Common Loon sightings in the years 2000-2019 in NY state](/images/v25.2/geospatial/tutorial/query-01.png)
</Frame>
...
2. Get the area of the [convex hull](/docs/v25.2/st_convexhull) of the resulting geometry.
3. Because the `birds.routes` data uses [SRID 4326](/docs/v25.2/srid-4326), the resulting area is measured in degrees, which can be converted to square miles by casting the data to a `GEOGRAPHY` type and dividing by 1609 (the number of meters in a mile) squared.
...
The result is an area of about 10,000 square miles, which can be visualized by a box with sides that are a little over 100 miles long. This answer can be verified by looking at the map output from [#1](#1-where-has-the-common-loon-been-sighted-by-the-nabbs-in-the-years-2000-2019-in-ny-state) and noting that most of the observation locations are well within 100 miles of each other.
...
In addition to the [loon observation point locations](#1-where-has-the-common-loon-been-sighted-by-the-nabbs-in-the-years-2000-2019-in-ny-state) and the [area of those observations](#2-what-is-the-total-area-of-loon-sightings) (which you can take as a proxy for the size of the loon's local habitat), you may want to know exactly how many times a loon has been sighted in the area.
...
To determine how many sightings there were in NY state in 2019, re-use the query from [#3](#3-how-many-loon-sightings-have-there-been-in-ny-state-in-the-years-2000-2019), with the additional constraint that the observation year is 2019.
...
2. Query the result table of the CTE from step 1 to divide the number of sightings by the area of the loon's habitat (the convex hull). As in [#2](#2-what-is-the-total-area-of-loon-sightings), do some arithmetic to convert from the unit of degrees returned by [SRID 4326](/docs/v25.2/srid-4326) to square miles.
...
1. Build a CTE that returns the [convex hull](/docs/v25.2/st_convexhull) of Common Loon habitat.
2. Join the results of the above CTE with a query against [the `bookstores` table](#the-bookstores-and-bookstore_routes-tables) that checks whether a bookstore's location is [contained](/docs/v25.2/st_contains) by the loon habitat. Note that the query below [orders by](/docs/v25.2/order-by) the store geometries so that stores in the list are clustered by location. This ordering may be useful if you want to travel between nearby stores. For more information about how this ordering is calculated, see [How CockroachDB's spatial indexing works](/docs/v25.2/spatial-indexes#how-cockroachdbs-spatial-indexing-works).
...
<Note>
The final [`SELECT`](/docs/v25.2/selection-queries) in the query below is doing a join that will not benefit from [spatial indexing](/docs/v25.2/spatial-indexes), since both sides of the join are the results of [CTEs](/docs/v25.2/common-table-expressions), and are therefore not indexed.
</Note>
...
It's great that you [know how many bird species may be near a given bookstore](#7-how-many-different-species-of-bird-habitats-contain-the-location-of-the-book-nook-in-saranac-lake-ny); however, that query didn't tell you which birds you have the best chance of seeing. Therefore, you'd like to figure out what birds are the most commonly seen in the area near a bookstore you want to visit: The Book Nook in Saranac Lake, NY.
...
<Note>
The query below can also be written using an explicit `ST_DWithin`, which is an [index-accelerated function](/docs/v25.2/query-spatial-data#performance). CockroachDB optimizes `ST_Distance(...) < $some_value` to use `ST_DWithin` (see this query's [`EXPLAIN`](/docs/v25.2/explain) output for details).
</Note>
...
You [already discovered which bookstores are located within loon habitat](#6-what-are-the-bookstores-that-lie-within-the-loons-habitat-range-in-ny-state). However, you cannot really tell where these stores are located based on reading that query's output. You need to see them on the map. Therefore, for trip planning purposes, you decide you would like to look at the shape of the convex hull of the store locations.

To answer this question:

1. Build a CTE that returns the [convex hull](/docs/v25.2/st_convexhull) of Common Loon habitat.
2. Join the results of the above CTE with a query against [the `bookstores` table](#the-bookstores-and-bookstore_routes-tables) that checks whether a bookstore's location is [contained](/docs/v25.2/st_contains) by the loon habitat.
3. Collect the geometries that result from the step above into a single geometry, calculate its convex hull, and return the results as [GeoJSON](/docs/v25.2/geojson).
...
Paste the result above into [https://geojson.io](https://geojson.io/) and you should see the following map:

<Frame>
  ![Convex hull of bookstore locations within Common Loon habitat](/images/v25.2/geospatial/tutorial/query-09.png)
</Frame>

### (10) What is the area of the shape of all bookstore locations that are in the Loon's habitat range within NY state?

You have already [visualized the convex hull](#9-what-does-the-shape-of-all-bookstore-locations-that-lie-within-the-loons-habitat-look-like), but now you would like to calculate its area in square miles.

To answer this question:

1. Build a CTE that returns the [convex hull](/docs/v25.2/st_convexhull) of Common Loon habitat.
2. Join the results of the above CTE with a query against [the `bookstores` table](#the-bookstores-and-bookstore_routes-tables) that checks whether a bookstore's location is [contained](/docs/v25.2/st_contains) by the loon habitat.
3. Get the area of the [convex hull](/docs/v25.2/st_convexhull) of the resulting geometry.
4. Collect the geometries that result from the step above into a single geometry, calculate its convex hull, and calculate the area of the hull. As in previous examples, note that because the `birds.routes` data uses [SRID 4326](/docs/v25.2/srid-4326), the resulting area is measured in degrees, which is converted to square miles by casting the data to a `GEOGRAPHY` type and dividing by 1609 (the number of meters in a mile) squared.
...
You have [determined how long the drive between these two stores will be](#11-how-long-is-the-route-from-mysteries-on-main-street-in-johnstown-ny-to-the-book-nook-in-saranac-lake-ny), but you would like to see what it looks like on the map.
...
<Frame>
  ![What does the route from Mysteries on Main Street in Johnstown, NY to The Book Nook in Saranac Lake, NY look like?](/images/v25.2/geospatial/tutorial/query-12.png)
</Frame>
...
Unfortunately, this query is a bit slower than you would like: about 1.5 seconds on a single-node [`cockroach demo`](/docs/v25.2/cockroach-demo) cluster on a laptop. There are several reasons for this:

1. You haven't created any indexes at all yet. The query is likely to be doing full table scans, which you will need to hunt down with [`EXPLAIN`](/docs/v25.2/explain).
...
1. Add an index to the `roads.geom` column. Note that creating the index on `roads.geom` takes about a minute on a single-node [`cockroach demo`](/docs/v25.2/cockroach-demo) cluster, since the table is relatively large.

    ```sql
    CREATE INDEX ON roads USING GIST(geom);
    ```
2. Update the SRID of the `roads.geom` column from 0 to [4326](/docs/v25.2/srid-4326). This will take a few seconds.
...
To see why, look at the [`EXPLAIN`](/docs/v25.2/explain) output:
...
- The [spatial index](/docs/v25.2/spatial-indexes) on `roads.geom` is being used by the join between that column and the loon habitat geometry. This is the biggest reason for improved performance, since [the `roads` table](#the-roads-table) has ~225,000 rows.
...
The answer to your question is that there are about 1,700 miles of roads within the Common Loon's habitat. Further, you can change the `WHERE` clause of the above query slightly to say `AND (NOT (ST_Contains(...)))` to find out how many miles of roads in NY state are *not* within loon habitat (it's about 14,600).
...
2. There is a full table scan on the `bookstores` table. Since the predicate is [`ST_Contains`](/docs/v25.2/st_contains), you probably need to add a [spatial index](/docs/v25.2/spatial-indexes) on `bookstores.geom`.
3. There is a full table scan happening on the `roads` table. This is a serious problem due to the size of that table: ~225,000 rows. You already have an index on the `roads.geom` column for the `ST_DWithin` predicate to use, so you need to find another way to reduce the number of rows scanned. You can do what you did in [query 16](#16-how-many-miles-of-roads-are-contained-within-the-portion-of-the-loon-habitat-that-lies-within-ny-state) and add a check that the `roads.state = 'NY'`, since you are only looking at roads inside New York State.
...
After adding the index, you modify your query as shown below to filter on `roads.state`. This looks only at roads in New York (as in [query 16](#16-how-many-miles-of-roads-are-contained-within-the-portion-of-the-loon-habitat-that-lies-within-ny-state)):
...
If you get tired of looking for Common Loons (and it's understandable that you might), you may want to widen your search to other types of birds. You have already looked at [different bird species based on frequency of observation](#13-what-were-the-25-most-commonly-sighted-birds-in-2019-within-10-miles-of-the-route-between-mysteries-on-main-street-in-johnstown-ny-and-the-bookstore-plus-in-lake-placid-ny). Now you turn your attention to a specific type of bird: hawks.
...
If you are a fan of owls as well as hawks, you can make several updates to [the previous query](#18-what-are-the-hawks-sighted-in-the-same-region-as-the-common-loon), which just looked for hawks. In particular, you want to group the hawks together and the owls together.

To answer this question:

1. Build a CTE that returns the [convex hull](/docs/v25.2/st_convexhull) of Common Loon habitat.
2. Join the results of the above subquery with [the `birds` database](#the-birds-database) to find the names and observation counts of the birds that:
    1. Are in the family "Accipitridae" (hawks) *or* the family "Strigidae" (owls).
    2. Have sighting locations whose geometry is contained by the hull describing the Common Loon's habitat.
...
- Learn more details about the [spatial features](/docs/v25.2/spatial-data-overview) supported by CockroachDB.
- Learn how to migrate spatial data from [shapefiles](/docs/v25.2/migrate-from-shapefiles), [GeoPackages](/docs/v25.2/migrate-from-geopackage), [GeoJSON](/docs/v25.2/migrate-from-geojson), or [OpenStreetMap](/docs/v25.2/migrate-from-openstreetmap) into CockroachDB.
- Learn how CockroachDB's [spatial indexes](/docs/v25.2/spatial-indexes) work.
...
However, you may just want to see what queries are exercising a specific [spatial feature](/docs/v25.2/spatial-data-overview) supported by CockroachDB. The table below provides a mapping from a feature (such as 'spatial indexing' or 'spatial joins') to the queries that use that feature.

<table><thead><tr><th>Feature</th><th>Queries</th><th>Description(s)</th></tr></thead><tbody><tr><td>Spatial data set preparation</td><td>N/A</td><td>For information about how this data set was prepared, see the relevant portions of the <a href="#data-set-description">Data set description</a>.</td></tr><tr><td>Loading spatial data</td><td>N/A</td><td>For loading instructions on loading the data set, see <a href="#step-3-load-the-data-set">Loading the data set</a>.</td></tr><tr><td>Spatial joins</td><td><a href="#6-what-are-the-bookstores-that-lie-within-the-loons-habitat-range-in-ny-state">#6</a>, <a href="#7-how-many-different-species-of-bird-habitats-contain-the-location-of-the-book-nook-in-saranac-lake-ny">#7</a>, <a href="#8-which-25-birds-were-most-often-sighted-within-10-miles-of-the-book-nook-in-saranac-lake-ny-during-the-2000-2019-observation-period">#8</a>, <a href="#9-what-does-the-shape-of-all-bookstore-locations-that-lie-within-the-loons-habitat-look-like">#9</a>, <a href="#10-what-is-the-area-of-the-shape-of-all-bookstore-locations-that-are-in-the-loons-habitat-range-within-ny-state">#10</a>, <a href="#13-what-were-the-25-most-commonly-sighted-birds-in-2019-within-10-miles-of-the-route-between-mysteries-on-main-street-in-johnstown-ny-and-the-bookstore-plus-in-lake-placid-ny">#13</a>, <a href="#14-what-are-the-25-least-often-sighted-other-species-of-birds-in-the-loons-sightings-range">#14</a>, <a href="#15-what-are-the-top-10-roads-nearest-to-a-loon-sighting-location-in-ny">#15</a>, <a href="#16-how-many-miles-of-roads-are-contained-within-the-portion-of-the-loon-habitat-that-lies-within-ny-state">#16</a>, <a href="#17-which-bookstore-in-the-loons-region-in-ny-has-the-fewest-miles-of-roads-within-a-10-mile-radius-of-the-store">#17</a>, <a href="#18-what-are-the-hawks-sighted-in-the-same-region-as-the-common-loon">#18</a>, <a href="#19-what-if-you-also-want-to-look-for-owls-as-well-as-hawks">#19</a></td><td>Joins that involve comparing spatial features from two or more tables, usually with a predicate such as <a href="/docs/v25.2/st_contains"><code>ST&#95;Contains</code></a>.</td></tr><tr><td>Spatial indexing</td><td><a href="#15-what-are-the-top-10-roads-nearest-to-a-loon-sighting-location-in-ny">#15</a>, <a href="#17-which-bookstore-in-the-loons-region-in-ny-has-the-fewest-miles-of-roads-within-a-10-mile-radius-of-the-store">#17</a></td><td>These queries show explicit spatial index creation; many of the spatial join queries listed above could use these indexes, once created.</td></tr><tr><td>Spatial relationships</td><td><a href="#6-what-are-the-bookstores-that-lie-within-the-loons-habitat-range-in-ny-state">#6</a>, <a href="#7-how-many-different-species-of-bird-habitats-contain-the-location-of-the-book-nook-in-saranac-lake-ny">#7</a>, <a href="#8-which-25-birds-were-most-often-sighted-within-10-miles-of-the-book-nook-in-saranac-lake-ny-during-the-2000-2019-observation-period">#8</a>, <a href="#9-what-does-the-shape-of-all-bookstore-locations-that-lie-within-the-loons-habitat-look-like">#9</a>, <a href="#10-what-is-the-area-of-the-shape-of-all-bookstore-locations-that-are-in-the-loons-habitat-range-within-ny-state">#10</a>, <a href="#13-what-were-the-25-most-commonly-sighted-birds-in-2019-within-10-miles-of-the-route-between-mysteries-on-main-street-in-johnstown-ny-and-the-bookstore-plus-in-lake-placid-ny">#13</a>, <a href="#14-what-are-the-25-least-often-sighted-other-species-of-birds-in-the-loons-sightings-range">#14</a>, <a href="#15-what-are-the-top-10-roads-nearest-to-a-loon-sighting-location-in-ny">#15</a>, <a href="#16-how-many-miles-of-roads-are-contained-within-the-portion-of-the-loon-habitat-that-lies-within-ny-state">#16</a>, <a href="#17-which-bookstore-in-the-loons-region-in-ny-has-the-fewest-miles-of-roads-within-a-10-mile-radius-of-the-store">#17</a>, <a href="#18-what-are-the-hawks-sighted-in-the-same-region-as-the-common-loon">#18</a>, <a href="#19-what-if-you-also-want-to-look-for-owls-as-well-as-hawks">#19</a></td><td>Comparing spatial relationships with predicates such as <a href="/docs/v25.2/st_contains"><code>ST&#95;Contains</code></a> and <code>ST&#95;DWithin</code>.</td></tr><tr><td>Nearest-neighbor searching</td><td><a href="#15-what-are-the-top-10-roads-nearest-to-a-loon-sighting-location-in-ny">#15</a></td><td>This query shows how to execute a nearest-neighbor search using CockroachDB.</td></tr></tbody></table>
...
Below is an entity-relationship diagram showing the `bookstores` and `bookstore_routes` tables (generated using [DBeaver](/docs/v25.2/dbeaver)):

<Frame>
  ![tutorial.bookstores and tutorial.bookstore_routes ER diagrams](/images/v25.2/geospatial/tutorial/er-bookstores.png)
</Frame>
...
Similarly, the paths (specifically [MultiLinestrings](/docs/v25.2/multilinestring)) between bookstores in the `bookstore_routes` table were also calculated using a script that made use of a simple greedy algorithm to find paths between the bookstores using the `roads` table (described below). The algorithm always looks for the next road that will bring you closer to the destination. It is fine for this small use case, but it is not robust enough for any kind of production use, since it does not perform any backtracking when it gets stuck in a local optimum. The algorithm works as follows, and assumes a starting bookstore *A* and an ending bookstore *B*:

1. (Start) Find the road nearest *A*; call this road *R* A.
2. Find the road nearest the destination *B*; call this road *R* B.
3. Do *R* A and *R* B intersect? If so, the algorithm is done. You have arrived at your destination. (This is the recursive base case.)
4. Otherwise, find the roads that intersect with *R* A. Of these roads, choose the one that passes closest to *R* B. Call it *R* A'.
5. Do *R* A' and *R* B intersect? If so, the algorithm is done. You have arrived at your destination.
6. If the algorithm has arrived at this step without finishing, it goes back to step 4 and applies that step to the current "closest road to destination" *R* A', and keeps operating steps 4-6 recursively until it has found a complete path or it hits some kind of heuristic timeout.
...
<Frame>
  ![tutorial.roads ER diagrams](/images/v25.2/geospatial/tutorial/er-roads.png)
</Frame>
...
<Note>
The `roads` table was imported from a [shapefile](https://prd-tnm.s3.amazonaws.com/StagedProducts/Small-scale/data/Transportation/roadtrl010g.shp_nt00920.tar.gz) using the method described in [Migrate from Shapefiles](/docs/v25.2/migrate-from-shapefiles).
</Note>

#### The `birds` database

The `birds` database contains several tables that you will have to [join](/docs/v25.2/joins) together in most of the queries shown elsewhere on this page. The multi-table design reflects [the schema of the actual data set](https://www.sciencebase.gov/catalog/file/get/5ea04e9a82cefae35a129d65?f=__disk__b4%2F2f%2Fcf%2Fb42fcfe28a799db6e8c97200829ea1ebaccbf8ea&transform=1&allowOpen=true), which is split across files that map to these tables.
...
- `routes` is a list of ~130 prescribed locations that the birdwatchers helping with the survey visit each year. The `geom` associated with each route is a [Point](/docs/v25.2/point) marking the latitude and longitude of the route's starting point. For details, see the [schema](https://www.sciencebase.gov/catalog/file/get/5ea04e9a82cefae35a129d65?f=__disk__b4%2F2f%2Fcf%2Fb42fcfe28a799db6e8c97200829ea1ebaccbf8ea&transform=1&allowOpen=true) (search for the text "routes.csv").
- `observations` describes the ~85,000 times and places in which birds of various species were actually seen. The `bird_id` is a [foreign key](/docs/v25.2/foreign-key) to the ID in the `birds` table, and the `route_id` points to the ID of the `routes` table.

<Frame>
  ![birds.birds, birds.routes, and birds.observations ER diagrams](/images/v25.2/geospatial/tutorial/er-birds.png)
</Frame>

Each of these tables were populated using a script that parsed [the CSV files available for download](https://www.sciencebase.gov/catalog/item/52b1dfa8e4b0d9b325230cd9) and added the data using [`INSERT`](/docs/v25.2/insert) statements. For the `routes` table, once again the `ST_MakePoint` function was used to create a geometry from the lon/lat values in the CSV as follows:
...
- [Spatial Data Overview](/docs/v25.2/spatial-data-overview)
- [Spatial indexes](/docs/v25.2/spatial-indexes)
- [Spatial &amp; GIS Glossary of Terms](/docs/v25.2/architecture/glossary)
