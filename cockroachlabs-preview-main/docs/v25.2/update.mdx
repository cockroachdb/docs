...
import { Update5 } from "/snippets/syntax-diagrams/Update5.jsx";

The `UPDATE` [statement](/docs/v25.2/sql-statements) updates rows in a table.

<Warning>
  If you update a row that contains a column referenced by a [foreign key constraint](/docs/v25.2/foreign-key) and has
  an [`ON UPDATE` action](/docs/v25.2/foreign-key#foreign-key-actions), all of the dependent rows will also be updated.
</Warning>

## Required privileges

The user must have the `SELECT` and `UPDATE` [privileges](/docs/v25.2/security-reference/authorization#managing-privileges) on the table.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>The name of the column whose values you want to update.</td>
    </tr>
    <tr>
      <td>
        <code>a&#95;expr</code>
      ...
      <td>
        The new value you want to use, the{" "}
        <a href="/docs/v25.2/functions-and-operators#aggregate-functions">aggregate function</a> you want to perform, or
        the <a href="/docs/v25.2/scalar-expressions">scalar expression</a> you want to use.
        <br />
        <br />
        To fill columns with their <a href="/docs/v25.2/default-value">default values</a>, use{" "}
        <code>DEFAULT VALUES</code> in place of <code>a&#95;expr</code>. To fill a specific column with its default
        value, leave the value out of the <code>a&#95;expr</code> or use <code>DEFAULT</code> at the appropriate
        position.
      </td>
    </tr>
    <tr>
      <td>
        <code>FROM table&#95;ref</code>
      </td>
      <td>
        Specify a table to reference, but not update, in <code>UPDATE</code> expressions, or in <code>RETURNING</code>{" "}
        and <code>WHERE</code> clauses. For more details, see{" "}
        <a href="#reference-other-tables-in-an-update">Reference other tables in an update</a>.
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
        ...
        <a href="/docs/v25.2/data-types">data type</a> of its column on the left side of <code>=</code>.
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
        <code>a&#95;expr</code> must be a <a href="/docs/v25.2/scalar-expressions">scalar expression</a> that returns
        Boolean values using columns (e.g., <code>&lt;column&gt; = &lt;value&gt;</code>). Update rows that return{" "}
        <code>TRUE</code>.<br />
        <br />
        <strong>
          Without a <code>WHERE</code> clause in your statement, <code>UPDATE</code> updates all rows in the table.
        </strong>
      </td>
    </tr>
    <tr>
      ...
      <td>
        An <code>ORDER BY</code> clause. See <a href="/docs/v25.2/order-by">Ordering Query Results</a> and{" "}
        <a href="/docs/v25.2/order-by#ordering-rows-in-dml-statements">Ordering of rows in DML statements</a> for more
        details.
      </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>
        Return values based on rows updated, where <code>target&#95;list</code> can be specific column names from the
...
By using the [explicit index annotation](/docs/v25.2/table-expressions#force-index-selection) (also known as "index hinting"), you can override [CockroachDB's index selection](https://www.cockroachlabs.com/blog/index-selection-cockroachdb-2) and use a specific [index](/docs/v25.2/indexes) for updating rows of a named table.
...
To view how the index hint modifies the query plan that CockroachDB follows for updating rows, use an [`EXPLAIN (OPT)`](/docs/v25.2/explain#opt-option) statement. To see all indexes available on a table, use [`SHOW INDEXES`](/docs/v25.2/show-index).
...
For guidance and an example, see [Bulk-update Data](/docs/v25.2/bulk-update-data).
...
<Note>
  If the [`sql_safe_updates`](/docs/v25.2/cockroach-sql#allow-potentially-unsafe-sql-statements) session variable is set
  to `true`, the client will prevent the update. `sql_safe_updates` is set to `true` by default.
</Note>
...
You can use a [`SET`](/docs/v25.2/set-vars) statement to set session variables.
...
In this example, the `RETURNING` clause returns the `id` value of the row updated. The language-specific versions assume that you have installed the relevant [client drivers](/docs/v25.2/install-client-drivers).

<Note>
  When a driver provides a `query()` method for statements that return results and an `exec()` method for statements
  that do not (e.g., Go), it's likely necessary to use the `query()` method for `UPDATE` statements with `RETURNING`.
</Note>
...
pool.connect(function (err, client, done) {
  ...
  async.waterfall(
    [
      function (next) {
        client.query(`UPDATE vehicles SET status = 'available' WHERE city = 'new york' RETURNING id;`, next);
      },
    ],
...
Now suppose you want to update a couple rows in the table, based on their contents. You can use the [`EXPLAIN (OPT)`](/docs/v25.2/explain#opt-option) command to see how the [cost-based optimizer](/docs/v25.2/cost-based-optimizer) decides to perform the `UPDATE` statement:
...
- [Ordering of rows in DML statements](/docs/v25.2/order-by#ordering-rows-in-dml-statements)
