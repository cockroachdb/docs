...
To view this dashboard, [access the DB Console](/docs/v25.2/ui-overview#db-console-access), click **Metrics** in the left-hand navigation, and select **Dashboard** &gt; **Networking**.

For additional information about node connectivity conditions, refer to the [Network page](/docs/v25.2/ui-network-latency-page).
...
Metric: [`sys.host.net.recv.bytes`](/docs/v25.2/essential-metrics-self-hosted#sys-host-net-recv-bytes) Bytes received on all network interfaces since the `cockroach` process started
...
Metric: [`sys.host.net.send.bytes`](/docs/v25.2/essential-metrics-self-hosted#sys-host-net-send-bytes) Bytes sent on all network interfaces since the `cockroach` process started
...
RPC heartbeat latency is the round-trip latency for recent successful outgoing heartbeats. It is the distribution of round-trip latencies with other nodes. This only reflects successful heartbeats and measures [gRPC](/docs/v25.2/architecture/distribution-layer#grpc) overhead as well as possible [head-of-line blocking](https://wikipedia.org/wiki/Head-of-line_blocking). Elevated values in this metric may hint at network issues or saturation or both, but they are not proof of them. [CPU overload](/docs/v25.2/cluster-setup-troubleshooting#cpu-issues) can similarly elevate this metric. To conclusively diagnose network issues, look at OS-level metrics such as packet loss and retransmits. Heartbeats are not very frequent (every 1 second), so they may not capture rare or short-lived degradations.
...
- In the node view, the graph shows the number of proxy attempts each [gateway node](/docs/v25.2/architecture/life-of-a-distributed-transaction#gateway) is initiating.
- In the cluster view, the graph shows the number of proxy attempts each [gateway node](/docs/v25.2/architecture/life-of-a-distributed-transaction#gateway) in the cluster is initiating.

Metric: `distsender.rpc.proxy.sent` Number of attempts by a gateway to proxy a request to an unreachable [leaseholder](/docs/v25.2/architecture/overview#leaseholder) via a [follower replica](/docs/v25.2/architecture/reads-and-writes-overview#write-scenario).

### Monitoring for partial network partition

Operators should alert if the number of proxy requests (`distsender.rpc.proxy.sent`) divided by [batches (`distsender.batches`)](/docs/v25.2/ui-distributed-dashboard#batches) is greater than 1% over a 1-minute window. This indicates that the system likely has a partial network partition. You can verify which nodes are partitioned by navigating to the [**Network** page](/docs/v25.2/ui-network-latency-page) and determining if the `cockroach` process has connectivity issues between nodes. To resolve this issue, you may need to work with your network administrator.

## Proxy request errors

- In the node view, the graph shows the number of proxy attempts by the [gateway node](/docs/v25.2/architecture/life-of-a-distributed-transaction#gateway) that resulted in an error.
- In the cluster view, the graph shows the number of proxy attempts by each [gateway node](/docs/v25.2/architecture/life-of-a-distributed-transaction#gateway) in the cluster that resulted in an error.
...
Metric: `distsender.rpc.proxy.forward.sent` Number of attempts on a [follower replica](/docs/v25.2/architecture/reads-and-writes-overview#write-scenario) to proxy a request to an unreachable [leaseholder](/docs/v25.2/architecture/overview#leaseholder).
...
Metrics: `distsender.rpc.proxy.forward.err` Number of attempts on a [follower replica](/docs/v25.2/architecture/reads-and-writes-overview#write-scenario) to proxy a request that resulted in a failure.
