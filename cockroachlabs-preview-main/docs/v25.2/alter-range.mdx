...
import { AlterRange5 } from "/snippets/syntax-diagrams/AlterRange5.jsx";

The `ALTER RANGE` [statement](/docs/v25.2/sql-statements) applies a [schema change](/docs/v25.2/online-schema-changes) to a range.
...
`ALTER RANGE... CONFIGURE ZONE` is used to add, modify, reset, or remove [replication zones](/docs/v25.2/configure-replication-zones) for a range. To view details about existing replication zones, see [`SHOW ZONE CONFIGURATIONS`](/docs/v25.2/show-zone-configurations).
...
`ALTER RANGE... RELOCATE` is used to move a lease or [replica](/docs/v25.2/architecture/overview#architecture-replica) between [stores](/docs/v25.2/cockroach-start#store). This is helpful in an emergency situation to relocate data in the cluster.

<Warning>
Most users should not need to use this statement; it is for use in emergency situations. If you are in an emergency situation where you think using this statement may help, Cockroach Labs recommends contacting [support](/docs/v25.2/support-resources).
</Warning>
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>LEASE</code></td><td>Move <a href="/docs/v25.2/architecture/replication-layer#leases">leases</a>.</td></tr><tr><td><code>VOTERS</code></td><td>Move <a href="/docs/v25.2/architecture/overview#architecture-replica">voting replicas</a> only.</td></tr><tr><td><code>NONVOTERS</code></td><td>Move <a href="/docs/v25.2/architecture/replication-layer#non-voting-replicas">non-voting replicas</a> only.</td></tr><tr><td><code>FROM a&#95;expr</code></td><td>Move a replica from a store ID.</td></tr><tr><td><code>TO a&#95;expr</code></td><td>Move a lease or replica to a new store ID.</td></tr><tr><td><code>select&#95;stmt</code></td><td>A <a href="/docs/v25.2/selection-queries">selection query</a> that produces one or more rows.</td></tr></tbody></table>
...
To use `ALTER RANGE... RELOCATE`, you need to know how to find the range ID, leaseholder, and other information for a [table](/docs/v25.2/show-ranges#show-ranges-for-a-table), [index](/docs/v25.2/show-ranges#show-ranges-for-an-index), or [database](/docs/v25.2/show-ranges#show-ranges-for-a-database). You can find this information using the [`SHOW RANGES`](/docs/v25.2/show-ranges) statement.

For example, to get all range IDs, leaseholder store IDs, and leaseholder localities for the [`movr.users`](/docs/v25.2/movr) table, use the following query:
...
To move the leases for all data in the [`movr.users`](/docs/v25.2/movr) table to a specific store:
...
To move the replicas for all data in the [`movr.users`](/docs/v25.2/movr) table on one store to another store:
...
To move a range's [non-voting replicas](/docs/v25.2/architecture/replication-layer#non-voting-replicas), use the statement below.

<Note>
This statement will only have an effect on clusters that have non-voting replicas configured, such as [multiregion clusters](/docs/v25.2/multiregion-overview). If your cluster is not a multiregion cluster, it doesn't do anything, and will display errors in the `result` field as shown below.
</Note>
...
- [Multiregion Capabilities Overview](/docs/v25.2/multiregion-overview)
