...
CockroachDB [changefeeds](/docs/v25.2/change-data-capture-overview) are long-running [jobs](/docs/v25.2/show-jobs) that work as a pipeline of change events emitted from a [*watched* table](/docs/v25.2/change-data-capture-overview#watched-table) through to the [downstream sink](/docs/v25.2/changefeed-sinks). The change events from the [storage layer](/docs/v25.2/architecture/storage-layer) of CockroachDB progress through the changefeed pipeline and are encoded into [messages](/docs/v25.2/changefeed-messages) that the changefeed job delivers to a sink, such as Kafka.
...
<Tip>
For details on how changefeeds work as jobs in CockroachDB, refer to the [technical overview](/docs/v25.2/how-does-a-changefeed-work).
</Tip>
...
The changefeed pipeline contains three main sections that start at the [storage layer](/docs/v25.2/architecture/storage-layer) of CockroachDB and end at the [downstream sink](/docs/v25.2/changefeed-sinks) with message delivery. The work completed in each of these pipeline sections feeds metrics that you can track and use to identify where issues could occur.

- [**Rangefeed**](#rangefeed): Connects changefeeds to a long-lived request called a [*rangefeed*](/docs/v25.2/create-and-configure-changefeeds#enable-rangefeeds), which pushes changes as they happen.
- [**Processing**](#processing-aggregation-and-encoding): Prepares the change events from the rangefeed into [changefeed messages](/docs/v25.2/changefeed-messages) by encoding messages into the specified [format](/docs/v25.2/changefeed-messages#message-formats).
- [**Sink**](#sink): Delivers changefeed messages to the [downstream sink](/docs/v25.2/changefeed-sinks).
...
Where noted in the following sections, you can use changefeed [metrics labels](/docs/v25.2/monitor-and-debug-changefeeds#using-changefeed-metrics-labels) to measure metrics per changefeed.

You can [enable the Datadog integration](/docs/v25.2/datadog) on your cluster to collect data and alert on [selected CockroachDB metrics](https://docs.datadoghq.com/integrations/cockroachdb?tab=host#data-collected) using the Datadog platform.
...
- Metrics:
    - `changefeed.max_behind_nanos`
        - Description: The maximum lag in nanoseconds between the timestamp of the most recent [resolved timestamp](/docs/v25.2/changefeed-messages#resolved-timestamp-frequency) emitted by the changefeed and the current time. Indicates how far behind the changefeed is in processing changes.
        - Use with [metrics labels](/docs/v25.2/monitor-and-debug-changefeeds#using-changefeed-metrics-labels).
        - Investigation needed: If `changefeed.max_behind_nanos` is consistently increasing.
    - `(now() - changefeed.checkpoint_progress)`
        - Description: The progress of changefeed [checkpointing](/docs/v25.2/how-does-a-changefeed-work). Indicates how recently the changefeed state was persisted durably. Critical for monitoring changefeed [recovery capability](/docs/v25.2/changefeed-messages#duplicate-messages).
...
- Description: The time between when a [transaction](/docs/v25.2/transactions) commits and when the changefeed emits the corresponding changes. This time includes rangefeed and buffer delays.
- Use with [metrics labels](/docs/v25.2/monitor-and-debug-changefeeds#using-changefeed-metrics-labels).
...
- Description: The age of the [protected timestamp](/docs/v25.2/architecture/storage-layer#protected-timestamps) record for the changefeed.
- Investigation needed: If this metric has high values, it may indicate changefeed processing delays or resource constraints.
- Impact: High values can [block garbage collection](/docs/v25.2/protect-changefeed-data) and lead to increased disk usage and degraded performance for some workloads.

#### Progress tracking

- Metrics:
    - `high_water_timestamp` from [`SHOW CHANGEFEED JOBS`](/docs/v25.2/show-jobs#show-changefeed-jobs)
...
- Impact: Potential bottlenecks in encoding, batching, sending data, or at the [sink](/docs/v25.2/changefeed-sinks). Use this metric in conjunction with the [downstream delivery metrics](#downstream-delivery).
...
<Note>
This metric is supported for [webhook](/docs/v25.2/changefeed-sinks#webhook-sink), [Google Cloud Pub/Sub](/docs/v25.2/changefeed-sinks#google-cloud-pub-sub), and the latest version of the CockroachDB [Kafka](/docs/v25.2/changefeed-sinks#kafka) sink.
</Note>
...
- [The DB Console Overview](/docs/v25.2/ui-overview)
- [`CREATE CHANGEFEED`](/docs/v25.2/create-changefeed)
- [Advanced Changefeed Configuration](/docs/v25.2/advanced-changefeed-configuration)
