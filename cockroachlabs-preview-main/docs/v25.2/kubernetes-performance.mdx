...
Kubernetes provides many useful abstractions for deploying and operating distributed systems, but some of the abstractions come with a performance overhead and an increase in underlying system complexity. This page explains potential bottlenecks to be aware of when [running CockroachDB in Kubernetes](/docs/v25.2/kubernetes-overview) and shows you how to optimize your deployment for better performance.

This page is for Kubernetes deployments that are not using the CockroachDB operator. For guidance specific to the CockroachDB operator, read [CockroachDB Performance on Kubernetes with the CockroachDB operator](/docs/v25.2/cockroachdb-operator-performance).
...
1. Before deploying on Kubernetes, ensure that performance is optimized for your workload on identical hardware. You may find that you first need to [modify your workload](/docs/v25.2/performance-best-practices-overview) or use [different machine specs](/docs/v25.2/recommended-production-settings#hardware) to achieve the performance you need.
2. Go through the documentation for [deploying CockroachDB in a single Kubernetes cluster](/docs/v25.2/deploy-cockroachdb-with-kubernetes) to familiarize yourself with the necessary Kubernetes terminology and deployment abstractions.
...
Your workload is the single most important factor in database performance. Read through our [SQL performance best practices](/docs/v25.2/performance-best-practices-overview) to determine whether there are any easy changes that you can make to speed up your application.

### Machine size

The size of the machines you're using isn't a Kubernetes-specific concern, but it's always a good place to start if you want more performance. See our [hardware recommendations](/docs/v25.2/recommended-production-settings#hardware) for specific suggestions, but using machines with more CPU will almost always allow for greater throughput. Be aware that because Kubernetes runs a set of processes on every machine in a cluster, you typically will get more bang for your buck by using fewer large machines than more small machines.

### Disk type

CockroachDB makes heavy use of the disks you provide it, so using faster disks is an easy way to improve your cluster's performance. Our provided configuration does not specify what type of disks it wants, so in most environments Kubernetes will auto-provision disks of the default type. In the common cloud environments (AWS, GCP, Azure) this means you'll get slow disks that aren't optimized for database workloads (e.g.,HDDs on GCE, SSDs without provisioned IOPS on AWS). However, we [strongly recommend using SSDs](/docs/v25.2/recommended-production-settings#hardware) for the best performance, and Kubernetes makes it relatively easy to use them.
...
For more information, see [Pod scheduling](/docs/v25.2/schedule-cockroachdb-kubernetes).

## See also

- [Orchestrate CockroachDB with Kubernetes](/docs/v25.2/deploy-cockroachdb-with-kubernetes)
...
- [Troubleshooting Performance Issues](/docs/v25.2/query-behavior-troubleshooting)
