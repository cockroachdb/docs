...
- [_Envelope_](/docs/v25.2/changefeed-message-envelopes): the structure of each message. Include or exclude row data, payload schema definitions, and source metadata.
- [_Format_](#message-formats): the available formats, such as JSON, CSV, and Avro.

<Note>
  Changefeed types are not fully integrated with [user-defined composite types](/docs/v25.2/create-type). Running
  changefeeds with user-defined composite types is in
  [Preview](/docs/v25.2/cockroachdb-feature-availability#feature-availability-phases). Certain changefeed types do not
  support user-defined composite types. Refer to the change data capture [Known
  Limitations](/docs/v25.2/create-and-configure-changefeeds#known-limitations) for more detail.
</Note>
...
3. Insert and update values in `employees`:
   ...
   <Note>
     In a [transaction](/docs/v25.2/transactions), if a row is modified more than once in the same transaction, the
...
<Note>
  [Webhook message batching](/docs/v25.2/changefeed-sinks#webhook-sink-configuration) is subject to the same key
...
In some cases, you may want to specify another column in a table as the key by using the [`key_column`](/docs/v25.2/create-changefeed#key-column) option, which will determine the partition your messages will emit to. However, if you implement `key_column` with a changefeed, consider that other columns may have arbitrary values that change. As a result, the same row (i.e., by primary key) may emit to any partition at the sink based upon the column value. A changefeed with a `key_column` specified will still maintain per-key and at-least-once delivery guarantees.

To confirm that messages may emit the same row to different partitions when an arbitrary column is used, you must include the [`unordered`](/docs/v25.2/create-changefeed#unordered) option:
...
When you use the [`enriched` envelope](/docs/v25.2/changefeed-message-envelopes#enriched), if you require timestamps to order messages based on the change event's commit time, then you must specify `envelope=enriched, enriched_properties=source, updated` when you create the changefeed, which will include `"ts_hlc"` and `"ts_ns"` in the [`"source"`](/docs/v25.2/changefeed-message-envelopes#source) field. (It is important to ignore the [`ts_ns` field](/docs/v25.2/changefeed-message-envelopes#ts_ns) at the top level when you're comparing changes for ordering.) For more details on configuring envelope fields, refer to the [Changefeed Message Envelope](/docs/v25.2/changefeed-message-envelopes) page.
...
<Note>
  When changes happen to a column that is part of a composite [key](/docs/v25.2/primary-key), the changefeed will
  produce a [delete message](/docs/v25.2/changefeed-messages#delete-messages) and then an insert message.
</Note>
...
When you create a changefeed with the [`resolved` option](/docs/v25.2/create-changefeed#resolved), the changefeed will emit resolved timestamp messages in a format dependent on the connected [sink](/docs/v25.2/changefeed-sinks). The resolved timestamp is the high-water mark that guarantees that no previously unseen rows with an [earlier update timestamp](#ordering-and-delivery-guarantees) will be emitted to the sink. That is, resolved timestamp messages do not emit until the changefeed job has reached a [checkpoint](/docs/v25.2/how-does-a-changefeed-work).

When you specify the `resolved` option at changefeed creation, the [job's coordinating node](/docs/v25.2/how-does-a-changefeed-work) will send the resolved timestamp to each endpoint at the sink. For example, each [Kafka](/docs/v25.2/changefeed-sinks#kafka) partition will receive a resolved timestamp message, or a [cloud storage sink](/docs/v25.2/changefeed-sinks#cloud-storage-sink) will receive a resolved timestamp file.
...
The changefeed job's coordinating node will emit resolved timestamp messages once the changefeed has reached a checkpoint. The frequency of the checkpoints determine how often the resolved timestamp messages emit to the sink. To configure how often the changefeed checkpoints, you can set the [`min_checkpoint_frequency`](/docs/v25.2/create-changefeed#min-checkpoint-frequency) option and [flush frequency](/docs/v25.2/changefeed-sinks) (if flushing is configurable for your sink).
...
<Frame>
  ...
  potentially sends duplicate messages.](/images/v24.2/changefeed-duplicate-messages-emit.png)
</Frame>

### Changefeed encounters an error

By default, changefeeds treat errors as [retryable except for some specific terminal errors](/docs/v25.2/monitor-and-debug-changefeeds#changefeed-retry-errors). When a changefeed encounters a retryable or non-retryable error, the job will pause until a successful retry or you resume the job once the error is solved. This can cause duplicate messages at the sink as the changefeed returns to the last checkpoint.

We recommend monitoring for changefeed retry errors and failures. Refer to the [Monitor and Debug Changefeeds](/docs/v25.2/monitor-and-debug-changefeeds#recommended-changefeed-metrics-to-track) page.

<Note>
A sink's batching behavior can increase the number of duplicate messages. For example, if Kafka receives a batch of `N` messages and successfully saves `N-1` of them, the changefeed job only knows that the batch failed, not which message failed to commit. As a result, the changefeed job will resend the full batch of messages, which means all but one of the messages are duplicates. For Kafka sinks, reducing the batch size with [`kafka_sink_config`](/docs/v25.2/changefeed-sinks#kafka-sink-configuration) may help to reduce the number of duplicate messages at the sink.

Refer to the [Changefeed Sinks](/docs/v25.2/changefeed-sinks) page for details on sink batching configuration.

</Note>

### Node restarts

When a node restarts, the changefeed will emit duplicates since the last checkpoint. During a rolling restart of nodes, a changefeed can fall behind as it tries to catch up during each node restart. For example, as part of a rolling upgrade or cluster maintenance, a node may [drain](/docs/v25.2/node-shutdown) every 5 minutes and the changefeed job checkpoints every 5 minutes.

To prevent the changefeed from falling too far behind, [pause](/docs/v25.2/create-and-configure-changefeeds#configuring-all-changefeeds) changefeed jobs before performing rolling node restarts.
...
When schema changes with column backfill (e.g., adding a column with a default, adding a [stored computed column](/docs/v25.2/computed-columns), adding a `NOT NULL` column, dropping a column) are made to watched rows, CockroachDB emits a copy of the table using the new schema.
...
Refer to the [`CREATE CHANGEFEED` option table](/docs/v25.2/create-changefeed#schema-change-events) for detail on the `schema_change_policy` option. You can also use the `schema_change_events` option to define the type of schema change event that triggers the behavior specified in `schema_change_policy`.

<Note>
  As of v22.1, changefeeds filter out [`VIRTUAL` computed columns](/docs/v25.2/computed-columns) from events by default.
  This is a [backward-incompatible change](/docs/releases/v22.1#v22-1-0-backward-incompatible-changes). To maintain the
  changefeed behavior in previous versions where [`NULL`](/docs/v25.2/null-handling) values are emitted for virtual
  computed columns, see the [`virtual_columns`](/docs/v25.2/create-changefeed#virtual-columns) option for more detail.
</Note>
...
To prevent changefeeds from emitting messages for any changes (e.g., `INSERT`, `UPDATE`) issued to watched tables during that session, set the `disable_changefeed_replication` [session variable](/docs/v25.2/session-variables) to `true`.
...
For details on syntax and examples, refer to the [Change Data Capture Queries](/docs/v25.2/cdc-queries) page.

### Specify a column as a Kafka header

<InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v25.2: Use the `headers_json_column_name`
option to specify a [JSONB](/docs/v25.2/jsonb) column that the changefeed emits as Kafka headers for each row’s change
event. You can send metadata, such as routing or tracing information, at the protocol level in the header, separate from
the message payload. This allows for Kafka brokers or routers to filter the metadata the header contains without
...
<Note>
  ...
  sinks](/docs/v25.2/changefeed-sinks).
</Note>
...
If you would like to filter the table columns that a changefeed emits, refer to the [CDC Queries](/docs/v25.2/cdc-queries) page. To customize the message envelope, refer to the [Changefeed Message Envelope](/docs/v25.2/changefeed-message-envelopes) page.

## Message formats

By default, changefeeds emit messages in JSON format. You can use a different format by [creating a changefeed](/docs/v25.2/create-changefeed) with the [`format`](/docs/v25.2/create-changefeed#format) option and specifying one of the following:
...
You can use the [`format=csv`](/docs/v25.2/create-changefeed#format) option to emit CSV format messages from your changefeed. However, there are the following limitations with this option:

- It **only** works in combination with the [`initial_scan = 'only'`](/docs/v25.2/create-changefeed#initial-scan) option.
- It does **not** work when used with the [`diff`](/docs/v25.2/create-changefeed#diff) or [`resolved`](/docs/v25.2/create-changefeed#resolved) options.
...
Changefeeds emit the same CSV format as [`EXPORT`](/docs/v25.2/export). In v22.1, changefeeds emitted CSV data that wrapped some values in single quotes, which were not wrapped when exporting data with the `EXPORT` statement.

See [Export Data with Changefeeds](/docs/v25.2/export-data-with-changefeeds) for detail on using changefeeds to export data from CockroachDB.
...
To distinguish between JSON `NULL` values and SQL `NULL` values in changefeed messages, you can use the [`encode_json_value_null_as_object` option](/docs/v25.2/create-changefeed#encode-json-value-null-as-object) with [`CREATE CHANGEFEED`](/docs/v25.2/create-changefeed). When you enable `encode_json_value_null_as_object`, JSON `NULL` values will emit as `{"__crdb_json_null__": true}`.

For example, the following `test` table has a primary key column and a [`JSONB`](/docs/v25.2/jsonb) column. The `INSERT` adds `NULL` values in each column. A changefeed without the option enabled will not distinguish between the SQL and JSON `NULL` values. With the `encode_json_value_null_as_object` option enabled, the changefeed emits the JSON `NULL` as `{"__crdb_json_null__": true}`:
...
When `encode_json_value_null_as_object` is enabled, if the changefeed encounters the literal value `{"__crdb_json_null__": true}` in JSON, it will have the same representation as a JSON `NULL` value and a warning will be printed to the [`DEV` logging channel](/docs/v25.2/logging#dev).
...
- [Create and Configure Changefeeds](/docs/v25.2/create-and-configure-changefeeds)
