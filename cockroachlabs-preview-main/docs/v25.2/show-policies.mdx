...
The `SHOW POLICIES` statement lists the [row-level security (RLS)](/docs/v25.2/row-level-security) policies for a [table](/docs/v25.2/schema-design-table).
...
Use the following query to view all RLS policies. This example uses the schema and policies from the [Row-Level Security Overview](/docs/v25.2/row-level-security).
