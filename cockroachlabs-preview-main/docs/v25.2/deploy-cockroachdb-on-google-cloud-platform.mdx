...
<LinkedPageTabs
  currentPath="/docs/v25.2/deploy-cockroachdb-on-google-cloud-platform"
  items={[
    {
      title: "Secure",
      href: "/docs/v25.2/deploy-cockroachdb-on-google-cloud-platform",
    },
    {
      title: "Insecure",
      href: "/docs/v25.2/deploy-cockroachdb-on-google-cloud-platform-insecure",
    },
  ]}
/>
...
3. [Configure TCP Proxy Load Balancing](https://cloud.google.com/load-balancing/docs/tcp/setting-up-tcp#configure_load_balancer).
    - During backend configuration, create a health check, setting the **Protocol** to `HTTPS`, the **Port** to `8080`, and the **Request path** to path `/health?ready=1`. This [health endpoint](/docs/v25.2/monitoring-and-alerting#health-ready-1) ensures that load balancers do not direct traffic to nodes that are live but not ready to receive requests.
