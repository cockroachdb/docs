...
This page explains how replication zones work and how to use the `ALTER... CONFIGURE ZONE` statement to manage them. `CONFIGURE ZONE` is a subcommand of the [`ALTER DATABASE`](/docs/v26.1/alter-database#configure-zone), [`ALTER TABLE`](/docs/v26.1/alter-table#configure-zone), [`ALTER INDEX`](/docs/v26.1/alter-index#configure-zone), [`ALTER PARTITION`](/docs/v26.1/alter-partition#create-a-replication-zone-for-a-partition), and [`ALTER RANGE`](/docs/v26.1/alter-range#configure-zone) statements.
...
<Note>
To configure replication zones, a user must be a member of the [`admin` role](/docs/v26.1/security-reference/authorization#admin-role) or have been granted [`CREATE`](/docs/v26.1/security-reference/authorization#supported-privileges) or [`ZONECONFIG`](/docs/v26.1/security-reference/authorization#supported-privileges) privileges. To configure [`system` objects](#for-system-data), the user must be a member of the `admin` role.
</Note>
...
Most users should use [multi-region SQL statements](/docs/v26.1/multiregion-overview) instead. If additional control is needed, use [Zone Config Extensions](/docs/v26.1/zone-config-extensions) to augment the multi-region SQL statements.
</Warning>

## Overview

Every [range](/docs/v26.1/architecture/overview#architecture-range) in the cluster is part of a replication zone. Each range's zone configuration is taken into account as ranges are rebalanced across the cluster to ensure that any constraints are honored.
...
1. If there's a replication zone [for the row](#create-a-replication-zone-for-a-partition) (a.k.a. [partition](/docs/v26.1/partitioning) ), CockroachDB uses it.
...
As [discussed previously](#level-priorities), CockroachDB always uses the most granular replication zone available for each schema object (database, table, etc.). More-specific settings applied to schema objects at lower levels of the inheritance tree will always override the settings of objects above them in the tree. All configurations will therefore be modified versions of the [`default` range](/docs/v26.1/configure-replication-zones#view-the-default-replication-zone), which acts as the root of the tree. This means that in practice, **any changes to a specific schema object's zone configurations are by definition user-initiated**, either via [Multi-region SQL abstractions](/docs/v26.1/multiregion-overview) or [manual changes](#why-manual-zone-config-management-is-not-recommended).
...
The "bottom-up" view of how zone config inheritance works is useful when you [are troubleshooting unexpected replication behavior](/docs/v26.1/troubleshoot-replication-zones).
...
<Frame>
  ![zone config inheritance diagram](/images/v26.1/zone-config-inheritance-diagram.png)
</Frame>
...
Start a [demo cluster](/docs/v26.1/cockroach-demo). Create a sample database and table:
...
The following [`SHOW ZONE CONFIGURATION`](/docs/v26.1/show-zone-configurations) statement confirms that the value of the `num_replicas` field on the `kv` table is no longer inherited from its parent database `test`.
...
To return the `test.kv` table to a state where it goes back to inheriting all of its values from its parent database `test`, use the [`ALTER TABLE... CONFIGURE ZONE DISCARD`](/docs/v26.1/alter-table#remove-a-replication-zone) statement:

```sql
ALTER TABLE test.kv CONFIGURE ZONE DISCARD;
```

As you can see from this example, these kinds of manual "tweaks" to zone configurations can leave the cluster in a state where its operators are managing its replication in a very manual and error-prone fashion. This example shows why, for most users, [manual zone config management is not recommended](#why-manual-zone-config-management-is-not-recommended). Instead, most users should use [Multi-region SQL](/docs/v26.1/multiregion-overview).

#### Why manual zone config management is not recommended

Reasons to avoid modifying zone configurations manually using [`CONFIGURE ZONE`](/docs/v26.1/alter-database#configure-zone) include:
...
Given the [previous description of how zone config inheritance works](#how-zone-config-inheritance-works), if you [manually modify zone configs using `CONFIGURE ZONE`](/docs/v26.1/alter-database#configure-zone), you will see the following behavior:
...
- In other words, once you touch any schema object *O* to manually edit some field *F*, a "dirty bit" is set on *O.F*, and CockroachDB will no longer modify *O.F* without user intervention. **Instead, you are now responsible for managing the state of *O.F* via direct calls to [`ALTER DATABASE... CONFIGURE ZONE`](/docs/v26.1/alter-database#configure-zone) anytime something needs to change in your configuration**.

Avoiding this error-prone manual state management was the motivation behind the development of the abstractions provided by [Multi-region SQL statements](/docs/v26.1/multiregion-overview) and [Multi-region Zone Config Extensions](/docs/v26.1/zone-config-extensions).
...
Use the `ALTER... CONFIGURE ZONE` [statement](/docs/v26.1/sql-statements) to set a replication zone:
...
<table><thead><tr><th>Variable</th><th>Description</th></tr></thead><tbody><tr><td><code>range&#95;min&#95;bytes</code></td><td><a></a> The minimum size, in bytes, for a range of data in the zone. When a range is less than this size, CockroachDB will merge it with an adjacent range.<br/><br/><strong>Default:</strong><code>134217728</code> (128 MiB)</td></tr><tr><td><code>range&#95;max&#95;bytes</code></td><td><a></a> The maximum size, in bytes, for a <a href="/docs/v26.1/architecture/glossary#architecture-range">range</a> of data in the zone. When a range reaches this size, CockroachDB will <a href="/docs/v26.1/architecture/distribution-layer#range-splits">split it</a> into two ranges.<br/><br/><strong>Default:</strong><code>536870912</code> (512 MiB)</td></tr><tr><td><code>gc.ttlseconds</code></td><td><a></a> The number of seconds overwritten <a href="/docs/v26.1/architecture/storage-layer#mvcc">MVCC values</a> will be retained before <a href="/docs/v26.1/architecture/storage-layer#garbage-collection">garbage collection</a>.<br/><br/><strong>Default:</strong><code>14400</code> (4 hours)<br/><br/>Smaller values can save disk space and improve performance if values are frequently overwritten or for <a href="/docs/v26.1/understand-hotspots#queueing-hotspot">queue-like workloads</a>. The smallest value we regularly test is <code>600</code> (10 minutes); smaller values are unlikely to be beneficial because of the frequency with which GC runs. If you use <a href="/docs/v26.1/take-full-and-incremental-backups#garbage-collection-and-backups">non-scheduled incremental backups</a>, the GC TTL must be greater than the interval between incremental backups. Otherwise, your incremental backups will fail with <a href="/docs/v26.1/common-errors#protected-ts-verification-error">the error message <code>protected ts verification error</code></a>. To avoid this problem, we recommend using <a href="/docs/v26.1/create-schedule-for-backup">scheduled backups</a> instead, which automatically <a href="/docs/v26.1/create-schedule-for-backup#protected-timestamps-and-scheduled-backups">use protected timestamps</a> to ensure they succeed.<br/><br/>Larger values increase the interval allowed for <a href="/docs/v26.1/as-of-system-time"><code>AS OF SYSTEM TIME</code></a> queries and allow for less frequent incremental backups. The largest value we regularly test is <code>90000</code> (25 hours). Increasing the GC TTL is not meant to be a solution for long-term retention of history; for that you should handle versioning in the <a href="/docs/v26.1/schema-design-overview">schema design at the application layer</a>. Setting the GC TTL too high can cause problems if the retained versions of a single row approach the <a href="#range-max-bytes">maximum range size</a>. This is important because all versions of a row are stored in a single range that never <a href="/docs/v26.1/architecture/distribution-layer#range-splits">splits</a>.</td></tr><tr><td><code>num&#95;replicas</code></td><td><a></a> The number of replicas in the zone, also called the "replication factor".<br/><br/><strong>Default:</strong><code>3</code><br/><br/>For the <code>system</code> database and <code>.meta</code>, <code>.liveness</code>, and <code>.system</code> ranges, the default value is <code>5</code>.<br/><br/>For <a href="/docs/v26.1/multiregion-survival-goals#survive-region-failures">multi-region databases configured to survive region failures</a>, the default value is <code>5</code>; this will include both <a href="#num_voters">voting</a> and <a href="/docs/v26.1/architecture/replication-layer#non-voting-replicas">non-voting replicas</a>.</td></tr><tr><td><code>constraints</code></td><td><a></a> An array of required (<code>+</code>) and/or prohibited (<code>-</code>) constraints influencing the location of replicas. See <a href="/docs/v26.1/configure-replication-zones#types-of-constraints">Types of Constraints</a> and <a href="/docs/v26.1/configure-replication-zones#scope-of-constraints">Scope of Constraints</a> for more details.<br/><br/>To prevent hard-to-detect typos, constraints placed on <a href="/docs/v26.1/configure-replication-zones#descriptive-attributes-assigned-to-nodes">store attributes and node localities</a> must match the values passed to at least one node in the cluster. If not, an error is signalled. To prevent this error, make sure at least one active node is configured to match the constraint. For example, apply <code>constraints = '&#91;+region=west&#93;'</code> only if you had set <code>--locality=region=west</code> for at least one node while starting the cluster.<br/><br/><strong>Default:</strong> No constraints, with CockroachDB locating each replica on a unique node and attempting to spread replicas evenly across localities.</td></tr><tr><td><code>lease&#95;preferences</code><a></a></td><td>An ordered list of required and/or prohibited constraints influencing the location of <a href="/docs/v26.1/architecture/glossary#architecture-leaseholder">leaseholders</a>. Whether each constraint is required or prohibited is expressed with a leading <code>+</code> or <code>-</code>, respectively. Note that lease preference constraints do not have to be shared with the <code>constraints</code> field. For example, it's valid for your configuration to define a <code>lease&#95;preferences</code> field that does not reference any values from the <code>constraints</code> field. It's also valid to define a <code>lease&#95;preferences</code> field with no <code>constraints</code> field at all.<br/><br/>If the first preference cannot be satisfied, CockroachDB will attempt to satisfy the second preference, and so on. If none of the preferences can be met, the lease will be placed using the default lease placement algorithm, which is to base lease placement decisions on how many leases each node already has, trying to make all the nodes have around the same amount.<br/><br/>Each value in the list can include multiple constraints. For example, the list <code>&#91;&#91;+zone=us-east-1b, +ssd&#93;, &#91;+zone=us-east-1a&#93;, &#91;+zone=us-east-1c, +ssd&#93;&#93;</code> means "prefer nodes with an SSD in <code>us-east-1b</code>, then any nodes in <code>us-east-1a</code>, then nodes in <code>us-east-1c</code> with an SSD."<br/><br/>For a usage example, see <a href="/docs/v26.1/configure-replication-zones#constrain-leaseholders-to-specific-availability-zones">Constrain leaseholders to specific availability zones</a>.<br/><br/><strong>Default</strong>: No lease location preferences are applied if this field is not specified.</td></tr><tr><td><code>global&#95;reads</code></td><td><a></a> If <code>true</code>, transactions operating on the range(s) affected by this zone config should be <a href="/docs/v26.1/architecture/transaction-layer#non-blocking-transactions">non-blocking</a>, which slows down writes but allows reads from any replica in the range. Most users will not need to modify this setting; it is applied automatically when you <a href="/docs/v26.1/global-tables">use the <code>GLOBAL</code> table locality in a multi-region cluster</a>.</td></tr><tr><td><code>num&#95;voters</code></td><td><a></a> Specifies the number of <a href="/docs/v26.1/architecture/life-of-a-distributed-transaction#consensus">voting replicas</a>. When set, <code>num&#95;replicas</code> will be the sum of voting and <a href="/docs/v26.1/architecture/replication-layer#non-voting-replicas">non-voting replicas</a>. Most users will not need to modify this setting; it is part of the underlying machinery that enables <a href="/docs/v26.1/multiregion-overview">improved multi-region capabilities in v21.1 and above</a>.</td></tr><tr><td><code>voter&#95;constraints</code></td><td><a></a> Specifies the constraints that govern the placement of voting replicas. This differs from the <code>constraints</code> field, which will govern the placement of all voting and non-voting replicas. Most users will not need to modify this setting; it is part of the underlying machinery that enables <a href="/docs/v26.1/multiregion-overview">improved multi-region capabilities in v21.1 and above</a>.</td></tr></tbody></table>
...
See [Cluster Topology](/docs/v26.1/recommended-production-settings#topology) recommendations for production deployments.
...
You can also use the [`SHOW PARTITIONS`](/docs/v26.1/show-partitions) statement to view the zone constraints on existing table partitions, or [`SHOW CREATE TABLE`](/docs/v26.1/show-create) to view zone configurations for a table.

<Tip>
In testing, scripting, and other programmatic environments, we recommend querying the `crdb_internal.partitions` internal table for partition information instead of using the `SHOW PARTITIONS` statement. For more information, see [Querying partitions programmatically](/docs/v26.1/show-partitions#querying-partitions-programmatically).
</Tip>
...
For more examples, see [`SHOW ZONE CONFIGURATIONS`](/docs/v26.1/show-zone-configurations).
...
For more information, see [`SHOW ZONE CONFIGURATIONS`](/docs/v26.1/show-zone-configurations).
...
For more information, see [`ALTER RANGE... CONFIGURE ZONE`](/docs/v26.1/alter-range#configure-zone).
...
For more information, see [`ALTER DATABASE... CONFIGURE ZONE`](/docs/v26.1/alter-database#configure-zone).
...
For more information, see [`ALTER TABLE... CONFIGURE ZONE`](/docs/v26.1/alter-table#configure-zone).

### Create a replication zone for a secondary index

<Tip>
The [Cost-based Optimizer](/docs/v26.1/cost-based-optimizer) can take advantage of replication zones for secondary indexes when optimizing queries.
</Tip>

The [secondary indexes](/docs/v26.1/indexes) on a table will automatically use the replication zone for the table. You can also add distinct replication zones for secondary indexes.
...
<Tip>
To get the name of a secondary index, which you need for the `CONFIGURE ZONE` statement, use the [`SHOW INDEX`](/docs/v26.1/show-index) or [`SHOW CREATE TABLE`](/docs/v26.1/show-create) statements.
</Tip>
...
For more information, see [`ALTER INDEX... CONFIGURE ZONE`](/docs/v26.1/alter-index#configure-zone).

### Create a replication zone for a partition

Once [partitions have been defined for a table or a secondary index](/docs/v26.1/partitioning), to control replication for a partition, use `ALTER PARTITION <partition> OF INDEX <table@index> CONFIGURE ZONE`:
...
<Tip>
You can also use the [`SHOW CREATE TABLE`](/docs/v26.1/show-create) statement or [`SHOW PARTITIONS`](/docs/v26.1/show-partitions) statements to view details about all of the replication zones defined for the partitions of a table and its secondary indexes.
</Tip>

For more information, see [`ALTER PARTITION... CONFIGURE ZONE`](/docs/v26.1/alter-partition#create-a-replication-zone-for-a-partition).
...
In addition to [constraining replicas to specific availability zones](/docs/v26.1/configure-replication-zones#per-replica-constraints-to-specific-availability-zones), you may also specify preferences for where the range's leaseholders should be placed. This can result in increased performance in some scenarios.

The [`ALTER TABLE... CONFIGURE ZONE`](/docs/v26.1/alter-table#configure-zone) statement below requires that the cluster try to place the ranges' leaseholders in zone `us-east1`; if that is not possible, it will try to place them in zone `us-west1`.

For more information about how the `lease_preferences` field works, see its description in the [Replication zone variables](/docs/v26.1/configure-replication-zones#replication-zone-variables) section.
...
1. Start each node with its availability zone location specified in the [`--locality`](/docs/v26.1/cockroach-start#locality) flag:
...
1. Start each node with its region and availability zone location specified in the [`--locality`](/docs/v26.1/cockroach-start#locality) flag:
    ...
    $ cockroach init --insecure --host=<any node hostname>
    ```
2. On any node, open the [built-in SQL client](/docs/v26.1/cockroach-sql):
...
1. Start each node with a different [locality](/docs/v26.1/cockroach-start#locality) attribute:
...
- [`SHOW ZONE CONFIGURATIONS`](/docs/v26.1/show-zone-configurations)
- [`ALTER DATABASE... CONFIGURE ZONE`](/docs/v26.1/alter-database#configure-zone)
- [`ALTER INDEX... CONFIGURE ZONE`](/docs/v26.1/alter-index#configure-zone)
- [`ALTER RANGE... CONFIGURE ZONE`](/docs/v26.1/alter-range#configure-zone)
- [`ALTER TABLE... CONFIGURE ZONE`](/docs/v26.1/alter-table#configure-zone)
- [`ALTER PARTITION... CONFIGURE ZONE`](/docs/v26.1/alter-partition#create-a-replication-zone-for-a-partition)
- [`SHOW PARTITIONS`](/docs/v26.1/show-partitions)
- [SQL Statements](/docs/v26.1/sql-statements)
- [Table Partitioning](/docs/v26.1/partitioning)
- [Critical nodes endpoint](/docs/v26.1/monitoring-and-alerting#critical-nodes-endpoint)
