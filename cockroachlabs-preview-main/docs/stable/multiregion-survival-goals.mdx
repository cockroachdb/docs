...
You can configure a database to survive zone failures using the [`ALTER DATABASE... SURVIVE ZONE FAILURE` statement](/docs/v26.1/alter-database#survive-zone-region-failure).
...
You can upgrade a database to survive region failures using the [`ALTER DATABASE... SURVIVE REGION FAILURE` statement](/docs/v26.1/alter-database#survive-zone-region-failure).

Setting this goal on a database in a cluster with 3 [cluster regions](/docs/v26.1/multiregion-overview#cluster-regions) will automatically increase the [replication factor](/docs/v26.1/configure-replication-zones#num_replicas) of the [ranges](/docs/v26.1/architecture/glossary#architecture-range) underlying the database from 3 (the default) to 5. This ensures that there will be 5 replicas of each range spread across the 3 regions (2+2+1=5). This is how CockroachDB is able to provide region level resiliency while maintaining good read performance in the leaseholder's region. For writes, CockroachDB will need to coordinate across 2 of the 3 regions, so you will pay additional write latency in exchange for the increased resiliency.

<Note>
To survive region failures, you must add at least 3 [database regions](/docs/v26.1/multiregion-overview#database-regions).
</Note>

## When to use zone vs. region survival goals

Set a [`ZONE` survival goal](/docs/v26.1/multiregion-survival-goals#survive-zone-failures) if:
...
Set a [`REGION` survival goal](/docs/v26.1/multiregion-survival-goals#survive-region-failures) if:
...
- The database can be or already is configured with 3 or more [database regions](/docs/v26.1/multiregion-overview#database-regions). At least three database regions are required to survive region failures.
...
- [Table Localities](/docs/v26.1/table-localities)
