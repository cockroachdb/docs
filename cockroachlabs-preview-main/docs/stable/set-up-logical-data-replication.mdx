...
- [Partial indexes](/docs/v26.1/partial-indexes) and [hash-sharded indexes](/docs/v26.1/hash-sharded-indexes)
- Indexes with a [virtual computed column](/docs/v26.1/computed-columns)
- Composite types in the [primary key](/docs/v26.1/primary-key)
- [Foreign key](/docs/v26.1/foreign-key) dependencies

Additionally, for the `CREATE LOGICALLY REPLICATED` syntax, you cannot use LDR on a table with a schema that contains:

- [User-defined types](/docs/v26.1/enum)

For more details, refer to the LDR [Known limitations](/docs/v26.1/logical-data-replication-overview#known-limitations).

#### Unique secondary indexes

When the destination table includes unique [secondary indexes](/docs/v26.1/schema-design-indexes), it can cause rows to enter the [*dead letter queue* (DLQ)](/docs/v26.1/manage-logical-data-replication). The two clusters in LDR operate independently, so writes to one cluster can conflict with writes to the other.

If the application modifies the same row in both clusters, LDR resolves the conflict using [*last write wins* (LWW)](/docs/v26.1/manage-logical-data-replication#conflict-resolution) conflict resolution. [`UNIQUE` constraints](/docs/v26.1/unique) are validated locally in each cluster, therefore if a replicated write violates a `UNIQUE` constraint on the destination cluster (possibly because a conflicting write was already applied to the row) the replicating row will be applied to the DLQ.
...
*Timestamp 5:* [Range](/docs/v26.1/architecture/glossary#range) containing primary key `1` on the destination cluster is unavailable for a few minutes due to a [network partition](/docs/v26.1/cluster-setup-troubleshooting#network-partition).
...
1. Enter the SQL shell for **both** clusters in separate terminal windows:
    ...
    ```
2. Enable the `kv.rangefeed.enabled` [cluster setting](/docs/v26.1/cluster-settings) on the **source** cluster:
...
4. Choose the appropriate privilege based on the SQL statement the user on the destination cluster will run. (For details on which syntax to use, refer to the [Syntax](#syntax) section at the beginning of this tutorial):
    - [`CREATE LOGICAL REPLICATION STREAM`](/docs/v26.1/create-logical-replication-stream) (replicating into an **existing table**). Grant the [`REPLICATIONDEST` privilege](/docs/v26.1/security-reference/authorization#replicationdest) on the **destination table**, which allows the user to stream data into the existing table:

        ```sql
        GRANT REPLICATIONDEST ON TABLE {your_db}.{your_schema}.{your_table} TO {your_username};
        ```
    - [`CREATE LOGICALLY REPLICATED`](/docs/v26.1/create-logically-replicated) (creating a **new table** as part of the replication). Grant the [`CREATE` privilege](/docs/v26.1/create-database#required-privileges) on the **parent database**, which allows the user to create a new table in the specified database, and the user will automatically have `REPLICATIONDEST` on the table they create:

        ```sql
        GRANT CREATE ON DATABASE {your_db} TO {your_username};
        ```
5. On the **source**, grant the user who will be [specified in the connection string to the source cluster](#step-2-connect-from-the-destination-to-the-source) the [`REPLICATIONSOURCE` privilege](/docs/v26.1/security-reference/authorization#replicationsource):
...
6. (Optional) If you are setting up **bidirectional** LDR, each cluster must authorize both stream directions using the table-level privileges depending on the syntax you're using:
    - [`CREATE LOGICAL REPLICATION STREAM`](/docs/v26.1/create-logical-replication-stream) (setting up a reverse stream manually). Grant `REPLICATIONDEST` and `REPLICATIONSOURCE` to the users in the reverse direction.
    - [`CREATE LOGICALLY REPLICATED`](/docs/v26.1/create-logically-replicated) (setting up a bidirectional stream automatically). Grant the original source user `REPLICATIONDEST` on the tables.
...
To change the password later, refer to [`ALTER USER`](/docs/v26.1/alter-user).

## Step 2. Connect from the destination to the source

In this step, you'll set up [external connection(s)](/docs/v26.1/create-external-connection) to store the connection string for one or both clusters. Depending on how you manage certificates, you must ensure that all nodes between the clusters have access to the certificate of the other cluster.
...
1. On the **source** cluster in a new terminal window, generate a connection string, by passing the replication user, node IP, and port, along with the directory to the source cluster's CA certificate:
    ...
    ```
2. In the SQL shell on the **destination** cluster, create an [external connection](/docs/v26.1/create-external-connection) using the source cluster's connection string. Prefix the `postgresql://` scheme to the connection string and replace `{source}` with your external connection name:
...
1. On cluster **B**, run:
    ...
    {user}:{password}@{node IP}:26257?options=-ccluster%3Dsystem&sslinline=true&sslmode=verify-full&sslrootcert=-----BEGIN+CERTIFICATE-----{encoded certificate}-----END+CERTIFICATE-----%0A
    ```
2. On cluster **A**, create an [external connection](/docs/v26.1/create-external-connection) using cluster B's connection string (source in LDR stream 2). Prefix the `postgresql://` scheme to the connection string and replace `{source}` with your external connection name:
...
Ensure you've created the table on the destination cluster with a matching schema definition to the source cluster table. From the **destination** cluster, start LDR. Use the [fully qualified](/docs/v26.1/sql-name-resolution) table name for the source and destination tables:
...
Once LDR has started, an LDR job will run on the destination cluster. You can [pause](/docs/v26.1/pause-job), [resume](/docs/v26.1/resume-job), or [cancel](/docs/v26.1/cancel-job) the LDR job with the job ID. Use [`SHOW LOGICAL REPLICATION JOBS`](/docs/v26.1/show-logical-replication-jobs) to display the LDR job IDs:
...
You can access the [DB Console](/docs/v26.1/ui-overview) and monitor the status and metrics for the created LDR jobs. Depending on which cluster you would like to view, follow the instructions for either the source or destination.

<Tip>
You can use the [DB Console](/docs/v26.1/ui-overview), the SQL shell, metrics export with Prometheus and Datadog, and [labels with some LDR metrics](/docs/v26.1/multi-dimensional-metrics) to monitor the job.

For a full reference on monitoring LDR, refer to [Logical Data Replication Monitoring](/docs/v26.1/logical-data-replication-monitoring).
</Tip>

1. Access the DB Console at `http://{node IP or hostname}:8080` and enter your user's credentials.
2. On the source cluster, navigate to the [**Jobs** page](/docs/v26.1/ui-jobs-page) to view a list of all jobs. Use the job **Type** dropdown and select **Replication Producer**. This will display the history retention job. This will run while the LDR job is active to protect changes to the table from [garbage collection](/docs/v26.1/architecture/storage-layer#garbage-collection) until they have been replicated to the destination cluster.
...
4. On the destination cluster, click on **Metrics** in the left-hand navigation menu. Use the **Dashboard** dropdown to select [**Logical Data Replication**](/docs/v26.1/ui-logical-data-replication-dashboard). This page shows graphs for monitoring LDR.

## What's next

- [Manage Logical Data Replication](/docs/v26.1/manage-logical-data-replication): Manage the DLQ and schema changes for the replicating tables.
- [`CREATE LOGICAL REPLICATION STREAM`](/docs/v26.1/create-logical-replication-stream)
