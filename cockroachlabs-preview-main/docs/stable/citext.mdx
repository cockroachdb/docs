...
The `CITEXT` [data type](/docs/v26.1/data-types) represents a case-insensitive string. Like `STRING` values, `CITEXT` values preserve their casing when stored and retrieved. Unlike `STRING` values, comparisons between `CITEXT` values are case-insensitive for all [Unicode characters](https://en.wikipedia.org/wiki/List_of_Unicode_characters) that have a defined uppercase/lowercase mapping (e.g., `'É' = 'é'`).
...
<Tip>
`CITEXT` compares values as a `STRING` column with the `und-u-ks-level2` [collation](/docs/v26.1/collate), meaning it is case-insensitive but accent-sensitive.
</Tip>
