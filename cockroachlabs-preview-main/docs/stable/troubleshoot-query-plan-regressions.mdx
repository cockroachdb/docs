...
This page provides guidance on identifying the source of [query plan](/docs/v26.1/cost-based-optimizer) regressions.

For any given SQL statement, if the [cost-based optimizer](/docs/v26.1/cost-based-optimizer) chooses a new query plan that slows performance, you may observe an unexpected increase in query latency. This is considered a query plan regression. There are several reasons that the optimizer might choose a plan that increases execution time. This guide helps you understand, identify, and diagnose query plan regressions using built-in CockroachDB tools.

## Before you begin

- [Understand how the cost-based optimizer chooses query plans](/docs/v26.1/cost-based-optimizer) based on table statistics and how those statistics are refreshed.
...
As a result, these latency spikes can be hard to identify. For example, if the problematic plan only affects a query that's run on an infrequent, ad-hoc basis, it might be difficult to notice a pattern among the graphs on the [**Metrics** page](/docs/v26.1/ui-overview#metrics).
...
1. Open the [DB Console](/docs/v26.1/ui-overview).
2. Go to the [**Metrics** page](/docs/v26.1/ui-overview#metrics).
3. Look at the graphs on the [**Overview Dashboard**](/docs/v26.1/ui-overview-dashboard). By default, the graphs display cluster data from the past hour, but you can adjust this with the time interval selector.
...
5. If you notice an unexplained latency increase, it's worth considering a query plan regression. Hover your cursor on the line in the graph at the beginning of the increase. The box that appears will indicate the time when the increase began.
    ...
    <Frame>
      ![Query Plan Regression metrics graph](/images/v26.1/troubleshooting-query-plan-regressions-1.png)
    </Frame>
...
1. Go to the [**SQL Activity** &gt; **Statements** page](/docs/v26.1/ui-statements-page) in the DB Console.
...
4. Click on the Statement Fingerprint to go to the page that details the statement and its executions.
    ...
    <Tip>
    Look for statements whose **Execution Count** is high. Statements that are run once, such as [`IMPORT INTO`](/docs/v26.1/import-into) statements, are unlikely to cause increased latency due to query plan regressions.
    </Tip>
...
If you suspect that stale statistics caused the plan change and resulting latency increase, consider [manually refreshing the table’s statistics](/docs/v26.1/create-statistics#examples).

You may also want to consider the rare case in which sampling for [histogram](/docs/v26.1/cost-based-optimizer#control-histogram-collection) collection missed important values that would impact planning. You might want to increase the [number of rows sampled for histograms](/docs/v26.1/cluster-settings#setting-sql-stats-histogram-samples-count) before refreshing the table statistics.
...
If you suspect the plan change caused the latency increase and was triggered by a changed query literal, table statistics may not accurately reflect how those values appear in the data. You may want to [manually refresh the statistics for the table](/docs/v26.1/create-statistics#examples). It’s also possible that the current indexes aren’t effective for queries with the new literal value. In that case, [check the **Insights** page for index recommendations](/docs/v26.1/ui-insights-page#suboptimal-plan).

If the issue persists, a more substantial redesign of the schema or application may be required.

#### View all events

1. Go to the [**Metrics** page](/docs/v26.1/ui-overview#metrics).
2. Go to the [**Events** panel](/docs/v26.1/ui-runtime-dashboard#events-panel) on the right. Scroll to the bottom, and click **View All Events**.
3. Scroll down to the approximate time when the latency increase began.

    Check for any events around that time that may have contributed to a query plan regression. These may include schema changes affecting tables in suspect SQL queries, [modified cluster settings](/docs/v26.1/set-cluster-setting), created or dropped indexes, and more.
    ...
    <Tip>
    If your cluster recently underwent a CockroachDB version upgrade, note when that went into effect. An upgrade may have affected default cluster settings or planning heuristics in a way that caused a query plan regression. You may want to [manually refresh the statistics for tables](/docs/v26.1/create-statistics#examples) that are affected by a suspect SQL statement.
    </Tip>

If none of the above methods are conclusive in diagnosing or fixing a query plan regression, consider [activating diagnostics](/docs/v26.1/ui-statements-page#diagnostics) for the suspect statement fingerprints, or [contact support](/docs/v26.1/support-resources).

## See also

- [Cost-Based Optimizer](/docs/v26.1/cost-based-optimizer)
