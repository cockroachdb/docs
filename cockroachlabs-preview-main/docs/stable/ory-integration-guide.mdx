...
old_description: "Learn how to provision a joint Ory and CockroachDB environment."
---

This tutorial demonstrates how to set up a CockroachDB environment that uses [Ory](/docs/v26.1/ory-overview) for Identity and Access Management (IAM). This page describes the architecture of the integration, then walks through how to perform and test it.
...
This example environment integrates [Ory Hydra](/docs/v26.1/ory-overview#ory-hydra), [Ory Kratos](/docs/v26.1/ory-overview#ory-kratos), and [Ory Keto](/docs/v26.1/ory-overview#ory-keto).
...
<Frame>
  ![Single Region MAZ](/images/v26.1/integrate-ory-single-region.svg)
</Frame>
...
In the middle of the diagram (`Ory VPC`): Ory is deployed as a Kubernetes cluster using [Amazon Elastic Kubernetes Service (EKS)](https://aws.amazon.com/eks). A worker [`node`](https://kubernetes.io/docs/concepts/architecture/nodes) is created in each zone, and these nodes are grouped together in a [namespace](https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces) (`ns`). Together they form a single logical cluster. Traffic is routed to each node via an [ingress](https://kubernetes.io/docs/concepts/services-networking/ingress) (`ing`) and a [service](https://kubernetes.io/docs/concepts/services-networking/service) (`svc`). Each Ory component (***Hydra***, ***Kratos***, and ***Keto***) is replicated as a [`pod`](https://kubernetes.io/docs/concepts/workloads/pods) and distributed across the EKS nodes to provide failover capabilities and remain highly available.

At the bottom of the diagram (`CRDB VPC`): The CockroachDB nodes in each zone form a single logical cluster that replicates data across zones using the consensus protocol (typically [Raft](/docs/v26.1/architecture/replication-layer#raft)).

This replication model ensures strong [consistency](/docs/v26.1/architecture/overview#consistency) — all nodes maintain a synchronized and always-on service. Even in the event of zone-level failure, the remaining pods and nodes — for both clusters — ensure that the solution remains available and consistent.
...
Before starting this tutorial, read the [Ory overview](/docs/v26.1/ory-overview).
...
- A configured [AWS CLI profile](https://docs.aws.amazon.com/cli/v1/userguide/cli-configure-files).
- [Terraform](https://developer.hashicorp.com/terraform), [kubectl](https://kubernetes.io/docs/reference/kubectl), [eksctl](https://docs.aws.amazon.com/eks/latest/userguide/getting-started-eksctl), and [Helm (v3+)](https://helm.sh/) installed locally.
...
Learn how to [deploy a CockroachDB cluster locally](/docs/v26.1/secure-a-cluster).
...
Learn how to [deploy a CockroachDB cluster on AWS](/docs/v26.1/deploy-cockroachdb-on-aws).
...
[Sign up for a CockroachDB Cloud account](https://cockroachlabs.cloud/) and [create a cluster](/docs/stable/cockroachcloud/create-your-cluster) using [trial credits](/docs/stable/cockroachcloud/free-trial).
...
- [Ory Hydra](/docs/v26.1/ory-overview#ory-hydra) manages OAuth2 clients, consent sessions, access/refresh tokens
- [Ory Kratos](/docs/v26.1/ory-overview#ory-kratos) handles identity, credentials, sessions, verification tokens
- [Ory Keto](/docs/v26.1/ory-overview#ory-keto) stores relation tuples (RBAC/ABAC data) for permissions
...
1. Go to your [CockroachDB SQL client](/docs/v26.1/cockroach-sql).
2. Replace `{certs-dir}` with the certificates directory that you established during the cluster setup, and `{crdb-fqdn}` with your CockroachDB load balancer domain name. Then run the following command:
    ...
    ```
3. Once connected to the SQL shell, [create the following databases](/docs/v26.1/create-database):
    ...
    ```
4. [Create a user](/docs/v26.1/create-user) and [grant them privileges](/docs/v26.1/grant) for each Ory database:
...
2. From your local workstation, start the Kubernetes cluster:
    ...
    <Tip>
    To ensure that all 3 nodes can be placed into a different AZ, you may want to first [confirm that at least 3 zones are available in the region](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/using-regions-availability-zones#availability-zones-describe) for your account.
    </Tip>
...
To test Ory Hydra, create an OAuth2 client, generate an access token, then introspect it. These steps use the `$HYDRA_ADMIN_URL` and `$HYDRA_PUBLIC_URL` that you exported at the end of the [Ory Hydra deployment](/docs/stable/ory-integration-guide?filters=hydra#step-4-deploy-ory-services-on-kubernetes).
...
To test Ory Kratos, you need to use the Kratos API endpoints to register the API flow, to start the log in flow, and verify the session token. These steps use the `$KRATOS_PUBLIC_URL` that you exported at the end of the [Ory Kratos deployment](/docs/stable/ory-integration-guide?filters=kratos#step-4-deploy-ory-services-on-kubernetes).
...
To test Ory Keto, create relationships between users and objects. Then use Ory Keto commands to check who has access to what objects. These steps use the `$KETO_WRITE_REMOTE` that you exported at the end of the [Ory Keto deployment](/docs/stable/ory-integration-guide?filters=keto#step-4-deploy-ory-services-on-kubernetes).
...
- [Ory Overview](/docs/v26.1/ory-overview)
- [Deploy a Local Cluster from Binary (Secure)](/docs/v26.1/secure-a-cluster)
- [Deploy CockroachDB on AWS EC2](/docs/v26.1/deploy-cockroachdb-on-aws)
