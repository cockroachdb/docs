...
<Note>
If you're on Hosted GKE, before starting, make sure the email address associated with your Google Cloud account is part of the `cluster-admin` RBAC group, as shown in [Deploy CockroachDB with Kubernetes](/docs/v26.1/deploy-cockroachdb-with-cockroachdb-operator).
</Note>
...
6. Access the Prometheus UI locally and verify that CockroachDB is feeding data into Prometheus:
    ...
    4. To verify that data is being collected, go to **Graph**, enter the `sys_uptime` variable in the field, click **Execute**, and then click the **Graph** tab. The screen should like this:
        ...
        <Note>
        Prometheus auto-completes CockroachDB time series metrics for you, but if you want to see a full listing, with descriptions, port-forward as described in [Access the DB Console](/docs/v26.1/deploy-cockroachdb-with-cockroachdb-operator#step-4-access-the-db-console) and then point your browser to [http://localhost:8080/_status/vars](http://localhost:8080/_status/vars).
        </Note>
...
You can use the operator to configure the CockroachDB logging system. This allows you to output logs to [configurable log sinks](/docs/v26.1/configure-logs#configure-log-sinks) such as file or network logging destinations.
...
The above configuration overrides the [default logging configuration](/docs/v26.1/configure-logs#default-logging-configuration) and saves debug-level logs (the `DEV` [log channel](/docs/v26.1/logging-overview#logging-channels)) to disk for troubleshooting.

The ConfigMap `name` must match the `cockroachdb.crdbCluster.loggingConfigMapName` object in the values file used to [deploy the cluster](/docs/v26.1/deploy-cockroachdb-with-cockroachdb-operator#initialize-the-cluster):
...
By default, the operator also modifies the [default logging configuration](/docs/v26.1/configure-logs#default-logging-configuration) with the following:
...
In this example, CockroachDB has already been deployed on a Kubernetes cluster. Override the [default logging configuration](/docs/v26.1/configure-logs#default-logging-configuration) to output [DEV](/docs/v26.1/logging#dev) logs to a `cockroach-dev.log` file.
