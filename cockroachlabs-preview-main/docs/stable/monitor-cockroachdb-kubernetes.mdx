...
Despite CockroachDB's various [built-in safeguards against failure](/docs/v26.1/architecture/replication-layer), it is critical to actively monitor the overall health and performance of a cluster running in production and to create alerting rules that promptly send notifications when there are events that require investigation or intervention.

This page is for Kubernetes deployments that are not using the CockroachDB operator. For guidance specific to the CockroachDB operator, read [Cluster Monitoring with the CockroachDB operator](/docs/v26.1/monitor-cockroachdb-operator).
...
<Note>
If you're on Hosted GKE, before starting, make sure the email address associated with your Google Cloud account is part of the `cluster-admin` RBAC group, as shown in [Deploy CockroachDB with Kubernetes](/docs/v26.1/deploy-cockroachdb-with-kubernetes#hosted-gke).
</Note>
...
4. Download our Prometheus manifest:
    ...
    <Note>
    By default, this manifest uses the secret name generated by the Public operator. If you generated your own certificates and keys when [starting CockroachDB](/docs/v26.1/deploy-cockroachdb-with-kubernetes#step-2-start-cockroachdb), be sure that `ca.secret.name` matches the name of the node secret you created.
    </Note>
...
6. Access the Prometheus UI locally and verify that CockroachDB is feeding data into Prometheus:
    ...
    3. To verify that each CockroachDB node is connected to Prometheus, go to **Status &gt; Targets**. The screen should look like this:
        ...
        <Frame>
          ![Prometheus targets](/images/v26.1/kubernetes-prometheus-targets.png)
        </Frame>
    4. To verify that data is being collected, go to **Graph**, enter the `sys_uptime` variable in the field, click **Execute**, and then click the **Graph** tab. The screen should like this:
        ...
        <Frame>
          ![Prometheus graph](/images/v26.1/kubernetes-prometheus-graph.png)
        </Frame>

        <Tip>
        Prometheus auto-completes CockroachDB time series metrics for you, but if you want to see a full listing, with descriptions, port-forward as described in [Access the DB Console](/docs/v26.1/deploy-cockroachdb-with-kubernetes#step-4-access-the-db-console) and then point your browser to the [Prometheus endpoint](/docs/v26.1/prometheus-endpoint).
...
5. Verify that Alertmanager is running:
    ...
    2. Go to [http://localhost:9093](http://localhost:9093/) in your browser. The screen should look like this:
        ...
        <Frame>
          ![Alertmanager](/images/v26.1/kubernetes-alertmanager-home.png)
        </Frame>
6. Ensure that the Alertmanagers are visible to Prometheus by opening [http://localhost:9090/status](http://localhost:9090/status). The screen should look like this:
    ...
    <Frame>
      ![Alertmanager](/images/v26.1/kubernetes-prometheus-alertmanagers.png)
    </Frame>
...
8. Ensure that the rules are visible to Prometheus by opening [http://localhost:9090/rules](http://localhost:9090/rules). The screen should look like this:
    ...
    <Frame>
      ![Alertmanager](/images/v26.1/kubernetes-prometheus-alertrules.png)
    </Frame>
9. Verify that the `TestAlertManager` example alert is firing by opening [http://localhost:9090/alerts](http://localhost:9090/alerts). The screen should look like this:
    ...
    <Frame>
      ![Alertmanager](/images/v26.1/kubernetes-prometheus-alerts.png)
    </Frame>
...
When running CockroachDB v21.1 and later, you can use the Public operator to configure the CockroachDB logging system. This allows you to output logs to [configurable log sinks](/docs/v26.1/configure-logs#configure-log-sinks) such as file or network logging destinations.
...
The above configuration overrides the [default logging configuration](/docs/v26.1/configure-logs#default-logging-configuration) and reflects our recommended Kubernetes logging configuration:

- Save debug-level logs (the `DEV` [log channel](/docs/v26.1/logging-overview#logging-channels) ) to disk for troubleshooting.
- Send operational- and security-level logs to a [network collector](/docs/v26.1/logging-use-cases#network-logging), in this case [Fluentd](/docs/v26.1/configure-logs#fluentd-logging-format).

The ConfigMap `name` must match the `logConfigMap` object of the Public operator's custom resource, which is used to [deploy the cluster](/docs/v26.1/deploy-cockroachdb-with-kubernetes#initialize-the-cluster):
...
By default, the Public operator also modifies the [default logging configuration](/docs/v26.1/configure-logs#default-logging-configuration) with the following:
...
This outputs logging events in the [`OPS`](/docs/v26.1/logging#ops) channel to a `cockroach-stderr.log` file.

### Example: Creating a troubleshooting log file on pods

In this example, CockroachDB has already been deployed on a Kubernetes cluster. We override the [default logging configuration](/docs/v26.1/configure-logs#default-logging-configuration) to output [`DEV`](/docs/v26.1/logging#dev) logs to a `cockroach-dev.log` file.

1. Create a ConfigMap named `logconfig`. Note that `namespace` is set to the Public operator's default namespace (`cockroach-operator-system`):
    ...
    This configuration outputs `DEV` logs that have severity [`WARNING`](/docs/v26.1/logging#logging-levels-severities) to a `cockroach-dev.log` file on each pod.
2. Apply the ConfigMap to the cluster:
    ...
    ```
3. Add the `name` of the ConfigMap in `logConfigMap` to the [Public operator's custom resource](/docs/v26.1/deploy-cockroachdb-with-kubernetes#initialize-the-cluster):
