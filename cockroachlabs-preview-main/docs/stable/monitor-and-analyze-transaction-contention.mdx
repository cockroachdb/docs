...
old_description: "How to monitor transaction contention using console, metrics, and crdb_internal tables. How to analyze transaction contention using crdb_internal tables."
---

It is crucial to understand where [transaction contention](/docs/v26.1/performance-best-practices-overview#transaction-contention) occurs and how it impacts your workload. Contention is a normal part of database operations. In many cases, it has limited impact on a workload. For example, a large number of contention events with a very short duration may not impact your workload. Similarly, a small number of contention events with longer duration may be acceptable if they occur outside the “user path”. However, if contention substantially contributes to [query latency](/docs/v26.1/ui-sql-dashboard#service-latency-sql-99th-percentile), it should be addressed.

This page shows how to monitor and analyze two types of [contention](/docs/v26.1/performance-best-practices-overview#transaction-contention): [lock contention](/docs/v26.1/troubleshoot-lock-contention) and [serializable conflicts](/docs/v26.1/transaction-retry-error-reference). These types of contention are exposed via different observability touchpoints:
...
The [CockroachDB Cloud Console](/docs/stable/cockroachcloud/cluster-overview-page) and [DB Console](/docs/v26.1/ui-overview) provide monitoring of contention using the following features:
...
On the SQL dashboard within the Metrics page, the **SQL Statement Contention** graph ([CockroachDB Cloud Console](/docs/stable/cockroachcloud/metrics-sql#sql-statement-contention) or [DB Console](/docs/v26.1/ui-sql-dashboard#sql-statement-contention)) shows the rate of contention events per second across the cluster. This graph is based on the [`sql.distsql.contended_queries.count`](/docs/v26.1/essential-metrics-self-hosted#sql-distsql-contended-queries-count) metric, which measures the rate of contention events per second across the cluster.
...
<Frame>
  ![DB Console SQL Statement Contention graph](/images/v26.1/contention-1.png)
</Frame>
...
The **Statements** page ([CockroachDB Cloud Console](/docs/stable/cockroachcloud/statements-page) or [DB Console](/docs/v26.1/ui-statements-page)) shows statements that were involved in contention events. To view these statements, change the search criteria to view the top 100 [statement fingerprints](/docs/v26.1/ui-statements-page#sql-statement-fingerprints) by Contention Time, then sort descending by **Contention Time** column.

The [**Contention Time** column](/docs/v26.1/ui-statements-page#statements-table) column displays the average time and standard deviation of contention events. Note that only lock contention events are included in the contention time.
...
<Frame>
  ![Statements page by Contention Time](/images/v26.1/contention-2.png)
</Frame>

#### Transactions page

The **Transactions** page ([CockroachDB Cloud Console](/docs/stable/cockroachcloud/transactions-page) or [DB Console](/docs/v26.1/ui-transactions-page)) shows transactions that were involved in contention events. To view these transactions, change the search criteria to view the top 100 transaction fingerprints by Contention Time, then sort descending by **Contention Time** column.

The [**Contention Time** column](/docs/v26.1/ui-transactions-page#transactions-table) displays the average and standard deviation of the time spent in contention. Note that only lock contention events are included in the contention time.
...
<Frame>
  ![Transactions page by Contention Time](/images/v26.1/contention-3.png)
</Frame>

### Insights page

The **Insights** page ([CockroachDB Cloud Console](/docs/stable/cockroachcloud/insights-page) or [DB Console](/docs/v26.1/ui-insights-page)) displays a variety of [workload insights](/docs/v26.1/ui-insights-page#workload-insights-tab) on statement and transaction executions related to transaction contention:

- [Slow Execution](/docs/v26.1/ui-insights-page#slow-execution)

    A statement or transaction is classified as Slow Execution if it meets either of the following criteria:

    - The execution took longer than the configured [`sql.insights.latency_threshold`](/docs/v26.1/cluster-settings#setting-sql-insights-latency-threshold).
    - Anomaly detection is enabled ( [`sql.insights.anomaly_detection.enabled`](/docs/v26.1/cluster-settings#setting-sql-insights-anomaly-detection-enabled) ) and the execution was greater than twice the median execution latency and higher than [`sql.insights.anomaly_detection.latency_threshold`](/docs/v26.1/cluster-settings#setting-sql-insights-anomaly-detection-latency-threshold).
- [High Contention](/docs/v26.1/ui-insights-page#high-contention)
...
- [Failed Execution](/docs/v26.1/ui-insights-page#failed-execution)
...
Several [workload insights settings](/docs/v26.1/ui-insights-page#workload-insights-settings) determine how long workload insights are persisted. If clusters are generating a large number of insights, insights may only be persisted for a short period of time. The [`sql.insights.execution_insights_capacity`](/docs/v26.1/cluster-settings#setting-sql-insights-execution-insights-capacity) setting determines the maximum number of insights stored per node, and the [`sql.contention.event_store.capacity`](/docs/v26.1/cluster-settings#setting-sql-contention-event-store-capacity) setting determines the maximum storage used per node for contention events.

## Monitor using metrics

As part of normal operation, CockroachDB continuously records [metrics](/docs/v26.1/essential-metrics-self-hosted#sql) that are often useful in troubleshooting performance. The way metrics are exposed depends on the deployment type. Refer to:

- [Export Metrics From a CockroachDB Standard Cluster](/docs/stable/cockroachcloud/export-metrics)
- [Export Metrics From a CockroachDB Advanced Cluster](/docs/stable/cockroachcloud/export-metrics-advanced)
- [Prometheus endpoint for a self-hosted cluster](/docs/v26.1/prometheus-endpoint)

The following metrics related to contention are available across all deployment types:

<table><thead><tr><th>CockroachDB Metric Name</th><th>Description</th><th>Type</th><th>Unit</th></tr></thead><tbody><tr><td><code>sql.distsql.contended&#95;queries.count</code></td><td>Number of SQL queries that experienced contention<br/><br/>Note: Only lock contention counts are included in this metric.</td><td>COUNTER</td><td>COUNT</td></tr><tr><td><code>sql.distsql.cumulative&#95;contention&#95;nanos</code></td><td>Cumulative contention across all queries (in nanoseconds)<br/><br/>Note: Only lock contention durations are included in this metric.</td><td>COUNTER</td><td>NANOSECOND</td></tr><tr><td><code>txn.restarts</code></td><td>Number of restarted KV transactions</td><td>HISTOGRAM</td><td>COUNT</td></tr><tr><td><code>txn.restarts.asyncwritefailure</code></td><td>Number of restarts due to async consensus writes that failed to leave intents</td><td>COUNTER</td><td>COUNT</td></tr><tr><td><code>txn.restarts.commitdeadlineexceeded</code></td><td>Number of restarts due to a transaction exceeding its deadline</td><td>COUNTER</td><td>COUNT</td></tr><tr><td><code>txn.restarts.readwithinuncertainty</code></td><td>Number of restarts due to reading a new value within the uncertainty interval</td><td>COUNTER</td><td>COUNT</td></tr><tr><td><code>txn.restarts.serializable</code></td><td>Number of restarts due to a forwarded commit timestamp and isolation=SERIALIZABLE</td><td>COUNTER</td><td>COUNT</td></tr><tr><td><code>txn.restarts.txnaborted</code></td><td>Number of restarts due to an abort by a concurrent transaction (usually due to deadlock)</td><td>COUNTER</td><td>COUNT</td></tr><tr><td><code>txn.restarts.txnpush</code></td><td>Number of restarts due to a transaction push failure</td><td>COUNTER</td><td>COUNT</td></tr><tr><td><code>txn.restarts.unknown</code></td><td>Number of restarts due to a unknown reasons</td><td>COUNTER</td><td>COUNT</td></tr><tr><td><code>txn.restarts.writetooold</code></td><td>Number of restarts due to a concurrent writer committing first</td><td>COUNTER</td><td>COUNT</td></tr></tbody></table>

Correlate these metrics with information from the [Insights](/docs/v26.1/ui-insights-page) and [SQL Activity](/docs/v26.1/ui-overview#sql-activity) pages to identify the affected database.

## Monitor using logs

CockroachDB periodically summarizes resolved contention activity as structured log events on the [`SQL_EXEC` logging channel](/docs/v26.1/logging-overview#logging-channels). Each `aggregated_contention_info` [event](/docs/v26.1/eventlog) reports:
...
[Configure log sinks](/docs/v26.1/configure-logs#configure-log-sinks) to route the `SQL_EXEC` channel to a destination, such as a log file or external collector, for long-term analysis.

These periodic reports complement the in-memory [`crdb_internal.transaction_contention_events`](#transaction_contention_events-table) table by providing a durable view of contention history that persists across [node restarts](/docs/v26.1/node-shutdown) or after contention data expires from the event store. Reports are generated at an interval defined by the cluster setting [`sql.contention.event_store.resolution_interval`](#sql-contention-event_store-resolution_interval).

The structured payload makes it easy to ingest the events into log analytics tools and correlate them with statement fingerprints or [key hotspots](/docs/v26.1/understand-hotspots#row-hotspot). A typical structured log entry looks like the following:
...
The [`crdb_internal`](/docs/v26.1/crdb-internal) system catalog is a schema that contains information about internal objects, processes, and metrics related to a specific database. `crdb_internal` tables are read-only.

<Warning>
Not all `crdb_internal` tables are production-ready. Consult the [`crdb_internal`](/docs/v26.1/crdb-internal#tables) page for their current status.
</Warning>

### `transaction_contention_events` table

The [`crdb_internal.transaction_contention_events`](/docs/v26.1/crdb-internal#transaction_contention_events) virtual table contains information about historical transaction contention events. By default, lock contention events and serializable conflicts with a blocking transaction are stored in memory on each node and exposed via SQL in this virtual table.
...
<table><thead><tr><th>Setting</th><th>Type</th><th>Default</th><th>Description</th></tr></thead><tbody><tr><td><code>sql.contention.event&#95;store.capacity</code></td><td>byte size</td><td><code>64 MiB</code></td><td>the in-memory storage capacity per-node of contention event store</td></tr><tr><td><code>sql.contention.event&#95;store.duration&#95;threshold</code></td><td>duration</td><td><code>0s</code></td><td>minimum contention duration to cause the contention events to be collected into crdb&#95;internal.transaction&#95;contention&#95;events. If <code>0</code>, always store.</td></tr><tr><td><a></a><code>sql.contention.event&#95;store.resolution&#95;interval</code></td><td>duration</td><td><code>30s</code></td><td>the interval at which transaction fingerprint ID resolution is performed (set to 0 to disable)</td></tr><tr><td><code>sql.contention.record&#95;serialization&#95;conflicts.enabled</code></td><td>boolean</td><td><code>true</code></td><td>enables recording 40001 errors with conflicting txn meta as SERIALIZATION&#95;CONFLICT contention events into crdb&#95;internal.transaction&#95;contention&#95;events</td></tr></tbody></table>
...
- [`crdb_internal.cluster_contended_indexes`](/docs/v26.1/crdb-internal#cluster_contended_indexes)
- [`crdb_internal.cluster_contended_tables`](/docs/v26.1/crdb-internal#cluster_contended_tables)
- [`crdb_internal.cluster_contended_keys`](/docs/v26.1/crdb-internal#cluster_contended_keys)
...
- [`crdb_internal.cluster_locks`](/docs/v26.1/crdb-internal#cluster_locks)
...
At the cluster level, track how often contention happens compared to overall throughput. For example, divide the rate of contention events ([`sql.distsql.contended_queries.count`](/docs/v26.1/metrics#sql.distsql.contended_queries.count) metric) by the total number of queries ([`sql.query.count`](/docs/v26.1/metrics#sql.query.count) metric) to get the percentage of queries affected by contention. This provides a high-level indicator of how frequently contention occurs but does not identify specific workloads.
...
To analyze contention causes, use [`crdb_internal.transaction_contention_events`](/docs/v26.1/crdb-internal#transaction_contention_events), [`crdb_internal.transaction_statistics`](/docs/v26.1/crdb-internal#transaction_statistics) and [`crdb_internal.statement_statistics`](/docs/v26.1/crdb-internal#statement_statistics) tables.
...
1. In Terminal 1, use the [`cockroach demo`](/docs/v26.1/cockroach-demo) command to start a temporary, in-memory CockroachDB cluster of one node. Once the cluster is started, an interactive SQL shell will connect to that cluster. This shell will be used in the analysis process.

    ```shell
    cockroach demo --no-example-database --insecure
    ```
2. In Terminal 2, use [cockroach workload](/docs/v26.1/cockroach-workload) to load the initial schema on the demo cluster:
    ...
    ```
3. In the interactive SQL shell in Terminal 1, run the SQL queries in this test scenario.
...
- Then, placing the resulting query in a [common table expression (CTE)](/docs/v26.1/common-table-expressions).
...
Review the [DB Console Metrics](/docs/v26.1/ui-overview#metrics) graphs to get a high-level understanding of the contention events. The [SQL Statement Errors](/docs/v26.1/ui-sql-dashboard#sql-statement-errors) graph shows an increase of errors during the time period of 9:16 to 9:23 UTC:

<Frame>
  ![DB Console SQL Statement Errors graph](/images/v26.1/contention-4.png)
</Frame>

The [SQL Statement Contention](/docs/v26.1/ui-sql-dashboard#sql-statement-contention) graph shows a corresponding increase between 9:16 and 9:23 UTC:

<Frame>
  ![DB Console SQL Statement Contention graph](/images/v26.1/contention-5.png)
</Frame>

The [Transaction Restarts](/docs/v26.1/ui-sql-dashboard#transaction-restarts) graph also shows a corresponding increase between 9:16 and 9:23 UTC:

<Frame>
  ![DB Console Transaction Restarts graph](/images/v26.1/contention-6.png)
</Frame>
...
Transactions from row 1 and row 2 executed the same SQL query. Both transactions involved [`INSERT`](/docs/v26.1/insert) and [`UPDATE`](/docs/v26.1/update) operations. Row 1 includes failed executions, while row 2 includes successful executions.
...
While the previous analysis process uses the `crdb_internal` tables, the performance tuning tutorial [Troubleshoot Lock Contention](/docs/v26.1/troubleshoot-lock-contention) gives basic examples to understand lock contention and how to analyze it using the Insights page.

## See Also

- [Transaction contention best practices](/docs/v26.1/performance-best-practices-overview#transaction-contention)
- [Transaction contention performance tuning recipe](/docs/v26.1/performance-recipes#transaction-contention)
- [Troubleshoot Lock Contention](/docs/v26.1/troubleshoot-lock-contention)
- [SQL Dashboard](/docs/v26.1/ui-sql-dashboard) (DB Console) or [SQL metrics](/docs/stable/cockroachcloud/metrics-sql) (Cloud Console)
- Statements page ( [CockroachDB Cloud Console](/docs/stable/cockroachcloud/statements-page) or [DB Console](/docs/v26.1/ui-statements-page) )
- Transactions page ( [CockroachDB Cloud Console](/docs/stable/cockroachcloud/transactions-page) or [DB Console](/docs/v26.1/ui-transactions-page) )
- Insights page ( [CockroachDB Cloud Console](/docs/stable/cockroachcloud/insights-page) or [DB Console](/docs/v26.1/ui-insights-page) )
- [`crdb_internal`](/docs/v26.1/crdb-internal)
