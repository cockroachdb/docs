...
When a [transaction](/docs/v26.1/transactions) is unable to complete due to [contention](/docs/v26.1/performance-best-practices-overview#transaction-contention) with another concurrent or recent transaction attempting to write to the same data, CockroachDB will [automatically attempt to retry the failed transaction](/docs/v26.1/transactions#automatic-retries) without involving the client (i.e., silently). An automatic retry may not always succeed if the transaction is `SERIALIZABLE`. In this case, a [transaction retry error](/docs/v26.1/transaction-retry-error-reference) is emitted to the client.
...
The Python-like pseudocode below shows how to implement an application-level retry loop; it does not require your driver or ORM to implement [advanced retry handling logic](/docs/v26.1/advanced-client-side-transaction-retries), so it can be used from any programming language or environment. In particular, your retry loop must:

- Raise an error if the `max_retries` limit is reached
- Retry on `40001` error codes
- [`COMMIT`](/docs/v26.1/commit-transaction) at the end of the `try` block
...
To test your application's transaction retry logic, use the [`inject_retry_errors_enabled` session variable](/docs/v26.1/set-vars#supported-variables). When `inject_retry_errors_enabled` is set to `true`, any statement (with the exception of [`SET` statements](/docs/v26.1/set-vars)) executed in the session inside of an explicit transaction will return a [transaction retry error](/docs/v26.1/transaction-retry-error-reference) with the message `restart transaction: TransactionRetryWithProtoRefreshError: injected by `inject_retry_errors_enabled` session variable`.

If the client retries the transaction using the special [`cockroach_restart` `SAVEPOINT` name](/docs/v26.1/savepoint#savepoints-for-client-side-transaction-retries), after the 3rd retry, the transaction will proceed as normal. Otherwise, the errors will continue until the client issues a `SET inject_retry_errors_enabled=false` statement.
