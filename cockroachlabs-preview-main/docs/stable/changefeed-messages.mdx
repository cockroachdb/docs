...
- [_Envelope_](/docs/v26.1/changefeed-message-envelopes): the structure of each message. Include or exclude row data, payload schema definitions, and source metadata.
- [_Format_](#message-formats): the available formats, such as JSON, CSV, and Avro.

<Note>
  Changefeed types are not fully integrated with [user-defined composite types](/docs/v26.1/create-type). Running
  changefeeds with user-defined composite types is in
  [Preview](/docs/v26.1/cockroachdb-feature-availability#feature-availability-phases). Certain changefeed types do not
  support user-defined composite types. Refer to the change data capture [Known
  Limitations](/docs/v26.1/create-and-configure-changefeeds#known-limitations) for more detail.
</Note>
...
3. Insert and update values in `employees`:
   ...
   <Note>
     In a [transaction](/docs/v26.1/transactions), if a row is modified more than once in the same transaction, the
...
<Note>
  [Webhook message batching](/docs/v26.1/changefeed-sinks#webhook-sink-configuration) is subject to the same key
...
When you use the [`enriched` envelope](/docs/v26.1/changefeed-message-envelopes#enriched), if you require timestamps to order messages based on the change event's commit time, then you must specify `envelope=enriched, enriched_properties=source, updated` when you create the changefeed, which will include `"ts_hlc"` and `"ts_ns"` in the [`"source"`](/docs/v26.1/changefeed-message-envelopes#source) field. (It is important to ignore the [`ts_ns` field](/docs/v26.1/changefeed-message-envelopes#ts_ns) at the top level when you're comparing changes for ordering.) For more details on configuring envelope fields, refer to the [Changefeed Message Envelope](/docs/v26.1/changefeed-message-envelopes) page.
...
<Note>
  When changes happen to a column that is part of a composite [key](/docs/v26.1/primary-key), the changefeed will
  produce a [delete message](/docs/v26.1/changefeed-messages#delete-messages) and then an insert message.
</Note>
...
When you create a changefeed with the [`resolved` option](/docs/v26.1/create-changefeed#resolved), the changefeed will emit resolved timestamp messages in a format dependent on the connected [sink](/docs/v26.1/changefeed-sinks). The resolved timestamp is the high-water mark that guarantees that no previously unseen rows with an [earlier update timestamp](#ordering-and-delivery-guarantees) will be emitted to the sink. That is, resolved timestamp messages do not emit until the changefeed job has reached a [checkpoint](/docs/v26.1/how-does-a-changefeed-work).

When you specify the `resolved` option at changefeed creation, the [job's coordinating node](/docs/v26.1/how-does-a-changefeed-work) will send the resolved timestamp to each endpoint at the sink. For example, each [Kafka](/docs/v26.1/changefeed-sinks#kafka) partition will receive a resolved timestamp message, or a [cloud storage sink](/docs/v26.1/changefeed-sinks#cloud-storage-sink) will receive a resolved timestamp file.
...
The changefeed job's coordinating node will emit resolved timestamp messages once the changefeed has reached a checkpoint. The frequency of the checkpoints determine how often the resolved timestamp messages emit to the sink. To configure how often the changefeed checkpoints, you can set the [`min_checkpoint_frequency`](/docs/v26.1/create-changefeed#min-checkpoint-frequency) option and [flush frequency](/docs/v26.1/changefeed-sinks) (if flushing is configurable for your sink).
...
<Frame>
  ...
  potentially sends duplicate messages.](/images/v26.1/changefeed-duplicate-messages-emit.png)
</Frame>

### Changefeed encounters an error

By default, changefeeds treat errors as [retryable except for some specific terminal errors](/docs/v26.1/monitor-and-debug-changefeeds#changefeed-retry-errors). When a changefeed encounters a retryable or non-retryable error, the job will pause until a successful retry or you resume the job once the error is solved. This can cause duplicate messages at the sink as the changefeed returns to the last checkpoint.

We recommend monitoring for changefeed retry errors and failures. Refer to the [Monitor and Debug Changefeeds](/docs/v26.1/monitor-and-debug-changefeeds#recommended-changefeed-metrics-to-track) page.

<Note>
A sink's batching behavior can increase the number of duplicate messages. For example, if Kafka receives a batch of `N` messages and successfully saves `N-1` of them, the changefeed job only knows that the batch failed, not which message failed to commit. As a result, the changefeed job will resend the full batch of messages, which means all but one of the messages are duplicates. For Kafka sinks, reducing the batch size with [`kafka_sink_config`](/docs/v26.1/changefeed-sinks#kafka-sink-configuration) may help to reduce the number of duplicate messages at the sink.

Refer to the [Changefeed Sinks](/docs/v26.1/changefeed-sinks) page for details on sink batching configuration.

</Note>

### Node restarts

When a node restarts, the changefeed will emit duplicates since the last checkpoint. During a rolling restart of nodes, a changefeed can fall behind as it tries to catch up during each node restart. For example, as part of a rolling upgrade or cluster maintenance, a node may [drain](/docs/v26.1/node-shutdown) every 5 minutes and the changefeed job checkpoints every 5 minutes.

To prevent the changefeed from falling too far behind, [pause](/docs/v26.1/create-and-configure-changefeeds#configuring-all-changefeeds) changefeed jobs before performing rolling node restarts.
...
When schema changes with column backfill (e.g., adding a column with a default, adding a [stored computed column](/docs/v26.1/computed-columns), adding a `NOT NULL` column, dropping a column) are made to watched rows, CockroachDB emits a copy of the table using the new schema.
...
Refer to the [`CREATE CHANGEFEED` option table](/docs/v26.1/create-changefeed#schema-change-events) for detail on the `schema_change_policy` option. You can also use the `schema_change_events` option to define the type of schema change event that triggers the behavior specified in `schema_change_policy`.

<Note>
  As of v22.1, changefeeds filter out [`VIRTUAL` computed columns](/docs/v26.1/computed-columns) from events by default.
  This is a [backward-incompatible change](/docs/releases/v22.1#v22-1-0-backward-incompatible-changes). To maintain the
  changefeed behavior in previous versions where [`NULL`](/docs/v26.1/null-handling) values are emitted for virtual
  computed columns, see the [`virtual_columns`](/docs/v26.1/create-changefeed#virtual-columns) option for more detail.
</Note>
...
To prevent changefeeds from emitting messages for any changes (e.g., `INSERT`, `UPDATE`) issued to watched tables during that session, set the `disable_changefeed_replication` [session variable](/docs/v26.1/session-variables) to `true`.
...
<Note>
  ...
  sinks](/docs/v26.1/changefeed-sinks).
</Note>
...
If you would like to filter the table columns that a changefeed emits, refer to the [CDC Queries](/docs/v26.1/cdc-queries) page. To customize the message envelope, refer to the [Changefeed Message Envelope](/docs/v26.1/changefeed-message-envelopes) page.
...
The following sections provide information on Avro usage with CockroachDB changefeeds. Creating a changefeed using Avro is available with the [`confluent_schema_registry`](/docs/v26.1/create-changefeed#confluent-schema-registry) option.
...
You can use the [`format=csv`](/docs/v26.1/create-changefeed#format) option to emit CSV format messages from your changefeed. However, there are the following limitations with this option:

- It **only** works in combination with the [`initial_scan = 'only'`](/docs/v26.1/create-changefeed#initial-scan) option.
- It does **not** work when used with the [`diff`](/docs/v26.1/create-changefeed#diff) or [`resolved`](/docs/v26.1/create-changefeed#resolved) options.
- A changefeed emitting [CSV](/docs/v26.1/changefeed-messages#csv) will include `AS` labels in the message format when the changefeed serializes a [user-defined composite type](/docs/v26.1/create-type). [#102905](https://github.com/cockroachdb/cockroach/issues/102905)
...
See [Export Data with Changefeeds](/docs/v26.1/export-data-with-changefeeds) for detail on using changefeeds to export data from CockroachDB.
...
When `encode_json_value_null_as_object` is enabled, if the changefeed encounters the literal value `{"__crdb_json_null__": true}` in JSON, it will have the same representation as a JSON `NULL` value and a warning will be printed to the [`DEV` logging channel](/docs/v26.1/logging#dev).

### Protobuf

You can use the [`format=protobuf`](/docs/v26.1/create-changefeed#format) option to emit [Protocol Buffer (protobuf)](https://protobuf.dev/overview) messages from your changefeed. Protobuf is a binary serialization format that can provide efficient integration with protobuf-native messaging infrastructure, such as Kafka-based streaming systems.
...
- [`headers_json_column_name`](/docs/v26.1/create-changefeed#headers-json-column-name): This option is specific to JSON format and does not work with protobuf messages.

The following changefeed options **are** supported with `format=protobuf`:

- [`diff`](/docs/v26.1/create-changefeed#diff)
- [`key_in_value`](/docs/v26.1/create-changefeed#key-in-value)
- [`mvcc_timestamp`](/docs/v26.1/create-changefeed#mvcc-timestamp)
- [`resolved`](/docs/v26.1/create-changefeed#resolved)
- [`split_column_families`](/docs/v26.1/create-changefeed#split-column-families)
- [`topic_in_value`](/docs/v26.1/create-changefeed#topic-in-value)
- [`updated`](/docs/v26.1/create-changefeed#updated)
- [`virtual_columns`](/docs/v26.1/create-changefeed#virtual-columns)
...
- [Cloud storage](/docs/v26.1/changefeed-sinks#cloud-storage-sink)
...
- [Create and Configure Changefeeds](/docs/v26.1/create-and-configure-changefeeds)
