...
<LinkedPageTabs
  currentPath="/docs/v26.1/build-a-java-app-with-cockroachdb"
  items={[
    {
      title: "Use JDBC",
...
2. `BasicExampleDAO`, which is used by the application to access the data store (in this case CockroachDB). This class also includes a helper function ( `runSql` ) that runs SQL statements inside a transaction, [retrying statements](/docs/v26.1/transactions#transaction-retries) as needed.
...
2. Set the `JDBC_DATABASE_URL` environment variable to a JDBC-compatible connection string:
    1. Use the `cockroach convert-url` command to convert the connection string that you copied earlier to a [valid connection string for JDBC connections](/docs/v26.1/connect-to-the-database?filters=java):
...
[Session variables](/docs/v26.1/session-variables) can be set in the JDBC connection string or as properties of the JDBC data source.

To set the session variable in the JDBC connection string, add them to the [`options` parameter](/docs/v26.1/connection-parameters#supported-options-parameters):
...
You can pass the [`--also-generate-pkcs8-key` flag](/docs/v26.1/cockroach-cert#flag-pkcs8) to [`cockroach cert`](/docs/v26.1/cockroach-cert) to generate a key in [PKCS#8 format](https://tools.ietf.org/html/rfc5208), which is the standard key encoding format in Java. For example, if you have the user `max`:
...
If you are trying to get a large data set into CockroachDB all at once (a bulk import), avoid writing client-side code altogether and use the [`IMPORT INTO`](/docs/v26.1/import-into) statement instead. It is much faster and more efficient than making a series of [`INSERT`s](/docs/v26.1/insert) and [`UPDATE`s](/docs/v26.1/update). It bypasses the [SQL layer](/docs/v26.1/architecture/sql-layer) altogether and writes directly to the [storage layer](/docs/v26.1/architecture/storage-layer) of the database.
...
For guidance on connection pooling, with an example using JDBC and [HikariCP](https://github.com/brettwooldridge/HikariCP), see [Connection Pooling](/docs/v26.1/connection-pooling).
