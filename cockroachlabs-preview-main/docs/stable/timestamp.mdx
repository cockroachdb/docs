...
old_description: "The TIMESTAMP and TIMESTAMPTZ data types stores a date and time pair in UTC."
---

The `TIMESTAMP` and `TIMESTAMPTZ` [data types](/docs/v26.1/data-types) store a date and time pair in UTC.
...
The difference between these two variants is that `TIMESTAMPTZ` uses the client's session [time zone](/docs/v26.1/set-vars#set-time-zone), while the other simply does not. This behavior extends to functions like `now()` and `extract()` on `TIMESTAMPTZ` values.

<Note>
A time zone offset of `+00:00` is displayed for all [`TIME`](/docs/v26.1/time) and `TIMESTAMP` values, but is not stored in the database.
</Note>

You can use the [`timezone()`](/docs/v26.1/functions-and-operators#date-and-time-functions) and [`AT TIME ZONE`](/docs/v26.1/functions-and-operators#special-syntax-forms) functions to convert a `TIMESTAMPTZ` into a `TIMESTAMP` at a specified timezone, or to convert a `TIMESTAMP` into a `TIMESTAMPTZ` at a specified timezone.

Explore the differences of `TIMESTAMP` and `TIMESTAMPTZ` in the following video:

<Frame>
  <iframe allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen frameborder="0" height="315" src="https://www.youtube.com/embed/jyfQmtISr3Y" title="YouTube video player" width="560"></iframe>
</Frame>

## Best practices

We recommend always using the `TIMESTAMPTZ` variant because the `TIMESTAMP` variant can sometimes lead to unexpected behaviors when it ignores a session offset. However, we also recommend you avoid [setting a session time zone offset](/docs/v26.1/set-vars#set-time-zone) for your database.
...
You can express a constant value of type `TIMESTAMP`/`TIMESTAMPTZ` using an [interpreted literal](/docs/v26.1/sql-constants#interpreted-literals), or a string literal [annotated with](/docs/v26.1/scalar-expressions#explicitly-typed-expressions) type `TIMESTAMP`/`TIMESTAMPTZ` or [coerced to](/docs/v26.1/scalar-expressions#explicit-type-coercions) type `TIMESTAMP`/`TIMESTAMPTZ`. When it is unambiguous, a simple unannotated [string literal](/docs/v26.1/sql-constants#string-literals) is automatically interpreted as type `TIMESTAMP` or `TIMESTAMPTZ`.

You can express `TIMESTAMP` constants using the following string literal formats:

<table><thead><tr><th>Format</th><th>Example</th></tr></thead><tbody><tr><td>Date only</td><td><code>TIMESTAMP '2016-01-25'</code></td></tr><tr><td>Date and Time</td><td><code>TIMESTAMP '2016-01-25 10:10:10.555555'</code></td></tr><tr><td>ISO 8601</td><td><code>TIMESTAMP '2016-01-25T10:10:10.555555'</code></td></tr></tbody></table>
...
By default, CockroachDB interprets truncated dates (e.g., `12-16-06`) as `MM-DD-YY`. To change the input string format of truncated dates, set the `datestyle` [session variable](/docs/v26.1/set-vars) or the `sql.defaults.datestyle` [cluster setting](/docs/v26.1/cluster-settings).
...
You can use an [`ALTER COLUMN... SET DATA TYPE`](/docs/v26.1/alter-table#alter-column) statement to change the precision level of a `TIMESTAMP`/`TIMESTAMPTZ`-typed column. If there is already a non-default precision level specified for the column, the precision level can only be changed to an equal or greater precision level. For an example, see [Create a table with a `TIMESTAMP`-typed column, with precision](#create-a-table-with-a-timestamp-typed-column-with-precision).

## Supported casting and conversion

`TIMESTAMP` values can be [cast](/docs/v26.1/data-types#data-type-conversions-and-casts) to any of the following data types:

<table><thead><tr><th>Type</th><th>Details</th></tr></thead><tbody><tr><td><code>DECIMAL</code></td><td>Converts to number of seconds since the Unix epoch (Jan. 1, 1970).</td></tr><tr><td><code>FLOAT</code></td><td>Converts to number of seconds since the Unix epoch (Jan. 1, 1970).</td></tr><tr><td><code>TIME</code></td><td>Converts to the time portion (<code>HH:MM:SS</code>) of the timestamp.</td></tr><tr><td><code>INT</code></td><td>Converts to number of seconds since the Unix epoch (Jan. 1, 1970).</td></tr><tr><td><code>DATE</code></td><td>Converts to the date (<code>YYYY-MM-DD</code>) of the timestamp and omits the other information.</td></tr><tr><td><code>STRING</code></td><td>Converts to the date and time portion (<code>YYYY-MM-DD HH:MM:SS</code>) of the timestamp and omits the time zone offset.</td></tr></tbody></table>
...
- [`TIME`](/docs/v26.1/time)
- [`INT`](/docs/v26.1/int)
- [`DATE`](/docs/v26.1/date)
