...
Change data capture queries allow you to define the change data emitted to your sink when you create a changefeed. The expression [syntax](#syntax) provides a way to select columns and apply filters to further restrict or transform the data in your [changefeed messages](/docs/v26.1/changefeed-messages).
...
- CDC queries support [system columns](/docs/v26.1/crdb-internal), for example:
...
<Note>
When changes happen to a column that is part of a composite [key](/docs/v26.1/primary-key), the changefeed will produce a [delete message](#filter-delete-messages) and then an insert message.
</Note>

## Known limitations

- You can only apply CDC queries on a single table in each statement.
- Some [stable functions](/docs/v26.1/functions-and-operators#built-in-functions), notably functions that return MVCC timestamps, are overridden to return the MVCC timestamp of the event, e.g., `transaction_timestamp` or `statement_timestamp`. Additionally, some [time-based functions](/docs/v26.1/functions-and-operators#date-and-time-functions), such as `now()` are not supported. We recommend using the `transaction_timestamp()` function or the `crdb_internal_mvcc_timestamp` column instead.
- The following are not permitted in CDC queries:
    - [Volatile functions](/docs/v26.1/functions-and-operators#function-volatility).
    - Sub-select queries.
    - [Aggregate](/docs/v26.1/functions-and-operators#aggregate-functions) and [window functions](/docs/v26.1/window-functions) (i.e., functions operating over many rows). [#98237](https://github.com/cockroachdb/cockroach/issues/98237)
- `delete` changefeed events will only contain the [primary key](/docs/v26.1/primary-key). All other columns will emit as `NULL`. See [Capture delete messages](/docs/v26.1/cdc-queries#capture-delete-messages) for detail on running a CDC query that emits the deleted values. [#83835](https://github.com/cockroachdb/cockroach/issues/83835)
- [`ALTER CHANGEFEED`](/docs/v26.1/alter-changefeed) is not fully supported with changefeeds that use CDC queries. You can alter the options that a changefeed uses, but you cannot alter the changefeed target tables. [#83033](https://github.com/cockroachdb/cockroach/issues/83033)
- Creating a changefeed with CDC queries on tables with more than one [column family](/docs/v26.1/changefeeds-on-tables-with-column-families) is not supported. [#127761](https://github.com/cockroachdb/cockroach/issues/127761)

## CDC query function support

The following table outlines functions that are useful with CDC queries:

<table><thead><tr><th>Function</th><th>Description</th></tr></thead><tbody><tr><td><code>changefeed&#95;creation&#95;timestamp()</code></td><td>Returns the decimal MVCC timestamp when the changefeed was created. Use this function to build CDC queries that restrict emitted events by time. <code>changefeed&#95;creation&#95;timestamp()</code> can serve a similar purpose to the <a href="/docs/v26.1/functions-and-operators#date-and-time-functions"><code>now()</code> time function</a>, which is not supported with CDC queries.</td></tr><tr><td><code>event&#95;op()</code></td><td>Returns a string describing the type of event. If a changefeed is running with the <a href="/docs/v26.1/create-changefeed#diff"><code>diff</code></a> option, then this function returns <code>'insert'</code>, <code>'update'</code>, or <code>'delete'</code>. If a changefeed is running without the <code>diff</code> option, it is not possible to determine an update from an insert, so <code>event&#95;op()</code> returns <a href="https://www.cockroachlabs.com/blog/sql-upsert"><code>'upsert'</code></a> or <code>'delete'</code>.<br/><br/>If you're using CDC queries to filter only for the type of change operation, we recommend specifying the <a href="/docs/v26.1/changefeed-message-envelopes#route-events-based-on-operation-type"><code>envelope=enriched</code> option</a> for this metadata instead.</td></tr><tr><td><code>event&#95;schema&#95;timestamp()</code></td><td>Returns the timestamp of <a href="/docs/v26.1/online-schema-changes">schema change</a> events that cause a <a href="/docs/v26.1/changefeed-messages">changefeed message</a> to emit. When the schema change event does not result in a table backfill or scan, <code>event&#95;schema&#95;timestamp()</code> will return the event's timestamp. When the schema change event does result in a table backfill or scan, <code>event&#95;schema&#95;timestamp()</code> will return the timestamp at which the backfill/scan is read — the <a href="/docs/v26.1/how-does-a-changefeed-work">high-water mark time</a> of the changefeed.</td></tr></tbody></table>

You can also use the following functions in CDC queries:

- Functions marked as "Immutable" on the [Functions and Operators page](/docs/v26.1/functions-and-operators).
- Non-volatile [user-defined functions](/docs/v26.1/user-defined-functions). See the [Queries and user-defined functions](#queries-and-user-defined-functions) example.
- Functions that rely on [session data](/docs/v26.1/show-sessions). At the time of changefeed creation, information about the current session is saved. When a CDC query includes one of the functions that use session data, the query will evaluate the saved session data.
...
- Functions marked as "Volatile" on the [Functions and Operators page](/docs/v26.1/functions-and-operators).
- Functions listed in the [Known limitations](#known-limitations) section on this page.
- Functions marked as "Stable" on the [Functions and Operators page](/docs/v26.1/functions-and-operators), **except** for those listed previously.
...
Refer to [`CREATE CHANGEFEED`](/docs/v26.1/create-changefeed) for examples on using the foundational syntax to create a changefeed. For information on sinks, refer to the [Changefeed Sinks](/docs/v26.1/changefeed-sinks) page.

### Filter columns

To only emit data from specific columns in a table, you can use `SELECT {columns}` to define the table's columns.

As an example, using the `users` table from the [`movr` database](/docs/v26.1/movr#the-movr-database), you can create a changefeed that will emit messages including only the `name` and `city` column data:
...
To remove the [delete messages](/docs/v26.1/changefeed-messages#delete-messages) from a changefeed stream, use the [`event_op()`](#cdc-query-function-support) function:
...
Delete changefeed messages will only contain the [primary key](/docs/v26.1/primary-key) value and all other columns will emit as `NULL` (see the [Known limitations](#known-limitations)). To emit the deleted values, use the [`envelope=wrapped`](/docs/v26.1/create-changefeed#envelope), [`format=json`](/docs/v26.1/create-changefeed#format), and [`diff`](/docs/v26.1/create-changefeed#diff) options:
...
<Note>
If you do not need to select specific columns in a table or filter rows from a changefeed, you can instead create a changefeed using the [`diff` option](/docs/v26.1/create-changefeed#diff) to emit a `before` field with each message. This field includes the value of the row before the update was applied.
</Note>

### Reference TTL in a CDC query

In CockroachDB, table row deletes occur as a result of [regular SQL transactions](/docs/v26.1/delete-data) or through [row-level TTL](/docs/v26.1/row-level-ttl). When your changefeed emits [delete event messages](/docs/v26.1/changefeed-messages#delete-messages), you may need to distinguish between these two types of deletion. For example, only emitting messages for row-level TTL deletes from your changefeed.
...
For more detail, refer to the [Batch Delete Expire Data with Row-Level TTL](/docs/v26.1/row-level-ttl) page.
</Note>
...
In the following example, the table uses the [`ttl_expiration_expression`](/docs/v26.1/row-level-ttl#syntax-overview) storage parameter to reference the `expired_at` column. To create a changefeed on this table to explicitly emit the previous state of the row for TTL deletions:
...
For the `CREATE TABLE` statement and further details on `ttl_expiration_expression`, refer to [Using `ttl_expiration_expression`](/docs/v26.1/row-level-ttl#using-ttl_expiration_expression).
...
For the `CREATE TABLE` statement and further details on `ttl_expire_after`, refer to [Using `ttl_expire_after`](/docs/v26.1/row-level-ttl#using-ttl_expire_after).

<Note>
This will only emit rows that were deleted **after** expiring. Furthermore, consider that a [transactional SQL delete](/docs/v26.1/delete-data) during the window between the row expiring and the TTL job running will also cause this message to emit from the changefeed.
</Note>
...
When you are working with a [`REGIONAL BY ROW` table](/docs/v26.1/alter-table#regional-by-row), you can filter the changefeed on the `crdb_region` column to create a region-specific changefeed:

```sql
CREATE CHANGEFEED INTO sink AS SELECT * FROM table WHERE crdb_region = 'europe-west2';
```

For more detail on targeting `REGIONAL BY ROW` tables with changefeeds, see [Changefeeds in Multi-Region Deployments](/docs/v26.1/changefeeds-in-multi-region-deployments).

<Tip>
If you are running changefeeds from a [multi-region](/docs/v26.1/multiregion-overview) cluster, you may want to define which nodes take part in running the changefeed job. You can use the [`execution_locality` option](/docs/v26.1/changefeeds-in-multi-region-deployments#run-a-changefeed-job-by-locality) with key-value pairs to specify the [locality designations](/docs/v26.1/cockroach-start#locality) nodes must meet.
</Tip>

### Stabilize the changefeed message schema

As changefeed messages emit from the database, message formats can vary as tables experience [schema changes](/docs/v26.1/changefeed-messages#schema-changes). You can select columns with [typecasting](/docs/v26.1/data-types#data-type-conversions-and-casts) to prevent message fields from changing during a changefeed's lifecycle:
...
For example, you may need to identify what recently changed in a specific row. You can use the [`cursor`](/docs/v26.1/create-changefeed#cursor) option with the desired start time and a `WHERE` clause describing the row in question. Instead of sending to a sink, a "sinkless" changefeed will allow you to view the results in the SQL shell.

1. Find the start time. Use the [`cluster_logical_timestamp()`](/docs/v26.1/functions-and-operators#system-info-functions) function to calculate the logical time. This will return the logical timestamp for an hour earlier than the statement run time:
...
2. Run the changefeed without a sink and pass the start time to the `cursor` option:
    ...
    ```
3. To find changes within a time period, use `cursor` with the [`end_time`](/docs/v26.1/create-changefeed#end-time) option:
...
The output will only include the row's history that has been changed within the [garbage collection window](/docs/v26.1/architecture/storage-layer#garbage-collection). If the change occurred outside of the garbage collection window, it will not be returned as part of this output. See [Garbage collection and changefeeds](/docs/v26.1/protect-changefeed-data) for more detail on how the garbage collection window interacts with changefeeds.

### Customize changefeed messages

You can adapt your [changefeed messages](/docs/v26.1/changefeed-messages) by filtering the columns, but it is also possible to build message fields with SQL expressions.
...
- Uses [`jsonb_build_object()`](/docs/v26.1/functions-and-operators) to construct the desired data field.
...
For example, when you delete a message in your outbox table after processing it (or with [row-level TTL](/docs/v26.1/row-level-ttl)). You can filter the [delete messages](#filter-delete-messages) from your changefeed:
...
You can create CDC queries that include [user-defined functions](/docs/v26.1/user-defined-functions).

The following [`CREATE FUNCTION`](/docs/v26.1/create-function) statement builds the `doubleRevenue()` function at the database level:
