...
old_description: "Migration guide detailing how to migrate away from a Helm deployment of CockroachDB to the &#123;&#123; site.data.products.cockroachdb-operator &#125;&#125;."
---
...
<Tip>
If your existing cluster was created using the Public operator, refer to the [Public operator migration guide](/docs/v26.1/migrate-cockroachdb-kubernetes-operator).
</Tip>
...
<Warning>
Commands that use RPCs (such as `cockroach node drain` and `cockroach node decommission`) will be unavailable until the public service is updated in step 4. The CockroachDB operator uses a different port than StatefulSets for RPC services, causing these commands to fail for a limited time.

The CockroachDB operator does not support custom store directories for [WAL failover](/docs/v26.1/wal-failover). If the existing Helm deployment uses custom store directories, they must be reconfigured to use the default directories prior to beginning migration.
</Warning>
...
2. Create the `crdbnode` resource that corresponds to the StatefulSet pod you just scaled down. Each manifest is labeled with the pattern `crdbnode-X.yaml`, where `X` corresponds to a StatefulSet pod named `{STS_NAME}-X`. Note the pod that was scaled down and specify its manifest in a command like the following:
    ...
    ```
3. Wait for the new pod to become ready. If it doesn’t, [check the operator logs](/docs/v26.1/monitor-cockroachdb-operator#monitor-the-operator) for errors.
