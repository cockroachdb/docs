...
This page is for Kubernetes deployments that are not using the CockroachDB operator. For guidance specific to the CockroachDB operator, read [Resource management with the CockroachDB operator](/docs/v26.1/configure-cockroachdb-operator).
...
These settings override the defaults used when [deploying CockroachDB on Kubernetes](/docs/v26.1/deploy-cockroachdb-with-kubernetes).
...
<Note>
1 CPU in Kubernetes is equivalent to 1 vCPU or 1 hyperthread. For best practices on provisioning CPU and memory for CockroachDB, see the [Production Checklist](/docs/v26.1/recommended-production-settings#hardware).
</Note>

Specify CPU and memory values in `resources.requests` and `resources.limits` in the Public operator's custom resource, which is used to [deploy the cluster](/docs/v26.1/deploy-cockroachdb-with-kubernetes#initialize-the-cluster):
...
Specify `cache` and `maxSQLMemory` in the Public operator's custom resource, which is used to [deploy the cluster](/docs/v26.1/deploy-cockroachdb-with-kubernetes#initialize-the-cluster):
...
<Note>
Specifying these values is equivalent to using the `--cache` and `--max-sql-memory` flags with [`cockroach start`](/docs/v26.1/cockroach-start#flags).
</Note>
...
The storage capacity of each volume is set in `pvc.spec.resources` in the Public operator's custom resource, which is used to [deploy the cluster](/docs/v26.1/deploy-cockroachdb-with-kubernetes#initialize-the-cluster):
...
If you discover that you need more capacity, you can expand the persistent volumes on a running cluster. Increasing disk size is often [beneficial for CockroachDB performance](/docs/v26.1/kubernetes-performance#disk-size).

Specify a new volume size in `resources.requests` and `resources.limits` in the Public operator's custom resource, which is used to [deploy the cluster](/docs/v26.1/deploy-cockroachdb-with-kubernetes#initialize-the-cluster):
...
<table><thead><tr><th>Protocol</th><th>Default</th><th>Description</th><th>Custom Resource Field</th></tr></thead><tbody><tr><td>gRPC</td><td>26258</td><td>Used for node connections</td><td><code>grpcPort</code></td></tr><tr><td>HTTP</td><td>8080</td><td>Used to <a href="/docs/v26.1/ui-overview#db-console-access">access the DB Console</a></td><td><code>httpPort</code></td></tr><tr><td>SQL</td><td>26257</td><td>Used for SQL shell access</td><td><code>sqlPort</code></td></tr></tbody></table>

Specify alternate port numbers in the Public operator's [custom resource](/docs/v26.1/deploy-cockroachdb-with-kubernetes#initialize-the-cluster) (for example, to match the default port `5432` on PostgreSQL):
...
Specify Ingress objects in `ingress.ui` (HTTP) or `ingress.sql` (SQL) in the Public operator's custom resource, which is used to [deploy the cluster](/docs/v26.1/deploy-cockroachdb-with-kubernetes#initialize-the-cluster):
...
Specify CPU and memory values in `resources.requests` and `resources.limits` in the StatefulSet manifest you used to [deploy the cluster](/docs/v26.1/deploy-cockroachdb-with-kubernetes?filters=manual#configure-the-cluster):
...
The storage capacity of each volume is initially set in `volumeClaimTemplates.spec.resources` in the StatefulSet manifest you used to [deploy the cluster](/docs/v26.1/deploy-cockroachdb-with-kubernetes?filters=manual#configure-the-cluster):
...
<Note>
These steps assume you followed the tutorial [Deploy CockroachDB on Kubernetes](/docs/v26.1/deploy-cockroachdb-with-kubernetes?filters=manual).
</Note>
...
Specify CPU and memory values in `resources.requests` and `resources.limits` in the custom values file you created when [deploying the cluster](/docs/v26.1/deploy-cockroachdb-with-kubernetes?filters=helm#step-2-start-cockroachdb):
...
Specify `cache` and `maxSQLMemory` in the custom values file you created when [deploying the cluster](/docs/v26.1/deploy-cockroachdb-with-kubernetes?filters=helm#step-2-start-cockroachdb):
...
<Note>
These steps assume you followed the tutorial [Deploy CockroachDB on Kubernetes](/docs/v26.1/deploy-cockroachdb-with-kubernetes?filters=helm).
</Note>
