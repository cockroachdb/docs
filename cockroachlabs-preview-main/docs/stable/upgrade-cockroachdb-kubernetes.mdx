...
This page shows how to upgrade a CockroachDB cluster that is [deployed on a Kubernetes cluster](/docs/v26.1/deploy-cockroachdb-with-kubernetes).

This page is for Kubernetes deployments that are not using the CockroachDB operator. For guidance specific to the CockroachDB operator, read [Upgrade a Cluster in Kubernetes with the CockroachDB operator](/docs/v26.1/upgrade-cockroachdb-operator).
...
<Tip>
If you [deployed CockroachDB on Red Hat OpenShift](/docs/v26.1/deploy-cockroachdb-with-kubernetes-openshift), substitute `kubectl` with `oc` in the following commands.
</Tip>
...
3. After the pod has been restarted with the new image, start the CockroachDB [built-in SQL client](/docs/v26.1/cockroach-sql):
...
5. After the pod has been restarted with the new image, start the CockroachDB [built-in SQL client](/docs/v26.1/cockroach-sql):
...
1. Connect to the cluster using the SQL shell:
    ...
    --host=cockroachdb-public
    ```
2. Set the [cluster setting](/docs/v26.1/cluster-settings) `cluster.preserve_downgrade_option` to the cluster's current major version. The Helm chart does not yet support the [cluster setting](/docs/v26.1/cluster-settings) `cluster.auto_upgrade.enabled`.
