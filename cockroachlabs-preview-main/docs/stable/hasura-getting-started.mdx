...
- An existing [CockroachDB Cloud](/docs/stable/cockroachcloud/quickstart) cluster, running CockroachDB v22.2 or later.
...
4. Paste the contents of the CA certificate file under **Value**.
    ...
    <Frame>
      ![Adding the SSL_ROOT_CERT environment variable to the Hasura project](/images/v26.1/hasura-ca-cert.png)
    </Frame>
5. Click **Add**.
...
4. In the **Connect Existing Database** tab configure the connection to your cluster.
    ...
    3. Select **Environment Variable**, then enter `CRDB_URL` in the **Environment Variable** input box.![Adding the connection string to the Hasura Data Manager](/images/v26.1/hasura-data-source.png)
...
Your CockroachDB Standard cluster needs to be configured to [allow incoming client connections](/docs/stable/cockroachcloud/network-authorization#ip-allowlisting) from Hasura Cloud.

1. In the Hasura Cloud overview page select **Projects**, then click the **Config** icon for your project.

    <Frame>
      ![Click the project Config icon](/images/v26.1/hasura-project-config.png)
    </Frame>
...
<Frame>
  ![Executing the GraphQL query in the Hasura Cloud API Explorer](/images/v26.1/hasura-execute-query.png)
</Frame>
