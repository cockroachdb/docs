---
title: "Storage Layer"
old_description: "The storage layer of CockroachDB's architecture reads and writes data to disk."
---
...
[CockroachDB uses the Pebble storage engine](/docs/v26.1/cockroach-start#storage-engine). Pebble is inspired by RocksDB, but differs in that it:
...
CockroachDB uses [Pebble](/docs/v26.1/cockroach-start#storage-engine)––an embedded key-value store inspired by RocksDB, and developed by Cockroach Labs––to read and write data to disk.
...
<Frame>![Structure of an SST file](/images/v26.1/sst.png)</Frame>

SST files are immutable; they are never modified, even during the [compaction process](#compaction).

##### LSM levels

The levels of the LSM are organized from L0 to L6. L0 is the top-most level. L6 is the bottom-most level. New data is added into L0 (e.g., using [`INSERT`](/docs/v26.1/insert) or [`IMPORT INTO`](/docs/v26.1/import-into)) and then merged down into lower levels over time.
...
- To allow LSM-based storage engines like Pebble to support ingesting large amounts of data, such as when using the [`IMPORT INTO`](/docs/v26.1/import-into) statement.
- To allow for easier and more efficient flushes of [memtables](#memtable-and-write-ahead-log).

<Frame>![LSM tree with SST files](/images/v26.1/lsm-with-ssts.png)</Frame>
...
Read amplification can be especially bad if a large [`IMPORT INTO`](/docs/v26.1/import-into) is overloading the cluster (due to insufficient CPU and/or IOPS) and the storage engine has to consult many small SSTs in L0 to determine the most up-to-date value of the keys being read (e.g., using a [`SELECT`](/docs/v26.1/select-clause)).

A certain amount of read amplification is expected in a normally functioning CockroachDB cluster. For example, a read amplification factor less than 10 as shown in the [**Read Amplification** graph on the **Storage** dashboard](/docs/v26.1/ui-storage-dashboard#other-graphs) is considered healthy.
...
Value separation is especially beneficial for workloads with large values relative to key size (for example, [Raft log](/docs/v26.1/architecture/replication-layer#raft) entries). It reduces [write amplification](#write-amplification) by about 50%, at the cost of about 20% in [space amplification](/docs/v26.1/operational-faqs#space-amplification). In practice, value separation causes the storage engine to use far fewer [IOPS](/docs/v26.1/common-issues-to-monitor#disk-iops) and storage bandwidth overall, which are expensive, at the cost of an increase in storage capacity, which is much cheaper.

To enable value separation, set the following [cluster setting](/docs/v26.1/set-cluster-setting):
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        <code>storage.value&#95;separation.blob&#95;files.count</code>
      </td>
      <td>
        The number of blob files that are used to store{" "}
        <a href="/docs/v26.1/architecture/storage-layer#value-separation">separated values</a> within the storage
        engine.
      </td>
      <td>
        Use this metric to track how many values (of key-value pairs) are being stored outside of the{" "}
        <a href="/docs/v26.1/architecture/storage-layer#log-structured-merge-trees">LSM</a> by the storage engine due to
        their large size.
      </td>
    </tr>
    <tr>
      <td>
        <code>storage.value&#95;separation.blob&#95;files.size</code>
      </td>
      <td>
        The size of the physical blob files that are used to store{" "}
        <a href="/docs/v26.1/architecture/storage-layer#value-separation">separated values</a> within the storage
        engine. This value is the physical post-compression sum of the{" "}
        <code>storage.value&#95;separation.value&#95;bytes.referenced</code> and{" "}
        <code>storage.value&#95;separation.value&#95;bytes.unreferenced</code> metrics.
      </td>
      <td>
        Use this metric to see how much of your physical storage capacity is being used by separated values in blob
        files.
      </td>
    </tr>
    <tr>
      <td>
        <code>storage.value&#95;separation.value&#95;bytes.referenced</code>
      </td>
      <td>
        The size of storage engine value bytes (pre-compression) that are{" "}
        <a href="/docs/v26.1/architecture/storage-layer#value-separation">stored separately in blob files</a> and
        referenced by a live <a href="/docs/v26.1/architecture/storage-layer#ssts">SSTable</a>.
      </td>
      <td>
        Use this metric to see how much live (i.e., not yet eligible for compaction) blob storage is in use by separated
        values.
      </td>
    </tr>
    <tr>
      <td>
        <code>storage.value&#95;separation.value&#95;bytes.unreferenced</code>
      </td>
      <td>
        The size of storage engine value bytes (pre-compression) that are{" "}
        <a href="/docs/v26.1/architecture/storage-layer#value-separation">stored separately in blob files</a> and not
        referenced by any live <a href="/docs/v26.1/architecture/storage-layer#ssts">SSTable</a>. These bytes are
        garbage that could be reclaimed by a <a href="/docs/v26.1/architecture/storage-layer#compaction">compaction</a>.
      </td>
      <td>Use this metric to see how much blob storage is no longer in use and waiting to be compacted.</td>
    </tr>
  </tbody>
</table>
...
<Tip>
  By default, the [storage engine](/docs/v26.1/architecture/storage-layer) uses a compaction concurrency of 3. If you
  have sufficient IOPS and CPU headroom, you can consider increasing this setting via the
  `COCKROACH_COMPACTION_CONCURRENCY` environment variable. This may help to reshape the LSM more quickly in [inverted
  LSM](/docs/v26.1/architecture/storage-layer#inverted-lsms) scenarios; and it can lead to increased overall performance
  for some workloads. Cockroach Labs **strongly recommends** testing your workload against non-default values of this
  setting.
</Tip>
...
For instructions showing how to monitor your cluster's LSM health, see [LSM Health](/docs/v26.1/common-issues-to-monitor#lsm-health). To monitor your cluster's LSM L0 health, see [IO Overload](/docs/v26.1/ui-overload-dashboard#io-overload).
...
<Frame>![Relationship between memtable, WAL, and SSTs](/images/v26.1/memtable-wal-sst.png)</Frame>
...
<Frame>
  <iframe
    ...
    frameborder="0"
    height="315"
    src="https://www.youtube.com/embed/Ctp5WQdbEd4"
...
CockroachDB regularly garbage collects MVCC values to reduce the size of data stored on disk. To do this, we compact old MVCC values when there is a newer MVCC value with a timestamp that's older than the garbage collection period. The garbage collection period can be set at the cluster, database, or table level by configuring the [`gc.ttlseconds` replication zone variable](/docs/v26.1/configure-replication-zones#gc-ttlseconds). For more information about replication zones, see [Replication Controls](/docs/v26.1/configure-replication-zones).

#### Protected timestamps

Garbage collection can only run on MVCC values which are not covered by a _protected timestamp_. The protected timestamp subsystem exists to ensure the safety of operations that rely on historical data, such as:

- [Backups](/docs/v26.1/create-schedule-for-backup#protected-timestamps-and-scheduled-backups)
- [Changefeeds](/docs/v26.1/protect-changefeed-data)

Protected timestamps ensure the safety of historical data while also enabling shorter [GC TTLs](/docs/v26.1/configure-replication-zones#gc-ttlseconds). A shorter GC TTL means that fewer previous MVCC values are kept around. This can help lower query execution costs for workloads which update rows frequently throughout the day, since [the SQL layer](/docs/v26.1/architecture/sql-layer) has to scan over previous MVCC values to find the current value of a row.

##### How protected timestamps work

Protected timestamps work by creating _protection records_, which are stored in an internal system table. When a long-running job such as a backup wants to protect data at a certain timestamp from being garbage collected, it creates a protection record associated with that data and timestamp.
...
Now that you've learned about our architecture, [start up a CockroachDB Standard cluster](/docs/cockroachcloud/quickstart) or [local cluster](/docs/v26.1/install-cockroachdb) and start [developing with CockroachDB](/docs/v26.1/developer-guide-overview).
