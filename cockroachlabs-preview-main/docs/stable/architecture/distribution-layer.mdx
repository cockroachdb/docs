...
old_description: "The distribution layer of CockroachDB's architecture provides a unified view of your cluster's data."
---
...
<Frame>
  ![range-lookup.png](/images/v26.1/range-lookup.png)
</Frame>
...
By default, the network timeout for RPC connections between nodes is 2s, with a connection timeout of 4s, in order to reduce unavailability and tail latencies during infrastructure outages. This can be changed via the environment variable `COCKROACH_NETWORK_TIMEOUT`, which defaults to 2s.

Note that the gRPC network timeouts described above are defined from the server's point of view, not the client's (the client is any other node that is trying to make an inbound connection). From the client, we send an additional RPC ping every 1s to check that connections are alive. These pings have a fairly high timeout of 6s (a multiple of the value of `COCKROACH_NETWORK_TIMEOUT`) because they are subject to contention with other RPC traffic; e.g., during a large [`IMPORT INTO`](/docs/v26.1/import-into) they can be delayed by several seconds.
...
<table><thead><tr><th>Element</th><th>Description</th></tr></thead><tbody><tr><td><code>metaX</code></td><td>The level of meta range. Here we use a simplified <code>meta1</code> or <code>meta2</code>, but these are actually represented in <code>cockroach</code> as <code>\x02</code> and <code>\x03</code> respectively.</td></tr><tr><td><code>successorKey</code></td><td>The first key <em>greater</em> than the key you're scanning for. This makes CockroachDB's scans efficient; it simply scans the keys until it finds a value greater than the key it's looking for, and that is where it finds the relevant data.<br/><br/>The <code>successorKey</code> for the end of a keyspace is identified as <code>maxKey</code>.</td></tr></tbody></table>
...
- Membership changes to a range's Raft group (discussed in more detail in the [Replication Layer](/docs/v26.1/architecture/replication-layer#membership-changes-rebalance-repair) )
- Range splits
- Range merges
...
By default, CockroachDB attempts to keep ranges/replicas at [the default range size](/docs/v26.1/configure-replication-zones#range-max-bytes). Once a range reaches that limit, we split it into two smaller ranges (composed of contiguous key spaces).
...
By default, CockroachDB automatically merges small ranges of data together to form fewer, larger ranges (up to [the default range size](/docs/v26.1/configure-replication-zones#range-max-bytes)). This can improve both query latency and cluster survivability.
...
Learn how CockroachDB copies data and ensures consistency in the [replication layer](/docs/v26.1/architecture/replication-layer).
