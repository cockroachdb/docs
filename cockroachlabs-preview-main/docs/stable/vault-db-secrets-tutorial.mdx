...
old_description: "Learn how to achieve enhanced credential security using HashiCorp Vault's Dynamic Secrets functionality."
---
...
- [CockroachDB Integration with HashiCorp Vault](/docs/v24.3/hashicorp-integration)
...
- Either [Create a CockroachDB Standard cluster](/docs/stable/cockroachcloud/create-your-cluster) or [Start a Local Cluster](/docs/v25.4/start-a-local-cluster). Make a note of the cluster's name.

    For a cluster in CockroachDB Cloud, download the cluster's CA certificate locally. Refer to [Connect to your cluster](/docs/stable/cockroachcloud/connect-to-your-cluster).
- Create a SQL user and make a note of the username and password. Refer [Create a SQL user in CockroachDB Cloud](/docs/stable/cockroachcloud/managing-access#create-a-sql-user) or [Create a SQL user in CockroachDB self-hosted](/docs/v25.4/create-user).
...
- [Install the `cockroach` CLI](/docs/v24.3/install-cockroachdb).
...
3. Execute a SQL statement to test your connection.
    ...
    Time: 107ms
    ```
4. Set your Vault target and the `admin` Vault namespace.
...
8. Revoke credentials by revoking a lease. A *lease* on a *role* in Vault terms corresponds to an actual credential pair for a SQL user on the CockroachDB cluster.
    ...
    ```
9. Next, provision a Vault *policy* for CockroachDB client operators so that they can access the client credentials without having the admin role on the CockroachDB cluster.
...
If you want user/role management operations to finish more quickly, and do not care whether concurrent transactions will immediately see the side effects of those operations, set the [session variable](/docs/v24.3/set-vars) `allow_role_memberships_to_change_during_transaction` to `true`. To learn more, refer to [How to speed up user / role assignment](/docs/v24.3/hashicorp-integration#how-to-speed-up-user-role-management) and [`GRANT`](/docs/v24.3/grant).
