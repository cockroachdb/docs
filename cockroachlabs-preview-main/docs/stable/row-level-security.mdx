...
old_description: "Restrict access to specific rows of data based on user roles, permissions, or other criteria"
---

*Row-level Security* (RLS) is a security feature that allows organizations to restrict access to specific rows of data in a database based on user [roles](/docs/v26.1/security-reference/authorization#roles), [permissions](/docs/v26.1/security-reference/authorization#authorization-models), or other criteria.

Row-level security complements standard SQL privileges ([`GRANT`](/docs/v26.1/grant)/[`REVOKE`](/docs/v26.1/revoke)) by allowing administrators to define policies that determine precisely which rows users can view or modify within a specific table.
...
1. Choose which [schema objects](/docs/v26.1/schema-design-overview) need row-level security. You can re-use existing schemas, or create new ones and [insert data](/docs/v26.1/insert-data). ( [`CREATE TABLE`](/docs/v26.1/create-table), [`INSERT`](/docs/v26.1/insert) )
2. [Create roles](/docs/v26.1/create-role) &amp; [grant access](/docs/v26.1/grant) to schema objects by those roles. ( [`CREATE ROLE`](/docs/v26.1/create-role), [`GRANT`](/docs/v26.1/grant) )
3. Enable row-level security on the schema objects. ( [`ALTER TABLE... ENABLE ROW LEVEL SECURITY`](/docs/v26.1/alter-table#enable-row-level-security) )
4. Define row-level security policies on the schema objects which are assigned to specific roles. ( [`CREATE POLICY`](/docs/v26.1/create-policy) )
...
Policies function as filters or constraints applied automatically by CockroachDB during [query execution](/docs/v26.1/architecture/sql-layer#query-execution). They are based on boolean expressions evaluated in the context of the current [user](/docs/v26.1/security-reference/authorization#roles), [session properties](/docs/v26.1/show-sessions), and the row data itself.

When row-level security is enabled on a table:

1. Existing [SQL privileges](/docs/v26.1/security-reference/authorization#supported-privileges) still determine **if** a user can access the table at all (e.g., `SELECT`, `INSERT` ).
2. [Row-level security policies](/docs/v26.1/show-policies) determine **which rows** within the table are accessible or modifiable for specific commands.

Further details about RLS evaluation include:

- All [policies](/docs/v26.1/show-policies) apply to a specific set of [roles](/docs/v26.1/security-reference/authorization#roles). For a policy to be applicable, it must match at least one of the roles assigned to it. If the policy is associated with the `PUBLIC` role, it applies to all roles.
...
- The `USING` clause of [`CREATE POLICY`](/docs/v26.1/create-policy) filters rows during reads; the `WITH CHECK` clause validates writes, and defaults to `USING` if absent.

## Considerations

### Performance

Complex [policy expressions](/docs/v26.1/create-policy) evaluated per-row can impact query performance. To limit the performance impacts of row-level security, optimize your policy expressions and consider [indexing](/docs/v26.1/indexes) relevant columns.

### Security privileges

[Policy expressions](/docs/v26.1/create-policy) execute with the [privileges](/docs/v26.1/security-reference/authorization#supported-privileges) of the user invoking the query, unless functions marked [`SECURITY DEFINER`](/docs/v26.1/create-function#create-a-security-definer-function) are used.
...
- [Foreign keys](/docs/v26.1/foreign-key) (including cascades)
- [Primary key constraints](/docs/v26.1/primary-key)
- [Unique constraints](/docs/v26.1/unique)
- [`TRUNCATE`](/docs/v26.1/truncate)

### Change data capture (CDC)

[CDC](/docs/v26.1/change-data-capture-overview) messages that are emitted from a table will not be filtered using RLS policies. Furthermore, [CDC queries](/docs/v26.1/cdc-queries) are not supported on tables using RLS, and will fail with the error message: `CDC queries are not supported on tables with row-level security enabled`.

### Backup and restore

[Backup and restore](/docs/v26.1/backup-and-restore-overview) functionality does not take RLS policies into account.

### Logical data replication (LDR) and Physical cluster replication (PCR)

[Logical Data Replication (LDR)](/docs/v26.1/logical-data-replication-overview) and [Physical Cluster Replication (PCR)](/docs/v26.1/physical-cluster-replication-overview) do not take RLS policies into account.

LDR's [limitations with respect to schema changes](/docs/v26.1/manage-logical-data-replication#schema-changes) also apply to RLS, since RLS policies amount to a schema change.
...
When [views](/docs/v26.1/views) are accessed, RLS policies on any underlying tables are applied. [Policies](/docs/v26.1/create-policy) can only be defined directly on tables, not views.

Views will only show rows that the current [user](/docs/v26.1/security-reference/authorization#roles) has access to on the table that the view is referencing.

## Examples

### Create a policy

Refer to [`CREATE POLICY`](/docs/v26.1/create-policy).

### Alter a policy

Refer to [`ALTER POLICY`](/docs/v26.1/alter-policy).

### Drop a policy

Refer to [`DROP POLICY`](/docs/v26.1/drop-policy).
...
- [`ALTER TABLE... FORCE ROW LEVEL SECURITY`](/docs/v26.1/alter-table#force-row-level-security).

### Detect when row-level security is applied to a query

The [`row_security` session variable](/docs/v26.1/session-variables#row-security) controls whether queries in the current [session](/docs/v26.1/show-sessions) should silently honor RLS policies or signal an error when those policies would filter out rows.

The variable defaults to `on`, which applies policies as normal. Setting it to `off` lets non-[admin](/docs/v26.1/security-reference/authorization#admin-role) users detect when an RLS policy would alter their results by returning an error instead of silently filtering rows. Admin users and table owners remain exempt from RLS by default regardless of this setting. To force table owners to be subject to RLS, you must use [`ALTER TABLE... FORCE ROW LEVEL SECURITY`](/docs/v26.1/alter-table#force-row-level-security).
...
In a fine-grained access control scenario, you will want to restrict access to specific rows within a table based on user [roles](/docs/v26.1/security-reference/authorization#roles), attributes, or relationships defined within the data itself. This goes beyond table-level [`GRANT`](/docs/v26.1/grant) permissions. Common examples include restricting access to salary information, personal data, or region-specific records.
...
Enable row-level security using the [`ALTER TABLE... ENABLE ROW LEVEL SECURITY`](/docs/v26.1/alter-table#enable-row-level-security) statement. Optionally, you may want to ensure that the table owner is also subject to RLS using [`ALTER TABLE... FORCE ROW LEVEL SECURITY`](/docs/v26.1/alter-table#force-row-level-security).
...
The following policy allows managers to view their direct reports' records. This requires a way to look up the manager's username. In this example, we use the `CURRENT_USER` special form of the [function with the same name](/docs/v26.1/functions-and-operators#special-syntax-forms).
...
Specifically, the [UUID](/docs/v26.1/uuid) following the period in `application_name` is the tenant ID. We will use the `current_setting()` function in our RLS policies to extract the ID.
...
For a demo showing how to combine row-level security with [Multi-region SQL](/docs/v26.1/multiregion-overview) to constrain access to specific rows based on a user's geographic region, play the following video:

<Frame>
  <iframe allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen frameborder="0" height="315" src="https://www.youtube.com/embed/ZG8RsfwMaa8" title="YouTube video player" width="560"></iframe>
</Frame>
