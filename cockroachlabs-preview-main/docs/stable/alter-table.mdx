...
import { AlterTable } from "/snippets/syntax-diagrams/AlterTable.jsx";
import { AlterTable2 } from "/snippets/syntax-diagrams/AlterTable2.jsx";

The `ALTER TABLE` [statement](/docs/v26.1/sql-statements) changes the definition of a table. For information on using `ALTER TABLE`, see the pages for its [subcommands](#subcommands).
...
<table><thead><tr><th>Subcommand</th><th>Description</th><th>Can combine with other subcommands?</th></tr></thead><tbody><tr><td><a href="#add-column"><code>ADD COLUMN</code></a></td><td>Add columns to tables.</td><td>Yes</td></tr><tr><td><a href="#add-constraint"><code>ADD CONSTRAINT</code></a></td><td>Add constraints to columns.</td><td>Yes</td></tr><tr><td><a href="#alter-column"><code>ALTER COLUMN</code></a></td><td>Change an existing column.</td><td>Yes</td></tr><tr><td><a href="#alter-primary-key"><code>ALTER PRIMARY KEY</code></a></td><td>Change the <a href="/docs/v26.1/primary-key">primary key</a> of a table.</td><td>Yes</td></tr><tr><td><a href="#configure-zone"><code>CONFIGURE ZONE</code></a></td><td><a href="/docs/v26.1/configure-replication-zones">Replication Controls</a> for a table.</td><td>No</td></tr><tr><td><a href="#disable-row-level-security"><code>DISABLE ROW LEVEL SECURITY</code></a></td><td>Disable <a href="/docs/v26.1/row-level-security">row-level security</a> for a table.</td><td>Yes</td></tr><tr><td><a href="#drop-column"><code>DROP COLUMN</code></a></td><td>Remove columns from tables.</td><td>Yes</td></tr><tr><td><a href="#drop-constraint"><code>DROP CONSTRAINT</code></a></td><td>Remove constraints from columns.</td><td>Yes</td></tr><tr><td><a href="#enable-row-level-security"><code>ENABLE ROW LEVEL SECURITY</code></a></td><td>Enable <a href="/docs/v26.1/row-level-security">row-level security</a> for a table.</td><td>Yes</td></tr><tr><td><a href="#experimental_audit"><code>EXPERIMENTAL&#95;AUDIT</code></a></td><td>Enable per-table audit logs, for security purposes.</td><td>Yes</td></tr><tr><td><a href="#force-row-level-security"><code>FORCE / NO FORCE ROW LEVEL SECURITY</code></a></td><td>Force the table owner to be subject to <a href="/docs/v26.1/row-level-security">row-level security</a> policies defined on a table.</td><td>Yes</td></tr><tr><td><a href="#owner-to"><code>OWNER TO</code></a></td><td>Change the owner of the table.</td><td>No</td></tr><tr><td><a href="#partition-by"><code>PARTITION BY</code></a></td><td>Partition, re-partition, or un-partition a table.</td><td>Yes</td></tr><tr><td><a href="#rename-column"><code>RENAME COLUMN</code></a></td><td>Change the names of columns.</td><td>Yes</td></tr><tr><td><a href="#rename-constraint"><code>RENAME CONSTRAINT</code></a></td><td>Change constraints columns.</td><td>Yes</td></tr><tr><td><a href="#rename-to"><code>RENAME TO</code></a></td><td>Change the names of tables.</td><td>No</td></tr><tr><td><a href="#reset-storage-parameter"><code>RESET &#123;storage parameter&#125;</code></a></td><td>Reset a storage parameter on a table to its default value.</td><td>Yes</td></tr><tr><td><a href="#set-storage-parameter"><code>SET &#123;storage parameter&#125;</code></a></td><td>Set a storage parameter on a table.</td><td>Yes</td></tr><tr><td><a href="#set-locality"><code>SET LOCALITY</code></a></td><td>Set the table locality for a table in a <a href="/docs/v26.1/multiregion-overview">multi-region database</a>.</td><td>No</td></tr><tr><td><a href="#set-schema"><code>SET SCHEMA</code></a></td><td>Change the <a href="/docs/v26.1/sql-name-resolution">schema</a> of a table.</td><td>No</td></tr><tr><td><a href="#scatter"><code>SCATTER</code></a></td><td>Makes a best-effort attempt to redistribute replicas and leaseholders for the ranges of a table or index. Note that it does not return an error even if replicas are not moved.</td><td>No</td></tr><tr><td><a href="#split-at"><code>SPLIT AT</code></a></td><td>Force a <a href="/docs/v26.1/architecture/distribution-layer#range-splits">range split</a> at the specified row in the table.</td><td>No</td></tr><tr><td><a href="#unsplit-at"><code>UNSPLIT AT</code></a></td><td>Remove a range split enforcement in the table.</td><td>No</td></tr><tr><td><a href="#validate-constraint"><code>VALIDATE CONSTRAINT</code></a></td><td>Check whether values in a column match a <a href="/docs/v26.1/constraints">constraint</a> on the column.</td><td>Yes</td></tr></tbody></table>
...
The user must have the `CREATE` [privilege](/docs/v26.1/security-reference/authorization#managing-privileges) on the table.

#### Parameters

<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>IF NOT EXISTS</code></td><td>Add a column only if a column of the same name does not already exist; if one does exist, do not return an error.</td></tr><tr><td><code>column&#95;name</code></td><td>The name of the column you want to add. The column name must follow these <a href="/docs/v26.1/keywords-and-identifiers#identifiers">identifier rules</a> and must be unique within the table but can have the same name as indexes or constraints.</td></tr><tr><td><code>typename</code></td><td>The <a href="/docs/v26.1/data-types">data type</a> of the new column.</td></tr><tr><td><code>col&#95;qualification</code></td><td>An optional list of <a href="/docs/v26.1/create-table#column-qualifications">column qualifications</a>.</td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

#### Known limitations

- It is currently not possible to [add a column](/docs/v26.1/alter-table#add-column) to a table when:
    - The column uses a [sequence](/docs/v26.1/create-sequence) as the [`DEFAULT`](/docs/v26.1/default-value) value, for example using `nextval()`.
...
- When executing an [`ALTER TABLE ADD COLUMN`](/docs/v26.1/alter-table#add-column) statement with a [`DEFAULT`](/docs/v26.1/default-value) expression, new values generated:
    - Use the default [search path](/docs/v26.1/sql-name-resolution#search-path) regardless of the search path configured in the current session via `SET SEARCH_PATH`.
    - Use the UTC time zone regardless of the time zone configured in the current session via [`SET TIME ZONE`](/docs/v26.1/set-vars).
    - Have no default database regardless of the default database configured in the current session via [`SET DATABASE`](/docs/v26.1/set-vars), so you must specify the database of any tables they reference.
...
Use `ALTER TABLE... ADD CONSTRAINT` to add the following [constraints](/docs/v26.1/constraints) to columns:

- [`UNIQUE`](#add-the-unique-constraint)
- [`CHECK`](#add-the-check-constraint)
- [`FOREIGN KEY`](#add-the-foreign-key-constraint-with-cascade)

To add a primary key constraint to a table, you should explicitly define the primary key at [table creation](/docs/v26.1/create-table). To replace an existing primary key, you can use `ADD CONSTRAINT... PRIMARY KEY`. For details, see [Changing primary keys with `ADD CONSTRAINT... PRIMARY KEY`](#changing-primary-keys-with-add-constraint-primary-key).

The [`DEFAULT`](/docs/v26.1/default-value) and [`NOT NULL`](/docs/v26.1/not-null) constraints are managed through [`ALTER COLUMN`](/docs/v26.1/alter-table#alter-column).
...
- No primary key was explicitly defined at [table creation](/docs/v26.1/create-table). In this case, the table is created with a default [primary key on `rowid`](/docs/v26.1/indexes#creation). Using `ADD CONSTRAINT... PRIMARY KEY` drops the default primary key and replaces it with a new primary key.
- A [`DROP CONSTRAINT`](/docs/v26.1/alter-table#drop-constraint) statement precedes the `ADD CONSTRAINT... PRIMARY KEY` statement, in the same transaction. For an example, see [Drop and add the primary key constraint](#drop-and-add-a-primary-key-constraint).
...
- Set, change, or drop a column's [`DEFAULT` constraint](/docs/v26.1/default-value).
- Set or drop a column's [`NOT NULL` constraint](/docs/v26.1/not-null).
- Set, change, or drop an [`ON UPDATE` expression](/docs/v26.1/create-table#on-update-expressions).
- Change a column's [data type](/docs/v26.1/data-types).
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>column&#95;name</code></td><td>The name of the column to modify.</td></tr><tr><td><code>a&#95;expr</code></td><td>The new <a href="/docs/v26.1/default-value">default value</a> to set.</td></tr><tr><td><code>b&#95;expr</code></td><td>The <a href="/docs/v26.1/create-table#on-update-expressions"><code>ON UPDATE</code> expression</a> to set.</td></tr><tr><td><code>&#91;NOT&#93; VISIBLE</code></td><td>The visibility of a column when using <code>&#42;</code> in a <a href="/docs/v26.1/select-clause"><code>SELECT</code> clause</a>.</td></tr><tr><td><code>typename</code></td><td>The new <a href="/docs/v26.1/data-types">data type</a> you want to use. For details, see <a href="#alter-column-data-types">Alter column data types</a>.</td></tr><tr><td><code>USING a&#95;expr</code></td><td>How to compute a new column value from the old column value.</td></tr></tbody></table>
...
- The column is part of an [index](/docs/v26.1/indexes).
- The column has [`CHECK` constraints](/docs/v26.1/check).
- The column owns a [sequence](/docs/v26.1/create-sequence).
- The `ALTER COLUMN TYPE` statement is part of a combined `ALTER TABLE` statement.
- The `ALTER COLUMN TYPE` statement is inside an [explicit transaction](/docs/v26.1/begin-transaction).
- The column is part of a [TTL expression](/docs/v26.1/row-level-ttl).
- The column is used in a [function body](/docs/v26.1/user-defined-functions).
- The column is part of a [computed column expression](/docs/v26.1/computed-columns).
- The column is referenced in a [view](/docs/v26.1/views).

### `ALTER PRIMARY KEY`

Use `ALTER TABLE... ALTER PRIMARY KEY` to change the [primary key](/docs/v26.1/primary-key) of a table.

Note the following:

- You cannot change the primary key of a table that is currently undergoing a primary key change, or any other [schema change](/docs/v26.1/online-schema-changes).
- `ALTER PRIMARY KEY` might need to rewrite multiple indexes, which can make it an expensive operation.
- When you change a primary key with `ALTER PRIMARY KEY`, the old primary key index becomes a [`UNIQUE`](/docs/v26.1/unique) secondary index. This helps optimize the performance of queries that still filter on the old primary key column.
- `ALTER PRIMARY KEY` does not alter the [partitions](/docs/v26.1/partitioning) on a table or its indexes, even if a partition is defined on [a column in the original primary key](/docs/v26.1/partitioning#partition-using-primary-key). If you alter the primary key of a partitioned table, you must update the table partition accordingly.
- The secondary index created by `ALTER PRIMARY KEY` will not be partitioned, even if a partition is defined on [a column in the original primary key](/docs/v26.1/partitioning#partition-using-primary-key). To ensure that the table is partitioned correctly, you must create a partition on the secondary index, or drop the secondary index.
- Any new primary key column set by `ALTER PRIMARY KEY` must have an existing [`NOT NULL` constraint](/docs/v26.1/not-null). To add a `NOT NULL` constraint to an existing column, use [`ALTER TABLE... ALTER COLUMN... SET NOT NULL`](#set-not-null-constraint).
...
The user must have the `CREATE` [privilege](/docs/v26.1/security-reference/authorization#managing-privileges) on a table to alter its primary key.

#### Parameters

<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>index&#95;params</code></td><td>The name of the column(s) that you want to use for the primary key. These columns replace the current primary key column(s).</td></tr><tr><td><code>USING HASH</code></td><td>Creates a <a href="/docs/v26.1/hash-sharded-indexes">hash-sharded index</a>.</td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

### `CONFIGURE ZONE`

`ALTER TABLE... CONFIGURE ZONE` is used to add, modify, reset, or remove replication zones for a table. To view details about existing replication zones, use [`SHOW ZONE CONFIGURATIONS`](/docs/v26.1/show-zone-configurations). For more information about replication zones, see [Replication Controls](/docs/v26.1/configure-replication-zones).
...
By default, `DROP COLUMN` drops any [indexes](/docs/v26.1/indexes) on the column being dropped, and any indexes that reference the column, including indexes with [`STORING` clauses](/docs/v26.1/create-index#store-columns) that reference the column.

For examples, see [Drop columns](#drop-columns).

#### Known limitations

CockroachDB prevents a column from being dropped using [`ALTER TABLE... DROP COLUMN`](/docs/v26.1/alter-table#drop-column) if it is referenced by a partial index predicate. To drop such a column, the partial indexes need to be dropped first using [`DROP INDEX`](/docs/v26.1/drop-index). [#97813](https://github.com/cockroachdb/cockroach/issues/97813).
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>column&#95;name</code></td><td>The name of the column you want to drop.<br/><br/>When a column with a <a href="/docs/v26.1/check"><code>CHECK</code></a> constraint is dropped, the <code>CHECK</code> constraint is also dropped.</td></tr><tr><td><code>CASCADE</code></td><td>Drop the column even if objects (such as <a href="/docs/v26.1/views">views</a>) depend on it; drop the dependent objects, as well. <code>CASCADE</code> will drop a column with a foreign key constraint if it is the only column in the reference.<br/><br/><code>CASCADE</code> does not list the objects it drops, so should be used cautiously.<br/><br/><code>CASCADE</code> is not required to drop an indexed column, or a column that is referenced by an index. By default, <code>DROP COLUMN</code> drops any <a href="/docs/v26.1/indexes">indexes</a> on the column being dropped, and any indexes that reference the column, including <a href="/docs/v26.1/partial-indexes">partial indexes</a> with predicates that reference the column and indexes with <a href="/docs/v26.1/create-index#store-columns"><code>STORING</code> clauses</a> that reference the column.</td></tr><tr><td><code>RESTRICT</code></td><td><em>(Default)</em> Do not drop the column if any objects (such as <a href="/docs/v26.1/views">views</a>) depend on it.</td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

### `DROP CONSTRAINT`

Use `ALTER TABLE... DROP CONSTRAINT` to remove [`CHECK`](/docs/v26.1/check) and [`FOREIGN KEY`](/docs/v26.1/foreign-key) constraints from columns.

[`PRIMARY KEY`](/docs/v26.1/primary-key) constraints can be dropped with `DROP CONSTRAINT` if an [`ADD CONSTRAINT`](/docs/v26.1/alter-table#add-constraint) statement follows the `DROP CONSTRAINT` statement in the same transaction.

<Tip>
When you change a primary key with [`ALTER TABLE... ALTER PRIMARY KEY`](/docs/v26.1/alter-table#alter-primary-key), the old primary key index becomes a secondary index. If you do not want the old primary key to become a secondary index, use `DROP CONSTRAINT`/[`ADD CONSTRAINT`](/docs/v26.1/alter-table#add-constraint) to change the primary key.
</Tip>

<Note>
For information about removing other constraints, see [Constraints: Remove Constraints](/docs/v26.1/constraints#remove-constraints).
</Note>
...
`ALTER TABLE... EXPERIMENTAL_AUDIT` enables or disables the recording of SQL audit events to the [`SENSITIVE_ACCESS`](/docs/v26.1/logging#sensitive_access) logging channel for a table. The `SENSITIVE_ACCESS` log output is also called the *SQL audit log*. For details on using SQL audit logs, see [SQL Audit Logging](/docs/v26.1/sql-audit-logging).
...
CockroachDB stores audit log information in a way that ensures durability, but negatively impacts performance. As a result, we recommend using SQL audit logs for security purposes only. For more information, see [Performance considerations](/docs/v26.1/sql-audit-logging#performance-considerations).
...
To change the owner of a table, the user must be an `admin` user, or the current owner of the table and a member of the new owner [role](/docs/v26.1/security-reference/authorization#roles). The new owner role must also have the `CREATE` [privilege](/docs/v26.1/security-reference/authorization#managing-privileges) on the schema to which the table belongs.
...
<Note>
It is not possible to rename a column referenced by a view. For more details, see [View Dependencies](/docs/v26.1/views#view-dependencies).
</Note>
...
<Note>
It is not possible to rename a constraint for a column referenced by a view. For more details, see [View Dependencies](/docs/v26.1/views#view-dependencies).
</Note>
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>IF EXISTS</code></td><td>Rename the constraint only if a constraint of <code>current&#95;name</code> exists; if one does not exist, do not return an error.</td></tr><tr><td><code>constraint&#95;name</code></td><td>The current name of the constraint.</td></tr><tr><td><code>constraint&#95;new&#95;name</code></td><td>The new <a href="/docs/v26.1/sql-grammar#name"><code>name</code></a> you want to use for the constraint, which must be unique to its table and follow these <a href="/docs/v26.1/keywords-and-identifiers#identifiers">identifier rules</a>.</td></tr></tbody></table>
...
`ALTER TABLE... RENAME TO` cannot be used to move a table from one database to another. To change a table's database, use [`BACKUP`](/docs/v26.1/backup#back-up-a-table-or-view) and [`RESTORE`](/docs/v26.1/restore#restore-a-table).
</Note>

<Note>
It is not possible to rename a table referenced by a view. For more details, see [View Dependencies](/docs/v26.1/views#view-dependencies).
</Note>
...
The user must have the `DROP` [privilege](/docs/v26.1/security-reference/authorization#managing-privileges) on the table and the `CREATE` on the parent database. When moving a table from one database to another, the user must have the `CREATE` privilege on both the source and target databases.

#### Parameters

<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>table&#95;new&#95;name</code></td><td>The new name of the table, which must be unique within its database and follow these <a href="/docs/v26.1/keywords-and-identifiers#identifiers">identifier rules</a>. When the parent database is not set as the default, the name must be formatted as <code>database.name</code>.<br/><br/>The <a href="/docs/v26.1/upsert"><code>UPSERT</code></a> and <a href="/docs/v26.1/insert"><code>INSERT ON CONFLICT</code></a> statements use a temporary table called <code>excluded</code> to handle uniqueness conflicts during execution. It's therefore not recommended to use the name <code>excluded</code> for any of your tables.</td></tr></tbody></table>
...
<Note>
To reset a storage parameter on an existing index, you must drop and [recreate the index without the storage parameter](/docs/v26.1/with-storage-parameter).
</Note>
...
The user must be a member of the [`admin`](/docs/v26.1/security-reference/authorization#roles) or [owner](/docs/v26.1/security-reference/authorization#object-ownership) roles, or have the [`CREATE` privilege](/docs/v26.1/security-reference/authorization#supported-privileges) on the table.
...
[Row-level security](/docs/v26.1/row-level-security) must be explicitly enabled per table. Typically, this is controlled by the [role](/docs/v26.1/security-reference/authorization#roles) that owns the table.

For examples, see [Enable row-level security](#enable-row-level-security).

<Note>
RLS applies to a table **only when explicitly enabled** using `ALTER TABLE... ENABLE ROW LEVEL SECURITY`. Roles exempt from RLS policies include [admins](/docs/v26.1/security-reference/authorization#roles), [table owners](/docs/v26.1/security-reference/authorization#object-ownership) (unless the table is set to [`FORCE ROW LEVEL SECURITY`](#force-row-level-security)), and [roles with `BYPASSRLS`](/docs/v26.1/alter-role#allow-a-role-to-bypass-row-level-security-rls).
</Note>

#### Required privileges

The user must be a member of the [`admin`](/docs/v26.1/security-reference/authorization#roles) or [owner](/docs/v26.1/security-reference/authorization#object-ownership) roles.

#### Parameters

<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>table&#95;name</code></td><td>The name of the table which is having <a href="/docs/v26.1/row-level-security">row-level security</a> (RLS) enabled or disabled.</td></tr><tr><td><code>ENABLE</code></td><td>Whether to enable RLS.</td></tr></tbody></table>

### `DISABLE ROW LEVEL SECURITY`

This statement disables [Row-level security](/docs/v26.1/row-level-security) per table. Typically, this is controlled by the [role](/docs/v26.1/security-reference/authorization#roles) that owns the table.
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>table&#95;name</code></td><td>The name of the table which is having <a href="/docs/v26.1/row-level-security">row-level security</a> (RLS) enabled or disabled.</td></tr><tr><td><code>DISABLE</code></td><td>Whether to disable RLS.</td></tr></tbody></table>

### `FORCE / NO FORCE ROW LEVEL SECURITY`

`ALTER TABLE... FORCE ROW LEVEL SECURITY` prevents table [owners](/docs/v26.1/security-reference/authorization#object-ownership) from bypassing [row-level security](/docs/v26.1/row-level-security) (RLS) policies.
...
<Warning>
Users with the `BYPASSRLS` [role option](/docs/v26.1/security-reference/authorization#role-options) can still bypass RLS even when `ALTER TABLE... FORCE ROW LEVEL SECURITY` is enabled.
</Warning>
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>table&#95;name</code></td><td>The name of the table on which <a href="/docs/v26.1/row-level-security">row-level security</a> (RLS) is enabled or disabled.</td></tr><tr><td><code>FORCE</code></td><td><code>FORCE</code> ensures that all access (even by the table owner) adheres to <a href="/docs/v26.1/row-level-security">row-level security</a> policies.</td></tr><tr><td><code>NO FORCE</code></td><td><code>NO FORCE</code> removes the restriction that all access (even by the table owner) adheres to <a href="/docs/v26.1/row-level-security">row-level security</a> policies.</td></tr></tbody></table>
...
<Note>
To set a storage parameter on an existing index, you must drop and [recreate the index with the storage parameter](/docs/v26.1/with-storage-parameter).
</Note>
...
<table><thead><tr><th>Parameter name</th><th>Description</th><th>Data type</th><th>Default value</th></tr></thead><tbody><tr><td><code>exclude&#95;data&#95;from&#95;backup</code></td><td>Exclude the data in this table from any future backups.</td><td>Boolean</td><td><code>false</code></td></tr><tr><td><code>infer&#95;rbr&#95;region&#95;col&#95;using&#95;constraint</code></td><td>For <a href="/docs/v26.1/table-localities#regional-by-row-tables"><code>REGIONAL BY ROW</code></a> tables, automatically populate the hidden <code>crdb&#95;region</code> column on <code>INSERT</code>, <code>UPDATE</code>, and <code>UPSERT</code> by looking up the region of the referenced parent row. Set this parameter to the name of a <a href="/docs/v26.1/foreign-key">foreign key</a> constraint on the table that includes the <code>crdb&#95;region</code> column. The foreign key cannot be dropped while the parameter is set.</td><td>String</td><td><code>NULL</code></td></tr><tr><td><a></a><code>schema&#95;locked</code></td><td>Indicates that a <a href="/docs/v26.1/online-schema-changes">schema change</a> is not currently ongoing on this table. CockroachDB automatically unsets this parameter before performing a schema change and reapplies it when done. Enabling <code>schema&#95;locked</code> can help <a href="/docs/v26.1/create-changefeed#disallow-schema-changes-on-tables-to-improve-changefeed-performance">improve performance of changefeeds</a> running on this table.</td><td>Boolean</td><td><code>false</code></td></tr><tr><td><code>sql&#95;stats&#95;automatic&#95;collection&#95;enabled</code></td><td>Enable automatic collection of <a href="/docs/v26.1/cost-based-optimizer#full-statistics">full</a> and <a href="/docs/v26.1/cost-based-optimizer#partial-statistics">partial</a> statistics for this table.</td><td>Boolean</td><td><code>true</code></td></tr><tr><td><code>sql&#95;stats&#95;automatic&#95;collection&#95;min&#95;stale&#95;rows</code></td><td>Minimum number of stale rows in this table that will trigger a full statistics refresh.</td><td>Integer</td><td>500</td></tr><tr><td><code>sql&#95;stats&#95;automatic&#95;collection&#95;fraction&#95;stale&#95;rows</code></td><td>Fraction of stale rows in this table that will trigger a full statistics refresh.</td><td>Float</td><td>0.2</td></tr><tr><td><code>sql&#95;stats&#95;automatic&#95;full&#95;collection&#95;enabled</code></td><td>Enable automatic collection of <a href="/docs/v26.1/cost-based-optimizer#full-statistics">full statistics</a> for this table. <code>sql&#95;stats&#95;automatic&#95;collection&#95;enabled</code> must be <code>true</code>.</td><td>Boolean</td><td><code>true</code></td></tr><tr><td><code>sql&#95;stats&#95;automatic&#95;partial&#95;collection&#95;enabled</code></td><td>Enable automatic collection of <a href="/docs/v26.1/cost-based-optimizer#automatically-collect-partial-statistics">partial statistics</a> for this table. <code>sql&#95;stats&#95;automatic&#95;collection&#95;enabled</code> must be <code>true</code>.</td><td>Boolean</td><td><code>true</code></td></tr><tr><td><code>sql&#95;stats&#95;automatic&#95;partial&#95;collection&#95;min&#95;stale&#95;rows</code></td><td>Minimum number of stale rows that triggers <a href="/docs/v26.1/cost-based-optimizer#automatically-collect-partial-statistics">partial statistics collection</a> for this table.</td><td>Integer</td><td>100</td></tr><tr><td><code>sql&#95;stats&#95;automatic&#95;partial&#95;collection&#95;fraction&#95;stale&#95;rows</code></td><td>Target fraction of stale rows that triggers <a href="/docs/v26.1/cost-based-optimizer#automatically-collect-partial-statistics">partial statistics collection</a> for this table.</td><td>Float</td><td>0.05</td></tr><tr><td><code>sql&#95;stats&#95;forecasts&#95;enabled</code></td><td>Enable <a href="/docs/v26.1/show-statistics#display-forecasted-statistics">forecasted statistics</a> collection for this table.</td><td>Boolean</td><td><code>true</code></td></tr></tbody></table>
...
For the list of storage parameters that affect how [Row-Level TTL](/docs/v26.1/row-level-ttl) works, see the list of [TTL storage parameters](/docs/v26.1/row-level-ttl#ttl-storage-parameters).

### `SET LOCALITY`

`ALTER TABLE.. SET LOCALITY` changes the [table locality](/docs/v26.1/multiregion-overview#table-locality) of a [table](/docs/v26.1/create-table) in a [multi-region database](/docs/v26.1/multiregion-overview).

While CockroachDB is processing an `ALTER TABLE.. SET LOCALITY` statement that enables or disables `REGIONAL BY ROW` on a table within a database, any [`ADD REGION`](/docs/v26.1/alter-database#add-region) and [`DROP REGION`](/docs/v26.1/alter-database#drop-region) statements on that database will fail.
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>locality</code></td><td>The <code>LOCALITY</code> clause, followed by the <a href="/docs/v26.1/multiregion-overview#table-locality">locality</a> to apply to this table. Allowed values:<br/><br/>- <a href="#regional-by-table"><code>REGIONAL BY TABLE</code></a> (default)<br/><br/>- <a href="#regional-by-row"><code>REGIONAL BY ROW</code></a><br/><br/>- <a href="#global"><code>GLOBAL</code></a></td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

For more information about which table locality is right for your use case, see [Table localities](/docs/v26.1/multiregion-overview#table-locality).

### `SET SCHEMA`

`ALTER TABLE... SET SCHEMA` changes the [schema](/docs/v26.1/sql-name-resolution) of a table.

<Note>
CockroachDB supports `SET SCHEMA` as an [alias for setting the `search_path` session variable](/docs/v26.1/set-vars#supported-variables).
</Note>
...
The user must have the `DROP` [privilege](/docs/v26.1/security-reference/authorization#managing-privileges) on the table, and the `CREATE` privilege on the schema.
...
`ALTER TABLE... SCATTER` runs a specified set of ranges for a table or index through the [replication layer](/docs/v26.1/architecture/replication-layer) queue. If many ranges have been created recently, the replication queue may transfer some leases to other replicas to balance load across the cluster.
...
The user must have the `INSERT` [privilege](/docs/v26.1/security-reference/authorization#managing-privileges) on the table or index.

#### Parameters

<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>table&#95;name</code></td><td>The name of the table that you want to scatter.</td></tr><tr><td><code>table&#95;index&#95;name</code></td><td>The name of the index that you want to scatter.</td></tr><tr><td><code>expr&#95;list</code></td><td>A list of <a href="/docs/v26.1/scalar-expressions">scalar expressions</a> in the form of the primary key of the table or the specified index.</td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

### `SPLIT AT`

`ALTER TABLE... SPLIT AT` forces a [range split](/docs/v26.1/architecture/distribution-layer#range-splits) at a specified row in the table.

CockroachDB breaks data into ranges. By default, CockroachDB attempts to keep ranges below [the default range size](/docs/v26.1/configure-replication-zones#range-max-bytes). To do this, the system will automatically [split a range](/docs/v26.1/architecture/distribution-layer#range-splits) if it grows larger than this limit. For most use cases, this automatic range splitting is sufficient, and you should never need to worry about when or where the system decides to split ranges.

However, there are reasons why you may want to perform manual splits on the ranges that store tables or indexes:

- When a table only consists of a single range, all writes and reads to the table will be served by that range's [leaseholder](/docs/v26.1/architecture/replication-layer#leases). If a table only holds a small amount of data but is serving a large amount of traffic, load distribution can become unbalanced and a [hotspot](/docs/v26.1/understand-hotspots) can occur. Splitting the table's ranges manually can allow the load on the table to be more evenly distributed across multiple nodes. For tables consisting of more than a few ranges, load will naturally be distributed across multiple nodes and this will not be a concern.
- When a table is created, it will only consist of a single range. If you know that a new table will immediately receive significant write traffic, you may want to preemptively split the table based on the expected distribution of writes before applying the load. This can help avoid reduced workload performance that results when automatic splits are unable to keep up with write traffic and a [hotspot](/docs/v26.1/understand-hotspots) occurs.

For examples, see [Split and unsplit tables](#split-and-unsplit-tables).

#### Required privileges

The user must have the `INSERT` [privilege](/docs/v26.1/security-reference/authorization#managing-privileges) on the table.

#### Parameters

<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>select&#95;stmt</code></td><td>A <a href="/docs/v26.1/selection-queries">selection query</a> that produces one or more rows at which to split the table.</td></tr><tr><td><code>a&#95;expr</code></td><td>The expiration of the split enforcement on the table. This can be a <a href="/docs/v26.1/decimal"><code>DECIMAL</code></a>, <a href="/docs/v26.1/interval"><code>INTERVAL</code></a>, <a href="/docs/v26.1/timestamp"><code>TIMESTAMP</code></a>, or <a href="/docs/v26.1/timestamp"><code>TIMESTAMPZ</code></a>.</td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

### `UNSPLIT AT`

`ALTER TABLE... UNSPLIT AT` removes a [split enforcement](#split-at) on a [range split](/docs/v26.1/architecture/distribution-layer#range-splits), at a specified row in the table.

Removing a split enforcement from a table or index ("unsplitting") allows CockroachDB to merge ranges as needed, to help improve your cluster's performance. For more information, see [Range Merges](/docs/v26.1/architecture/distribution-layer#range-merges).
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>select&#95;stmt</code></td><td>A <a href="/docs/v26.1/selection-queries">selection query</a> that produces one or more rows at which to unsplit a table.</td></tr><tr><td><code>ALL</code></td><td>Remove all split enforcements for a table.</td></tr></tbody></table>

For usage, see [Synopsis](#synopsis).

### `VALIDATE CONSTRAINT`

`ALTER TABLE... VALIDATE CONSTRAINT` checks whether values in a column match a [constraint](/docs/v26.1/constraints) on the column.
...
The following examples use the [`bank` demo database schema](/docs/v26.1/cockroach-demo#datasets).

To follow along, run [`cockroach demo bank`](/docs/v26.1/cockroach-demo) to start a temporary, in-memory cluster with the `bank` schema and dataset preloaded:
...
Moving frequently updated columns to their own [column family can increase performance](/docs/v26.1/column-families#default-behavior).

To move a column from one column family to another column family, create a temporary, [non-visible](/docs/v26.1/create-table#not-visible-property) [stored computed column](/docs/v26.1/computed-columns) in the target column family, then rename the columns. Once this succeeds, you can drop the original, now renamed column.
...
3. Drop the old column:
    ...
    <Note>
    You must set the [`sql_safe_updates` session variable](/docs/v26.1/set-vars#sql-safe-updates) to `false` to drop a column in a table that has data.
    </Note>

Moving a column to another column family executes writes to the underlying storage equal to two times the number of rows. For example, if the table has 10 million rows, there will be 20 million writes to the [storage layer](/docs/v26.1/architecture/storage-layer): 10 million writes when creating the temporary stored computed column, and 10 million writes when removing the original column.
...
Adding the [`UNIQUE` constraint](/docs/v26.1/unique) requires that all of a column's values be distinct from one another (except for `NULL` values).
...
Adding the [`CHECK` constraint](/docs/v26.1/check) requires that all of a column's values evaluate to `TRUE` for a Boolean expression.
...
- If a new column has a default value or is a [computed column](/docs/v26.1/computed-columns) that would have contained values that violate the new constraint.
</Note>
...
You can include a [foreign key action](/docs/v26.1/foreign-key#foreign-key-actions) to specify what happens when a foreign key is updated or deleted.
...
<Note>
By default, referenced columns must be in the same database as the referencing foreign key column. To enable cross-database foreign key references, set the `sql.cross_db_fks.enabled` [cluster setting](/docs/v26.1/cluster-settings) to `true`.
</Note>
...
1. View the details of the `promo_codes` table:
    ...
    (1 row)
    ```
2. Add a [`NOT NULL`](/docs/v26.1/not-null) constraint to the `creation_time` column with [`ALTER COLUMN`](#alter-column):

    ```sql
    ALTER TABLE promo_codes ALTER COLUMN creation_time SET NOT NULL;
    ```
3. The `promo_codes` table is schema-locked with the [`schema_locked` table parameter](/docs/v26.1/with-storage-parameter#table-parameters), and the `DROP CONSTRAINT` and `ADD CONSTRAINT` schema changes cannot automatically unset `schema_locked`. In this case, you must manually unlock the table with `schema_locked = false`, complete the schema change, and then lock the table again with `schema_locked = true`.
    ...
    ```
4. To issue the schema change atomically, use single statements as an implicit transaction. `DROP CONSTRAINT` and `ADD CONSTRAINT` can be combined in a single `ALTER TABLE` statement:
    ...
    <Note>
    You should **not** execute the schema change with multiple statements within an explicit transaction. Refer to [Schema changes within transactions](/docs/v26.1/online-schema-changes#schema-changes-within-transactions).
    </Note>
5. Re-enable `schema_locked` on the table:

    ```sql
    ALTER TABLE promo_codes SET (schema_locked = true);
    ```
6. View the updated table structure:
...
Using [`ALTER PRIMARY KEY`](/docs/v26.1/alter-table#alter-primary-key) would have created a `UNIQUE` secondary index called `promo_codes_code_key`. Instead, there is just one index for the primary key constraint.
...
This example assumes you have a simulated multi-region database running on your local machine following the steps described in [Low Latency Reads and Writes in a Multi-Region Cluster](/docs/v26.1/demo-low-latency-multi-region-deployment). It shows how a `UNIQUE` index is partitioned, but it's similar to how all indexes are partitioned on `REGIONAL BY ROW` tables.

To show how the automatic partitioning of indexes on `REGIONAL BY ROW` tables works, we will:

1. [Add a column](/docs/v26.1/alter-table#add-column) to the `users` table in the [MovR dataset](/docs/v26.1/movr).
2. Add a [`UNIQUE` constraint](/docs/v26.1/unique) to that column.
3. Verify that the index is automatically partitioned for better multi-region performance by using [`SHOW INDEXES`](/docs/v26.1/show-index) and [`SHOW PARTITIONS`](/docs/v26.1/show-partitions).
...
Next, issue the [`SHOW INDEXES`](/docs/v26.1/show-index) statement. You will see that [the implicit region column](#set-the-table-locality-to-regional-by-row) that was added when the table [was converted to regional by row](/docs/v26.1/demo-low-latency-multi-region-deployment#configure-regional-by-row-tables) is now indexed:
...
Next, issue the [`SHOW PARTITIONS`](/docs/v26.1/show-partitions) statement. The following output (which is edited for length) will verify that the unique index was automatically [partitioned](/docs/v26.1/partitioning) for you. It shows that the `user_email_unique` index is now partitioned by the database regions `europe-west1`, `us-east1`, and `us-west1`.
...
2. Thereafter, the [optimizer](/docs/v26.1/cost-based-optimizer) will automatically add a "uniqueness check" when necessary to any [`INSERT`](/docs/v26.1/insert), [`UPDATE`](/docs/v26.1/update), or [`UPSERT`](/docs/v26.1/upsert) statement affecting the columns in the unique constraint.

Note that the [SQL engine](/docs/v26.1/architecture/sql-layer) will avoid sending requests to nodes in other regions when it can instead read a value from a unique column that is stored locally. This capability is known as [*locality optimized search*](/docs/v26.1/cost-based-optimizer#locality-optimized-search-in-multi-region-clusters).

#### Using `DEFAULT gen_random_uuid()` in `REGIONAL BY ROW` tables

To auto-generate unique row identifiers in `REGIONAL BY ROW` tables, use the [`UUID`](/docs/v26.1/uuid) column with the `gen_random_uuid()` [function](/docs/v26.1/functions-and-operators#id-generation-functions) as the [default value](/docs/v26.1/default-value):
...
<Note>
When using `DEFAULT gen_random_uuid()` on columns in `REGIONAL BY ROW` tables, uniqueness checks on those columns are disabled by default for performance purposes. CockroachDB assumes uniqueness based on the way this column generates [`UUIDs`](/docs/v26.1/uuid#create-a-table-with-auto-generated-unique-row-ids). To enable this check, you can modify the `sql.optimizer.uniqueness_checks_for_gen_random_uuid.enabled` [cluster setting](/docs/v26.1/cluster-settings). Note that while there is virtually no chance of a [collision](https://wikipedia.org/wiki/Universally_unique_identifier#Collisions) occurring when enabling this setting, it is not truly zero.
</Note>

#### Using implicit vs. explicit index partitioning in `REGIONAL BY ROW` tables

In `REGIONAL BY ROW` tables, all indexes are partitioned on the region column (usually called [`crdb_region`](/docs/v26.1/alter-table#crdb_region)).

These indexes can either include or exclude the partitioning key (`crdb_region`) as the first column in the index definition:

- If `crdb_region` is included in the index definition, a [`UNIQUE` index](/docs/v26.1/unique) will enforce uniqueness on the set of columns, just like it would in a non-partitioned table.
...
In the latter case, the index alone cannot enforce uniqueness on columns that are not a prefix of the index columns, so any time rows are [inserted](/docs/v26.1/insert) or [updated](/docs/v26.1/update) in a `REGIONAL BY ROW` table that has an implicitly partitioned `UNIQUE` index, the [optimizer](/docs/v26.1/cost-based-optimizer) must add uniqueness checks.

Whether or not to explicitly include `crdb_region` in the index definition depends on the context:

- If you only need to enforce uniqueness at the region level, then including `crdb_region` in the `UNIQUE` index definition will enforce these semantics and allow you to get better performance on [`INSERT`](/docs/v26.1/insert) s, [`UPDATE`](/docs/v26.1/update) s, and [`UPSERT`](/docs/v26.1/upsert) s, since there will not be any added latency from uniqueness checks.
- If you need to enforce global uniqueness, you should not include `crdb_region` in the `UNIQUE` (or [`PRIMARY KEY`](/docs/v26.1/primary-key) ) index definition, and the database will automatically ensure that the constraint is enforced.
...
1. Start [`cockroach demo`](/docs/v26.1/cockroach-demo) as follows:
...
3. In the following statement, we add a new user with the required `id`, `email`, and `desk_id` columns. CockroachDB needs to do additional work to enforce global uniqueness for the `id` and `email` columns, which are implicitly partitioned. This additional work is in the form of "uniqueness checks" that the optimizer adds as part of mutation queries.
    ...
    The following `EXPLAIN` output shows that the optimizer has added two `constraint-check` post queries to check the uniqueness of the implicitly partitioned indexes `id` and `email`. There is no check needed for `desk_id` (really `(crdb_region, desk_id)`), since that constraint is automatically enforced by the explicitly partitioned index we added in the preceding [`CREATE TABLE`](/docs/v26.1/create-table) statement.
...
Setting the [`DEFAULT` value constraint](/docs/v26.1/default-value) inserts the value when data's written to the table without explicitly defining the value for the column. If the column already has a `DEFAULT` value set, you can use this statement to change it.
...
If the column has a defined [`DEFAULT` value](/docs/v26.1/default-value), you can remove the constraint, which means the column will no longer insert a value by default if one is not explicitly defined for the column.
...
To specify that the column cannot contain `NULL` values, set the [`NOT NULL` constraint](/docs/v26.1/not-null).
...
If the column has the [`NOT NULL` constraint](/docs/v26.1/not-null) applied to it, you can remove the constraint, which means the column becomes optional and can have `NULL` values written into it.
...
The `full_name` column is computed from the `first_name` and `last_name` columns without the need to define a [view](/docs/v26.1/views). You can view the column details with the [`SHOW COLUMNS`](/docs/v26.1/show-columns) statement:
...
To alter the formula for a computed column, you must [`DROP`](/docs/v26.1/alter-table#drop-column) and [`ADD`](/docs/v26.1/alter-table#add-column) the column back with the new definition. Take the following table for instance:
...
The [TPC-C](/docs/v26.1/performance-benchmarking-with-tpcc-small) database has a `customer` table with a column `c_credit_lim` of type `DECIMAL(10,2)`:
...
The [TPC-C](/docs/v26.1/performance-benchmarking-with-tpcc-small) `customer` table contains a column `c_balance` of type `DECIMAL(12,2)`:
...
To specify that a column won't be returned when using `*` in a [`SELECT` clause](/docs/v26.1/select-clause), set the `NOT VISIBLE` property. You can set the `NOT VISIBLE` property only on individual columns.

For example, the `users` table of the [`movr` database](/docs/v26.1/movr) contains the `credit_card` column. If you don't want users to see that column when running `SELECT * FROM users;`, you can hide it as follows:
...
The primary key of this table is on the `name` column. This is a poor choice, as some users likely have the same name, and all primary keys enforce a `UNIQUE` constraint on row values of the primary key column. Per our [best practices](/docs/v26.1/performance-best-practices-overview#use-functions-to-generate-unique-ids), you should instead use a [`UUID`](/docs/v26.1/uuid) for single-column primary keys, and populate the rows of the table with generated, unique values.
...
You cannot `DISCARD` any zone configurations on multi-region tables, indexes, or partitions if the [multi-region abstractions](/docs/v26.1/migrate-to-multiregion-sql#replication-zone-patterns-and-multi-region-sql-abstractions) created the zone configuration.
</Note>
...
If there is data in the table, the `sql_safe_updates` [session variable](/docs/v26.1/set-vars) must be set to `false`.
...
If the column has dependent objects, such as [views](/docs/v26.1/views), CockroachDB will not drop the column by default. However, if you want to be sure of the behavior you can include the `RESTRICT` clause.
...
Now, every access of customer data is logged to the `SENSITIVE_ACCESS` channel in a [`sensitive_table_access`](/docs/v26.1/eventlog#sensitive_table_access) event that looks like the following:
...
<Note>
The preceding example shows the default [`crdb-v2`](/docs/v26.1/log-formats#format-crdb-v2) log format. This can be changed to a different format (e.g., JSON). For details, see [Configure Logs](/docs/v26.1/configure-logs#file-logging-format).
</Note>

<Tip>
For descriptions of all SQL audit event types and their fields, see [Notable Event Types](/docs/v26.1/eventlog#sql-access-audit-events).
</Tip>

To turn on auditing for more than one table, issue a separate `ALTER` statement for each table.

<Tip>
For a more detailed example, see [SQL Audit Logging](/docs/v26.1/sql-audit-logging).
</Tip>
...
<Note>
If the user running the command is not an admin user, they must own the table and be a member of the new owning role. Also, the new owner role must also have the `CREATE` [privilege](/docs/v26.1/security-reference/authorization#managing-privileges) on the schema to which the table belongs.
</Note>
...
Some subcommands can be used in combination in a single [`ALTER TABLE`](/docs/v26.1/alter-table) statement. For example, let's say you create a `users` table with 2 columns, an `id` column for the primary key and a `name` column for each user's last name:
...
Then you decide you want distinct columns for each user's first name, last name, and full name, so you execute a single `ALTER TABLE` statement renaming `name` to `last_name`, adding `first_name`, and adding a [computed column](/docs/v26.1/computed-columns) called `name` that concatenates `first_name` and `last_name`:
...
In some situations, you may want to exclude a table's row data from a [backup](/docs/v26.1/backup). For example, you have a table that contains high-churn data that you would like to [garbage collect](/docs/v26.1/architecture/storage-layer#garbage-collection) more quickly than the [incremental backup](/docs/v26.1/take-full-and-incremental-backups#incremental-backups) schedule for the database or cluster holding the table. You can use the `exclude_data_from_backup = true` parameter with a [`CREATE TABLE`](/docs/v26.1/create-table#create-a-table-with-data-excluded-from-backup) or `ALTER TABLE` statement to mark a table's row data for exclusion from a backup.

For more detail and an example through the backup and [restore](/docs/v26.1/restore) process using this parameter, see [Take Full and Incremental Backups](/docs/v26.1/take-full-and-incremental-backups#exclude-a-tables-data-from-backups).
...
<Note>
[`RESTORE`](/docs/v26.1/restore) on [`REGIONAL BY TABLE`](#regional-by-table), [`REGIONAL BY ROW`](#regional-by-row), and [`GLOBAL`](#global) tables is supported with some limitations — see [Restoring to multi-region databases](/docs/v26.1/restore#restoring-to-multi-region-databases) for more detail.
</Note>
...
For more information about how this table locality works, see [Regional tables](/docs/v26.1/table-localities#regional-tables).

#### Set the table locality to `REGIONAL BY ROW`

<Note>
Before setting the locality to `REGIONAL BY ROW` on a table targeted by a changefeed, read the considerations in [Changefeeds on regional by row tables](/docs/v26.1/changefeeds-in-multi-region-deployments).
</Note>
...
Every row in a regional by row table has a column of type `crdb_internal_region` that represents the row's [home region](/docs/v26.1/multiregion-overview#table-localities). By default, this column is called `crdb_region` and is hidden. To see a row's home region, issue a statement like the following:

```sql
SELECT crdb_region, id FROM {table};
```

To update an existing row's home region, use an [`UPDATE`](/docs/v26.1/update) statement like the following:
...
- Let CockroachDB set the row's home region automatically. It will use the region of the [gateway node](/docs/v26.1/architecture/life-of-a-distributed-transaction#gateway) from which the row is inserted.
- Set the home region explicitly using an [`INSERT`](/docs/v26.1/insert) statement like the following:
...
For more information about how this table locality works, see [Regional by row tables](/docs/v26.1/table-localities#regional-by-row-tables).
...
In fact, you can specify any column definition you like for the `REGIONAL BY ROW AS` column, as long as the column is of type `crdb_internal_region` and is not nullable. For example, you could modify the [movr schema](/docs/v26.1/movr#the-movr-database) to have a region column generated as:
...
The following instructions show how to change the mapping of the [`crdb_internal_region`](/docs/v26.1/alter-table#crdb_region) column that determines row locality for a [regional by row table](/docs/v26.1/regional-tables#regional-by-row-tables) where the [column was already defined with `REGIONAL BY ROW AS {column}`](#rename-crdb_region). This method [alters the computed column's expression](/docs/v26.1/computed-columns#alter-the-formula-for-a-computed-column).

1. [Add a new region column](/docs/v26.1/alter-table#add-column) of the same type (`crdb_internal_region`) with the updated scalar expression for the computed column:

    ```sql
    ALTER TABLE app.public.users ADD COLUMN region_new crdb_internal_region AS ({new_expression}) STORED;
    ```
2. Atomically [swap the column names](/docs/v26.1/alter-table#rename-column) so the new computed column takes the original name:

    ```sql
    ALTER TABLE app.public.users RENAME COLUMN region TO region_prev, RENAME COLUMN region_new TO region;
    ```
3. Point the table locality at the new computed column using [`ALTER TABLE... SET LOCALITY`](/docs/v26.1/alter-table#set-locality):

    ```sql
    ALTER TABLE app.public.users SET LOCALITY REGIONAL BY ROW AS region;
    ```
4. After verifying the changes have occurred (using a query like `SELECT region, * FROM app.public.users WHERE...`), [drop the previous computed column](/docs/v26.1/alter-table#drop-column):
...
The [`infer_rbr_region_col_using_constraint` table storage parameter](/docs/v26.1/with-storage-parameter#table-parameters) lets a [`REGIONAL BY ROW`](/docs/v26.1/table-localities#regional-by-row-tables) child table automatically set the hidden `crdb_region` column by looking up the referenced parent row. The parameter must be set to the name of a [foreign key](/docs/v26.1/foreign-key) constraint on the child table that includes the `crdb_region` column.
...
1. Start a [multi-node, multi-region `cockroach demo` cluster](/docs/v26.1/cockroach-demo#start-a-multi-region-demo-cluster):
...
2. In the SQL shell, create a multi-region `demo` database:

    ```sql
    CREATE DATABASE demo PRIMARY REGION "us-east1" REGIONS "us-west1", "europe-west1";
    USE demo;
    ```
3. Verify the regions that are available to the `demo` database:
    ...
    (3 rows)
    ```
4. Enable the following cluster setting:

    ```sql
    SET CLUSTER SETTING feature.infer_rbr_region_col_using_constraint.enabled = true;
    ```
5. Create a parent table that is `REGIONAL BY ROW`:
    ...
    ```
6. Create a child table whose rows should live in the same region as their parent rows. The table's foreign key **must** include `crdb_region`, and the `infer_rbr_region_col_using_constraint` parameter must be set to the name of the foreign key (in this example, `fk_parent`):
    ...
    CREATE TABLE child (
      ...
      LOCALITY REGIONAL BY ROW;
    ```
7. Insert a row into the parent table that lives in the primary region (`us-east1`):

    ```sql
    INSERT INTO parent (id, data, crdb_region) VALUES (1, 'east row', 'us-east1');
    ```
8. Run the following command to view IDs and connection strings for all nodes in the cluster:
    ...
    node 9:
      ...
      Application tenant:
       ...
       (rpc)      127.0.0.1:26365
    ```
9. Follow one of the `webui` URLs in the preceding output and open the [**Network Latency** Page](/docs/v26.1/ui-network-latency-page) in the DB Console. This page displays the region associated with each node ID.
    ...
    ```
10. Verify that the node you're connected to is in a non-primary region:
    ...
    ```
11. Insert a row into the child table from the node you're connected to:

    ```sql
    INSERT INTO child (id, parent_id, info) VALUES (10, 1, 'hello from gateway');
    ```
12. View the value of the hidden `crdb_region` column. Even though the row was inserted from a non-primary gateway region, CockroachDB uses the foreign-key constraint to look up and copy the parent row's region into the child row:
    ...
    -----+-----------+--------------
      10 |         1 | us-east1
    ```
13. If you later need to disable the behavior, [reset the storage parameter](#reset-storage-parameter):
...
When auto-rehoming is enabled, the [home regions](#crdb_region) of rows in [`REGIONAL BY ROW`](#set-the-table-locality-to-regional-by-row) tables are automatically set to the gateway region of any [`UPDATE`](/docs/v26.1/update) or [`UPSERT`](/docs/v26.1/upsert) statements that write to those rows. This functionality is provided by adding an [`ON UPDATE` expression](/docs/v26.1/create-table#on-update-expressions) to the [`crdb_region`](#crdb_region) column of newly created regional by row tables.

To enable auto-rehoming using the [session setting](/docs/v26.1/set-vars), issue the following statement:
...
- The [`crdb_region`](#crdb_region) column from a [`REGIONAL BY ROW`](#set-the-table-locality-to-regional-by-row) table cannot be referenced as a [foreign key](/docs/v26.1/foreign-key) from another table. This also means the [`infer_rbr_region_col_using_constraint` table storage parameter](/docs/v26.1/with-storage-parameter#table-parameters) cannot be used to [infer a row's home region](#infer-a-rows-home-region-from-a-foreign-key).
...
1. Follow steps 1 and 2 from the [Low Latency Reads and Writes in a Multi-Region Cluster](/docs/v26.1/demo-low-latency-multi-region-deployment) tutorial. This will involve starting a [`cockroach demo`](/docs/v26.1/cockroach-demo) cluster in a terminal window (call it *terminal 1*).
2. From the [SQL client](/docs/v26.1/cockroach-sql) running in terminal 1, set the setting that enables auto-rehoming. You must issue this setting before creating the `REGIONAL BY ROW` tables that you want auto-rehomed.

    ```sql
    SET enable_auto_rehoming = on;
    ```
3. In a second terminal window (call it *terminal 2*), [finish the tutorial starting from step 3](/docs/v26.1/demo-low-latency-multi-region-deployment#step-3-load-and-run-movr) onward to finish loading the cluster with data and applying the multi-region SQL configuration.
4. Switch back to terminal 1, and check the gateway region of the node you are currently connected to:
    ...
    (1 row)
    ```
5. Open another terminal (call it *terminal 3*), and use [`cockroach sql`](/docs/v26.1/cockroach-sql) to connect to a node in a different region in the demo cluster:
    ...
    # To exit, type: \q.
    #
    # Server version: CockroachDB CCL v26.1.1 (x86_64-apple-darwin19, built 2026-03-09 00:00:00) (same version as client)
...
For more information about how this table locality works, see [Global tables](/docs/v26.1/table-localities#global-tables).
...
By default, [unqualified tables](/docs/v26.1/sql-name-resolution#lookup-with-unqualified-names) created in the database belong to the `public` schema:
...
If the new schema does not already exist, [create it](/docs/v26.1/create-schema):
...
The table's compound primary key is on the `city` and `dl` columns. Note that the table automatically generates an `id` and a `dl` value [using supported SQL functions](/docs/v26.1/functions-and-operators) if they are not provided.
...
You can specify the time at which a split enforcement expires by adding a `WITH EXPIRATION` clause to your `SPLIT` statement. Supported expiration values include [`DECIMAL`](/docs/v26.1/decimal), [`INTERVAL`](/docs/v26.1/interval), [`TIMESTAMP`](/docs/v26.1/timestamp), and [`TIMESTAMPZ`](/docs/v26.1/timestamp).
...
You can see the split's expiration date in the `split_enforced_until` column. The [`crdb_internal.ranges`](/docs/v26.1/crdb-internal) table also contains information about ranges in your CockroachDB cluster, including the `split_enforced_until` column.
...
<Note>
If present in a [`CREATE TABLE`](/docs/v26.1/create-table) statement, the table is considered validated because an empty table trivially meets its constraints.
</Note>

### Enable row-level security

To enable [row-level security](/docs/v26.1/row-level-security) (RLS) on a table, issue the following statement:
...
To ensure that all access, including by the table [owner](/docs/v26.1/security-reference/authorization#object-ownership), adheres to the defined [row-level security](/docs/v26.1/row-level-security) policies, issue the following statement:
...
- [SQL Audit Logging](/docs/v26.1/sql-audit-logging)
- [`CREATE TABLE`](/docs/v26.1/create-table)
- [`CREATE INDEX`](/docs/v26.1/create-index)
- [`ALTER INDEX`](/docs/v26.1/alter-index)
- [`ALTER PARTITION`](/docs/v26.1/alter-partition)
- [`SHOW JOBS`](/docs/v26.1/show-jobs)
- [`BACKUP`](/docs/v26.1/backup)
- [`RESTORE`](/docs/v26.1/restore)
- [`WITH` (storage parameter)](/docs/v26.1/with-storage-parameter)
- [Selection Queries](/docs/v26.1/selection-queries)
- [Distribution Layer](/docs/v26.1/architecture/distribution-layer)
