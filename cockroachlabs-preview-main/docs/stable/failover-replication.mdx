...
- [Point-in-time](#fail-over-to-a-point-in-time):
    - Past: A past timestamp within the [failover window](/docs/v26.1/physical-cluster-replication-technical-overview#failover-and-promotion-process) of up to 4 hours in the past.
...
1. To view the current replication timestamp, use:
    ...
    <Tip>
    You can view the [**Replication Lag** graph](/docs/v26.1/ui-physical-cluster-replication-dashboard#replication-lag) in the standby cluster's DB Console.
    </Tip>
...
1. To select a [specific time](/docs/v26.1/as-of-system-time) in the past, use:
...
2. Specify a timestamp:
    ...
    Refer to [Using different timestamp formats](/docs/v26.1/as-of-system-time#using-different-timestamp-formats) for more information.
...
2. Run `SHOW JOB WHEN COMPLETE job_id`. Refer to the `SHOW JOBS` page for [details](/docs/v26.1/show-jobs#parameters) and an [example](/docs/v26.1/show-jobs#show-job-when-complete).
</Note>
...
2. Once complete, bring the standby's virtual cluster online with:
    ...
    (3 rows)
    ```
3. To make the standby's virtual cluster the default for connection strings, set the following [cluster setting](/docs/v26.1/cluster-settings):
...
<Note>
To move back to a different cluster that was not involved in the original PCR stream, set up a new PCR stream following the PCR [setup](/docs/v26.1/set-up-physical-cluster-replication) guide.
</Note>
...
2. From the system virtual cluster on **Cluster A**, ensure that service to the virtual cluster has stopped:
    ...
    ```
3. Open another terminal window and generate a connection string for **Cluster B** using [`cockroach encode-uri`](/docs/v26.1/set-up-physical-cluster-replication#step-3-manage-cluster-certificates-and-generate-connection-strings):
    ...
    <Tip>
    For details on connection strings, refer to the [Connection reference](/docs/v26.1/set-up-physical-cluster-replication#connection-reference).
    </Tip>
...
6. From the system virtual cluster on **Cluster A**, start the replication from **Cluster B** to **Cluster A**. Include the connection string for **Cluster B**:
    ...
    This will reset the virtual cluster on **Cluster A** back to the time at which the same virtual cluster on **Cluster B** diverged from it. **Cluster A** will check with **Cluster B** to confirm that its virtual cluster was replicated from **Cluster A** as part of the original [PCR stream](/docs/v26.1/set-up-physical-cluster-replication).

    <Note>
    ([**Preview**](/docs/v26.1/cockroachdb-feature-availability#features-in-preview)) If you want to start the PCR stream with a read-only virtual cluster on the standby after failing back to the original primary cluster, run the [`ALTER VIRTUAL CLUSTER`](/docs/v26.1/alter-virtual-cluster) statement in this step with the `READ VIRTUAL CLUSTER` option.
    </Note>
...
9. From **Cluster A**, bring the virtual cluster online:
    ...
    ```
10. To make **Cluster A's** virtual cluster the default for [connection strings](/docs/v26.1/work-with-virtual-clusters#sql-clients), set the following [cluster setting](/docs/v26.1/cluster-settings):

    ```sql
    SET CLUSTER SETTING server.controller.default_target_cluster='{cluster_a}';
    ```

At this point, **Cluster A** has caught up to **Cluster B**. The clusters are entirely independent. To enable PCR again from the primary to the standby, refer to [Set Up Physical Cluster Replication](/docs/v26.1/set-up-physical-cluster-replication).

### Fail back after replicating from an existing primary cluster

You can replicate data from an existing CockroachDB cluster that does not have [cluster virtualization](/docs/v26.1/cluster-virtualization-overview) enabled to a standby cluster with cluster virtualization enabled. For instructions on setting up a PCR in this way, refer to [Set up PCR from an existing cluster](/docs/v26.1/set-up-physical-cluster-replication#set-up-pcr-from-an-existing-cluster).
...
4. You need to create a standby cluster for cluster **B** to replicate changes to. You can do one of the following:
    - [Create a new virtual cluster](/docs/v26.1/set-up-physical-cluster-replication#step-4-start-replication) ( `main` ) on cluster **A** from the replication of cluster **B**. Cluster **A** is now virtualized. This will start an initial scan because the PCR stream will ignore the former workload tables in the system virtual cluster that were [originally replicated to **B**](/docs/v26.1/set-up-physical-cluster-replication#set-up-pcr-from-an-existing-cluster). You can [drop the tables](/docs/v26.1/drop-table) that were in the system virtual cluster, because the new virtual cluster will now hold the workload replicating from cluster **B**.
    - [Start an entirely new cluster](/docs/v26.1/cockroach-start) **C** and [create](/docs/v26.1/set-up-physical-cluster-replication#step-4-start-replication) a `main` virtual cluster on it from the replication of cluster **B**. This will start an initial scan because cluster **C** is empty.
...
[Backup schedules](/docs/v26.1/manage-a-backup-schedule) pause after failover on the promoted standby cluster. Take the following steps to resume jobs:

1. Verify that there are no other schedules running backups to the same [collection of backups](/docs/v26.1/take-full-and-incremental-backups#backup-collections), i.e., the schedule that was running on the original primary cluster.
2. [Resume](/docs/v26.1/resume-schedules) the backup schedule on the promoted cluster.

<Note>
If your backup schedule was created on a cluster in v23.1 or earlier, it will **not** pause automatically on the promoted cluster after failover. In this case, you must [pause](/docs/v26.1/pause-schedules) the schedule manually on the promoted cluster and then take the outlined steps.
</Note>

### Changefeeds

Currently running [changefeeds](/docs/v26.1/change-data-capture-overview) will fail on the promoted cluster immediately after failover to avoid two clusters running the same changefeed to one sink. We recommend that you recreate changefeeds on the promoted cluster.

To avoid multiple clusters running the same schedule concurrently, [changefeed schedules](/docs/v26.1/create-schedule-for-changefeed) will [pause](/docs/v26.1/pause-schedules) after physical cluster replication has completed.

<Note>
If your changefeed schedule was created on a cluster in v24.1 or earlier, it will **not** pause automatically on the promoted cluster after failover. In this case, you will need to manage [pausing](/docs/v26.1/pause-schedules) or [canceling](/docs/v26.1/drop-schedules) the schedule on the promoted standby cluster to avoid two clusters running the same changefeed to one sink.
</Note>
