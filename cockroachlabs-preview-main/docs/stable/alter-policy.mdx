...
The `ALTER POLICY` statement changes an existing [row-level security (RLS)](/docs/v26.1/row-level-security) policy on a table.

Allowed changes to a policy using `ALTER POLICY` include:

- Rename the policy.
- Change the applicable [roles](/docs/v26.1/security-reference/authorization#roles).
- Modify the [`USING` expression](#parameters).
- Modify the [`WITH CHECK` expression](#parameters).

<Note>
You cannot use `ALTER POLICY` to change the `PERMISSIVE`, `RESTRICTIVE`, or `FOR` clauses of a policy, as defined in `CREATE POLICY... ON... ( PERMISSIVE | RESTRICTIVE )... FOR ( ALL | SELECT |... )`. To make these changes, drop the policy with [`DROP POLICY`](/docs/v26.1/drop-policy) and issue a new [`CREATE POLICY`](/docs/v26.1/create-policy) statement.
</Note>
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>policy&#95;name</code></td><td>The identifier of the existing policy to be modified. Must be unique for the specified <code>table&#95;name</code>.</td></tr><tr><td><code>ON table&#95;name</code></td><td>The name of the table on which the policy <code>policy&#95;name</code> is defined.</td></tr><tr><td><code>new&#95;policy&#95;name</code></td><td>The new identifier for the policy. The <code>new&#95;policy&#95;name</code> must be a unique name on <code>table&#95;name</code>.</td></tr><tr><td>`TO (role&#95;name</td><td>PUBLIC</td></tr><tr><td><code>USING ( using&#95;expression )</code></td><td>Replaces the previous value of this expression. For details about this expression, refer to <a href="/docs/v26.1/create-policy#parameters"><code>CREATE POLICY</code></a>.</td></tr><tr><td><code>WITH CHECK ( check&#95;expression )</code></td><td>Replaces the previous value of this expression. For details about this expression, refer to <a href="/docs/v26.1/create-policy#parameters"><code>CREATE POLICY</code></a>.</td></tr></tbody></table>
