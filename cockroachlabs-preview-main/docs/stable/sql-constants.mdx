...
Bit array literals are acceptable both when values of types [`BIT`](/docs/v26.1/bit) or [`VARBIT`](/docs/v26.1/bit) (`BIT VARYING`) are expected.
...
Additionally, for compatibility with PostgreSQL, the notation `'string'::type` and `CAST('string' AS type)` is also recognized as an interpreted literal. These are special cases of [cast expressions](/docs/v26.1/scalar-expressions).

For more information about the allowable format of interpreted literals, refer to the "Syntax" section of the respective data types: [`DATE`](/docs/v26.1/date#syntax), [`INET`](/docs/v26.1/inet#syntax), [`INTERVAL`](/docs/v26.1/interval#syntax), [`TIME`](/docs/v26.1/time#syntax), [`TIMESTAMP`/`TIMESTAMPTZ`](/docs/v26.1/timestamp#syntax).
