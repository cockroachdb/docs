...
If you manage users through a service compatible with the Lightweight Directory Access Protocol (LDAP), such as Active Directory or Microsoft Entra ID, you can configure CockroachDB to automatically assign [roles](/docs/v26.1/security-reference/authorization) to users based on LDAP group memberships, simplifying access control.
...
4. In conjunction with [automatic user provisioning](/docs/v26.1/ldap-authentication#option-1-automatic-user-provisioning-recommended) if enabled, users are created automatically during their first authentication and simultaneously receive specified role memberships.

## Prerequisites

- Enable [LDAP Authentication](/docs/v26.1/ldap-authentication).
...
Add the `ldapgrouplistfilter` parameter to the HBA configuration that you enabled for [LDAP Authentication](/docs/v26.1/ldap-authentication). The configuration will include two important LDAP filters:
...
Create CockroachDB roles that match your LDAP group names and grant appropriate privileges to each role. Remember that role names must comply with CockroachDB's [identifier requirements](/docs/v26.1/create-user#user-names).
...
<Note>
If you are going to use [automatic user provisioning](/docs/v26.1/ldap-authentication#option-1-automatic-user-provisioning-recommended) in conjunction with LDAP authorization, be sure to complete the creation of group roles before enabling automatic user provisioning. Auto-provisioned users will only receive roles for groups that already exist as CockroachDB roles.
</Note>

### Step 3: Confirm configuration

1. On the LDAP server, set up test users with memberships in groups that should be synced to CockroachDB users.
2. If [automatic user provisioning](/docs/v26.1/ldap-authentication#option-1-automatic-user-provisioning-recommended) is not enabled, create the matching test users when logged in as an admin to CockroachDB:
...
3. Log in to CockroachDB as each test user (refer to [Connect to a cluster using LDAP](/docs/v26.1/ldap-authentication#connect-to-a-cluster-using-ldap)).
