...
The `SERIAL` pseudo [data type](/docs/v26.1/data-types) is a keyword that can be used *in lieu* of a real data type when defining table columns. It is approximately equivalent to using an [integer type](/docs/v26.1/int) with a [`DEFAULT` expression](/docs/v26.1/default-value) that generates different values every time it is evaluated. This default expression in turn ensures that inserts that do not specify this column will receive an automatically generated value instead of `NULL`.
...
In most cases, we recommend using the [`UUID`](/docs/v26.1/uuid) data type with the `gen_random_uuid()` function as the default value, which generates 128-bit values (larger than `SERIAL`'s maximum of 64 bits) and more uniformly scatters them across all of a table's underlying key-value ranges. UUIDs ensure more effectively that multiple nodes share the insert load when a UUID column is used in an index or primary key.

See [this FAQ entry](/docs/v26.1/sql-faqs#how-do-i-auto-generate-unique-row-ids-in-cockroachdb) for more details.
</Note>

## Modes of operation

The keyword `SERIAL` is recognized in `CREATE TABLE` and is automatically translated to a real data type and a [`DEFAULT` expression](/docs/v26.1/default-value) during table creation. The result of this translation is then used internally by CockroachDB, and can be observed using [`SHOW CREATE`](/docs/v26.1/show-create).
...
In both modes `rowid` and `virtual_sequence`, a value is automatically generated using the `unique_rowid()` function. The difference between `rowid` and `virtual_sequence` is that the latter setting also creates a virtual (pseudo) sequence in the database. However, in both cases the `unique_rowid()` function is ultimately used to generate new values. This function produces a 64-bit integer (i.e., [`INT8`](/docs/v26.1/int)) from the current timestamp and ID of the node executing the [`INSERT`](/docs/v26.1/insert) or [`UPSERT`](/docs/v26.1/upsert) operation. This behavior is statistically likely to be globally unique except in extreme cases (see [this FAQ entry](/docs/v26.1/sql-faqs#how-do-i-auto-generate-unique-row-ids-in-cockroachdb) for more details).
...
<Note>
Values generated by the `unique_rowid()` function do not respect the `default_int_size` [session variable](/docs/v26.1/set-vars) or `sql.defaults.default_int_size` [cluster setting](/docs/v26.1/cluster-settings).
</Note>

### Generated values for mode `sql_sequence` and `sql_sequence_cached`

In both modes, a regular [SQL sequence](/docs/v26.1/create-sequence) is automatically created alongside the table where `SERIAL` is specified.
...
Because SQL sequences persist the current sequence value in the database, inter-node coordination is required when multiple clients use the sequence concurrently via different nodes. This can cause [contention](/docs/v26.1/sql-faqs#what-is-transaction-contention) and impact performance negatively.

Therefore, applications should consider using `unique_rowid()` or `gen_random_uuid()` as discussed in [this FAQ entry](/docs/v26.1/sql-faqs#how-do-i-auto-generate-unique-row-ids-in-cockroachdb) instead of sequences when possible.
...
In this mode, a value is generated using the `unordered_unique_rowid()` [function](/docs/v26.1/functions-and-operators#id-generation-functions). This function is similar to the [`rowid` mode](#generated-values-for-modes-rowid-and-virtual_sequence) but does not guarantee ordering.
...
The [`SHOW COLUMNS`](/docs/v26.1/show-columns) statement shows that the `SERIAL` type is just an alias for `INT` with `unique_rowid()` as the default.
...
- [FAQ: How do I auto-generate unique row IDs in CockroachDB?](/docs/v26.1/sql-faqs#how-do-i-auto-generate-unique-row-ids-in-cockroachdb)
- [Data Types](/docs/v26.1/data-types)
- [`UUID`](/docs/v26.1/uuid)
