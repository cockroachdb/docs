...
import { CreateFunction } from "/snippets/syntax-diagrams/CreateFunction.jsx";

The `CREATE FUNCTION` [statement](/docs/v26.1/sql-statements) creates a [user-defined function](/docs/v26.1/user-defined-functions).

<Note>
The `CREATE FUNCTION` statement performs a schema change. For more information about how online schema changes work in CockroachDB, see [Online Schema Changes](/docs/v26.1/online-schema-changes).
</Note>

## Required privileges

- To create a function, a user must have [`CREATE` privilege](/docs/v26.1/security-reference/authorization#supported-privileges) on the schema of the function. The user must also have privileges on all the objects referenced in the function body.
- To define a function with a [user-defined type](/docs/v26.1/create-type), a user must have `USAGE` privilege on the user-defined type.
...
<table><thead><tr><th>Parameter</th><th>Description</th></tr></thead><tbody><tr><td><code>routine&#95;create&#95;name</code></td><td>The name of the function.</td></tr><tr><td><code>routine&#95;param</code></td><td>A comma-separated list of function parameters, specifying the mode, name, and type.</td></tr><tr><td><code>routine&#95;return&#95;type</code></td><td>The type returned by the function: any built-in <a href="/docs/v26.1/data-types">SQL type</a>, user-defined <a href="/docs/v26.1/enum"><code>ENUM</code></a> or <a href="/docs/v26.1/create-type#create-a-composite-data-type">composite</a> type, <a href="#create-a-function-that-returns-a-record-type"><code>RECORD</code></a>, <a href="#create-a-function-that-returns-a-table"><code>TABLE</code></a>, PL/pgSQL <a href="/docs/v26.1/plpgsql#declare-cursor-variables"><code>REFCURSOR</code></a> type, <a href="/docs/v26.1/triggers#trigger-function"><code>TRIGGER</code></a>, or <code>VOID</code>.</td></tr><tr><td><code>routine&#95;body&#95;str</code></td><td>The body of the function. For allowed contents, refer to <a href="/docs/v26.1/user-defined-functions#overview">User-Defined Functions</a>.</td></tr></tbody></table>
...
The `RETURNS TABLE` clause specifies the column names to output: `id`, `name`, `city`, and `end_time`. A [common table expression](/docs/v26.1/common-table-expressions) reads the most recent rides from the `rides` table.
...
A trigger function is a [function that is executed by a trigger](/docs/v26.1/triggers#trigger-function). A trigger function must return type `TRIGGER` and is written in [PL/pgSQL](/docs/v26.1/plpgsql).
...
The preceding example modifies a given `name` value and returns the `NEW` [trigger variable](/docs/v26.1/triggers#trigger-variables) because it is meant to be executed by a `BEFORE` trigger. For details, refer to [Triggers](/docs/v26.1/triggers).
...
1. Create two roles:
    ...
    CREATE ROLE invoker;
    ```
2. Grant a [`SELECT` privilege](/docs/v26.1/grant#supported-privileges) on the `user_promo_codes` table to the `owner` role.
...
4. Create a simple `SECURITY DEFINER` function that reads the contents of `user_promo_codes`.
    ...
    CREATE OR REPLACE FUNCTION get_codes() 
      ...
      $$;
    ```
5. Grant the [`EXECUTE` privilege](/docs/v26.1/grant#supported-privileges) on the `get_codes` function to the `invoker` role.
