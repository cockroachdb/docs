...
Specify the labels in `cockroachdb.crdbCluster.podTemplate.spec.nodeSelector` in the values file used to [deploy the cluster](/docs/v26.1/deploy-cockroachdb-with-cockroachdb-operator#initialize-the-cluster). If you specify multiple `nodeSelector` labels, the node must match all of them.
...
Specify node affinities in `cockroachdb.crdbCluster.podTemplate.spec.affinity.nodeAffinity` in the values file used to [deploy the cluster](/docs/v26.1/deploy-cockroachdb-with-cockroachdb-operator#initialize-the-cluster). If you specify multiple `matchExpressions` labels, the node must match all of them. If you specify multiple `values` for a label, the node can match any of the values.
...
The CockroachDB operator hard-codes the pod template to only allow one pod per Kubernetes node. If you need to override this value, you can [override the pod template](/docs/v26.1/override-templates-cockroachdb-operator#override-the-default-pod).
...
3. Add the following rules to the values file used to [deploy the cluster](/docs/v26.1/deploy-cockroachdb-with-cockroachdb-operator#initialize-the-cluster):
...
Specify pod tolerations in the `cockroachdb.crdbCluster.podTemplate.spec.tolerations` object in the values file used to [deploy the cluster](/docs/v26.1/deploy-cockroachdb-with-cockroachdb-operator#initialize-the-cluster).
...
A `NoSchedule` taint on a node prevents pods from being scheduled onto the node. The matching toleration allows a pod to be scheduled onto the node. A `NoSchedule` toleration is therefore best included before [deploying the cluster](/docs/v26.1/deploy-cockroachdb-with-cockroachdb-operator#initialize-the-cluster).
...
2. Add a taint to a running worker node:
    ...
    ```
3. Add a matching tolerations object in the values file used to [deploy the cluster](/docs/v26.1/deploy-cockroachdb-with-cockroachdb-operator#initialize-the-cluster).
...
Specify pod topology spread constraints in the `cockroachdb.crdbCluster.podTemplate.spec.topologySpreadConstraints` object of the values file used to [deploy the cluster](/docs/v26.1/deploy-cockroachdb-with-cockroachdb-operator#initialize-the-cluster). If you specify multiple `topologySpreadConstraints` objects, the matching pods must satisfy all of the constraints.
...
Specify labels in `cockroachdb.crdbCluster.podTemplate.metadata.labels` and annotations in `cockroachdb.crdbCluster.podTemplate.metadata.annotations` in the values file used to [deploy the cluster](/docs/v26.1/deploy-cockroachdb-with-cockroachdb-operator#initialize-the-cluster):
