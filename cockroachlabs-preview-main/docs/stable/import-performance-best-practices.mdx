...
1. [`CSV`](/docs/v26.1/migrate-from-csv) or [`DELIMITED DATA`](/docs/v26.1/import-into) (both have about the same import performance).
2. [`AVRO`](/docs/v26.1/migrate-from-avro).

We recommend formatting your import files as `CSV` or `AVRO`. These formats can be processed in parallel by all the nodes in the cluster, which increases performance. To import in these formats, use [`IMPORT INTO`](/docs/v26.1/import-into).
...
Convert the schema-only file using the [Schema Conversion Tool](/docs/stable/cockroachcloud/migrations-page). The Schema Conversion Tool automatically creates a new CockroachDB Cloud database with the converted schema. If you are migrating to a CockroachDB self-hosted database, you can [export the converted schema](/docs/stable/cockroachcloud/migrations-page#export-the-schema) from the Schema Conversion Tool and execute the statements in [`cockroach sql`](/docs/v26.1/cockroach-sql), or use a third-party schema migration tool such as [Alembic](/docs/v26.1/alembic), [Flyway](/docs/v26.1/flyway), or [Liquibase](/docs/v26.1/liquibase).

Then use the [`IMPORT INTO`](/docs/v26.1/import-into) statement to import the CSV data into the newly created table:
...
When importing data into a table with [foreign keys](/docs/v26.1/foreign-key), temporarily remove the foreign keys, and add them after the initial migration with an [`ALTER TABLE` statement](/docs/v26.1/alter-table#add-the-foreign-key-constraint-with-cascade).

### Data type sizes

Above a certain size, many data types such as [`STRING`](/docs/v26.1/string)s, [`DECIMAL`](/docs/v26.1/decimal)s, [`ARRAY`](/docs/v26.1/array), [`BYTES`](/docs/v26.1/bytes), and [`JSONB`](/docs/v26.1/jsonb) may run into performance issues due to [write amplification](/docs/v26.1/architecture/storage-layer#write-amplification). See each data type's documentation for its recommended size limits.
...
For example, if you have a 3-node cluster, split your data into at least 3 files, create your table schema, and [import into that table](/docs/v26.1/import-into):
...
You should split the files so that the first file contains rows where the [primary key](/docs/v26.1/primary-key) begins with the letters "a" to "i," the second file "j" to "r," and the third file "s" to "z". No file should contain duplicate rows, and within each file the data is sorted alphabetically by the primary key.
...
During migration, all of the features of [`IMPORT INTO`](/docs/v26.1/import-into) that interact with external file storage assume that every node has the exact same view of that storage. In other words, in order to import from a file, every node needs to have the same access to that file.
...
- [Migrate from Oracle](/docs/v26.1/migrate-from-oracle)
...
- [Migrate from Avro](/docs/v26.1/migrate-from-avro)
