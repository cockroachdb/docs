...
CockroachDB multi-region capabilities make it easier to run global applications. For an overview of these capabilities, see [Multi-Region Capabilities Overview](/docs/v26.1/multiregion-overview).

In multi-region clusters, the distribution of data becomes a performance consideration. This makes it important to think about the [survival goals](/docs/v26.1/multiregion-overview#survival-goals) of each database. Then, for each table in the database, use the right [table locality](/docs/v26.1/multiregion-overview#table-locality) to locate data for optimal performance.

In this tutorial, you will:

1. Simulate a multi-region CockroachDB cluster on your local machine.
2. Run a workload on the cluster using our fictional vehicle-sharing application called [MovR](/docs/v26.1/movr).
3. See the effects of network latency on SQL query performance in the default (non-multi-region) configuration.
4. Configure the cluster for good multi-region performance by issuing SQL statements that choose the right [survival goals](/docs/v26.1/multiregion-overview#survival-goals) and [table localities](/docs/v26.1/multiregion-overview#table-locality).

## Considerations

This page describes a demo cluster; it does not show best practices for a production deployment. For more information about production deployments of multi-region applications, see [Orchestrate CockroachDB Across Multiple Kubernetes Clusters](/docs/v26.1/orchestrate-cockroachdb-with-kubernetes-multi-cluster) and the [Production Checklist](/docs/v26.1/recommended-production-settings).

Because the instructions on this page describe how to simulate a multi-region cluster on a single machine, the absolute performance numbers described below are not reflective of [the performance you can expect of single-point reads and writes against CockroachDB in a production setting](/docs/v26.1/frequently-asked-questions#single-row-perf). Instead, the instructions are designed with the following goals:
...
The workload you'll run against the cluster is our open-source, fictional, peer-to-peer vehicle-sharing app, [MovR](/docs/v26.1/movr). Each instance represents users in a specific region:
...
All of the tables except `promo_codes` have a composite primary key of `city` and `id`, in that order. This means that the rows in these tables are ordered by their geography. These tables are read from and written to very frequently. To keep read and write latency low, you'll use the [`REGIONAL BY ROW` table locality pattern](/docs/v26.1/table-localities#regional-by-row-tables) for these tables.

The data in the `promo_codes` table is different: it is not tied to geography, and it is rarely updated. This type of table is often referred to as a "reference table" or "lookup table". In this case, you'll use the [Global table locality pattern](/docs/v26.1/table-localities#global-tables) to keep read latencies low.

For a description of the sequence of SQL statements issued by the MovR application in response to user actions, see [How the MovR application works](/docs/v26.1/movr#how-the-movr-application-works).
...
- Viewing the [DB Console](/docs/v26.1/ui-overview): `http://127.0.0.1:8080`.
- Connecting to the database from a [SQL shell](/docs/v26.1/cockroach-sql) or a [programming language](/docs/v26.1/connect-to-the-database): `postgres://root@127.0.0.1:26257?sslmode=disable`.
...
To verify that the simulated latencies are working as expected, check the [Network Latency Page](/docs/v26.1/ui-network-latency-page) in the DB Console. Round trip times between `us-west1` and `europe-west1` should be in the 150 ms range.
...
<Frame>
  ![Geo-partitioning network latency](/images/v26.1/geo-partitioning-network-latency.png)
</Frame>
...
In the [DB Console](/docs/v26.1/ui-overview) at [http://127.0.0.1:8080](http://127.0.0.1:8080/), click [**Metrics**](/docs/v26.1/ui-overview-dashboard) on the left and hover over the [**Service Latency: SQL, 99th percentile**](/docs/v26.1/ui-overview-dashboard#service-latency-sql-99th-percentile) timeseries graph. You should see the effects of network latency on this workload.

<Frame>
  ![Geo-partitioning SQL latency](/images/v26.1/geo-partitioning-sql-latency-before.png)
</Frame>
...
To see the network latency between any two nodes in the cluster, click [**Network Latency**](/docs/v26.1/ui-network-latency-page) in the left-hand navigation.
...
This information is necessary so that CockroachDB can move data around to optimize access to particular data from particular regions. The main focus is reducing latency in a global deployment. For more information about how this works at a high level, see the [Multi-Region Capabilities Overview](/docs/v26.1/multiregion-overview).
