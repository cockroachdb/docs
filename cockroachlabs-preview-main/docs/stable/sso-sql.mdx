...
CockroachDB clusters allow users to authenticate with Single Sign-on (SSO), both to the [DB Console](/docs/v26.1/ui-overview), and for SQL client access.
...
- **Cluster Settings Access**: You must have the ability to update your cluster settings. Refer to [`SET CLUSTER SETTING`: Required permissions](/docs/v26.1/set-cluster-setting#required-privileges).
- **SQL User Provisioning**:
  ...
  - If automatic user provisioning is disabled, a SQL user that corresponds with your external identity must be pre-created on the cluster. To create users, you must have access to the [`admin` role](/docs/v26.1/security-reference/authorization#admin-role).
- **(Optional) DB Console JWT Generation**: To use the DB Console to generate JWTs (instead of obtaining them directly from your IdP), you must have your cluster pre-configured for OIDC/SSO authentication for DB Console. Use the [Single Sign-on (SSO) for DB Console](/docs/v26.1/sso-db-console) guide to set this up.
- **(Optional) JWT Authorization**: To enable automatic role synchronization based on IdP group claims, see [JWT Authorization](/docs/v26.1/jwt-authorization).

## Configure your cluster for SSO

### Cluster Settings

You must configure the [cluster settings](/docs/v26.1/cluster-settings) in the following table to enable JWT authentication to your cluster. Refer to the [Update your cluster settings](#update-your-cluster-settings) section to configure your cluster settings.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        <a href="/docs/v26.1/sso-db-console">Single Sign-on (SSO) for DB Console</a>.
      </td>
    </tr>
    <tr>
      <td>
    ...
    <tr>
      ...
      <td>
        Enables automatic role synchronization based on JWT groups claim. See{" "}
        <a href="/docs/v26.1/jwt-authorization">JWT Authorization</a>. Defaults to <code>false</code>.
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
        JWT field containing groups for authorization. Defaults to <code>groups</code>. See{" "}
        <a href="/docs/v26.1/jwt-authorization">JWT Authorization</a>.
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
        Userinfo endpoint JSON key for groups fallback. Defaults to <code>groups</code>. See{" "}
        <a href="/docs/v26.1/jwt-authorization">JWT Authorization</a>.
      </td>
    </tr>
    <tr>
      ...
      <td>
        Enables automatic user creation on first JWT login. Defaults to <code>false</code>. See{" "}
...
<Note>
  [Single Sign-on (SSO) for DB Console](/docs/v26.1/sso-db-console) and cluster SSO cannot be configured with different
  IdPs.
</Note>
...
3. Add your IdP's issuer URL to your cluster's list of accepted token issuers. If you have already configured [Single Sign-on (SSO) for DB Console](/docs/v26.1/sso-db-console#cluster-settings), the issuer URL must match the value of `server.oidc_authentication.provider_url`.

   ```sql
   SET CLUSTER SETTING server.jwt_authentication.issuers.configuration = 'https://cockroachlabs.cloud';
   ```

4. Configure your cluster's audience, which is your cluster's ID (or JSON array of IDs) in your IdP. The audience is a mechanism for limiting the clients that can obtain JWTs from your IdP. If you have already configured [Single Sign-on (SSO) for DB Console](/docs/v26.1/sso-db-console#cluster-settings), the issuer URL must match the value of `server.oidc_authentication.client_id`.
...
6. Add your IdP's public signing key, which you found previously, to your cluster's list of acceptable signing JSON web keys (JWKS). Your cluster will reject JWTs that are not signed by a signing key that is listed in `server.jwt_authentication.jwks`. If you want to [enable automatic fetching of signing keys](#configuration-examples) instead of configuring static signing keys, skip this step.
   ...
   By default, CockroachDB Advanced clusters are configured with CockroachDB Cloud's public key, so that CockroachDB Cloud can serve as an IdP. This is required for [SSO with `ccloud`](/docs/stable/cockroachcloud/cloud-sso-sql). When modifying this cluster setting, do not omit the CockroachDB Cloud public key from the key set, or SSO with `ccloud` will no longer work.
...
- `https://accounts.google.com 1232316645658094244789 roach`
  ...
  Maps a single external identity with the hard-coded ID to the [SQL user](/docs/stable/cockroachcloud/managing-access#manage-sql-users-on-a-cluster) `roach`.
...
4. If [JWT authorization](/docs/v26.1/jwt-authorization) is also enabled, roles are synchronized immediately after user creation.
...
- **Combine with JWT authorization**: Consider enabling [JWT authorization](/docs/v26.1/jwt-authorization) to automatically grant appropriate roles to provisioned users based on their IdP group memberships.
- **Password management**: Provisioned users cannot change their own passwords, as authentication is managed through the IdP.

<Note>
  If you are going to use JWT user provisioning in conjunction with [JWT authorization](/docs/v26.1/jwt-authorization),
...
2. **Generate JWTs via DB Console**: If you have [OIDC authentication for DB Console](/docs/v26.1/sso-db-console) configured, users can obtain JWT tokens through the DB Console UI.
...
<Tip>
  This example uses [`cockroach sql`](/docs/v26.1/cockroach-sql), but you can use any SQL client that supports
...
- Learn more about [Authentication](/docs/v26.1/authentication) in CockroachDB.
