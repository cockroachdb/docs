...
A view is a stored and named [selection query](/docs/v26.1/selection-queries). By default, CockroachDB's views are **dematerialized**: they do not store the results of the underlying queries. Instead, the underlying query is executed anew every time the view is used.
...
<Note>
By default, views created in a database cannot reference objects in a different database. To enable cross-database references for views, set the `sql.cross_db_views.enabled` [cluster setting](/docs/v26.1/cluster-settings) to `true`.
</Note>
...
Let's say you're using our [sample `startrek` database](/docs/v26.1/cockroach-gen#generate-example-data), which contains two tables, `episodes` and `quotes`.
...
To create a view, use the [`CREATE VIEW`](/docs/v26.1/create-view) statement:
...
<Note>
Any [selection query](/docs/v26.1/selection-queries) is valid as operand to `CREATE VIEW`, not just [simple `SELECT` clauses](/docs/v26.1/select-clause).
</Note>
...
To list just views, you can query the `views` table in the [Information Schema](/docs/v26.1/information-schema):
...
To query a view, target it with a [table expression](/docs/v26.1/table-expressions#table-and-view-names), for example using a [`SELECT` clause](/docs/v26.1/select-clause), just as you would with a stored table:
...
`SELECT`ing a view executes the view's stored `SELECT` statement, which returns the relevant data from the underlying table(s). To inspect the `SELECT` statement executed by the view, use the [`SHOW CREATE`](/docs/v26.1/show-create) statement:
...
You can also inspect the `SELECT` statement executed by a view by querying the `views` table in the [Information Schema](/docs/v26.1/information-schema):
...
A view depends on the objects targeted by its underlying query. Attempting to [rename an object](/docs/v26.1/alter-table#rename-to) referenced in a view's stored query therefore results in an error:
...
Likewise, attempting to [drop an object](/docs/v26.1/drop-table) referenced in a view's stored query results in an error:
...
When [dropping a table](/docs/v26.1/drop-table) or [dropping a view](/docs/v26.1/drop-view), you can use the `CASCADE` keyword to drop all dependent objects as well:
...
To rename a view, use the [`ALTER VIEW`](/docs/v26.1/alter-view) statement:
...
To replace a view, use [`CREATE OR REPLACE VIEW`](/docs/v26.1/create-view):
...
To remove a view, use the [`DROP VIEW`](/docs/v26.1/drop-view) statement:
...
When you [select](/docs/v26.1/selection-queries) from a materialized view, the stored query data that is returned might be out-of-date. This contrasts with a standard (i.e., "dematerialized") view, which runs its underlying query every time it is used, returning the latest results. In order to get the latest results from a materialized view, you must [refresh the view](/docs/v26.1/refresh), and then select from it.
...
Materialized views and standard views share similar syntax for [creating](/docs/v26.1/create-view), [showing](/docs/v26.1/show-tables), [renaming](/docs/v26.1/alter-view), and [dropping](/docs/v26.1/drop-view).

To create a materialized view, use a [`CREATE MATERIALIZED VIEW`](/docs/v26.1/create-view) statement.

For example, suppose that you have the [sample `bank` database](/docs/v26.1/cockroach-workload#bank-workload) loaded to a CockroachDB cluster, and populated with some workload values:
...
To show existing materialized views, use a [`SHOW TABLES`](/docs/v26.1/show-tables) statement:
...
You can also create or refresh materialized views using historical data with the [`AS OF SYSTEM TIME`](/docs/v26.1/as-of-system-time) clause. This is useful for reducing [contention](/docs/v26.1/performance-best-practices-overview#transaction-contention) by leveraging [follower reads](/docs/v26.1/follower-reads).
...
For more information, see [`CREATE VIEW`](/docs/v26.1/create-view#create-a-materialized-view-with-historical-data-using-as-of-system-time) and [`REFRESH`](/docs/v26.1/refresh#refresh-a-materialized-view-with-historical-data-using-as-of-system-time).

To rename the materialized view, use [`ALTER MATERIALIZED VIEW`](/docs/v26.1/alter-view):
...
To remove the materialized view, use [`DROP MATERIALIZED VIEW`](/docs/v26.1/drop-view):
...
To speed up queries on materialized views, you can add an [index](/docs/v26.1/schema-design-indexes) to the view.

1. Create a materialized view of the [MovR](/docs/v26.1/movr) rides table where the revenue is less than &#36;20.00:
...
CockroachDB supports session-scoped temporary views. Unlike persistent views, temporary views can only be accessed from the session in which they were created, and they are dropped at the end of the session. You can create temporary views on both persistent tables and [temporary tables](/docs/v26.1/temporary-tables).
...
<Note>
Temporary tables must be enabled in order to use temporary views. By default, temporary tables are disabled in CockroachDB. To enable temporary tables, set the `experimental_enable_temp_tables` [session variable](/docs/v26.1/set-vars) to `on`.
</Note>
...
- Temporary views can be created on both persistent tables and [temporary tables](/docs/v26.1/temporary-tables).
- When you create a view on a temporary table, the view automatically becomes temporary.

<Note>
Like [temporary tables](/docs/v26.1/temporary-tables), temporary views are not in the `public` schema. Instead, when you create the first temporary table, view, or sequence for a session, CockroachDB generates a single temporary schema (`pg_temp_<id>`) for all of the temporary objects in the current session for a database.
</Note>

### Usage

To create a temporary view, add [`TEMP`/`TEMPORARY`](/docs/v26.1/sql-grammar#opt_temp) to a [`CREATE VIEW`](/docs/v26.1/create-view) statement.
...
- Casting subqueries to [user-defined types](/docs/v26.1/create-type) (including ENUMs) in views is not supported. [#108184](https://github.com/cockroachdb/cockroach/issues/108184)
- The optimizer may not select the most optimal query plan when querying materialized views because CockroachDB does not [collect statistics](/docs/v26.1/cost-based-optimizer#table-statistics) on materialized views. [#78181](https://github.com/cockroachdb/cockroach/issues/78181).
- CockroachDB cannot refresh materialized views inside [explicit transactions](/docs/v26.1/begin-transaction). Trying to refresh a materialized view inside an explicit transaction will result in an error.
    1. Start [`cockroach demo`](/docs/v26.1/cockroach-demo) with the sample `bank` data set:

        ```shell
        cockroach demo bank
        ```
    2. Create the materialized view described in [Usage](/docs/v26.1/views#usage).
    3. Start a new multi-statement transaction with [`BEGIN TRANSACTION`](/docs/v26.1/begin-transaction):

        ```sql
          BEGIN TRANSACTION;
        ```
    4. Inside the open transaction, attempt to [refresh the view](/docs/v26.1/refresh). This will result in an error.
...
- [Simple `SELECT` Clauses](/docs/v26.1/select-clause)
