...
- Your data is kept in a safe, [consistent](/docs/v26.1/frequently-asked-questions#how-is-cockroachdb-strongly-consistent) state throughout the entire schema change process.
- The schema change runs as a [background job](/docs/v26.1/show-jobs) without holding locks on the underlying table data.
    - <InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v26.1: As soon as the statement is accepted, CockroachDB returns a SQL `NOTICE` (as shown below) that includes the ID of the background job. To track the job's progress, use [`SELECT * FROM [SHOW JOBS] WHERE job_id = {job_id}`](/docs/v26.1/show-jobs) or view the [DB Console **Jobs Page**](/docs/v26.1/ui-jobs-page).
...
1. You initiate a schema change by executing [`ALTER TABLE`](/docs/v26.1/alter-table), [`CREATE INDEX`](/docs/v26.1/create-index), [`TRUNCATE`](/docs/v26.1/truncate), etc.
2. The schema change engine converts the original schema to the new schema in discrete steps while ensuring that the underlying table data is always in a consistent state. These changes are executed as a [background job](/docs/v26.1/show-jobs), and can be [paused](/docs/v26.1/pause-job), [resumed](/docs/v26.1/resume-job), and [canceled](/docs/v26.1/cancel-job).
...
- The following statements:
    - [`ADD COLUMN`](/docs/v26.1/alter-table#add-column) when the statement also features `INDEX` or `UNIQUE`.
    - [`ALTER PRIMARY KEY`](/docs/v26.1/alter-table#alter-primary-key)
    - [`CREATE INDEX`](/docs/v26.1/create-index)
    - [`CREATE MATERIALIZED VIEW`](/docs/v26.1/views#materialized-views)
    - [`CREATE TABLE AS`](/docs/v26.1/create-table-as)
    - [`REFRESH`](/docs/v26.1/refresh)
    - [`SET LOCALITY`](/docs/v26.1/alter-table#set-locality) under one of the following conditions:
    - The locality changes from [`REGIONAL BY ROW`](/docs/v26.1/alter-table#regional-by-row) to something that is not `REGIONAL BY ROW`.
...
Set the [`schema_locked` table storage parameter](/docs/v26.1/with-storage-parameter#storage-parameter-schema-locked) to `true` to indicate that a schema change is not currently ongoing on a table. CockroachDB automatically unsets this parameter before performing a schema change and reapplies it when done. Enabling `schema_locked` can help [improve performance of changefeeds](/docs/v26.1/create-changefeed#disallow-schema-changes-on-tables-to-improve-changefeed-performance) running on the table, which can reduce commit-to-emit latency.

Use the [`create_table_with_schema_locked` session variable](/docs/v26.1/set-vars#create_table_with_schema_locked) to set this storage parameter to `true` on every table created in the session. In v26.1 and later, it is enabled by default.
...
To estimate the size of the indexes in your table, use the [`SHOW RANGES`](/docs/v26.1/show-ranges) statement.
...
1. Start a 3 node [`cockroach demo`](/docs/v26.1/cockroach-demo) cluster with the MovR dataset.
...
Online schema changes that result in large backfill operations (for example, [`ALTER TABLE... ALTER COLUMN`](/docs/v26.1/alter-table#alter-column) statements) are computationally expensive, and can result in degraded performance. The [admission control system](/docs/v26.1/admission-control) will help keep high-priority operations running, but it's recommended to run backfill-heavy schema changes during times when the cluster is under relatively low loads.

### Schema changes in multi-region clusters

To reduce latency while making online schema changes, we recommend specifying a `lease_preference` [zone configuration](/docs/v26.1/configure-replication-zones) on the `system` database to a single region and running all subsequent schema changes from a node within that region. For example, if the majority of online schema changes come from machines that are geographically close to `us-east1`, run the following:
...
<Note>
Access to tables and built-in functions in the `system` database is controlled by the [`allow_unsafe_internals` session variable](/docs/v26.1/session-variables#allow-unsafe-internals). The above `ALTER DATABASE system` statement executes regardless of the variable's setting because it does not access tables or invoke built-in functions.
</Note>

Run all subsequent schema changes from a node in the specified region.

If you do not intend to run more schema changes from that region, you can safely [remove the lease preference from the zone configuration](/docs/v26.1/alter-database#remove-a-replication-zone) for the system database.

## Examples

<Tip>
For more examples of schema change statements, see the [`ALTER TABLE`](/docs/v26.1/alter-table) subcommands.
</Tip>
...
However, you can run schema changes inside the same transaction as a [`CREATE TABLE`](/docs/v26.1/create-table) statement. For example:
...
Some schema changes can be used in combination in a single `ALTER TABLE` statement. For a list of commands that can be combined, refer to [`ALTER TABLE`](/docs/v26.1/alter-table#subcommands). For examples, refer to [Add and rename columns atomically](/docs/v26.1/alter-table#add-and-rename-columns-atomically) and [Drop and add a primary key constraint](/docs/v26.1/alter-table#drop-and-add-a-primary-key-constraint).

### Show all schema change jobs

You can check on the status of the schema change jobs on your system at any time using the [`SHOW JOBS`](/docs/v26.1/show-jobs) statement:
...
All schema change jobs can be [paused](/docs/v26.1/pause-job), [resumed](/docs/v26.1/resume-job), and [canceled](/docs/v26.1/cancel-job).

### Enable automatic commit before running schema changes inside transactions

When the [`autocommit_before_ddl` session setting](/docs/v26.1/set-vars#autocommit-before-ddl) is set to `on`, any schema change statement that is sent during an [explicit transaction](/docs/v26.1/transactions) will cause the transaction to [commit](/docs/v26.1/commit-transaction) before executing the schema change.
...
- Use schema changes more easily under the [`READ COMMITTED` isolation level](/docs/v26.1/read-committed). The error message returned when running schema changes under `READ COMMITTED` isolation includes a hint to use this setting.

With `autocommit_before_ddl` enabled, [`COMMIT`](/docs/v26.1/commit-transaction), [`ROLLBACK`](/docs/v26.1/rollback-transaction), and other statements that normally return **errors** when used outside of an explicit transaction will instead return **warnings**. This behavior change is necessary because this setting can cause a transaction to end earlier than a client application may expect.

To enable this setting for the current [session](/docs/v26.1/show-sessions):

```sql
SET autocommit_before_ddl = on;
```

To enable it for [all users](/docs/v26.1/alter-role#set-default-session-variable-values-for-all-users):

```sql
ALTER ROLE ALL SET autocommit_before_ddl = on
```

You can also enable the setting [from your application's connection string](/docs/v26.1/connection-parameters#supported-options-parameters).
...
Prior to [garbage collection](/docs/v26.1/architecture/storage-layer#garbage-collection), it's possible to recover data that may have been lost prior to schema changes by using the [`AS OF SYSTEM TIME`](/docs/v26.1/as-of-system-time) parameter. However, this solution is limited in terms of time, and doesn't work beyond the designated garbage collection window.

For more long-term recovery solutions, consider taking either a [full or incremental backup](/docs/v26.1/take-full-and-incremental-backups) of your cluster.
...
Within a single [transaction](/docs/v26.1/transactions):

- You can run schema changes inside the same transaction as a [`CREATE TABLE`](/docs/v26.1/create-table) statement. For more information, see [Run schema changes inside a transaction with `CREATE TABLE`](/docs/v26.1/online-schema-changes#run-schema-changes-inside-a-transaction-with-create-table). However, a `CREATE TABLE` statement containing [`FOREIGN KEY`](/docs/v26.1/foreign-key) clauses cannot be followed by statements that reference the new table.
...
- [`DROP COLUMN`](/docs/v26.1/alter-table#drop-column) can result in data loss if one of the other schema changes in the transaction fails or is canceled. To work around this, move the `DROP COLUMN` statement to its own explicit transaction or run it in a single statement outside the existing transaction.
...
Most schema change [DDL](https://wikipedia.org/wiki/Data_definition_language#ALTER_statement) statements that run inside a multi-statement transaction with non-DDL statements can fail at [`COMMIT`](/docs/v26.1/commit-transaction) time, even if other statements in the transaction succeed. This leaves such transactions in a "partially committed, partially aborted" state that may require manual intervention to determine whether the DDL statements succeeded.
...
In this example, the [`INSERT`](/docs/v26.1/insert) statement committed, but the [`ALTER TABLE`](/docs/v26.1/alter-table) statement adding a [`UNIQUE` constraint](/docs/v26.1/unique) failed. We can verify this by looking at the data in table `t` and seeing that the additional non-unique value `3` was successfully inserted.
...
You cannot start an online schema change on a table if a [primary key change](/docs/v26.1/alter-table#alter-primary-key) is currently in progress on the same table.
...
Some [schema changes](/docs/v26.1/online-schema-changes) that [drop columns](/docs/v26.1/alter-table#drop-column) cannot be [rolled back](/docs/v26.1/rollback-transaction) properly.
...
- Drop all [constraints](/docs/v26.1/constraints) (including [unique indexes](/docs/v26.1/unique) ) on the column in a separate transaction, before dropping the column.
- Drop any [default values](/docs/v26.1/default-value) or [computed expressions](/docs/v26.1/computed-columns) on a column before attempting to drop the column. This prevents conflicts between constraints and default/computed values during a column drop rollback.

If you think a rollback of a column-dropping schema change has occurred, check the [jobs table](/docs/v26.1/show-jobs). Schema changes with an error prefaced by `cannot be reverted, manual cleanup may be required` might require manual intervention.
...
- [`ALTER DATABASE`](/docs/v26.1/alter-database)
- [`ALTER INDEX`](/docs/v26.1/alter-index)
- [`ALTER RANGE`](/docs/v26.1/alter-range)
...
- [`CREATE INDEX`](/docs/v26.1/create-index)
- [`CREATE SEQUENCE`](/docs/v26.1/create-sequence)
...
- [`DROP SEQUENCE`](/docs/v26.1/drop-sequence)
- [`DROP TABLE`](/docs/v26.1/drop-table)
- [`DROP VIEW`](/docs/v26.1/drop-view)
- [`TRUNCATE`](/docs/v26.1/truncate)
