...
This page has instructions for migrating data from CSV files into CockroachDB using [`IMPORT INTO`](/docs/v26.1/import-into).
...
- Files must be in [valid CSV format](https://tools.ietf.org/html/rfc4180), with the caveat that the delimiter must be a single character. To use a character other than comma (such as a tab), set a custom delimiter using the [`delimiter` option](/docs/v26.1/import-into#import-options).
...
You will need to write a [`CREATE TABLE`](/docs/v26.1/create-table) statement that matches the schema of the table data you're importing.
...
<Note>
To match the `nullif` setting, a CSV input value must be unquoted. For details, see [`IMPORT INTO`](/docs/v26.1/import-into#import-options).
</Note>
