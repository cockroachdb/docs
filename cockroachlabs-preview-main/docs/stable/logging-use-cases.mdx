...
This page describes some common logging use cases, their relevant [logging channels](/docs/v26.1/logging-overview#logging-channels), and examples of notable events to be found in the logs:
...
<Note>
In a future major release, certain events will be directed by default to new [logging channels](/docs/v26.1/logging-overview#logging-channels).

To prepare for the change and assess potential downstream impacts on your logging setup and pipelines, review the [`log.channel_compatibility_mode.enabled`](/docs/v26.1/logging-overview#log-channel_compatibility_mode-enabled) cluster setting. After reviewing the documentation, set `log.channel_compatibility_mode.enabled` to `false` in a non-production environment.
</Note>

<Note>
All log examples on this page use the default `crdb-v2` format, except for the [network logging](#network-logging) configuration, which uses the default `json-fluent-compact` format for network output. Most log entries for non-`DEV` channels record *structured* events, which use a standardized format that can be reliably parsed by an external collector. All structured event types and their fields are detailed in the [Notable events reference](/docs/v26.1/eventlog).
...
<Note>
`‹` and `›` are placed around values in log messages that may contain sensitive data (PII). To customize this behavior, see [Redact logs](/docs/v26.1/configure-logs#redact-logs).
</Note>
...
In this example configuration, the channels are grouped into a file sink called `ops`. The combined logging output will be found in a `cockroach-ops.log` file at the configured [logging directory](/docs/v26.1/configure-logs#logging-directory).
...
<Tip>
When monitoring your cluster, consider using these logs in conjunction with [Prometheus](/docs/v26.1/monitor-cockroachdb-with-prometheus), which can be set up to track node-level metrics.
</Tip>

### OPS

The [`OPS`](/docs/v26.1/logging#ops) channel logs operational events initiated by users or automation. These can include node additions and removals, process starts and shutdowns, [gossip](https://wikipedia.org/wiki/Gossip_protocol) connection events, and [zone configuration changes](/docs/v26.1/configure-replication-zones) on the SQL schema or system ranges.

#### Example: Node decommissioning

This [`node_decommissioning`](/docs/v26.1/eventlog#node_decommissioning) event shows that a node is in the [decommissioning](/docs/v26.1/node-shutdown?filters=decommission#decommissioning) state:
...
- Preceding the `=` character is the `crdb-v2` event metadata. See the [reference documentation](/docs/v26.1/log-formats#format-crdb-v2) for details on the fields.
...
This [`node_restart`](/docs/v26.1/eventlog#node_restart) event shows that a node has rejoined the cluster after being offline (e.g., by being [restarted](/docs/v26.1/cockroach-start) after being fully decommissioned):
...
<Note>
All possible `OPS` event types are detailed in the [reference documentation](/docs/v26.1/eventlog).
</Note>

### HEALTH

The [`HEALTH`](/docs/v26.1/logging#health) channel logs operational events initiated by CockroachDB or reported by automatic processes. These can include resource usage details, connection errors, [gossip](https://wikipedia.org/wiki/Gossip_protocol) status, [replication](/docs/v26.1/architecture/replication-layer) events, and runtime statistics.

#### Example: Runtime stats

A [`runtime_stats`](/docs/v26.1/eventlog#runtime_stats) event is recorded every 10 seconds to reflect server health:
...
<Note>
`runtime_stats` events are typically used for troubleshooting. To monitor your cluster's health, see [Monitoring and Alerting](/docs/v26.1/monitoring-and-alerting).
</Note>

#### Example: Hot ranges stats

Each node checks for [hot ranges](/docs/v26.1/understand-hotspots#hot-range) every minute. If a single [replica](/docs/v26.1/architecture/overview#replica) exceeds 250 ms of CPU time per second, a [`hot_ranges_stats`](/docs/v26.1/eventlog#hot_ranges_stats) event is recorded. In [virtual clusters](/docs/v26.1/cluster-virtualization-overview) and [CockroachDB Standard and Basic deployments](/docs/cockroachcloud), nodes perform this check every 5 minutes at the cluster level. In addition to these conditional logs, `hot_ranges_stats` logs are automatically emitted every 4 hours.
...
<Note>
`hot_ranges_stats` events are typically used for troubleshooting [hotspots](/docs/v26.1/understand-hotspots).
</Note>

### SQL_SCHEMA

The [`SQL_SCHEMA`](/docs/v26.1/logging#sql_schema) channel logs changes to the SQL logical schema resulting from DDL operations.

#### Example: Schema change initiated

This [`alter_table`](/docs/v26.1/eventlog#alter_table) event shows an [`ALTER TABLE... ADD FOREIGN KEY`](/docs/v26.1/alter-table#add-constraint) schema change being initiated on a `movr.public.vehicles` table:
...
This [`finish_schema_change`](/docs/v26.1/eventlog#finish_schema_change) event shows that the above schema change has completed:
...
<Note>
All possible `SQL_SCHEMA` event types are detailed in the [reference documentation](/docs/v26.1/eventlog#sql-logical-schema-changes).
</Note>
...
In this example configuration, the channels are grouped into a file sink called `security`. The combined logging output will be found in a `cockroach-security.log` file at the configured [logging directory](/docs/v26.1/configure-logs#logging-directory).
...
The [`SESSIONS`](/docs/v26.1/logging#sessions) channel logs SQL session events. This includes client connection and session authentication events, for which logging must be enabled separately. For complete logging of client connections, we recommend enabling both types of events.
...
To log SQL client connection events to the `SESSIONS` channel, enable the `server.auth_log.sql_connections.enabled` [cluster setting](/docs/v26.1/cluster-settings):
...
These logs show a [`client_connection_start`](/docs/v26.1/eventlog#client_connection_start) (client connection established) and a [`client_connection_end`](/docs/v26.1/eventlog#client_connection_end) (client connection terminated) event over a `hostssl` (TLS transport over TCP) connection:
...
To log SQL session authentication events to the `SESSIONS` channel, enable the `server.auth_log.sql_sessions.enabled` [cluster setting](/docs/v26.1/cluster-settings) on every cluster:
...
- Preceding the `=` character is the `crdb-v2` event metadata. See the [reference documentation](/docs/v26.1/log-formats#format-crdb-v2) for details on the fields.
- The two [`client_authentication_info`](/docs/v26.1/eventlog#client_authentication_info) events show the progress of certificate authentication. The `Info` fields show the progress of certificate validation.
- The [`client_authentication_ok`](/docs/v26.1/eventlog#client_authentication_ok) event shows that certificate authentication was successful.
...
- Preceding the `=` character is the `crdb-v2` event metadata. See the [reference documentation](/docs/v26.1/log-formats#format-crdb-v2) for details on the fields.
- The two [`client_authentication_info`](/docs/v26.1/eventlog#client_authentication_info) events show the progress of certificate authentication. The `Info` fields show that password authentication was attempted, in the absence of a client certificate.
- The [`client_authentication_failed`](/docs/v26.1/eventlog#client_authentication_failed) event shows that password authentication was unsuccessful. The `Detail` field shows the related error.
- The [`client_session_end`](/docs/v26.1/eventlog#client_session_end) event shows that the SQL session was terminated. This would typically be followed by a [`client_connection_end`](/docs/v26.1/eventlog#client_connection_end) event.
- `User` shows that the SQL session authentication was attempted for user `roach`.

<Note>
All possible `SESSIONS` event types are detailed in the [reference documentation](/docs/v26.1/eventlog#sql-session-events). For more details on certificate and password authentication, see [Authentication](/docs/v26.1/authentication).
</Note>

### SENSITIVE_ACCESS

The [`SENSITIVE_ACCESS`](/docs/v26.1/logging#sensitive_access) channel logs SQL audit events. These include all queries run against [audited tables](/docs/v26.1/alter-table#experimental_audit), when enabled, and queries executed by users with the [`admin`](/docs/v26.1/security-reference/authorization#admin-role) role. It also logs when a user overrides or is denied access by the [`allow_unsafe_internals` session variable](/docs/v26.1/crdb-internal#access-control), creating an audit log of access to system internals.
...
To log all queries against a specific table, enable auditing on the table with [`ALTER TABLE... EXPERIMENTAL_AUDIT`](/docs/v26.1/alter-table#experimental_audit).
...
This [`sensitive_table_access`](/docs/v26.1/eventlog#sensitive_table_access) event shows that the audited table `customers` was accessed by user `root` issuing an [`INSERT`](/docs/v26.1/insert) statement:
...
- `ApplicationName` shows that the event originated from the [`cockroach sql`](/docs/v26.1/cockroach-sql) shell. You can use this field to filter the logging output by application.

<Note>
All possible `SENSITIVE_ACCESS` event types are detailed in the [reference documentation](/docs/v26.1/eventlog#sql-access-audit-events). For a detailed tutorial on table auditing, see [SQL Audit Logging](/docs/v26.1/sql-audit-logging).
</Note>

#### Example: Unsafe internals

The [`allow_unsafe_internals` session variable](/docs/v26.1/session-variables#allow-unsafe-internals) defaults to `off`, restricting access to the `system` and `crdb_internal` namespaces. Queries to these namespaces will fail unless access is manually enabled.

CockroachDB emits log events to the `SENSITIVE_ACCESS` channel when a user overrides or is denied access to [unsafe internals](/docs/v26.1/crdb-internal#access-control), creating an audit log of access to system internals.

The following events may be logged to the `SENSITIVE_ACCESS` channel, depending on whether the [`allow_unsafe_internals` session variable](/docs/v26.1/session-variables#allow-unsafe-internals) is enabled:
...
This `unsafe_internals_accessed` event indicates that the internal table `crdb_internal.active_range_feeds` was accessed by user `can_access_unsafe_internals`, who issued a [`SELECT`](/docs/v26.1/selection-queries) statement:
...
This `unsafe_internals_denied` event indicates that access to the internal table `crdb_internal.active_range_feeds` was denied for the user `can_not_access_unsafe_internals`, who issued a [`SELECT`](/docs/v26.1/selection-queries) statement:
...
<Note>
All possible `SENSITIVE_ACCESS` event types are detailed in the [reference documentation](/docs/v26.1/eventlog#sql-access-audit-events).
</Note>

### PRIVILEGES

The [`PRIVILEGES`](/docs/v26.1/logging#privileges) channel logs SQL privilege changes. These include DDL operations performed by SQL operations that [modify the privileges](/docs/v26.1/security-reference/authorization#managing-privileges) granted to [roles and users](/docs/v26.1/security-reference/authorization#users-and-roles) on databases, schemas, tables, and [user-defined types](/docs/v26.1/enum).

#### Example: Database privileges

This [`change_database_privilege`](/docs/v26.1/eventlog#change_database_privilege) event shows that user `root` granted all privileges to user `roach` on the database `movr`:
...
<Note>
All possible `PRIVILEGE` event types are detailed in the [reference documentation](/docs/v26.1/eventlog#sql-privilege-changes).
</Note>

### USER_ADMIN

The [`USER_ADMIN`](/docs/v26.1/logging#user_admin) channel logs changes to users and roles. This includes user and role [creation and assignment](/docs/v26.1/security-reference/authorization#create-and-manage-users) and changes to [privileges](/docs/v26.1/security-reference/authorization#managing-privileges), [options](/docs/v26.1/create-role#parameters), and [passwords](/docs/v26.1/authentication#client-authentication).

#### Example: SQL user creation

This [`create_role`](/docs/v26.1/eventlog#create_role) event shows that a user `roach` was created and assigned a password by user `root`. Note that the password in the SQL statement is pre-redacted even if `redact` is set to `false` for the logging sink. For more details on redaction behavior, see [Redact logs](/docs/v26.1/configure-logs#redact-logs).
...
- Preceding the `=` character is the `crdb-v2` event metadata. See the [reference documentation](/docs/v26.1/log-formats#format-crdb-v2) for details on the fields.
- `ApplicationName` shows that the event originated from the [`cockroach sql`](/docs/v26.1/cockroach-sql) shell. You can use this field to filter the logging output by application.
- `RoleName` shows the name of the user/role. For details on user and role terminology, see [Users and roles](/docs/v26.1/security-reference/authorization#users-and-roles).

<Note>
All possible `USER_ADMIN` event types are detailed in the [reference documentation](/docs/v26.1/eventlog#sql-user-and-role-operations).
</Note>
...
In this example configuration, the channels are grouped into a file sink called `performance`. The combined logging output will be found in a `cockroach-performance.log` file at the configured [logging directory](/docs/v26.1/configure-logs#logging-directory).
...
The [`SQL_EXEC`](/docs/v26.1/logging#sql_exec) channel reports all SQL executions on the cluster, when enabled.

To log cluster-wide executions, enable the `sql.trace.log_statement_execute` [cluster setting](/docs/v26.1/cluster-settings):

```sql
> SET CLUSTER SETTING sql.trace.log_statement_execute = true;
```

Each node of the cluster will write all SQL queries it executes to the `SQL_EXEC` channel. These are recorded as [`query_execute`](/docs/v26.1/eventlog#query_execute) events.
...
<Note>
All possible `SQL_EXEC` event types are detailed in the [reference documentation](/docs/v26.1/eventlog#sql-execution-log).
</Note>

### SQL_PERF

The [`SQL_PERF`](/docs/v26.1/logging#sql_perf) channel reports slow SQL queries, when enabled. This includes queries whose latency exceeds a configured threshold, as well as queries that perform a full table or index scan.

To enable slow query logging, enable the `sql.log.slow_query.latency_threshold` [cluster setting](/docs/v26.1/cluster-settings) by setting it to a non-zero value. This will log queries whose service latency exceeds a specified threshold value. The threshold value must be specified with a unit of time (e.g., `500ms` for 500 milliseconds, `5us` for 5 nanoseconds, or `5s` for 5 seconds). A threshold of `0s` disables the slow query log.
...
To log all queries that perform full table or index scans to `SQL_PERF`, regardless of query latency, set the `sql.log.slow_query.experimental_full_table_scans.enabled` [cluster setting](/docs/v26.1/cluster-settings) to `true`.
...
Each gateway node will now record queries that take longer than 100 milliseconds to the `SQL_PERF` channel as [`slow_query` events](/docs/v26.1/eventlog#slow_query).
...
The following query was logged with a service latency (`age`) of 9329.26 milliseconds, a very high latency that resulted from a [transaction retry error](/docs/v26.1/transaction-retry-error-reference#retry_write_too_old):
...
- Preceding the `=` character is the `crdb-v2` event metadata. See the [reference documentation](/docs/v26.1/log-formats#format-crdb-v2) for details on the fields.
- `ApplicationName` shows that the events originated from an application named `bank`. You can use this field to filter the logging output by application.
- `ErrorText` shows that this query encountered a [type of transaction retry error](/docs/v26.1/transaction-retry-error-reference#retry_write_too_old). For details on transaction retry errors and how to resolve them, see the [Transaction Retry Error Reference](/docs/v26.1/transaction-retry-error-reference#actions-to-take).
- `NumRetries` shows that the transaction was retried once before succeeding.

<Note>
All possible `SQL_PERF` event types are detailed in the [reference documentation](/docs/v26.1/eventlog#sql-slow-query-log).
</Note>
...
- [Enable redaction](/docs/v26.1/configure-logs#redact-logs) on the `ops` logs.
...
By default, `fluent-servers` log messages use the [`json-fluent-compact`](/docs/v26.1/log-formats#format-json-fluent-compact) format for ease of processing over a stream.

For example, this JSON message found in the `OPS` logging channel contains a [`node_restart`](/docs/v26.1/eventlog#node_restart) event. The event shows that a node has rejoined the cluster after being offline (e.g., by being [restarted](/docs/v26.1/cockroach-start) after being fully decommissioned):
...
- `sev` shows that the message has the `INFO` [severity level](/docs/v26.1/logging#logging-levels-severities).
- `event` contains the fields for the structured [`node_restart`](/docs/v26.1/eventlog#node_restart) event.
    - `NodeID` shows that the restarted node is `1`.
    - `StartedAt` shows the timestamp when the node was most recently restarted.
    - `LastUp` shows the timestamp when the node was up before being restarted.

See the [reference documentation](/docs/v26.1/log-formats#format-json-fluent-compact) for details on the remaining JSON fields.

### Network logging with log buffering

A database operator can configure CockroachDB to buffer log messages for a configurable time period or collected message size before writing them to the [log sink](/docs/v26.1/configure-logs#configure-log-sinks). This is especially useful for writing log messages to network log sinks, such as [Fluentd-compatible servers](/docs/v26.1/configure-logs#output-to-fluentd-compatible-network-collectors), [HTTP servers](/docs/v26.1/configure-logs#output-to-http-network-collectors), or [OTLP-compatible servers](/docs/v26.1/configure-logs#output-to-otlp-compatible-network-collectors), where high-traffic or high-contention scenarios can result in log message write latency.
...
For example, the following logging configuration adjusts the default log buffering behavior for both an [OTLP-compatible](/docs/v26.1/configure-logs#output-to-otlp-compatible-network-collectors) and an [HTTP](/docs/v26.1/configure-logs#output-to-http-network-collectors) log sink destination:
...
Together, this configuration ensures that log messages to the OTLP log sink target are buffered for up to `2MiB` in accumulated size, and log messages to the HTTP server log sink target are buffered for up to `10s` duration (with a limit of up to `75MiB` accumulated message size in the buffer before messages begin being dropped), before being written to the log sink. Further, each long sink target is configured with an overridden value for these settings specific to log messages in the `HEALTH` [log channel](/docs/v26.1/logging-overview#logging-channels), which are flushed more aggressively in both cases.

See [Log buffering](/docs/v26.1/configure-logs#log-buffering-for-network-sinks) for more information.

## See also

- [Notable Event Types](/docs/v26.1/eventlog)
- [SQL Audit Logging](/docs/v26.1/sql-audit-logging)
- [Use Prometheus and Alertmanager](/docs/v26.1/monitor-cockroachdb-with-prometheus)
