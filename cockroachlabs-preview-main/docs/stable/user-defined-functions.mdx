...
A user-defined function (UDF) is a named function defined at the database level that can be called in queries and other contexts. CockroachDB supports invoking UDFs in `SELECT`, `FROM`, and `WHERE` clauses of [DML statements](/docs/v26.1/sql-statements#data-manipulation-statements).

Both [stored procedures](/docs/v26.1/stored-procedures) and user-defined functions are types of *routines*. However, they differ in the following ways:

- Functions return a value, and procedures do not return a value.
- Procedures must be invoked using a [`CALL`](/docs/v26.1/call) statement. Functions can be invoked in nearly any context, such as `SELECT`, `FROM`, and `WHERE` clauses, [`DEFAULT`](/docs/v26.1/default-value) expressions, and [computed column](/docs/v26.1/computed-columns) expressions.
- Functions have [volatility](/docs/v26.1/functions-and-operators#function-volatility) settings, and procedures do not.
...
- An argument has a *mode* and a *type*.
    - CockroachDB supports the `IN` (default), `OUT`, and `INOUT` argument modes. For an example, see [Create a function that uses `OUT` and `INOUT` parameters](/docs/v26.1/create-function#create-a-function-that-uses-out-and-inout-parameters).
    - The type can be a built-in type, [user-defined `ENUM`](/docs/v26.1/enum) or [composite](/docs/v26.1/create-type#create-a-composite-data-type) type, or implicit record type. A type can have a `DEFAULT` value.
- The return type can be a built-in [SQL type](/docs/v26.1/data-types), user-defined [`ENUM`](/docs/v26.1/enum) or [composite](/docs/v26.1/create-type#create-a-composite-data-type) type, [`RECORD`](/docs/v26.1/create-function#create-a-function-that-returns-a-record-type), [`TABLE`](/docs/v26.1/create-function#create-a-function-that-returns-a-table), PL/pgSQL [`REFCURSOR`](/docs/v26.1/plpgsql#declare-cursor-variables) type, implicit record type, [`TRIGGER`](/docs/v26.1/triggers#trigger-function), or `VOID`.
    - Preceding a type with `SETOF` indicates that a set, or multiple rows, may be returned. For an example, see [Create a function that returns a set of results](/docs/v26.1/create-function#create-a-function-that-returns-a-set-of-results).
    - `VOID` indicates that there is no return type and `NULL` will always be returned.
- The [volatility](/docs/v26.1/functions-and-operators#function-volatility) indicates whether the function has side effects. `VOLATILE` and `NOT LEAKPROOF` are the default.
    - Annotate a function with side effects with `VOLATILE`. This also prevents the [cost-based optimizer](/docs/v26.1/cost-based-optimizer) from pre-evaluating the function.
...
- `LANGUAGE` specifies the language of the function body. CockroachDB supports the languages `SQL` and [`PLpgSQL` (PL/pgSQL)](/docs/v26.1/plpgsql).
- The function body:
...
SQL statements executed within user-defined functions are tracked in the SQL statistics subsystem and will appear in the [**SQL Activity** &gt; **Statements**](/docs/v26.1/ui-statements-page) page and the [**Insights**](/docs/v26.1/ui-insights-page) page in the DB Console. This allows you to monitor the performance and execution statistics of individual statements within your functions.

When the UDF is invoked as part of a transaction, the statements executed within the function also appear in the [**Transaction details**](/docs/v26.1/ui-transactions-page#transaction-details-page) in the **Statement Fingerprints** table.

<Note>
[Statement diagnostics](/docs/v26.1/explain-analyze#debug-option) cannot be collected for statements executed inside user-defined functions. You can request statement diagnostics for the top-level invocation of the function, and the resulting trace includes spans for each statement executed. However, there is no way to target statements executed inside the function with a statement diagnostics request. For details, refer to [Known limitations](#known-limitations).
</Note>
...
For more examples of UDF creation, see [`CREATE FUNCTION`](/docs/v26.1/create-function).
...
You invoke a UDF like a [built-in function](/docs/v26.1/functions-and-operators).
...
You can include UDF calls in a [view](/docs/v26.1/views) definition. The function is invoked each time the view is queried. A [materialized view](/docs/v26.1/views#materialized-views) will invoke the function only when the view is created or refreshed.

Refer to [Create a view that references routines](/docs/v26.1/create-view#create-a-view-that-references-routines).

### Create a UDF using PL/pgSQL

The following user-defined function returns the `n`th integer in the Fibonacci sequence.

It uses the [PL/pgSQL](/docs/v26.1/plpgsql) [`LOOP`](/docs/v26.1/plpgsql#write-loops) syntax to iterate through a simple calculation, and [`RAISE EXCEPTION`](/docs/v26.1/plpgsql#report-messages-and-handle-exceptions) to return an error message if the specified `n` is negative.
...
When possible, the [cost-based optimizer](/docs/v26.1/cost-based-optimizer) will improve a function's performance by inlining the UDF within the query plan. The UDF must have the following attributes:

- It is labeled as `IMMUTABLE`, `STABLE`, or `LEAKPROOF` (i.e., non- `VOLATILE` ).
- It has a single statement.
- It is not a [set-returning function](/docs/v26.1/create-function#create-a-function-that-returns-a-set-of-results).
- Its arguments are only variable or constant expressions.
- It is not a [record-returning function](/docs/v26.1/create-function#create-a-function-that-returns-a-record-type).
...
5. View the query plan when using `foo_s()` (the `STABLE` function) instead:
    ...
    The query takes only `4ms` to execute because the function is inlined and transformed to a [join](/docs/v26.1/joins) with an equality comparison `(a) = (b)`, which has much less overhead than invoking a function for each row scanned in table `a`.
...
- Casting subqueries to [user-defined types](/docs/v26.1/create-type) in UDFs is not supported. [#108184](https://github.com/cockroachdb/cockroach/issues/108184)
...
- Routine parameters and return types cannot be declared using the `ANYENUM` polymorphic type, which is able to match any [`ENUM`](/docs/v26.1/enum) type. [#123048](https://github.com/cockroachdb/cockroach/issues/123048)
- [Statement diagnostics](/docs/v26.1/explain-analyze#debug-option) cannot be collected for statements executed inside UDFs or stored procedures. You can request statement diagnostics for the top-level invocation of the function or procedure, and the resulting trace includes spans for each statement executed. However, there is no way to target statements executed inside the function or procedure with a statement diagnostics request. [#159526](https://github.com/cockroachdb/cockroach/issues/159526)

Also refer to the [PL/pgSQL known limitations](/docs/v26.1/plpgsql#known-limitations).

## See also

- [`CREATE FUNCTION`](/docs/v26.1/create-function)
- [`ALTER FUNCTION`](/docs/v26.1/alter-function)
- [`DROP FUNCTION`](/docs/v26.1/drop-function)
