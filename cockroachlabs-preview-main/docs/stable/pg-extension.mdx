...
The `pg_extension` [system catalogs](/docs/v26.1/system-catalogs) provides information about CockroachDB extensions.

## Data exposed by `pg_extension`

In CockroachDB v26.1, `pg_extension` contains the following tables, all of which provide information about CockroachDB's [spatial extension](/docs/v26.1/spatial-data-overview):
...
To see the list of tables in `pg_extension` for the [current database](/docs/v26.1/sql-name-resolution#current-database), use the following [`SHOW TABLES`](/docs/v26.1/show-tables) statement:
...
You can run [`SELECT` queries](/docs/v26.1/selection-queries) on the tables in `pg_extension`.

<Tip>
To ensure that you can view all of the tables in `pg_extension`, query the tables as a user with [`admin` privileges](/docs/v26.1/security-reference/authorization#admin-role).
</Tip>

<Note>
Unless specified otherwise, queries to `pg_extension` assume the [current database](/docs/v26.1/sql-name-resolution#current-database).
</Note>
