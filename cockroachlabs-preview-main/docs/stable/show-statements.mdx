...
The `SHOW STATEMENTS` [statement](/docs/v26.1/sql-statements) lists details about currently active SQL queries, including:
...
These details let you monitor the progress of active queries and, if necessary, identify those that may need to be [cancelled](/docs/v26.1/cancel-query) to prevent unwanted resource consumption.

<Note>
Schema changes and [`BACKUP`](/docs/v26.1/backup)/[`RESTORE`](/docs/v26.1/restore) statements are not executed as queries internally and so are not listed by `SHOW STATEMENTS`. To monitor such statements, use [`SHOW JOBS`](/docs/v26.1/show-jobs) instead.
</Note>
...
All users can see their own currently active queries. Users with the [`VIEWACTIVITY` or `VIEWACTIVITYREDACTED` privilege](/docs/v26.1/security-reference/authorization#supported-privileges) can view see all users' currently active queries. `VIEWACTIVITYREDACTED` causes constants in queries being executed by other users to be redacted.

## Synopsis

<Frame>
  ![SQL syntax diagram](/images/generated/docs/stable/show-statements/syntax-diagram-6bdd04b800b5.svg)
</Frame>
...
You can use a [`SELECT`](/docs/v26.1/select-clause) statement to filter the list of active queries by one or more of the [response fields](#response).
...
To exclude queries from the [built-in SQL client](/docs/v26.1/cockroach-sql), filter for queries that do not show `$ cockroach sql` as the `application_name`:
...
When you see a query that is taking too long to complete, you can use the [`CANCEL QUERY`](/docs/v26.1/cancel-query) statement to end it.
