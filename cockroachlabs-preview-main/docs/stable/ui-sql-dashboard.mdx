...
To view this dashboard, [access the DB Console](/docs/v26.1/ui-overview#db-console-access), click **Metrics** in the left-hand navigation, and then select **Dashboard** &gt; **SQL**.
...
If this metric is non-zero, then transactions are being [upgraded to stronger isolation levels](/docs/v26.1/transactions#isolation-level-upgrades).
...
See the [Transactions page](/docs/v26.1/ui-transactions-page) for more details on the transactions.
...
See the [Statements page](/docs/v26.1/ui-statements-page) for more details on the cluster's SQL statements.
...
- [`INSERT... ON CONFLICT DO UPDATE...`](/docs/v26.1/insert#on-conflict-clause): Even when the `DO UPDATE` clause is actually executed, the root of the [abstract syntax tree (AST)](/docs/v26.1/architecture/sql-layer#parsing) is used to increment the metric, rather than the actual execution details.
- [`UPSERT`](/docs/v26.1/upsert)

<Note>
[Data manipulation statements](/docs/v26.1/sql-statements#data-manipulation-statements) other than `SELECT`/`INSERT`/`UPDATE`/`DELETE`/`UPSERT` update the `sql.misc.count` metric, which is *not* displayed on this graph.
</Note>

## SQL Statement Errors

- In the node view, the graph shows the 10-second average of the number of SQL statements issued to the node that returned a [planning](/docs/v26.1/architecture/sql-layer#sql-parser-planner-executor), [runtime](/docs/v26.1/architecture/sql-layer#sql-parser-planner-executor), or [retry error](/docs/v26.1/transactions#error-handling).
...
The statement contention metric is a counter that represents the number of statements that have experienced [contention](/docs/v26.1/performance-best-practices-overview#transaction-contention). If a statement experiences at least one contention "event" (i.e., the statement is forced to wait for another transaction), the counter is incremented at most once.

- In the node view, the graph shows the total number of SQL statements that experienced [contention](/docs/v26.1/performance-best-practices-overview#transaction-contention) on that node.
- In the cluster view, the graph shows the total number of SQL statements that experienced [contention](/docs/v26.1/performance-best-practices-overview#transaction-contention) across all nodes in the cluster.
...
You can [identify the specific statements](/docs/v26.1/ui-statements-page#filter) that result in full table scans by filtering for them on the **Statements** page. [Examine these statements](/docs/v26.1/sql-tuning-with-explain) that result in full table scans and consider adding [secondary indexes](/docs/v26.1/schema-design-indexes#create-a-secondary-index).

## Active Flows for Distributed SQL Statements

- In the node view, the graph shows the number of flows on that node contributing to the running [distributed SQL](/docs/v26.1/architecture/sql-layer#distsql) statements.
- In the cluster view, the graph shows the number of flows across all nodes in the cluster contributing to the running [distributed SQL](/docs/v26.1/architecture/sql-layer#distsql) statements.
...
KV execution latency is calculated as the time in milliseconds between when the [KV layer](/docs/v26.1/architecture/overview) receives the request and delivers a response.
...
- In the node view, the graph shows the 10-second average of the number of opened (`Begin`), committed (`Commits`), rolled back (`Rollbacks`), and aborted (`Aborts`) [transactions](/docs/v26.1/transactions) per second issued by SQL clients on the node.
- In the cluster view, the graph shows the sum of the per-node averages, that is, an aggregate estimation of the current [transactions](/docs/v26.1/transactions) load over the cluster, assuming the last 10 seconds of activity per node are representative of this load.

If the graph shows excessive aborts or rollbacks, it might indicate issues with the SQL statements. In that case, re-examine [statements](/docs/v26.1/ui-statements-page) to lower contention.
...
See the [Transaction Retry Error Reference](/docs/v26.1/transaction-retry-error-reference) for details on the errors that caused the transaction to restart.

## Transaction Latency: 99th percentile

Transaction latency is calculated as the total time in nanoseconds a [transaction](/docs/v26.1/transactions) took to complete.
...
- In the node view, the graph shows the current amount of memory in KiB allocated to the SQL layer on this node. This amount is what is compared against the node's [`--max-sql-memory` flag](/docs/v26.1/cockroach-start#general).
- In the cluster view, the graph shows the current amount of memory in KiB allocated to the SQL layer on all nodes in the cluster. This amount is what is compared against the node's [`--max-sql-memory` flag](/docs/v26.1/cockroach-start#general).
...
- In the node view, the graph shows the total number of [DDL statements](/docs/v26.1/online-schema-changes) per second on the node.
- In the cluster view, the graph shows the total number of [DDL statements](/docs/v26.1/online-schema-changes) per second across all nodes in the cluster.

## Statement Denials: Cluster Settings

Statement denials are statements that were denied due to a [cluster setting](/docs/v26.1/cluster-settings) with the following format:
