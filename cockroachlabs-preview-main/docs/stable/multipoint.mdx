...
A `MULTIPOINT` is a collection of [Points](/docs/v26.1/point). MultiPoints are useful for gathering a group of Points into one geometry. For example, you may want to gather the points denoting all of the State Capitols in the U.S. into a single geometry.
...
A MultiPoint can be created from SQL by calling an aggregate function such as `ST_Collect` or [`ST_Union`](/docs/v26.1/st_union) on a column that contains [Point](/docs/v26.1/point) geometries. In the example below, we will build a MultiPoint from several Points.
...
1. Insert the Points:
    ...
    ```
2. Build a MultiPoint from the individual [Points](/docs/v26.1/point) using `ST_Collect`, and check the output with `ST_GeometryType` to verify that it is indeed a MultiPoint:
...
A MultiPoint can be created from SQL by calling the `st_geomfromtext` function on a MultiPoint definition expressed in the [Well Known Text (WKT)](/docs/v26.1/architecture/glossary#wkt) format.
