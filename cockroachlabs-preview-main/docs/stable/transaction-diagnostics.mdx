...
Transaction diagnostics allow operators and support engineers to investigate issues involving [transactions](/docs/v26.1/transactions) in user workloads. Use the built-in function [`crdb_internal`](/docs/v26.1/crdb-internal)`.request_transaction_bundle` to request a transaction diagnostics bundle for a specified [transaction fingerprint ID](/docs/v26.1/ui-transactions-page).
...
<Note>
Transaction diagnostics do not contain a [statement diagnostic bundle](/docs/v26.1/explain-analyze#debug-option) for statements executed inside [user-defined functions](/docs/v26.1/user-defined-functions) or [stored procedures](/docs/v26.1/stored-procedures). However, they do contain a statement diagnostic bundle for the top-level invocation, or call, of the user-defined function or stored procedure.
</Note>

## Required privileges

To use this function on a [secure cluster](/docs/v26.1/secure-a-cluster), you must be an [`admin` user](/docs/v26.1/security-reference/authorization#admin-role) or a SQL user with the [`VIEWACTIVITY`](/docs/v26.1/security-reference/authorization#viewactivity) or [`VIEWACTIVITYREDACTED`](/docs/v26.1/security-reference/authorization#viewactivityredacted) [system privilege](/docs/v26.1/security-reference/authorization#supported-privileges). If the user has only `VIEWACTIVITYREDACTED`, they can request only redacted bundles.
...
- `transaction_fingerprint_id`: A hex-encoded ID of the [transaction fingerprint](/docs/v26.1/ui-transactions-page#transaction_fingerprint) to capture. The fingerprint ID must exist in `crdb_internal.transaction_statistics`, which is the system of record for transaction fingerprints.
...
3. [Download the bundle](#step-3-download-the-bundle) from [DB Console](/docs/v26.1/ui-overview).
4. [Inspect the bundle](#step-4-inspect-the-bundle).

### Setup

For this example, set the [`application_name`](/docs/v26.1/map-sql-activity-to-app) to enable filtering:
...
Identify the transaction fingerprint ID of the example transaction by navigating to the [**SQL Activity**](/docs/v26.1/ui-overview#sql-activity) page in the [DB Console](/docs/v26.1/ui-overview). On the [**Transactions**](/docs/v26.1/ui-transactions-page) tab, search the Top `500` by `Transaction Time` in the `Past Hour` and click **Apply** to list transactions. Filter the transactions by **Application Name**: `cockroachdb_test`.

In the **Transactions** column, click the transaction fingerprint `SELECT pg_sleep(_), _` to open the [**Transaction Details**](/docs/v26.1/ui-transactions-page#transaction-details-page) page for that fingerprint.

<Frame>
  ![Transactions tab](/images/v26.1/transaction-diagnostics-1.png)
</Frame>

From the **Transaction Details** page, copy the hexadecimal **Fingerprint ID** for this transaction, `afdd4059a899442e`.

<Frame>
  ![Transactions Details page](/images/v26.1/transaction-diagnostics-2.png)
</Frame>
...
In the DB Console, go to [**Advanced Debug**](/docs/v26.1/ui-debug-pages) &gt; [**Diagnostics History**](/docs/v26.1/ui-debug-pages#diagnostics-history). Under the **Transactions** tab, there will be a row for the bundle request. You could also use the URL `https://{host}:{port}/#/reports/diagnosticshistory?tab=Transactions`. The page initially displays the following information:
...
<Frame>
  ![Diagnostics History, Transactions, Status Waiting](/images/v26.1/transaction-diagnostics-3.png)
</Frame>
...
Navigate to the [**Advanced Debug**](/docs/v26.1/ui-debug-pages) &gt; [**Diagnostics History**](/docs/v26.1/ui-debug-pages#diagnostics-history) page in the DB Console. Under the **Transactions** tab, the row for the bundle request now shows:
...
<Frame>
  ![Diagnostics History, Transactions, Status Ready](/images/v26.1/transaction-diagnostics-4.png)
</Frame>
...
Unzip the transaction diagnostic bundle and examine its contents. It contains [statement diagnostic bundle](/docs/v26.1/ui-statements-page#diagnostics) `zip` files for each statement as well as a `zip` file of the transaction traces:
...
Unzip the `zip` file `transaction-1113386693458034689.zip`. It consists of transaction traces in various formats (including a JSON file that can be [imported to Jaeger](/docs/v26.1/query-behavior-troubleshooting#visualize-statement-traces-in-jaeger)):
...
- [DB Console Statements page, Diagnostics](/docs/v26.1/ui-statements-page#diagnostics)
- [`cockroach statement-diag`](/docs/v26.1/cockroach-statement-diag)
- [Identify slow queries](/docs/v26.1/query-behavior-troubleshooting#identify-slow-queries)
- [Visualize statement traces in Jaeger](/docs/v26.1/query-behavior-troubleshooting#visualize-statement-traces-in-jaeger)
