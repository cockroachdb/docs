...
You can configure automatic Go execution trace capture with the following [cluster settings](/docs/v26.1/cluster-settings):
...
- The automatic Go execution tracer saves the captured traces to disk on each node's file system in the [logging directory](/docs/v26.1/configure-logs#logging-directory). The default path is `cockroach-data/logs/executiontrace_dump`.
...
- Enabling the automatic Go execution tracer does add Go execution traces to [debug zips](/docs/v26.1/cockroach-debug-zip) in the `nodes/*/executiontrace` paths.
