...
CockroachDB supports efficiently storing and querying [spatial data](/docs/v26.1/export-spatial-data).

This page has instructions for migrating data from ESRI [Shapefiles](/docs/v26.1/architecture/glossary#shapefile) into CockroachDB using [`ogr2ogr`](https://gdal.org/programs/ogr2ogr) and [`IMPORT INTO`](/docs/v26.1/import-into).
...
- CockroachDB [installed](/docs/v26.1/install-cockroachdb) and [running](/docs/v26.1/start-a-local-cluster)
...
Each node in the CockroachDB cluster needs to have access to the files being imported. There are several ways for the cluster to access the data; for a complete list of the types of storage [`IMPORT INTO`](/docs/v26.1/import-into) can pull from, see [Import file location](/docs/v26.1/import-into#import-file-location).

For local testing, you can [start a local file server](/docs/v26.1/use-a-local-file-server). The following command will start a local file server listening on port 3000:
...
Since the file is being served from a local server and is formatted as CSV, you can import the data using the following [`IMPORT INTO`](/docs/v26.1/import-into) statement:
