...
import { SelectClause } from "/snippets/syntax-diagrams/SelectClause.jsx";

The simple `SELECT` clause is the main SQL syntax to read and process existing data.

When used as a stand-alone statement, the simple `SELECT` clause is also called "the `SELECT` statement". However, it is also a [selection clause](/docs/v26.1/selection-queries#selection-clauses) that can be combined with other constructs to form more complex [selection queries](/docs/v26.1/selection-queries).
...
<Tip>
  The simple `SELECT` clause also has other applications not covered here, such as executing
  [functions](/docs/v26.1/functions-and-operators) like `SELECT current_timestamp();`.
</Tip>

## Required privileges

The user must have the `SELECT` [privilege](/docs/v26.1/security-reference/authorization#managing-privileges) on the tables used as operands.
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        <code>DISTINCT</code> or <code>ALL</code>
      </td>
      <td>
        See <a href="#eliminate-duplicate-rows">Eliminate Duplicate Rows</a>.
      </td>
    </tr>
    <tr>
      <td>
        <code>DISTINCT ON ( a&#95;expr &#91;, ...&#93; )</code>
      </td>
      <td>
        <code>DISTINCT ON</code> followed by a list of <a href="/docs/v26.1/scalar-expressions">scalar expressions</a>{" "}
        within parentheses. See <a href="#eliminate-duplicate-rows">Eliminate Duplicate Rows</a>.
      </td>
    </tr>
    <tr>
      <td>
        <code>target&#95;elem</code>
      </td>
      <td>
        A <a href="/docs/v26.1/scalar-expressions">scalar expression</a> to compute a column in each result row.
        <br />
        <br />
        <code>&#42;</code> automatically retrieves all columns. If a column has the{" "}
        <a href="/docs/v26.1/create-table#not-visible-property">
          <code>NOT VISIBLE</code> property
        </a>
        , it is not returned with <code>&#42;</code>.<br />
        <br />
        If <code>target&#95;elem</code> contains an{" "}
        <a href="/docs/v26.1/functions-and-operators#aggregate-functions">aggregate function</a>, a{" "}
        <code>GROUP BY</code> clause can be used to further control the aggregation.
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
        The <a href="/docs/v26.1/table-expressions">table expression</a> you want to retrieve data from.
        <br />
        <br />
        Using two or more table expressions in the <code>FROM</code> sub-clause, separated with a comma, is equivalent
        to a{" "}
        <a href="/docs/v26.1/joins">
    ...
    <tr>
      <td>
        <code>AS OF SYSTEM TIME timestamp</code>
      </td>
      <td>
        Retrieve data as it existed{" "}
        <a href="/docs/v26.1/as-of-system-time">
          as of <code>timestamp</code>
        </a>
        .<br />
        <br />
        <strong>Note</strong>: Because <code>AS OF SYSTEM TIME</code> returns historical data, your reads might be
        stale.
      </td>
    </tr>
    <tr>
      <td>
        <code>WHERE a&#95;expr</code>
      </td>
      <td>
        <a></a> Only retrieve rows that return <code>TRUE</code> for <code>a&#95;expr</code>, which must be a{" "}
        <a href="/docs/v26.1/scalar-expressions">scalar expression</a> that returns Boolean values using columns (e.g.,{" "}
        <code>&lt;column&gt; = &lt;value&gt;</code>).
      </td>
    </tr>
    <tr>
      <td>
        <code>GROUP BY a&#95;expr</code>
      </td>
      <td>
        Group results on one or more columns.
        <br />
        <br />
        When an <a href="/docs/v26.1/functions-and-operators#aggregate-functions">aggregate function</a> follows{" "}
        <code>SELECT</code> as a <code>target&#95;elem</code>, or <code>HAVING</code> as an <code>a&#95;expr</code>, you
        can <a href="#create-aggregate-groups">create aggregate groups</a> on column groupings listed after{" "}
        <code>GROUP BY</code>.<br />
        You can group columns by an alias (i.e., a label assigned to the column with an <code>AS</code> clause) rather
        than the column name.
        <br />
        If aggregate groups are created on a full primary key, any column in the table can be selected as a{" "}
        <code>target&#95;elem</code>, or specified in a <code>HAVING</code> clause.
        <br />
        If a selected column is in a <a href="/docs/v26.1/subqueries">subquery</a>, and the column references a higher
        scope, the column does not need to be included in the <code>GROUP BY</code> clause (if one exists).
        <br />
        <br />
        Using a <code>GROUP BY</code> clause in a statement without an aggregate function is equivalent to using a{" "}
        <a href="#eliminate-duplicate-rows">
          <code>DISTINCT ON</code>
        </a>{" "}
        clause on the grouping columns.
      </td>
    </tr>
    <tr>
      <td>
        <code>HAVING a&#95;expr</code>
      </td>
      <td>
        Only retrieve aggregate function groups that return <code>TRUE</code> for <code>a&#95;expr</code>, which must be
        a <a href="/docs/v26.1/scalar-expressions">scalar expression</a> that returns Boolean values using an aggregate
        function (e.g., <code>&lt;aggregate function&gt; = &lt;value&gt;</code>).
        <br />
        <br />
        <code>HAVING</code> works like the <code>WHERE</code> clause, but for aggregate functions.
      </td>
    </tr>
    <tr>
      <td>
        <code>WINDOW window&#95;definition&#95;list</code>
      </td>
      <td>
        A list of <a href="/docs/v26.1/window-functions#window-definitions">window definitions</a>.
      </td>
    </tr>
  </tbody>
</table>
...
With `ON`, two rows are considered duplicates if they are equal only using the [scalar expressions](/docs/v26.1/scalar-expressions) listed with `ON`. When two rows are considered duplicates according to `DISTINCT ON`, the values from the first `FROM` row in the order specified by [`ORDER BY`](/docs/v26.1/order-by) are used to compute the remaining target expressions. If `ORDER BY` is not specified, CockroachDB will pick any one of the duplicate rows as first row, non-deterministically.
...
Columns without the [Primary Key](/docs/v26.1/primary-key) or [Unique](/docs/v26.1/unique) constraints can have multiple instances of the same value:
...
This _does not_ change the name of the column in the table. To do that, use [`RENAME COLUMN`](/docs/v26.1/alter-table#rename-column).

### Search for string values

Search for partial [string](/docs/v26.1/string) matches in columns using `LIKE`, which supports the following wildcard operators:
...
[Aggregate functions](/docs/v26.1/functions-and-operators#aggregate-functions) perform calculations on retrieved rows.
...
<Note>
  If the group is created on a primary key column, any column in the table can be selected as a `target_elem`. If a
  selected column is in a [subquery](/docs/v26.1/subqueries) that references a higher scope, a `GROUP BY` clause is not
  needed.
</Note>
...
Non-commutative aggregate functions are sensitive to the order in which the rows are processed in the surrounding `SELECT` clause. To specify the order in which input rows are processed, you can add an [`ORDER BY`](/docs/v26.1/order-by) clause within the function argument list.
...
By using the explicit index annotation, you can override [CockroachDB's index selection](https://www.cockroachlabs.com/blog/index-selection-cockroachdb-2) and use a specific [index](/docs/v26.1/indexes) when reading from a named table. This is called an _index hint_.

Index selection can impact [performance](/docs/v26.1/performance-best-practices-overview), but does not change the result of a query.

<Tip>
  <InlineImage alt="Megaphone" src="/images/common/icon-megaphone.png" /> New in v26.1: You can use [hint
  injection](/docs/v26.1/cost-based-optimizer#hint-injection) to apply index hints without modifying the original query
  text.
</Tip>
...
Forcing a reverse scan can help with [performance tuning](/docs/v26.1/performance-best-practices-overview). To choose an index and its scan direction:
...
When a direction is specified, that scan direction is forced; otherwise the [cost-based optimizer](/docs/v26.1/cost-based-optimizer) is free to choose the direction it calculates will result in the best performance.

You can verify that the optimizer is choosing your desired scan direction using [`EXPLAIN (OPT)`](/docs/v26.1/explain#opt-option). For example, given the table
...
To force a scan of any [inverted index](/docs/v26.1/inverted-indexes) of the hinted table:
...
To force a [partial index scan](/docs/v26.1/partial-indexes), your statement must have a `WHERE` clause that implies the partial index filter.
...
To force a [partial GIN index](/docs/v26.1/inverted-indexes#partial-gin-indexes) scan, your statement must have a `WHERE` clause that:
...
<Tip>
  ...
  scans](/docs/v26.1/performance-best-practices-overview#prevent-the-optimizer-from-planning-full-scans).
</Tip>

### Select historical data (time-travel)

CockroachDB lets you find data as it was stored at a given point in time using `AS OF SYSTEM TIME` with various [supported formats](/docs/v26.1/as-of-system-time). This can be also advantageous for performance. For more details, see [`AS OF SYSTEM
TIME`](/docs/v26.1/as-of-system-time).

## Advanced uses of `SELECT` clauses

CockroachDB supports numerous ways to combine results from `SELECT` clauses together.

See [Selection Queries](/docs/v26.1/selection-queries) for details. A few examples follow.

### Sorting and limiting query results

To order the results of a `SELECT` clause or limit the number of rows in the result, you can combine it with `ORDER BY` or `LIMIT` / `OFFSET` to form a [selection query](/docs/v26.1/selection-queries) or [subquery](/docs/v26.1/table-expressions#use-a-subquery).

See [Ordering Query Results](/docs/v26.1/order-by) and [Limiting Query Results](/docs/v26.1/limit-offset) for more details.

<Note>
  When `ORDER BY` is not included in a query, rows are not sorted by any consistent criteria. Instead, CockroachDB
  returns them as the coordinating node receives them. Also, CockroachDB sorts [`NULL`
  values](/docs/v26.1/null-handling#nulls-and-sorting) first with `ASC` and last with `DESC`. This differs from
  PostgreSQL, which sorts `NULL` values last with `ASC` and first with `DESC`.
</Note>
...
- Using [`JOIN` expressions](/docs/v26.1/joins) to combine rows according to conditions on specific columns.
- Using [set operations](/docs/v26.1/selection-queries#set-operations) to combine rows using inclusion/exclusion rules.
...
<Note>
  By default, CockroachDB uses the `SELECT... FOR UPDATE` locking mechanism during the initial row scan performed in
  [`UPDATE`](/docs/v26.1/update) and [`UPSERT`](/docs/v26.1/upsert) statement execution. To turn off implicit `SELECT...
  FOR UPDATE` locking for `UPDATE` and `UPSERT` statements, set the `enable_implicit_select_for_update` [session
  variable](/docs/v26.1/set-vars) to `false`.
</Note>
...
- [Selection Clauses](/docs/v26.1/selection-queries#selection-clauses)
- [`SELECT FOR UPDATE`](/docs/v26.1/select-for-update)
- [Set Operations](/docs/v26.1/selection-queries#set-operations)
