...
The `encrypt` and `encrypt_iv` functions encrypt a column's data with a given key and cipher method. For more information, see [Cryptographic functions](/docs/v26.1/functions-and-operators#cryptographic-functions).
...
If you do not have a [license](/docs/v26.1/licensing-faqs), you will see an error message like the following if you try to use them:
...
The `decrypt` and `decrypt_iv` functions decrypt an encrypted column's data with a given key and cipher method. For more information, see [Cryptographic functions](/docs/v26.1/functions-and-operators#cryptographic-functions).
...
<Note>
The columns that will store the encrypted values must be of type [`BYTES`](/docs/v26.1/bytes) as shown below.
</Note>
