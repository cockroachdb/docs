...
CockroachDB Advanced users can use the <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/cloud-api" data-versioned="true">Cloud API</a> to configure metrics export to [Amazon CloudWatch](https://aws.amazon.com/cloudwatch), [Datadog](https://www.datadoghq.com/), [Prometheus](https://prometheus.io/), or [Azure Monitor](https://learn.microsoft.com/azure/azure-monitor/essentials/data-platform-metrics). Once the export is configured, metrics will flow from all nodes in all regions of your CockroachDB Advanced cluster to your chosen cloud metrics sink.

<Tip>
To export metrics from a CockroachDB self-hosted cluster, refer to <a href="/docs/VERSION_PLACEHOLDER/monitoring-and-alerting" data-versioned="true">Monitoring and Alerting</a> instead of this page.
</Tip>
...
Exporting metrics to Amazon CloudWatch is only available on CockroachDB Advanced clusters that are hosted on AWS. If your CockroachDB Advanced cluster is hosted on Azure, you can export metrics to <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/export-metrics-advanced?filters=azure-monitor-metrics-export" data-versioned="true">Azure Monitor</a>. If your CockroachDB Advanced cluster is hosted on GCP, you can export metrics to <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/export-metrics-advanced?filters=datadog-metrics-export" data-versioned="true">Datadog</a> or <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/export-metrics-advanced?filters=prometheus-metrics-export" data-versioned="true">Prometheus</a> instead.
...
3. Determine your cluster's cloud provider account ID. This command uses the third-party JSON parsing tool [`jq`](https://stedolan.github.io/jq/download) to isolate just the needed `account_id` field:
    ...
    - `{secret_key}` is your CockroachDB Advanced API key. See <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/managing-access" data-versioned="true">API Access</a> for more details.
...
8. Issue the following <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/cloud-api" data-versioned="true">Cloud API</a> command to enable metrics export for your CockroachDB Advanced cluster:
    ...
    - `{secret_key}` is your CockroachDB Advanced API key. See <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/managing-access" data-versioned="true">API Access</a> for instructions on generating this key.
...
For CockroachDB Advanced clusters hosted on AWS and GCP: This feature is in <a href="/docs/VERSION_PLACEHOLDER/cockroachdb-feature-availability" data-versioned="true">general availability</a>.
</Note>
...
1. Find your CockroachDB Advanced cluster ID:
    ...
    2. Click on the name of your cluster.
    3. Find your cluster ID in the URL of the single cluster overview page: `https://cockroachlabs.cloud/cluster/{your_cluster_id}/overview`. The ID should resemble `f78b7feb-b6cf-4396-9d7f-494982d7d81e`.
2. Issue the following <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/cloud-api" data-versioned="true">Cloud API</a> command to enable metrics export for your CockroachDB Advanced cluster:
...
4. You can test a scrape endpoint by using the following Cloud API command:
    ...
    - `{cluster_id}` is your CockroachDB Advanced cluster ID as determined in step 1, resembling `f78b7feb-b6cf-4396-9d7f-494982d7d81e`.
    - `{cluster_region}` is a region of your CockroachDB Advanced cluster as shown in the `targets` of step 3, such as `us-east4`. You can also find your cluster’s region(s) on the <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/cluster-overview-page" data-versioned="true">Cluster Overview page</a>.
    - `{secret_key}` is your CockroachDB Advanced API key. See <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/managing-access" data-versioned="true">API Access</a> for instructions on generating this key.
...
<Note>
Exporting Metrics to Azure Monitor from a CockroachDB Advanced cluster is in **<a href="/docs/VERSION_PLACEHOLDER/cockroachdb-feature-availability" data-versioned="true">limited access</a>** and is only available to enrolled organizations. To enroll your organization, contact your Cockroach Labs account team. This feature is subject to change.
</Note>

Exporting metrics to Azure Monitor is available only on CockroachDB Advanced clusters that are hosted on Azure. If your CockroachDB Advanced cluster is hosted on AWS, you can export metrics to <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/export-metrics-advanced?filters=aws-metrics-export#enable-metrics-export" data-versioned="true">Amazon CloudWatch</a>. If it is hosted on GCP, you can export metrics to <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/export-metrics-advanced?filters=datadog-metrics-export#enable-metrics-export" data-versioned="true">Datadog</a> or <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/export-metrics-advanced?filters=prometheus-metrics-export#enable-metrics-export" data-versioned="true">Prometheus</a> instead.
...
1. Find your CockroachDB Advanced cluster ID, which will be used in step 5:
    ...
    2. Click on the name of your cluster.
    3. Find your cluster ID in the URL of the single cluster overview page: `https://cockroachlabs.cloud/cluster/{your_cluster_id}/overview`. The ID should resemble `f78b7feb-b6cf-4396-9d7f-494982d7d81e`.
2. Create a private key and certificate pair, using a tool such as OpenSSL. Refer to <a href="/docs/VERSION_PLACEHOLDER/create-security-certificates-openssl#step-1-create-the-ca-key-and-certificate-pair" data-versioned="true">Create the CA key and certificate pair</a>. In this example, the private key file is named `ca.key` and the certificate file is named `ca.crt`. If you use other file names, adjust the commands accordingly. The certificate file will be uploaded in step 3.
...
5. To enable metrics export for your CockroachDB Advanced cluster, issue the following <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/cloud-api" data-versioned="true">Cloud API</a> command:
