...
This page explains how to organize and manage access to your organization's clusters with folders using the CockroachDB Cloud Console. You can also use the <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/cloud-api" data-versioned="true">CockroachDB Cloud API</a>, or the [Terraform provider](https://registry.terraform.io/providers/cockroachdb/cockroach) v1.1.0 or above. For more details about managing access to resources, refer to <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/managing-access" data-versioned="true">Managing Users, Roles, and Service Accounts in</a>.

## How folders work

Folders allow you to organize and manage access to your clusters according to your organization's requirements, and to <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/billing-management#view-invoices" data-versioned="true">summarize billing by folder</a>. For example, you can create top-level folders for each business unit in your organization, and within those folders, you can organize clusters by geographic location and then by their level of maturity, such as production, staging, and testing. For more details, refer to [Folder Structure](#folder-structure).
...
- A cluster or folder created at the root level of the organization has no parent ID. When using the <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/cloud-api" data-versioned="true">CockroachDB Cloud API</a>, you can either omit the parent ID for a folder or cluster, or set it to `root`, to create it at or move it to the root level.
...
A role assigned to a folder is inherited on its descendant folders and clusters. All existing organizational roles, such as <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/authorization#cluster-admin" data-versioned="true">Cluster Admin</a> or <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/authorization#cluster-creator" data-versioned="true">Cluster Creator</a>, can be assigned at the folder scope.
...
The <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/authorization#folder-admin" data-versioned="true">Folder Admin</a> and <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/authorization#folder-mover" data-versioned="true">Folder Mover</a> roles explicitly allow management of folders and their contents.
...
- <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/authorization#folder-admin" data-versioned="true">Folder Admin</a>

<Tip>
An <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/authorization#organization-admin" data-versioned="true">Organization Admin</a> can assign themselves, another user, or a service account the Folder Admin role.
</Tip>
...
3. Set **Scope** to **Organization** or to a folder in the hierarchy. The role is assigned on all of the folder's descendants.
...
2. In the row for the target member, click the three-dots **Action** button and select **Edit Roles**.
3. Set **Scope** to the folder you just created. The role is assigned on all of the folder's descendants.
4. Set **Role** to **Folder Admin** or **Folder Mover**.
...
- <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/authorization#folder-admin" data-versioned="true">Folder Admin</a> or <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/authorization#folder-mover" data-versioned="true">Folder Mover</a>.
- <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/authorization#cluster-admin" data-versioned="true">Cluster Admin</a>, <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/authorization#cluster-developer" data-versioned="true">Cluster Developer</a>, <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/authorization#cluster-creator" data-versioned="true">Cluster Creator</a>, or <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/authorization#cluster-operator" data-versioned="true">Cluster Operator</a>.
...
- <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/authorization#cluster-admin" data-versioned="true">Cluster Admin</a> or <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/authorization#cluster-creator" data-versioned="true">Cluster Creator</a>
...
5. To assign others roles directly on the newly-created cluster:
    1. Go to **Organization** &gt; **Access Management**.
    2. In the row for the target member, click the three-dots **Action** button and select **Edit Roles**.
    3. Set **Scope** to the folder you just created.
    4. Set **Role** to the role you want to assign.
    5. Click **Confirm**.
...
<Note>
A cluster is <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/billing-management#view-invoices" data-versioned="true">billed</a> to the folder it resides in at the end of the billing period, even if it was moved from one folder to another during the billing period.
</Note>
...
- An organization can have a maximum of 65 folders, regardless of how they are organized.
- You can manage folders using the CockroachDB Cloud Console, the <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/cloud-api" data-versioned="true">CockroachDB Cloud API</a>, the [Terraform provider](https://registry.terraform.io/providers/cockroachdb/cockroach) v1.1.0 or above, or the <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/ccloud-reference#manage-folders" data-versioned="true">`ccloud` CLI tool</a>.

## See also

- <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/authorization" data-versioned="true">CockroachDB Cloud Access Management (Authorization) Overview</a>
- <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/managing-access" data-versioned="true">Manage Users, Roles, and Service Accounts</a>
