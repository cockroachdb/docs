...
This page has instructions for migrating data from a CockroachDB Standard cluster to a CockroachDB Advanced cluster, by exporting to CSV and using <a href="/docs/VERSION_PLACEHOLDER/import-into" data-versioned="true">`IMPORT INTO`</a>. You may want to migrate to CockroachDB Advanced if you want a single-tenant cluster with no shared resources.

The following steps use sample data from the <a href="/docs/VERSION_PLACEHOLDER/cockroach-workload#workloads" data-versioned="true">`tpcc` workload</a>.

## Before you begin

These instructions assume you already have the following:

- A <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/quickstart" data-versioned="true">CockroachDB Standard cluster</a> from which you want to migrate data.
- A <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/quickstart-trial-cluster" data-versioned="true">CockroachDB Advanced cluster</a>.
- <a href="/docs/VERSION_PLACEHOLDER/use-cloud-storage" data-versioned="true">Cloud storage</a>.
...
1. <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/connect-to-your-cluster" data-versioned="true">Connect to your CockroachDB Standard cluster</a> and run the <a href="/docs/VERSION_PLACEHOLDER/export" data-versioned="true">`EXPORT`</a> statement for each table you need to migrate. For example, the following statement exports the `warehouse` table from the <a href="/docs/VERSION_PLACEHOLDER/cockroach-workload#workloads" data-versioned="true">`tpcc`</a> database to an Amazon S3 bucket:
...
2. Repeat this step for each table you want to migrate. For example, export one more table (`district`) from the `tpcc` database:
    ...
    <Tip>
    For more information about using cloud storage with CockroachDB, refer to <a href="/docs/VERSION_PLACEHOLDER/use-cloud-storage" data-versioned="true">Use Cloud Storage</a>.
    </Tip>

## Step 2. Import the CSV

<Tip>
For best practices for optimizing import performance in CockroachDB, refer to <a href="/docs/VERSION_PLACEHOLDER/import-performance-best-practices" data-versioned="true">Import Performance Best Practices</a>.
</Tip>

1. <a href="/docs/VERSION_PLACEHOLDER/cockroachcloud/connect-to-your-cluster" data-versioned="true">Connect to your CockroachDB Advanced cluster</a> and <a href="/docs/VERSION_PLACEHOLDER/create-database" data-versioned="true">create the database</a> you want to import the tables into. For example:

    ```sql
    CREATE DATABASE tpcc;
    ```
2. Write a <a href="/docs/VERSION_PLACEHOLDER/create-table" data-versioned="true">`CREATE TABLE`</a> statement that matches the schema of the table data you're importing.

    <Tip>
    You can use the <a href="/docs/VERSION_PLACEHOLDER/show-create#show-the-create-table-statement-for-a-table" data-versioned="true">`SHOW CREATE TABLE`</a> statement in the CockroachDB Standard cluster to view the `CREATE` statement for the table you're migrating.
    </Tip>
...
3. Repeat the previous command for each CSV file you want to import. For example, import the `tpcc.district` data:
    ...
    (1 row)
    ```
4. **(Optional)** To verify that the data was imported, use <a href="/docs/VERSION_PLACEHOLDER/show-tables" data-versioned="true">`SHOW TABLES`</a>:
...
Once all of the tables you want to migrate have been imported into the CockroachDB Advanced cluster, add the <a href="/docs/VERSION_PLACEHOLDER/foreign-key" data-versioned="true">foreign key</a> relationships. To do this, use <a href="/docs/VERSION_PLACEHOLDER/alter-table#add-constraint" data-versioned="true">`ALTER TABLE... ADD CONSTRAINT`</a>. For example:
...
- <a href="/docs/VERSION_PLACEHOLDER/migrate-from-csv" data-versioned="true">Migrate from CSV</a>
- <a href="/docs/VERSION_PLACEHOLDER/import-performance-best-practices" data-versioned="true">Import Performance Best Practices</a>
- <a href="/docs/VERSION_PLACEHOLDER/cockroach-sql" data-versioned="true">Use the Built-in SQL Client</a>
- <a href="/docs/VERSION_PLACEHOLDER/cockroach-commands" data-versioned="true">Other Cockroach Commands</a>
