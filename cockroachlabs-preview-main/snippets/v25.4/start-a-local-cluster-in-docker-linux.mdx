...
1. Start the cluster node and configure it to listen on port 26257 for SQL clients and run DB Console on port 8080.
    ...
    docker run -d \
      ...
      cockroachdb/cockroach:v25.4.6 start-single-node \
...
2. After the cluster is initialized, the cluster node prints helpful [startup details](/docs/v25.4/cockroach-start#standard-output) to its log, including the DB Console URL and the SQL connection string. To retrieve `roach-single`'s startup details:
