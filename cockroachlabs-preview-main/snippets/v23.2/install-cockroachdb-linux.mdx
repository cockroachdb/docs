...
- To install a FIPS-compliant CockroachDB binary, refer to [Install a FIPS-compliant build of CockroachDB](/docs/v23.2/fips) instead of this page.
...
1. Visit [Releases](/docs/releases) to download the CockroachDB archive for the architecture of your Linux host. The archive contains the `cockroach` binary and the supporting libraries that are used to provide [spatial features](/docs/v23.2/spatial-data-overview). Extract the archive and optionally copy the `cockroach` binary into your `PATH` so you can execute [cockroach commands](/docs/v23.2/cockroach-commands) from any shell. If you get a permission error, use `sudo`.

    <Note>
    If you plan to use CockroachDB's [spatial features](/docs/v23.2/spatial-data-overview), you must complete the following steps. Otherwise, your installation is now complete.
    </Note>
2. CockroachDB uses custom-built versions of the [GEOS](/docs/v23.2/architecture/glossary#geos) libraries. Copy these libraries to one of the locations where CockroachDB expects to find them.
    ...
    2. Copy the library files to the directory. In the following commands, replace `{ARCHITECTURE}` with `linux-amd64` for Intel, or with `linux-arm64` for ARM.
        ...
        ```shell
        cp -i cockroach-v23.2.29.linux-{ARCHITECTURE}/lib/libgeos.so /usr/local/lib/cockroach/
        ```

        ```shell
        cp -i cockroach-v23.2.29.linux-{ARCHITECTURE}/lib/libgeos_c.so /usr/local/lib/cockroach/
        ```
...
- [Orchestrate CockroachDB Locally with Minikube](/docs/v23.2/orchestrate-a-local-cluster-with-kubernetes)
- [Orchestrate CockroachDB in a Single Kubernetes Cluster](/docs/v23.2/deploy-cockroachdb-with-kubernetes)
- [Orchestrate CockroachDB Across Multiple Kubernetes Clusters](/docs/v23.2/orchestrate-cockroachdb-with-kubernetes-multi-cluster)
...
CockroachDB on ARM is in **[Limited Access](/docs/v23.2/cockroachdb-feature-availability#feature-availability-phases)** in v22.2.13, and is **experimental** in all other versions. Experimental images are not qualified for production use and not eligible for support or uptime SLA commitments.
...
3. Pull the image for the v23.2.29 release of CockroachDB from [Docker Hub](https://hub.docker.com/r/cockroachdb/cockroach):
    ...
    ```text
    $ sudo docker pull cockroachdb/cockroach:v23.2.29
    ```
4. Keep up-to-date with CockroachDB releases and best practices:
