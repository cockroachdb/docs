...
Once you've [installed the official CockroachDB Docker image](/docs/v23.2/install-cockroachdb), it's simple to run an insecure multi-node cluster across multiple Docker containers on a single host, using Docker volumes to persist node data.
...
- Make sure you have already [installed the official CockroachDB Docker image](/docs/v23.2/install-cockroachdb).
...
1. Start the first node and configure it to listen on `roach1:26257` for SQL clients and `roach1:8080` for the DB Console and to publish these ports, and to use `roach1:26357`for inter-node traffic. The Docker host will forward traffic to a published port to the publishing container.
    ...
    cockroachdb/cockroach:v23.2.29 start '
...
2. This command creates a container and starts the first CockroachDB node inside it. Take a moment to understand each part:
    ...
    - `cockroachdb/cockroach:v23.2.29 start (...) --join`: The CockroachDB command to [start a node](/docs/v23.2/cockroach-start) in the container. The `--advertise-addr`, `--http-addr`, `--listen-addr`, and `--sql-addr` flags cause CockroachDB to listen on separate ports for inter-node traffic, DB Console traffic, and SQL traffic. The `--join` flag contains each node's hostname or IP address and the port where it listens for inter-node traffic from other nodes.
...
5. Perform a one-time initialization of the cluster. This example runs the `cockroach init` command from within the `roach1` container, but you can run it from any container or from an external system that can reach the Docker host.
    ...
    build:               CCL v23.2.29 @ 2026-02-19 00:00:00 (go1.19.6)
...
Now that your cluster is live, you can use any node as a SQL gateway. To test this out, let's use the `docker exec` command to start the [built-in SQL shell](/docs/v23.2/cockroach-sql) in the `roach1` container.
...
The [DB Console](/docs/v23.2/ui-overview) gives you insight into the overall health of your cluster as well as the performance of the client workload.
...
2. On the [**Cluster Overview**](/docs/v23.2/ui-cluster-overview-page), notice that three nodes are live, with an identical replica count on each node:
