...
Once you've [installed the official CockroachDB Docker image](/docs/v26.1/install-cockroachdb), it's simple to run an insecure multi-node cluster across multiple Docker containers on a single host, using Docker volumes to persist node data.
...
1. Start the first node and configure it to listen on `roach1:26257` for SQL clients and `roach1:8080` for the DB Console and to publish these ports, and to use `roach1:26357`for inter-node traffic. The Docker host will forward traffic to a published port to the publishing container.
    ...
    cockroachdb/cockroach:v26.1.1 start '
...
2. This command creates a container and starts the first CockroachDB node inside it. Take a moment to understand each part:
    ...
    - `cockroachdb/cockroach:v26.1.1 start (...) --join`: The CockroachDB command to [start a node](/docs/v26.1/cockroach-start) in the container. The `--advertise-addr`, `--http-addr`, `--listen-addr`, and `--sql-addr` flags cause CockroachDB to listen on separate ports for inter-node traffic, DB Console traffic, and SQL traffic. The `--join` flag contains each node's hostname or IP address and the port where it listens for inter-node traffic from other nodes.
...
5. Perform a one-time initialization of the cluster. This example runs the `cockroach init` command from within the `roach1` container, but you can run it from any container or from an external system that can reach the Docker host.
    ...
    build:               CCL v26.1.1 @ 2026-03-09 00:00:00 (go1.19.6)
...
Now that your cluster is live, you can use any node as a SQL gateway. To test this out, let's use the `docker exec` command to start the [built-in SQL shell](/docs/v26.1/cockroach-sql) in the `roach1` container.
...
CockroachDB also comes with a number of [built-in workloads](/docs/v26.1/cockroach-workload) for simulating client traffic. Let's run the workload based on CockroachDB's sample vehicle-sharing application, [MovR](/docs/v26.1/movr).
...
The [DB Console](/docs/v26.1/ui-overview) gives you insight into the overall health of your cluster as well as the performance of the client workload.
...
2. On the [**Cluster Overview**](/docs/v26.1/ui-cluster-overview-page), notice that three nodes are live, with an identical replica count on each node:

    <Frame>
      ![DB Console](/images/v26.1/ui_cluster_overview_3_nodes.png)
    </Frame>

    This demonstrates CockroachDB's [automated replication](/docs/v26.1/demo-replication-and-rebalancing) of data via the Raft consensus protocol.

    <Note>
    Capacity metrics can be incorrect when running multiple nodes on a single machine. For more details, refer to [Capacity metrics](/docs/v26.1/ui-cluster-overview-page#capacity-metrics).
    </Note>
3. Click [**Metrics**](/docs/v26.1/ui-overview-dashboard) to access a variety of time series dashboards, including graphs of SQL queries and service latency over time:

    <Frame>
      ![DB Console](/images/v26.1/ui_overview_dashboard_3_nodes.png)
    </Frame>
4. Use the [**Databases**](/docs/v26.1/ui-databases-page), [**Statements**](/docs/v26.1/ui-statements-page), and [**Jobs**](/docs/v26.1/ui-jobs-page) pages to view details about your databases and tables, to assess the performance of specific queries, and to monitor the status of long-running operations like schema changes, respectively.
...
The CockroachDB [DB Console](/docs/v26.1/ui-overview) gives you insight into the overall health of your cluster as well as the performance of the client workload.
...
- Learn more about [CockroachDB SQL](/docs/v26.1/learn-cockroachdb-sql) and the [built-in SQL client](/docs/v26.1/cockroach-sql)
- [Install the client driver](/docs/v26.1/install-client-drivers) for your preferred language
- Further explore CockroachDB capabilities like [fault tolerance and automated repair](/docs/v26.1/demo-cockroachdb-resilience), [multi-region performance](/docs/v26.1/demo-low-latency-multi-region-deployment), [serializable transactions](/docs/v26.1/demo-serializable), and [JSON support](/docs/v26.1/demo-json-support)
