...
To add the [Foreign Key](foreign-key.html) constraint to an existing column or group of columns, you must first use [`CREATE INDEX`](create-index.html) to index the column or columns that you plan to constrain. Then you can use the `ADD CONSTRAINT` statement to add the Foreign Key constraint to the column or columns.
