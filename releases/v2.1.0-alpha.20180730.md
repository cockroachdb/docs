...
- The `SHOW TRACE FOR <stmt>` statement was incomplete and incorrect and has thus been removed. To turn on tracing, use `SET tracing = ...` and `SHOW SESSION TRACE`, or enable the new `auto_trace` client-side option for `cockroach sql`. [#27805][#27805] {% comment %}doc{% endcomment %}instead. [#26729][#26729] {% comment %}doc{% endcomment %}
...
- Added the `sql.distsql.{flow_stream_timeout,max_running_flows}` [cluster setting](../v2.1/cluster-settings.html) to fine-tune flow setup [#27404][#27404] {% comment %}doc{% endcomment %}
