...
Core | **Automated TLS Cert Setup** | Using the new `cockroach connect` command and a secure token you provide, you can now [automate the creation and distribution of a cluster's CA (certificate authority) and node certificates](../v21.1/auto-tls.html). Note that this feature is an alpha release with core functionality that may not meet your requirements.
