...
**Bulk Import Improvements** | The [`IMPORT`](../v20.1/import.html) and [`IMPORT INTO`](../v20.1/import-into.html) statements now support bulk importing [from Avro files](../v20.1/migrate-from-avro.html) and [from local storage](../v20.1/import.html#import-a-table-from-a-local-file). The former makes it easier to migrate from systems like Spanner that export data in the Avro format, and the latter makes it possible to import without cloud storage. The new [`cockroach nodelocal upload`](../v20.1/cockroach-nodelocal-upload.html) command can be used to upload a file to a node's external IO directory for local import.<br><br>Also, [paused](../v20.1/pause-job.html) imports, when [resumed](../v20.1/resume-job.html), now continue from their internally recorded progress instead of starting over.
**RBAC Changes** | All [role-based access control (RBAC) features](../v20.1/authorization.html#roles) (`CREATE ROLE`, `ALTER ROLE`, `DROP ROLE`, `GRANT ROLE`, `REVOKE ROLE`) are now [BSL features](../v20.1/licensing-faqs.html) and available to non-enterprise users.
...
Feature | Description
--------|------------
**Full-cluster backups** | You can now use CockroachDB's `BACKUP` feature to [back up an entire cluster's data](../v20.1/backup.html#backup-targets), including all configuration and system information such as [user privileges](../v20.1/authorization.html#privileges), [zone configurations](../v20.1/configure-replication-zones.html), and [cluster settings](../v20.1/cluster-settings.html). In rare disaster recovery situations, you can use the `RESTORE` feature to [restore a full cluster backup to a new cluster](../v20.1/restore.html#full-cluster). Restoring a full cluster backup to an existing cluster is not supported.
**Encrypted backups** | You can now use an encryption key to [encrypt data in Enterprise `BACKUP` files](../v20.1/backup.html#encrypted-backups), and to [decrypt the data upon `RESTORE`](../v20.1/restore.html#restore-from-an-encrypted-backup).
