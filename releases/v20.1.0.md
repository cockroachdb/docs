...
Area | Feature | Description
-----|---------|------------
Recovery | **Full-cluster backup and restore** | CockroachDB's `BACKUP` feature now supports [backing up an entire cluster's data](../v20.1/backup-and-restore.html#full-backups), including all configuration and system information such as [user privileges](../v20.1/authorization.html#privileges), [zone configurations](../v20.1/configure-replication-zones.html), and [cluster settings](../v20.1/cluster-settings.html). In rare disaster recovery situations, CockroachDB's `RESTORE` feature can now [restore a full cluster backup to a new cluster](../v20.1/restore.html#full-cluster). Restoring a full cluster backup to an existing cluster is not supported.
