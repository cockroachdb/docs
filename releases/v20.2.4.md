...
- A table can now be successfully dropped in a transaction following other [schema changes](../v20.2/online-schema-changes.html) to the table in the same transaction. [#58255][#58255]
- The `crdb_internal.cluster_id` [function](../v20.2/functions-and-operators.html) now returns the ID of the underlying KV cluster in multi-tenant scenarios rather than the [Nil UUID](https://www.ietf.org/rfc/rfc4122.html#section-4.1.7). [#58441][#58441]
