...
- New command `IMPORT CSV` is now available and provides the fastest way to import large amounts of data into CockroachDB. This command is only available in builds that include CCL-licensed code, but using it does not require an enterprise license. TODO(jseldess): work with product/marketing on this language [#17539](https://github.com/cockroachdb/cockroach/pull/17539)
- The session variable `sql_safe_updates` can be set to disable certain "dangerous" SQL statements, such as `DELETE` or `UPDATE` without a `WHERE` clause. It defaults to true for the `cockroach sql` shell. [#17604](https://github.com/cockroachdb/cockroach/pull/17604)
...
- Query cancellation is now supported for queries using the distributed SQL engine. [#17527](https://github.com/cockroachdb/cockroach/pull/17527)
- The output of `SHOW SESSIONS` now includes a `last_active_query` column. [#17579](https://github.com/cockroachdb/cockroach/pull/17579)
- The `SET TRANSACTION` statement no longer requires commas when multiple directives are given. [#17723](https://github.com/cockroachdb/cockroach/pull/17723)
...
- Statements that spawn jobs (`BACKUP`, `RESTORE`, schema changes) no longer appear in `SHOW QUERIES`. [#17786](https://github.com/cockroachdb/cockroach/pull/17786)
...
- `cockroach dump` now understands views and orders tables to satisfy foreign key constraints (as long as there are no cycles). [#17581](https://github.com/cockroachdb/cockroach/pull/17581)
- The `cockroach sql` shell now sets a default `application_name` of `cockroach`. [#17602](https://github.com/cockroachdb/cockroach/pull/17602)
- The `cockroach sql` shell now prints hints and warnings when available from the server. [#17604](https://github.com/cockroachdb/cockroach/pull/17604)

### Admin UI Changes

- Various debugging pages are now available at `/#/debug`. [#17635](https://github.com/cockroachdb/cockroach/pull/17635)
- Changes to cluster settings are now recorded in the event log. [#17619](https://github.com/cockroachdb/cockroach/pull/17619) [#17866](https://github.com/cockroachdb/cockroach/pull/17866)
...
- Foreign keys that point to a prefix of an existing index now work correctly. Previously, rows could be deleted from the referenced table without recognizing the constraint violation if rows in the referring table pointed to them. These foreign keys are also now shown correctly in `SHOW CREATE TABLE`. [#17638](https://github.com/cockroachdb/cockroach/pull/17638)
...
- Queries are not removed from `SHOW QUERIES` or `SHOW SESSIONS` until all their results have been sent to the client. [#17579](https://github.com/cockroachdb/cockroach/pull/17579)
...
- Nodes now panic if they fail a consistency check. The `server.consistency_check.interval` setting replacaes the `COCKROACH_CONSISTENCY_CHECK_INTERVAL` environment variable for configuring the consistency checker. [#17853](https://github.com/cockroachdb/cockroach/pull/17853)
- Dropping an interleaved index no longer leaves the table in a broken state. [#17860](https://github.com/cockroachdb/cockroach/pull/17860)
...
- The `sql.defaults.distsql.tempstorage` setting now defaults to true. [#17665](https://github.com/cockroachdb/cockroach/pull/17665)
