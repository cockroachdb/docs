...
- The [`SHOW RANGE FOR ROW`](../v20.2/show-range-for-row.html) now takes a tuple of the row's index columns instead of the full column set of the row. [#50479][#50479]
...
- The docker image available with CockroachDB now includes the GEOS files required to work with Geospatial types in CockroachDB. [#50362][#50362]
...
- Links returned in error messages to point to unimplemented issues now use the CockroachLabs redirect/short-link server. [#49836][#49836]
...
- `BACKUP` can now run in `detached` mode, which means the Job ID is now returned immediately without waiting for the `BACKUP` job to finish. [#50775][#50775]
...
- The [vectorized execution engine](../v20.2/cost-based-optimizer.html) now supports `AVG` and `SUM` aggregate function on `Int`s and `Interval`s. [#49900][#49900]
- The [vectorized execution engine](../v20.2/cost-based-optimizer.html) now fully supports `SUM` aggregate function (previously, the summation of integers was incomplete). [#49900][#49900]
- The [vectorized execution engine](../v20.2/cost-based-optimizer.html) now supports `MIN` and `MAX` aggregate functions on columns of `INT2` and `INT4` [`types`](../v20.2/data-types.html). [#49900][#49900]
...
- The command-line flag `--log-dir-max-size` is now deprecated and has been replaced with a new flag named `--log-group-max-size`. The flags limit the combined size of all files generated by one *logging group* inside CockroachDB. Currently, the following logging groups are already defined:  
| File name prefix      | Description                                                                                                | |-----------------------|------------------------------------------------------------------------------------------------------------| | `cockroach-rocksdb`   | RocksDB Storage logs                                                                                       | | `cockroach-pebble`    | Pebble Storage logs                                                                                        | | `cockroach-sql-exec`  | SQL Executed Statements when `sql.trace.log_statement_execute` is set                                      | | `cockroach-auth`      | SQL Connections and Authentication events when `server.auth_log.sql_{connections,sessions}.enabled` is set | | `cockroach-sql-audit` | SQL Table Access Audit events                                                                              | | `cockroach`           | General logs ("main logger")                                                                               | [#50599][#50599]
...
- Fixed icons alignment for the Timeframe navigation buttons. [#50010][#50010]
...
- Added a collection of new metrics to track the method of requests sent as RPCs between cluster nodes. These metrics are named `distsender.rpc.<method>.sent`, with examples of `<method>` being `scan` and `conditionalput`. [#46747][#46747]
...
- Previously, if full cluster restore failed while restoring the system table data, it would not clean up properly and leave some temporary tables public and not dropped. This is now fixed. [#50003][#50003]
- The [`Statements` page](../v20.2/admin-ui-statements-page.html) of Admin UI previously displayed incorrect information about whether or not the statement execution was distributed. [#50256][#50256]
...
- CockroachDB now correctly populates `rolvaliduntil` value for roles in `pg_roles` and `pg_authid`. [#50307][#50307]
