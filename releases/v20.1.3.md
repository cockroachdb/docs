...
~~~shell
$ docker pull cockroachdb/cockroach-unstable:v20.1.3
~~~
...
- Fix a bug in which restarting CockroachDB with the Pebble storage engine after a crash during write-ahead logging could, in some rare cases, return an "unexpected EOF" error. [#50282][#50282]
- Previously, the [Admin UI Statements page](../v20.1/admin-ui-statements-page.html) was incorrectly displaying the information about whether the or not a statement was distributed (it was always `Yes`). This is now fixed. [#50347][#50347]
...
- Updated guidance on [node decommissioning](remove-nodes.html). [#7304][#7304]
