...
With the release of CockroachDB v1.1, we’ve included support for faster migrations from other databases for developers, simplified cluster management for operators, and improved performance in real-world environments.
...
- Then [upgrade to CockroachDB v1.1](../v1.1/upgrade-cockroach-version.html). The process of upgrading from v1.0 to v1.1 is different than upgrading within the v1.0.x series, so please follow the upgrade instructions carefully.
...
This section summarizes the most significant, user-facing changes in v1.1. For a complete list of changes, including bug fixes and performance improvements, see the [release notes](index.html#testing-releases) for previous testing releases.
