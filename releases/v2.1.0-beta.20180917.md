...
- [`CHANGEFEED`](../v2.1/change-data-capture.html)s now error when a watched table backfills (instead of undefined behavior). [#29559][#29559]
...
- Enterprise [`CHANGEFEED`](../v2.1/change-data-capture.html)s now correctly skip the initial scan when started with the `cursor=` option. [#29613][#29613]
- Hash functions with `NULL` input now return `NULL`. [#29974][#29974]
- Prevented a very rare premature failure in [`CHANGEFEED`](../v2.1/change-data-capture.html)s caused by a race condition with range splits. [#30009][#30009]
- Fixed a crash when `SELECT MIN(NULL)` was run with the [SQL optimizer](../v2.1/sql-optimizer.html) enabled. [#30014][#30014]
