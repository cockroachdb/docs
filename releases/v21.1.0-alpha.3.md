...
- This change teaches CockroachDB to apply zone configs to new [unique constraints](../v21.1/unique.html) in `REGIONAL BY ROW` tables. [#59364][#59364]
- A new, unused field called `global_reads` was added to [zone configurations](../v21.1/configure-replication-zones). The field does not yet have any effect. [#59304][#59304]
- A new [private cluster setting](../v21.1/vectorized-execution.html) `sql.distsql.temp_storage.hash_agg.enabled` was added that allows users to disable the disk spilling capability of the hash aggregation in the vectorized execution engine. It is `true` by default which incurs some performance hit. Setting it to `false` will improve the performance, but the queries might hit an out-of-memory limit error. [#59414][#59414]
...
- [`REGIONAL BY ROW` tables](../v21.1/create-table.html) now preserve [zone configurations](../v21.1/configure-replication-zones) when using [`ALTER PRIMARY KEY`](../v21.1/alter-primary-key.html). [#59365][#59365]
- Implements [`ALTER TABLE ... LOCALITY REGIONAL BY TABLE`](../v21.1/alter-table.html) from `LOCALITY GLOBAL`. [#59407][#59407]
- [Zone configs](../v21.1/configure-replication-zones) now support new attributes `num_voters` and `voter_constraints`. `num_voters` will specify the number of voting replicas. When `num_voters` is explicitly specified, `num_replicas` will be the sum of voting and non-voting replicas. `voter_constraints` will specify the constraints that govern the placement of just the voting replicas, whereas the existing `constraints` attributes will govern the placement of all replicas (voting as well as non-voting). [#57184][#57184]
...
- Multi-column [inverted indexes](../v21.1/create-index.html#create-inverted-indexes) can now be created. The last indexed column must be inverted types such as [`JSON`](../v21.1/jsonb.html), [`ARRAY`](../v21.1/array.html), `GEOMETRY`, and `GEOGRAPHY`. All preceding columns must have types that are indexable. These indexes may be used for queries that constrain all index columns. [#59565][#59565]
...
- The `channels` parameter for the log sink [configurations](https://github.com/cockroachdb/cockroach/blob/master/docs/generated/logsinks.md) now supports a more flexible input configuration format:  
  ...
  This makes it possible to e.g. do:  ``` sinks:   stderr:     channels:     - DEV     - SQL_SESSIONS ```  which uses the "native" YAML syntax for lists. [#59352][#59352]
- The notification that `SIGHUP` was received, and that log files are flushed to disk as a result, is now sent to the (OPS logging channel)[]. [#59345][#59345]
...
- Added a new API tree, in `/api/v2/*`, currently undocumented, that avoids the use of and cookie-based authentication in favor of sessions in headers, and support for pagination. [#58436][#58436]
...
- Fixed a bug included in 20.2.1 for for the [`JSON`](../v21.1/jsonb.html) fetch value operator, `->` which resulted in chained `->` operators in query filters not being index accelerated, e.g., `j->'a'->'b' = '1'`. Chained `->` are now index accelerated. [#59494][#59494]
...
- Ernie Cordeiro (first-time contributor, CockroachDB team member)
- John Seekins (first-time contributor)
- Marc Shirley (first-time contributor, CockroachDB team member)
...
- Yufen Huang (first-time contributor, CockroachDB team member)
