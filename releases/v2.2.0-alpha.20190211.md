...
- Added binary varbit support over pgwire. [#34050][#34050]
...
- The pgwire result buffer size can now be controlled on a per-connection basis with the `results_buffer_size` connection string parameter. [#34385][#34385] {% comment %}doc{% endcomment %}
...
- [Window functions](../v2.2/window-functions.html) with non-empty `PARTITION BY` and `ORDER BY` clauses are now handled correctly when invoked via an external driver. [#33591][#33591]
...
- `CHANGEFEED`s now can be started on tables that were previously been backfilled by schema changes. [#34317][#34317]
