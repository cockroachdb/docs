...
- Improved the output of `EXPLAIN` for `index-join` and `lookup-join`. [#34138][#34138] {% comment %}doc{% endcomment %}
- `FILTER` expressions are now supported by the cost-based optimizer. [#34077][#34077] {% comment %}doc{% endcomment %}
- `EXPLAIN (OPT)` now has a much shorter output. `EXPLAIN (OPT,VERBOSE)` and `EXPLAIN (OPT,TYPES)` can be used for more verbose output. [#34128][#34128] {% comment %}doc{% endcomment %}
- Using a sequence as a `SELECT` target is now supported by the cost-based optimizer. [#33196][#33196] {% comment %}doc{% endcomment %}
...
- Fixed a bug in `cockroach node status` that prevented it from displaying down nodes in the cluster in some circumstances. [#34448][#34448]
- Fixed several related panics in the optimizer related to plan exploration. [#34667][#34667]
- Resolved a cluster degradation scenario that could occur during `IMPORT`/`RESTORE` operations, manifested through a high number of pending Raft snapshots. [#33582][#33582]
...
- Fixed a panic that occurred when performing an `INSERT ON CONFLICT` with a `SET UPDATE` that uses values from a subquery. [#33553][#33553]
...
- `CHANGEFEED`s now can be started on tables that have been backfilled by schema changes. [#34317][#34317]
