...
- Using [`SESSION_USER`](../{{site.versions["stable"]}}/functions-and-operators.html#special-syntax-forms) in a projection or `WHERE` clause now returns the `SESSION_USER` instead of the `CURRENT_USER`. For backward compatibility, use [`session_user()`](../{{site.versions["stable"]}}/functions-and-operators.html#system-info-functions) for `SESSION_USER` and `current_user()` for `CURRENT_USER`. [#70444][#70444]
...
- Added a new flag `--external-io-enable-non-admin-implicit-access` that can remove the [`admin`](../{{site.versions["stable"]}}/authorization.html#admin-role)-only restriction on interacting with arbitrary network endpoints and using `implicit` auth in operations such as [`BACKUP`](../{{site.versions["stable"]}}/backup.html), [`IMPORT`](../{{site.versions["stable"]}}/import.html), or [`EXPORT`](../{{site.versions["stable"]}}/export.html). [#71594][#71594]
...
- Temporary tables are now [restored](../{{site.versions["stable"]}}/restore.html) to their original database instead of to `defaultdb` during a [full cluster restore](../{{site.versions["stable"]}}/restore.html#full-cluster). Furthermore, `defaultdb` and `postgres` are dropped before a full cluster restore and will only be restored if they are present in the [backup](../{{site.versions["stable"]}}/backup.html) being restored. [#71890][#71890]
...
- Added new job control statements allowing an operator to manipulate all jobs of a specific type: `<Command> ALL <JobType> JOBS`. This is supported in [`CHANGEFEED`](../{{site.versions["stable"]}}/stream-data-out-of-cockroachdb-using-changefeeds.html), [`BACKUP`](../{{site.versions["stable"]}}/backup.html), [`IMPORT`](../{{site.versions["stable"]}}/import.html), and [`RESTORE`](../{{site.versions["stable"]}}/restore.html) jobs. For example: `PAUSE ALL CHANGEFEED JOBS`. [#69314][#69314]
- [`EXPLAIN ANALYZE`](../{{site.versions["stable"]}}/explain-analyze.html) now contains more information about the [MVCC](../{{site.versions["stable"]}}/architecture/storage-layer.html#mvcc) behavior of operators that scan data from disk. [#64503][#64503]
...
- The array [built-in functions](../{{site.versions["stable"]}}/functions-and-operators.html#built-in-functions) (`array_agg`, `array_cat`, `array_position`, etc.) now operate on record types. [#70332][#70332]
...
- Added a new SQL [built-in function](../{{site.versions["stable"]}}/functions-and-operators.html#built-in-functions) `unordered_unique_rowid `, which generates a globally unique 64-bit integer that does not have ordering. [#70338][#70338]
...
- Disallowed cross-database references for sequences by default. This can be enabled with the [cluster setting](../{{site.versions["stable"]}}/cluster-settings.html) `sql.cross_db_sequence_references.enabled`. [#70581][#70581]
...
- Implicit casts performed during [`INSERT`](../{{site.versions["stable"]}}/insert.html) statements now more closely follow PostgreSQL's behavior. Several minor bugs related to these types of casts have been fixed. [#70722][#70722]
- Newly created tables now have `<table_name>_pkey` by default as their index/constraint name. [#70604][#70604]
- A newly created [`FOREIGN KEY`](../{{site.versions["stable"]}}/foreign-key.html) now has the same constraint name as PostgreSQL— `<table>_<cols>_fkey`. Previously, this was `fk_<cols>_ref_<target>`. [#70658][#70658]
...
- Previously, running [`COMMENT ON CONSTRAINT`](../{{site.versions["stable"]}}/comment-on.html) on a table in a schema would succeed but the comment would not actually be created. Now the comment is successfully created. [#71985][#71985]
...
- [`ALTER DATABASE CONVERT TO SCHEMA`](../{{site.versions["stable"]}}/alter-database.html#subcommands) is now disabled in v22.1 and later. [#72000][#72000]
...
- Previously, the output from [`SHOW CREATE VIEW`](../{{site.versions["stable"]}}/show-create.html#show-the-create-view-statement-for-a-view) returned on a single line. The format has now been improved to be more readable. [#73642][#73642]
...
- In the experimental [`RELOCATE`](../{{site.versions["stable"]}}/experimental-features.html#relocate-leases-and-replicas) syntax forms, the positional keyword that indicates that the statement should move non-voter replicas is now spelled `NONVOTERS`, instead of `NON_VOTERS`. [#73803][#73803]
...
- Added the `digest` and `hmac` [built-in functions](../{{site.versions["stable"]}}/functions-and-operators.html). They match the PostgreSQL (pgcrypto) implementation. Supported hash algorithms are `md5`, `sha1`, `sha224`, `sha256`, `sha384`, and `sha512`. [#73935][#73935]
...
- The shard column of a [hash-sharded index](../{{site.versions["stable"]}}/hash-sharded-indexes.html) is now a [virtual column](../{{site.versions["stable"]}}/computed-columns.html) and not a stored computed column. [#74138][#74138]
...
- Statements are now formatted prior to being sent to [the DB Console](../{{site.versions["stable"]}}/ui-statements-page.html). This is done using a new [built-in function](../{{site.versions["stable"]}}/functions-and-operators.html) that formats statements. [#73853][#73853]

### Operational changes

- [`cockroach debug zip`](../{{site.versions["stable"]}}/cockroach-debug-zip.html) now includes the raw `system.settings` table. This table makes it possible to determine whether a [cluster setting](../{{site.versions["stable"]}}/cluster-settings.html) has been explicitly set. [#70498][#70498]
- The meaning of `sql.distsql.max_running_flows` [cluster setting](../{{site.versions["stable"]}}/cluster-settings.html) has been extended so that when the value is negative, it will be multiplied by the number of CPUs on the node to get the maximum number of concurrent remote flows on the node. The default value is `-128`, meaning that a 4-CPU machine will have up to `512` concurrent remote DistSQL flows, but a 8-CPU machine will have up to `1024`. The previous default was `500`. [#71787][#71787]
...
- The [**Advanced Debug**](../{{site.versions["stable"]}}/ui-debug-pages.html) page now contains an additional link under the **Metrics** header called Rules. This endpoint exposes [Prometheus-compatible alerting](../{{site.versions["stable"]}}/monitoring-and-alerting.html#events-to-alert-on) and aggregation rules for CockroachDB metrics. [#72677][#72677]
...
- The **Terminate Session** and **Terminate Query** buttons are again available to be enabled on the [**Sessions Page**](../{{site.versions["stable"]}}/ui-sessions-page.html). [#74408][#74408]
...
- Fixed a bug where `CURRENT_USER` and [`SESSION_USER`](../{{site.versions["stable"]}}/functions-and-operators.html#special-syntax-forms) were parsed incorrectly. [#70439][#70439]
...
- Previously, [`DROP DATABASE CASCADE`](../{{site.versions["stable"]}}/drop-database.html) could fail while resolving a schema in certain scenarios with the following error: `ERROR: error resolving referenced table ID <ID>: descriptor is being dropped`. This is now fixed. [#69789][#69789]
...
- The [`cockroach debug merge-logs`](../{{site.versions["stable"]}}/cockroach-debug-merge-logs.html) command no longer returns an error when the log decoder attempts to parse older logs. [#68282][#68282]
...
- [`IMPORT`](../{{site.versions["stable"]}}/import.html) now respects the spatial index storage options specified in `PGDUMP` files on indexes it creates. [#66903][#66903]
- Fixed `IMPORT` in [`tpcc`](../{{site.versions["stable"]}}/cockroach-workload.html#tpcc-workload) workload. [#71013][#71013]
- Some query patterns that previously could cause a single node to become a [hotspot](../{{site.versions["stable"]}}/query-behavior-troubleshooting.html#single-hot-node) have been fixed so that the load is evenly distributed across the whole cluster. [#70648][#70648]
...
- Previously, adding new values to a user-defined [`ENUM`](../{{site.versions["stable"]}}/enum.html) type would cause a prepared statement using that type to not work. This now works as expected. [#71632][#71632]
...
- Raft snapshots now detect timeouts earlier and avoid spamming the logs with [`context deadline exceeded`](../{{site.versions["stable"]}}/common-errors.html#context-deadline-exceeded) errors. [#73279][#73279]
...
- CockroachDB now prohibits mixed dimension [`LINESTRING`](../{{site.versions["stable"]}}/linestring.html) in `ST_MakePolygon`. [#73489][#73489]
...
- Previously, during [`RESTORE`](../{{site.versions["stable"]}}/restore.html) `system.namespace` entry wouldn't be inserted for synthetic public schemas. This is now fixed. [#73875][#73875]
...
- [Foreign keys](../{{site.versions["stable"]}}/foreign-key.html) referencing a hash-sharded key will not fail anymore. [#74140][#74140]
- Raft snapshots no longer risk starvation under very high concurrency. Before this fix, it was possible that many of Raft snapshots could be starved and prevented from succeeding due to timeouts, which were accompanied by errors like [`error rate limiting bulk io write: context deadline exceeded`](../{{site.versions["stable"]}}/common-errors.html#context-deadline-exceeded). [#73288][#73288]
...
- Fixed a bug that, in very rare cases, could result in a node terminating with a fatal error: `unable to remove placeholder: corrupted replicasByKey map`. To avoid potential data corruption, users affected by this crash should not restart the node, but instead [decommission it](../{{site.versions["stable"]}}/remove-nodes.html) in absentia and have it rejoin the cluster under a new `nodeID`. [#73734][#73734]
- Previously, when [foreign keys](../{{site.versions["stable"]}}/foreign-key.html) were included inside an `ADD COLUMN` statement and multiple columns were added in a single statement then the first added column would have the foreign key applied (or an error generated based on the wrong column). This is now fixed. [#74411][#74411]
- Previously, a double-nested [`ENUM`](../{{site.versions["stable"]}}/enum.html) in a DistSQL query would not get hydrated on remote nodes resulting in panic. This is now fixed. [#74189][#74189]
...
- Previously, setting [`sslmode=require`](../{{site.versions["stable"]}}/connection-parameters.html#additional-connection-parameters) would check for local certificates, so omitting a certs path would cause an error even though `require` does not verify server certificates. This has been fixed by bypassing certificate path checking for `sslmode=require`. This bug had been present since v21.2.0. [#73776][#73776]
- Previously, incorrect results would be returned, or internal errors, on queries with window functions returning [`INT`](../{{site.versions["stable"]}}/data-types.html), `FLOAT`, `BYTES`, `STRING`, `UUID`, or `JSON` type when the [disk spilling](../{{site.versions["stable"]}}/vectorized-execution.html#disk-spilling-operations) occurred. The bug was introduced in v21.2.0 and is now fixed. [#74491][#74491]
- Previously, `MIN`/`MAX` could be incorrectly calculated when used as window functions in some cases after [spilling to disk](../{{site.versions["stable"]}}/vectorized-execution.html#disk-spilling-operations). The bug was introduced in v21.2.0 and is now fixed. [#74491][#74491]
- Previously, [`IMPORT TABLE ... PGDUMP`](../{{site.versions["stable"]}}/import.html#import-a-table-from-a-postgres-database-dump) with a `COPY FROM` statement in the dump file that has less target columns than the `CREATE TABLE` schema definition would result in a nil pointer exception. This is now fixed. [#74601][#74601]
