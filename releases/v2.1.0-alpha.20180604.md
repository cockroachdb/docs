...
- New clusters and existing clusters [upgraded](../v2.1/upgrade-cluster-version.html) to this version of CockroachDB will include two new empty databases, `defaultdb` and `postgres`. The `defaultdb` database is automatically used for clients that connect without a current database set (e.g., without a database component in the connection URL). The `postgres` database is provided for compatibility with PostgreSQL client frameworks that require it to exist when the database server has ben freshly installed. Both new databases behave like any other regular database and, if deemed unnecessary, can be [manually deleted](../v2.1/drop-database.html). [#24735][#24735] {% comment %}doc{% endcomment %}
...
- Added the `public` pseudo-role, to which all users are assigned, and which gives all users `SELECT` access to virtual schemas. [#25099][#25099]
...
- The new `--enterprise-encryption` flag of the [`cockroach start`](../v2.1/start-a-node.html) command provides transparent encryption for node data on the local disk. This is an experimental feature and is subject to change. [#26061][#26061]
...
- Reduced the CPU overhead of idle ranges. [#24956][#24956]
