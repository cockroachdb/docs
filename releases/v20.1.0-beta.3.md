...
- **Statement tracing in the Admin UI**: Statement diagnostic information is now available in the Admin UI. When viewing a statement fingerprint in the Admin UI, you can also collect the EXPLAIN output and table statistics that were used to plan the query, and a trace on the query execution.
...
- Client and node certificates are now allowed to specify the principal in either the SubjectCommonName field or the SubjectAlternateNames field. Previously, the principal could only be specified in the SubjectCommonName field. [#45819][#45819]
...
- Now allow no-op `CREATE SCHEMA IF NOT EXISTS` statements. [#42703][#42703]
...
- The creation of a database and table between incremental full cluster backups is now allowed. [#46066][#46066]
