...
feature was available only in previous 2.1 alpha releases. Before upgrading to this release, tables with the `TIMETZ` type must be dropped entirely; it is not possible to convert the data or drop a single `TIMETZ` column. [#28095][#28095]
- Support for the `BIT` data type has been removed due to incorrect implementation and incompatibility with some client apps. A PostgreSQL-compatible replacement will likely be added at a later time. [#28814][#28814]

### General Changes

- CockroachDB now supports a separate CA (`ca-ui.crt`) and certificate (`ui.crt`) for the Web UI. [#27916][#27916]
- The ability to set lease placement preferences in zone configs is now fully supported. Existing lease placement preferences will continue to function as in v2.0. [#28261][#28261]
- The new `/_admin/v1/enqueue_range` admin server endpoint runs a specified range through a specified internal queue on one or all nodes. The `skip_should_queue` parameter can also be specified to tell the system to blindly run without first checking whether it needs to be run. This endpoint is intended primarily for debugging purposes. [#26554][#26554]
- If enabled, anonymous [diagnostics reporting](../v2.1/diagnostics-reporting.html) now includes hardware and OS information as well as basic stats about the size of `IMPORT` jobs. [#28676][#28676] [#28726][#28726]

### Enterprise Edition Changes

- This release includes several changes to the [Change Data Capture](../v2.1/change-data-capture.html) feature:
    - `CHANGEFEED`s now support interleaved tables [#27991][#27991]
    - `CREATE CHANGEFEED` now requires an enterprise license when used with Kafka. [#27962][#27962]
    - `CHANGEFEED`s now error when column families are added (instead of returning incorrect results) and when targeting `system` tables (instead of operating with undefined behavior). [#27962][#27962]
    - `CREATE CHANGEFEED` is now restricted to superusers. [#27962][#27962]
    - `CHANGEFEED` job descriptions now substitute values for SQL placeholders. [#28220][#28220]
    - `CHANGEFEED`s can now only target lists of physical tables. [#27996][#27996]
    - `CHANGEFEED`s now error when a watched table is truncated, dropped, or renamed. [#28204][#28204]
    ...
    - `CHANGEFEED`s now export metrics for production monitoring. [#28162][#28162]
    - The `CHANGEFEED` "timestamp" option has been split into "updated" and "resolved". [#28733][#28733]
    - `CHANGEFEED`s are now executed using our distributed SQL framework. [#28555][#28555]
- This release includes the following changes to the [Encryption at Rest](../v2.1/encryption.html) feature:
    - The status of encryption is now written to [debug logs](../v2.1/debug-and-error-logs.html). [#27880][#27880]
    - Encryption keys are now rotated while encryption is running. [#28148][#28148]
    - The new `cockroach debug encryption-status` command displays encryption key information. [#28582][#28582]
