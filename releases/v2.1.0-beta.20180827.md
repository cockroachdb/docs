...
- This release includes several changes to the [Change Data Capture](../v2.1/change-data-capture.html) feature:
    ...
    - `CHANGEFEED`s now error when column families are added (instead of returning incorrect results) and when targeting `system` tables (instead of operating with undefined behavior). [#27962][#27962] {% comment %}doc{% endcomment %}
    ...
    - `CHANGEFEED`s now error when a watched table is truncated, dropped, or renamed. [#28204][#28204] {% comment %}doc{% endcomment %}
...
- This release includes the following changes to the [Encryption At Rest](../v2.1/encryption.html) feature:
    ...
    - Encryption keys are now rotated while encryption is running. [#28148][#28148] {% comment %}doc{% endcomment %}
...
- Statement statistics are now grouped separately for queries using the [cost-based optimizer](../v2.1/sql-optimizer.html) and heuristic planner. [#27806][#27806]
- CockroachDB now supports empty tuples with the syntax `()`, 1-valued tuples with the syntax `(x,)` in addition to `row(x)`, and the ability to use `IN` with an empty tuple as right operand. [#28143][#28143] {% comment %}doc{% endcomment %}
...
- Normalized the case of tables names imported via [`IMPORT ... MYSQLDUMP`](../v2.1/import.html). [#28397][#28397] {% comment %}doc{% endcomment %}
...
- An empty column list is no longer accepted in queries such as `SELECT * FROM [53() as t]`. [#28159][#28159]

### Command-line changes

- The `cockroach` [commands](../v2.1/cockroach-commands.html) that print out SQL results now issue a warning if more than 10000 result rows are buffered in the `table` formatter. [#28490][#28490]
...
- Fix a bug where the [**Statements** page](../v2.1/admin-ui-statements-page.html) in the Web UI blanked out after reloading itself. [#28108][#28108]
...
- Prevented a node from freezing after `DROP DATABASE` when the command abords, and fixed the rare use of an older descriptor after [`DROP INDEX`](../v2.1/drop-index.html). [#28381][#28381]
...
- Prevent lorge buffer allocation for DML statements with `RETURNING` clauses. [#27944][#27944]
