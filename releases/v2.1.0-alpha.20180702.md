...
- The output columns for the statement [`SHOW CONSTRAINTS`](../v2.1/show-constraints.html), which was previously experimental, were changed. The new interface will now be considered stable. [#26478][#26478] {% comment %}doc{% endcomment %}
...
- [`cockroach quit`](../v2.1/stop-a-node.html) now emits warning message on its standard error stream, not standard output. [#26158][#26158] {% comment %}doc{% endcomment %}
...
- [`UPSERT`](../v2.1/upsert.html) is now properly able to write `NULL` values to every columns in tables containing more than one column family. [#26169][#26169]
...
- Using [`SHOW TRACE FOR SESSION`](../v2.1/show-trace.html) multiple times without intervening `SET tracing` statement now properly outputs the trace without introducing extraneous duplicate rows. [#26746][#26746]
