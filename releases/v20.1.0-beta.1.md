...
- **Full cluster restore**: You can now use CockroachDB's [Enterprise `RESTORE`](../v20.1/restore.html) feature to restore a full cluster `BACKUP` to a new cluster, including all configuration and system information such as [user privileges](../v20.1/authorization.html#privileges), [zone configurations](../v20.1/configure-replication-zones.html), and [cluster settings](../v20.1/cluster-settings.html). Restoring a full cluster backup to an existing cluster is not supported.
...
- [`SHOW RANGES`](../v20.1/show-ranges.html) now shows locality information consistent with the range descriptor when node ID and store ID don't match. [#43807][#43807] {% comment %}doc{% endcomment %}
