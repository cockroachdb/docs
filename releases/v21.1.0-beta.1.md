...
- Added `WITH avro_schema_prefix` option to Avro [changefeeds](create-changefeed.html) to prevent collisions in a shared schema registry [#59710][#59710]
...
- [`ALTER DATABASE ... DROP REGION`](../v21.1/alter-database.html) is now implemented. [#59989][#59989]
...
- CockroachDB now supports the `DETACHED` option when running [`IMPORT`](../v21.1/import.html). The detached import will return `jobID` and the user can later use [`SHOW JOB`](../v21.1/show-job.html) to check the result of the detached import. This allows users to run `IMPORT` under explicit transactions with `DETACHED` option specified. [#60442][#60442]
...
- Added new virtual tables `crdb_internal.cluster_contention_events` and `crdb_internal.node_contention_events`, which expose cluster-wide and gateway-only, respectively, contention information. In order to access them, the user needs to be either an admin or have `VIEWACTIVITY` grant. [#60713][#60713]
...
- [`ALTER TYPE ... DROP VALUE`](../v21.1/alter-type.html) is gated behind a feature flag. The [session setting](../v21.1/set-vars.html) is called `enable_drop_enum_value` and the corresponding [cluster setting](../v21.1/cluster-settings.html) is called `sql.defaults.drop_enum_value.enabled`. [#61723][#61723]
...
- Added back support for [`cockroach dump --dump-mode=schema`](../v21.1/cockroach-dump). This command calls [`SHOW CREATE ALL TABLES`](../v21.1/show-create.html) and returns the output. This gives users a migration path to use in v21.1 while switching over to `SHOW CREATE ALL TABLES` for v21.2. When executing this, the user is warned that the command is being removed in v21.2 and they should use `SHOW CREATE ALL TABLES` instead. [#61871][#61871]
