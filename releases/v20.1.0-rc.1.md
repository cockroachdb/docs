...
- Schema changes started in v19.2 before [upgrading to v20.1](../v20.1/upgrade-cockroach-version.html) are now now automatically migrated in the background after the upgrade is finalized so that they can run to completion. [#47073][#47073] {% comment %}doc{% endcomment %}
...
- [`BACKUP`](../v20.1/backup.html) and [`RESTORE`](../v20.1/restore.html) now collect some anonymous telemetry on throughput and feature usage. [#46755][#46755]
...
- [`BACKUP`](../v20.1/backup.html) and [`RESTORE`](../v20.1/restore.html) to HTTP storage locations now requires explicitly specifying incremental storage locations. This means HTTP storage is not supported when using the new automatically appended incremental syntax. [#46967][#46967] {% comment %}doc{% endcomment %}
- The new appended incremental backup syntax does not allow converting a full cluster backup to a specific table or database backup. [#46966][#46966] {% comment %}doc{% endcomment %}
...
- The [`IMPORT`](../v20.1/import.html) statement now records additional anonymous telemetry about its performance and reliability. [#46763][#46763]
...
- The [vectorized execution engine](../v20.1/vectorized-execution.html) now only run queries with streaming operators. To enable vectorized execution for buffering operators, use `SET vectorize=on`. [#46925][#46925] {% comment %}doc{% endcomment %}
...
- The [`cockroach debug zip`](../v20.1/cockroach-debug-zip.html) command now avoids creating invalid zip files if some of its requests encounter an error. [#46634][#46634]
- The [`cockroach demo`](../v20.1/cockroach-demo.html) command now displays a TCP-based connection URL and unix diagram socket for each node of a demo cluster. [#46935][#46935] {% comment %}doc{% endcomment %}
...
- Fixed a bug where `SET TIME ZONE` with a string prefixed with `UTC` or `GMT` and time zones that have colons had it's offset inverted the wrong way. [#46510][#46510]
...
- Fixed a bug when queries with projections only of `INT2` and/or `INT4` columns were executed via the vectorized engine. [#46977][#46977]
...
- Building from source now requires Go 1.13.19. [#46619][#46619]
- It is now possible to build CockroachDB with the Clang++ v10 compiler. [#46860][#46860]
...
- Various updates related to RBAC moving under the BSL license. [#7003](https://github.com/cockroachdb/docs/pull/7003)
