...
- The file names for generated goroutine, CPU, and memory profiles were sometimes incorrect, which resulted in repeated warnings like `strconv.ParseUint: parsing "txt": invalid syntax` in log files. This has been corrected. [#55366][#55366]
...
- An [`INSERT`](../v202.insert.html) into a table with a [foreign key](../v20.2/foreign-key.html) reference to a table with a [partial index](../v20.2/partial-index.html) no longer causes an error. [#55703][#55703]
...
- Previously, cluster [backups](../v20.2/backups.html) taken from before v20.2 could not be restored in v20.2 clusters, and would result in an error message about failing to restore a system table. This is now fixed. [#55719][#55719]
