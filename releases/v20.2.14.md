...
- The DB Console will now redirect the user to the originaly requested page after user login. [#67859][#67859]
...
- Catching up [Raft](../v20.2/architecture/replication-layer#raft) followers on the Raft log is now more efficient in the presence of many large Raft log entries. This helps avoid situations where Raft leaders struggle to retain leadership while catching up their followers. [#67126][#67126]
- Fixed a rare bug in v20.2 that would happen when a lease change occured during a range merge. This bug allowed [`AS OF SYSTEM TIME`](..//v20.2/as-of-system-time.html) queries to serve inconsistent results. [#65823][#65823]
...
- [`INSERT`](../20.2/insert.html) and [`UPDATE`](../20.2/update.html) statements which operate on larger rows are now split into batches using the `sql.mutations.mutation_batch_byte_size` setting. [#67963][#67963]
- Fixed a rare bug that could result in a crash while creating a [`debug.zip`](../v20.2/cockroach-debug-zip.html). The bug was only possible to hit if a `debug.zip` was captured during a period of rapid lease movement. [#67727][#67727]
