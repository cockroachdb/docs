...
- For compatibility with PostgreSQL, the `transaction isolation level`, `transaction priority`, and `transaction status` [session variables](../v1.1/set-vars) have been renamed `transaction_isolation`, `transaction_priority`, and `transaction_status`. [#20301](https://github.com/cockroachdb/cockroach/pull/20301)
...
- Fix a crash caused by null [collated strings](../v1.1/collate.html). [#20648](https://github.com/cockroachdb/cockroach/pull/20648)
...
- [`BACKUP`](../v1.1/backup.html)/[`RESTORE`](../v1.1/restore.html) jobs now support `201` and `204` return codes from HTTP storage.

### Doc Updates

- Added a tutorial on [orchestrating CockroachDB v1.1 with Mesosphere DC/OS](../v1.1/orchestrate-cockroachdb-with-mesosphere-insecure.html). [#2321](https://github.com/cockroachdb/docs/pull/2321)
