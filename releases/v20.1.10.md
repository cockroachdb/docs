...
- Previously, dumps of tables with a [`BIT`](../v20.1/bit) type column would result in an error. This column type is now supported. [#56452][#56452]
...
- CockroachDB previously would crash when executing a query with an [`AS OF SYSTEM TIME`](../v20.1/as-of-system-time) clause that used a placeholder (note that it wasn't a prepared statement, it was an attempt to use an unspecified placeholder value on a non-prepared statement). This is now fixed. [#56781][#56781]
...
- The [`CREATE TEMP TABLE AS`](../v20.1/temporary-tables) statement previously created a non-temporary table. Now it makes a temporary one. [#57550][#57550]
...
- Fixed a bug which can occur when canceling [schema changes](../v20.1/online-schema-changes) when there are multiple queued schema changes that could result in future schema changes being stuck. [#55058][#55058]
- Fixed a bug which can lead to canceled [schema change](../v20.1/online-schema-changes) jobs ending in the failed rather than canceled state. [#55058][#55058]
...
- The reserved, non-documented [cluster settings](../v20.1/cluster-settings) `server.heap_profile.xxx` have been renamed to `server.mem_profile.xxx`. They now control collection of multiple sorts of memory profiles besides just Go heap allocations. [#55260][#55260]
