...
- As part of the transition to v20.2, the migration to upgrade schema change jobs started prior to v20.1 is no longer run. [#52968][#52968]
...
- Implemented the geometry [builtin](../v20.2/functions-and-operators.html#built-in-functions) `ST_SymmetricDifference`. [#53688][#53688]
...
- Implemented the geometry [builtin](../v20.2/functions-and-operators.html#built-in-functions) `ST_Affine`. [#53856][#53856]
...
- The CLI no longer prints a blanket `Time` for queries. Instead, if `show_times` is turned on and the server version is v20.2 or later, the CLI prints two separate times: the server execution time and the network latency. [#52233][#52233]
