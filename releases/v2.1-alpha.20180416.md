...
- Added a [cluster setting](../v2.1/cluster-settings.html) for HLC to panic on clock jumps. [#23717][#23717]
- Statistics on the types of errors encountered are now included in [diagnostics reporting](../v2.1/diagnostics-reporting.html). [#22912][#22912]

### Enterprise Edition Changes

- It is now possible to [`RESTORE`](../v2.1/restore.html) views when using the `into_db` option. [#24555][#24555]
...
- The new `jobs.registry.leniency` [cluster setting](../v2.1/cluster-settings.html) can be used to allow long-running import jobs to survive temporary node saturation. [#23913][#23913]
- Added configurable limits on the number of `BACKUP`/`RESTORE` requests each store will process. [#23517][#23517]

### SQL Language Changes

- Added configurable limits on the number of [`IMPORT`](../v2.1/import.html) requests each store will process. [#23517][#23517]
- The new `ALTER TABLE ... INJECT STATS` command injects table statistics from a JSON object (which can be obtained via `SHOW HISTOGRAM USING JSON`). [#24488][#24488]
- The new `SHOW STATISTICS USING JSON` variant of `SHOW STATISTICS` outputs table statistics as a JSON object; it can be used to extract statistics from clusters to reproduce issues. [#24488][#24488]
- `VIRTUAL` and `WORK` are no longer reserved keywords and can again be used as unrestricted names. [#24491][#24491]
- Added the `CANCEL SESSION` statement as well as an `IF EXISTS` variant to `CANCEL QUERY`. [#23861][#23861]
- Added a new `session_id` column to the result of `SHOW SESSIONS`. [#23861][#23861]
...
- Added the `statement_timeout` session variable. [#23399][#23399]
...
- Added the `skip` option to the `IMPORT` command. [#23466][#23466]
- The service latency tracked for SQL statements now includes the wait time of the execute message in the
  input queue. [#22880][#22880]

### Command-Line Changes

- The `cockroach gen autocomplete` command can now generate zsh-compatible completion files by passing `zsh` as an
  argument. [#24400][#24400]
- Removed the `cockroach load csv` subcommand. [#24319][#24319]
- When `cockroach gen haproxy` is run, if an `haproxy.cfg` file already exists in the current directory, it now gets fully overwritten instead of potentially resulting in an unusable config. [#24332][#24332]
- The new `cockroach demo` command opens a SQL shell connected to a fully in-memory store, and an empty database named `demo`. It's useful for users or developers who wish to test out Cockroach's SQL dialect. [#24259][#24259]
...
- Per-replica constraints in replication zones no longer have to add up to the total number of replicas in a range. If all replicas aren't specified, then the remaining replicas will be allowed on any store. [#23057][#23057]
...
- The **Node Map** now shows how long a node has been dead. [#23255][#23255]
...
- The `cockroach dump` command is now able to dump sequences with non-default parameters. [#23051][#23051]
- `SHOW TABLES` is once again able to inspect virtual schemas. [#22994][#22994]
