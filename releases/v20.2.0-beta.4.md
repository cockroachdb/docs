...
- The display of statement timings in the SQL shell ([`cockroach sql`](../v20.2/cockroach-sql.html) and [`cockroach demo`](../v20.2/cockroach-demo.html)) has been further simplified. [#54765][#54765] {% comment %}doc{% endcomment %}

### Bug fixes

- CockroachDB would crash when `json_object_agg` and `jsonb_object_agg` aggregate functions were used as window functions. Those functions were added in a v20.2 testing release, so only testing 20.2 releases were affected. This is now fixed. [#54657][#54657]
- Fixed a bug that could cause the storage-level corruption under rare circumstances while using the Pebble storage engine. [#54800][#54800]

### Performance improvements

- Optimize compactions in Pebble to improve read/write performance in some write-heavy workloads. [#54800][#54800]
