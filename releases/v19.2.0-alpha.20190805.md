...
- Replicas are now added using a raft learner and go through the normal raft snapshot process to catch them up, eliminating technical debt. No user-facing changes are expected. [#38149][#38149]
...
- Added support for `NOT VALID` option for [Foreign Key constraints](../v19.2/foreign-key.html). Updated logic tests to more thoroughly test the added support of `NOT VALID`. [#38663][#38663]
...
- [`cockroach dump`](../v19.2/sql-dump.html) will now more clearly refer to issue [37768](https://github.com/cockroachdb/cockroach/issues/37768) when it encounters a table with no visible columns, which (currently) cannot be dumped successfully. [#38725][#38725]
- CockroachDB now requires square brackets to specify IPv6 addresses in `--join`, `--host`, `--listen-addr` and similar [flags](../v19.2/start-a-node.html#flags), for example `--listen-addr=[::1]`. The syntax without square brackets had been deprecated since CockroachDB 2.1. [#38924][#38924]
...
- [`Sqlfmt`](../v19.2/use-the-query-formatter.html#flags) no longer strips `ORDER BY` from aggregates. [#38623][#38623]
...
- Perfromance improvement to the [`BACKUP`](../v19.2/backup.html) process. [#37887][#37887]
...
- Updated the client-side transaction retry recommendations in Java, Python, and Go code samples. [#5084](https://github.com/cockroachdb/docs/pull/5084), [#5085](https://github.com/cockroachdb/docs/pull/5085), [#5088](https://github.com/cockroachdb/docs/pull/5088)
...
- [Build an App with CockroachDB](../v19.2/build-an-app-with-cockroachdb.html) examples now include [C++](../v19.2/build-a-c++-app-with-cockroachdb.html) and [Clojure](../v19.2/build-a-clojure-app-with-cockroachdb.html) application examples for secure clusters. [#5028](https://github.com/cockroachdb/docs/pull/5028)
