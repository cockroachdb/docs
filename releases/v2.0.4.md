...
- [`CHECK`](../v2.0/check.html) constraints are now checked when updating a conflicting row in [`INSERT ... ON CONFLICT DO UPDATE`]() statements. [#26699][#26699] {% comment %}doc{% endcomment %}
...
- Failed [`IMPORT`](../v2.0/import.htmal)s now begin to clean up partially imported data immediately and in a faster manner. [#26986][#26986]
...
- Fixed a panic in [`IMPORT`](../v2.0/import.htmal) when creating a table using a sequence operation (e.g., `nextval()`) in a column's [DEFAULT](../v2.0/default-value.html) expression. [#27294][#27294]
