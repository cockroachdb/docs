...
- Cross-database references will be deprecated in a future release (see [tracking issue](https://github.com/cockroachdb/cockroach/issues/55791)). In v20.2, cross-database references are disabled for [foreign keys](../v20.2/foreign-key.html), [views](../v20.2/views.html), and [sequence ownership](create-sequence.html) with the `sql.cross_db_fks.enabled`, `sql.cross_db_views.enabled`, and `sql.cross_db_sequence_owners.enabled` [cluster settings](../v20.2/cluster-settings.html) set to `false` by default. Before [upgrading to v20.2](../v20.2/upgrade-cockroach-version.html), we recommend removing all cross-database references.
