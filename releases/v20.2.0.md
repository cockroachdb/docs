...
- You can create a 30-day free CockroachCloud cluster using the code `CRDB30`. The [Quickstart](../cockroachcloud/quickstart.html) guide shows you how to create and connect to your free cluster and run your first query.
- You can now [add or remove nodes](../cockroachcloud/cluster-management#add-or-remove-nodes-from-a-cluster) from your cluster through the Console.
- You can now [set up VPC peering](../cockroachcloud/network-authorization.html) for clusters running on GCP.
- You can now [restore data from a backup](../cockroachcloud/backups-page.html) for clusters running on AWS.
...
Before [upgrading to CockroachDB v20.2](../stable/upgrade-cockroach-version.html), be sure to review the following backward-incompatible changes and adjust your deployment as necessary.
...
- The Docker image is now based on RedHat's ubi8/ubi-minimal image (instead of ubi8/ubi). This image is smaller.
