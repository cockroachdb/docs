...
~~~shell
$ docker pull cockroachdb/cockroach-unstable:v20.1.11
~~~


### SQL language changes

- Fix a bug in [`RESTORE`](../v20.1/restore.html) where some unusual range boundaries in interleaved tables caused an error. [#58260][#58260]

### Bug fixes

- In v20.1.8, we attempted to fix `age`'s lack of normalization of `H:M:S` into the years, months and days field. However, this was also broken for values > 1 month, as well as breaking `a::timestamp(tz) - b::timestamp(tz)` operators. This has now been resolved. [#57956][#57956]
