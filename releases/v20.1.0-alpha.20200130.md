...
- **Full cluster backup:** CockroachDB's [Enterprise `BACKUP`](../v20.1/backup.html) feature can now back up an entire cluster's data, including configuration information such as user privileges, zone configurations, and cluster settings. At this time, individual databases and tables can be restored from a full cluster backup. In a future release, it'll be possible to restore an entire cluster as well.       
...
- **Import from Avro format**: The [`IMPORT`](../v20.1/import.html) and [`IMPORT INTO`](../v20.1/import-into.html) statements now support files in the Avro format. This makes it easier to migrate data from systems like Spanner that export data in the Avro format.
