...
- Guidance on [how to contribute to CockroachDB](https://cockroachlabs.atlassian.net/wiki/x/QQFdB) have been moved to the public wiki at wiki.crdb.io. [#41542][#41542]
- Removed the `kv.bulk_io_write.addsstable_max_rate` [cluster setting](../20.1/cluster-settings.html). [#41745][#41745] {% comment %}doc{% endcomment %}
- Improved the consistency checker's log output. [#41893][#41893]
- When the replicas within a range are found to been corrupted, the outliers will be terminated. Previously, the leaseholder replica would terminate, regardless of which replicas disagreed with each other. This is expected to curb the spreading of corrupted data better than the previous approach. [#41902][#41902]

### Enterprise edition changes

- [`IMPORT`](../20.1/import.html) statements now support specifying CSV filenames using wildcard characters. This behavior can be disabled with the `WITH disabled_glob_matching` option. [#40714][#40714] {% comment %}doc{% endcomment %}
- When using a `nodelocal` file URL for [`IMPORT`](../20.1/import.html)/[`BACKUP`](../v20.1/backup.html)/[`RESTORE`](../20.1/restore.html)/[`EXPORT`](../v20.1/export.html), you can now specify which node's local file system to use by including the node's ID in the URL: `nodelocal://<nodeID>/path/file.csv`. [#41990][#41990] {% comment %}doc{% endcomment %}
...
- Sub-second `extract()` now behaves the same as in Postgres. [#41069][#41069]
...
- Fixed bugs where `<date>:date` would result in the previous day (`<date-1>::date>`) when context local timestamp is set and the timezone is less than UTC+00:00, where `date_trunc()` for `TIMESTAMP` would produce incorrect results if a local timezone was set, and where `date_trunc()` for `DATE` would produce an incorrect negative timezone offset in a local timezone. [#42267][#42267]
