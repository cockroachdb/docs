...
- Added four new metrics, `sql.guardrails.max_row_size_{log|err}.count{.internal}` which are incremented whenever a large row violates the corresponding `sql.guardrails.max_row_size_{log|err}` limit. [#69946][#69946]
...
- Fixed a bug where running [`IMPORT PGDUMP`](../v21.1/migrate-from-postgres) with a UDT would result in a null pointer exception. This change makes it fail gracefully. [#69249][#69249]
...
- Fixed a bug where [`COPY FROM ... CSV`](../v21.1/copy-from.html) did not correctly handle octal byte escape sequences such as `\011` when using a BYTEA column`. [#69278][#69278]
...
- Fixed a bug introduced in v21.1 where CockroachDB could return an internal error when performing streaming aggregation in some edge cases, and this is now fixed. [#69181][#69181]
...
This release includes 38 merged PRs by 19 authors.
