...
- The following [cluster settings](cluster-settings.html) were added: 
  - `sql.defaults.statement_timeout` controls the default value for the statement_timeout session setting.
  - `sql.defaults.idle_in_transaction_session_timeout` controls the default value for the `idle_in_transaction_session_timeout` timeout setting.
  - `sql.defaults.idle_in_session_timeout` already existed, but is now a public cluter setting. [#62185][#62185]

### Bug fixes

- Previously, [changefeeds](change-data-capture.html) were checking the `SELECT` privilege on every descriptor associated with targeted tables, including the database and custom types. They now only check `SELECT` on the targeted tables. [#61661][#61661]
...
- Fixed an npe observed with a `SpanFromContext` call in the stack trace. [#61703][#61703]
- Exporting to userfile locations now works correctly. [#61789][#61789]
...
- Fixed a bug where random numbers generated as default expressions during [`IMPORT`](import.html) would collide a few hundred rows apart from each other. [#61630][#61630]
- Previously, using [`EXPLAIN (OPT, ENV)`](explain.html) on a query that referenced a table in a user-defined schema would fail. This is now fixed. [#61890][#61890]
- Fixed a bug that caused `column does not exist` errors in specific cases of [`UPDATE..FROM`](update.html) statements. The error occurred when updating a `DECIMAL` column to a column in the `FROM` clause, and the column had a `CHECK` constraint or was referenced by a partial index predicate. [#61953][#61953]
- Fixed `command is too large` errors in some cases when using [`EXPLAIN ANALYZE (DEBUG)`](explain-analyze.html) or statement diagnostics on complex queries. [#61910][#61910]
- Previously, the `target` column of `crdb_internal.zones` would show names without properly accounting for user-defined schemas. Now it does. [#62019][#62019]
...
- Previously, the `pg_type_is_visible` [builtin](functions-and-operators.html#built-in-functions) function did not correctly handle user-defined types. This is fixed now. [#62231][#62231]
- Fix a bug where [full cluster restore](restore#full-cluster) would sometimes (very rarely) fail after retrying. [#61217][#61217]
