...
- Added timeseries metrics for debugging [`CHANGEFEED`](../v2.1/create-changefeed.html) performance issues. [#32872][#32872]

### Bug fixes

- Resolved a cluster degradation scenario that could occur during [`IMPORT`](../v2.1/import.html)/[`RESTORE`](../v2.1/restore.html) operations, manifested through a high number of pending Raft snapshots. [#33015][#33015]
...
- Ncurses is now linked statically so that the `cockroach` binary no longer requires a particular version of the `ncurses` shared library to be available on deployment machines. [#32960][#32960]
