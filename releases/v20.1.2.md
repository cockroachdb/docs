...
- Fixed a bug where [`\demo node restart`](../v20.1/cockroacb-demo.html) would not work due to an invalid certificate directory. [#49390][#49390]
...
- When performing incremental backups with revision history on a database (or full cluster), and a table in the database was dropped and then other tables were later create, the backup no longer returns an error. [#49925][#49925]
...
- Revised the node [decommissioning guidance](../v20.1/https://www.cockroachlabs.com/docs/stable/remove-nodes.html). [#7304][#7304]
