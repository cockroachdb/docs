...
- Fixes a bug where queries could get stuck for seconds or minutes, usually following node restarts. [#31350][#31350]
...
- Fixed a bug causing transactions to appear partially committed. CockroachDB was sometimes claiming to have failed to commit a transaction when some (or all) of its writes were actually persisted. [#32223][#32223]
...
- Fixed a bug where calling `CREATE STATISTICS` on a large table could cause the server to crash due to running out of memory. [#32636][#32636]
