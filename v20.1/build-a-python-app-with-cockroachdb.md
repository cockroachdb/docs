...
Find the `parse_cmdline()` function. Update the `default` parameter of the `--dsn` argument as follows:
...
- Comment out the `default` parameter for `cockroach demo`, and uncomment the `default` parameter for CockroachCloud.
