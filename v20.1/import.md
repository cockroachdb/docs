...
You can specify file patterns to match instead of explicitly listing every file. Use the `*` wildcard character to include matching files directly under the specified path. A wildcard can be used to include:
...
Then, specify the node to access by including the `nodeID` in the `IMPORT` statement:
