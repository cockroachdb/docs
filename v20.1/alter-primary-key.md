...
This is currently an experimental feature. The interface and output are subject to change.
...
When you change a primary key with `ALTER PRIMARY KEY`, the old primary key index becomes a secondary index. This helps optimize the performance of queries that still filter on the old primary key column, but it makes `ALTER PRIMARY KEY` an expensive operation, as it performs rewrites of multiple indexes.
{{site.data.alerts.end}}
