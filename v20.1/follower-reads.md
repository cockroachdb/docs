...
Follower reads return consistent historical reads at a minimum duration in the past. This duration is defined by the [follower read timestamp](#experimental-follower-read-timestamp).

As long as your [`SELECT` operations](select-clause.html) can tolerate reading data that may be slightly less than current, follower reads can reduce read latencies and increase throughput.
...
### Make queries that use follower reads
...
### Make transactions that use follower reads
