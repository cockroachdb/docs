...
Use the **Sort By** menu to arrange the latency matrix by <a href="#" data-toggle="tooltip" data-original-title="{{site.data.glossary.locality}}">locality</a> (e.g., cloud, region, availability zone, datacenter).
...
Each cell in the matrix displays the <a href="#" data-toggle="tooltip" title="{{site.data.glossary.round-trip-latency}}">round-trip latency</a> in milliseconds between two nodes in your cluster. Round-trip latency includes the return time of a packet. Latencies are color-coded by their standard deviation from the mean latency on the network: green for lower values, and blue for higher.
...
{{site.data.glossary.network-partition}}
