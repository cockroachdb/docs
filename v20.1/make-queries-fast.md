...
There is a `rider_id` field that we can use to match each ride to a user. There is also a `start_time` field which we can use to filter the rides by date.
...
{% include copy-clipboard.html %}
~~~ sql
CREATE INDEX ON rides (start_time) storing (start_time);
~~~
...
As you can see, this query is no longer scanning the entire (larger) `rides` table.  Instead, it is now doing a much smaller range scan using against just the values in `rides` that match the index we just created on the `start_time` column.
...
The query is also not faster when we force CockroachDB to use a merge join.  The results are consistently about 5-7ms slower than before.
