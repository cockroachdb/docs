...
- A table table can only be accessed from the session in which it was created.
- Temp tables persist across transactions in the same session.
- Persistent tables cannot reference temp tables, and temp tables cannot reference persistent tables.
- Temp tables cannot be converted to persistent tables.

## Temporary schemas

Temp tables are not part of the `public` schema. Instead, when you create the first temp table for a session, CockroachDB generates a single temporary schema (`pg_temp_<id>`) for all of the temp tables in the current session for a database.
...
To find the schema name, use [`SHOW SCHEMAS`](show-schemas.html):
...
To show the tables in the temporary schema:

{% include copy-clipboard.html %}
~~~ sql
> SHOW TABLES FROM pg_temp_1584540651942455000_1;
~~~
...
Although temp tables are not included in the `public` schema, they are included in the [`information_schema`](information-schema.html) schema. For example, the [`information_schema.tables`](information-schema.html#tables) table includes information about all tables in all schemas in all databases, including temp tables:
