...
This page contains a tutorial for working with spatial data using CockroachDB. In this tutorial, we will plan a vacation from New York City to the [Adirondack Mountains](https://visitadirondacks.com/about) in northern New York State to do some birdwatching while visiting local independent bookstores.

In the process, we will explore the following [spatial features](spatial-features.html) supported by CockroachDB:
...
This tutorial can be accomplished in any running CockroachDB cluster running [v20.2](../releases/v20.2.0.html) or later.
...
A [SQL client prompt](cockroach-sql.html) will appear that looks like the one shown below. We will use this prompt for the rest of the tutorial.
...
First, create a `tutorial` database, and use it.
...
Next, create a `birds` database, and use it.
...
3. Give the resulting table a name, `loon_sightings`, and query against it. In this case the query is rather simple: since the geometries have been collected into one in step 2 above, we output the geometry as [GeoJSON](geojson.html) so we can paste the result into <https://geojson.io> and see a map of the sightings.
...
We can verify our results by checking how many bird species have been sighted by the bird survey overall; we should expect that this number will be much larger than 124, and indeed it is:
...
So far, we have learned (approximately) [where the birds are](#step-4-scout-loon-locations), [where the overlaps are between birds and bookstores](#step-5-find-bookstores-to-visit), and [how to travel between bookstores while finding birds](#step-6-plan-your-route).
...
Unfortunately this query is a bit slower than we would like: about 1.5 seconds on this single-node [`cockroach demo`](cockroach-demo.html) cluster. There are several reasons for this:
...
Let's start by adding an index to the `roads.geom` column. Note that creating the index on `roads.geom` takes about a minute on this single-node [`cockroach demo`](cockroach-demo.html) cluster, since the table is relatively large.
...
It looks like the answer is yes; we have sped up our query by about 30%:
...
{{site.data.alerts.callout_info}}
Because we are using `ST_Contains`, the query below only sums the road mileages of roads whose geometries lie entirely within the loon habitat.
{{site.data.alerts.end}}
...
The answer is, there are about 1,700 miles of roads within the Common Loon's habitat. Further, you can change the `WHERE` clause of the above query slightly to say `AND (NOT (ST_Contains(...)))` to find out how many miles of roads in NY state are _not_ within loon habitat (it's about 14,600).
...
When we look at `EXPLAIN` for the modified query, we see why: the filter on `roads.state='NY'` means we are scanning far fewer columns of the `roads` table (~8,900), and the index on `bookstores.geom` means we are using that index now as well:
...
- Learn more about CockroachDB's [spatial Features](spatial-features.html).

- Learn how CockroachDB's [spatial indexes](spatial-indexes.html) work.

- Learn how to [migrate spatial data from Shapefiles](migrate-from-shapefiles.html) into CockroachDB.
...
The `birds` database contains several tables that we will have to [join](joins.html) together in most of our queries below. The multi-table design reflects [the schema of the actual data set](https://www.sciencebase.gov/catalog/file/get/5ea04e9a82cefae35a129d65?f=__disk__b4%2F2f%2Fcf%2Fb42fcfe28a799db6e8c97200829ea1ebaccbf8ea&transform=1&allowOpen=true), which is split across files that map to these tables.
