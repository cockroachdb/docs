...
This page contains a tutorial for working with spatial data using CockroachDB. In this tutorial, we will explore the following [spatial features](spatial-features.html) supported by CockroachDB:
...
+ How to use various [built-in functions](functions-and-operators.html#spatial-functions) for operating on spatial data.
+ How to create [indexes](spatial-indexes.html) on spatial data.
+ Performing [joins](joins.html) on spatial data, and how to use [`EXPLAIN`](explain.html) to make sure we are using indexes effectively.
...
You live in New York City and are an avid bird watcher and reader of books. You are going on a vacation up to the [Adirondack Mountains](https://visitadirondacks.com/about) in northern New York State. Although you are interested in many bird species, you are most interested in seeing (and [hearing](https://macaulaylibrary.org/asset/107964)) the [Common Loon](https://ebird.org/species/comloo), a bird that can be found near the lakes and ponds of the north woods.

As a connoisseur of book stores, you also want to make sure to visit as many of the local independent bookstores as possible on your trip, as long as they are nearby to Loon habitat.
...
+ NY State bird sighting data from the years 2000-2019, taken from the [North American Breeding Bird Survey (NABBS)](https://www.sciencebase.gov/catalog/item/52b1dfa8e4b0d9b325230cd9). The full survey covers the US and Canada from the years 1966-2019 and is very large, so you have decided to import just the recentish NY data since that is all you need to plan your trip.
...
## Spatial features covered in this tutorial

The queries are presented below in a "narrative order" that corresponds roughly to the order in which you might ask questions about bookstores and loon habitat as you plan for your vacation.

However, you may just want to see what queries are exercising a specific [spatial feature](spatial-features.html) supported by CockroachDB. The table below provides a mapping from features (such as 'spatial indexing' or 'spatial joins') to the queries that use those features.
...
| Loading spatial data         | N/A                                                                                                                                                    | For loading instructions on loading the data set, see [Loading the data set](#loading-the-data-set).                                                                      |
...
| Nearest-neighbor searching   | [#15][q_15]                                                                                                                                            | CockroachDB does not support index-based nearest neighbor queries. For details, see [cockroachdb/cockroach#55227](https://github.com/cockroachdb/cockroach/issues/55227). |

## Data set description

In this section we will provide an overview of the data sets and how they were constructed and put together. Because this data set is a "compound data set" that is built from several other data sets, there is some complexity to the schema we will have to navigate.
...
<img src="{{ 'images/v21.1/geospatial/tutorial/er-bookstores.png' | relative_url }}" alt="tutorial.bookstores and tutorial.bookstore_routes ER diagrams" style="max-width:100%" />
...
Similarly, the paths (specifically [MultiLinestrings](multilinestring.html)) between bookstores in the `bookstore_routes` were also calculated using a script, that made use of a simple greedy algorithm to find paths between the bookstores using the `roads` table (described below). The algorithm always looks for the next road that will bring us closer to our destination than we currently are. It is fine for this small use case, but it is not robust enough for any kind of production use, since it does not perform any backtracking when it gets stuck in a local optimum. The algorithm works as follows, and assumes a starting bookstore _A_ and an ending bookstore _B_:
...
3. Do _R_<sub>A</sub> and _R_<sub>B</sub> intersect?  If so, we're done. We have arrived at our destination. (This is our recursive base case.)
...
5. Do _R_<sub>A'</sub> and _R_<sub>B</sub> intersect?  If so, we're done. We have arrived at our destination.
6. If we have arrived here without finishing, we go back to step 4 and apply that step to the current "closest road to destination" _R_<sub>A'</sub>, and keep operating steps 4-6 recursively until we have found a complete path or we hit some kind of heuristic timeout.
...
Meanwhile, the `roads` table has many columns, but we will be ignoring most of them except `state`, `geom`, `miles`, and `prime_name` (the human-readable name of the road).

<img src="{{ 'images/v21.1/geospatial/tutorial/er-roads.png' | relative_url }}" alt="tutorial.roads ER diagrams" style="max-width:100%" />

For more information about what the other columns in `roads` mean (if you are interested), see the [full data set description](https://www.sciencebase.gov/catalog/file/get/581d052be4b08da350d524ce?f=__disk__60%2F6b%2F4e%2F606b4e564884da8cca57ffeb229cd817006616e0&transform=1&allowOpen=true).
...
The `birds` database contains several tables. This design provides for a lot of flexibility at the cost that we will have to [join](joins.html) the different `birds.*` tables in most of our queries below in order to get useful answers. The multi-table design is a result of [the schema of the actual data set](https://www.sciencebase.gov/catalog/file/get/5ea04e9a82cefae35a129d65?f=__disk__b4%2F2f%2Fcf%2Fb42fcfe28a799db6e8c97200829ea1ebaccbf8ea&transform=1&allowOpen=true), which is split across files that map to these tables.

The tables in the `birds` database are diagrammed below:

- `birds` is a list of ~750 bird species. Most of our queries will use the `name` column.
- `routes` is a list of ~130 prescribed routes that the bird watchers helping with the survey travel along each year. The `geom` associated with each route is a [Point](point.html).
...
<img src="{{ 'images/v21.1/geospatial/tutorial/er-birds.png' | relative_url }}" alt="birds.birds, birds.routes, and birds.observations ER diagrams" style="max-width:100%" />
...
First, create a `tutorial` database, and switch to it.
...
Next, [`IMPORT`](import.html) the parts of [the data set](#data-set-description) that live in the `tutorial` database.
...
Next, create a `birds` database, and switch to it.
...
Next, [`IMPORT`](import.html) the parts of [the data set](#data-set-description) that live in the `birds` database.
...
Finally, switch back to the `tutorial` database.  All of the queries in this tutorial assume you are in the `tutorial` database.
...
As a first step, we'd like to know where exactly the Common Loon has been sighted in New York State. We know that many of the loon sightings will have taken place in the Adirondacks (our overall destination) due to the nature of the loon's preferred habitat of northern lakes and ponds, but it will be useful to know the precise locations for trip planning, and for asking further related questions about other bird habitats as well as nearby bookstores we'd like to visit.

Because of the structure of [the `birds` database](#the-birds-database), we will wrap the results of a [subquery](subqueries.html) against the `birds` database in a [common table expression (CTE)](common-table-expressions.html). This step will be necessary every time we want to get information about bird sightings. Therefore the general pattern for many of our queries will be something like:

1. Get bird information (usually including location data) and store the results in a named CTE.
2. Using the named CTE, perform additional processing against the results of the CTE combined with other data we have (e.g. bookstores, roads). Depending on the complexity of the questions we ask, we may even create multiple CTEs.

In the query below, to answer the question "where are the loons?", we take the following steps:
...
3. Give the resulting table a name, `loon_sightings`, and query against it. In this case the query is rather simple: since the geometries have been collected into one in step 2 above, we output the geometry as [GeoJSON](geojson.html).

{% include copy-clipboard.html %}
~~~ sql
USE tutorial;  -- All queries assume you are in the 'tutorial' database.
...
### (2) What is the area of the Loon's sightings range?

<!-- Time: 75ms -->

Now that we have some sense of how loon sightings are distributed across the state, we may wonder: what is the total area covered by these sightings?  In other words, what is the area of the loon's approximate habitat (per these sightings) in New York State?

To find the answer, we:

1. Once again collect the geometries of all loon sightings together in a CTE as one geometry.
2. Get the area of the [convex hull](st_convexhull.html) of the resulting geometry.
3. Because the `birds.routes` data uses [SRID 4326](srid-4326.html), the resulting area is measured in degrees, which we convert to square miles by casting the data to a `GEOGRAPHY` type and dividing by 1609 (the number of meters in a mile) squared.
...
The result is an area of about 10,000 square miles, which can be visualized by a box with sides that are a little over 100 miles long. This answer can be sanity checked by looking at the map output from [#1][q_01] and noting that most of the observation locations are well within 100 miles of each other.
...
In addition to the [loon observation point locations][q_01] and the [area of those observations][q_02] (which we can take as a proxy for the size of the loon's local habitat), we may want to know exactly how many times a loon has been sighted in the area.

To find the answer, we:

1. Join `birds.birds` and `birds.observations` on the bird ID where the bird name is "Common Loon".
2. Except that this time we also sum all of the sightings; the `GROUP BY` on bird names is necessary due to the use of the `sum` aggregate function.
...
We might like to get a sense of how many of the loon sightings were more recent. For example, if there have been fewer sightings of loons in recent years, one might wonder if the population were declining in NY State.

To determine how many sightings there were in NY state in 2019, we re-use the query from [#3][q_03], with the additional constraint that the observation year is 2019.
...
If the sightings are evenly distributed, we would expect this to yield about 13 sightings. In fact, it's pretty close. This suggests that the loon population may have remained fairly stable over the years 2000-2019.
...
2. Query the result table of the CTE from step 1 to divide the number of sightings by the area of the loon's habitat (the convex hull). As in [#2][q_02], we do some arithmetic to convert from the unit of degrees returned by [SRID 4326](srid-4326.html) to square miles.
...
Now that we have asked (and answered) some exploratory questions that may inform our birdwatching activities while on vacation, it would be good to start thinking about what bookstores we may want to visit as part of our travels.

A natural question that arises is: given that we are looking for loon habitat, what are the bookstores located within that habitat?

To answer this question, we:

1. Build a CTE that returns the [convex hull](st_convexhull.html) of Common Loon habitat.
2. Join the results of the above CTE with a query against [the `bookstores` table](#the-bookstores-and-bookstore_routes-tables) that checks whether a bookstore's location is [contained](st_contains.html) by the loon habitat. Note that we [`ORDER BY`](query-order.html) the store geometries so that stores in the list are clustered by location. This ordering may be useful if we want to travel between nearby stores.
...
1. Build a CTE that returns some information about the bookstore we want to visit.
...
3. Joins the results of the above CTEs and queries the count of birds whose habitats contain the location of the bookstore.
...
We can sanity check our results by checking how many bird species have been sighted by the bird survey overall; we should expect that this number will be much larger than 124, and indeed it is:
...
It's great that [we know how many bird species may be near a given bookstore][q_07]; however, that query didn't tell us which birds we have the best chance of seeing. Therefore we'd like to figure out what birds are the most commonly seen in the area near a bookstore we want to visit: The Book Nook in Saranac Lake, NY.
...
We have already [visualized the convex hull][q_09], but now we would like to calculate its area in square miles.
...
2. Join the results of the above CTE with a query against [the `bookstores` table](#the-bookstores-and-bookstore_routes-tables) that checks whether a bookstore's location is [contained](st_contains.html) by the loon habitat.
2. Get the area of the [convex hull](st_convexhull.html) of the resulting geometry.
3. Collect the geometries that result from the step above into a single geometry, calculate its convex hull, and calculate the area of the hull. As in previous examples, note that because the `birds.routes` data uses [SRID 4326](srid-4326.html), the resulting area is measured in degrees, which we convert to square miles by casting the data to a `GEOGRAPHY` type and dividing by 1609 (the number of meters in a mile) squared.
...
The result is an area of about 3,500 square miles, which can be visualized by a box with sides that are approximately 60 miles long. This answer can be sanity checked by looking at the map output from [#9][q_09] and noting that most of the points on the hull are within about 60 miles of each other.
...
So far, we have looked at queries that ask questions about bird habitats, bookstore locations, or some combination of the two. Because we are planning a trip, we now want to think about how we might travel between some of the bookstores in the loon habitat area. Depending on how far apart the bookstores are, we may want to visit more than one of them in a day, perhaps doing some birdwatching along the way.

To start, let's get used to working with [the `bookstore_routes` table](#the-bookstores-and-bookstore_routes-tables) by seeing how far the distance is in miles between two of the bookstores we'd like to visit.

To answer this question, we:

1. Issue subqueries that find the IDs of two of the bookstores we'd like to travel between, as start and end points.
2. Measure the length of the geometry that corresponds to those start and end IDs. Note that because the `bookstore_routes.geom` column has a SRID of 0 (which it inherited from the `roads` database from which it was created), we convert to miles by casting the data to a `GEOGRAPHY` type, which uses meters, and then dividing by 1609 (the number of meters in a mile).
...
We have [determined how long the drive between these two stores will be][q_11], but we would like to see what it looks like on the map.

To find this out, we can re-use the query from the previous question, using `ST_AsGeoJSON` instead of `ST_Length`, and skipping the distance math to generate output in miles.
...
So far, we have looked at queries that ask questions about bird habitats, bookstore locations, or some combination of the two. Because we are planning a trip, we now want to think about how we might travel between some of the bookstores in the loon habitat area, perhaps doing some birdwatching along the way. In order to accomplish this, we will start working with [the `bookstore_routes` table](#the-bookstores-and-bookstore_routes-tables).

A natural question that arises once we start traveling between stores is: if I decide to visit two of these stores in a day, and do some birdwatching in between, what are some of the bird species I may expect to find? In this case we are looking at what species are near the route between [Mysteries on Main Street in Johnstown, NY](https://www.facebook.com/MysteriesOnMainStreet), and [The Bookstore Plus in Lake Placid, NY](http://www.thebookstoreplus.com).

To answer this question, we:

1. Build a CTE that returns the route between our two stores of interest (a geometry).
2. Join the results of the above CTE with a query against [the `birds` database](#the-birds-database) that checks whether the distance between the route geometry and the location of the bird observation (`birds.routes.geom`) bookstore's location is less than our desired length of 10 miles. Note that because the call to `ST_Distance` is operating on shapes cast to `GEOGRAPHY` data type, the results are in meters, which we then have to convert to miles by dividing the result by 1609 (the number of meters in a mile).
...
Since we are already looking for loons, we would like to know if there are other, more challenging bird watching opportunities available within the loon habitat we are already visiting. Specifically, we would like to know what are the least-often-observed bird species in the loon's habitat?
...
So far, we have learned (approximately) [where the birds are](#scouting-loon-locations), [where the overlaps are between birds and bookstores](#things-about-bookstores-near-loons), and [how to travel between bookstores while finding birds](#trips-between-bookstores-near-loons).

Now we will start asking questions that involve looking at [the `roads` table](#the-roads-table) as well.

Specifically, we would like to know: what are the 10 roads that are closest to a loon observation site?  The answer to this question may help us figure out exactly which roads we should travel down to start our search for loons. The closer a road is to the sighting location, the better: it means there is probably less work we will have to do to see the birds we are looking for.

To answer this question, we:

1. Build a CTE that collects all of the Common Loon sightings into a single geometry.
2. Build another CTE that joins the results of the subquery above with [the `roads` table](#the-roads-table) and pulls out the roads in NY state that are within a degree (about 60 nautical miles). Order the roads returned by their distance from a loon sighting. This will return some duplicate roads (since loons can be sighted multiple times along a single road), which is why we `LIMIT` to 25 here so we can get the list down to 10 later. Because the data in the `roads` table has an SRID of 0, we need to use `ST_SetSRID` to set it the SRID to [4326](srid-4326.html) because `ST_Distance` cannot operate on geometries with differing SRIDs.  We used `ST_SetSRID` rather than `ST_Transform`, because the latter actually modifes the coordinates of the geometry.  We don't want to modify the coordinates, since we don't necessarily want to modify the underlying data in the `roads` table, just query it.
3. Finally, we query the results of the second subquery above to get a list of 10 distinct road names (`road.prime_name`) that we can plan to visit.
...
Unfortunately this query is a bit slower than we would like: about 1.5 seconds. There are several reasons for this:

1. We haven't created any indexes at all yet. The query is likely to be doing full table scans, which we will need to hunt down with [`EXPLAIN`](explain.html).
...
Let's look at the `EXPLAIN` output to see if there is something we can do to improve this query's performance:
...
As you can see above, there are several full table scans being performed (see `FULL SCAN`). For each table, we have a hypothesis on what is causing the scan; usually our hypothesis is that we are missing an index.

- For [the `roads` table](#the-roads-table), there is no spatial index on the `road.geom` column, nor is there an index on the `roads.state` column which we're filtering on. This is probably the worst performance offender of the bunch, since this table has almost a quarter of a million rows. It makes sense that it's quite large, since it represents a road map of the entire United States.
- On the `birds.observations` table, since there is no index on the `birds.observations.route_id` or `birds.observations.bird_id` columns, which are used for all bird information lookups.
- On the `birds.birds` table, since there is no index on the `birds.birds.name` column which we're filtering on (this is also used by almost all of our bird-related queries).
- On the `birds.routes` table, since there is no spatial index on the `birds.routes.geom` column, which is being used in the join with the `ST_Distance` predicate. We want all of the inputs to this function to be indexed, since as you can see in the `EXPLAIN` output above, it is using `ST_DWithin` under the hood, which is an [index-accelerated function](spatial-data.html#performance).

Let's start by adding an index to the `roads.geom` and `roads.name` columns. Note that creating the index on `roads.geom` will take a few seconds, since the table is relatively large.
...
Next let's add indexes on the `birds.observations.route_id` or `birds.observations.bird_id` columns, which are used for all bird information lookups:
...
Next let's add an index on the `birds.birds.name` column:
...
Finally, let's add an index on the `birds.routes.geom` column, which is needed to make sure the join on the `ST_Distance` predicate is using spatial indexes to go faster.

{% include copy-clipboard.html %}
~~~ sql
CREATE INDEX ON birds.routes USING GIST(geom);
~~~

Did adding all of these indexes make our query any faster?  Let's check:
...
It looks like adding these indexes sped up our query by about 1/3, going from ~1.5 seconds to ~1 second:
...
To see why, let's look at some `EXPLAIN` output for the query above now that we have indexes:
...
It looks like we are still performing a full table scan on `birds.routes`.  This means that at least one of our hypotheses above was incorrect.

However, some of our other indexes appear to be paying off. The index on the `roads.state` column looks like the primary reason for the speedup, since we can see that instead of scanning ~225,000 rows of the `roads` table, we are now scanning ~8700, or about 4% as many rows as before. In addition, we no longer appear to be scanning all ~85000 rows of the `birds.observations` table thanks to the index on the `birds.observations.bird_id` column. Finally, we are probably getting a tiny speedup from the index on `birds.birds.name` (although that table is not very large, only about 750 rows).

### (16) How many miles of roads are contained within the portion of the Loon habitat that lies within NY state?

<!-- Time: 174ms -->

It may not be immediately relevant to our birdwatching or book-buying travels, but a question of general interest that could arise is: how many miles of roads are contained within loon habitat?

To answer this, we:

1. Build a CTE that returns the [convex hull](st_convexhull.html) of Common Loon habitat.
2. Join the results of the above CTE and a query against [the `roads` table](#the-roads-table) that sums the mileage of all roads that are contained within the convex hull of loon habitat. Because the data in the `roads` table has an SRID of 0, we need to use `ST_SetSRID` to set it to [SRID 4326](srid-4326.html) because `ST_Contains` cannot operate on geometries with differing SRIDs.

{{site.data.alerts.callout_info}}
Because we are using `ST_Contains`, we are only summing road mileages of roads whose geometries lie entirely within the loon habitat.
{{site.data.alerts.end}}
...
The answer is, there are about 1700 miles of roads within the Common Loon's habitat. Further, you can change the `WHERE` clause of the above query slightly to say `AND (NOT (ST_Contains(...)))` to find out how many miles of roads in NY state are _not_ within loon habitat (it's about 14,600).
...
As you are driving around the Adirondacks, searching for loons as well as your next great read, the question occurs to you: which bookstore is the most remotely located?  We don't have population density data, but we do have road data. Therefore we decide to use "miles of roads near the store" as a rough proxy for which store location is the most remote: the fewer miles of roads near the store, the more remote we can assume it is.
...
3. Finally, we generate a query that joins the results of the above subquery against [the `roads` table](#the-roads-table) based on which roads are within a 10 mile radius. This generate a list of bookstores and the number of miles of nearby roads, sorted in order of which store has the fewest miles of road nearby.
...
We get an answer, but unfortunately it's rather slow: about 6 seconds!
  ...
  sort                                                          |                       |
   ...
   └── render                                                   |                       |
        └── group                                               |                       |
             ...
             └── cross join                                     |                       |
                  │                                             | pred                  | st_dwithinexclusive(st_setsrid(geom, 4326), geom, 0.14492753623188406)
                  ├── scan                                      |                       |
                  │                                             | estimated row count   | 225838
                  │                                             | table                 | roads@primary
                  │                                             | spans                 | FULL SCAN
...
Looking at these results from the bottom up, we can see that:
...
2. There is a full table scan on the `bookstores` table. Since the predicate is [`ST_Contains`](st_contains.html), we probably need to add a [spatial index](spatial-indexes.html) on `bookstores.geom`.
3. There is a full table scan happening on the `roads` table. This is a serious problem due to the size of that table: ~225,000 rows. We already have an index on the `roads.geom` column for the `ST_DWithin` predicate to use, so we need to reduce the number of rows scanned using some other method. We can do what we did in [query 16][q_16] and add a check that the `roads.state = 'NY'`, since we are only looking at roads inside New York State.

First, let's add an index to the `bookstores.geom` column:
...
Next, let's modify our query to filter on `roads.state` to look at just roads in New York (as in [query 16][q_16]) and see what happens:
...
That brings our query execution down to about 400ms!
...
When we look at `EXPLAIN` for the modified query, we see why: the filter on `roads.state='NY'` means we are scanning far fewer columns of the `roads` table (~8900), and the index on `bookstores.geom` means we are using that index now as well:
...
If you get tired of looking for Common Loons (and it's understandable that you might), you may want to widen your search to other types of birds. We have already looked at [different bird species based on frequency of observation][q_13]. Now let's turn our attention to a specific type of bird: hawks.

After doing some searches on <https://ebird.org>, you discover that all of the hawks you can find have the family "Accipitridae". Even though you are not an expert in such things, you decide to make the assumption that all hawks are in that family to answer the question: what hawks live in the same region as the Common Loon in New York State?
...
### (19) What if we also want to look for owls as well as hawks?
...
[q_02]: #2-what-is-the-area-of-the-loons-sightings-range
...
[q_09]: #9-what-does-the-convex-hull-of-bookstore-locations-that-lie-within-the-loons-habitat-look-like
[q_10]: #10-what-is-the-area-of-the-convex-hull-of-bookstore-locations-that-are-in-the-loons-habitat-range-within-ny-state
...
[q_19]: #19-what-if-we-also-want-to-look-for-owls-as-well-as-hawks
