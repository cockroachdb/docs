...
In v21.1 and earlier, Google Cloud Storage connections can use `default`. This means that the connection will only use the key provided in the `cloudstorage.gs.default.key` [cluster setting](cluster-settings.html), and will error if not present.
