...
- The [logging channels](logging-system.html#logging-channels) that are mapped to each sink.
...
Log *sinks* route events from specified [logging channels](logging-system.html#logging-channels) to destinations outside CockroachDB. These destinations currently include [log files](#output-to-files), [Fluentd](https://www.fluentd.org/)-compatible [servers](#output-to-network), and the [standard error stream (`stderr`)](#output-to-stderr).
