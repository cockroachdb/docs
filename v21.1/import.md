...
`ignore_unsupported_statements` | `PGDUMP`  | {% include_cached new-in.html version="v21.1" %} Ignore SQL statements in the dump file that are unsupported by CockroachDB.
`log_ignored_statements` | `PGDUMP` | {% include_cached new-in.html version="v21.1" %} Log unsupported statements when using `ignore_unsupported_statements` to a specified destination (i.e., [cloud storage](use-cloud-storage-for-bulk-operations.html) or [userfile storage](use-userfile-for-bulk-operations.html).
