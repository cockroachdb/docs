...
`--global` | <a name="global-flag"></a> Simulate a [multi-region cluster](simulate-a-multi-region-cluster-on-localhost.html) which sets the [node localities](cockroach-start.html#locality) in different regions, and also simulates the network latency that would occur between them given the specified localities.  In order for this to operate as expected, with 3 nodes in each of 3 regions, you must also pass the `--nodes 9` argument.
