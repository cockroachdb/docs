...
- Each instance of `cockroach demo` loads a temporary [{{ site.data.products.enterprise }} license](https://www.cockroachlabs.com/get-cockroachdb) that expires after an hour. To prevent the loading of a temporary license, set the `--disable-demo-license` flag.
...
`--disable-demo-license` |  Start the demo cluster without loading a temporary [{{ site.data.products.enterprise }} license](https://www.cockroachlabs.com/get-cockroachdb) that expires after an hour.<br><br>Setting the `COCKROACH_SKIP_ENABLING_DIAGNOSTIC_REPORTING` environment variable will also prevent the loading of a temporary license, along with preventing the sharing of anonymized [diagnostic details](diagnostics-reporting.html) with Cockroach Labs.
...
#
# This demo session will attempt to enable {{ site.data.products.enterprise }} features
