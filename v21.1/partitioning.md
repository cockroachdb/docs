...
#### Partition using foreign key

If a partitioned table contains a [foreign key](foreign-key.html) to a non-partitioned table, the secondary index created auto-generated for the foreign key will not be partitioned. This can lead to performance impacts when querying against the partitioned table, as the data may exist in a distant node.
