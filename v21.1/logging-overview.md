...
- A payload that contains events either structured in JSON or conveyed in an arbitrary string. For details on event types and their fields, see [Notable Event Types](eventlog.html).
...
| [`SQL_EXEC`](logging.html#sql_exec)                 | SQL statement executions (when enabled via the `sql.trace.log_statement_execute`) [cluster setting](cluster-settings.html) and pgwire events (when enabled via TK).                                                                                                                                                        |
