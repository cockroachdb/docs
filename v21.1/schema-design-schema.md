...
For the examples in the rest of the [**Design a Database Schema** sections](schema-design-table.html), we'll add all database schema change statements to a `.sql` file, and then pass the file to the [SQL client](cockroach-sql.html) for execution as the `max` user.
{{site.data.alerts.end}}
...
The SQL client will execute any statements in `dbinit.sql`, with `cockroachlabs` as the database and `max` as the user.
...
You're now ready to start adding tables to the `movr` user-defined schema as the `max` user. For guidance on creating tables, see at [Create a Table](schema-design-table.html).
