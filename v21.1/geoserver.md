...
- CockroachDB [uses a different indexing strategy based on space-filling curves](spatial-indexes.html) since this is necessary for [scaling horizontally](frequently-asked-questions.html#how-does-cockroachdb-scale). This means that the coverings generated by CockroachDB's `&&`, `~` and `@` operators are not the same as the bounding box coverings generated by PostGIS. In practice, this means that CockroachDB may return a smaller set of results for the same query, as the space-filling curve covering is often more exact than a bounding box covering.
