...
Start a CockroachDB cluster following the instructions in [Start a Local Cluster](start-a-local-cluster.html).
...
Some GeoServer features require the use of the `ST_HasArc` builtin function, which is not yet implemented in CockroachDB.
{{site.data.alerts.end}}
...
In this and the following steps we will set up GeoServer so it can access the spatial data we loaded in Step 2.
...
In the left-hand side navigation menu, click **Data > Workspaces**. The **Workspaces** page will load. Click the **Add new workspace** button.
...
In the left-hand side navigation menu, click **Data > Stores**. The **Stores** page will load. Click the **Add new Store** button.
...
1. Uncheck the **Support on the fly geometry simplification** checkbox
...
In the left-hand side navigation menu, click **Data > Layer Preview**.
...
Your browser should open a new tab with the title **OpenLayers map preview**. It should show a map view that looks something like the following:
