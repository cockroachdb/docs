...
Therefore, we recommend making sure to design your schema such that information which must remain domiciled _cannot_ be deduced from the schema design (e.g., primary keys, table names, column names, usernames).
...
As mentioned above, the best way to keep specific data sets cordoned off from each other in CockroachDB is to keep it in a separate database. To create a separate database for EU-based users, run the following statement:
...
Next, make sure that CockroachDB is storing the data in the eu_users database in European regions using the [`ALTER DATABASE ... SET PRIMARY REGION`](set-primary-region.html) and [`... ADD REGION`](add-region.html) statements:
...
As noted above, there are several limitations of this approach:
