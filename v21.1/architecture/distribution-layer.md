...
{{site.data.alerts.callout_info}}
A range will only attempt to merge with its right-hand neighbor. It will not attempt to merge with its left-hand neighbor (i.e. the previous range that ends where the current range starts).
{{site.data.alerts.end}}
