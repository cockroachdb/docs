...
Successful `EXPLAIN` statements return tables with the following details in the `info` column:
 ...
 Detail | Description
-----------|-------------
Global properties | The `distribution` and `vectorized` properties apply to the entire query plan.
Query plan tree | A tree representation of the hierarchy of the query plan.
...
The output shows the tree structure of the query plan, in this case a `sort`, a `filter`, and then a `scan`.
