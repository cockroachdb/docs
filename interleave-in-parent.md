...
Interleaving tables improves query performance by optimizing the key-value structure of closely related tables, attempting to keep data on the same CockroachDB node if it's likely to be read and written together.
...
In the key-value store, interleaving tables writes data from one table (known as the **child**) directly into another (known as the **parent**) by matching the two table's Primary Keys, acting as a kind of join between the tables.
...
By writing data in this way, related data's more likely remain on the same CockroachDB node, which can make it much faster to read from and write to. Using the above example, all of customer 1's data is going to be written to the same node, including its representation in both the `customers` and `orders` tables.
...
{{site.data.alerts.callout_info}}Regardless of your interleaved hierarchy's depth, the interleave prefix is always the entire Primary Key of the parent table and does not have a directly relationship with any other ancestor table. However, a child table might still have a Foreign Key constraints referencing a grandparent, but the interleave prefix itself only directly references the parent table's Primary Key.{{site.data.alerts.end}}

### Benefits

- In the general case, reads and writes of data related through the interleave prefix is faster.
- Reads and writes involving only tables in the interleave hierarchy using the same values for the root table's Primary Key are faster because they're most likely to be on the same node.<br/><br/>For example, a query like "Get all of a specific user's orders" would execute more quickly if `orders` was interleaved into `customers`. However, a query like "Get all users who placed an order today" would not be improved.
- Reads and writes with greater levels of specificity in the interleave prefix will be faster the more specific they are.<br/><br/>For example, "Get all packages that have been delivered from a specific user's specific order" would be fast assuming `packages`, `orders, and `customers` formed an interleaved hierarchy.
- `JOIN`s between tables in the interleaved hierarchy using the interleave prefix as the join condition are much faster.

### Tradeoffs

- In the general case, reads and deletes over ranges of data (e.g., `WHERE col > val`) in interleaved tables are slower.<br/><br/>However, an exception to this is performing range operations on the greatest descendent in the interleaved hierarchy that filters on all columns of the interleave prefix with constant values.<br/><br/>For example, if `packages` is an interleaved child of `orders`, which is itself an interleaved child of `customers`, a statement that filters on the interleave prefix, like `WHERE customer = 1 AND order = 1001 AND delivery_date > DATE '2016-01-25'`, would still be fast because all of the values from the interleave prefix (`packages.customer`, `packages.order`) are equal.
- If the amount of interleaved data stored for any Primary Key value of the root table is larger than [a range's maximum size](configure-replication-zones.html#replicaton-zone-format) (64MB by default), the interleaved optimizations will be diminished.<br/><br/>For example, if one customer has 200MB of order data, their data is likely to be spread across multiple nodes and CockroachDB will not be able to access it as quickly, despite it being interleaved.
