...
- [Mac](https://d3a3ys8r4slo6p.cloudfront.net/cockroach-beta-20160414.darwin-10.9-amd64.tgz)
- [Linux](https://d3a3ys8r4slo6p.cloudfront.net/cockroach-beta-20160414.linux-amd64.tgz)
...
* The [SQL function](functions-and-operators.html) `transaction_timestamp_unique` has been removed in favor of the new `cluster_logical_timestamp` function described below.

### New Features

* [`ALTER TABLE`](alter-table.html) supports several new operations: `ADD COLUMN` with a
  default value
...
* The `TIMESTAMPTZ` type is now accepted as an alias for [`TIMESTAMP`](timestamp.html)
...
* The [`STRING`](string.html) type now accepts a length parameter (e.g.,
...
* The `RETURNING` clause of [`INSERT`](insert.html), [`DELETE`](delete.html), and [`UPDATE`](update.html)
...
* The `experimental_uuid_v4()` [SQL function](functions-and-operators.html) has been renamed to
...
* [SQL functions](functions-and-operators.html) related to the current time (`now`,
...
* [Logs](cockroach-commands.html#logging-flags) are no longer written to `stderr` by default, only to the
