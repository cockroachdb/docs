...
* Any databases using the [`DECIMAL`](decimal.html) type will need to
  be deleted and created from scratch. Columns of this type were
  encoded incorrectly older beta releases. (Again! We apologize for
  the inconvenience)
  [#5994](https://github.com/cockroachdb/cockroach/pull/5994)
* The SQL function `transaction_timestamp_unique` has been removed in
  favor of the new `cluster_logical_timestamp` function described
  below.

### New Features

* `ALTER TABLE` supports several new operations: `ADD COLUMN` with a
  default value
...
* The `TIMESTAMPTZ` type is now accepted as an alias for `TIMESTAMP`
...
* The `STRING` type now accepts a length parameter (e.g.
...
* The `RETURNING` clause of `INSERT`, `DELETE`, and `UPDATE`
...
* The `experimental_uuid_v4()` SQL function has been renamed to
...
* SQL functions related to the current time (`now`,
...
* Logs are no longer written to `stderr` by default, only to the
