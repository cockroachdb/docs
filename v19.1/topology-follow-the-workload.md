...
If read performance is your main focus for a table, but you want low-latency reads everywhere instead of just in the most active region, consider the [Pinned Index Leaseholders](topology-pinned-index-leaseholders.html) or [Follower Reads](topology-follower-reads.html) pattern.
{{site.data.alerts.end}}
...
{{site.data.alerts.callout_info}}
This pattern is also used by the [system ranges containing important internal data](#cluster-resiliency).
{{site.data.alerts.end}}
...
3. The gateway node routes the request to the leaseholder replica in `us-east`.
...
1. The write request reaches the regional load balancer.
2. The load balancer routes the request to a gateway node.
