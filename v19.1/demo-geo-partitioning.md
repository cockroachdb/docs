...
CockroachDB's [geo-partitioning](partitioning.html) feature gives you low-latency reads and writes in a broadly distributed CockroachDB cluster. This tutorial walks you through the process of using geo-partitioning in a 9-node cluster deployed across 3 US regions on GCE. If you follow along, you'll see dramatic reductions in latency, with the majority of reads and writes executing in 2 milliseconds or less.

## Watch a demo

STILL NEED TO POST VIDEO TO YOUTUBE
...
1. A user loads the app and see the 25 closest vehicles. Behind the scenes, this is a `SELECT` from the `vehicles` table:
...
5. One any of the VMs in the US East region, run the one-time [`cockroach init`](initialize-a-cluster.html) command to join the first nodes into a cluster:
...
5. Run the [`cockroach gen haproxy`](generate-cockroachdb-resources.html) command to generate an HAProxy config file, specifying the address of any CockroachDB node:

    {% include copy-clipboard.html %}
	  ~~~ shell
	  $ cockroach gen haproxy --insecure --host=<address of any node>
	  ~~~
    ...
    listen psql
        ...
        server cockroach4 <node4 address>:26257 check port 8080
        server cockroach5 <node5 address>:26257 check port 8080
        server cockroach6 <node6 address>:26257 check port 8080
        server cockroach7 <node7 address>:26257 check port 8080
        server cockroach8 <node8 address>:26257 check port 8080
        server cockroach9 <node9 address>:26257 check port 8080
    ~~~

6. Edit the config and remove all but the `us-east1` node addresses. This ensures that the load balancer in the US East region will only direct traffic to CockroachDB nodes in the same region.

7. Start HAProxy, with the `-f` flag pointing to the `haproxy.cfg` file:
...
8. Repeat the steps above for the client VMs in the other two regions. For each region, be sure to edit the `haproxy.cfg` files to include only the node addresses for the region.
...
5. Set the longitude and latitude of the regions where you are running CockroachDB nodes:
    ...
    {% include copy-clipboard.html %}
    ~~~ sql
    > INSERT into system.locations VALUES ('region', 'us-east1', 33.836082, -81.163727); INSERT into system.locations VALUES ('region', 'us-central1', 42.032974, -93.581543); INSERT into system.locations VALUES ('region', 'us-west1', 43.804133, -120.554201);
    ~~~
...
1. Still on the client VM in the US East region, load the MovR schema and initial data for the cities of New York, Chicago, and Seattle, pointing at the address of the US East load balancer:
    ...
    {% include copy-clipboard.html %}
    ~~~ shell
    $ sudo docker run -it --rm cockroachdb/movr:19.03.2 --app-name "movr-load" --url "postgres://root@<address of HAProxy in US East>:26257/movr?sslmode=disable" load --num-users 100 --num-rides 100 --num-vehicles 10 --city-pair us_east:"new york" --city-pair central:chicago --city-pair us_west:seattle
    ~~~
...
2. Start MovR in the US East region, representing users in New York. Be sure to point at the address of the US East load balancer:
    ...
    {% include copy-clipboard.html %}
    ~~~ shell
    $ sudo docker run -it --rm cockroachdb/movr:19.03.2 --app-name "movr-east" --url "postgres://root@<address of HAProxy in US East>:26257/movr?sslmode=disable" --num-threads=15 run --city="new york"
    ~~~
...
2. Start MovR in the US Central region, representing users in Chicago. Be sure to point at the address of the US Central load balancer:
    ...
    {% include copy-clipboard.html %}
    ~~~ shell
    $ sudo docker run -it --rm cockroachdb/movr:19.03.2 --app-name "movr-central" --url "postgres://root@<address of HAProxy in US Central>:26257/movr?sslmode=disable" --num-threads=15 run --city="chicago"
    ~~~
...
2. Start MovR in the US West region, representing users in Seattle. Be sure to point at the address of the US West load balancer:
    ...
    {% include copy-clipboard.html %}
    ~~~ shell
    $ sudo docker run -it --rm cockroachdb/movr:19.03.2 --app-name "movr-west" --url "postgres://root@<address of HAProxy in US West>:26257/movr?sslmode=disable" --num-threads=15 run --city="seattle"
    ~~~
...
Now that all tables and secondary indexes have been partitioned by city, for each partition, you can now create a replication zone that pins the partition's replicas to nodes in a specific region, using the localities specified when nodes were started.
...
You'll see that the replicas for each partition are now located on nodes in the relevant region: New York partitions are on nodes 1 - 3, Chicago partitions are all on nodes 4 - 6, and Seattle partitions are on nodes 7 - 9. This means that requests from users in a city no longer leave the region, thus removing all cross-region latencies.

## Step 14. Re-check SQL latency

Now that data is partitioned by city and you've verified that replicas are located properly, go back to the Admin UI, click **Metrics** on the left, and hover over the **Service Latency: SQL, 99th percentile** timeseries graph:
...
99th percentile latency can be influenced by occasional slow queries. For a more accurate sense of average SQL latency, go to the following URL to view a custom graph for 90th percentile latency:
