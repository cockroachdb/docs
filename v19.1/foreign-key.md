...
Note that allowing _NULL_ values in either your foreign key or referenced columns can degrade their referential integrity, since any key with a _NULL_ value is never checked against the referenced table. To avoid this, you can use a [`NOT NULL` constraint](not-null.html) on foreign keys when [creating your tables](create-table.html). 
...
In versions 2.1 and earlier, the only option for composite foreign key matching was an incorrect implementation of `MATCH FULL`. This allowed _NULL_ values in the referencing key columns to correspond to _NULL_ values in the referenced key columns. This was incorrect in two ways:

1. `MATCH FULL` should not allow mixed _NULL_ and non-null values. See below for more details on the differences between comparison methods. 
2. _NULL_ values cannot ever be compared to each other.
...
- **Valid** keys may not contain any _NULL_ values.

- **Invalid** keys contain one or more _NULL_ values.
...
- **Invalid** keys must have all _NULL_ values.

- **Unacceptable** keys have any combination of both _NULL_ and non-_NULL_ values. In other words, `MATCH FULL` requires that if any column of a composite key is _NULL_, then all columns of the key must be _NULL_.
