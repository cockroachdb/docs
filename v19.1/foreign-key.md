...
In versions 2.1 and earlier, the only option for composite foreign key matching was that of an incorrect implementation of `MATCH FULL` that allowed _NULL_ values in the referencing key columns to correspond to _NULL_ values in the referenced key columns. This was incorrect in two ways. The first is that `MATCH FULL` should not allow mixed _NULL_ and non-null values. See below for more details on the differences between comparison methods. Secondly, _NULL_ values cannot ever be compared to each other.
