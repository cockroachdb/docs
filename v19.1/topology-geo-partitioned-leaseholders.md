...
In a multi-region deployment, the geo-partitioned leaseholders topology is a good choice for tables with the following characteristics:

- The table is read frequently but updated infrequently.
- Reads must be fast, but writes can be slower.
...
- Partitions must remain available during a region failure.
...
If reads from a table can be historical (48 seconds or more in the past), consider the [Follower Reads](topology-follower-reads.html) pattern. If rows in the table **cannot** be tied to specific geographies, consider the [Pinned Index Leaseholders](topology-pinned-index-leaseholders.html) pattern.
{{site.data.alerts.end}}
...
Because this pattern balances the replicas for each partition across regions, one entire region can fail without interrupting access to any partitions:
...
However, if an additional machine holding a replica for a partition fails at the same time as the region failure, the partition becomes unavailable for reads and writes:
