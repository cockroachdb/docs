...
1. The database descriptor (an internal data structure) is updated, and announced to every other node.
...
The workaround is to set a value in a "lock" table (using the values 1 vs 0 in a table cell) to indicate that a database rename is in progress. Since table access is transactional, that would create isolation around the database rename process.

This is a [known limitation](known-limitations.html).  For an issue tracking this limitation, see [cockroach#12123](https://github.com/cockroachdb/cockroach/issues/12123).
