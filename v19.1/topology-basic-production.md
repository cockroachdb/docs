...
Since all ranges, including leaseholder replicas, are in a single region, reads are very fast.
...
Just like for reads, since all ranges are in a single region, writes can achieve consensus very fast.

For example, in the animation below:

1. The write request reachs the load balancer.
2. The load balancer routes the request to a gateway node.
...
For the cluster as a whole to remain available, these system ranges must retain a majority of their replicas, so CockroachDB comes with [pre-configured replication zones](configure-replication-zones.html#create-a-replication-zone-for-a-system-range) for them with the replication factor increased to `5` by default. Based on the [cluster setup](#configuration) described above, with each node started with the `--locality` flag specifying its AZ, each system range's replicas are spread across 3 AZs. This means that one entire AZ can fail without causing any system range to lose consensus:
