...
In a multi-region deployment, the geo-partitioned replicas topology is a good choice for tables with the following characteristics:

- The table is read from and written to frequently.
- Reads and writes must be fast.
- Rows in the table can be tied to specific geographies, e.g., city, state, region.
- Partitions must remain available during an AZ failure.
...
<!-- <img src="{{ 'images/v19.1/topology-patterns/topology_geo-partitioning1_no-map.png' | relative_url }}" alt="Geo-partitioning topology" style="max-width:100%" /> -->
...
<img src="{{ 'images/v19.1/topology-patterns/topology_geo-partitioning_reads.gif' | relative_url }}" alt="Geo-partitoning topology" style="max-width:100%" />

#### Writes

Just like for reads, because each partition is constrained to the relevant region (e.g., the `la` and `la_idx` partitions are located in the `us-west` region), writes that specify the local region key access the relevant replicas without leaving the region. This makes writes very fast, with the exception of writes that refer to a partition in another region.
