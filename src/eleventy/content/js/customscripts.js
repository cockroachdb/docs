...
$(function() {
  ...
  // Dynamically add copy buttons to all code blocks that don't have them
  // This allows us to remove {% include "copy-clipboard.html" %} from markdown
  var copyButtonHtml = '<div class="copy-clipboard">' +
    ...
    '<svg id="copy-check" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 12 10"><style>.st1{fill:#6933FF;}</style><path id="path-1_2_" class="st1" d="M3.8 9.1c-.3 0-.5-.1-.6-.2L.3 6C0 5.7-.1 5.2.2 4.8c.3-.4.9-.4 1.3-.1L3.8 7 10.6.2c.3-.3.9-.4 1.2 0 .3.3.3.9 0 1.2L4.4 8.9c-.2.1-.4.2-.6.2z"/></svg>' +
    '</div>';

  // Find all code blocks (highlighter-rouge divs) and add copy button if not present
  // Wrap in a container for proper positioning
  $('.highlighter-rouge').each(function() {
    var $codeBlock = $(this);
    var $prev = $codeBlock.prev();
    // Skip if already has a copy-clipboard (from old includes) or already wrapped
    if (!$prev.hasClass('copy-clipboard') && !$codeBlock.parent().hasClass('code-block-wrapper')) {
      // Wrap the code block and prepend the copy button inside the wrapper
      $codeBlock.wrap('<div class="code-block-wrapper"></div>');
      $codeBlock.parent().prepend(copyButtonHtml);
    }
  });
