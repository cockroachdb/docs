...
{% dynamic_include page.version.version, "/prod-deployment/topology-recommendations.md" %}
...
- For cluster stability, Cockroach Labs recommends a _minimum_ of {% dynamic_include page.version.version, "/prod-deployment/provision-cpu.md" %}, and strongly recommends no fewer than {% dynamic_include page.version.version, "/prod-deployment/provision-cpu.md" %} per node. In a cluster with too few CPU resources, foreground client workloads will compete with the cluster's background maintenance tasks. For more information, see [capacity planning issues]({% link "{{ page.version.version }}/cluster-setup-troubleshooting.md" %}#capacity-planning-issues).
...
- Cockroach Labs does not extensively test clusters with more than {% dynamic_include page.version.version, "/prod-deployment/provision-cpu.md" %} per node. This is the recommended _maximum_ threshold.
...
{% capture cap_per_vcpu %}{% include "v22.1/prod-deployment/provision-storage.md" %}{% endcapture %}
...
Provision at least {% dynamic_include page.version.version, "/prod-deployment/provision-memory.md" %} for consistency across a variety of workload complexities. The minimum acceptable ratio is 2 GiB of RAM per vCPU, which is only suitable for testing.
...
We recommend provisioning volumes with {% dynamic_include page.version.version, "/prod-deployment/provision-storage.md" %}. It's fine to have less storage per vCPU if your workload does not have significant capacity needs.

- The maximum recommended storage capacity per node is 10 TiB, regardless of the number of vCPUs.

- {% dynamic_include page.version.version, "/prod-deployment/prod-guidance-store-volume.md" %}

- {% dynamic_include page.version.version, "/prod-deployment/prod-guidance-log-volume.md" %}
...
Disks must be able to achieve {% dynamic_include page.version.version, "/prod-deployment/provision-disk-io.md" %}.
...
- {% dynamic_include page.version.version, "/prod-deployment/prod-guidance-lvm.md" %}
...
{% dynamic_include page.version.version, "/prod-deployment/recommended-instances-aws.md" %}
...
{% dynamic_include page.version.version, "/prod-deployment/recommended-instances-azure.md" %}
...
{% dynamic_include page.version.version, "/prod-deployment/recommended-instances-gcp.md" %}
...
{{site.data.alerts.callout_info}}
{% dynamic_include page.version.version, "/misc/cert-auth-using-x509-subject.md" %}
{{site.data.alerts.end}}
...
{% dynamic_include page.version.version, "/prod-deployment/prod-guidance-connection-pooling.md" %}
...
{% dynamic_include page.version.version, "/sql/server-side-connection-limit.md" %}

## Monitoring and alerting

{% dynamic_include page.version.version, "/prod-deployment/monitor-cluster.md" %}
...
{% dynamic_include page.version.version, "/faq/clock-synchronization-effects.md" %}
