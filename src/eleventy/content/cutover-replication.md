...
{% include "new-in.html", version: "v24.2" %} To avoid multiple clusters running the same schedule concurrently, [changefeed schedules]({% link "{{ page.version.version }}/create-schedule-for-changefeed.md" %}) will [pause]({% link "{{ page.version.version }}/pause-schedules.md" %}) after physical cluster replication has completed.
