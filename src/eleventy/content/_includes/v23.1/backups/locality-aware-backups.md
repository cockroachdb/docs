...
{% include "new-in.html", version: "v23.1" %} `SHOW BACKUP` is able to display metadata using `check_files` for locality-aware backups taken with the [`incremental_location`]({% link "{{ page.version.version }}/show-backup.md" %}#show-a-backup-taken-with-the-incremental-location-option) option.
