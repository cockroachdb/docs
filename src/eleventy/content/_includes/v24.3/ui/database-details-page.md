...
{% include "new-in.html", version: "v24.3" %} To filter databases based on the nodes on which the database tables are located, use the nodes multi-select dropdown above the list table:
...
{% include "new-in.html", version: "v24.3" %} Click on the **Grants** tab [Database Details Page](#database-details-page) to show the [privileges]({% link "{{ version_prefix }}/security-reference/authorization.md" %}#managing-privileges) granted to users and roles on the database.
