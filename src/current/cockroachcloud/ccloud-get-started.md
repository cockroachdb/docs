...
You can set the cluster name, cloud infrastructure provider, region, and [resource limits]({{site.current_cloud_version}}/architecture/glossary.md#resource-limits) as command options. The following command is equivalent to the previous command that uses the default values.
...
Use the `--sso` flag to connect to your cluster using [single sign-on (SSO) authentication](cloud-sso-sql.md), which will allow you to start a SQL shell without using a password.
...
By default, the `ccloud cluster sql` command will allow connections only from IP addresses in your cluster's [allowlist](network-authorization.md#ip-allowlisting). Use the `--skip-ip-check` flag to disable the client-side IP allowlist check:
...
To get the [connection URL]({{site.current_cloud_version}}/connection-parameters.md#connect-using-a-url), use the `--connection-url` option.
...
- {% include "_includes/cockroachcloud/limitations/limitation-ccloud-folders.md" %}
