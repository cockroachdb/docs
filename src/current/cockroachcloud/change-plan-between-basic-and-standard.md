...
- A single-region cluster is unable to utilize many of the features available with the CockroachDB {{ site.data.products.standard }} plan. An existing single-region cluster cannot be modified in place into a multi-region cluster, so follow the procedure to [migrate to a new {{ site.data.products.standard }} cluster](#migrate-to-a-new-standard-cluster).
- If the option to Change Plan Type is unavailable, hovering your mouse on this option may display a note that the cluster was created on legacy architecture and, therefore, its plan cannot be changed in the Cloud Console. Such a cluster will not support multi-region features or private connections on any plan. To use these features, follow the procedure to [migrate to a new {{ site.data.products.standard }} cluster](#migrate-to-a-new-standard-cluster). Alternatively, to change the plan for your current cluster despite these feature limitations, you can use the [Cloud API]({% link cockroachcloud/cloud-api.md %}#change-a-clusters-plan).
