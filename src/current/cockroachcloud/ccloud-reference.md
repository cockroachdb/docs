...
The `ccloud quickstart create` command interactively guides you through creating and connecting to a new CockroachDB {{ site.data.products.basic }} cluster.
...
### Create a {{ site.data.products.standard }} cluster

Use the `ccloud cluster create standard` command to create a new CockroachDB {{ site.data.products.standard }} cluster.
...
This command creates a CockroachDB {{ site.data.products.standard }} cluster in the default cloud infrastructure provider (GCP) and the closest region for that provider. It will generate a cluster name.
