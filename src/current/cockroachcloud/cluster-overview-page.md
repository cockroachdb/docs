...
| [Request Units]({% link cockroachcloud/plan-your-cluster-basic.md %}#request-units) | The CPU and I/O resources being used by queries on the cluster, and whether throttling is in effect. The total available RUs are determined by your configured capacity. For more context, see [CockroachDB {{ site.data.products.cloud }} Glossary]({% link {{ page.version.version }}/architecture/glossary.md %}).              |
