...
Egress Perimeter Controls are not supported for {{ site.data.products.dedicated }} clusters on Azure ([limited access](/docs/{{site.versions["stable"]}}/cockroachdb-feature-availability.html)). Refer to [{{ site.data.products.dedicated }} on Azure](cockroachdb-dedicated-on-azure.html).
