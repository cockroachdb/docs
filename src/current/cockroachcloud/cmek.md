...
Customer-Managed Encryption Keys (CMEK) allow you to protect data at rest in a CockroachDB {{ site.data.products.advanced }} [private cluster](private-clusters.md) using a cryptographic key that is entirely within your control, hosted in a supported cloud provider key-management system (KMS). This key is called the _CMEK key_.
...
To learn more, visit [Managing Customer-Managed Encryption Keys (CMEK) for CockroachDB {{ site.data.products.advanced }}](managing-cmek.md).
...
You can learn more about the [supported integrations between CockroachDB and HashiCorp Vault]({{site.current_cloud_version}}/hashicorp-integration.md).
...
This article describes how CMEK works in CockroachDB {{ site.data.products.advanced }} clusters. To configure CMEK, see [Manage Customer-Managed Encryption Keys (CMEK) for CockroachDB {{ site.data.products.advanced }}](managing-cmek.md).
...
When you enable CMEK on a CockroachDB {{ site.data.products.advanced }} [private cluster](private-clusters.md), CockroachDB {{ site.data.products.cloud }} creates two kinds of encryption keys and begins to use them to protect newly-written data at rest. CockroachDB {{ site.data.products.cloud }} manages these encryption keys and propagates them to cluster nodes.
...
For more details about encryption in CockroachDB {{ site.data.products.cloud }}, see [Encryption At Rest]({{site.current_cloud_version}}/encryption.md).
...
{% include "_includes/cockroachcloud/cmek-rotation-types.md" %}

To learn more about rotating a CMEK key using the CockroachDB {{ site.data.products.cloud }} API, visit [Rotate a CMEK key](managing-cmek.md#rotate-a-cmek-key).
...
CMEK can be enabled only on a private cluster on CockroachDB {{ site.data.products.advanced }} with advanced security features enabled. Advanced security features can be enabled only when the cluster is created. Refer to [Private Clusters](private-clusters.md) and [Enable advanced security features](create-an-advanced-cluster.md#step-6-configure-advanced-security-features).

#### Can we enable CMEK for an existing cluster that wasn't created as a private cluster?

An existing cluster cannot be migrated to a private cluster. Instead, create a new CockroachDB {{ site.data.products.advanced }} cluster with advanced security features enabled, manually back up the cluster data to a secure location, then manually import the data to the new cluster. Refer to [Enable advanced security features](create-an-advanced-cluster.md#step-6-configure-advanced-security-features).
...
Yes, when you add a new region to a CMEK-enabled cluster, you must enable CMEK for that region. Refer to [Add a Region to a CMEK-enabled Cluster](managing-cmek.md#add-a-region-to-a-cmek-enabled-cluster).
...
Yes, the [managed backups](managed-backups.md) stored in CockroachDB {{ site.data.products.cloud }} infrastructure are also encrypted using the CMEK, using CoackroachDB’s [encrypted backup]({{site.current_cloud_version}}/take-and-restore-encrypted-backups.md) capability. Internally, a backup data key is wrapped by the CMEK, and then the backup data key is used for encrypting the backup.
...
Not yet. Currently, you must use the [CockroachDB {{ site.data.products.cloud }} API](cloud-api.md) or the [CockroachDB Terraform provider](https://registry.terraform.io/providers/cockroachdb/cockroach/latest).
...
- CMEK is not yet available for [CockroachDB {{ site.data.products.advanced }} on Azure](cockroachdb-advanced-on-azure.md). To express interest, contact your Cockroach Labs account team.
- To enable or revoke a CMEK on a cluster, you must use the [Cloud API](cloud-api.md) or the [CockroachDB Terraform provider](https://registry.terraform.io/providers/cockroachdb/cockroach/latest). It's not possible to enable a CMEK using the CockroachDB {{ site.data.products.cloud }} Console.
...
- [Managing Customer-Managed Encryption Keys (CMEK) for CockroachDB {{ site.data.products.advanced }}](managing-cmek.md)
- [Create Private Clusters](private-clusters.md)
- [Encryption At Rest]({{site.current_cloud_version}}/encryption.md)
