...
To configure SCIM provisioning, an IAM admin creates a SCIM app integration in your IdP and configures it to authenticate to CockroachDB {{ site.data.products.cloud }} using a CockroachDB {{ site.data.products.cloud }} service account with the [**Org Administrator**](authorization.md#org-administrator) role.
...
When a user is directly or indirectly unassigned, their CockroachDB {{ site.data.products.cloud }} account is disabled or removed, depending on the capabilities of your IdP. To remove a disabled user from CockroachDB {{ site.data.products.cloud }}, refer to [Manage an Organization's Members](managing-access.md#manage-an-organizations-users).
...
1. As a user with the [**Org Administrator**](authorization.md#org-administrator) role:

   1. [Enable Cloud Organization SSO](configure-cloud-org-sso.md#enable-cloud-organization-sso).
   1. [Create a service account](managing-access.md#create-a-service-account) with the [**Org Administrator** role](authorization.md#org-administrator) and make a note of its API token. This is the bearer token the IdP will use to authenticate to the CockroachDB {{ site.data.products.cloud }} API.

1. If your IdP is Okta, SCIM provisioning can be enabled only on a [custom SAML authentication method](configure-cloud-org-sso.md#saml). This requirement is imposed by Okta, and is not part of the SCIM or SAML protocols.
...
- The API token of a CockroachDB {{ site.data.products.cloud }} service account with the [**Org Administrator**](authorization.md#org-administrator) role.

To add SCIM provisioning to a SAML app integration in Okta:

1. Log in to Okta Admin Dashboard as an admin user.
1. Click **Applications** and edit the [SAML](configure-cloud-org-sso.md#saml) app integration for your CockroachDB {{ site.data.products.cloud }} organization.
1. Click **Edit**.
...
1. Click **Applications** and click the [SAML](configure-cloud-org-sso.md#saml) application for your CockroachDB {{ site.data.products.cloud }} organization.
...
1. Log in to Okta Admin Dashboard as an admin user.
1. Click **Applications** and click the [SAML](configure-cloud-org-sso.md#saml) app integration for your CockroachDB {{ site.data.products.cloud }} organization.
...
Within CockroachDB {{ site.data.products.cloud }}, you can grant [roles](authorization.md#organization-user-roles) to a pushed group, and those roles are automatically granted to the group's members who have accounts in CockroachDB {{ site.data.products.cloud }}.
...
This section shows how to view and manage a group's roles in the CockroachDB {{ site.data.products.cloud }} Console or using the [Cloud API](cloud-api.md).
...
- Learn more about [Cloud Organization SSO](cloud-org-sso.md)
