...
To understand these resources, you need to understand a bit about the CockroachDB {{ site.data.products.basic }} architecture. A CockroachDB {{ site.data.products.basic }} cluster is divided into two layers that run in separate processes: the SQL layer and the storage layer. The SQL layer receives and runs your SQL queries and background jobs. When the SQL layer needs to read or write data rows, it calls the storage layer, which manages a replicated, transactional row store that is distributed across many machines.
