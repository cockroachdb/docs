...
`started` | The [`TIMESTAMPTZ`]({% link {{ page.version.version }}/timestamp.md %}) when the job began running first.
`finished` | The [`TIMESTAMPTZ`]({% link {{ page.version.version }}/timestamp.md %}) when the job was `succeeded`, `failed`, or `canceled`.
`modified` | The [`TIMESTAMPTZ`]({% link {{ page.version.version }}/timestamp.md %}) when the [job record](backup-architecture.html#job-creation-phase) is updated with the job's progress, or when the job was paused or resumed.
