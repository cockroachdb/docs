...
It is important to note that the backup responds with an empty table, excluding the data. The backup process does not send requests for spans from excluded tables when conducting database, table, or cluster backups. Tenant backups will dispatch export requests to every range for the tenant span, and the ranges that host excluded tables will persist in responding with no data. The backups will succeed even when a table is unavailable, if, and only if, that table is excluded.
