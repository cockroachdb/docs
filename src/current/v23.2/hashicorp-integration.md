...
Refer to [Certificate Authentication for SQL Clients in Dedicated Clusters](https://www.cockroachlabs.com/docs/cockroachcloud/client-certs-dedicated) for procedures in involved in adminstering PKI for a CockroachDB {{ site.data.products.dedicated }} cluster.
...
Refer to [Manage PKI certificates for a CockroachDB deployment with HashiCorp Vault]({% link {{ page.version.version }}/manage-certs-vault.md %}) for procedures in involved in adminstering PKI for a CockroachDB {{ site.data.products.core }} cluster.
