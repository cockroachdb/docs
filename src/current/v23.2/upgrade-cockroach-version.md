...
- The following [admission control](https://www.cockroachlabs.com/docs/{{ page.version.version }}/admission-control#operations-subject-to-admission-control) mechanisms, which help to maintain cluster performance and availability when some nodes experience high load: <ul><li>Delete operations</li><li>Replication</li><li>`COPY` commands by default</li><li>Intents using the `sendImmediately` option when the [cluster setting](https://www.cockroachlabs.com/docs/{{ page.version.version }}/cluster-settings) `kv.intent_resolver.send_immediately.bypass_admission_control.enabled` is `false` (defaults to `true`), and batched intents when the cluster setting `kv.intent_resolver.batch.bypass_admission_control.enabled` is `false` (the default).</li></ul><br />[#98308][#98308]
