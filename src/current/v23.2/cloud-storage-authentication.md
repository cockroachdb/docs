...
### Amazon S3 external ID

Setting up an external ID allows a user to grant Amazon S3 resources to a third-party organization. The external ID is optional information that is passed in an IAM role trust policy to designate who can assume the role, provided a certain value is present in the external ID. Using an external ID helps avoid the [confused deputy problem](https://en.wikipedia.org/wiki/Confused_deputy_problem). An external ID could be useful when you have configured a role for a third-party organization to access your Amazon S3 bucket and act on your behalf. Or, your organization needs to assume roles for different customers and you require a unique identifier for each customer. The external ID should be a random string generated by the third-party.

To require that the third-party organization provide an external ID when assuming a role, update your role's trust policy with the external ID:
...
To access the bucket, pass the assumed role's ARN along with the external ID, which you can copy from the IAM role's summary page. Alternatively, you can use the [AWS CLI](https://docs.aws.amazon.com/cli/v1/userguide/cli-configure-quickstart.html).

{% include_cached copy-clipboard.html %}
~~~sql
BACKUP DATABASE movr INTO 's3://{bucket name}?AWS_ACCESS_KEY_ID={user key}&AWS_SECRET_ACCESS_KEY={user secret key}&ASSUME_ROLE=arn:aws:iam::{account ID}:role/{role name};external_id={external ID}' AS OF SYSTEM TIME '-10s';
~~~

When [role chaining](#amazon-s3-role-chaining), each role in the chain can be associated with a different external ID:

{% include_cached copy-clipboard.html %}
~~~sql
BACKUP DATABASE movr INTO "s3://{bucket name}?AWS_ACCESS_KEY_ID={user's key}&AWS_SECRET_ACCESS_KEY={user's secret key}&ASSUME_ROLE={role A ARN};external_id={external A ID},{role B ARN};external_id={external B ID},{role C ARN}" AS OF SYSTEM TIME '-10s';
~~~

You can copy the ARN of the role from its summary page in the [AWS Management console](https://aws.amazon.com/console/). For more information on AWS external IDs, see [How to use an external ID](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create_for-user_externalid.html).
