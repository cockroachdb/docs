...
- An argument has a _mode_ and a _type_.
  ...
  - The type can be a built-in type, [user-defined `ENUM`]({% link {{ page.version.version }}/enum.md %}), or implicit record type. A type can have a [`DEFAULT` value]({% link {{ page.version.version }}/default-value.md %}).
