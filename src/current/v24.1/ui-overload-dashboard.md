...
This graph shows the relative time the node had exhausted IO tokens for all IO-bound work per second of wall time, measured in microseconds/second, as tracked by the `admission.granter.io_tokens_exhausted_duration.kv` and `admission.granter.elastic_io_tokens_exhausted_duration.kv` metrics. Increased IO token exhausted duration indicates IO resource exhaustion.
...
This graph specifically shows the number of sub-levels and files in Level 0 normalized by admission thresholds. The 1-normalized float indicates whether IO admission control considers the store as overloaded with respect to compaction out of Level 0 (considers sub-level and file counts).
...
- In the cluster view, the graph shows the Elastic CPU Exhausted Duration in microseconds per second all nodes in the cluster.

## Admission Queueing Delay p99 – Foreground (Regular) CPU

This graph shows the 99th percentile latency of requests waiting in the various Admission Control CPU queues, as tracked by the `admission.wait_durations.kv-p99`, `admission.wait_durations.sql-kv-response-p99`, and `admission.wait_durations.sql-sql-response-p99` metrics.
...
This graph shows the 99th percentile latency of requests waiting in the Admission Control store queue, as tracked by the `admission.wait_durations.kv-stores-p99` and the `admission.wait_durations.elastic-stores-p99` metrics.
...
- In the node view, the graph shows the percentage of CPU Utilization of Elastic CPU Utilization and Elastic CPU Utilization Limit on the selected node.
- In the cluster view, the graph shows the percentage of CPU Utilization of Elastic CPU Utilization and Elastic CPU Utilization Limit across all nodes in the cluster.
