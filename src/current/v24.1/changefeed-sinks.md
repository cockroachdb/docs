...
{{site.data.alerts.callout_info}}
Since CockroachDB v23.2, Google Cloud Pub/Sub v2 is the default when emitting to `gcpubsub` sinks, which provides improved throughput. In Pub/Sub v1, changefeeds with JSON encoders emitted events with the top-level message fields all lowercase. In Pub/Sub v2, the top-level fields are capitalized. For more details, refer to the [Pub/Sub sink messages](#pub-sub-sink-messages) section.
{{site.data.alerts.end}}
...
Google Cloud Pub/Sub v2 is the default when emitting to `gcpubsub` sinks. In Pub/Sub v1, changefeeds with JSON encoders emitted events with the top-level message fields all lowercase:
