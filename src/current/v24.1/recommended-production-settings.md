...
- When choosing on-premises storage, distinguish between these categories:
    ...
    - **SAN-backed storage** is different. If the SAN presents a block device to the operating system and you format it with a recommended Linux filesystem such as `ext4` or `xfs`, it is closer to cloud network-attached block storage than to a distributed file system. SAN-backed storage can be used, but be aware that the additional layers in I/O could break `fsync()` fidelity and lead to undesired behavior.
