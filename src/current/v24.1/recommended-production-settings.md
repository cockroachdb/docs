...
- When choosing on-premises storage, distinguish between these categories:
    - **Distributed file systems** such as CephFS and GlusterFS should not be used for CockroachDB store volumes. CockroachDB already handles [data distribution]({% link {{ page.version.version }}/architecture/distribution-layer.md %}), [replication]({% link {{ page.version.version }}/architecture/replication-layer.md %}), and fault tolerance using [Raft]({% link {{ page.version.version }}/architecture/replication-layer.md %}#raft). Adding a distributed file system underneath creates a second, uncoordinated layer that can cause duplicate replication, higher and more variable latency, and more complex failures.
    ...
    - **SAN-backed storage** is different. If the SAN presents a block device to the operating system and you format it with a recommended Linux filesystem such as `ext4` or `xfs`, it is closer to cloud network-attached block storage than to a distributed file system.
