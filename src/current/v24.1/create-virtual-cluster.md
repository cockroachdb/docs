...
The `CREATE VIRTUAL CLUSTER` statement creates a new virtual cluster. It is supported only starting a [physical cluster replication job]({% link {{ page.version.version }}/set-up-physical-cluster-replication.md %}).
