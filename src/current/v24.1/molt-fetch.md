...
- Review the information in [Best practices](#best-practices).
...
- If a MySQL database is set as a [source](#source-and-target-databases), the `--table-concurrency` and `--export-concurrency` flags **cannot** be set above `1`. If these values are changed, MOLT Fetch returns an error. This is required in order to guarantee consistency when moving data from MySQL, due to MySQL limitations. MySQL data is migrated to CockroachDB one table and one thread at a time, using [`WITH CONSISTENT SNAPSHOT`](https://dev.mysql.com/doc/refman/8.0/en/commit.html) transactions.
...
| `--table-concurrency`                         | Number of tables to move at a time. **Note:** This number will be multiplied by the value of `--export-concurrency`. Ensure your machine has sufficient resources to handle this level of concurrency.<br><br>This value **cannot** be set higher than `1` when moving data from MySQL. See [Best practices](#best-practices).<br><br>**Default:** `4` with a PostgreSQL source; `1` with a MySQL source                                        |
