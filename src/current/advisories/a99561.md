...
WITH
	...
	table_descriptors
		AS (
			SELECT
				id, descriptor->'table' AS descriptor
			FROM
				descriptors
			WHERE
	...
	secondary_indexes
		AS (
			SELECT
				id,
				json_array_elements(descriptor->'indexes')
					AS idx
			FROM
				table_descriptors
		),
	bad_secondary_indexes_store_columns
		AS (
			SELECT
				id,
				idx->'name' AS name,
				json_array_elements(idx->'storeColumnIds')
					AS storecolumnid
			FROM
				secondary_indexes
			WHERE
	...
	bad_secondary_indexes_with_encoding
		AS (
			SELECT
				id, idx->'name' AS name
			FROM
