...
This is resolved in CockroachDB by [#105828](https://github.com/cockroachdb/cockroach/pull/105828), which makes a post-deserialization change to alter the encoding version for secondary indexes that are encoded as primary indexes and are not a mutation. This PR will also add validation to detect tables with this same problem so that they can be fixed.
