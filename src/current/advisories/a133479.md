...
The data would be sent only if the [cluster setting](/docs/v24.2/cluster-settings.md) `diagnostics.reporting.enabled` was set to `true` and `server.redact_sensitive_settings.enabled` was set to `false`. These are the default values for those cluster settings.
...
1. Set the value of the `diagnostics.reporting.enabled` cluster setting to `false` using [`SET CLUSTER SETTING`](/docs/v24.2/set-cluster-setting.md):
...
1. Update either of the following cluster settings using [`SET CLUSTER SETTING`](/docs/v24.2/set-cluster-setting.md):
