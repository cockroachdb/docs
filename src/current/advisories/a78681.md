...
1. The prefix columns are constrained to a set of constant values via the query filter or a [`CHECK` constraint](https://www.cockroachlabs.com/v21.2/check), for example via an `IN` operator. In the case of a `CHECK` constraint, the columns must be `NOT NULL`.
...
Users of CockroachDB are encouraged to upgrade to a maintenance version with the fix applied: [v21.1.17](https://www.cockroachlabs.com/releases/v21.1/#v21-1-17) and [v21.2.8](https://www.cockroachlabs.com/docs/releases/v21.2/#v21-2-8).
