...
Users can run the script included below to detect part of the symptoms of this technical advisory. The script accepts a [debug.zip]({% link {{ page.version.version }}/cockroach-debug-zip.md %}) and determines whether any lease expiration regressions are evident from the logs.
