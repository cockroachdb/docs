...
Versions prior to 23.1 are no longer eligible for [maintenance support]( "releases/release-support-policy.md"), and this issue will not be addressed in those versions.
...
Users can run the script [a131639_check.sh](a131639_check.sh) to determine whether any lease expiration regressions are evident from their logs. The script must be run from within a decompressed [debug.zip](/docs/v24.3/cockroach-debug-zip) directory.
