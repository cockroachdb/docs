...
Cockroach Labs has discovered a bug in which setting the `sql.trace.txn.enable_threshold` [cluster setting](https://www.cockroachlabs.com/v21.1/cluster-settings) to a non-0 value causes nodes to crash loop in CockroachDB v21.1.0 to v21.1.6. The cause of this bug was a misuse of an internal tracing API.
...
The mitigation is to upgrade to CockroachDB v21.1.7+ or to [restore](https://www.cockroachlabs.com/v21.1/restore) from a backup that did not have the cluster setting set to a non-0 value.
