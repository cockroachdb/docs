...
If there is no host at the target IP address, or if a firewall rule blocks traffic to the target address and port, a  while the client network stack waits for a TCP packet in response to network requests.
