...
Example cluster settings in `crdb_internal.cluster_settings.txt` without redaction enabled:
...
`server.identity_map.configuration` is redacted since `sensitive` equals `true`.
...
Cluster settings in `crdb_internal.cluster_settings.txt` with redaction enabled:
...
`server.identity_map.configuration` is still redacted. `cluster.organization` is now redacted since `reportable` equals `false` and the value of `Cockroach Labs Testing` does not equal the default (in this case, the empty string).
...
Then, generate `.zip` with redaction enabled:
