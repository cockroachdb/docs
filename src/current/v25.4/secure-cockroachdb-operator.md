...
Self-signed certificates like those created by the [default TLS configuration](#all-self-signed-certificates) do not auto-rotate. TODO: Give example of how to rotate self-signed (not signed with cockroach cert) certificates

Certificates managed with [cert-manager](https://cert-manager.io/) auto-renew but the CockroachDB pods must be restarted in order to pick up the new certificates.

### Example: Rotate certificates signed with `cockroach cert`

If you previously created and signed certificates with `cockroach cert`, follow these steps to rotate the certificates using the same CA:
