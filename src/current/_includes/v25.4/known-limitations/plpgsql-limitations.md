...
- The following statements are not supported:
	- `FOR` cursor loops, `FOR` query loops, and `FOREACH` loops. #105246
	- `PERFORM`, `EXECUTE`, `GET DIAGNOSTICS`, and `CASE`. #117744
- PL/pgSQL exception blocks cannot catch [transaction retry errors]({% link {{ page.version.version }}/transaction-retry-error-reference.md %}). #111446
...
- `RECORD` parameters and variables are not supported in [user-defined functions]({% link {{ page.version.version }}/user-defined-functions.md %}). #105713
