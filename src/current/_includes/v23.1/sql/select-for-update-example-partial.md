This example assumes you are running a [local unsecured cluster]({{ page.version.version }}/start-a-local-cluster.md).
...
Next, we'll start a [transaction]({{ page.version.version }}/transactions.md) and lock the row we want to operate on:
...
Press **Enter** twice in the [SQL client]({{ page.version.version }}/cockroach-sql.md) to send the statements to be evaluated.  This will result in the following output:
