In a _regional by row_ table, individual rows are optimized for access from different home regions. Every row in a regional by row table has a [column of type `crdb_internal_region`](alter-table.html#crdb_region) that represents the row's home region. The `REGIONAL BY ROW` setting automatically divides a table and all of [its indexes](table-localities.html#indexes-on-regional-by-row-tables) into [partitions](partitioning.html), with each partition optimized for access from a different region. Like [regional tables](table-localities.html#regional-tables), _regional by row_ tables are optimized for access from a single region. However, that region is specified at the row level instead of applying to the whole table.
...
For instructions showing how to set a table's locality to `REGIONAL BY ROW` and configure its home regions, see [`ALTER TABLE ... SET LOCALITY`](alter-table.html#crdb_region).
