...
CPU Time | Average CPU time spent executing within the specified time interval. <br><br> The gray bar indicates mean CPU time. The blue bar indicates one standard deviation from the mean. <br><br>The CPU time includes time spent in the [SQL layer](architecture/sql-layer.html). It does not include time spent in the [storage layer](architecture/storage-layer.html).
...
Regions/Nodes | The region and nodes in which the transaction was executed. <br><br>Nodes are not visible for {{ site.data.products.serverless }} clusters.
