...
1. Use the [`cockroach node decommission`]({{ page.version.version }}/cockroach-node.md) command to decommission the node with the highest number in its address, specifying its ID (in this example, node ID `4` because its address is `cockroachdb-3`):
