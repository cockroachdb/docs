...
`molt` is bundled with the latest `replicator` version available at the time of the MOLT release. This means that the MOLT download always contains the latest version of [MOLT Replicator]({% link molt/molt-replicator.md %}). To verify that the `molt` and `replicator` versions match, run `molt --version` and `replicator --version`.
{{site.data.alerts.end}}
