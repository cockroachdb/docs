To safely remove a node from your cluster, you must first [decommission the node]({% link {{ page.version.version }}/decommission-a-node.md %}) and only then adjust the `spec.replicas` value of your StatefulSet configuration to permanently remove it. This sequence is important because the decommissioning process lets a node finish in-flight requests, rejects any new requests, and transfers all range replicas and range leases off the node.
