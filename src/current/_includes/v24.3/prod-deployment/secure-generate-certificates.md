You can use `cockroach cert` commands, [`openssl` commands]({{ page.version.version }}/create-security-certificates-openssl.md), or [Auto TLS cert generation]({{ page.version.version }}/auto-tls.md) (alpha) to generate security certificates. This section features the `cockroach cert` commands.

Locally, you'll need to [create the following certificates and keys]({{ page.version.version }}/cockroach-cert.md):
...
1. Upload the CA certificate and client certificate and key to the machine where you will run a sample workload:
    ...
    In later steps, you'll also use the `root` user's certificate to run [`cockroach`]({{ page.version.version }}/cockroach-commands.md) client commands from your local machine. If you might also want to run `cockroach` client commands directly on a node (e.g., for local debugging), you'll need to copy the `root` user's certificate and key to that node as well.

{{site.data.alerts.callout_info}}
On accessing the DB Console in a later step, your browser will consider the CockroachDB-created certificate invalid and you’ll need to click through a warning message to get to the UI. You can avoid this issue by [using a certificate issued by a public CA]({{ page.version.version }}/create-security-certificates-custom-ca.md#accessing-the-db-console-for-a-secure-cluster).
{{site.data.alerts.end}}
