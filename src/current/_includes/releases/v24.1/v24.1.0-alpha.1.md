...
- Fixed a bug where statements like `ADD COLUMN j INT, ADD UNIQUE WITHOUT INDEX (j)`, which [add new columns]({% link alter-table.html}#add-column) with unique constraints without creating associated indexes, would fail with an internal error. [#118291][#118291]
- Previously, altering from a [Regional By Row]({% link v23.2/alter-table.md %}#regional-by-row) table to a [Regional By Table]({% link v23.2/alter-table.md %}#regional-by-table) table could cause leaseholders to never move to the database's primary region. This is now fixed. [#118001][#118001]
