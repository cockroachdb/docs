...
- Virtual clusters previously serving as replication sources for [physical cluster replication]({% link v23.2/physical-cluster-replication-overview.md %}) can now be reconfigured as standbys to their promoted clusters, reversing the direction of replication while maximizing data reuse. This is performed with the new syntax [`ALTER VIRTUAL CLUSTER a START REPLICATION FROM a ON x`]({% link v23.2/alter-virtual-cluster.md %}), which rewinds virtual cluster `a` to the timestamp at which `x` was made active and diverged from `a`. [#117656][#117656]
