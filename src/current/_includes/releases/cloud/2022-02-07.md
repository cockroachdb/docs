...
- The [**Terminate Session** and **Terminate Statement**](https://www.cockroachlabs.com/docs/cockroachcloud/sessions-page#sessions-table) options are now enabled for {{ site.data.products.db }} clusters running CockroachDB [v21.2.2]({% link releases/v21.2.md %}#v21-2-2) or later.
...
- Fixed a bug where {{ site.data.products.serverless }} users were seeing occasional dips and spikes in a cluster's [**Request Units**](https://www.cockroachlabs.com/docs/cockroachcloud/cluster-overview-page#request-units) usage graph while running a steady workload.
