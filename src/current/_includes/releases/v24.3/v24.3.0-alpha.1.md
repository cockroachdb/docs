...
- Cluster settings for host based authentication configuration (`server.host_based_authentication.configuration`) and identity map configuration(`server.identity_map.configuration`) need to be redacted as they can  be configured to contain LDAP bind usernames, passwords and mapping of external identities to sql users which are sensitive and should be configurable for redaction via `server.redact_sensitive_settings.enabled` cluster setting. [#131150][#131150]
...
- Added timers around key parts of the changefeed pipeline to help debug feeds experiencing issues. The `changefeed.stage.<stage>.latency` metrics now emit latency histograms for each stage. The metric respects the changefeed `scope` label for debugging specific feeds. [#128794][#128794]
...
- The SHOW SESSIONS command was changed to include an `authentication_method` column in the result. This column will show the method used to authenticate the session, e.g., password, cert, LDAP, etc. [#131625][#131625]
...
- `changefeed_failed` and `create_changefeed` log events now include a `JobId` field for enterprise changefeeds. [#131396][#131396]
- Users may set the log format for the stderr sink by setting the `format` field in the logging config under the `stderr` sink section. [#131529][#131529]
- * Adds connection latency metrics per auth method used * It includes the following new metrics - auth_cert_conn_latency -> Certificate - auth_gss_conn_latency -> Kerberos GSS - auth_jwt_conn_latency -> JWT Token - auth_ldap_conn_latency -> LDAP - auth_password_conn_latency -> Password - auth_scram_conn_latency -> SCRAm SHA 265 [#131578][#131578]
