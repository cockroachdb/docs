...
- Mutation statements like [`UPDATE`](https://www.cockroachlabs.com/docs/v19.1/update), [`INSERT`](https://www.cockroachlabs.com/docs/v19.1/insert), and [`DELETE`](../v19.1/delete.html) no longer attempt to guarantee mutation or output ordering when an `ORDER BY` clause is present. It is now an error to use `ORDER BY` without `LIMIT` with the `UPDATE` statement.
