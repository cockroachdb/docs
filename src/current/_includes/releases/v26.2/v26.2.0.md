 ...
 <tbody>
  ...
  <tr>
   <td>
    ...
    <p class="feature-description"><code>SHOW BACKUPS</code> can now return unique <a href="{% link v26.2/show-backup.md %}#query-backups-more-efficiently">backup IDs</a> which can be used directly in <code>RESTORE</code> operations with <code>FROM {backup_id} IN {collection}</code>, eliminating the need for subdirectory paths or <code>AS OF SYSTEM TIME</code> clauses. Server-side time filtering with <code>NEWER THAN</code> and <code>OLDER THAN</code> makes it easier to find backups within a specific recovery window.</p>
    <p>This functionality is disabled by default, and can be <a href="{% link v26.2/show-backup.md %}#query-backups-more-efficiently">enabled</a> using a session variable.
   </td>
   <td class="icon-center">Preview</td>
   <td class="icon-center">{% include icon-yes.html %}</td>
   <td class="icon-center">{% include icon-no.html %}</td>
