...
- The default value of the
  `sql.guardrails.max_row_size_log` cluster setting has been lowered from
  64 MiB to 16 MiB, and the default value of
  `sql.guardrails.max_row_size_err` has been lowered from 512 MiB to
  80 MiB. These settings control the maximum size of a row (or column
  family) that SQL can write before logging a warning or returning an
  error, respectively. The previous defaults were high enough that large
  rows would hit other limits first (such as the raft command size limit or
  the backup SST size limit), producing confusing errors. The new defaults
  align with existing system limits to provide clearer diagnostics. If your
  workload legitimately writes rows larger than these new defaults, you can
  restore the previous behavior by increasing these settings.
  
  Co-Authored-By: Claude Opus 4.6 <noreply@anthropic.com> [#164468][#164468]
...
- Fixed a bug where generating a debug zip could trigger an out-of-memory (OOM) condition on a node if malformed log entries were present in logs using `json` or `json-compact` formatting. [#163224][#163224]
