...
- When customizing the [SQL shell's interactive prompt](/docs/v23.2/cockroach-sql.md), the special sequence `%M` now expands to the full host name instead of the combination of host name and port number. To include the port number explicitly, use `%>`. The special sequence `%m` now expands to the host name up to the first period. [#105137][#105137]
- The [`cockroach debug zip`](/docs/v23.2/cockroach-debug-zip.md) command stores data retrieved from SQL tables in the remote cluster using the TSV format by default. [#107474][#107474]
- The [`changefeed.protect_timestamp.max_age` cluster setting](/docs/v23.2/protect-changefeed-data.md) will only apply to newly created changefeeds in v23.2. For existing changefeeds, you can set the [`protect_data_from_gc_on_pause`](/docs/v23.2/create-changefeed.md#protect-pause) option so that changefeeds do not experience infinite retries and accumulate protected change data. You can use the [`ALTER CHANGEFEED`](/docs/v23.2/alter-changefeed.md) statement to add `protect_data_from_gc_on_pause` to existing changefeeds. [#103539][#103539]
...
- Users who have the [`CREATEROLE` role option](/docs/v23.2/grant.md) can now grant and revoke role membership in any non-admin role. This change also removes the [`sql.auth.createrole_allows_grant_role_membership.enabled` cluster setting](/docs/v23.2/cluster-settings.md), which was added in v23.1. In v23.2, the cluster setting is effectively always true. [#104376][#104376]

<h3 id="v23-2-0-alpha-1-general-changes">General changes</h3>

- You can now set [Docker](/docs/v23.2/start-a-local-cluster-in-docker-mac.md) command arguments using the `COCKROACH_ARGS` environment variable. [#98899][#98899]
...
- [Spatial libraries](/docs/v23.2/spatial-data-overview.md) for CockroachDB now rely on GEOS 3.11 instead of GEOS 3.8. [#106642][#106642]
- CockroachDB no longer distributes `libgeos` for the experimental [Windows build](/docs/v23.2/install-cockroachdb-windows.md). Users can instead install GEOS directly from the source: <https://libgeos.org/usage/download/>. [#106642][#106642]
...
- CockroachDB now has a new [CLI option](/docs/v23.2/cockroach-commands.md), `--experimental-shared-storage` to rebalance data faster from node to node. [#105839][#105839]
- Fixed a bug where, internally, if we print a 0 decimal with a very low exponent we use excessive memory. This is not possible when using the [DECIMAL](/docs/v23.2/decimal.md) type, but may be possible when using `crdb_internal` functions. [#110527][#110527]

<h3 id="v23-2-0-alpha-1-{{-site.data.products.enterprise-}}-edition-changes">{{ site.data.products.enterprise }} edition changes</h3>

- The [`kafka_sink_config`](/docs/v23.2/advanced-changefeed-configuration.md) `Compression` and `RequiredAcks` options are now case-insensitive. [#100929][#100929]
- [Changefeeds](/docs/v23.2/change-data-capture-overview.md) emit significantly fewer duplicate messages during node and cluster restarts. [#102717][#102717]
...
- The `pgcrypto` [functions](/docs/v23.2/functions-and-operators.md) `encrypt`, `encrypt_iv`, `decrypt`, and `decrypt_iv` are now implemented. These functions require an enterprise license on a CCL distribution. [#105654][#105654]
...
- Fixed the helper message on [UPDATE](/docs/v23.2/update.md) SQL statements to include the optional FROM cause. [#98709][#98709]
- CockroachDB now supports enabling forward [indexes](/docs/v23.2/indexes.md) and ordering on [JSON](/docs/v23.2/jsonb.md) values. [#99275][#99275]
...
- `ALTER INDEX ... VISIBILITY ...` is now supported. It can change an index visibility to any visibility between 0.0 and 1.0. Visibility 0.0 means the index is not visible to the [optimizer](/docs/v23.2/cost-based-optimizer.md), while visibility 1.0 means the index is fully visible. A value in the range between 0.0 and 1.0 means the index will be visible to the corresponding fraction of queries. [#87301][#87301]
...
- CockroachDB now supports [`CREATE INDEX ... VISIBILITY ...`](/docs/v23.2/create-index.md) and [`CREATE TABLE ... (... INDEX (...) VISIBILITY ...)`](/docs/v23.2/create-table.md). This allows users to set the index visibility to any visibility between 0.0 and 1.0. Visibility 0.0 means the index is not visible to the optimizer, while visibility 1.0 means the index is fully visible. A value in the range between 0.0 and 1.0 means the index will be visible to the corresponding fraction of queries. [#101812][#101812]
- [Row level TTL](/docs/v23.2/row-level-ttl.md) now supports `DESC` order primary key columns. [#101869][#101869]
...
- [`SHOW SCHEDULES`](/docs/v23.2/show-schedules.md) now shows the schedule options with which the schedules were created. `SHOW SCHEDULES FOR BACKUP` additionally shows if the schedule is a full or incremental backup schedule. [#102890][#102890]
- You can no longer use `PREPARE` with [`EXPLAIN ANALYZE`](/docs/v23.2/explain-analyze.md) statements. Previously, this was allowed, but attempts to `EXECUTE` the prepared `EXPLAIN ANALYZE` statements would result in an error. [#103259][#103259]
...
- CockroachDB now allows `INSERT` commands in [UDF](/docs/v23.2/user-defined-functions.md) statement bodies. [#102773][#102773]
- CockroachDB now allows `UPDATE` and `UPSERT` commands in UDF statement bodies. [#102773][#102773]
- The `READ COMMITTED` [isolation level](/docs/v23.2/transactions.md#isolation-levels) is now supported. It can be used in the following ways:
...
- Table names are now allowed in `SELECT` lists inside [view](/docs/v23.2/views.md) and UDF definitions. [#104929][#104929]
- [`SHOW JOB WITH EXECUTION DETAILS`](/docs/v23.2/show-jobs.md) for a backup job will regenerate the DistSQL plan diagram with per-node and per-processor progress information. This will help users better understand the state of a running backup job. [#103145][#103145]
...
- The [`SHOW RANGES`](/docs/v23.2/show-ranges.md) command will now emit span statistics when the `DETAILS` option is specified. The statistics are included in a new column named `span_stats`, as a `JSON` object. The statistics are calculated for the identifier of each row. `SHOW RANGES WITH DETAILS` will compute span statistics for each range. `SHOW RANGES WITH TABLES, DETAILS` will compute span statistics for each table, and so on. The `span_stats` `JSON` object has the following keys:
...
- Users now can issue one [`ALTER TABLE` statement](/docs/v23.2/alter-table.md) with a combination of any number of `ADD COLUMN`, any number of `DROP COLUMN`, one `ALTER PRIMARY KEY`, and any number of `ADD CONSTRAINT` clauses. For example, with this PR, we now support statements like:
...
- [`RESTORE`](/docs/v23.2/restore.md) can now be passed a `WITH EXECUTION LOCALITY` option similar to [`BACKUP`](/docs/v23.2/backup.md), to restrict execution of the job to nodes with matching localities. [#104439][#104439]
- Added the `REPLICATION` user role option, which allows a user to use the streaming replication protocol. There is a corresponding [`REPLICATION` system privilege](/docs/v23.2/grant.md). [#106082][#106082]
- A new view-only [session variable](/docs/v23.2/session-variables.md), `max_connections` was added. This can be used with `SHOW` to view the maximum amount of non-superuser SQL connections allowed at a given time. [#106952][#106952]
...
- [`SHOW BACKUP`](/docs/v23.2/show-backup.md)'s timestamp columns are now `TIMESTAMPTZ`, meaning they render in the session offset. [#108290][#108290]
- Attempting to [drop a column](/docs/v23.2/alter-table.md#drop-column) when safe updates are enabled (`sql_safe_updates = on`) now additionally warns users that indexes referencing that column will be automatically dropped. [#108047][#108047]
...
- Added a cluster setting `sql.schema.force_declarative_statements` to enable/disable DDL in the [declarative schema changer](/docs/v23.2/online-schema-changes.md#declarative-schema-changer). [#107815][#107815]
...
- Added the [`VIEWSYSTEMTABLE` system privilege](/docs/v23.2/grant.md). Users with this privilege have `SELECT` privileges for all tables in the system database. [#109474][#109474]
...
- The hash function used by [hash-sharded indexes](/docs/v23.2/hash-sharded-indexes.md) was changed to `mod(fnv32(md5(crdb_internal.datums_to_bytes(columns))), bucket_count)`. Previously, it did not use `md5`. This change was made to enhance the uniformity of bucket distribution in cases when the bucket count is a power of 2, and the columns being sharded have numerical properties that make the `fnv32` function return values with a non-uniformly distributed modulus. [#109374][#109374]
...
- The new [cluster setting](/docs/v23.2/cluster-settings.md) `sql.txn.read_committed_syntax.enabled`, controls whether transactions run under `READ COMMITTED` or `SERIALIZABLE` isolation. It defaults to `false`. When set to `true`, the following statements will configure transactions to run under `READ COMMITTED` isolation:
...
- The [cluster setting](/docs/v23.2/cluster-settings.md) `sql.metrics.statement_details.gateway_node.enabled` now defaults to false, to reduce the number of rows generated in SQL Statistics pages. [#107788][#107788]
...
- The new cluster setting `admission.l0_min_size_per_sub_level` reduces the probability of [admission control](/docs/v23.2/admission-control.md) throttling when there is a sequence of small `memtable` flushes or small files ingested into L0. [#109332][#109332]
...
- [`BACKUP`](/docs/v23.2/backup.md) now skips contacting the ranges for tables on which `exclude_data_from_backup` is set, and can thus succeed even if an excluded table is unavailable. [#108627][#108627]
...
- The default value of `timeout` for `http-servers` [logging sinks](/docs/v23.2/configure-logs.md#configure-log-sinks) has been changed from `0` (i.e., "no timeout") to `2s`. This is reflected in the `http-defaults` section of the log configuration. Users still maintain the ability to override the timeout, or disable it by explicitly setting it to `0` (e.g. `timeout: 0`). [#109264][#109264]
- [Changefeed](/docs/v23.2/change-data-capture-overview.md) metrics now include a `changefeed.checkpoint_progress` metric which is similar to `changefeed.max_behind_nanos` but supports metrics labels, as well as a `changefeed.aggregator_progress` metric which can track the progress of individual aggregators (the lowest timestamp for which all aggregators with the label have emitted all values they're responsible for). [#108757][#108757]
- Added support for Prometheus native histograms behind an environment variable flag. [#104302][#104302]
- Requests for database details or table details from the UI, or usages of [`SHOW RANGES WITH DETAILS`]/docs/v23.2/show-ranges.md are no longer subject to errors if the number of requested spans is too large. [#109464][#109464]
...
- The default value for the `--max-sql-memory` parameter of the [`cockroach demo` command](/docs/v23.2/cockroach-demo.md) has been increased from 128 MiB to 256 MiB. [#103642][#103642]
- The command `\demo recommission` has been removed from [`cockroach demo`](/docs/v23.2/cockroach-demo.md). It had been obsolete and non-functional ever since v20.2. [#108566][#108566]
...
- The [Job Details page](/docs/v23.2/ui-jobs-page.md#job-details) now has a tabbed UI that will allow users to toggle between the Overview and other future views for advanced debugging and observability. [#102737][#102737]
- Renamed "recent executions" to "active executions" in the UI. [#103784][#103784]
- The [Changefeed Dashboard](/docs/v23.2/ui-cdc-dashboard.md) has been updated with new graphs to track backfill progress, protected timestamps age, and the number of schema registry registrations. The updates include renaming the **Sink Byte Traffic** graph to **Emitted Bytes** and the **Max Changefeed Latency** graph to **Max Checkpoint Latency**. [#101790][#101790]
...
- Add columns for p50, p90, p99 percentiles and latency min and max on Explain Plan tab on the [Statement Execution Details page](/docs/v23.2/ui-statements-page.md#statement-execution-details-page). [#107719][#107719]
...
- The DB Console now allows non-admin users to view the [Databases page](/docs/v23.2/ui-databases-page.md). [#109245][#109245]
...
- The "SQL Connection Rate" metric on the [SQL Dashboard](/docs/v23.2/ui-sql-dashboard.md) is downsampled using the MAX function instead of SUM. This improves situations where zooming out would cause the connection rate to increase for downsampled data. [#110391][#110391]


<h3 id="v23-2-0-alpha-1-bug-fixes">Bug fixes</h3>

- Fixed an internal error that can occur when [`CREATE OR REPLACE VIEW`](/docs/v23.2/create-view.md) replaces a view with fewer columns and another entity depended on the view. [#99057][#99057]
...
- Fixed a potential bug whereby a failed or cancelled [IMPORT](/docs/v23.2/import.md) could in some cases leave some of the imported rows behind after it was cancelled, in the rare event that the writing processes were slow enough to continue writing after the cleanup process started. [#97071][#97071]
...
- It is now possible to properly redirect the output of SQL queries using the `ndjson` output table format in [`cockroach sql`](/docs/v23.2/cockroach-sql.md). This bug had been introduced in v22.2. [#102595][#102595]
- The `unaccent` built-in [function](/docs/v23.2/functions-and-operators.md) no longer removes spaces. [#103819][#103819]
...
- Fixed a bug that could cause a [UDF](/docs/v23.2/user-defined-functions.md) to return a value that does not conform to the return type of the UDF. This bug was only present for UDFs that return user-defined types. The bug was present since v23.1. [#104151][#104151]
...
- The [Raft](/docs/v23.2/architecture/replication-layer.md#raft) `PreVote` and `CheckQuorum` mechanisms are now fully enabled. These prevent spurious elections when followers already have an active leader, and cause leaders to step down if they don't hear back from a quorum of followers. This improves reliability under partial and asymmetric network partitions, by avoiding spurious elections and preventing unavailability where a partially partitioned node could steal leadership away from an established leaseholder who would then no longer be able to reach the leader and submit writes. [#104042][#104042]
- Fixed a bug that could produce incorrect values for [virtual computed columns](/docs/v23.2/computed-columns.md) in rare cases. The bug only occurred when the virtual column expression's type did not match the type of the virtual column. [#105736][#105736]
...
- Fixed the `StatementStatistics.Nodes` to contain all of the nodes involved in the query. Fixed the region info in [`EXPLAIN ANALYZE (DISTSQL)`](/docs/v23.2/explain-analyze.md) for virtual clusters. [#106587][#106587]
- Fixed a bug that caused [backups](/docs/v23.2/take-full-and-incremental-backups.md) to fail if there are tables and functions of the same name. [#106626][#106626]
...
- Avoid displaying `undefined` regions on the [Databases page](/docs/v23.2/ui-databases-page.md). [#106778][#106778]
- The [`cockroach userfile upload` command](/docs/v23.2/cockroach-userfile-upload.md) uses less memory when uploading a file. [#106056][#106056]
...
- Fixed a bug that caused internal errors when using an aggregate function in an `ORDER BY` clause of a [`DELETE`](/docs/v23.2/delete.md) or [`UPDATE`](/docs/v23.2/update.md) statement. Aggregate functions are no longer allowed in these contexts. The bug has been present since at least v20.2. [#107641][#107641]
- The filter on the [Statements page](/docs/v23.2/ui-statements-page.md) works when application name is an empty string. [#107750][#107750]
- The [Transaction Details page](/docs/v23.2/ui-transactions-page.md#transaction-details-page) now loads with the fingerprint details even if no application is specified in the URL. [#107742][#107742]
- The [Schema Insights page](/docs/v23.2/ui-insights-page.md#schema-insights-tab) no longer times out. [#107292][#107292]
...
- Fixed a bug where a release [save point](/docs/v23.2/savepoint.md) could incorrectly emit a "cannot publish new versions for descriptors" error instead of a retryable error. [#108133][#108133]
...
- Fixed a bug in [`cockroach demo`](/docs/v23.2/cockroach-demo.md) whereby `\demo add` could sometimes crash with an error "`index out of range [...] with length ...`". This bug had been introduced in v19.x. [#108566][#108566]
...
- Fixed a bug where [`cockroach start`](/docs/v23.2/cockroach-start.md) would sometimes incorrectly hang upon shutting down a server after encountering an internal error. This bug had been introduced some time in v22.x. [#108612][#108612]
- Fixed a bug in the index recommendations provided in the [`EXPLAIN`](/docs/v23.2/explain.md) output where `ALTER INDEX ... VISIBLE` index recommendations may suggest making the wrong index visible when there are multiple invisible indexes in a table. [#108576][#108576]
- Users with the [`VIEWACTIVITY` privilege](/docs/v23.2/grant.md) can now view correct values for timezones. [#108486][#108486]
...
- Fixed an issue on the [Metrics page](/docs/v23.2/ui-overview-dashboard.md) where no metrics would load when viewing metrics for a virtual cluster with a hyphenated name in a global context. [#109174][#109174]
...
- Removed buggy [TTL](/docs/v23.2/row-level-ttl.md) descriptor repair. Previously, upgrading from v22.2.X to v23.1.9 incorrectly removed TTL storage parameters from tables (visible by running a `SHOW CREATE TABLE <ttl-table>;` statement) while attempting to repair table descriptors. This resulted in the node that attempted to run the TTL job crashing due to a panic caused by the missing TTL storage parameters. [#110364][#110364]
...
- Fixed a bug where `CREATE INDEX` for [partial indexes](/docs/v23.2/partial-indexes.md) could fail with `ERROR: duplicate key value violates unique constraint` if concurrent inserts happened simultaneously. [#110216][#110216]
- Observability pages no longer crash when they encounter zeros (e.g., a session with no memory allocated). [#108752][#108752]
- Removed the [cluster setting](/docs/v23.2/cluster-settings.md) `kv.snapshot_recovery.max_rate`:
  - In v23.2, this setting is disabled; it is a no-op. If you previously set `kv.snapshot_recovery.max_rate` on a cluster running v23.1 and upgraded to v23.2, the setting is ignored, and the [`kv.snapshot_rebalance.max_rate`](/docs/v23.2/cluster-settings.md#setting-kv-snapshot-rebalance-max-rate ) setting is used instead.
...
- The [optimizer](/docs/v23.2/cost-based-optimizer.md) now plans inverted index scans for queries using `IN` or the `=` operators without the fetch val (`->`) operator. For example: `json_col = '{"b":"c"}' OR json_col IN ('"a"', '1')` [#101178][#101178]
- Queries that have subqueries in equality expressions are now more efficiently planned by the optimizer. [#100881][#100881]
- Query planning time has been reduced for some queries with multiple [joins](/docs/v23.2/joins.md). [#102011][#102011]
...
- Some large, long-running [`INSERT`](/docs/v23.2/insert.md) statements now perform less work during their commit phase and can run faster. [#103241][#103241]
...
- The optimizer can now avoid a grouping stage in more cases when de-duplicating the input to an [`UPSERT`](/docs/v23.2/upsert.md) or `INSERT ... ON CONFLICT` statement. [#105206][#105206]
...
- [Changefeeds](/docs/v23.2/change-data-capture-overview.md) to Webhook or Pub/Sub endpoints now support much higher throughput [#109351][#109351]
