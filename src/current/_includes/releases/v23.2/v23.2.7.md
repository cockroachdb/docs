...
- [Changefeeds]({% link {{ page.version.version }}/change-data-capture-overview.md %}) now defaults to using the bulk oracle for planning, which distributes work evenly across all replica in the locality filter, including followers if enabled. To disable this behavior and use the previous bin-packing oracle, set the [cluster setting]({% link {{ page.version.version }}/cluster-settings.md %}) `changefeed.balanced_distribution.enabled` to `false`. [#124925][#124925]
- [`ALTER CHANGEFEED`]({% link {{ page.version.version }}/alter-changefeed.md %}) no longer removes the CDC query when [modifying changefeed properties]({% link {{ page.version.version }}/create-and-configure-changefeeds.md %}#modify-a-changefeed). [#125437][#125437]
...
- When the new `optimizer_push_offset_into_index_join` [session setting]({% link {{ page.version.version}}/set-vars.md %}) is enabled, the optimizer attempts to produce more efficient query plans by attempting to push offset expressions into index join expressions to produce more efficient query plans. [#124492][#124492]
...
- Fixed a bug where the `results_buffer_size` [session setting]({% link {{ page.version.version }}/set-vars.md %}) could not be configured using the `options` query parameter in the connection string, but only as a top-level query parameter. This variable cannot be changed by using the `SET` command after the session begins. [#124774][#124774]
- Fixed a bug where dropping a role or user could leave references behind inside types. This in turn could prevent the [`SHOW GRANTS`]({% link {{ page.version.version }}/show-grants.md %}) command from working correctly [#124644][#124644]

- Fixed a bug where the [`ALTER PRIMARY KEY`]({% link {{ page.version.version }}/alter-primary-key.md %}) command could hang for a table if its indexes are referred to by views or functions using the `force syntax` syntax. [#124569][#124569]
- Fixed a bug where the [`SHOW TYPES`]({% link {{ page.version.version }}/show-types.md %}) command omitted user-defined composite types. This bug was introduced in v23.1. [#124816][#124816]
- Fixed a bug where if a column name that contains UTF-8 characters is referenced in the `STORING()` clause of the [`CREATE INDEX`]({% link {{ page.version.version }} %}) command, the [declarative schema changer]({% link {{ page.version.version }}/online-schema-changes.md %}#declarative-schema-changer) cannot detect whether the column is already handled by an existing index. [#125211][#125211]
- Fixed a bug where the [declarative schema changer]({% link {{ page.version.version }}/online-schema-changes.md %}#declarative-schema-changer) erroneously includes virtual columns that are referenced in the `STORING()` clause of the [`CREATE INDEX`]({% link {{ page.version.version }} %}) command. [#125211][#125211]
- Fixed a bug introduced in v20.2, where a change to a user-defined type could cause queries against tables using that type to fail with the error like `histogram.go: span must be fully contained in the bucket`. This bug could occur if the change was from an [`ALTER TABLE`]({% link {{ page.version.version }}/alter-table.md %}) command or from an [`ALTER DATABASE ... ADD REGION`]({% link {{ page.version.version }}/alter-database.md %}) or [`ALTER DATABASE ... DROP REGION`]({% link {{ page.version.version }}/alter-database.md %}) command, which implicitly change the non-public `crdb_internal_region` type. [#124853][#124853]
...
- Fixed a bug where the [declarative schema changer]({% link {{ page.version.version }}/online-schema-changes.md %}#declarative-schema-changer) could erroneously succeed despite a violation of an `ALTER TABLE ... ADD CONSTRAINT UNIQUE` constraint. Now such a violation results in an error message with the error code `42601`. [#125418][#125418]
- Fixed a [changefeed]({% link {{ page.version.version }}/create-and-configure-changefeeds.md %}) panic in v24.1, v23.2, and v23.1 when the [cluster setting]({% link {{ page.version.version }}/cluster-settings.md %}) `changefeed.aggregator.flush_jitter` [cluster setting]({% link {{ page.version.version }}/cluster-settings.md %}) is configured and a changefeed's `min_checkpoint_frequency` option is set to zero. [#125469][#125469]
...
- Fixed a bug introduced in v23.2.0 where inserting rows into a [`REGIONAL BY ROW` table]({% link {{ page.version.version }}/table-localities.md %}#regional-by-row-tables) could cause an internal error if the source was a `VALUES` clause with a single row and at least one boolean expression. [#126208][#126208]

<h3 id="v23-2-7-performance-improvements">Performance improvements</h3>

- The optimizer now generates more efficient query plans for some queries with [`OFFSET`]({% link {{ page.version.version }}/limit-offset.md %}#offset) clauses. [#124492][#124492]
