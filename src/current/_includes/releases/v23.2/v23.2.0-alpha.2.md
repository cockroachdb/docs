...
- The `SIGHUP` signal now clears the cached expiration times for [client certificates](/docs/v23.2/cockroach-cert.md#how-security-certificates-work) that are reported by the `security.certificate.expiration.client` metric. [#110726][#110726]
...
- The `discard` [log message](/docs/v23.2/logging-overview.md) is now limited to once per minute by default. The message now includes both the number of transactions and the number of statements that were discarded. [#110805][#110805]
...
- The [Overload Dashboard page](/docs/v23.2/ui-overload-dashboard.md) now includes the following graphs to monitor [admission control](https://www.cockroachlabs.com/docs/v23.2/admission-control.html):
...
- Fixed a bug where a [`CREATE TABLE`](/docs/v23.2/create-table.md) command with an `IDENTITY` column did not properly propagate the type of the column into the sequence. [#110621][#110621]
...
- Fixed a rare internal error in the `unnest` and `information_schema._pg_expandarray` [built-in functions](/docs/v23.2/functions-and-operators.md) where passed string arguments could be cast to an array. [#110956][#110956]
