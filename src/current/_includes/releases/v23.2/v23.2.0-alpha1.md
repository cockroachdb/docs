...
<h3 id="v23-2-0-alpha1-backward-incompatible-changes">Backward-incompatible changes</h3>

- The pre-v23.1 output produced by `SHOW RANGES`, `crdb_internal.ranges`, `crdb_internal.ranges_no_leases` was deprecated in 23.1, and is now replaced by default with output that's compatible with coalesced ranges (i.e. ranges that pack multiple tables/indexes/partitions into individual ranges). See the [v23.1 release notes]({% link releases/v23.1.md %}) for `SHOW RANGES` for more details. [#102961][#102961]
...
<h3 id="v23-2-0-alpha1-security-updates">Security updates</h3>
...
<h3 id="v23-2-0-alpha1-general-changes">General changes</h3>
...
<h3 id="v23-2-0-alpha1-{{-site.data.products.enterprise-}}-edition-changes">{{ site.data.products.enterprise }} edition changes</h3>
...
<h3 id="v23-2-0-alpha1-sql-language-changes">SQL language changes</h3>
...
- CockroachDD now allows `UPDATE` and `UPSERT` commands in UDF statement bodies. [#102773][#102773]
- The `READ COMMITTED` [isolation level]({% link v23.2/transactions.md %}#isolation-levels) is now supported. It can be used in the following ways: 
  ...
  - Configure it as the default isolation level using the `default_transaction_isolation` session variable. To see the isolation level of the currently running transaction, use either `SHOW TRANSACTION ISOLATION LEVEL` or `SHOW transaction_isolation`. [#103482][#103482]
...
- [`EXPLAIN (DDL) `]({% link v23.2/explain.md %}) statements now have descriptor, index, column, constraint and other ID values decorated with names when available. There is now also a new `EXPLAIN (DDL, SHAPE)` statement which provides information on costly operations planned by the declarative schema changer, like which index backfills and validations will get performed. [#103930][#103930]
...
- Implemented the `pg_lsn - pg_lsn = decimal` builtin function, which subtracts 2 LSNs to return a decimal. [#105326][#105326]
...
- Implemented the spatial builtin `ST_AsMVTGeom`. [#105530][#105530]
...
- Added the `pg_sequence_last_value` builtin function, which returns the last value generated by the sequence. [#106445][#106445]
...
- Added the `nameconcatoid` builtin function, which concatenates a name with an OID. [#105944][#105944]
...
- Added the `gen_random_bytes` builtin function, which generates cryptographically secure random bytes. [#110107][#110107]
...
<h3 id="v23-2-0-alpha1-operational-changes">Operational changes</h3>
...
- Two new store metrics, `range.snapshots.cross-zone.sent-bytes` and `range.snapshots.cross-zone.rcvd-bytes`, were added to track the aggregate of snapshot bytes sent from and received at a store across different zones.  For accurate metrics, follow these assumptions:
...
- Added six new metrics were added to DistSender metrics: `distsender.batch_requests.replica_addressed.bytes`, `distsender.batch_responses.replica_addressed.bytes`, `distsender.batch_requests.cross_region.bytes`, `distsender.batch_responses.cross_region.bytes`, `distsender.batch_requests.cross_zone.bytes`, `distsender.batch_responses.cross_zone.bytes`. For accurate metrics, follow these recommendations:
...
- Added six new metrics to the Node metrics: `batch_requests.bytes`, `batch_responses.bytes`, `batch_requests.cross_region.bytes`, `batch_responses.cross_region.bytes`, `batch_requests.cross_zone.bytes`, `batch_responses.cross_zone.bytes`. For accurate metrics, follow these recommendations:
...
- Added six new metrics for each store: `raft.rcvd.bytes`, `raft.sent.bytes`, `raft.rcvd.cross_region.bytes`, r`aft.sent.cross_region.bytes`, `raft.rcvd.cross_zone.bytes`, `raft.sent.cross_zone.bytes`. For accurate metrics, follow these recommendations:
...
- Added a new cluster setting `server.hot_ranges_request.node.timeout`, with a default value of 5 minutes. The setting controls the maximum amount of time that a hot ranges request will spend waiting for a node to provide a response. Set to 0 to disable timeouts. [#107796][#107796]
...
- The following closed timestamp side-transport settings can no longer be set from secondary tenants (they did not have an effect in secondary tenants): `kv.closed_timestamp.target_duration`, `kv.closed_timestamp.side_transport_interval`, and `kv.closed_timestamp.lead_for_global_reads_override`. [#108678][#108678]
...
- The cluster setting `admission.l0_min_size_per_sub_level` was introduced, which reduces the probability of [admission control]({% link v23.2/admission-control.md %}) throttling when there is a sequence of small memtable flushes or tiny files ingested into L0. [#109332][#109332]
...
- Introduced a new cluster setting `kv.intent_resolver.batcher.in_flight_backpressure_limit.enabled` decides whether an in-flight RPC limit is enforced on intent resolution RPCs. It defaults to `false`. [#109899][#109899]
...
- The [DB Console]({% link v23.2/ui-overview.md %}) now constructs client-side requests using relative URLs instead of absolute ones. This enables proxying of the DB Console at arbitrary subpaths. [#109694][#109694]
- Improve rangefeed observability by adding additional metrics indicating the reason for rangefeed restarts. [#109346][#109346]
- Added two new cluster settings to control whether intent resolution is subject to admission control: `kv.intent_resolver.send_immediately.bypass_admission_control.enabled` and `kv.intent_resolver.batch.bypass_admission_control.enabled`. [#109932][#109932]
...
<h3 id="v23-2-0-alpha1-cluster-virtualization-and-multi-tenancy">Cluster virtualization and multi-tenancy</h3>

- The name of the virtual cluster (tenant), when known, is now reported in logging events. [#108807][#108807]

<h3 id="v23-2-0-alpha1-command-line-changes">Command-line changes</h3>
...
- The SQL shell now supports argument quoting for client-side commands in a similar way to `psql`: inside single quotes, `\` can escape characters and recognize octal/hexadecimal sequences; and inside double quotes characters are passed through. The quote characters themselves, when doubled, result in themselves as part of the string.
    ...
    Note that the quoting rules implemented here are similar to `psql`, but are different from POSIX shells and that of other programming languages like Python or Go. For example, octal and hex escape sequences support a variable number of digits, and double quoted strings preserve the surrounding quotes. When in doubt, refer to the `psql` documentation. [#104610][#104610]
...
- When `cockroach debug zip` is pointed at a multi-tenant cluster or a cluster with virtualization enabled, data about virtual clusters is now stored in a `virtual` sub-directory (this was previously named `tenants`). [#106117][#106117]
...
<h3 id="v23-2-0-alpha1-db-console-changes">DB Console changes</h3>
...
- Added a dropdown on the metrics page dashboards which allows users to view metrics per tenant. The dropdown is only visible on the system tenant and if there are secondary tenants. [#103308][#103308]
...
- Added a tenant dropdown and per tenant checkbox to custom charts. [#103780][#103780]
- The DB Console metrics dashboard now has a "Networking" tab, and a few metrics previously displayed on the Hardware tab have moved there. [#104394][#104394]
- Added a no data empty graph state when switching to a secondary tenant with no data. [#103971][#103971]
...
<h3 id="v23-2-0-alpha1-bug-fixes">Bug fixes</h3>
...
- The `unaccent` builtin [function]({% link v23.2/functions-and-operators.md %}) no longer removes spaces. [#103819][#103819]
...
- Fixed the `StatementStatistics.Nodes` to contain all the nodes involved in the query. Fixed the region info in [`EXPLAIN ANALYZE (DISTSQL)`]({% link v23.2/explain-analyze.md %}) for secondary tenants. [#106587][#106587]
...
- Fixed an issue on the [Metrics page]({% link v23.2/ui-overview-dashboard.md %}) where no metrics would load when viewing the non-system tenant in a global context when the tenant name was hyphenated. [#109174][#109174]
...
- The `difference` builtin had its return type incorrectly set to a string instead of an integer. [#109731][#109731]
...
- Removed the user configurable setting `kv.snapshot_recovery.max_rate`. Guidance to customers has been to always set this equal to `kv.snapshot_rebalance.max_rate` and CockroachDB will use that setting for all snapshots. If you previously set `kv.snapshot_recovery.max_rate` it will be cleared and future attempts to set it will fail with: `ERROR: unknown cluster setting 'kv.snapshot_recovery.max_rate'` [#102596][#102596]
...
<h3 id="v23-2-0-alpha1-performance-improvements">Performance improvements</h3>
...
<h3 id="v23-2-0-alpha1-contributors">Contributors</h3>

This release includes 3116 merged PRs by 122 authors.
We would like to thank the following contributors from the CockroachDB community:

- 9bany (first-time contributor)
- Arjun Sharma (first-time contributor)
- Blake Kostner (first-time contributor)
- David López
- Eric.Yang
- Farye Nwede (first-time contributor)
- Giuseppe Grossi (first-time contributor)
- Josh Carp (first-time contributor)
- Marcin Knychała (first-time contributor)
- ajwerner
- craig (first-time contributor)
- cty123 (first-time contributor)
- e-mbrown
- highpon (first-time contributor)
- rharding6373
- shralex
- zachlite
- 真如赝 (first-time contributor)
