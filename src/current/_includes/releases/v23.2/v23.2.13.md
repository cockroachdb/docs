...
- The description for the [cluster setting]({% link v23.2/cluster-settings.md %}) `changefeed.sink_io_workers` now lists all sinks that support the setting. [#130372][#130372]
...
- The following groups of [metrics]({% link v23.2/metrics.md %}) and [logs]({% link v23.2/logging.md %}) have been renamed to include the buffer they are associated with. The previous metrics are still maintained for backward compatibility.
      ...
      [#131417][#131417]
- Timers have been added to key parts of the [changefeed]({% link v23.2/change-data-capture-overview.md %}) pipeline to help debug problems with feeds. [Metrics]({% link v23.2/metrics.md %}) `changefeed.stage.{stage}.latency` now emit latency histograms for each stage. The changefeed `scope` label allows debugging of a specific changefeed. [#131428][#131428]
...
- The [DB Console]({% link v23.2/ui-overview.md %}) now shows an alert message if the cluster's [enterprise license]({% link v23.2/licensing-faqs.md %}) is expired or will expire in fewer than 15 days, or if the cluster has no enterprise license set. [#130425][#130425]
