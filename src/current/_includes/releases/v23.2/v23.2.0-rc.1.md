...
- The [cluster settings](../v23.2/cluster-settings.html) `storage.single_delete.crash_on_invariant_violation.enabled` and `storage.single_delete.crash_on_ineffectual.enabled` now default to `true` because these can be indicative of a serious bug that could corrupt data. [#116604][#116604]
...
- Updated the **CPU Time** label to **SQL CPU Time** in the [DB Console](../v23.2/ui-runtime-dashboard.html) and added clarification to its tooltip. [#116450][#116450]
- Removed the ID from the event description in the [DB Console](../v23.2/ui-runtime-dashboard.html) when it is `undefined`. [#116519][#116519]
...
- Fixed the formatting for `plpgsql` routines, which could prevent creating a routine with loop labels, and could prevent some expressions from being redacted correctly. The bug only existed in alpha and beta versions of v23.2. [#116711][#116711]
