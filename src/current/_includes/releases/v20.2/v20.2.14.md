...
- The [node status retrieval endpoints over HTTP](https://www.cockroachlabs.com/docs/v20.2/monitoring-and-alerting) [`/_status/nodes`, `/_status/nodes/<N>` and the [DB Console](https://www.cockroachlabs.com/docs/v20.2/ui-overview) `/#/reports/nodes`) now require the [`admin` role](../v20.2/authorization.html) from the requesting user. This ensures that operational details such as network addresses and command-line flags do not leak to unprivileged users. This change means that the [Overview page](https://www.cockroachlabs.com/docs/v20.2/ui-overview) and [Hardware dashboard](../v20.2/ui-hardware-dashboard.html) of the DB Console will not show all details for non-`admin` users. [#67069][#67069]
