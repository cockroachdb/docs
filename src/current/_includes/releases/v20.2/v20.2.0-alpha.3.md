...
- A CockroachDB node started with [`cockroach start`](https://www.cockroachlabs.com/docs/v20.2/cockroach-start) without the `--join` flag no longer automatically initializes the cluster. The `cockroach init` command is now mandatory. The auto-initialization behavior had been deprecated in version 19.2. [#51245][#51245]
...
- Improved `WITH` option parsing for [`RESTORE`](https://www.cockroachlabs.com/docs/v20.2/restore). Does not allow the same option to be specified twice, and also prevents usage of quoted option names. [#52302][#52302]
...
- [`cockroach node recommission`](https://www.cockroachlabs.com/docs/v20.2/remove-nodes#recommission-nodes) has new semantics. Previously it was able to recommission any decommissioning node, regardless of how long ago its was decommissioned, or removed from the cluster. Now `cockroach node recommission` serves to only cancel an accidental inflight decommissioning process that wasn't finalized. [#50329][#50329]
- CPU profiles are now taken as part of the [`cockroach debug zip`](https://www.cockroachlabs.com/docs/v20.2/cockroach-debug-zip) command. [#50625][#50625]
...
- The [CockroachDB tarballs](#v20-2-0-alpha-3-downloads) now include the `libgeos` and `libgeos_c` libraries, which are required to be copied to `/usr/local/lib/cockroach` for UNIX/MacOS systems to [perform certain geospatial operations](https://www.cockroachlabs.com/docs/v20.2/spatial-data). The Windows location will be finalized at a later date. This location is configurable with the `--geo-libs` flag to `cockroach start`. [#51108][#51108]
- The Docker container that ships with CockroachDB now includes the [GEOS](https://www.cockroachlabs.com/docs/v20.2/spatial-glossary#geos) library needed for geospatial functionality in `/usr/local/lib/cockroach` (which is the default location where the `cockroach` binary looks for the GEOS libraries). [#51392][#51392]
- [Timestamp](https://www.cockroachlabs.com/docs/v20.2/timestamp) functions are now supported by [`IMPORT INTO`](https://www.cockroachlabs.com/docs/v20.2/import-into). [#51390][#51390]
- [`IMPORT INTO`](https://www.cockroachlabs.com/docs/v20.2/import-into) now supports `unique_rowid()` as a default expression. [#50922][#50922]
...
- [`RESTORE`](https://www.cockroachlabs.com/docs/v20.2/restore) has a new option `skip_missing_sequence_owners` that must be supplied when restoring only the table/sequence that was previously a sequence owner/owned by a table. Additionally, this fixes a bug where ownership relationships would not be remapped after a `RESTORE`. [#50949][#50949]
- Disabled the use of [`IMPORT`](https://www.cockroachlabs.com/docs/v20.2/import) with user defined types. Users should use [`IMPORT INTO`](https://www.cockroachlabs.com/docs/v20.2/import-into) instead. [#51149][#51149]
...
- The [`BACKUP`](https://www.cockroachlabs.com/docs/v20.2/backup) statement now takes priority over other transactions if its initial attempts to export a range fail. [#51624][#51624]
...
- The [`RESTORE`](https://www.cockroachlabs.com/docs/v20.2/restore) statement can now run in "detached" mode. That is, instead of waiting for the `RESTORE` job to finish, the job ID is returned immediately, and the job itself runs in the background. [#52779][#52779]
- Enabled [`BACKUP`](https://www.cockroachlabs.com/docs/v20.2/backup) and [`RESTORE`](https://www.cockroachlabs.com/docs/v20.2/restore) when user-defined schemas are in use. [#52588][#52588]
...
- Indexes are no longer required on the origin side of a [foreign key relationship](https://www.cockroachlabs.com/docs/v20.2/foreign-key), and are no longer automatically created. [#50771][#50771]
...
- Removed the `SYMVAR` modifier to [`EXPLAIN (PLAN)`](https://www.cockroachlabs.com/docs/v20.2/explain). [#51349][#51349]
- The [vectorized engine](https://www.cockroachlabs.com/docs/v20.2/vectorized-execution) now supports the aggregate function `concat_agg`. [#51148][#51148]
...
- Added a deprecation notice for the use of [`ALTER TABLE .. RENAME`](https://www.cockroachlabs.com/docs/v20.2/alter-table) with a qualified name. [#51433][#51433]
- Disabled using [`ALTER TABLE .. RENAME`](https://www.cockroachlabs.com/docs/v20.2/alter-table) to change the schema of a table. [#51433][#51433]
- Unqualified names in the target of [`ALTER TABLE ... RENAME`](https://www.cockroachlabs.com/docs/v20.2/alter-table) are now treated as having the same database and schema as the table being renamed. [#51433][#51433]
- Interleaved table joins now show up in [`EXPLAIN (PLAN)`](https://www.cockroachlabs.com/docs/v20.2/explain) output. [#51469][#51469]
...
- Made a minor change to remove the outdated CCL-only message from the [`GRANT`](https://www.cockroachlabs.com/docs/v20.2/grant-roles) and [`REVOKE ROLE`](https://www.cockroachlabs.com/docs/v20.2/revoke-roles) help messages. [#51465][#51465]
...
- Added support for the `idle_in_session_timeout` variable to allow automatically terminating [sessions](https://www.cockroachlabs.com/docs/v20.2/show-sessions) that idle past a certain threshold. [#51223][#51223]
...
- Casting [`TIMESTAMP`](https://www.cockroachlabs.com/docs/v20.2/timestamp) types to `TEXT`-related types now omits the timezone component. For example, `'2001-12-15 15:14:13'::TIMESTAMP` will now format as `'2001-12-15 15:14:13'` instead of `'2001-12-15 15:14:13+00:00'`. [#51692][#51692]
- Added the built-in function `crdb_internal.approximate_timestamp` to convert the `DECIMAL` returned from the `crdb_internal_mvcc_timestamp` system column into a [`TIMESTAMP`](https://www.cockroachlabs.com/docs/v20.2/timestamp). [#51662][#51662]
- [`EXPLAIN`](https://www.cockroachlabs.com/docs/v20.2/explain) no longer shows the "hidden" annotation for columns. [#51837][#51837]
...
- Dropping a column always drops all indexes that index the column. It is no longer necessary to provide the `CASCADE` option to [`DROP COLUMN`](https://www.cockroachlabs.com/docs/v20.2/drop-column). [#51661][#51661]
- The [`SHOW CREATE`](https://www.cockroachlabs.com/docs/v20.2/show-create) statement now contains the schema of a table in the `CREATE` statement. [#50889][#50889]
- The [`EXPORT`](https://www.cockroachlabs.com/docs/v20.2/export) statement no longer requires an enterprise license. [#52029][#52029]
...
- Added cosmetic improvements to [`EXPLAIN`](https://www.cockroachlabs.com/docs/v20.2/explain) node naming. [#52195][#52195]
...
- [`EXPLAIN`](https://www.cockroachlabs.com/docs/v20.2/explain) no longer shows grouping columns as "aggregations" for the group operator. [#52232][#52232]
- It's now possible to specify a KMS URI or a list of KMS URIs to encrypt [`BACKUP`](https://www.cockroachlabs.com/docs/v20.2/backup) data/manifests with by passing the `kms` option to `BACKUP`. [#52091][#52091]
- Query statistics now include the disk bytes and rows read from lookup and index [joins](https://www.cockroachlabs.com/docs/v20.2/joins), in addition to ordinary table scans. [#52237][#52237]
...
- Added support for `ALTER TABLE ... ALTER COLUMN ... SET NULL` syntax for [`IMPORT PGDUMP`](https://www.cockroachlabs.com/docs/v20.2/import#import-a-postgres-database-dump) type backups. [#52442][#52442]
- Added a hint for `"line too long"` errors when importing a backup with long lines. [#52432][#52432]
- Added "ownership" concept objects. Objects must now have an owner; all objects that do not have owners currently will have `admin` set as the default owner except system objects. System objects without owners will have `node` as their owner. By default, owners are the creator of the object. Owners have all privileges to the objects they own. Similarly, any [roles](https://www.cockroachlabs.com/docs/v20.2/authorization#create-and-manage-roles) that are members of the owner role also have all privileges on the object. Roles cannot be dropped if they own objects. This PR does not add support for changing the ownership of objects; that will be added in a future PR to support dropping roles. [#51856][#51856]
- When using [`IMPORT PGDUMP`](https://www.cockroachlabs.com/docs/v20.2/import#import-a-postgres-database-dump) with [`INSERT INTO`](https://www.cockroachlabs.com/docs/v20.2/insert) clauses, specifying a column name that is case sensitive (e.g., `"cApItAls"`)  would previously error specifying the column name was not found. This has been fixed. [#52485][#52485]
...
- Extended the [`RESTORE`](https://www.cockroachlabs.com/docs/v20.2/restore) statement to support the 'KMS' option. This can be used to decrypt encrypted [`BACKUP`](https://www.cockroachlabs.com/docs/v20.2/backup)(s). [#52434][#52434]
- Added support for parsing the syntax `CREATE TABLE ... WITH (autovacuum_enabled = bool)`, which results in a no-op. [#52652][#52652]
- Added various improvements to [`EXPLAIN`](https://www.cockroachlabs.com/docs/v20.2/explain). [#52730][#52730]
- Implemented the `ALTER SCHEMA RENAME TO` command. [#52349][#52349]
- The [vectorized execution engine](https://www.cockroachlabs.com/docs/v20.2/vectorized-execution) now fully supports comparison operators (things like `ILIKE`, `IS NOT DISTINCT FROM`, `SIMILAR TO`, and several others). [#52313][#52313]
- The `experimental_follower_read_timestamp()` function has been renamed to `follower_read_timestamp()`, signifying more confidence in CockroachDB's [follower read](https://www.cockroachlabs.com/docs/v20.2/follower-reads) implementation. The previous name remains a supported alias. [#52359][#52359]
...
For instructions showing how to get started with CockroachDB Spatial, see [Working with Spatial Data](https://www.cockroachlabs.com/docs/v20.2/spatial-data).
{{site.data.alerts.end}}
...
- Added support for the `USING GIST` syntax to create a [GIN index](https://www.cockroachlabs.com/docs/v20.2/inverted-indexes) on `GEOMETRY` and `GEOGRAPHY` columns. [#51516][#51516]
...
- Added the `ST_RelatePattern` built-in function, which checks whether a given [DE-9IM intersection matrix](https://wikipedia.org/wiki/DE-9IM) matches a given pattern. [#51858][#51858]
...
- Implemented the `ST_Intersection` function for [`STRING`](https://www.cockroachlabs.com/docs/v20.2/string) types. [#51537][#51537]
- When [ordering by](https://www.cockroachlabs.com/docs/v20.2/order-by) geospatial columns, they will now be ordered by the [Hilbert Space-filling Curve](https://wikipedia.org/wiki/Hilbert_curve) index so that points which are geographically similar are clustered together. [#51898][#51898]
...
- The v20.2 CLI [`cockroach node`](https://www.cockroachlabs.com/docs/v20.2/cockroach-node) family of subcommands will not work with servers running older versions of `cockroach`, but the v20.1 CLI `cockroach node` subcommands will work against v20.2 servers. [#50329][#50329]
- The `is_decommissioning` column found in the output of [`cockroach node decommission`](https://www.cockroachlabs.com/docs/v20.2/remove-nodes) is slated for removal in v20.1. Operators should instead use the new `membership` column to determine node membership status. [#50329][#50329]
- The `--wait` flag to [`cockroach node decommission`](https://www.cockroachlabs.com/docs/v20.2/remove-nodes) now takes any of the following values:
...
- Added Kerberos (GSS) support to [`cockroach sql`](https://www.cockroachlabs.com/docs/v20.2/cockroach-sql) and other CLI commands that only use the SQL protocol (such as [`node ls`](https://www.cockroachlabs.com/docs/v20.2/cockroach-node), or `node status`). Other `cockroach` CLI commands that also use the RPC protocol still cannot use Kerberos (such as `node decommission`, `debug zip`, etc.). [#51570][#51570]
...
- [`cockroach dump`](https://www.cockroachlabs.com/docs/v20.2/cockroach-dump) now supports dumping databases that contain user-defined schemas, as well as accepting schema-qualified table names as arguments. [#52079][#52079]
...
- Crashes in [`cockroach demo`](https://www.cockroachlabs.com/docs/v20.2/cockroach-demo) sessions are now reported to telemetry, [if telemetry is enabled](https://www.cockroachlabs.com/docs/v20.2/diagnostics-reporting). [#52696][#52696]
...
- Added better support for large statement [diagnostic bundles](https://www.cockroachlabs.com/docs/v20.2/explain-analyze#debug-option). [#50974][#50974]
...
- Fixed a bug where very long-running [incremental backups](https://www.cockroachlabs.com/docs/v20.2/backup#incremental-backups) could fail if the data they were backing up was garbage collected. [#51069][#51069]
- Fixed a bug causing the raw trace file collected inside a statement [diagnostics bundle](https://www.cockroachlabs.com/docs/v20.2/explain-analyze#debug-option) to be sometimes empty when the cluster setting `sql.trace.txn.enable_threshold` was in use. [#50914][#50914]
...
- Fixed an internal error involving [`CASE` statements](https://www.cockroachlabs.com/docs/v20.2/scalar-expressions#simple-case-expressions) and boolean expressions with _NULL_ operands. [#51154][#51154]
- [`IMPORT`](https://www.cockroachlabs.com/docs/v20.2/import) no longer fails when run in a database which has a user with privileges on the database. [#51315][#51315]
- [`cockroach dump`](https://www.cockroachlabs.com/docs/v20.2/cockroach-dump) no longer fails with an error when dumping [temporary tables](https://www.cockroachlabs.com/docs/v20.2/temporary-tables), views, or sequences. It either ignores them or throws an informative error if the temporary object is explicitly requested to be dumped via the CLI. [#51185][#51185]
- Fixed incorrect results in some cases involving [joins](https://www.cockroachlabs.com/docs/v20.2/joins) on [interleaved tables](https://www.cockroachlabs.com/docs/v20.2/interleave-in-parent) with [limits](https://www.cockroachlabs.com/docs/v20.2/limit-offset). [#51398][#51398]
- Fixed a bug where [`cockroach dump`](https://www.cockroachlabs.com/docs/v20.2/cockroach-dump) would not properly escape quotes within [table comments](https://www.cockroachlabs.com/docs/v20.2/comment-on). [#51463][#51463]
- Fixed a bug where [`cockroach dump`](https://www.cockroachlabs.com/docs/v20.2/cockroach-dump) would not emit a correct statement for comments on indexes. [#51463][#51463]
...
- Some queries in the [vectorized execution engine](https://www.cockroachlabs.com/docs/v20.2/vectorized-execution) could previously hang during cleanup. This is now fixed. [#51375][#51375]
...
- Updated the [vectorized engine](https://www.cockroachlabs.com/docs/v20.2/vectorized-execution) to show the same statistics for 'rows'/'bytes read' as the row-at-a-time engine. [#51591][#51591]
- Previously, CockroachDB could throw a "command is too large" error when performing [`UPSERT`](https://www.cockroachlabs.com/docs/v20.2/upsert) operations with many values. Internally, we attempted to perform such operations by splitting them into "batches", but the batching mechanism was broken. This is now fixed. [#51608][#51608]
...
- CockroachDB now tracks the location of follower replicas for all ranges much more effectively than before. This means that more queries will be successfully served as ["follower reads"](https://www.cockroachlabs.com/docs/v20.2/follower-reads). [#51437][#51437]
- Before this change, we would leave orphaned `system.namespace`/`system.descriptor` entries if we ran a [`DROP DATABASE CASCADE`](https://www.cockroachlabs.com/docs/v20.2/drop-database) and the database contained "dependency" relations. For example, if the database included a view which depended on a table in the database, dropping the database would result in an orphaned entry for the view. Same thing for a sequence that was used by a table in the database. (See [#51782][#51782] for reproduction steps). This bug is now fixed, and cleanup of entries happens as expected. [#51813][#51813]
- Fixed instances of slow plans for prepared queries involving [CTEs](https://www.cockroachlabs.com/docs/v20.2/common-table-expressions) or [foreign key checks](https://www.cockroachlabs.com/docs/v20.2/foreign-key). [#51788][#51788]
- Fixed the `"unhandled op: json-object-agg"` internal error. [#51881][#51881]
- CockroachDB would previously emit benign internal "context canceled" errors when queries were executed by the [vectorized engine](https://www.cockroachlabs.com/docs/v20.2/vectorized-execution). This is now fixed. [#51772][#51772]
...
- Increased robustness of [`RESTORE`](https://www.cockroachlabs.com/docs/v20.2/restore) against descriptors which may be in an unexpected state. [#51848][#51848]
- Previously, CockroachDB could return incorrect results on query that encountered a [`ReadWithinUncertaintyInterval`](https://www.cockroachlabs.com/docs/v20.2/transaction-retry-error-reference#readwithinuncertaintyinterval) error, and this has been fixed. [#51518][#51518]
- Previously, a [`BACKUP`](https://www.cockroachlabs.com/docs/v20.2/backup) job would block once it had finished backing up the data. This is now fixed. [#51999][#51999]
- Previously, [`RESTORE`](https://www.cockroachlabs.com/docs/v20.2/restore) would sometimes block at the end of the job when sending its results back if the connection that started the job disconnected. This is now fixed. [#51999][#51999]
...
- Fixed an internal error that could occur when an aggregate function argument contained a [correlated subquery](https://www.cockroachlabs.com/docs/v20.2/subqueries#correlated-subqueries) with another aggregate function referencing the outer scope. This now returns an appropriate user-friendly error: `"aggregate function calls cannot be nested"`. [#52092][#52092]
- CockroachDB could previously crash on some queries with [merge joins](https://www.cockroachlabs.com/docs/v20.2/joins#merge-joins); this has now been fixed. [#52042][#52042]
- Resolved an internal error that occurred with 0-argument built-in functions in the [vectorized engine](https://www.cockroachlabs.com/docs/v20.2/vectorized-execution). [#52103][#52103]
- Previously, subtracting months from a [`TIMESTAMP`](https://www.cockroachlabs.com/docs/v20.2/timestamp)/`DATE`/`TIMESTAMPTZ` whose date value was greater than 28 could subtract a further year off. This is now fixed. [#52138][#52138]
...
- Previously, shutting down a connection with an open transaction that had active [temporary tables](https://www.cockroachlabs.com/docs/v20.2/temporary-tables) caused deadlocks or general brokenness. This was because the table cleanup was done inside of the open transaction. This commit moves the table cleanup to be after the user transaction is shut down. [#52148][#52148]
- Fixed a rare bug that could cause actionable [closed timestamps](https://www.cockroachlabs.com/docs/v20.2/follower-reads#how-follower-reads-work) to effectively regress over a given keyspan. This could in turn lead to a serializability violation when using follower reads. This was due to ill-defined interactions between [range merges](https://www.cockroachlabs.com/docs/v20.2/architecture/distribution-layer#range-merges) and the closed timestamp subsystem. [#50265][#50265]
...
- Fixed a bug when using the `COPY` protocol which could prevent [schema changes](https://www.cockroachlabs.com/docs/v20.2/online-schema-changes) for up to 5 minutes. [#52384][#52384]
...
- Updated [`IMPORT PGDUMP`](https://www.cockroachlabs.com/docs/v20.2/import) and `IMPORT MYSQLDUMP` to reject specifying multiple files. [#52500][#52500]
...
- Don't display a value for [`gc.ttlseconds`](https://www.cockroachlabs.com/docs/v20.2/configure-replication-zones) in the [DB Console](https://www.cockroachlabs.com/docs/v20.2/ui-overview) if it is not set. [#52721][#52721]
- Updated CockroachDB's handling of an unknown condition that would previously crash a node with the message `"committed txn with writeTooOld err"`. This condition no longer crashes a node; instead, an error message is printed to [the logs](https://www.cockroachlabs.com/docs/v20.2/debug-and-error-logs) asking for help in the investigation. [#51619][#51619]

<h3 id="v20-2-0-alpha-3-performance-improvements">Performance improvements</h3>

- `SELECT IN` performance on longer filter rows was improved in the [vectorized engine](https://www.cockroachlabs.com/docs/v20.2/vectorized-execution) by replacing a linear scan with binary search. [#50337][#50337]
