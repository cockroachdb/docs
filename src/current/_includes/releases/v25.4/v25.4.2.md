...
<h3 id="v25-4-2-backward-incompatible-changes">Backward-incompatible changes</h3>
...
<h3 id="v25-4-2-security-updates">Security updates</h3>
...
- CockroachDB can now synchronise SQL role membership from the groups claim provided by an OpenID Connect (OIDC) Identity Provider when `server.oidc_authentication.authorization.enabled = true`.

    At login the DB Console gathers the `groups` claim from both the verified ID token and, when available, the access token (if it is a JWT). Any groups found in either token are combined and de-duplicated. If no claim is present in either token, the server will query the provider's `/userinfo` endpoint and extract groups from there (using `server.oidc_authentication.userinfo_group_key`) as a final fallback.

    The resulting list of groups is normalised to SQL role names and compared to the user's current role memberships. Any newly required roles are GRANTed and any stale ones are REVOKEd, matching the behaviour already available for JWT and LDAP-based role synchronisation. [#147706][#147706]
...
<h3 id="v25-4-2-general-changes">General changes</h3>
...
- Added new changefeed metrics to measure the performance of managing protected timestamp records: `changefeed.stage.pts.create.latency`, `changefeed.stage.pts.manage.latency`, and `changefeed.stage.pts.manage_error.latency`. [#148471][#148471]
...
- For Kafka v2 changefeed sinks, detailed error logging is now supported, even for messages exceeding the Kafka v2 size limit. This functionality is gated by the cluster setting `changefeed.kafka_v2_include_error_details`, enabled by default in v25.4, and disabled by default in prior supported versions. [#148753][#148753]
...
- Restore jobs now log errors to
  the job messages table when retrying. [#149821][#149821]
- A warning is now emitted when creating or altering a changefeed with `resolved` or `min_checkpoint_frequency` set below 500ms. This helps users understand the tradeoff between message latency and cluster CPU usage. [#149975][#149975]
...
<h3 id="v25-4-2-{{-site.data.products.enterprise-}}-edition-changes">{{ site.data.products.enterprise }} edition changes</h3>
...
- The SHOW ROLES command now includes a column
  ...
  root@localhost:26257/defaultdb> select * from [show roles] as r WHERE EXISTS (SELECT 1 FROM unnest(r.options) AS m(option) where option like 'SUBJECT=cn%');
    ...
    (1 row)
  ``` [#148532][#148532]
- CockroachDB now automatically provisions users authenticating via JWT when the cluster setting `security.provisioning.jwt.enabled` is set to true. When a non-existent user successfully authenticates with JWT, that user is created in CockroachDB with the `PROVISIONSRC` role option set to `jwt_token:<issuer>`, identifying the token's issuer as the source of the provisioned user. [#149415][#149415]
...
- Fixed a bug where extra quotes or escaped quote characters were added to topic names in changefeeds. This fix can be disabled by setting `feature.changefeed.bare_table_names` to false. [#149438][#149438]
...
- Added the `sql.statements.index_rows_written.count` metric that counts the number of primary and secondary index rows modified by SQL statements. [#156459][#156459]
- Added the `sql.statements.index_bytes_written.count` metric that counts the number of primary and secondary index bytes modified by SQL statements. [#156459][#156459]
...
<h3 id="v25-4-2-operational-changes">Operational changes</h3>
...
- Added four new latency metrics for easier query optimization: `sql.service.latency.historical`, `sql.service.latency.consistent`, `sql.exec.latency.historical`, and `sql.exec.latency.consistent`. [#142826][#142826]
...
- Physical Cluster Replication now automatically destroys the reader tenant on all cutovers. Previously, this only occurred when an explicit cutover time was specified. [#152509][#152509]
...
- Added a new `changefeed` file group that collects changefeed logs to the default logging configuration. [#153381][#153381]
- Removed the
  `bulkio.backup.deprecated_full_backup_with_subdir.enabled` cluster setting. [#153628][#153628]
...
<h3 id="v25-4-2-command-line-changes">Command-line changes</h3>
...
- Added validation on column ids when starting an immediate
...
- Fixed a bug where setting `changefeed.aggregator.flush_jitter` and `min_checkpoint_frequency` such that their product is less than 1 would cause a panic. Jitter is now disabled in this case. [#144304][#144304]
...
- Fixed a bug where the job responsible for compacting stats for the SQL activity state could enter an unschedulable state. Fixes: #155165 [#155963][#155963]
...
- Fixed a bug that could cause prepared statements to fail with the error
  ...
  functions. This bug has been present since v25.4.0. [#159209][#159209]

<h3 id="v25-4-2-performance-improvements">Performance improvements</h3>
...
- Multi-table changefeeds now use separate protected timestamps for each table. This means only tables that are behind in changefeed processing need to retain extra historical data, allowing other tables to be garbage-collected normally. This improves performance and reduces storage. [#149616][#149616]
...
- Fixed an issue where vector indexes created in beta releases had an encoding issue that could result in failed inserts. These indexes should be dropped and re-created before being used with later releases. [#145158][#145158]
- Fixed an issue where vector indexes created in beta releases had an encoding issue that could result in failed inserts. These indexes should be dropped and re-created before being used with later releases. [#145405][#145405]
...
- Restore jobs now re-attempt `AdminSplit` key-value requests
  instead of immediately failing the requests and pausing the jobs. [#148484][#148484]
...
- When using fast restore, the download phase now retries downloads
  before failing. [#148821][#148821]
