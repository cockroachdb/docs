...
- Fix a bug in which PL/pgSQL UDFs with many IF
  ...
  v24.3.9, v25.1.2, v25.2.0. [#162559][#162559]
- Fixed a bug where generating a debug zip could trigger an out-of-memory (OOM) condition on a node if malformed log entries were present in logs using `json` or `json-compact` formatting. [#163352][#163352]
- Fixed a rare data race during parallel constraint
  checks where a fresh descriptor collection could resolve a stale enum
  type version. This data race should only affect 26.1.0. [#163960][#163960]
