...
<h3 id="v22-2-13-{{-site.data.products.enterprise-}}-edition-changes">{{ site.data.products.enterprise }} edition changes</h3>
...
- Previously, flushing SQL stats that resulting in an update statement in mixed version v22.1 and v22.2 clusters would fail due to attempting to update the `index_recommendations` column, which is new in v22.2. [#105664][#105664]
...
- [`SHOW SCHEMAS FROM db_name`](../v22.2/show-schemas.html) will no longer incorrectly show schemas from the current database when the current database has a schema named `db_name`. It will now look up the database name instead of the previous schema look up logic. [#106198][#106198]
...
- This patch fixes an internal error in `UPDATE`, `UPSERT`, `INSERT`, or `DELETE` statements run concurrently with [`ALTER TABLE..ADD COLUMN`](../v22.2/alter-table.html#add-column) of a [virtual computed column](../v22.2/computed-columns.html) on the same table. [#107408][#107408]
...
- This change reduces contention on the `system.statement_statistics` table, which has caused the [SQL statistics compaction job](../v22.2/show-jobs.html#show-automatic-jobs) to fail. [#107794][#107794]
...
- Fixed  `SHOW COMPLETIONS` slice bounds out of range. [#108061][#108061]
