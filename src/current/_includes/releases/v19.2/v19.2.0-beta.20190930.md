...
- **Locality-aware backup and restore**: You can now back up or restore a subset of a table's data according to [zone configurations](https://www.cockroachlabs.com/docs/v19.2/configure-replication-zones) (e.g., send your EU backup partition to an EU cloud storage location).
- [**Import performance improvements**](https://www.cockroachlabs.com/docs/v19.2/import): Internal improvements have been made to `IMPORT` to enable more efficient loading of large data sets.
- **Show partitions using SQL**: CockroachDB now supports the [`SHOW PARTITIONS`](https://www.cockroachlabs.com/docs/v19.2/show-partitions) statement, which can be used to identify partitions. It also enhances [`SHOW CREATE TABLE`](https://www.cockroachlabs.com/docs/v19.2/show-create) to show constraints as described.
...
- **Improve Streaming Behavior on JDBC Driver**: PostgreSQL wire protocol cursors have been implemented. As a result, PGJDBC is able to use streaming behavior to return pagination-related results. Limitations still apply. For more information, see [Build a Java App with CockroachDB](https://www.cockroachlabs.com/docs/v19.2/build-a-java-app-with-cockroachdb#retrieve-large-data-sets-in-chunks-using-cursors).
- [**Improve partitioning user experience**](https://www.cockroachlabs.com/docs/v19.2/partitioning): CockroachDB now prevents users from specifying a non-partitioned secondary index on a partitioned table by default to improve performance. Users will have the ability to override this default as it may be necessary in some rare use cases. Additionally, users can now specify the same name for the partition for tables and for all corresponding indexes.
- [**Improve setting of zone configs via SQL**](https://www.cockroachlabs.com/docs/v19.2/configure-replication-zones): [`ALTER TABLE`](https://www.cockroachlabs.com/docs/v19.2/alter-table) now returns an error when users select conflicting or impossible zone constraints due to user error.
- **Make it easy to focus queries on partitioned data**: CockroachDB now automatically limits a query to only the applicable [partition](https://www.cockroachlabs.com/docs/v19.2/partitioning) without having to specify the particular partition for a geo-distributed table. This prevents the query from conducting a full table scan.
- **Better insight into constraint conformance**: You can now tell if your cluster is in a healthy state based on the status of conformance to constraints like [zone configurations](https://www.cockroachlabs.com/docs/v19.2/configure-replication-zones), lease preferences, and replica diversification.
...
- Added support for locality-aware [backups](https://www.cockroachlabs.com/docs/v19.2/backup) across multiple backup destinations. [#39250][#39250] {% comment %}doc{% endcomment %}
- Added support for displaying creation statements of relations stored in a [backups](https://www.cockroachlabs.com/docs/v19.2/backup). [#39323][#39323] {% comment %}doc{% endcomment %}
- [Table statistics](https://www.cockroachlabs.com/docs/v19.2/create-statistics) for the tables that are being backed up are now also backed up. [#39644][#39644] {% comment %}doc{% endcomment %}
...
- [Zone configurations](https://www.cockroachlabs.com/docs/v19.2/configure-replication-zones) now error out when they conflict with each other. [#39081][#39081] {% comment %}doc{% endcomment %}
- Renamed `SHOW EXPERIMENTAL_RANGES` to [`SHOW RANGES`](https://www.cockroachlabs.com/docs/v19.2/show-ranges) command, as it is no longer an experimental feature. [#39153][#39153] {% comment %}doc{% endcomment %}
...
- [`CREATE VIEW`](https://www.cockroachlabs.com/docs/v19.2/create-view) now supports all queries supported by the optimizer, including those containing [correlated subqueries](https://www.cockroachlabs.com/docs/v19.2/subqueries#correlated-subqueries). [#39169][#39169] {% comment %}doc{% endcomment %}
- Mutations are no longer allowed inside [views](https://www.cockroachlabs.com/docs/v19.2/views). [#39169][#39169] {% comment %}doc{% endcomment %}
- Added the `COLLATION FOR` syntax and `pg_collation_for()` [built-in function](https://www.cockroachlabs.com/docs/v19.2/functions-and-operators). [#39198][#39198] {% comment %}doc{% endcomment %}
...
- The new `vectorize_row_count_threshold` [cluster setting](https://www.cockroachlabs.com/docs/v19.2/cluster-settings) controls when the vectorized execution engine is used. [#39329][#39329] {% comment %}doc{% endcomment %}
- Added a `VEC` option to the [`EXPLAIN`](https://www.cockroachlabs.com/docs/v19.2/explain) command, which shows detailed plan information for vectorized plans. [#39280][#39280] {% comment %}doc{% endcomment %}
...
- Added [`SHOW JOB <jobid>`](https://www.cockroachlabs.com/docs/v19.2/show-jobs), `SHOW JOB <a_expr>`, and `SHOW JOBS <selectclause>`. [#39324][#39324] {% comment %}doc{% endcomment %}
- Added [`SHOW JOB WHEN COMPLETE <a_expr>`](https://www.cockroachlabs.com/docs/v19.2/show-jobs) and `SHOW JOBS WHEN COMPLETE <selectclause>`. [#39457][#39457] {% comment %}doc{% endcomment %}
- [`IMPORT INTO`](https://www.cockroachlabs.com/docs/v19.2/import-into) now cleans up any imported rows if it fails. [#39459][#39459]
- Updated [`SHOW CREATE`](https://www.cockroachlabs.com/docs/v19.2/show-create) to display information about partition and table zone configurations. [#39102][#39102] {% comment %}doc{% endcomment %}
- Explicitly closing portals over the wire after partial use is now supported. [#39639][#39639]
- Added progress tracking for new implementation of [`IMPORT`](https://www.cockroachlabs.com/docs/v19.2/import). [#39646][#39646] {% comment %}doc{% endcomment %}
...
- When [`DROP CONSTRAINT`](https://www.cockroachlabs.com/docs/v19.2/drop-constraint) is executed in a [transaction](https://www.cockroachlabs.com/docs/v19.2/transactions) on a validated constraint, it no longer takes effect immediately, and subsequent writes will now fail if they violate that constraint. This is to enforce consistency when dropping constraints asynchronously in the schema changer. [#39648][#39648]
- The [`ALTER PARTITION`](https://www.cockroachlabs.com/docs/v19.2/configure-zone) statement now supports applying a [zone configuration](https://www.cockroachlabs.com/docs/v19.2/configure-replication-zones) to all the partitions of a table and its indexes that share the same partition name. The syntax for this is `ALTER PARTITION <partition name> OF INDEX <table name>@*`. [#39750][#39750]
- CockroachDB now supports the `FOR UPDATE` modifier on [`SELECT`](https://www.cockroachlabs.com/docs/v19.2/select-clause) statements for compatibility with third-party tools. It has no effect because `SERIALIZABLE` [transactions](https://www.cockroachlabs.com/docs/v19.2/transactions) do not require explicit write locks to maintain consistency. However, this means that `SELECT FOR UPDATE` cannot be used as an advisory exclusive lock in CockroachDB, as used by some tools.[#40206][#40206]
- [`SHOW PARTITIONS`](https://www.cockroachlabs.com/docs/v19.2/show-partitions) now shows the full zone configuration statement used to configure the partition. [#39814][#39814]
- Added the `pg_my_temp_schema()` [built-in](https://www.cockroachlabs.com/docs/v19.2/functions-and-operators) for Postgres compatibility. [#40194][#40194]
- Added the overlaps (`&&`), contains (`@>`), and contained_by (`<@`) operators for [arrays](https://www.cockroachlabs.com/docs/v19.2/array). [#40194][#40194]
- Added `pg_get_function_identity_arguments()` [built-in](https://www.cockroachlabs.com/docs/v19.2/functions-and-operators). [#40194][#40194]
- Added the `width_bucket()` builtin function. [#39263][#39263]
- Added range size information to [`SHOW RANGES`](https://www.cockroachlabs.com/docs/v19.2/show-ranges). [#40281][#40281]
- Added replica locality information to [`SHOW RANGES`](https://www.cockroachlabs.com/docs/v19.2/show-ranges). [#39813][#39813]
- Reordered columns in [`SHOW RANGES`](https://www.cockroachlabs.com/docs/v19.2/show-ranges) output [#40501][#40501]
- [`SHOW CREATE TABLE`](https://www.cockroachlabs.com/docs/v19.2/show-create) now warns users if they have a partitioned table that does not contain zone configurations [#40475][#40475]
...
- [Common table expressions](https://www.cockroachlabs.com/docs/v19.2/common-table-expressions) that are used a single time no longer create an optimization fence. [#40673][#40673]
- [`EXPLAIN`](https://www.cockroachlabs.com/docs/v19.2/explain) now has additional output that shows if a query will run with the vectorized execution engine, and if the query will be distributed across nodes. [#40652][#40652]
- Column references are no longer allowed in `ROWS` and `RANGE` clauses in [window functions](https://www.cockroachlabs.com/docs/v19.2/window-functions). [#40832][#40832]
- Added the `pg_function_is_visible` and `pg_get_function_result` [built-in functions](https://www.cockroachlabs.com/docs/v19.2/functions-and-operators). [#40902][#40902]
...
- `EXPLAIN ANALYSE` can now be used as an alias to [`EXPLAIN ANALYZE`](https://www.cockroachlabs.com/docs/v19.2/explain-analyze). [#41090][#41090]
- [`SHOW PARTITIONS`](https://www.cockroachlabs.com/docs/v19.2/show-partitions) now displays inherited zone configurations. [#40493][#40493]
...
- Added `FOR ROW` clause to [`SHOW RANGES`](https://www.cockroachlabs.com/docs/v19.2/show-ranges) to show range information for a particular row. [#40954][#40954]

<h3 id="v19-2-0-beta-20190930-command-line-changes">Command-line changes</h3>

- [`cockroach demo`](https://www.cockroachlabs.com/docs/v19.2/cockroach-demo) now accepts a `--nodes` flag, which permits customization of the size of the demo cluster. The default remains at `1`. [#39088][#39088] {% comment %}doc{% endcomment %}
- [`cockroach demo`](https://www.cockroachlabs.com/docs/v19.2/cockroach-demo) now accepts a `--demo-locality` flag, which makes it possible to set node localities. [#39454][#39454] {% comment %}doc{% endcomment %}
- [`cockroach demo`](https://www.cockroachlabs.com/docs/v19.2/cockroach-demo) without an argument is now equivalent to `cockroach demo movr`. The previous behavior (no pre-defined dataset) is still available via `cockroach demo --empty`. [#39298][#39298] {% comment %}doc{% endcomment %}
- [`cockroach start`](https://www.cockroachlabs.com/docs/v19.2/cockroach-start) now writes the client URL to the file specified via `--listen-url-file` as soon as the server is ready to accept connections. This also happens before the server detaches from the terminal when `--background` is specified. [#39300][#39300] {% comment %}doc{% endcomment %}
- Using [`cockroach start`](https://www.cockroachlabs.com/docs/v19.2/cockroach-start) without `--join` is now deprecated and this mode of execution will be removed in a later version of CockroachDB. Consider using `cockroach start-single-node` instead or combine `cockroach start` with [`cockroach init`](https://www.cockroachlabs.com/docs/v19.2/cockroach-init). [#28495][#28495] {% comment %}doc{% endcomment %}
- The advisory/informative check performed by [`cockroach start`](https://www.cockroachlabs.com/docs/v19.2/cockroach-start) on the validity of addresses contained in the node certificate is now relaxed to focus on the advertised node address, and to tolerate cases when the cert contains an IP address but a hostname is specified as advertised address. [#39166][#39166]
- Added locality information to the output of [`cockroach node status`](https://www.cockroachlabs.com/docs/v19.2/cockroach-node). [#39059][#39059] {% comment %}doc{% endcomment %}
- Removed the deprecated `cockroach zone` CLI commands. Please use the equivalent [`SHOW ZONE CONFIGURATIONS`](https://www.cockroachlabs.com/docs/v19.2/show-zone-configurations) and `CONFIGURE ZONE` commands in a SQL client. [#39177][#39177] {% comment %}doc{% endcomment %}
- Added a new `cockroach start-single-node` [command](https://www.cockroachlabs.com/docs/v19.2/cockroach-commands) to start single-node clusters with replication disabled. [#28495][#28495] {% comment %}doc{% endcomment %}
- The [`cockroach user`](https://www.cockroachlabs.com/docs/v19.2/authorization) sub-commands are now deprecated. Users and roles can be managed using SQL statements instead. This functionality will be removed in the next release. [#39297][#39297] {% comment %}doc{% endcomment %}
- [`cockroach start`](https://www.cockroachlabs.com/docs/v19.2/cockroach-start) now accepts a `--cluster-name` flag. If specified, it must match when a node connects to a cluster. This feature is meant for use in combination with the cluster ID, which is still checked in any case, to protect newly created nodes (that do not have a cluster ID yet) from joining the wrong cluster. To introduce a cluster name into an existing cluster without one, the new flag can be temporarily paired with `--disable-cluster-name-verification`. [#39270][#39270] {% comment %}doc{% endcomment %}
- [`cockroach start`](https://www.cockroachlabs.com/docs/v19.2/cockroach-start) now accepts a `--sql-addr` [flag](https://www.cockroachlabs.com/docs/v19.2/cockroach-start#flags), which makes it possible to accept connections by clients on a separate TCP address and/or port number from the one used for intra-cluster (node-node) connections. This is aimed to enable firewalling client traffic from server traffic. [#39305][#39305] {% comment %}doc{% endcomment %}
- [`cockroach workload run`](https://www.cockroachlabs.com/docs/v19.2/cockroach-workload) and `cockroach workload init` now support loading initial table data using [`IMPORT`](https://www.cockroachlabs.com/docs/v19.2/import). [#35312][#35312] {% comment %}doc{% endcomment %}
- [`cockroach workload`](https://www.cockroachlabs.com/docs/v19.2/cockroach-workload) now supports additional command-line parameters to customize the output, to facilitate the integration with third-party testing tools:
...
- Added `d`, `dt`, `du`, `l` metacommands to the [cockroach sql](https://www.cockroachlabs.com/docs/v19.2/cockroach-sql) interactive shell. [#39141][#39141] {% comment %}doc{% endcomment %}
- Added default cluster locality topologies for multi-node [`cockroach demo`](https://www.cockroachlabs.com/docs/v19.2/cockroach-demo) clusters. [#40221][#40221]
- Errors in [`cockroach demo`](https://www.cockroachlabs.com/docs/v19.2/cockroach-demo) are no longer logged to the command-line interface. [#40196][#40196]
- Added `d` client-side command with no arguments as an alias for `dt`, for more ease of adoption by `psql` users. [#40309][#40309]
- Added a [MovR](https://www.cockroachlabs.com/docs/v19.2/movr) workload to run with [`cockroach demo`](https://www.cockroachlabs.com/docs/v19.2/cockroach-demo). [#40271][#40271]
- Added location information for default localities to populate Admin UI for [`cockroach demo`](https://www.cockroachlabs.com/docs/v19.2/cockroach-demo). [#40229][#40229]
- [`cockroach demo`](https://www.cockroachlabs.com/docs/v19.2/cockroach-demo) now attempts to contact a license server to obtain a temporary license for demo usage. [#40273][#40273]
- [`cockroach demo`](https://www.cockroachlabs.com/docs/v19.2/cockroach-demo) now enables telemetry for demo clusters. You can opt out of telemetry by setting the `COCKROACH_SKIP_ENABLING_DIAGNOSTIC_REPORTING` environment variable. For more information, see [Diagnostics Reporting](https://www.cockroachlabs.com/docs/v19.2/diagnostics-reporting). [#40273][#40273]
- Deprecated the `--init` flag on the [`workload run`](https://www.cockroachlabs.com/docs/v19.2/cockroach-workload#subcommands) command. Use `workload init` instead. [#40426][#40426]
- Added support for a new `DELIMITED` data format to the [`IMPORT`](https://www.cockroachlabs.com/docs/v19.2/import) statement. [#40482][#40482]
- Added an option for [`cockroach demo`](https://www.cockroachlabs.com/docs/v19.2/cockroach-demo) to automatically apply the geo-partitioned replicas topology to the `movr` dataset using the `--geo-partitioned-replicas` flag. [#40355][#40355]
- [`cockroach sql`](https://www.cockroachlabs.com/docs/v19.2/cockroach-commands) now supports the `--watch` flag. When used to specified a duration, the client will repeat the statement(s) specified by `-e` until an error occurs. This is intended for simple monitoring scenarios during development and testing. [#40594][#40594]
- Added support for redirecting license acquisition to a replacement server (e.g., a suitably configured HTTP proxy), with the environment variable `COCKROACH_DEMO_LICENSE_URL`. This enables [`cockroach demo`](https://www.cockroachlabs.com/docs/v19.2/cockroach-demo) with enterprise features in firewalled network environments. [#41029][#41029]

<h3 id="v19-2-0-beta-20190930-admin-ui-changes">Admin UI changes</h3>

- Metrics about p90 and p99 percentiles of per-application average transaction time are now exposed in the [Admin UI](https://www.cockroachlabs.com/docs/v19.2/admin-ui-access-and-navigate). [#39429][#39429]
...
- `crdb_internal.ranges` can now be used inside [views](https://www.cockroachlabs.com/docs/v19.2/views). Note that such views can become invalid in future releases if `crdb_internal.ranges` changes. [#39195][#39195]
...
- `pg_catalog` now shows correct type lengths for [`INT`](https://www.cockroachlabs.com/docs/v19.2/int) and [`FLOAT`](https://www.cockroachlabs.com/docs/v19.2/float). [#39337][#39337]
...
- [Zone configurations](https://www.cockroachlabs.com/docs/v19.2/configure-replication-zones) now propagate to non-gossiped system tables. [#39638][#39638]
- [`cockroach dump`](https://www.cockroachlabs.com/docs/v19.2/cockroach-dump) now works properly when handling [`INTERLEAVED`](https://www.cockroachlabs.com/docs/v19.2/interleave-in-parent) tables, printing them outside of [`CREATE TABLE`](https://www.cockroachlabs.com/docs/v19.2/create-table) statements. [#39486][#39486]
- Fixed internal errors generated during execution of some complicated cases of [correlated subqueries](https://www.cockroachlabs.com/docs/v19.2/subqueries#correlated-subqueries). [#39818][#39818]
- Using `ORDER BY` inside aggregation functions is now fully supported. [#40242][#40242]
- [Lookup joins](https://www.cockroachlabs.com/docs/v19.2/joins#lookup-joins) now spill to disk if the index lookups return more rows than can be stored in memory. [#40208][#40208]
...
- `width_bucket` [function](https://www.cockroachlabs.com/docs/v19.2/functions-and-operators) no longer errors on 0-length arrays [#40626][#40626]
...
- The [`cockroach` commands](https://www.cockroachlabs.com/docs/v19.2/cockroach-commands) are now able to connect to a server with the environment variable `COCKROACH_URL`. [#40824][#40824]
- Index hints used with [`DELETE`](https://www.cockroachlabs.com/docs/v19.2/delete) and [`UPDATE`](https://www.cockroachlabs.com/docs/v19.2/update) are now taken into consideration. [#40715][#40715]
...
- Fixed panic in [`cockroach workload movr run`](https://www.cockroachlabs.com/docs/v19.2/cockroach-workload). [#41072][#41072]
- Resolved an issue with server hanging on [`cockroach quit`](https://www.cockroachlabs.com/docs/v19.2/cockroach-commands) on Windows. [#40994][#40994]
- The `cockroach zip`, `cockroach node` and `cockroach user` [`cockroach` commands](https://www.cockroachlabs.com/docs/v19.2/cockroach-commands#commands) now work properly when the `defaultdb` database is manually dropped and the connection URL does not specify a database. `cockroach user` is deprecated in 19.2. [#41121][#41121]
- [`cockroach workload init ycsb`](https://www.cockroachlabs.com/docs/v19.2/cockroach-workload#ycsb-workload) now works with `--data-loader=import`. [#41071][#41071]
- Fixed bug where [zone configurations](https://www.cockroachlabs.com/docs/v19.2/configure-replication-zones) on indexes could leak into configurations on [partitions](https://www.cockroachlabs.com/docs/v19.2/partitioning). [#41089][#41089
- Statements inside `[ ... ]` can no longer refer to outer columns or [common table expressions](https://www.cockroachlabs.com/docs/v19.2/common-table-expressions). [#41158][#41158]
- The SQL for non-`SELECT` [common table expressions](https://www.cockroachlabs.com/docs/v19.2/common-table-expressions) now shows up in `EXPLAIN` output. [#41159][#41159]
...
- [`IMPORT`](https://www.cockroachlabs.com/docs/v19.2/import) now writes unsorted data directly, reducing `IMPORT`'s overhead (but potentially increasing RocksDB compaction overhead). [#39487][#39487]
...
- Added an [Oracle migration guide](https://www.cockroachlabs.com/docs/v19.2/migrate-from-oracle), which guides users through the process of converting their schema and then exporting their data so they can load it into CockroachDB via [`IMPORT`](https://www.cockroachlabs.com/docs/v19.2/import). [#5902](https://github.com/cockroachdb/docs/pull/5092)
- Added an [overview of MovR](https://www.cockroachlabs.com/docs/v19.2/movr), CockroachDB's fictional vehicle-sharing dataset and application, and updated several SQL pages and examples to use the built-in MovR dataset, for example, [Learn CockroachDB SQL](https://www.cockroachlabs.com/docs/v19.2/learn-cockroachdb-sql). [#5075](https://github.com/cockroachdb/docs/pull/5075), [#5216](https://github.com/cockroachdb/docs/pull/5216)
- Added a tutorial on [how to use `EXPLAIN` to identify and resolve common SQL performance problems](https://www.cockroachlabs.com/docs/v19.2/learn-cockroachdb-sql). [#5178](https://github.com/cockroachdb/docs/pull/5178)
