...
- Fixed a deadlock that could occur when a statistics creation task panicked. #160419
- Fixed a bug where dropping a trigger on a table with a self-referencing foreign key could cause a missing reference between the objects. #161025
...
- Fixed an error that occurred when using generic
  ...
  computed columns. #162339

<h3 id="v25-2-12-performance-improvements">Performance improvements</h3>

- Added a new session variable, `distsql_prevent_partitioning_soft_limited_scans`, which, when true, prevents scans with soft limits from being planned as multiple TableReaders by the physical planner. This should decrease the initial setup costs of some fully-distributed query plans. #160630

<h3 id="v25-2-12-miscellaneous">Miscellaneous</h3>

- `kv.transaction.write_buffering.enabled` is removed from the public cluster settings to better reflect its preview status. #162027
