...
- DB Console features that check for the `VIEWACTIVITYREDACTED` privilege now also account for [system-level privileges](../v23.1/security-reference/authorization.html#supported-privileges). [#95258][#95258]
...
- Fixed a bug where the `NOSQLLOGIN` [system-level privilege](../v23.1/security-reference/authorization.html#supported-privileges) had a bug that made CockroachDB ignore it entirely, so it had no effect. The bug was introduced in v22.2.0-alpha.1. The `NOSQLLOGIN` [role option](../v23.1/alter-role.html#role-options) is unaffected by this bug. [#96466][#96466]
