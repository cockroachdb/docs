...
- The new [`sql.auth.createrole_allows_grant_role_membership.enabled`](/docs/v23.1/cluster-settings.md#setting-sql-auth-createrole-allows-grant-role-membership-enabled) cluster setting allows a user with the [`CREATEROLE` role option](/docs/v23.1/create-user.md#role-options) to grant and revoke non-admin roles. This cluster setting defaults to `false`, but is expected to become the default behavior in the future. [#104445][#104445]

<h3 id="v23-1-4-{{-site.data.products.enterprise-}}-edition-changes">{{ site.data.products.enterprise }} edition changes</h3>

- Fixed an initialization race condition in [changefeed schema feeds](/docs/v23.1/changefeed-examples.md) that could cause a node to crash with a null pointer exception. [#104934][#104934]

<h3 id="v23-1-4-sql-language-changes">SQL language changes</h3>

- Users with [`MODIFYSQLCLUSTERSETTING`](https://www.cockroachlabs.com/docs/v23.1/security-reference/authorization#supported-privileges) can now view only `sql.defaults` [cluster settings](/docs/v23.1/cluster-settings.md) instead of all cluster settings. [#104542][#104542]

<h3 id="v23-1-4-operational-changes">Operational changes</h3>

- The [`debug.zip`](/docs/v23.1/cockroach-debug-zip.md) archive now contains the files formerly located at `nodes/*/ranges/*.json` in one file per node, `nodes/*/ranges.json`. [#104248][#104248]
- The `http-defaults` and `http-servers` sections of the [log config](/docs/v23.1/configure-logs.md) will now accept a `headers` field containing a map of key-value string pairs which will comprise custom HTTP headers appended to every request. Additionally, a `compression` value can now be set to `gzip` or `none` to select a compression method for the HTTP request body. By default, `gzip` is selected. Previous functionality did not compress by default. [#104814][#104814]

<h3 id="v23-1-4-command-line-changes">Command-line changes</h3>

- The new log config option [`buffering`](/docs/v23.1/configure-logs.md#log-buffering-for-network-sinks) allows you to [format buffer output as JSON arrays](/docs/v23.1/configure-logs.md#file-logging-format). This is useful for APIs that consume JSON arrays, such as the Datadog logs API. [#104790][#104790]
...
- The histogram window merge calculation now interpolates quantile values more accurately, and [**Metrics** charts](/docs/v23.1/ui-overview.md#metrics) in the DB Console are smoother and more accurate. [#104815][#104815]

<h3 id="v23-1-4-bug-fixes">Bug fixes</h3>

- Fixed a bug where a transaction retry could miss job rows during the backfill of the [**Jobs** table](/docs/v23.1/ui-jobs-page.md#jobs-table). [#104757][#104757]
- Fixed a bug where admin or root user privileges were erroneously required to use [`SHOW SYSTEM GRANTS`](/docs/v23.1/show-system-grants.md). [#104732][#104732]
- Fixed a bug that prevented display of the column selector on the [**Jobs** page](/docs/v23.1/ui-jobs-page.md). [#104738][#104738]
