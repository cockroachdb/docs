...
- Google Cloud Pub/Sub sink [changefeeds](https://www.cockroachlabs.com/docs/v23.1/create-changefeed) can now support higher throughputs by enabling the `changefeed.new_pubsub_sink_enabled` [cluster setting](https://www.cockroachlabs.com/docs/v23.1/cluster-settings). Enabling this setting will cause changefeeds to use the v2 Pub/Sub sink, which has capitalized top-level fields in the message: `{Key: ..., Value: ..., Topic: ...}`. The top-level message fields in v1 Pub/Sub are all lowercase: `{key: ..., value: ..., topic: ...}`. You may need to reconfigure downstream systems to parse the new message format. [#100930][#100930]
