...
- Fixed an incorrect results bug where predicates on [computed columns](../v23.1/computed-columns.html) are derived when a predicate that contains an OR condition on a column in the computed column expression is present. This bug only affects CockroachDB when the `optimizer_use_improved_computed_column_filters_derivation` [session setting](../v23.1/set-vars.html) is `true`. It currently defaults to `false` in releases v23.1.2 and v22.1.10. This bug does not affect releases prior to these versions. [#104276][#104276]
