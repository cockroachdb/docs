...
- Added a new `server.client_cert_expiration_cache.capacity` [cluster setting](../v23.1/cluster-settings.html), which when set to a non-zero number will store the minimum time-until-expiration for the set of client certificates per user. This setting can be used to export client certificate expirations as a metric. For example, if set to zero, the `security.certificate.expiration.client` metric will have a value of zero. [#104165][#104165]
...
- Introduced role-based auditing through the new `sql.log.user_audit` [cluster setting](../v23.1/cluster-settings.html). Using the cluster setting, a user is able to set an audit logging configuration using a table-like syntax. Each row in the configuration represents an _audit setting_ in the configuration. An audit setting is comprised of the following columns: `USER/ROLE` and `STATEMENT_FILTER`:
    ...
    In this example, users with the username/role `test_role` will emit audit logs for all statements they issue. Users with the username/role `another_role` will not emit audit logs for any statements they issue. All remaining users will not emit audit logs for any statements they issue.

    To determine whether a statement should be emitted to the audit log, CockroachDB checks whether there is a corresponding audit setting for the statement (i.e., the statement filter must be `ALL`). At the **first** audit setting match (based on role/username), the check iteration will stop. Then the statement is added to a collection of audit events to be logged later. If the statement filter is `NONE`, the check stops. As a result, the order in which the audit settings in the configuration are specified is important, as only the **first matching audit setting** will apply. Note that this is an initial implementation and is subject to change and development. [#104179][#104179]

<h3 id="v23-1-3-sql-language-changes">SQL language changes</h3>

- Fixed [`crdb_internal.reset_sql_stats()`](../v23.1/crdb-internal.html#statement-statistics) to clean up the `sql_activity` table that works as a cache for the statistics. [#104361][#104361]
...
- When the `sql.trace.stmt.enable_threshold` or `sql.trace.txn.enable_threshold` [cluster settings](../v23.1/cluster-settings.html) are enabled, the logging output is now emitted on the [`SQL_EXEC` channel](../v23.1/logging.html#sql_exec). Previously, this was emitted to the [`DEV` channel](../v23.1/logging.html#dev). [#104047][#104047]
- [`SHOW GRANTS`](../v23.1/show-grants.html) now lists roles that inherit privileges granted to each role. `SHOW GRANTS ON ROLE` statements no longer require any privileges and also lists implicit grantees. [#104587][#104587]

<h3 id="v23-1-3-operational-changes">Operational changes</h3>

- Added the [metric](../v23.1/metrics.html) `leases.liveness` showing the number of [liveness range](../v23.1/configure-replication-zones.html#create-a-replication-zone-for-a-system-range) leases per node (generally 1 or 0) to track the liveness range leaseholder. [#104076][#104076]
- Added a gauge metric named `sql.conns_waiting_to_hash`, which counts the number of connection attempts that are being limited in order to constrain the amount of concurrent password hashing operations. This behavior has been present since v21.2 to [prevent password hashing from increasing CPU load](../v23.1/error-handling-and-troubleshooting.html#high-client-cpu-load-connection-pool-exhaustion-or-increased-connection-latency-when-scram-password-based-authentication-is-enabled). The metric is expected to be `0`, or close to `0`, in a healthy setup. If the metric is consistently high and connection latencies are high, then an operator should do one or more of the following:
...
- Added the **Completed time** field on the [**Jobs**](../v23.1/ui-jobs-page.html) and [**Job details**](../v23.1/ui-jobs-page.html#job-details) pages. Also, changed the time format on those pages to include seconds and milliseconds. [#104063][#104063]
...
- The DB Console will no longer show `DROP INDEX` recommendations for unqiue indexes. [#104287][#104287]
...
- Fixed a bug that could cause queries with with [joins](../v23.1/joins.html) or [subqueries](../v23.1/subqueries.html) to omit rows where column values are `NULL` in very rare cases. This bug was present since v20.2. [#103803][#103803]
...
- Fixed an incorrect results bug where predicates on [computed columns](../v23.1/computed-columns.html) are derived when an ORed predicate on a column in the computed column expression is present. This bug only affects CockroachDB when the `optimizer_use_improved_computed_column_filters_derivation` [session setting](../v23.1/set-vars.html) is `true`. It currently defaults to `false` in releases v23.1.2 and v22.1.10. This bug does not affect releases prior to these versions. [#104276][#104276]
...
- Fixed a bug which could cause a panic when a [`CREATE FUNCTION`](../v23.1/create-function.html) statement used the [`setval()` builtin function](../v23.1/functions-and-operators.html). [#104297][#104297]
