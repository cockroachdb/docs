...
A change included in this patch release was discovered to [contain a bug](https://github.com/cockroachdb/cockroach/issues/104798) that can affect clusters upgrading from [v22.2.x](v22.2.html). This can result in any jobs that were running during the upgrade to hang or fail to run after the upgrade is finalized. Users upgrading from v22.2.x are advised to use [v23.1.2](#v23-1-2) to upgrade, or to set the [`cluster.preserve_downgrade_option`](../v23.1/upgrade-cockroach-version.html#step-3-decide-how-the-upgrade-will-be-finalized) cluster setting to prevent finalization of the upgrade until they can do so using v23.1.4.
