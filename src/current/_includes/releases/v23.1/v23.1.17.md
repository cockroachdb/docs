...
- A user with the [`VIEWACTIVITY` privilege](https://www.cockroachlabs.com/docs/v23.1/security-reference/authorization#supported-privileges) can now request a statement bundle from the [**Statements** page](/docs/v23.1/ui-statements-page.md) in the DB Console or with the internal builtin function `crdb_internal.requests_statement_bundle`. [#119638][#119638]
- Clusters using [Cluster Single Sign-on (SSO) with JSON web tokens (JWTs)](/docs/v23.1/sso-sql.md) can now optionally fetch signing keys from configured issuers instead of configuring static signing keys for each issuer. When the new [cluster setting](/docs/v23.1/cluster-settings.md) `server.jwt_authentication.jwks_auto_fetch.enabled` is set to `true`, signing keys are automatically fetched from the issuer using metadata published in its OpenID configuration. In this case, static signing keys in `server.jwt_authentication.jwks` are ignored. When automatic fetching is enabled, there may be a slight increase in network latency for each JWT authentication request, proportional to the latency between the cluster and the issuer's endpoint. [#120063][#120063]
- [DB Console](/docs/v23.1/ui-overview.md) cookies are marked `Secure` for the browser when the cluster is running in secure mode. [#119262][#119262]
- The [DB Console](/docs/v23.1/ui-overview.md) `session` cookie is now marked `HttpOnly` to prevent it from being read by any Javascript code. [#119262][#119262]
...
- Added support for [index hinting](/docs/v23.1/table-expressions.md#force-index-selection) with [`INSERT`](/docs/v23.1/insert.md) and [`UPSERT`](/docs/v23.1/upsert.md) statements. This allows `INSERT ... ON CONFLICT` and `UPSERT` queries to use index hints in the same way as [`UPDATE`](/docs/v23.1/update.md) and [`DELETE`](/docs/v23.1/delete.md) statements. [#119600][#119600]

<h3 id="v23-1-17-operational-changes">Operational changes</h3>

- You can now pass the `--include-range-info` flag when [generating a diagnostic bundle](/docs/v23.1/cockroach-debug-zip.md) to include problem ranges. [#119233][#119233]
...
- The [**Statements** page](/docs/v23.1/ui-statements-page.md) now always shows the entire selected period, instead of only the period that contains data. [#118811][#118811]
- The [**Overload Dashboard**](/docs/v23.1/ui-overload-dashboard.md) page now includes two additional graphs:
...
- Fixed a bug where creating a [changefeed](/docs/v23.1/change-data-capture-overview.md) that targeted tables with a zero-scale [`DECIMAL(n)`](/docs/v23.1/decimal.md) column, `format='avro'`, and `diff` would cause a panic. [#118848][#118848]
- Fixed a bug where a [changefeed](https://www.cockroachlabs.com/docs/v23.1/change-data-capture-overview.html) could omit events in rare cases, logging the error `cdc ux violation: detected timestamp ... that is less or equal to the local frontier`. This could happen in the following scenario:
    ...
    1. This may cause the rangefeed to prematurely emit a checkpoint before emitting writes at lower timestamps, which in turn may cause the [changefeed](/docs/v23.1/change-data-capture-overview.md) to drop these events entirely, never emitting them.

    [#118415][#118415]
- Fixed a bug introduced in v22.2 where queries issued by [TTL jobs](/docs/v23.1/row-level-ttl.md#view-scheduled-ttl-jobs) did not use optimal plans. [#118496][#118496]
...
- Fixed a bug introduced in v22.2 where adding multiple columns with [`UNIQUE` constraints](/docs/v23.1/unique.md) in a single statement could result in the error `secondary index for backfill contains physical column not present in source primary index`. [#118137][#118137]
...
- Fixed a bug where an [`AUTO CREATE STATS` job](/docs/v23.1/show-jobs.md#show-automatic-jobs) could cause an internal system table to grow, and this could cause slow queries related to the job system. [#118944][#118944]
- Fixed a bug that caused a confusing error when a sequence name allocated by [`SERIAL`](/docs/v23.1/serial.md) conflicted with an existing type name. [#118948][#118948]
- Fixed a bug where an [`ALTER PRIMARY KEY`](/docs/v23.1/alter-table.md#alter-primary-key) query could fail with the error `non-nullable column {x} with no value! Index scanned ..` when validating a recreated [secondary index](/docs/v23.1/indexes.md). [#118972][#118972]
...
- Fixed a bug introduced in v23.1.15 where a [rangefeed](/docs/v23.1/create-and-configure-changefeeds.md#enable-rangefeeds)-resolved timestamp could get stuck after a [range merge](https://www.cockroachlabs.com/docs/v23.1/architecture/distribution-layer#range-merges) if the range cache thinks the barrier spans multiple ranges. The log message `pushing old intents failed: range barrier failed, range split` is constantly emitted. [#119543][#119543]
- Fixed a rare panic that could happen during a [`pg_dump` import](/docs/v23.1/import.md) that contains a subquery in one of its arguments, such as a function like `SELECT addgeometrycolumn(...)`. Now, attempting to import a `pg_dump` with a function that has a subquery in one of its arguments results in an error rather than a panic. [#118611][#118611]
- Fixed a bug in the [webhook sink](/docs/v23.1/changefeed-sinks.md#webhook-sink) where the `http` request body may not be initialized on retries, resulting in the error `http: ContentLength=... with Body length 0`. [#119495][#119495]
- Fixed a bug where some files were not closed when inspecting backup metadata during [`BACKUP`](/docs/v23.1/backup.md) and [`RESTORE`](/docs/v23.1/restore.md) operations. [#119634][#119634]
- Fixed a bug that could cause internal errors to be logged when executing an [`EXPORT`](/docs/v23.1/export.md) statement. [#119712][#119712]
- Fixed a bug where a [schema change](/docs/v23.1/online-schema-changes.md) with a large number of descriptors could perform a [full table scan](/docs/v23.1/show-full-table-scans.md) on the `system.leases` table. [#119466][#119466]
