...
- The full set of TLS ciphers that was present in v22.1 have ben included in the old cipher suites list, which can be enabled with the `COCKROACH_TLS_ENABLE_OLD_CIPHER_SUITES` environment variable. [#105370][#105370]
...
- Added columns `default_value` and `origin` (default, override, external-override) to the [`SHOW CLUSTER SETTING`](../v23.1/show-cluster-setting.html) command. [#105452][#105452]
- Added the `VIEWACTIVITY` and `VIEWACTIVITYREDACTED` [system privileges](../v23.1/authorization.html#supported-privileges) for the `crdb_internal.cluster_queries` and `crdb_internal.node_queries` [tables](../v23.1/crdb-internal.html). [#105467][#105467]
...
- The [**Range Report** page](../v23.1/ui-hot-ranges-page#range-report) (route `/reports/range/:rangeID`) shows the **Hot Ranges** menu item as highlighted in the left side menu. The back button in the Range Report page redirects back to the Hot Ranges page. [#104980][#104980]
...
- Fixed a race  condition that can occur when multiple SQLServers are created simultaneously, causing simultaneous writes to an unprotected global variable used to configure a CCL audit logging feature. [#105725][#105725]
...
- If the `sql.optimizer.uniqueness_checks_for_gen_random_uuid.enabled` [cluster setting](../v23.1/cluster-setting.html) is disabled, the optimizer can now eliminate uniqueness checks for `STRING` and `BYTES` columns when the value is set to `gen_random_uuid()` (with an implicit or explicit cast to `STRING` or `BYTES`). If you still want the checks, you can set `sql.optimizer.uniqueness_checks_for_gen_random_uuid.enabled` to `true` (the default is `false`). [#105446][#105446]
