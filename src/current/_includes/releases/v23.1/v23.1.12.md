...
- Fixed a bug where, internally, if we print a 0 decimal with a very low exponent we use excessive memory. This is not possible when using the [DECIMAL](https://www.cockroachlabs.com/docs/v23.1/decimal.md) type, but may be possible when using `crdb_internal` functions. [#110564][#110564]
...
- Fixed a buggy TTL descriptor repair by removing it. Previously, upgrading from v22.2.x to 23.1.9 incorrectly removed [TTL storage parameters](https://www.cockroachlabs.com/docs/v23.1/row-level-ttl#ttl-storage-parameters) from tables (visible via [`SHOW CREATE TABLE  <ttl-table>`](https://www.cockroachlabs.com/docs/v23.1/show-create)) while attempting to repair table descriptors. This resulted in the node that attempts to run the TTL [job](https://www.cockroachlabs.com/docs/v23.1/show-jobs.html) crashing due to a panic caused by the missing TTL storage parameters. Clusters currently on v22.2.x should **not** be upgraded to v23.1.9 and should be upgraded directly to v23.1.10 or later. For more information, see [Technical Advisory 110363](../advisories/a110363.html). [#110500][#110500]
- Fixed a bug causing performance regression when disabling `sql.metrics.statement_details.enabled` which caused execution stats to be collected for all queries instead of the default one percent. [#109881][#109881]
...
- The [**Database** pages](https://www.cockroachlabs.com/docs/cockroachcloud/databases-page) will no longer display `undefined` regions and outdated node information. [#110741][#110741]
- Fixed a bug where `RESET (ttl_expire_after)` could incorrectly remove `ttl_expiration_expression`. [#110746][#110746]
- Fixed a bug where an [`ALTER TABLE ... ADD CONSTRAINT CHECK ...`](https://www.cockroachlabs.com/docs/v23.1/alter-table#add-constraint) statement that utilized a user-defined function in the `CHECK` could cause a validation error. [#110720][#110720]
...
- Fixed a bug that could occur when the "multiple active portals" execution mode (Preview) was enabled to evaluate queries such as lookup joins. The bug could result in an internal error like `unexpected 40960 leftover bytes` if the portal was not fully consumed. [#110666][#110666]
...
- Added a check for values before using `mean` on the [Plan Details page](https://www.cockroachlabs.com/docs/v23.1/ui-statements-page), fixing a crash. [#111505][#111505]
...
- Removed buggy [TTL](https://www.cockroachlabs.com/docs/v23.1/row-level-ttl) descriptor repair. Previously, upgrading from v22.2.X to v23.1.9 incorrectly removed TTL storage parameters from tables (visible by running a `SHOW CREATE TABLE <ttl-table>;` statement) while attempting to repair table descriptors. This resulted in the node that attempted to run the TTL job crashing due to a panic caused by the missing TTL storage parameters. [#111600][#111600]
...
- Fixed a bug in [transaction insight details](https://www.cockroachlabs.com/docs/v23.1/ui-insights-page) where it was possible to see the contention details of other transactions. Now, CockroachDB will only surface contention details for the current transaction. [#111880][#111880]
- Fixed a bug where `indoption` inside `pg_index` was not properly encoded causing clients to be unable to decode it as `int2vector`. [#111957][#111957]
...
- Fixed an issue where the [optimizer](https://www.cockroachlabs.com/docs/v23.1/cost-based-optimizer) fails to honor the `statement_timeout` session setting when generating constrained index scans for queries with large `IN` lists or `= ANY` predicates on multiple index key columns, which may lead to an out of memory condition on the node. [#112076][#112076]
...
- Fixed an issue where lookup and index joins on tables with at least 3 column families were sometimes incorrectly evaluated, leading to "non-nullable column with no value" errors or incorrect query results. The bug was introduced in 22.2. [#113107][#113107]
