...
1. Run the [`cockroach start`]({{ page.version.version }}/cockroach-start.md) command:
    ...
    When deploying across multiple datacenters, or when there is otherwise high latency between nodes, it is recommended to set `--locality` as well. It is also required to use certain enterprise features. For more details, see [Locality]({{ page.version.version }}/cockroach-start.md#locality).
...
1. In the sample configuration template, specify values for the following flags:
    ...
    For other flags not explicitly set, the command uses default values. For example, the node stores data in `--store=cockroach-data` and binds DB Console HTTP requests to `--http-port=8080`. To set these options manually, see [Start a Node]({{ page.version.version }}/cockroach-start.md).
