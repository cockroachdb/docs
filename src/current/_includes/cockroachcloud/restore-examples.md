...
`BACKUP ... INTO` adds a backup to a [collection]({{site.current_cloud_version}}/take-full-and-incremental-backups.md#backup-collections) within the backup destination. The path to the backup is created using a date-based naming scheme. To view the backup paths in a given destination, use [`SHOW BACKUPS`]({{site.current_cloud_version}}/restore.md#view-the-backup-subdirectories):
...
To view the available subdirectories to restore a backup from, use [`SHOW BACKUPS`]({{site.current_cloud_version}}/restore.md#view-the-backup-subdirectories).
