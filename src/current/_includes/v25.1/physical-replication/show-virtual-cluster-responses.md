...
`replication_lag` | The time between the most up-to-date replicated time and the actual time. Refer to the [Technical Overview]({{ page.version.version }}/physical-cluster-replication-technical-overview.md) for more detail.
`failover_time` | The time at which the failover will begin. This can be in the past or the future. Refer to [Fail over to a point in time]({{ page.version.version }}/failover-replication.md#fail-over-to-a-point-in-time).
