...
{% assign all_versions = site.data.versions | where_exp: "versions", "released_versions contains versions.major_version" | sort: "release_date" | reverse %}
{% assign initial_versions = all_versions | slice: 0, 2 %}
{% assign remaining_versions = all_versions | slice: 3, all_versions.size %}
{% comment %} Fetch all major versions (e.g., v21.2), sorted in reverse chronological order. {% endcomment %}

{% assign all_releases = site.data.releases | sort: "release_date" | reverse %}
{% assign latest_version = all_releases | map: "major_version" | first %}
{% assign all_platforms = "linux,mac,windows,docker,source" | split: "," %}
...
.load-more-btn {
  ...
  will-change: background-color; /* Optimize animations */
}
...
{% for v in all_versions %} {% comment %} Iterate through all major versions {% endcomment %}
<div class="version-section {% if forloop.index > 1 %}hidden{% endif %}" data-version="{{ version.major_version }}">
    {% comment %}
...
{% if all_versions.size > 2 %}
  <button id="loadMoreBtn" class="load-more-btn">Load More Versions</button>
{% endif %}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const elements = {
    ...
    versionSections: document.querySelectorAll('.version-section'),
  ...
  document.getElementById('os-tabs')?.addEventListener('click', (e) => {
    ...
    elements.filterContents.forEach(content => 
      content.style.display = content.dataset.scope === scope ? 'block' : 'none');
  });

  // Optimize version loading
  elements.loadMoreBtn?.addEventListener('click', () => {
    const {visibleVersions, versionsPerLoad} = state;
    const nextBatch = Array.from(elements.versionSections)
      .slice(visibleVersions, visibleVersions + versionsPerLoad);
    
    nextBatch.forEach(section => section.classList.remove('hidden'));
    
    state.visibleVersions += versionsPerLoad;
    
    if (state.visibleVersions >= elements.versionSections.length) {
      elements.loadMoreBtn.classList.add('hidden');
    }
  });

  // Set initial state
  const defaultButton = elements.filterButtons[0];
  if (defaultButton) {
    defaultButton.classList.add('active');
    const defaultScope = defaultButton.dataset.scope;
    elements.filterContents.forEach(content => 
      content.style.display = content.dataset.scope === defaultScope ? 'block' : 'none');
  }
});
</script>