...
You can omit the `INNER INVERTED JOIN` statement by putting `v1.ext` on the left side of a `@>` join condition in a `WHERE` clause and using an [index hint]({{ page.version.version }}/table-expressions.md#force-index-selection) for the GIN index.
