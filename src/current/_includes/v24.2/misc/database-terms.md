...
### Isolation
The degree to which a transaction may be affected by other transactions running at the same time. CockroachDB provides the [`SERIALIZABLE`](https://wikipedia.org/wiki/Serializability) and `READ COMMITTED` isolation levels. For more information, see [Isolation levels]({{ page.version.version }}/transactions.md#isolation-levels).
...
<a name="architecture-overview-contention"></a> A [state of conflict]({{ page.version.version }}/performance-best-practices-overview.md#transaction-contention) that occurs when: 
...
Steps should be taken to [reduce transaction contention]({{ page.version.version }}/performance-best-practices-overview.md#reduce-transaction-contention) in the first place.
