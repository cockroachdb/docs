...
Now that you've learned about our architecture, [start up a {{ site.data.products.serverless }} cluster]({% link cockroachcloud/quickstart.md %}) or [local cluster]({% link {{ page.version.version }}/install-cockroachdb.md %}) and start [building an app with CockroachDB](https://www.cockroachlabs.com/docs/example-apps).
