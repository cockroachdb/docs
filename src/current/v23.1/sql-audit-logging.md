...
It consists of using the [`ALTER TABLE ... EXPERIMENTAL_AUDIT`](alter-table.html#experimental_audit) command to enable the [`SENSITIVE_ACCESS`](logging.html#sensitive_access) logging channel per table.
...
We turn on auditing for a table using the [`EXPERIMENTAL_AUDIT`](alter-table.html#experimental_audit) subcommand of [`ALTER TABLE`](alter-table.html).
