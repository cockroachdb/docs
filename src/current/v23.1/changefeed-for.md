...
<a name="format"></a>`format` | `json` / `avro` / `csv` / `parquet` | Format of the emitted message. <br><br>`avro`: `confluent_schema_registry` is required with this format. Support for [Avro is limited](#avro-limitations). <br><br>`csv`: [`initial_scan=only`](#initial-scan) is required with this format.<br><br>Default: `format=json`.
