...
In the [example deployment]({% link {{ page.version.version }}/movr-flask-deployment.md %}), the application and the database deployments are separate and not co-located on the same machine.
...
We cover setting up an external load balancer in [Deploy a Global, Serverless Application]({% link {{ page.version.version }}/movr-flask-deployment.md %}).

## Next steps

You should now be ready to start [creating a multi-region database schema]({% link {{ page.version.version }}/movr-flask-database.md %}).
