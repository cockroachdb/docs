...
`table stats mode` | Whether the execution plan for the statement was determined using [canary or stable statistics]({% link {{ page.version.version }}/canary-statistics.md %}). This field is only displayed when canary statistics are enabled.
...
`canary window` | The amount of time new statistics for the table remain in [canary status]({% link {{ page.version.version }}/canary-statistics.md %}#the-canary-window) before being promoted to stable status. This field is only displayed when canary statistics are enabled.                             
