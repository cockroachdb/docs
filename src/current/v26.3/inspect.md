...
`INDEX ({index_name} [, ...])` | Run checks only for the specified indexes. On supported [`REGIONAL BY ROW`]({% link {{ page.version.version }}/alter-table.md %}#regional-by-row) tables, naming the primary index can also trigger a uniqueness check. Note that `INDEX ALL` and this option are mutually exclusive.
...
- `INSPECT` runs secondary-index consistency checks on supported secondary indexes. On supported [`REGIONAL BY ROW`]({% link {{ page.version.version }}/alter-table.md %}#regional-by-row) tables, it can also run uniqueness checks on the primary index to detect duplicate unique values across regions.
...
On supported [`REGIONAL BY ROW`]({% link {{ page.version.version }}/alter-table.md %}#regional-by-row) tables, `INSPECT` will run uniqueness checks on the primary index to detect duplicate unique values across regions. This will also run any other checks supported by `INSPECT`:
