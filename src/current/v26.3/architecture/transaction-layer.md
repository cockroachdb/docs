...
Buffered writes are enabled by default for [explicit transactions]({% link {{ page.version.version }}/run-multi-statement-transactions.md %}) that use [`SERIALIZABLE`]({% link {{ page.version.version }}/demo-serializable.md %}) isolation. [Implicit transactions]({% link {{ page.version.version }}/transactions.md %}#individual-statements) and transactions that use weaker isolation levels do not use buffered writes by default. To enable buffered writes for implicit transactions in the [current session]({% link {{ page.version.version }}/session-variables.md %}), issue the following statement:
