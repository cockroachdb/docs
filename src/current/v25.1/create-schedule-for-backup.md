...
---
 ...
 The `CREATE SCHEDULE FOR BACKUP` [statement]({{ page.version.version }}/sql-statements.md) creates a schedule for periodic [backups]({{ page.version.version }}/backup.md).

For more information about creating, managing, monitoring, and restoring from a scheduled backup, see [Manage a Backup Schedule]({{ page.version.version }}/manage-a-backup-schedule.md).
...
`table_pattern`                         | The [table(s)]({{ page.version.version }}/create-table.md) or [view(s)]({{ page.version.version }}/views.md) you want to back up.
`database_name`                         | The name of the [database(s)]({{ page.version.version }}/create-database.md) you want to back up (i.e., create backups of all tables and views in the database).
...
For schedules that include both [full and incremental backups]({{ page.version.version }}/take-full-and-incremental-backups.md), CockroachDB will create two schedules (one for each type). See [Incremental backup schedules](#incremental-backup-schedules) for more information.
...
- Ensure you are monitoring your backup schedule (e.g., [Prometheus]({{ page.version.version }}/monitor-cockroachdb-with-prometheus.md)) and alerting metrics that will confirm that your backups are completing, but also that they're not running more concurrently than you expect.
...
- See the [Backup and Restore Monitoring]({{ page.version.version }}/backup-and-restore-monitoring.md) page for a general overview and list of metrics available for backup, scheduled backup, and restore jobs.
- See [Set up monitoring for the backup schedule]({{ page.version.version }}/manage-a-backup-schedule.md#set-up-monitoring-for-the-backup-schedule) for metrics and monitoring backup schedules specifically.
...
-----------------------+-----------------
View the schedule      | [`SHOW SCHEDULES`]({{ page.version.version }}/show-schedules.md)
Pause the schedule     | [`PAUSE SCHEDULES`]({{ page.version.version }}/pause-schedules.md)
Resume the schedule    | [`RESUME SCHEDULES`]({{ page.version.version }}/resume-schedules.md)
Drop the schedule      | [`DROP SCHEDULES`]({{ page.version.version }}/drop-schedules.md)
Alter the schedule     | [`ALTER BACKUP SCHEDULE`]({{ page.version.version }}/alter-backup-schedule.md)
...
- [`ALTER BACKUP SCHEDULE`]({{ page.version.version }}/alter-backup-schedule.md)
