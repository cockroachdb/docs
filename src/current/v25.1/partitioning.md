...
To store partitions in specific locations (e.g., geo-partitioning), or on machines with specific attributes (e.g., archival-partitioning), the nodes of your cluster must be [started]({{ page.version.version }}/cockroach-start.md) with the relevant flags:
...
For more details about these flags, see the [`cockroach start`]({{ page.version.version }}/cockroach-start.md) documentation.

### Table creation

You can define partitions and subpartitions over one or more columns of a table. During [table creation]({{ page.version.version }}/create-table.md), you declare which values belong to each partition in one of two ways:
...
[`PARTITION BY LIST`]({{ page.version.version }}/alter-table.md#partition-by) lets you map one or more tuples to a partition.

To partition a table by list, use the [`PARTITION BY LIST`]({{ page.version.version }}/alter-table.md#partition-by) syntax while creating the table. While defining a list partition, you can also set the `DEFAULT` partition that acts as a catch-all if none of the rows match the requirements for the defined partitions.

See [Partition by List](#define-table-partitions-by-list) example below for more details.

#### Partition by range

[`PARTITION BY RANGE`]({{ page.version.version }}/alter-table.md#partition-by) lets you map ranges of tuples to a partition.

To define a table partition by range, use the [`PARTITION BY RANGE`]({{ page.version.version }}/alter-table.md#partition-by) syntax while creating the table.  While defining a range partition, you can use CockroachDB-defined `MINVALUE` and `MAXVALUE` parameters to define the lower and upper bounds of the ranges respectively.
...
To ensure uniqueness or fast lookups, create a [secondary index]({{ page.version.version }}/indexes.md) on the identifier.
...
If a partitioned table contains a [foreign key reference]({{ page.version.version }}/foreign-key.md) to a non-partitioned table, the secondary index created automatically for the foreign key reference will not be partitioned. This can impact performance when querying against the partitioned table, as the data may exist in a distant node.

To minimize potential latency issues, configure the non-partitioned table to be a [`GLOBAL` table]({{ page.version.version }}/global-tables.md).

### Replication zones

On their own, partitions are inert and simply apply a label to the rows of the table that satisfy the criteria of the defined partitions. Applying functionality to a partition requires creating and applying [replication zone]({{ page.version.version }}/configure-replication-zones.md) to the corresponding partitions.
...
See [Set the Trial or Enterprise License Key]({{ page.version.version }}/licensing-faqs.md#set-a-license).
...
To create replication zone and apply them to corresponding partitions, use the [`ALTER PARTITION ... CONFIGURE ZONE`]({{ page.version.version }}/alter-partition.md#create-a-replication-zone-for-a-partition) statement:
...
1. After creating these replication zones, you can view them using the [`SHOW ZONE CONFIGURATION`]({{ page.version.version }}/show-zone-configurations.md) statement:
...
To retrieve table partitions, you can use the [`SHOW PARTITIONS`]({{ page.version.version }}/show-partitions.md) statement:
...
You can also view partitions by [database]({{ page.version.version }}/show-partitions.md#show-partitions-by-database) and [index]({{ page.version.version }}/show-partitions.md#show-partitions-by-index).
...
To set the Enterprise license, see [Set the Trial or Enterprise License Key]({{ page.version.version }}/licensing-faqs.md#set-a-license).
...
To create zone configurations and apply them to corresponding partitions, use the [`ALTER PARTITION ... CONFIGURE ZONE`]({{ page.version.version }}/alter-partition.md#create-a-replication-zone-for-a-partition) statement:
...
Consider the partitioned table of students of RoachLearn. Suppose the table has been partitioned on range to store the current students on fast and expensive storage devices (example: SSD) and store the data of the graduated students on slower, cheaper storage devices(example: HDD). Now suppose we want to change the date after which the students will be considered current to `2018-08-15`. We can achieve this by using the [`PARTITION BY`]({{ page.version.version }}/alter-table.md#partition-by) subcommand of the [`ALTER TABLE`]({{ page.version.version }}/alter-table.md) command.
...
You can remove the partitions on a table by using the [`PARTITION BY NOTHING`]({{ page.version.version }}/alter-table.md#partition-by) syntax:
...
To view the replication zone for a partition, use the [`SHOW ZONE CONFIGURATION`]({{ page.version.version }}/show-zone-configurations.md) statement.
