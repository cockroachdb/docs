...
It consists of using the [`ALTER TABLE ... EXPERIMENTAL_AUDIT`]({{ page.version.version }}/alter-table.md#experimental_audit) command to enable the [`SENSITIVE_ACCESS`]({{ page.version.version }}/logging.md#sensitive_access) logging channel per table.
...
For the best visibility into security-related events on your cluster, we recommend configuring `SENSITIVE_ACCESS` together with the `USER_ADMIN`, `PRIVILEGES`, and `SESSIONS` logging channels. To learn more, see [Logging Use Cases]({{ page.version.version }}/logging-use-cases.md#security-and-audit-monitoring).
...
To ensure [non-repudiation](https://wikipedia.org/wiki/Non-repudiation) in audit logs, we recommend [enabling `auditable`]({{ page.version.version }}/configure-logs.md#configure-log-sinks) for the `SENSITIVE_ACCESS` channel. CockroachDB will then synchronously log all of the activity of every user on a cluster in a way that is durable to system failures. Note that every query that causes a logging event must access the disk of the node on which audit logging is enabled. As a result, enabling `auditable` on a logging channel negatively impacts performance, and we recommend using this setting for security purposes only.

For debugging and troubleshooting on production clusters, the most performant way to log all queries is to enable the `SQL_EXEC` logging channel. For details, see [Logging Use Cases]({{ page.version.version }}/logging-use-cases.md#sql_exec).
...
We turn on auditing for a table using the [`EXPERIMENTAL_AUDIT`]({{ page.version.version }}/alter-table.md#experimental_audit) subcommand of [`ALTER TABLE`]({{ page.version.version }}/alter-table.md).

~~~ sql
> ALTER TABLE customers EXPERIMENTAL_AUDIT SET READ WRITE;
~~~

This directs [SQL audit events]({{ page.version.version }}/eventlog.md#sql-access-audit-events) for the `customers` table into the [`SENSITIVE_ACCESS`]({{ page.version.version }}/logging.md#logging-channels) logging channel.
...
By [default]({{ page.version.version }}/configure-logs.md#default-logging-configuration), events in the `SENSITIVE_ACCESS` channel are output to a log file that is prefixed `cockroach-sql-audit` and stored in the [same directory]({{ page.version.version }}/configure-logs.md#logging-directory) as the other logs generated by CockroachDB.

To store the audit log files in a specific directory, [configure the `SENSITIVE_ACCESS` channel]({{ page.version.version }}/configure-logs.md#output-to-files) with a custom `dir` path. Like the other log files, it's rotated according to the [`max-file-size` setting]({{ page.version.version }}/configure-logs.md#set-file-defaults).
...
{{site.data.alerts.callout_info}}
The above example shows the default [`crdb-v2`]({{ page.version.version }}/log-formats.md#format-crdb-v2) log format. This can be changed to another format (e.g., JSON). For details, see [Configure Logs]({{ page.version.version }}/configure-logs.md#file-logging-format).
{{site.data.alerts.end}}
...
Let's populate the `orders` table with some placeholder data using [`CREATE SEQUENCE`]({{ page.version.version }}/create-sequence.md):

~~~ sql
> CREATE SEQUENCE product_ids_asc START 1 INCREMENT 1;
~~~

Evaluate the below a few times to generate data; note that this would error if [`SELECT`]({{ page.version.version }}/select-clause.md) returned multiple results, but it doesn't in this case.
...
- [Role-based SQL Audit Logging]({{ page.version.version }}/role-based-audit-logging.md)
- [`ALTER TABLE ... EXPERIMENTAL_AUDIT`]({{ page.version.version }}/alter-table.md#experimental_audit)
- [`cockroach start` logging flags]({{ page.version.version }}/cockroach-start.md#logging)
- [Logging Use Cases]({{ page.version.version }}/logging-use-cases.md)
- [SQL FAQ - generating unique row IDs]({{ page.version.version }}/sql-faqs.md#how-do-i-auto-generate-unique-row-ids-in-cockroachdb)
