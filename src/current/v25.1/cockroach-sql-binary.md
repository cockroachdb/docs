...
`cockroach-sql` is functionally equivalent to the [`cockroach sql` command]({{ page.version.version }}/cockroach-sql.md).
...
1. Visit [Releases](releases/index.md) and download the SQL Shell binary for CockroachDB.

<section class="filter-content" markdown="1" data-scope="linux">
1. Follow the instructions to [install CockroachDB]({{ page.version.version }}/install-cockroachdb.md) on your local system. The resulting `cockroach` binary supports only `cockroach sql` subcommands.
</section>
...
1. Follow the instructions to [install CockroachDB]({{ page.version.version }}/install-cockroachdb.md) on your local system. The resulting `cockroach.exe` binary supports only `cockroach sql` subcommands.
</section>

## Before you begin

- The [role option of the user]({{ page.version.version }}/security-reference/authorization.md#role-options) logging in must be `LOGIN` or `SQLLOGIN`, which are granted by default. If the user has been set to use the `NOLOGIN` role or the `NOSQLLOGIN` [system privilege]({{ page.version.version }}/security-reference/authorization.md#supported-privileges) (or the legacy `NOSQLLOGIN` role option), the user cannot log in using the SQL CLI with any authentication method.
...
- When the client and server versions are different, the shell prints both the `Client version` and `Server version`. In this case, you may want to [plan an upgrade]({{ page.version.version }}/upgrade-cockroach-version.md) of earlier client or server versions.
