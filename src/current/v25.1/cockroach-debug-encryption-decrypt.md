...
We strongly recommend only using `cockroach debug encryption-decrypt` when working directly with the [Cockroach Labs support team]({{ page.version.version }}/support-resources.md).
{{site.data.alerts.end}}

The [`cockroach debug encryption-decrypt`]({{ page.version.version }}/cockroach-commands.md) command decrypts store files on a node that are [encrypted at rest]({{ page.version.version }}/encryption.md) so that they can be examined to add more context to logs gathered from the [`cockroach debug zip`]({{ page.version.version }}/cockroach-debug-zip.md) command. Unless they are decrypted, Cockroach Labs cannot examine store files that are encrypted at rest.
...
- `{encryption_spec}`: The cluster's [encryption details]({{ page.version.version }}/encryption.md#starting-a-node-with-encryption).
- `{store_directory}`: The directory where the node [stores data files]({{ page.version.version }}/cockroach-start.md#store).
