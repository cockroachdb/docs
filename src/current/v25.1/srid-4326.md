...
Every geometric shape has a spatial reference system associated with it, and each such reference system has a Spatial Reference System ID ([SRID]({{ page.version.version }}/architecture/glossary.md#srid)).  The SRID is used to tell which spatial reference system will be used to interpret each spatial object.

A common SRID in use is 4326, which represents spatial data using longitude and latitude coordinates on the Earth's surface as defined in the [WGS84]({{ page.version.version }}/architecture/glossary.md#wgs84) standard, which is also used for the [Global Positioning System (GPS)](https://wikipedia.org/wiki/Global_Positioning_System).
...
To add an SRID to a shape you are defining with [Well Known Text]({{ page.version.version }}/well-known-text.md), prepend the text with `SRID=N;` as shown below:
...
To add an SRID to a shape in SQL, use the `ST_SetSRID` [function]({{ page.version.version }}/functions-and-operators.md#spatial-functions):
...
When setting a shape's SRID, you can only use SRIDs that are defined in the [`spatial_ref_sys`]({{ page.version.version }}/architecture/glossary.md#spatial_ref_sys) table.  For more information, see [Getting SRID information](#getting-srid-information).
{{site.data.alerts.end}}

## Getting SRID information

You can get more detailed information about the SRIDs supported in CockroachDB from the [SQL client]({{ page.version.version }}/cockroach-sql.md) by querying the [`spatial_ref_sys`]({{ page.version.version }}/architecture/glossary.md#spatial_ref_sys) table.
