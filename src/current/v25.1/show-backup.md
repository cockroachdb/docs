...
The `SHOW BACKUP` [statement]({{ page.version.version }}/sql-statements.md) lists the contents of a backup created with the [`BACKUP`]({{ page.version.version }}/backup.md) statement.
...
----------|------------
`SHOW BACKUPS IN collectionURI` | List the backup paths in the given [collection URI]({{ page.version.version }}/backup.md#backup-file-urls). [See the example](#view-a-list-of-the-available-full-backup-subdirectories).
`SHOW BACKUP FROM subdirectory IN collectionURI` | Show the details of backups in the subdirectory at the given [collection URI]({{ page.version.version }}/backup.md#backup-file-urls). Also, use `FROM LATEST in collectionURI` to show the most recent backup. [See the example](#show-the-most-recent-backup).
`SHOW BACKUP SCHEMAS FROM subdirectory IN collectionURI` | Show the schema details of the backup in the given [collection URI]({{ page.version.version }}/backup.md#backup-file-urls). [See the example](#show-a-backup-with-schemas).
`collectionURI` | The URI for the [backup storage]({{ page.version.version }}/use-cloud-storage.md). <br>Note that `SHOW BACKUP` does not support listing backups if the [`nodelocal`]({{ page.version.version }}/cockroach-nodelocal-upload.md) storage location is a symlink. Cockroach Labs recommends using remote storage for backups.
...
`encryption_passphrase`<a name="with-encryption-passphrase"></a> | [`STRING`]({{ page.version.version }}/string.md) |  The passphrase used to [encrypt the files]({{ page.version.version }}/take-and-restore-encrypted-backups.md) that the `BACKUP` statement generates (the data files and its manifest, containing the backup's metadata).
`kms`                                                            | [`STRING`]({{ page.version.version }}/string.md) |  The URI of the cryptographic key stored in a key management service (KMS), or a comma-separated list of key URIs, used to [take and restore encrypted backups]({{ page.version.version }}/take-and-restore-encrypted-backups.md#examples). Refer to [URI Formats]({{ page.version.version }}/take-and-restore-encrypted-backups.md#uri-formats).
`incremental_location` | [`STRING`]({{ page.version.version }}/string.md) | [List the details of an incremental backup](#show-a-backup-taken-with-the-incremental-location-option) taken with the [`incremental_location` option]({{ page.version.version }}/backup.md#incr-location).
...
------|-------|------------
`database_name` | [`STRING`]({{ page.version.version }}/string.md) | The database name.
`parent_schema_name` | [`STRING`]({{ page.version.version }}/string.md) | The name of the parent schema.
`object_name` | [`STRING`]({{ page.version.version }}/string.md) | The name of the [database]({{ page.version.version }}/create-database.md), [table]({{ page.version.version }}/create-table.md), [type]({{ page.version.version }}/create-type.md), or schema.
`object_type` | [`STRING`]({{ page.version.version }}/string.md) | The type of object: [database]({{ page.version.version }}/create-database.md), [table]({{ page.version.version }}/create-table.md), [type]({{ page.version.version }}/create-type.md), or schema.
`backup_type` | [`STRING`]({{ page.version.version }}/string.md) | The type of backup: [full]({{ page.version.version }}/take-full-and-incremental-backups.md#full-backups) or [incremental]({{ page.version.version }}/take-full-and-incremental-backups.md#incremental-backups).
`start_time` | [`TIMESTAMPTZ`]({{ page.version.version }}/timestamp.md) | The time of the earliest data encapsulated in the backup. Note that this only displays for incremental backups. For a full backup, this is `NULL`.
`end_time` | [`TIMESTAMPTZ`]({{ page.version.version }}/timestamp.md) | The time to which data can be restored. This is equivalent to the [`AS OF SYSTEM TIME`]({{ page.version.version }}/as-of-system-time.md) of the backup. If the backup was _not_ taken with [revision history]({{ page.version.version }}/take-backups-with-revision-history-and-restore-from-a-point-in-time.md), the `end_time` is the _only_ time the data can be restored to. If the backup was taken with revision history, the `end_time` is the latest time the data can be restored to.
`size_bytes` | [`INT`]({{ page.version.version }}/int.md) | The size of the backup objects, in bytes. Note that `size_bytes` indicates the logical size of the table objects, which is computed as the sum of the size of each key value pair. See `file_bytes` in this table for more detail.
`rows` | [`INT`]({{ page.version.version }}/int.md) | Number of rows in tables that are part of the backup.
`create_statement` | [`STRING`]({{ page.version.version }}/string.md) | The `CREATE` statement used to create [table(s)]({{ page.version.version }}/create-table.md), [view(s)]({{ page.version.version }}/create-view.md), or [sequence(s)]({{ page.version.version }}/create-sequence.md) that are stored within the backup. This displays when `SHOW BACKUP SCHEMAS` is used. Note that tables with references to [foreign keys]({{ page.version.version }}/foreign-key.md) will only display foreign key constraints if the table to which the constraint relates to is also included in the backup.
`is_full_cluster` |  [`BOOL`]({{ page.version.version }}/bool.md) | Whether the backup is of a full cluster or not.
`regions` | [`STRING`]({{ page.version.version }}/string.md) | The [`ALTER DATABASE`]({{ page.version.version }}/alter-database.md) statement(s) used to configure the database with [multi-region zone configurations]({{ page.version.version }}/multiregion-overview.md#database-regions), if they exist. If the database is a part of a single region cluster configuration, `NULL` will show.
`file_bytes` | [`INT`]({{ page.version.version }}/int.md) | (With the `check_files` option only) The estimated bytes in external storage for a particular table object. This is the physical bytes that a given table object is taking up. For example, when the files are written to disk in storage they could be compressed. If you total all file bytes, the result is the physical bytes in your storage location. Note that for smaller tables the byte size in `file_bytes` may be larger than `size_bytes` because of the overhead required to create an SST file.
`path` | [`STRING`]({{ page.version.version }}/string.md) | The list of the [full backup]({{ page.version.version }}/take-full-and-incremental-backups.md#full-backups)'s subdirectories. This field is returned for `SHOW BACKUPS IN collectionURI` only. The path format is `<year>/<month>/<day>-<timestamp>`.
...
<a name="show-backups-in"></a>To view a list of the available [full backups]({{ page.version.version }}/take-full-and-incremental-backups.md#full-backups) subdirectories, use the following command:
...
To view a list of the [full]({{ page.version.version }}/take-full-and-incremental-backups.md#full-backups) and [incremental]({{ page.version.version }}/take-full-and-incremental-backups.md#incremental-backups) backups in a specific subdirectory, use the following command:
...
### Show details for scheduled backups
 ...
 When a [backup is created by a schedule]({{ page.version.version }}/create-schedule-for-backup.md), it is stored within a collection of backups in the given collection URI. To view details for a backup created by a schedule, you can use the following:
...
Depending on how the backup was [encrypted]({{ page.version.version }}/take-and-restore-encrypted-backups.md), use the [`encryption_passphrase` option]({{ page.version.version }}/backup.md#with-encryption-passphrase) and the same passphrase that was used to create the backup:
...
For more information on validating a backup, see the [Backup Validation]({{ page.version.version }}/backup-validation.md) page.
...
The response will include a `manifest` column with the file's contents as the JSON value. Use [JSONB functions]({{ page.version.version }}/functions-and-operators.md#jsonb-functions) to query particular data or edit the format of the response.
...
For example, to return a specific entry from the JSON response as a [`string`]({{ page.version.version }}/string.md) indented and with newlines use the [`jsonb_pretty()`]({{ page.version.version }}/functions-and-operators.md#jsonb-functions) function:
...
To query for particular data, use the [`jsonb_array_elements()` function]({{ page.version.version }}/functions-and-operators.md#jsonb-functions) to expand the desired elements from the JSON response. The following query returns the paths to each of the data files within the backup:
