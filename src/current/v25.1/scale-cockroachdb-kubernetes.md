...
1. Update `nodes` in the Operator's custom resource, which you downloaded when [deploying the cluster]({{ page.version.version }}/deploy-cockroachdb-with-kubernetes.md#initialize-the-cluster), with the target size of the CockroachDB cluster. This value refers to the number of CockroachDB nodes, each running in one pod:
...
Before scaling down CockroachDB, note the following [topology recommendation]({{ page.version.version }}/recommended-production-settings.md#topology):

- Each availability zone should have the same number of CockroachDB nodes.

If your nodes are distributed across 3 availability zones (as in our [deployment example]({{ page.version.version }}/deploy-cockroachdb-with-kubernetes.md)), we recommend scaling down by a multiple of 3 to retain an even distribution. If your cluster has 6 CockroachDB nodes, you should therefore scale down to 3, with 1 node in each zone.

1. Update `nodes` in the custom resource, which you downloaded when [deploying the cluster]({{ page.version.version }}/deploy-cockroachdb-with-kubernetes.md#initialize-the-cluster), with the target size of the CockroachDB cluster. For instance, to scale down to 3 nodes:
