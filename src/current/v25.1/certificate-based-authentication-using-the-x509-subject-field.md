...
For customers that need to use their established Certificate Authority (CA) infrastructure to manage SQL user authentication, CockroachDB supports mapping of SQL user [roles]({{ page.version.version }}/security-reference/authorization.md#roles) to values in the Subject field of the [X.509 certificate](https://en.wikipedia.org/wiki/X.509) used for [TLS authentication]({{ page.version.version }}/security-reference/transport-layer-security.md#what-is-transport-layer-security-tls).
...
- You run your own [Certificate Authority (CA)]({{ page.version.version }}/security-reference/transport-layer-security.md#certificates-signing-trust-and-authority) infrastructure.
- You need to use your existing CA infrastructure to manage [SQL user authentication]({{ page.version.version }}/create-user.md#user-authentication).
...
The [default workflow using `cockroach cert`]({{ page.version.version }}/cockroach-cert.md#examples) only sets the `CommonName` field in its certificates, so the functionality described in this page is unnecessary. It is not possible to share a single CA managed by `cockroach cert` across multiple CockroachDB clusters.

## Example

This example shows how to set up a mapping between the Subject field of an [X.509 certificate](https://en.wikipedia.org/wiki/X.509) and a SQL [user or role]({{page.version.version}}/security-reference/authorization.md#roles).
...
- The [`root` user]({{ page.version.version }}/security-reference/authorization.md#root-user).
- The [`node` user]({{ page.version.version }}/security-reference/authorization.md#node-user).
- Any [SQL user]({{ page.version.version }}/security-reference/authorization.md#sql-users) who needs to authenticate with your cluster. In this example, we will call the user `maxroach`.
...
At cluster startup, you'll need to pass the [`cockroach start`]({{ page.version.version }}/cockroach-start.md) flags [`--node-cert-distinguished-name`]({{ page.version.version }}/cockroach-start.md#flags-node-cert-distinguished-name) and [`--root-cert-distinguished-name`]({{ page.version.version }}/cockroach-start.md#flags-root-cert-distinguished-name).
...
Alternatively, you can update existing users with [`ALTER ROLE ... SUBJECT`]({{ page.version.version }}/alter-role.md#set-the-subject-role-option-for-certificate-based-authentication).
...
If you do not have an [enterprise license]({{ page.version.version }}/licensing-faqs.md#types-of-licenses), the following error is signaled:
...
Once the cluster is started, enable the following [cluster setting]({{ page.version.version }}/cluster-settings.md#setting-security-client-cert-subject-required-enabled):
