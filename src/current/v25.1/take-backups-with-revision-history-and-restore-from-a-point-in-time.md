...
You can create full or incremental backups [with revision history]({{ page.version.version }}/backup.md#with-revision-history):
...
You can configure garbage collection periods using the `ttlseconds` [replication zone setting]({{ page.version.version }}/configure-replication-zones.md). Taking backups with revision history allows for point-in-time restores within the revision history. 
...
For guidance on connecting to Amazon S3, Google Cloud Storage, Azure Storage, and other storage options, read [Use Cloud Storage]({{ page.version.version }}/use-cloud-storage.md).
...
If the full or incremental backup was taken [with revision history](#create-a-backup-with-revision-history), you can restore the data as it existed at an arbitrary point-in-time within the revision history captured by that backup. Use the [`AS OF SYSTEM TIME`]({{ page.version.version }}/as-of-system-time.md) clause to specify the point-in-time.

Additionally, if you want to restore a specific incremental backup, you can do so by specifying the `end_time` of the backup by using the [`AS OF SYSTEM TIME`]({{ page.version.version }}/as-of-system-time.md) clause. To find the incremental backup's `end_time`, use [`SHOW BACKUP`]({{ page.version.version }}/show-backup.md).
...
To view the available backup subdirectories you can restore from, use [`SHOW BACKUPS`]({{ page.version.version }}/restore.md#view-the-backup-subdirectories).
