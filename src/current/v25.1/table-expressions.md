...
A _table expression_ defines a data source in the `FROM` sub-clause of a [`SELECT` clause]({{ page.version.version }}/select-clause.md) or as parameter to a [`TABLE` clause]({{ page.version.version }}/selection-queries.md#table-clause).

A [join]({{ page.version.version }}/joins.md) is a particular kind of table expression.
...
`row_source_extension_stmt` | [Result rows](#use-the-output-of-another-statement) from a [supported statement]({{ page.version.version }}/sql-grammar.md#row_source_extension_stmt).
`select_stmt` | A [selection query]({{ page.version.version }}/selection-queries.md) to use as [subquery](#use-a-subquery).
`joined_table` | A [join expression]({{ page.version.version }}/joins.md).
...
`<table expr> JOIN <table expr> ON ...` | [Join expression]({{ page.version.version }}/joins.md). | `orders o JOIN customers c ON o.customer_id = c.id`
`(... subquery ...)` | A [selection query]({{ page.version.version }}/selection-queries.md) used as [subquery](#use-a-subquery). | `(SELECT * FROM customers c)`
`[... statement ...]` | Use the result rows of an [explainable statement]({{ page.version.version }}/sql-grammar.md#preparable_stmt).<br><br>This is a CockroachDB extension. However, Cockroach Labs recommends that you use the standard SQL [CTE syntax]({{ page.version.version }}/common-table-expressions.md) instead. See [Use the output of another statement](#use-the-output-of-another-statement) for an example. | `[SHOW COLUMNS FROM accounts]`
...
the contents of the table, [view]({{ page.version.version }}/views.md), or sequence with that name
in the current database, as configured by [`SET DATABASE`]({{ page.version.version }}/set-vars.md).

If the name is composed of two or more identifiers, [name resolution]({{ page.version.version }}/sql-name-resolution.md) rules apply.
...
You can also force index selection for [`DELETE`]({{ page.version.version }}/delete.md#force-index-selection-for-deletes) and [`UPDATE`]({{ page.version.version }}/update.md#force-index-selection-for-updates) statements.
{{site.data.alerts.end}}

### Access a common table expression

A single identifier in a table expression can refer to a
[common table expression]({{ page.version.version }}/common-table-expressions.md) defined
earlier.
...
by zero or more [scalar expressions]({{ page.version.version }}/scalar-expressions.md), followed by
a closing parenthesis.

The resolution of the function name follows the same rules as the
resolution of table names. See [Name Resolution]({{ page.version.version }}/sql-name-resolution.md) for more details.

#### Scalar function as data source

When a [function returning a single value]({{ page.version.version }}/scalar-expressions.md#function-calls-and-sql-special-forms) is
...
For example (the output of queries against [`information_schema`]({{ page.version.version }}/information-schema.md) will vary per database):
...
{{site.data.alerts.callout_info}}
`WITH ORDINALITY` necessarily prevents some optimizations of the surrounding query. Use it sparingly if performance is a concern, and always check the output of [`EXPLAIN`]({{ page.version.version }}/explain.md) in case of doubt.
{{site.data.alerts.end}}
...
See [`JOIN` expressions]({{ page.version.version }}/joins.md) for more details.
...
You can use a [selection query]({{ page.version.version }}/selection-queries.md) enclosed between parentheses `()`
as a table expression. This is called a _[subquery]({{ page.version.version }}/subqueries.md)_.
...
A [`WITH` query]({{ page.version.version }}/common-table-expressions.md) designates the output of executing a statement as a row source. The following statements are supported as row sources for table expressions:
...
- [`SHOW`]({{ page.version.version }}/sql-statements.md#data-definition-statements)
...
You can use table expressions in the [`SELECT` clause]({{ page.version.version }}/select-clause.md) and
[`TABLE` clause]({{ page.version.version }}/selection-queries.md#table-clause) variants of [selection clauses]({{ page.version.version }}/selection-queries.md#selection-clauses).
...
For more options to compose query results, see [Selection Queries]({{ page.version.version }}/selection-queries.md).

## See also

- [Constants]({{ page.version.version }}/sql-constants.md)
- [Explainable statements]({{ page.version.version }}/sql-grammar.md#preparable_stmt)
