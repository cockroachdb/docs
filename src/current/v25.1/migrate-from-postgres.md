...
For syntax differences, refer to [Features that differ from PostgreSQL]({{ page.version.version }}/postgresql-compatibility.md#features-that-differ-from-postgresql).
...
You can use [MOLT Fetch](molt/molt-fetch.md) to migrate PostgreSQL data to CockroachDB.
...
In the context of a full migration, these steps ensure that PostgreSQL data can be properly migrated to CockroachDB and your application queries tested against the cluster. For details, see the [Migration Overview]({{ page.version.version }}/migration-overview.md#prepare-for-migration).
...
1. Create a free [{{ site.data.products.cloud }} account](create-an-account.md), which is used to access the [Schema Conversion Tool](migrations-page.md) and [create the CockroachDB {{ site.data.products.standard }} cluster](create-your-cluster.md).
...
Use the [Schema Conversion Tool](migrations-page.md) to convert the `frenchtowns` schema for compatibility with CockroachDB. The schema has three tables: `regions`, `departments`, and `towns`.
...
1. Open the [Schema Conversion Tool](migrations-page.md) in the {{ site.data.products.cloud }} Console and [add a new PostgreSQL schema](migrations-page.md#convert-a-schema).

       After conversion is complete, [review the results](migrations-page.md#review-the-schema). The [**Summary Report**](migrations-page.md#summary-report) shows that there are errors under **Required Fixes**. You must resolve these in order to migrate the schema to CockroachDB.

       {{site.data.alerts.callout_success}}
       You can also [add your PostgreSQL database credentials](migrations-page.md#use-credentials) to have the Schema Conversion Tool obtain the schema directly from the PostgreSQL database.
       {{site.data.alerts.end}}
...
1. Review the `CREATE SEQUENCE` statements listed under **Suggestions**. Cockroach Labs does not recommend using a sequence to define a primary key column. For more information, see [Unique ID best practices]({{ page.version.version }}/performance-best-practices-overview.md#unique-id-best-practices).

       For this example, **Acknowledge** the suggestion without making further changes. In practice, after [conducting the full migration]({{ page.version.version }}/migration-overview.md#conduct-the-migration) to CockroachDB, you would modify your CockroachDB schema to use unique and non-sequential primary keys.
...
1. Click [**Migrate Schema**](migrations-page.md#migrate-the-schema) to create a new CockroachDB {{ site.data.products.standard }} cluster with the converted schema. Name the database `frenchtowns`.
...
1. Use [`IMPORT INTO`]({{ page.version.version }}/import-into.md) to import each PostgreSQL dump file into the corresponding table in the `frenchtowns` database.
...
1. Recall that `IMPORT INTO` invalidates all [foreign key]({{ page.version.version }}/foreign-key.md) constraints on the target table. View the constraints that are defined on `departments` and `towns`:
...
Use [MOLT Verify](molt/molt-verify.md) to check that the data on PostgreSQL and CockroachDB are consistent.
