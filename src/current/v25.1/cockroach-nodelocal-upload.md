...
---
 ...
 The `cockroach nodelocal upload` [command]({{ page.version.version }}/cockroach-commands.md) uploads a file to the external IO directory on a node's (the gateway node, by default) local file system.
...
The [`--external-io`]({{ page.version.version }}/cockroach-start.md#general) flag on the node you're uploading to **cannot** be set to `disabled`.
