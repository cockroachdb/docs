...
This page describes some common logging use cases, their relevant [logging channels]({{ page.version.version }}/logging-overview.md#logging-channels), and examples of notable events to be found in the logs:
...
{{site.data.alerts.callout_info}}
`‹` and `›` are placed around values in log messages that may contain sensitive data (PII). To customize this behavior, see [Redact logs]({{ page.version.version }}/configure-logs.md#redact-logs).
{{site.data.alerts.end}}
...
In this example configuration, the channels are grouped into a file sink called `ops`. The combined logging output will be found in a `cockroach-ops.log` file at the configured [logging directory]({{ page.version.version }}/configure-logs.md#logging-directory).
...
When monitoring your cluster, consider using these logs in conjunction with [Prometheus]({{ page.version.version }}/monitor-cockroachdb-with-prometheus.md), which can be set up to track node-level metrics.
{{site.data.alerts.end}}

### OPS

The [`OPS`]({{ page.version.version }}/logging.md#ops) channel logs operational events initiated by users or automation. These can include node additions and removals, process starts and shutdowns, [gossip](https://wikipedia.org/wiki/Gossip_protocol) connection events, and [zone configuration changes]({{ page.version.version }}/configure-replication-zones.md) on the SQL schema or system ranges.

#### Example: Node decommissioning

This [`node_decommissioning`]({{ page.version.version }}/eventlog.md#node_decommissioning) event shows that a node is in the [decommissioning](node-shutdown.html?filters=decommission#decommissioning) state:
...
- Preceding the `=` character is the `crdb-v2` event metadata. See the [reference documentation]({{ page.version.version }}/log-formats.md#format-crdb-v2) for details on the fields.
...
This [`node_restart`]({{ page.version.version }}/eventlog.md#node_restart) event shows that a node has rejoined the cluster after being offline (e.g., by being [restarted]({{ page.version.version }}/cockroach-start.md) after being fully decommissioned):
...
{{site.data.alerts.callout_info}}
All possible `OPS` event types are detailed in the [reference documentation]({{ page.version.version }}/eventlog.md).
{{site.data.alerts.end}}

### HEALTH

The [`HEALTH`]({{ page.version.version }}/logging.md#health) channel logs operational events initiated by CockroachDB or reported by automatic processes. These can include resource usage details, connection errors, [gossip](https://wikipedia.org/wiki/Gossip_protocol) status, [replication]({{ page.version.version }}/architecture/replication-layer.md) events, and runtime statistics.

#### Example: Runtime stats

A [`runtime_stats`]({{ page.version.version }}/eventlog.md#runtime_stats) event is recorded every 10 seconds to reflect server health:
...
{{site.data.alerts.callout_info}}
`runtime_stats` events are typically used for troubleshooting. To monitor your cluster's health, see [Monitoring and Alerting]({{ page.version.version }}/monitoring-and-alerting.md).
{{site.data.alerts.end}}

### SQL_SCHEMA

The [`SQL_SCHEMA`]({{ page.version.version }}/logging.md#sql_schema) channel logs changes to the SQL logical schema resulting from DDL operations.

#### Example: Schema change initiated

This [`alter_table`]({{ page.version.version }}/eventlog.md#alter_table) event shows an [`ALTER TABLE ... ADD FOREIGN KEY`]({{ page.version.version }}/alter-table.md#add-constraint) schema change being initiated on a `movr.public.vehicles` table:
...
This [`finish_schema_change`]({{ page.version.version }}/eventlog.md#finish_schema_change) event shows that the above schema change has completed:
...
{{site.data.alerts.callout_info}}
All possible `SQL_SCHEMA` event types are detailed in the [reference documentation]({{ page.version.version }}/eventlog.md#sql-logical-schema-changes).
{{site.data.alerts.end}}
...
In this example configuration, the channels are grouped into a file sink called `security`. The combined logging output will be found in a `cockroach-security.log` file at the configured [logging directory]({{ page.version.version }}/configure-logs.md#logging-directory).
...
The [`SESSIONS`]({{ page.version.version }}/logging.md#sessions) channel logs SQL session events. This includes client connection and session authentication events, for which logging must be enabled separately. For complete logging of client connections, we recommend enabling both types of events.
...
To log SQL client connection events to the `SESSIONS` channel, enable the `server.auth_log.sql_connections.enabled` [cluster setting]({{ page.version.version }}/cluster-settings.md):
...
These logs show a [`client_connection_start`]({{ page.version.version }}/eventlog.md#client_connection_start) (client connection established) and a [`client_connection_end`]({{ page.version.version }}/eventlog.md#client_connection_end) (client connection terminated) event over a `hostssl` (TLS transport over TCP) connection:
...
To log SQL session authentication events to the `SESSIONS` channel, enable the `server.auth_log.sql_sessions.enabled` [cluster setting]({{ page.version.version }}/cluster-settings.md) on every cluster:
...
- Preceding the `=` character is the `crdb-v2` event metadata. See the [reference documentation]({{ page.version.version }}/log-formats.md#format-crdb-v2) for details on the fields.
- The two [`client_authentication_info`]({{ page.version.version }}/eventlog.md#client_authentication_info) events show the progress of certificate authentication. The `Info` fields show the progress of certificate validation.
- The [`client_authentication_ok`]({{ page.version.version }}/eventlog.md#client_authentication_ok) event shows that certificate authentication was successful.
...
- Preceding the `=` character is the `crdb-v2` event metadata. See the [reference documentation]({{ page.version.version }}/log-formats.md#format-crdb-v2) for details on the fields.
- The two [`client_authentication_info`]({{ page.version.version }}/eventlog.md#client_authentication_info) events show the progress of certificate authentication. The `Info` fields show that password authentication was attempted, in the absence of a client certificate.
- The [`client_authentication_failed`]({{ page.version.version }}/eventlog.md#client_authentication_failed) event shows that password authentication was unsuccessful. The `Detail` field shows the related error.
- The [`client_session_end`]({{ page.version.version }}/eventlog.md#client_session_end) event shows that the SQL session was terminated. This would typically be followed by a [`client_connection_end`]({{ page.version.version }}/eventlog.md#client_connection_end) event.
- `User` shows that the SQL session authentication was attempted for user `roach`.

{{site.data.alerts.callout_info}}
All possible `SESSIONS` event types are detailed in the [reference documentation]({{ page.version.version }}/eventlog.md#sql-session-events). For more details on certificate and password authentication, see [Authentication]({{ page.version.version }}/authentication.md).
{{site.data.alerts.end}}

### SENSITIVE_ACCESS

The [`SENSITIVE_ACCESS`]({{ page.version.version }}/logging.md#sensitive_access) channel logs SQL audit events. These include all queries being run against [audited tables]({{ page.version.version }}/alter-table.md#experimental_audit), when enabled, as well as queries executed by users with the [`admin`]({{ page.version.version }}/security-reference/authorization.md#admin-role) role.
...
To log all queries against a specific table, enable auditing on the table with [`ALTER TABLE ... EXPERIMENTAL_AUDIT`]({{ page.version.version }}/alter-table.md#experimental_audit).
...
This [`sensitive_table_access`]({{ page.version.version }}/eventlog.md#sensitive_table_access) event shows that the audited table `customers` was accessed by user `root` issuing an [`INSERT`]({{ page.version.version }}/insert.md) statement:
...
- `ApplicationName` shows that the event originated from the [`cockroach sql`]({{ page.version.version }}/cockroach-sql.md) shell. You can use this field to filter the logging output by application.

{{site.data.alerts.callout_info}}
All possible `SENSITIVE_ACCESS` event types are detailed in the [reference documentation]({{ page.version.version }}/eventlog.md#sql-access-audit-events). For a detailed tutorial on table auditing, see [SQL Audit Logging]({{ page.version.version }}/sql-audit-logging.md).
{{site.data.alerts.end}}

### PRIVILEGES

The [`PRIVILEGES`]({{ page.version.version }}/logging.md#privileges) channel logs SQL privilege changes. These include DDL operations performed by SQL operations that [modify the privileges]({{ page.version.version }}/security-reference/authorization.md#managing-privileges) granted to [roles and users]({{ page.version.version }}/security-reference/authorization.md#users-and-roles) on databases, schemas, tables, and [user-defined types]({{ page.version.version }}/enum.md).

#### Example: Database privileges

This [`change_database_privilege`]({{ page.version.version }}/eventlog.md#change_database_privilege) event shows that user `root` granted all privileges to user `roach` on the database `movr`:
...
{{site.data.alerts.callout_info}}
All possible `PRIVILEGE` event types are detailed in the [reference documentation]({{ page.version.version }}/eventlog.md#sql-privilege-changes).
{{site.data.alerts.end}}

### USER_ADMIN

The [`USER_ADMIN`]({{ page.version.version }}/logging.md#user_admin) channel logs changes to users and roles. This includes user and role [creation and assignment]({{ page.version.version }}/security-reference/authorization.md#create-and-manage-users) and changes to [privileges]({{ page.version.version }}/security-reference/authorization.md#managing-privileges), [options]({{ page.version.version }}/create-role.md#parameters), and [passwords]({{ page.version.version }}/authentication.md#client-authentication).

#### Example: SQL user creation

This [`create_role`]({{ page.version.version }}/eventlog.md#create_role) event shows that a user `roach` was created and assigned a password by user `root`. Note that the password in the SQL statement is pre-redacted even if `redact` is set to `false` for the logging sink. For more details on redaction behavior, see [Redact logs]({{ page.version.version }}/configure-logs.md#redact-logs).
...
- Preceding the `=` character is the `crdb-v2` event metadata. See the [reference documentation]({{ page.version.version }}/log-formats.md#format-crdb-v2) for details on the fields.
- `ApplicationName` shows that the event originated from the [`cockroach sql`]({{ page.version.version }}/cockroach-sql.md) shell. You can use this field to filter the logging output by application.
- `RoleName` shows the name of the user/role. For details on user and role terminology, see [Users and roles]({{ page.version.version }}/security-reference/authorization.md#users-and-roles).

{{site.data.alerts.callout_info}}
All possible `USER_ADMIN` event types are detailed in the [reference documentation]({{ page.version.version }}/eventlog.md#sql-user-and-role-operations).
{{site.data.alerts.end}}
...
In this example configuration, the channels are grouped into a file sink called `performance`. The combined logging output will be found in a `cockroach-performance.log` file at the configured [logging directory]({{ page.version.version }}/configure-logs.md#logging-directory).
...
The [`SQL_EXEC`]({{ page.version.version }}/logging.md#sql_exec) channel reports all SQL executions on the cluster, when enabled.

To log cluster-wide executions, enable the `sql.trace.log_statement_execute` [cluster setting]({{ page.version.version }}/cluster-settings.md):

~~~ sql
> SET CLUSTER SETTING sql.trace.log_statement_execute = true;
~~~

Each node of the cluster will write all SQL queries it executes to the `SQL_EXEC` channel. These are recorded as [`query_execute`]({{ page.version.version }}/eventlog.md#query_execute) events.
...
Note the `FullTableScan` value in the logged event, which shows that this query performed a full table scan and likely caused a performance hit. To learn more about when this issue appears and how it can be resolved, see [Statement Tuning with `EXPLAIN`]({{ page.version.version }}/sql-tuning-with-explain.md#issue-full-table-scans).
...
{{site.data.alerts.callout_info}}
All possible `SQL_EXEC` event types are detailed in the [reference documentation]({{ page.version.version }}/eventlog.md#sql-execution-log).
{{site.data.alerts.end}}

### SQL_PERF

The [`SQL_PERF`]({{ page.version.version }}/logging.md#sql_perf) channel reports slow SQL queries, when enabled. This includes queries whose latency exceeds a configured threshold, as well as queries that perform a full table or index scan.

To enable slow query logging, enable the `sql.log.slow_query.latency_threshold` [cluster setting]({{ page.version.version }}/cluster-settings.md) by setting it to a non-zero value. This will log queries whose service latency exceeds a specified threshold value. The threshold value must be specified with a unit of time (e.g., `500ms` for 500 milliseconds, `5us` for 5 nanoseconds, or `5s` for 5 seconds). A threshold of `0s` disables the slow query log.
...
To log all queries that perform full table or index scans to `SQL_PERF`, regardless of query latency, set the `sql.log.slow_query.experimental_full_table_scans.enabled` [cluster setting]({{ page.version.version }}/cluster-settings.md) to `true`.
...
Each gateway node will now record queries that take longer than 100 milliseconds to the `SQL_PERF` channel as [`slow_query` events]({{ page.version.version }}/eventlog.md#slow_query).
...
The following query was logged with a service latency (`age`) of 9329.26 milliseconds, a very high latency that resulted from a [transaction retry error]({{ page.version.version }}/transaction-retry-error-reference.md#retry_write_too_old):
...
- Preceding the `=` character is the `crdb-v2` event metadata. See the [reference documentation]({{ page.version.version }}/log-formats.md#format-crdb-v2) for details on the fields.
- `ApplicationName` shows that the events originated from an application named `bank`. You can use this field to filter the logging output by application.
- `ErrorText` shows that this query encountered a [type of transaction retry error]({{ page.version.version }}/transaction-retry-error-reference.md#retry_write_too_old). For details on transaction retry errors and how to resolve them, see the [Transaction Retry Error Reference]({{ page.version.version }}/transaction-retry-error-reference.md#actions-to-take).
- `NumRetries` shows that the transaction was retried once before succeeding.

{{site.data.alerts.callout_info}}
All possible `SQL_PERF` event types are detailed in the [reference documentation]({{ page.version.version }}/eventlog.md#sql-slow-query-log).
{{site.data.alerts.end}}
...
In this case, defining separate `ops` and `security` network sinks allows us to:
 ...
 - [Enable redaction]({{ page.version.version }}/configure-logs.md#redact-logs) on the `ops` logs.
...
By default, `fluent-servers` log messages use the [`json-fluent-compact`]({{ page.version.version }}/log-formats.md#format-json-fluent-compact) format for ease of processing over a stream.

For example, this JSON message found in the `OPS` logging channel contains a [`node_restart`]({{ page.version.version }}/eventlog.md#node_restart) event. The event shows that a node has rejoined the cluster after being offline (e.g., by being [restarted]({{ page.version.version }}/cockroach-start.md) after being fully decommissioned):
...
- `sev` shows that the message has the `INFO` [severity level]({{ page.version.version }}/logging.md#logging-levels-severities).
- `event` contains the fields for the structured [`node_restart`]({{ page.version.version }}/eventlog.md#node_restart) event.
  - `NodeID` shows that the restarted node is `1`.
  - `StartedAt` shows the timestamp when the node was most recently restarted.
  - `LastUp` shows the timestamp when the node was up before being restarted.

See the [reference documentation]({{ page.version.version }}/log-formats.md#format-json-fluent-compact) for details on the remaining JSON fields.

### Network logging with log buffering

A database operator can configure CockroachDB to buffer log messages for a configurable time period or collected message size before writing them to the [log sink]({{ page.version.version }}/configure-logs.md#configure-log-sinks). This is especially useful for writing log messages to network log sinks, such as [Fluentd-compatible servers]({{ page.version.version }}/configure-logs.md#output-to-fluentd-compatible-network-collectors) or [HTTP servers]({{ page.version.version }}/configure-logs.md#output-to-http-network-collectors), where high-traffic or high-contention scenarios can result in log message write latency.

Log buffering is enabled by default on the [Fluentd-compatible]({{ page.version.version }}/configure-logs.md#output-to-fluentd-compatible-network-collectors) and [HTTP]({{ page.version.version }}/configure-logs.md#output-to-http-network-collectors) log sink destinations, but you may wish to adjust the buffering configuration for these log sinks based on your needs.

For example, the following logging configuration adjusts the default log buffering behavior for both a [Fluentd-compatible]({{ page.version.version }}/configure-logs.md#output-to-fluentd-compatible-network-collectors) and an [HTTP]({{ page.version.version }}/configure-logs.md#output-to-http-network-collectors) log sink destination:
...
Together, this configuration ensures that log messages to the Fluentd log sink target are buffered for up to `2MiB` in accumulated size, and log messages to the HTTP server log sink target are buffered for up to `10s` duration (with a limit of up to `75MiB` accumulated message size in the buffer before messages begin being dropped), before being written to the log sink. Further, each long sink target is configured with an overridden value for these settings specific to log messages in the `HEALTH` [log channel]({{ page.version.version }}/logging-overview.md#logging-channels), which are flushed more aggressively in both cases.

See [Log buffering]({{ page.version.version }}/configure-logs.md#log-buffering-for-network-sinks) for more information.

## See also

- [Notable Event Types]({{ page.version.version }}/eventlog.md)
- [SQL Audit Logging]({{ page.version.version }}/sql-audit-logging.md)
- [Use Prometheus and Alertmanager]({{ page.version.version }}/monitor-cockroachdb-with-prometheus.md)