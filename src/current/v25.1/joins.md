...
A `JOIN` expression, also called a _join_, combines the results of two or more [table expressions]({{ page.version.version }}/table-expressions.md) based on conditions on the values of particular columns (such as equality conditions). A join is a particular kind of table expression.

A `JOIN` expression defines a data source in the `FROM` sub-clause of a [`SELECT` clause]({{ page.version.version }}/select-clause.md) or as parameter to a [`TABLE` clause]({{ page.version.version }}/selection-queries.md#table-clause).

{{site.data.alerts.callout_success}}
The [cost-based optimizer]({{ page.version.version }}/cost-based-optimizer.md) supports hint syntax to force the use of a specific join algorithm.  For more information, see [Join hints]({{ page.version.version }}/cost-based-optimizer.md#join-hints).
...
`table_ref` | A [table expression]({{ page.version.version }}/table-expressions.md).
`opt_join_hint` | A [join hint]({{ page.version.version }}/cost-based-optimizer.md#join-hints).
`a_expr` | A [scalar expression]({{ page.version.version }}/scalar-expressions.md) to use as an [`ON` join condition](#supported-join-conditions).
...
- `ON` predicates: a Boolean [scalar expression]({{ page.version.version }}/scalar-expressions.md)
...
The [cost-based optimizer]({{ page.version.version }}/cost-based-optimizer.md) decides when it would be beneficial to use a lookup join. Lookup joins are used when there is a large imbalance in size between the two tables, as it only reads the smaller table and then looks up matches in the larger table. A lookup join requires that the right-hand (i.e., larger) table be indexed on the columns involved in the join condition. A [partial index]({{ page.version.version }}/partial-indexes.md) can only be used if it contains the subset of rows being looked up.
...
1. If the lookup condition contains no equalities (i.e., is composed only of an inequality), either the input of the join must return only one row or the join must have a `LOOKUP` [hint]({{ page.version.version }}/cost-based-optimizer.md#join-hints). This prevents poor performance of the current lookup join implementation.
1. If the index column is `DESC` and the inequality is of the form `idxCol < inputCol` or equivalently `inputCol > idxCol`, the column type must be countable in order to support retrieving the immediate previous value. This allows types like [`BOOL`]({{ page.version.version }}/bool.md), [`FLOAT`]({{ page.version.version }}/float.md), and [`INT`]({{ page.version.version }}/int.md), but disallows types like [`STRING`]({{ page.version.version }}/string.md) or [`BYTES`](bytes.html).

You can override the use of lookup joins using [join hints]({{ page.version.version }}/cost-based-optimizer.md#join-hints).
...
The output of [`EXPLAIN (VERBOSE)`]({{ page.version.version }}/explain.md#verbose-option) shows whether `equality cols are key` for lookup joins, which means that the lookup columns form a key in the target table such that each lookup has at most one result.

### Inverted joins

 Inverted joins force the optimizer to use a join using a [GIN index]({{ page.version.version }}/inverted-indexes.md) on the right side of the join. Inverted joins can only be used with `INNER` and `LEFT` joins.
...
See the [cost-based optimizer examples]({{ page.version.version }}/cost-based-optimizer.md#inverted-join-examples) for statements that use inverted joins.

## `LATERAL` joins

CockroachDB supports `LATERAL` subquery joins for `INNER` and `LEFT` cross joins. For more information about `LATERAL` subqueries, see [Lateral subqueries]({{ page.version.version }}/subqueries.md#lateral-subqueries).

## Apply joins

Apply join is the operator that executes a lateral join if the optimizer is not able to de-correlate it (i.e., rewrite the query to use a regular join). Most of the time, the optimizer can de-correlate most queries. However, there are some cases where the optimizer cannot perform this rewrite, and `apply-join` would show up in the [`EXPLAIN`]({{ page.version.version }}/explain.md#join-queries) output for the query. The optimizer also replaces correlated subqueries with apply joins, and therefore `apply-join` may appear in the `EXPLAIN` output even if `LATERAL` was not used.
...
- Use [`EXPLAIN`]({{ page.version.version }}/explain.md) over queries containing joins to verify that indexes are used.
- Use [indexes]({{ page.version.version }}/indexes.md) for faster joins.

## See also

- [Join hints]({{ page.version.version }}/cost-based-optimizer.md#join-hints)
