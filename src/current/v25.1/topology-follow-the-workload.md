...
In a multi-region deployment, follow-the-workload is the default behavior for tables that do not have a [table locality]({{ page.version.version }}/multiregion-overview.md#table-locality). In general, this is a good choice only for tables with the following requirements:
...
If read performance is your main focus for a table, but you want low-latency reads everywhere instead of just in the most active region, consider [global tables]({{ page.version.version }}/global-tables.md) or [follower reads]({{ page.version.version }}/topology-follower-reads.md).

Note that if you start using the [multi-region SQL abstractions]({{ page.version.version }}/multiregion-overview.md) for a database, CockroachDB will no longer provide the follow-the-workload behavior described on this page for that database.
...
Aside from [deploying a cluster across three regions](#cluster-setup) properly, with each node started with the [`--locality`]({{ page.version.version }}/cockroach-start.md#locality) flag specifying its region and zone combination, this behavior requires no extra configuration. CockroachDB will balance the replicas for a table across the three regions and will assign the range lease to the replica in the region with the greatest demand at any given time (the follow-the-workload feature). This means that read latency in the active region will be low while read latency in other regions will be higher due to having to leave the region to reach the leaseholder. Write latency will be higher as well due to always involving replicas in multiple regions.

![Follow-the-workload table replication](/images/v24.2/topology-patterns/topology_follower_reads1.png)

{{site.data.alerts.callout_info}}
Follow-the-workload is also used by [system ranges containing important internal data]({{ page.version.version }}/configure-replication-zones.md#create-a-replication-zone-for-a-system-range).
{{site.data.alerts.end}}
...
![Follow-the-workload topology reads](/images/v24.2/topology-patterns/topology_follow_the_workload_reads.png)
...
![Follow-the-workload topology writes](/images/v24.2/topology-patterns/topology_follow_the_workload_writes.gif)
...
![Follow-the-workload topology region failure](/images/v24.2/topology-patterns/topology_follower_reads_resiliency.png)
...
![Follow-the-workload topology](/images/v24.2/topology-patterns/topology_follower_reads3.png) {% endcomment %}
