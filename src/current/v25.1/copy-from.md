...
The `COPY FROM` statement copies data from [`cockroach sql`]({{ page.version.version }}/cockroach-sql.md) or other [third party clients]({{ page.version.version }}/install-client-drivers.md) to tables in your cluster.
...
To run the examples, use [`cockroach demo`]({{ page.version.version }}/cockroach-demo.md) to start a temporary, in-memory cluster with the [`movr` database]({{ page.version.version }}/movr.md) preloaded.
...
1. Set the `bytea_output` [session variable]({{ page.version.version }}/set-vars.md#supported-variables) to specify that CockroachDB should ingest hex-encoded byte array data:
