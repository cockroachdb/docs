...
1. Start the [built-in SQL shell]({{ page.version.version }}/cockroach-sql.md) using the connection string you got from the CockroachDB {{ site.data.products.cloud }} Console earlier:
...
1. Create a new directory named `certs` at the top level of the `example-app-python-django` project, and then copy the root certificate that you downloaded for your cluster to the new directory.
...
1. Send a GET request to read from the `cockroach_example_customers` table:
    ...
    You can also query the table directly in the [SQL shell]({{ page.version.version }}/cockroach-sql.md) to see the changes:
