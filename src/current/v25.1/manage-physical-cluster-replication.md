...
[Physical cluster replication (PCR)]({% link {{ page.version.version }}/physical-cluster-replication-overview.md %}) enables continuous replication of data from an active primary cluster to a passive standby cluster, supporting high availability and disaster recovery scenarios. This page describes:

- Connecting to and interacting with the system virtual cluser and virtual clusters on each physical cluster. 
- Managing replication using SQL statements
...
[`ALTER VIRTUAL CLUSTER`]({% link {{ page.version.version }}/alter-virtual-cluster.md %}) | Resume a paused replication stream.
[`ALTER VIRTUAL CLUSTER`]({% link {{ page.version.version }}/alter-virtual-cluster.md %}#start-a-virtual-cluster) | Initiate a [failover]({% link {{ page.version.version }}/failover-replication.md %}).
...
{% comment  %}
pause the PCR stream, resume
Complete the PCR stream
Change the duration of the retention window
Failover, failback, Fast failback
Allow reads from the standby cluster
{% endcomment %}
...
You must finalize the upgrade on each virtual cluster in the following order:
...
Repeat these steps for **each** virtual cluster in the specified order.
