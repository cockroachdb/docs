...
The `CREATE DATABASE` [statement]({{ page.version.version }}/sql-statements.md) creates a new CockroachDB database.


## Required privileges

To create a database, the user must be a member of the `admin` role or must have the [`CREATEDB`]({{ page.version.version }}/create-role.md#create-a-role-that-can-create-and-rename-databases) parameter set.
...
`PRIMARY REGION region_name` |  Create a [multi-region database]({{ page.version.version }}/multiregion-overview.md) with `region_name` as [the primary region]({{ page.version.version }}/multiregion-overview.md#database-regions).<br>Allowed values include any region returned by [`SHOW REGIONS FROM CLUSTER`]({{ page.version.version }}/show-regions.md).
`REGIONS region_name_list` |  Create a [multi-region database]({{ page.version.version }}/multiregion-overview.md) with `region_name_list` as [database regions]({{ page.version.version }}/multiregion-overview.md#database-regions).<br>Allowed values include any region returned by [`SHOW REGIONS FROM CLUSTER`]({{ page.version.version }}/show-regions.md).<br>To set database regions at database creation, a primary region must be specified in the same `CREATE DATABASE` statement.
`SURVIVE ZONE FAILURE` (*Default*)<br>`SURVIVE REGION FAILURE` |  Create a [multi-region database]({{ page.version.version }}/multiregion-overview.md) with regional failure or zone failure [survival goals]({{ page.version.version }}/multiregion-overview.md#survival-goals).<br>To set the regional failure survival goal, the database must have at least 3 [database regions]({{ page.version.version }}/multiregion-overview.md#database-regions).<br>Surviving zone failures is the default setting for multi-region databases.
...
Suppose you start a cluster with region and zone [localities specified at startup]({{ page.version.version }}/cockroach-start.md#locality).

For this example, let's use a [demo cluster]({{ page.version.version }}/cockroach-demo.md), with the [`--demo-locality` flag]({{ page.version.version }}/cockroach-demo.md#general) to simulate a multi-region cluster:
...
You can add a [secondary region]({{ page.version.version }}/multiregion-overview.md#database-regions) to a [multi-region database]({{ page.version.version }}/multiregion-overview.md) for failover purposes. If the [primary region]({{ page.version.version }}/alter-database.md#set-primary-region) fails, the secondary region becomes the new primary region.
...
For more information about secondary regions, see [Secondary regions]({{ page.version.version }}/multiregion-overview.md#secondary-regions).

## See also

- [`SHOW DATABASES`]({{ page.version.version }}/show-databases.md)
- [`SHOW CREATE DATABASE`]({{ page.version.version }}/show-create.md)
...
- [Multiregion overview]({{ page.version.version }}/multiregion-overview.md)
