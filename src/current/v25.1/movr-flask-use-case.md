...
This page guides you through an example use-case for global application development and deployment. It is the first section of the [Develop and Deploy a Global Application]({{ page.version.version }}/movr.md#develop-and-deploy-a-global-application) tutorial.
...
In the [example deployment]({{ page.version.version }}/movr-flask-deployment.md), the application and the database deployments are deployed on separate machines.
...
Limiting latency improves the user experience, and it can also help you avoid problems with data integrity, like [transaction contention]({{ page.version.version }}/performance-best-practices-overview.md#transaction-contention).
...
- Deploy the application so that each instance of the application communicates with the closest database instance. This can be configured during [application deployment]({{ page.version.version }}/movr-flask-deployment.md).

- Design the application's persistence layer to limit the number of queries on data stored outside of the region closest to the application. You can use CockroachDB's [multi-region features]({{ page.version.version }}/multiregion-overview.md) to control where data is stored and from where it is accessed.
...
You will set up an external load balancer in [Deploy a Global Serverless Application]({{ page.version.version }}/movr-flask-deployment.md).

## Next steps

Next, learn more about [creating a multi-region database schema]({{ page.version.version }}/movr-flask-database.md).
