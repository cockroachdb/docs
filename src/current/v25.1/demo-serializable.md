...
- In PostgreSQL, with the default [`READ COMMITTED`]({{ page.version.version }}/read-committed.md) isolation level, the [write skew](#write-skew) anomaly results in both doctors successfully booking leave and the hospital having no doctors on call for that particular shift.
...
![Schema for serializable transaction tutorial](/images/v24.2/serializable_schema.png)
...
Again, this anomaly is the result of PostgreSQL's default isolation level of [`READ COMMITTED`]({{ page.version.version }}/read-committed.md), but note that this would happen with any isolation level except `SERIALIZABLE` and some implementations of `REPEATABLE READ`:
...
1. If you haven't already, [install CockroachDB]({{ page.version.version }}/install-cockroachdb.md) locally.

1. Use the [`cockroach start-single-node`]({{ page.version.version }}/cockroach-start-single-node.md) command to start a one-node CockroachDB cluster in insecure mode:
...
1. In a new terminal window, open the [built-in SQL client]({{ page.version.version }}/cockroach-sql.md) and connect to `localhost`:
...
1. In the terminal for doctor 2, the application tries to commit the transaction:
    ...
    For this kind of error, CockroachDB recommends a [client-side transaction retry loop]({{ page.version.version }}/transaction-retry-error-reference.md#client-side-retry-handling) that would transparently observe that the one doctor cannot take time off because the other doctor already succeeded in asking for it. You can find generic transaction retry functions for various languages in our [Build an App]({{ page.version.version }}/example-apps.md) tutorials.

    For more information about the error message for the `RETRY_SERIALIZABLE` error type, see the [Transaction Retry Error Reference]({{ page.version.version }}/transaction-retry-error-reference.md#retry_serializable).
    {{site.data.alerts.end}}
...
- [Transactions Overview]({{ page.version.version }}/transactions.md)
