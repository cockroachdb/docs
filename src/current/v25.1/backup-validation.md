...
You can validate a backup of a [cluster]({{ page.version.version }}/backup.md#back-up-a-cluster), [database]({{ page.version.version }}/backup.md#back-up-a-database), or [table]({{ page.version.version }}/backup.md#back-up-a-table-or-view) backup with one of the following [`SHOW BACKUP`]({{ page.version.version }}/show-backup.md) or [`RESTORE`]({{ page.version.version }}/restore.md) statements. The three options result in increasing levels of backup validation:
...
1. Somewhat frequent ["full"]({{ page.version.version }}/restore.md) restores: Validates a backup completely.
...
Using `SHOW BACKUP` with the `check_files` option, you can check that all [SST and metadata files]({{ page.version.version }}/backup-architecture.md) that belong to a backup are present in the storage location.
...
To validate that a backup is restorable, you can run `RESTORE` with the `schema_only` option, which will complete a restore **without** restoring any rows. This process is significantly faster than running a [regular restore]({{ page.version.version }}/restore.md#examples) for the purposes of validation.
...
You can also use the [`new_db_name` option]({{ page.version.version }}/restore.md#rename-a-database-on-restore) to restore a database to a different name. For example, `new_db_name = test_movr`.
