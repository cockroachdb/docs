...
This page explains the `cockroach start-single-node` [command]({{ page.version.version }}/cockroach-commands.md), which you use to start a single-node cluster with replication disabled. A single-node cluster is appropriate for quick SQL testing or app development.

{{site.data.alerts.callout_danger}}
A single-node cluster is not appropriate for use in production or for performance testing. To run a multi-node cluster with replicated data for availability, consistency and resiliency, including load balancing across multiple nodes, use [`cockroach start`]({{ page.version.version }}/cockroach-start.md) and [`cockroach init`]({{ page.version.version }}/cockroach-init.md) to start a multi-node cluster with a minimum of three nodes instead.
...
`--locality` | **Not relevant for single-node clusters.** Arbitrary key-value pairs that describe the location of the node. Locality might include country, region, datacenter, rack, etc. For more details, see [Locality]({{ page.version.version }}/cockroach-start.md#locality) below.
...
1. Restart the node with the [`cockroach start`]({{ page.version.version }}/cockroach-start.md) command:
...
1. Open the [built-in SQL shell]({{ page.version.version }}/cockroach-sql.md):

    ~~~ shell
    $ cockroach sql --certs-dir=certs --host=localhost:26257
    ~~~

1. Update preconfigured [replication zones]({{ page.version.version }}/configure-replication-zones.md) to replicate user data 3 times and import internal data 5 times:
...
- Running a local multi-node cluster:
    - [From Binary]({{ page.version.version }}/start-a-local-cluster.md)
    - [In Kubernetes]({{ page.version.version }}/orchestrate-a-local-cluster-with-kubernetes.md)
    - [In Docker]({{ page.version.version }}/start-a-local-cluster-in-docker-mac.md)
- Running a distributed multi-node cluster:
    - [From Binary]({{ page.version.version }}/manual-deployment.md)
    - [In Kubernetes]({{ page.version.version }}/deploy-cockroachdb-with-kubernetes.md)
