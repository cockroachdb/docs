...
**Cockroach Labs recommends using [changefeeds to export data]({{ page.version.version }}/export-data-with-changefeeds.md)** because they provide better performance for growing workloads. Additionally, changefeeds operate as jobs, which offer [observability]({{ page.version.version }}/monitor-and-debug-changefeeds.md), [scheduling]({{ page.version.version }}/export-data-with-changefeeds.md#create-a-scheduled-changefeed-to-export-filtered-data), and [job management]({{ page.version.version }}/create-and-configure-changefeeds.md).
{{site.data.alerts.end}}

The `EXPORT` [statement]({{ page.version.version }}/sql-statements.md) exports tabular data or the results of arbitrary `SELECT` statements to the following:

- CSV files
- Parquet files

Using the [CockroachDB distributed execution engine]({{ page.version.version }}/architecture/sql-layer.md#distsql), `EXPORT` parallelizes file creation across all nodes in the cluster, making it possible to quickly get large sets of data out of CockroachDB in a format that can be ingested by downstream systems.
...
After the export has been initiated, you can cancel it with [`CANCEL QUERY`]({{ page.version.version }}/cancel-query.md).
...
{{site.data.alerts.callout_info}}
The `EXPORT` statement cannot be used within a [transaction]({{ page.version.version }}/transactions.md).
{{site.data.alerts.end}}

## Required privileges

 The user must have the `SELECT` [privilege]({{ page.version.version }}/security-reference/authorization.md#managing-privileges) on the table being exported, unless the [destination URI requires `admin` privileges]({{ page.version.version }}/import-into.md#source-privileges).
...
- Using [HTTP]({{ page.version.version }}/use-a-local-file-server.md) or HTTPS.
...
- Use a [changefeed export]({{ page.version.version }}/export-data-with-changefeeds.md) instead because it operates as a job, offering more observability into failures.
...
You can control the [`EXPORT`]({{ page.version.version }}/export.md) process's behavior using any of the following key-value pairs as a `kv_option`.
...
--------------------|--------------|----------------------
| [`BOOL`]({{ page.version.version }}/bool.md) | `BOOLEAN` | `nil` |
| [`STRING`]({{ page.version.version }}/string.md) | byte array | `STRING` |
| [`COLLATE`]({{ page.version.version }}/collate.md) | byte array | `STRING` |
| [`INET`]({{ page.version.version }}/inet.md) | byte array | `STRING` |
| [`JSONB`]({{ page.version.version }}/jsonb.md) | byte array | `JSON` |
| [`INT`]({{ page.version.version }}/int.md) [`INT8`]({{ page.version.version }}/int.md) | `INT64` | `nil` |
| [`INT2`]({{ page.version.version }}/int.md) [`INT4`]({{ page.version.version }}/int.md) | `INT32` | `nil` |
| [`FLOAT`]({{ page.version.version }}/float.md) [`FLOAT8`]({{ page.version.version }}/float.md) | `FLOAT64` | `nil` |
| [`FLOAT4`]({{ page.version.version }}/float.md) | `FLOAT32` | `nil` |
| [`DECIMAL`]({{ page.version.version }}/decimal.md) | byte array | `DECIMAL` <br>Note: scale and precision data are preserved in the Parquet file. |
| [`UUID`]({{ page.version.version }}/uuid.md) | `fixed_len_byte_array` | `nil` |
| [`BYTES`]({{ page.version.version }}/bytes.md) | byte array | `nil` |
| [`BIT`]({{ page.version.version }}/bit.md) | byte array | `nil` |
| [`ENUM`]({{ page.version.version }}/enum.md) | byte array | `ENUM` |
| [`Box2D`]({{ page.version.version }}/data-types.md#data-type-conversions-and-casts) | byte array | `STRING` |
| [`GEOGRAPHY`]({{ page.version.version }}/data-types.md#data-type-conversions-and-casts) | byte array | `nil` |
| [`GEOMETRY`]({{ page.version.version }}/data-types.md#data-type-conversions-and-casts) | byte array | `nil` |
| [`DATE`]({{ page.version.version }}/date.md) | byte array | `STRING` |
| [`TIME`]({{ page.version.version }}/time.md) | `INT64` | `TIME` <br>Note: microseconds after midnight; <br>exporting to microsecond precision. |
| [`TIMETZ`]({{ page.version.version }}/time.md) | byte array | `STRING` <br>Note: exporting to microsecond precision. |
| [`INTERVAL`]({{ page.version.version }}/interval.md) | byte array | `STRING` <br>Note: specifically represented as ISO8601. |
| [`TIMESTAMP`]({{ page.version.version }}/timestamp.md) | byte array | `STRING` <br>Note: exporting to microsecond precision. |
| [`TIMESTAMPTZ`]({{ page.version.version }}/timestamp.md) | byte array | `STRING` <br>Note: exporting to microsecond precision. |
| [`ARRAY`]({{ page.version.version }}/array.md) | Encoded as a repeated field; <br>each array value is encoded as per the preceding types. | `nil` |

## Exports and `AS OF SYSTEM TIME`

The [`AS OF SYSTEM TIME`]({{ page.version.version }}/as-of-system-time.md) clause is not required in `EXPORT` statements, even though they are long-running queries. If it is omitted, `AS OF SYSTEM TIME` is implicitly set to the start of the statement's execution. The risk of [contention]({{ page.version.version }}/performance-best-practices-overview.md#transaction-contention) is low because other transactions would need to have exactly the same transaction start time as the `EXPORT` statement's start time.
...
For more information, see [selection queries]({{ page.version.version }}/selection-queries.md).
...
For more information about the SQL client, see [`cockroach sql`]({{ page.version.version }}/cockroach-sql.md).
...
To associate your export objects with a [specific storage class]({{ page.version.version }}/use-cloud-storage.md#amazon-s3-storage-classes) in your Amazon S3 bucket, use the `S3_STORAGE_CLASS` parameter with the class. For example, the following S3 connection URI specifies the `INTELLIGENT_TIERING` storage class:
...
Using `EXPORT` with [`userfile`]({{ page.version.version }}/use-userfile-storage.md) is not recommended. You can either export data to [cloud storage]({{ page.version.version }}/use-cloud-storage.md) or to a local CSV file by using [`cockroach sql --execute`]({{ page.version.version }}/cockroach-sql.md#general):
...
View running exports by using [`SHOW STATEMENTS`]({{ page.version.version }}/show-statements.md):
...
Use [`SHOW STATEMENTS`]({{ page.version.version }}/show-statements.md) to get a running export's `query_id`, which can be used to [cancel the export]({{ page.version.version }}/cancel-query.md):
