...
Use the `ALTER ROLE` [statement]({{ page.version.version }}/sql-statements.md) to add, change, or remove a [role's]({{ page.version.version }}/create-role.md) password, change the role options for a role, and set default [session variable]({{ page.version.version }}/set-vars.md) values for a role.

You can use the keywords `ROLE` and `USER` interchangeably. [`ALTER USER`]({{ page.version.version }}/alter-user.md) is an alias for `ALTER ROLE`.
...
- To alter an [`admin` role]({{ page.version.version }}/security-reference/authorization.md#admin-role), the user must be a member of the `admin` role.
- To alter other roles, the user must be a member of the `admin` role or have the [`CREATEROLE`]({{ page.version.version }}/create-role.md#create-a-role-that-can-create-other-roles-and-manage-authentication-methods-for-the-new-roles) [role option](#role-options).
...
`SET {session variable}` | Set default [session variable]({{ page.version.version }}/set-vars.md) values for a role.
...
The following example allows a role to log in to the database with a [password]({{ page.version.version }}/authentication.md#client-authentication):
...
The following example allows the role to [create other roles]({{ page.version.version }}/create-role.md) and [manage authentication methods]({{ page.version.version }}/authentication.md#client-authentication) for them:
...
In the following example, the `root` user creates a role named `max`, and sets the default value of the `timezone` [session variable]({{ page.version.version }}/set-vars.md#supported-variables) for the `max` role.
...
In the following example, the `root` user creates a role named `max` and a database named `movr`, and sets the default value of the `statement_timeout` [session variable]({{ page.version.version }}/set-vars.md#supported-variables) for the `max` role in the `movr` database.
...
In the following example, the `root` user creates a database named `movr`, and sets the default value of the `timezone` [session variable]({{ page.version.version }}/set-vars.md#supported-variables) for all roles in that database.
...
{{site.data.alerts.callout_info}}
This statement is identical to [`ALTER DATABASE movr SET timezone = 'America/New_York';`]({{ page.version.version }}/alter-database.md).
{{site.data.alerts.end}}
...
To set a default value for all users for any [session variable]({{ page.version.version }}/set-vars.md) that applies during login, issue a statement like the following:
