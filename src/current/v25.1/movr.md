...
- The `movr` dataset, which contains rows of data that populate tables in the `movr` database. The `movr` dataset is built into [`cockroach demo`]({{ page.version.version }}/cockroach-demo.md) and [`cockroach workload`]({{ page.version.version }}/cockroach-workload.md).
...
[`cockroach demo`]({{ page.version.version }}/cockroach-demo.md) opens a SQL shell to a temporary, in-memory cluster. To open a SQL shell to a demo cluster with the `movr` database preloaded and set as the [current database]({{ page.version.version }}/sql-name-resolution.md#current-database), use the following command:

~~~ shell
$ cockroach demo movr
~~~

[`cockroach workload`]({{ page.version.version }}/cockroach-workload.md) loads sample datasets and workloads into running clusters. To load the `movr` database and some sample data into a running cluster, do the following:
...
For a tutorial on running MovR against a multi-region cluster, using two important multi-region [data topologies]({{ page.version.version }}/topology-patterns.md) to get very low latency reads and writes, see [Low Latency, Multi-Region Deployment]({{ page.version.version }}/demo-low-latency-multi-region-deployment.md).
...
1. [MovR: A Global Application Use-case]({{ page.version.version }}/movr-flask-use-case.md)
1. [Create a Multi-region Database Schema]({{ page.version.version }}/movr-flask-database.md)
1. [Set up a Virtual Environment for Developing Global Applications]({{ page.version.version }}/movr-flask-setup.md)
1. [Develop a Global Application]({{ page.version.version }}/movr-flask-application.md)
1. [Deploy a Global Application]({{ page.version.version }}/movr-flask-deployment.md)
...
- [Features in Preview]({{ page.version.version }}/cockroachdb-feature-availability.md)