...
We strongly recommend only using `cockroach debug job-trace` when working directly with the [Cockroach Labs support team]({{ page.version.version }}/support-resources.md).
{{site.data.alerts.end}}

The [`cockroach debug job-trace`]({{ page.version.version }}/cockroach-commands.md) command connects to your cluster and collects trace payloads for a running, traceable [job]({{ page.version.version }}/show-jobs.md#show-jobs) ([**imports**]({{ page.version.version }}/import-into.md) or [**backups**]({{ page.version.version }}/take-full-and-incremental-backups.md)). The trace payloads are helpful for debugging why a job is not running as expected or to add more context to logs gathered from the [`cockroach debug zip`]({{ page.version.version }}/cockroach-debug-zip.md) command.

The node that `cockroach debug job-trace` is run against will communicate to all nodes in the cluster in order to retrieve the trace payloads. This will deliver a zip file that contains [trace files](#files) for all the nodes participating in the execution of the job. The files hold information on the executing job's [trace spans]({{ page.version.version }}/show-trace.md#trace-description), which describe the sub-operations being performed. Specifically, these files will contain the spans that have not yet completed and are associated with the execution of that particular job. Using this command for a job that is not currently running will result in an empty zip file.
...
See [`SHOW JOBS`]({{ page.version.version }}/show-jobs.md#show-jobs) for details on capturing a `job_id`.
...
Flag | Description
-----|------------
`--user`<br><br>`-u` | The [SQL user]({{ page.version.version }}/create-user.md) that will own the client session.<br><br>**Env Variable:** `COCKROACH_USER`<br>**Default:** `root`
...
<a name="sql-flag-url"></a> `--url` | A [connection URL]({{ page.version.version }}/connection-parameters.md#connect-using-a-url) to use instead of the other arguments.<br><br>**Env Variable:** `COCKROACH_URL`<br>**Default:** no URL
...
See the [`SHOW TRACE FOR SESSION`]({{ page.version.version }}/show-trace.md#response) page for more information on trace responses.

## Example

### Generate a job-trace zip file

To generate the `job-trace` zip file, use your [connection string]({{ page.version.version }}/cockroach-start.md#standard-output) to pull the trace spans:
