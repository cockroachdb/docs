...
The `DROP VIRTUAL CLUSTER` statement removes virtual clusters. Virtual clusters are used only as part of the [**physical cluster replication (PCR)**]({{ page.version.version }}/physical-cluster-replication-overview.md) workflow.
...
`IMMEDIATE` | Drop a virtual cluster immediately instead of waiting for garbage collection ([GC TTL]({{ page.version.version }}/configure-replication-zones.md#gc-ttlseconds)).
