...
The `SHOW SCHEDULES` [statement]({{ page.version.version }}/sql-statements.md) lists all of the currently active [backup schedules]({{ page.version.version }}/create-schedule-for-backup.md) and [changefeed schedules]({{ page.version.version }}/create-schedule-for-changefeed.md).

It also lists all of the currently active scheduled jobs for [Row-Level TTL]({{ page.version.version }}/row-level-ttl.md).
...
Field | Description
------+------------
`id` | A unique ID to identify each schedule. This value is used if you want to control schedules (i.e., [pause]({{ page.version.version }}/pause-schedules.md), [resume]({{ page.version.version }}/resume-schedules.md), or [drop]({{ page.version.version }}/drop-schedules.md) it).
...
`next_run`  | The [`TIMESTAMP`]({{ page.version.version }}/timestamp.md) at which the next scheduled job is slated to run.
...
`jobsrunning` | The number of [jobs]({{ page.version.version }}/show-jobs.md) currently running for the schedule.
`owner` | The [user]({{ page.version.version }}/create-user.md) who created the schedule. Users with active schedules cannot be dropped.
`created` | The [`TIMESTAMP`]({{ page.version.version }}/timestamp.md) when the job was created.
`on_previous_running` | The status of the scheduled job if the previous backup is still running. This [option]({{ page.version.version }}/create-schedule-for-backup.md#schedule-options) by default is set to `WAIT`. 
`on_execution_failure` | The status of the scheduled job if an error occurs during the job execution. This [option]({{ page.version.version }}/create-schedule-for-backup.md#schedule-options) by default is set to `RETRY_SCHED`. 
`command` | The command that the scheduled job will run. This can be derived from the [`CREATE SCHEDULE FOR BACKUP`]({{ page.version.version }}/create-schedule-for-backup.md) statement, [`CREATE SCHEDULE FOR CHANGEFEED`]({{ page.version.version }}/create-schedule-for-changefeed.md) statement, or generated by the system for [Row-Level TTL]({{ page.version.version }}/row-level-ttl.md) jobs.
`backup_type` | Displays whether a scheduled backup job is [`FULL`]({{ page.version.version }}/take-full-and-incremental-backups.md#full-backups) or [`INCREMENTAL`]({{ page.version.version }}/take-full-and-incremental-backups.md#incremental-backups) when running `SHOW SCHEDULES FOR BACKUP`.  
...
To view the `CREATE SCHEDULE` statements for scheduled jobs, use [`SHOW CREATE SCHEDULE`]({{ page.version.version }}/show-create-schedule.md).
