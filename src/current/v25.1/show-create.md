...
The `SHOW CREATE` [statement]({{ page.version.version }}/sql-statements.md) shows the `CREATE` statement for an existing [database]({{ page.version.version }}/create-database.md), [function]({{ page.version.version }}/create-function.md), [table]({{ page.version.version }}/create-table.md), [view]({{ page.version.version }}/create-view.md), or [sequence]({{ page.version.version }}/create-sequence.md).

## Required privileges

The user must have any [privilege]({{ page.version.version }}/security-reference/authorization.md#managing-privileges) on the target database, function, table, view, or sequence.
...
`ALL TABLES` | Show the `CREATE` statements for all tables, views, and sequences in the current database.<br>This option is intended to provide the statements required to recreate the objects in the current database. As a result, `SHOW CREATE ALL TABLES` also returns the [`ALTER` statements]({{ page.version.version }}/alter-table.md) that add, modify, and validate an object's [constraints]({{ page.version.version }}/constraints.md). The `ALTER` statements follow the `CREATE` statements to guarantee that all objects are added before their references.
`ALL SCHEMAS` | Show the `CREATE` statements for all [schemas]({{ page.version.version }}/create-schema.md) in the current database.
`ALL TYPES` | Show the `CREATE` statements for all [types]({{ page.version.version }}/create-type.md) in the current database.
...
If one or more columns is [`NOT VISIBLE`]({{ page.version.version }}/create-table.md#not-visible-property) within a table, `SHOW CREATE` will display the `NOT VISIBLE` flag after those columns.
...
If you [add a comment]({{ page.version.version }}/comment-on.md) on a table, `SHOW CREATE TABLE` will display the comment.
...
Use the `SHOW CREATE TABLE` command to view [multi-region-defined]({{ page.version.version }}/multiregion-overview.md) table localities.



To add the first region to the database, or to set an already-added region as the primary region, use a [`SET PRIMARY REGION`]({{ page.version.version }}/alter-database.md#set-primary-region) statement:
...
All tables will be [`REGIONAL BY TABLE`]({{ page.version.version }}/alter-table.md#regional-by-table) in `us-east` by default. Configure the `users` table to be [`REGIONAL BY ROW`]({{ page.version.version }}/alter-table.md#regional-by-row) instead:
...
Note that this statement also returns the [`ALTER` statements]({{ page.version.version }}/alter-table.md) that add, modify, and validate an object's [constraints]({{ page.version.version }}/constraints.md).
...
Suppose that you have a multi-region cluster, and you want to return the `SHOW CREATE DATABASE` statement for a [multi-region database]({{ page.version.version }}/multiregion-overview.md).
...
In the SQL shell, [add regions to the database]({{ page.version.version }}/alter-database.md#add-region):
...
- [`CREATE TABLE`]({{ page.version.version }}/create-sequence.md)
