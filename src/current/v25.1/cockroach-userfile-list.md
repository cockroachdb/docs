...
---
 ...
 The `cockroach userfile list` [command]({{ page.version.version }}/cockroach-commands.md) lists the files stored in the [user-scoped file storage]({{ page.version.version }}/use-userfile-storage.md) which match the [provided pattern]({{ page.version.version }}/cockroach-userfile-upload.md#file-destination), using a SQL connection. If no pattern is provided, all files in the specified (or default, if unspecified) user scoped file storage will be listed.

## Required privileges

The user must have the `CREATE` [privilege]({{ page.version.version }}/security-reference/authorization.md#managing-privileges) on the target database. CockroachDB will proactively grant the user `GRANT`, `SELECT`, `INSERT`, `DROP`, `DELETE` on the metadata and file tables.

A user can only view files in their own user-scoped storage, which is accessed through the [userfile URI]({{ page.version.version }}/cockroach-userfile-upload.md#file-destination) used during the upload. CockroachDB will revoke all access from every other user in the cluster except users in the `admin` role.
...
`--user`<br>`-u` | The [SQL user]({{ page.version.version }}/create-user.md) that will own the client session.<br><br>**Env Variable:** `COCKROACH_USER`<br>**Default:** `root`
...
If you [uploaded a file to a non-default userfile URI]({{ page.version.version }}/cockroach-userfile-upload.md#upload-a-file-to-a-non-default-userfile-uri) (e.g., `userfile://testdb.public.uploads`):
