...
For more details on use cases, refer to the [Logical Data Replication Overview]({{ page.version.version }}/logical-data-replication-overview.md).
...
1. Set up an [external connection]({{ page.version.version }}/create-external-connection.md) on cluster B (which will be the destination cluster initially) to hold the connection URI for cluster A.
...
1. Check the status of the LDR job in the [DB Console]({{ page.version.version }}/ui-overview.md).
...
- Two separate {{ page.version.version }} CockroachDB {{ site.data.products.core }} clusters with connectivity between every node in both clusters. That is, all nodes in cluster A must be able to contact each node in cluster B and vice versa. The SQL advertised address should be the cluster node advertise address so that the LDR job can plan node-to-node connections between clusters for maximum performance.
    - To set up each cluster, you can follow [Deploy CockroachDB on Premises]({{ page.version.version }}/deploy-cockroachdb-on-premises.md).
...
- [Partial indexes]({{ page.version.version }}/partial-indexes.md) and [hash-sharded indexes]({{ page.version.version }}/hash-sharded-indexes.md)
- Indexes with a [virtual computed column]({{ page.version.version }}/computed-columns.md)
- Composite types in the [primary key]({{ page.version.version }}/primary-key.md)

For more details, refer to the LDR [Known limitations]({{ page.version.version }}/logical-data-replication-overview.md#known-limitations).

When you run LDR in [`immediate` mode](#modes), you cannot replicate a table with [foreign key constraints]({{ page.version.version }}/foreign-key.md). In [`validated` mode](#modes), foreign key constraints **must** match. All constraints are enforced at the time of SQL/application write.
...
1. Enable the `kv.rangefeed.enabled` [cluster setting]({{ page.version.version }}/cluster-settings.md) on the **source** cluster:

    ~~~ sql
    SET CLUSTER SETTING kv.rangefeed.enabled = true;
    ~~~

1. On the **destination**, create a user with the [`REPLICATION` system privilege]({{ page.version.version }}/security-reference/authorization.md#supported-privileges) who will start the LDR job:
...
In this step, you'll set up an [external connection]({{ page.version.version }}/create-external-connection.md) from the destination cluster to the source cluster. Depending on how you manage certificates, you must ensure that all nodes between the clusters have access to the certificate of the other cluster.
...
1. In the SQL shell on the **destination** cluster, create an [external connection]({{ page.version.version }}/create-external-connection.md) using the source cluster's connection string. Prefix the `postgresql://` scheme to the connection string and replace `{source}` with your external connection name:
...
1. From the **destination** cluster, start LDR. Use the fully qualified table name for the source and destination tables:
    ...
    Once LDR has started, an LDR job will start on the destination cluster. You can [pause]({{ page.version.version }}/pause-job.md), [resume]({{ page.version.version }}/resume-job.md), or [cancel]({{ page.version.version }}/cancel-job.md) the LDR job with the job ID. Use [`SHOW LOGICAL REPLICATION JOBS`]({{ page.version.version }}/show-logical-replication-jobs.md) to display the LDR job IDs:
...
In this step, you'll access the [DB Console]({{ page.version.version }}/ui-overview.md) and monitor the status and metrics for the created LDR jobs. Depending on which cluster you would like to view, follow the instructions for either the source or destination.

{{site.data.alerts.callout_success}}
You can use the [DB Console]({{ page.version.version }}/ui-overview.md), the SQL shell, [Metrics Export]({{ page.version.version }}/datadog.md#enable-metrics-collection) with Prometheus and Datadog, and [labels with some LDR metrics]({{ page.version.version }}/child-metrics.md) to monitor the job.

For a full reference on monitoring LDR, refer to [Logical Data Replication Monitoring]({{ page.version.version }}/logical-data-replication-monitoring.md).
{{site.data.alerts.end}}

1. Access the DB Console at `http://{node IP or hostname}:8080` and enter your user's credentials.
1. On the source cluster, navigate to the [**Jobs** page]({{ page.version.version }}/ui-jobs-page.md) to view a list of all jobs. Use the job **Type** dropdown and select **Replication Producer**. This will display the history retention job. This will run while the LDR job is active to protect changes to the table from [garbage collection]({{ page.version.version }}/architecture/storage-layer.md#garbage-collection) until they have been replicated to the destination cluster.
...
1. On the destination cluster, click on **Metrics** in the left-hand navigation menu. Use the **Dashboard** dropdown to select [**Logical Data Replication**]({{ page.version.version }}/ui-logical-data-replication-dashboard.md). This page shows graphs for monitoring LDR.

## What's next

- [Manage Logical Data Replication]({{ page.version.version }}/manage-logical-data-replication.md): Manage the DLQ and schema changes for the replicating tables.
- [`CREATE LOGICAL REPLICATION STREAM`]({{ page.version.version }}/create-logical-replication-stream.md)
- [Data Resilience]({{ page.version.version }}/data-resilience.md)