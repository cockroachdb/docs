...
1. Prepare the clusters for with the required, settings, users, and privileges according to the LDR setup.
...
1. On the **source** cluster in a new terminal window, generate a connection string, by passing the user, node IP, and port, along with the directory to the source cluster's CA certificate:
    ...
    If you're setting up bidirectional LDR, use `cockroach encode-uri` on the destination cluster to generate its connection string as well. You'll need the connection string to create an external connection for both clusters.
...
(Optional) For bidirectional LDR, you'll need to repeat creating the certificate output and the external connection for the opposite clusters. Both clusters will act as the source and destination. At this point, you've created an external connection for LDR stream 1, so cluster **A** (source) to **B** (destination). Now, create the same for LDR stream 2 cluster **B** (source) to cluster **A** (destination).
...
1. On cluster **A**, create an [external connection]({% link {{ page.version.version }}/create-external-connection.md %}) using cluster **B's** connection string. Prefix the `postgresql://` scheme to the connection string and replace `{source}` with your external connection name:
...
{{site.data.alerts.callout_info}}
Ensure you use the fully qualified table names.
{{site.data.alerts.end}}
