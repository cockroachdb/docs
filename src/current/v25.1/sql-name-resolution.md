...
A CockroachDB cluster can store multiple databases. Each database can store multiple schemas, and each schema can store multiple [tables]({{ page.version.version }}/create-table.md), [views]({{ page.version.version }}/views.md), [sequences]({{ page.version.version }}/create-sequence.md), and [user-defined types]({{ page.version.version }}/enum.md).

When you first [start a cluster]({{ page.version.version }}/start-a-local-cluster.md), a number of [preloaded databases]({{ page.version.version }}/show-databases.md#preloaded-databases) and schemas are included, including the `defaultdb` database and the `public` schema. By default, objects (e.g., tables) are stored in the preloaded `public` schema, in the [current database](#current-database) (`defaultdb`, by default).

In addition to the `public` schema, CockroachDB supports a fixed set of [system catalog schemas]({{ page.version.version }}/system-catalogs.md), available in every database, that provide ancillary, non-stored data to client applications. For example, [`information_schema`]({{ page.version.version }}/information-schema.md) is provided for compatibility with the SQL standard, and [`pg_catalog`]({{ page.version.version }}/pg-catalog.md) and [`pg_extension`]({{ page.version.version }}/architecture/glossary.md#spatial-system-tables) are provided for compatibility with PostgreSQL.

To create a new database, use a [`CREATE DATABASE`]({{ page.version.version }}/create-database.md) statement. To create a new schema, use a [`CREATE SCHEMA`]({{ page.version.version }}/create-schema.md) statement. The list of all databases can be obtained with [`SHOW DATABASES`]({{ page.version.version }}/show-databases.md). The list of all schemas for a given database can be obtained with [`SHOW SCHEMAS`]({{ page.version.version }}/show-schemas.md). The list of all objects for a given schema can be obtained with other `SHOW` statements.

### Migrating namespaces from previous versions of CockroachDB

In CockroachDB versions < v20.2, [user-defined schemas]({{ page.version.version }}/create-schema.md) are not supported, and all objects created in a given database use the `public` schema. To provide a multi-level structure for stored objects in earlier versions of CockroachDB, we have recommended using [database]({{ page.version.version }}/create-database.md) namespaces instead of schema namespaces.
...
- [Create new schemas]({{ page.version.version }}/create-schema.md) in databases on your cluster. After the schemas are created, use [`ALTER TABLE ... RENAME`]({{ page.version.version }}/alter-table.md#rename-to), [`ALTER SEQUENCE ... RENAME`]({{ page.version.version }}/alter-sequence.md), [`ALTER TYPE ... RENAME`]({{ page.version.version }}/alter-type.md), or [`ALTER VIEW ... RENAME`]({{ page.version.version }}/alter-view.md) statements to move objects between databases as needed. To move objects between schemas, use [`ALTER TABLE ... SET SCHEMA`]({{ page.version.version }}/alter-table.md#set-schema), [`ALTER SEQUENCE ... SET SCHEMA`]({{ page.version.version }}/alter-sequence.md), or [`ALTER VIEW ... SET SCHEMA`]({{ page.version.version }}/alter-view.md).

- If your cluster contains cross-database references (e.g., a cross-database foreign key reference, or a cross-database view reference), use the relevant [`ALTER TABLE`]({{ page.version.version }}/alter-table.md), [`ALTER SEQUENCE`]({{ page.version.version }}/alter-sequence.md), [`ALTER TYPE`]({{ page.version.version }}/alter-type.md), or [`ALTER VIEW `]({{ page.version.version }}/alter-view.md) statements to move any cross-referencing objects to the same database, but different schemas. Cross-database object references were allowed in earlier versions of CockroachDB to make database-object naming hierarchies more flexible for users. In v20.2, creating cross-database references are disabled for [foreign keys](foreign-key.html), [views]({{ page.version.version }}/views.md), and [sequence ownership]({{ page.version.version }}/create-sequence.md). For details, see [tracking issue](https://github.com/cockroachdb/cockroach/issues/55791).
...
database`]({{ page.version.version }}/show-vars.md) and change it with [`SET database`]({{ page.version.version }}/set-vars.md).

- You can inspect the list of valid database names that can be specified in `database` with [`SHOW DATABASES`]({{ page.version.version }}/show-databases.md).
...
- You can set the current search path with [`SET search_path`]({{ page.version.version }}/set-vars.md) and inspected it with [`SHOW
search_path`]({{ page.version.version }}/show-vars.md).
- You can inspect the list of valid schemas that can be listed in `search_path` with [`SHOW SCHEMAS`]({{ page.version.version }}/show-schemas.md).
...
CockroachDB supports the following ways to specify an index name for statements that require one (e.g., [`DROP INDEX`]({{ page.version.version }}/drop-index.md), [`ALTER INDEX ... RENAME`]({{ page.version.version }}/alter-index.md), etc.):
...
Suppose that a client frequently accesses a stored table as well as a virtual table in the [Information Schema]({{ page.version.version }}/information-schema.md). Because `information_schema` is not in the search path by default, all queries that need to access it must mention it explicitly.
...
database to be called `information_schema`, `public`, `pg_catalog`, [`pg_extension`]({{ page.version.version }}/architecture/glossary.md#spatial-system-tables),
or `crdb_internal`.
...
The [`CREATE TABLE`]({{ page.version.version }}/create-table.md) statement in this example uses a partially
...
- [Keywords and Identifiers]({{ page.version.version }}/keywords-and-identifiers.md)