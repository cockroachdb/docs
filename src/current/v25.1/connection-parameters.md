...
commands]({{ page.version.version }}/cockroach-commands.md), work by establishing a network
...
----------|-------------|---------
`<username>` | The [SQL user]({{ page.version.version }}/create-user.md) that will own the client session. | ✗
`<password>` | The user's password. It is not recommended to pass the password in the URL directly.<br><br>Note that passwords with special characters must be passed as [query string parameters](#additional-connection-parameters) (e.g., `postgres://maxroach@localhost:26257/movr?password=<password>`) and not as a component in the connection URL (e.g., `postgres://maxroach:<password>@localhost:26257/movr`).<br><br>[Find more detail about how CockroachDB handles passwords.]({{ page.version.version }}/authentication.md#client-authentication) | ✗
...
`<database>` | A database name to use as [current database]({{ page.version.version }}/sql-name-resolution.md#current-database). Defaults to `defaultdb` when using `cockroach` client commands. Drivers and ORMs may have different defaults. | ✗
...
commands]({{ page.version.version }}/cockroach-commands.md) other than [`cockroach
sql`]({{ page.version.version }}/cockroach-sql.md). A warning
...
----------|-------------|---------------
`application_name` | An initial value for the [`application_name` session variable]({{ page.version.version }}/set-vars.md).<br><br>Note: For [Java JDBC]({{ page.version.version }}/build-a-java-app-with-cockroachdb.md), use `ApplicationName`. | Empty string.
...
`sslrootcert` | Path to the [CA certificate]({{ page.version.version }}/cockroach-cert.md), when `sslmode` is not `disable`. | Empty string.
`sslcert` | Path to the [client certificate]({{ page.version.version }}/cockroach-cert.md), when `sslmode` is not `disable`. | Empty string.
`sslkey` | Path to the [client private key]({{ page.version.version }}/cockroach-cert.md), when `sslmode` is not `disable`. | Empty string.
...
`results_buffer_size` | Default size of the buffer that accumulates results for a statement or a batch of statements before they are sent to the client. Can also be set using the [`sql.defaults.results_buffer.size` cluster setting]({{ page.version.version }}/cluster-settings.md#setting-sql-defaults-results-buffer-size). Can be set as a top-level query parameter or as an `options` parameter.
...
`-c <session_variable>=<value>` |  Sets a [session variable]({{ page.version.version }}/set-vars.md) for the SQL session.
`results_buffer_size` | Default size of the buffer that accumulates results for a statement or a batch of statements before they are sent to the client. Can also be set using the [`sql.defaults.results_buffer.size` cluster setting]({{ page.version.version }}/cluster-settings.md#setting-sql-defaults-results-buffer-size). Can be set as a top-level query parameter or as an `options` parameter.
...
`sslmode=verify-full` | Force a secure connection, verify that the server certificate is signed by a known CA, and verify that the server address matches that specified in the certificate. | Use for [secure deployments]({{ page.version.version }}/secure-a-cluster.md).
...
### Convert a URL for different drivers
 ...
 The subcommand `cockroach convert-url` converts a connection URL, such as those printed out by [`cockroach start`]({{ page.version.version }}/cockroach-start.md) or included in the online documentation, to the syntax recognized by various [client drivers]({{ page.version.version }}/third-party-database-tools.md#drivers). For example:
...
[Create Security Certificates]({{ page.version.version }}/cockroach-cert.md) and
[Rotate Certificates]({{ page.version.version }}/rotate-certificates.md).
...
This specifies a connection for the `root` user to an insecure cluster listening for a socket connection (e.g., a cluster started with the [`--socket-dir` flag]({{ page.version.version }}/cockroach-start.md#networking)) at `/path/to/client`, and on port 26257.
...
Most [`cockroach` commands]({{ page.version.version }}/cockroach-commands.md) accept connection
...
To specify `mydb` as the current database using [`cockroach sql`]({{ page.version.version }}/cockroach-sql.md), run the following command:
...
- [Secure a Cluster]({{ page.version.version }}/secure-a-cluster.md)
- [Create and Manage Users]({{ page.version.version }}/security-reference/authorization.md#create-and-manage-users)