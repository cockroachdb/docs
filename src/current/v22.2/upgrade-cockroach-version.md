...
{% capture homebrew_commands %}
    {% include_cached copy-clipboard.html %}
    ~~~ shell
    brew update
    brew upgrade cockroach@{{ page.version.version | remove_first: "v" }}
    ~~~
{% endcapture %}
...
1. To download the binary, visit [What's New in {{ page.version.version }}?](https://www.cockroachlabs.com/docs/releases/{{ page.version.version }}). Extract the archive. In the following instructions, replace `{COCKROACHDB_DIR}` with the path to the extracted archive directory.

    <div class="filter-content" markdown="1" data-scope="mac">
        If you installed using Homebrew, update the installation, then upgrade CockroachDB: {{ homebrew_commands }}
    </div>
    <div class="filter-content" markdown="1" data-scope="mac">
        If you installed using Docker, stop and remove the container, then restart it and specify the new version. Data stored directly in the container is destroyed, but data stored in a Docker volume is preserved. Refer to [Deploy in Docker]({% link {{ page.version.version }}/install-cockroachdb-mac.md %}#install-docker).
    </div>
    <div class="filter-content" markdown="1" data-scope="linux">
        If you installed using Docker, stop and remove the container, then restart it and specify the new version. Data stored directly in the container is destroyed, but data stored in a Docker volume is preserved. Refer to [Deploy in Docker]({% link {{ page.version.version }}/install-cockroachdb.md %}#use-docker).
    </div>

1. If you have a previous version of the `cockroach` binary in your `$PATH`, or if you previously installed CockroachDB using a different installation method, either remove the outdated installation or rename the outdated `cockroach` binary, and then move the new one into its place.
