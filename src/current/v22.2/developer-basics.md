...
CockroachDB is designed to make your data scalable and consistent. All stored records are replicated across a distributed deployment of database instances, and all database transactions committed in CockroachDB satisfy [ACID properties](https://en.wikipedia.org/wiki/ACID). This makes CockroachDB a CAP-Consistent (CP) system under the [CAP theorem](https://wikipedia.org/wiki/CAP_theorem), and means that when you store data in CockroachDB, the data will be valid and anomaly-free, even in the event of a system error or power failure.
...
As a CAP-Consistent (CP) system under the CAP theorem, CockroachDB prioritizes data consistency. Each piece of data has at least 3 replicas. When reading or writing data, one of the replicas is granted a [lease](architecture/replication-layer.html#leases), or temporary ownership, of the data. The lease expires [after a few seconds by default](architecture/replication-layer.html#important-values-and-timeouts). The leaseholder can complete a read without needing to communicate with the other replicas. When the leaseholder is sent a write request, a majority of the replica nodes must be able to communicate with each other to coordinate the write. This ensures that the most recent write is always available to subsequent reads.

If the leaseholder replica cannot communicate with the other replicas, it will be allowed to continue to serve reads until the lease expires, and then one of the other replicas will become the leaseholder. This allows the system to recover quickly from any outages, and provides high-availability despite being a CP system under the CAP theorem.
