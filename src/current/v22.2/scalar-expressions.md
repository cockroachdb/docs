...
- If the name `a` refers to an `ARRAY` of 10 values, `a[3]` will retrieve the third value. The first value in an `ARRAY` has index `1`.
- If the name `j` refers to a `JSONB` array of 10 values, `j[3]` will retrieve the fourth value. The first value in a `JSONB` array has index `0`.
