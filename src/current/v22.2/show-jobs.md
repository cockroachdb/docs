...
`modified` | The `TIMESTAMP` when the [job record](backup-architecture.html#job-creation-phase) is updated with the job's progress, or when the job was paused or resumed.
...
`trace_id` |  The job's internal [trace ID](show-trace.html#trace-description) for inflight debugging. **Note**: This ID can only be used by the Cockroach Labs support team for internal observability.
