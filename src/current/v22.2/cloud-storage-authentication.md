...
Setting up an external ID allows a user to grant Amazon resources to a third-party organization. The external ID is optional information that is passed in an IAM role trust policy to designate who can assume the role, provided a certain value is present in the external ID. Using an external ID helps avoid the [confused deputy problem](https://en.wikipedia.org/wiki/Confused_deputy_problem). An external ID could be useful when you have configured a role for a third-party organization to access your Amazon S3 bucket and act on your behalf. Or, your organization needs to assume roles for different customers and you require a unique identifier for each customer. The external ID should be a random string generated by the third-party.
...
To access the bucket, pass the assumed role's ARN along with the external ID, which you can copy from the IAM role's summary page. Alternately, you can use the [AWS CLI](https://docs.aws.amazon.com/cli/v1/userguide/cli-configure-quickstart.html).
