...
The [SQL Connection Attempts graph]({% link cockroachcloud/metrics.md %}#identify-sql-problems) shows how many new connections are being created each second. The [SQL Open Sessions graph]({% link cockroachcloud/metrics.md %}#sql-open-sessions) shows the total number of SQL client connections across the cluster. To determine if your connection pool is correctly configured use the metrics from these graphs in the following formula:
...
<section class="filter-content" markdown="1" data-scope="serverless">
The [SQL Connection Attempts graph]({% link cockroachcloud/metrics.md %}#identify-sql-problems) shows the number of new SQL connections per second.
</section>
...
The [SQL Open Sessions graph]({% link cockroachcloud/metrics.md %}#sql-open-sessions) shows the number of open connections on your cluster.
