...
{% include new-in.md version="v21.2" %} CockroachDB supports setting session variables for the duration of a single transaction, using [the `LOCAL` keyword](#set-local).
...
<div>
  {% remote_include https://raw.githubusercontent.com/cockroachdb/generated-diagrams/release-21.2/grammar_svg/set_local.html %}
</div>
...
{% include new-in.md version="v21.2" %} To set a variable for the duration of a single transaction, use the `SET LOCAL` statement.
...
{% include new-in.md version="v21.2" %} You can roll back session variable settings to [savepoints](savepoint.html).
...
{% include new-in.md version="v21.2" %} To assume another [role](security-reference/authorization.html#roles) for the duration of a session, use `SET ROLE <role>`. `SET ROLE <role>` is equivalent to `SET role = <role>`.
