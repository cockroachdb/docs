...
### Restoring a multi-region database to a new database in CockroachDB {{ site.data.products.dedicated }}

To restore a multi-region database to a new database in a CockroachDB {{ site.data.products.dedicated }} cluster, the following additional steps are required to ensure the regions match exactly between backup and restore:
