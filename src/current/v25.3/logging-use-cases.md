...
This page describes some common logging use cases, their relevant [logging channels]({% link {{ page.version.version }}/logging-overview.md %}#logging-channels), and examples of notable events to be found in the logs:
...
{{site.data.alerts.callout_info}}
`‹` and `›` are placed around values in log messages that may contain sensitive data (PII). To customize this behavior, see [Redact logs]({% link {{ page.version.version }}/configure-logs.md %}#redact-logs).
{{site.data.alerts.end}}
...
In this example configuration, the channels are grouped into a file sink called `ops`. The combined logging output will be found in a `cockroach-ops.log` file at the configured [logging directory]({% link {{ page.version.version }}/configure-logs.md %}#logging-directory).
...
When monitoring your cluster, consider using these logs in conjunction with [Prometheus]({% link {{ page.version.version }}/monitor-cockroachdb-with-prometheus.md %}), which can be set up to track node-level metrics.
{{site.data.alerts.end}}

### OPS

The [`OPS`]({% link {{ page.version.version }}/logging.md %}#ops) channel logs operational events initiated by users or automation. These can include node additions and removals, process starts and shutdowns, [gossip](https://wikipedia.org/wiki/Gossip_protocol) connection events, and [zone configuration changes]({% link {{ page.version.version }}/configure-replication-zones.md %}) on the SQL schema or system ranges.
...
This [`node_restart`]({% link {{ page.version.version }}/eventlog.md %}#node_restart) event shows that a node has rejoined the cluster after being offline (e.g., by being [restarted]({% link {{ page.version.version }}/cockroach-start.md %}) after being fully decommissioned):
...
{{site.data.alerts.callout_info}}
All possible `OPS` event types are detailed in the [reference documentation]({% link {{ page.version.version }}/eventlog.md %}).
{{site.data.alerts.end}}

### HEALTH

The [`HEALTH`]({% link {{ page.version.version }}/logging.md %}#health) channel logs operational events initiated by CockroachDB or reported by automatic processes. These can include resource usage details, connection errors, [gossip](https://wikipedia.org/wiki/Gossip_protocol) status, [replication]({% link {{ page.version.version }}/architecture/replication-layer.md %}) events, and runtime statistics.

#### Example: Runtime stats

A [`runtime_stats`]({% link {{ page.version.version }}/eventlog.md %}#runtime_stats) event is recorded every 10 seconds to reflect server health:
...
This [`alter_table`]({% link {{ page.version.version }}/eventlog.md %}#alter_table) event shows an [`ALTER TABLE ... ADD FOREIGN KEY`]({% link {{ page.version.version }}/alter-table.md %}#add-constraint) schema change being initiated on a `movr.public.vehicles` table:
...
This [`finish_schema_change`]({% link {{ page.version.version }}/eventlog.md %}#finish_schema_change) event shows that the above schema change has completed:
...
{{site.data.alerts.callout_info}}
All possible `SQL_SCHEMA` event types are detailed in the [reference documentation]({% link {{ page.version.version }}/eventlog.md %}#sql-logical-schema-changes).
{{site.data.alerts.end}}
...
In this example configuration, the channels are grouped into a file sink called `security`. The combined logging output will be found in a `cockroach-security.log` file at the configured [logging directory]({% link {{ page.version.version }}/configure-logs.md %}#logging-directory).
...
The [`SESSIONS`]({% link {{ page.version.version }}/logging.md %}#sessions) channel logs SQL session events. This includes client connection and session authentication events, for which logging must be enabled separately. For complete logging of client connections, we recommend enabling both types of events.
...
These logs show a [`client_connection_start`]({% link {{ page.version.version }}/eventlog.md %}#client_connection_start) (client connection established) and a [`client_connection_end`]({% link {{ page.version.version }}/eventlog.md %}#client_connection_end) (client connection terminated) event over a `hostssl` (TLS transport over TCP) connection:
...
- Preceding the `=` character is the `crdb-v2` event metadata. See the [reference documentation]({% link {{ page.version.version }}/log-formats.md %}#format-crdb-v2) for details on the fields.
- The two [`client_authentication_info`]({% link {{ page.version.version }}/eventlog.md %}#client_authentication_info) events show the progress of certificate authentication. The `Info` fields show the progress of certificate validation.
- The [`client_authentication_ok`]({% link {{ page.version.version }}/eventlog.md %}#client_authentication_ok) event shows that certificate authentication was successful.
...
- Preceding the `=` character is the `crdb-v2` event metadata. See the [reference documentation]({% link {{ page.version.version }}/log-formats.md %}#format-crdb-v2) for details on the fields.
- The two [`client_authentication_info`]({% link {{ page.version.version }}/eventlog.md %}#client_authentication_info) events show the progress of certificate authentication. The `Info` fields show that password authentication was attempted, in the absence of a client certificate.
- The [`client_authentication_failed`]({% link {{ page.version.version }}/eventlog.md %}#client_authentication_failed) event shows that password authentication was unsuccessful. The `Detail` field shows the related error.
- The [`client_session_end`]({% link {{ page.version.version }}/eventlog.md %}#client_session_end) event shows that the SQL session was terminated. This would typically be followed by a [`client_connection_end`]({% link {{ page.version.version }}/eventlog.md %}#client_connection_end) event.
- `User` shows that the SQL session authentication was attempted for user `roach`.

{{site.data.alerts.callout_info}}
All possible `SESSIONS` event types are detailed in the [reference documentation]({% link {{ page.version.version }}/eventlog.md %}#sql-session-events). For more details on certificate and password authentication, see [Authentication]({% link {{ page.version.version }}/authentication.md %}).
{{site.data.alerts.end}}
...
This [`sensitive_table_access`]({% link {{ page.version.version }}/eventlog.md %}#sensitive_table_access) event shows that the audited table `customers` was accessed by user `root` issuing an [`INSERT`]({% link {{ page.version.version }}/insert.md %}) statement:
...
- `ApplicationName` shows that the event originated from the [`cockroach sql`]({% link {{ page.version.version }}/cockroach-sql.md %}) shell. You can use this field to filter the logging output by application.
...
This [`change_database_privilege`]({% link {{ page.version.version }}/eventlog.md %}#change_database_privilege) event shows that user `root` granted all privileges to user `roach` on the database `movr`:
...
{{site.data.alerts.callout_info}}
All possible `PRIVILEGE` event types are detailed in the [reference documentation]({% link {{ page.version.version }}/eventlog.md %}#sql-privilege-changes).
{{site.data.alerts.end}}

### USER_ADMIN

The [`USER_ADMIN`]({% link {{ page.version.version }}/logging.md %}#user_admin) channel logs changes to users and roles. This includes user and role [creation and assignment]({% link {{ page.version.version }}/security-reference/authorization.md %}#create-and-manage-users) and changes to [privileges]({% link {{ page.version.version }}/security-reference/authorization.md %}#managing-privileges), [options]({% link {{ page.version.version }}/create-role.md %}#parameters), and [passwords]({% link {{ page.version.version }}/authentication.md %}#client-authentication).

#### Example: SQL user creation

This [`create_role`]({% link {{ page.version.version }}/eventlog.md %}#create_role) event shows that a user `roach` was created and assigned a password by user `root`. Note that the password in the SQL statement is pre-redacted even if `redact` is set to `false` for the logging sink. For more details on redaction behavior, see [Redact logs]({% link {{ page.version.version }}/configure-logs.md %}#redact-logs).
...
- Preceding the `=` character is the `crdb-v2` event metadata. See the [reference documentation]({% link {{ page.version.version }}/log-formats.md %}#format-crdb-v2) for details on the fields.
- `ApplicationName` shows that the event originated from the [`cockroach sql`]({% link {{ page.version.version }}/cockroach-sql.md %}) shell. You can use this field to filter the logging output by application.
- `RoleName` shows the name of the user/role. For details on user and role terminology, see [Users and roles]({% link {{ page.version.version }}/security-reference/authorization.md %}#users-and-roles).

{{site.data.alerts.callout_info}}
All possible `USER_ADMIN` event types are detailed in the [reference documentation]({% link {{ page.version.version }}/eventlog.md %}#sql-user-and-role-operations).
{{site.data.alerts.end}}
...
In this example configuration, the channels are grouped into a file sink called `performance`. The combined logging output will be found in a `cockroach-performance.log` file at the configured [logging directory]({% link {{ page.version.version }}/configure-logs.md %}#logging-directory).
...
The [`SQL_EXEC`]({% link {{ page.version.version }}/logging.md %}#sql_exec) channel reports all SQL executions on the cluster, when enabled.
...
{{site.data.alerts.callout_info}}
All possible `SQL_EXEC` event types are detailed in the [reference documentation]({% link {{ page.version.version }}/eventlog.md %}#sql-execution-log).
{{site.data.alerts.end}}

### SQL_PERF

The [`SQL_PERF`]({% link {{ page.version.version }}/logging.md %}#sql_perf) channel reports slow SQL queries, when enabled. This includes queries whose latency exceeds a configured threshold, as well as queries that perform a full table or index scan.

To enable slow query logging, enable the `sql.log.slow_query.latency_threshold` [cluster setting]({% link {{ page.version.version }}/cluster-settings.md %}) by setting it to a non-zero value. This will log queries whose service latency exceeds a specified threshold value. The threshold value must be specified with a unit of time (e.g., `500ms` for 500 milliseconds, `5us` for 5 nanoseconds, or `5s` for 5 seconds). A threshold of `0s` disables the slow query log.
...
To log all queries that perform full table or index scans to `SQL_PERF`, regardless of query latency, set the `sql.log.slow_query.experimental_full_table_scans.enabled` [cluster setting]({% link {{ page.version.version }}/cluster-settings.md %}) to `true`.
...
Each gateway node will now record queries that take longer than 100 milliseconds to the `SQL_PERF` channel as [`slow_query` events]({% link {{ page.version.version }}/eventlog.md %}#slow_query).
...
The following query was logged with a service latency (`age`) of 9329.26 milliseconds, a very high latency that resulted from a [transaction retry error]({% link {{ page.version.version }}/transaction-retry-error-reference.md %}#retry_write_too_old):
...
In this case, defining separate `ops` and `security` network sinks allows us to:
 ...
 - [Enable redaction]({% link {{ page.version.version }}/configure-logs.md %}#redact-logs) on the `ops` logs.
...
By default, `fluent-servers` log messages use the [`json-fluent-compact`]({% link {{ page.version.version }}/log-formats.md %}#format-json-fluent-compact) format for ease of processing over a stream.

For example, this JSON message found in the `OPS` logging channel contains a [`node_restart`]({% link {{ page.version.version }}/eventlog.md %}#node_restart) event. The event shows that a node has rejoined the cluster after being offline (e.g., by being [restarted]({% link {{ page.version.version }}/cockroach-start.md %}) after being fully decommissioned):
...
- `sev` shows that the message has the `INFO` [severity level]({% link {{ page.version.version }}/logging.md %}#logging-levels-severities).
- `event` contains the fields for the structured [`node_restart`]({% link {{ page.version.version }}/eventlog.md %}#node_restart) event.
  - `NodeID` shows that the restarted node is `1`.
  - `StartedAt` shows the timestamp when the node was most recently restarted.
  - `LastUp` shows the timestamp when the node was up before being restarted.

See the [reference documentation]({% link {{ page.version.version }}/log-formats.md %}#format-json-fluent-compact) for details on the remaining JSON fields.
...
- [Use Prometheus and Alertmanager]({% link {{ page.version.version }}/monitor-cockroachdb-with-prometheus.md %})
