...
<code>statistics -> kvCPUTimeNanos -> [mean&#124;sqDiff]</code> | `NumericStat` | The KV CPU time (in nanoseconds) spent executing during query execution. This can be thought of as [KV]({% link {{ page.version.version }}/architecture/overview.md %}#layers) work that is on the critical path of serving the query. It does not include any asynchronous replication related work in the [storage layer]({% link {{ page.version.version }}/architecture/storage-layer.md %}).
