...
SIDEBAR_HTML_PATH = DOCS_ROOT / "_internal" / "sidebar-v22.2.html"

TARGET_VERSION = "v22.2"
...
class OfflineArchiver:
    ...
    def load_sidebar(self):
        ...
        if SIDEBAR_HTML_PATH.exists():
            ...
            alt_paths = [
                DOCS_ROOT / "_internal" / "sidebar-v22.2.html",
                SITE_ROOT / "_internal" / "sidebar-v22.2.html",
            ]
        ...
        if self.sidebar_html:
            ...
            # Simplify isVersionDirectory function for v2.1 only
            self.sidebar_html = re.sub(
                r'isVersionDirectory:\s*function\s*\([^}]*\{[^}]*\}',
                'isVersionDirectory: function (d) { return d === "v2.1" || d === "stable"; }',
                self.sidebar_html
            )
            ...
            if comprehensive_sidebar_js and len(comprehensive_sidebar_js) > 100:
                ...
                if broken_line in comprehensive_sidebar_js:
                    ...
                    if (url === '/' || url === '' || url === 'index' || url === 'index.html') {
                        ...
                        url = url.replace(/^stable\\//, 'v2.1/').replace(/\\/stable\\//, '/v2.1/');
    ...
    def copy_selective_assets(self):
        """Copy only necessary assets, excluding non-target version assets"""
        ...
        # Handle images selectively - include target version, v2.1 (legacy), and global images
        ...
        if images_src.exists():
            ...
            for img_file in images_src.rglob("*"):
                if img_file.is_file():
                    ...
                    # Skip version-specific images except TARGET_VERSION and v2.1 (legacy)
                    path_parts = rel_path.parts
                    if (len(path_parts) > 0 and
                        path_parts[0].startswith('v') and
                        path_parts[0] != TARGET_VERSION and
                        path_parts[0] not in ['v2.1']):  # v2.1 images are always included for legacy support
                        skipped_count += 1
                        continue
    ...
    def fix_sidebar_javascript(self, html):
        ...
        offline_snap_path = ""  # Use relative paths for portability
        html = re.sub(
            r'baseUrl:\s*["\'][^"\']*["\']',
                ...
                if (url === '/' || url === '' || url === 'index' || url === 'index.html') {
                    ...
                    if (pathMatch) {
                        ...
                        for (var i = pathParts.length - 2; i >= 0; i--) {
                            if (pathParts[i] === 'cockroachcloud' || pathParts[i] === 'v2.1' || 
                    ...
                    if (url.startsWith('releases/') || url.startsWith('cockroachcloud/') || url.startsWith('advisories/')) {
                        // These should go up from v2.1 directory to the root level
                        if (currentDir === 'v2.1') {
                            url = '../' + url;
                        }
                    }
...
code, pre { font-family: Consolas, Monaco, "Courier New", monospace; }"""
    ...
    def build(self):
        ...
        self.log("✅ Filtered out non-v2.1 version links (v25.1, v24.x, etc.)", "SUCCESS")
