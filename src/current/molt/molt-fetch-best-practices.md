...
- MOLT Fetch buffers data in memory before flushing to files or the target database, regardless of which [data path]({% link molt/molt-fetch.md %}#define-intermediate-storage) you use. To prevent memory outages during [data export]({% link molt/molt-fetch.md %}#data-export-phase), estimate the amount of memory used to export tables:

    ~~~
    average size of each row * --row-batch-size * --export-concurrency * --table-concurrency
    ~~~

    If you are exporting multiple tables concurrently (i.e., [`--table-concurrency`]({% link molt/molt-fetch-commands-and-flags.md %}#global-flags) is set higher than `1`), estimate the memory requirement using the table with the largest average row size. For details on how concurrency and sharding interact, refer to [Table sharding]({% link molt/molt-fetch.md %}#shard-tables-for-concurrent-export).
    ...
    - Adjust [`--row-batch-size`]({% link molt/molt-fetch-commands-and-flags.md %}#global-flags), [`--export-concurrency`]({% link molt/molt-fetch-commands-and-flags.md %}#global-flags), or [`--table-concurrency`]({% link molt/molt-fetch-commands-and-flags.md %}#global-flags) if memory is constrained.
