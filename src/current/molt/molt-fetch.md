...
- If a PostgreSQL database is set as a [source](#source-and-target-databases), ensure that [`idle_in_transaction_session_timeout`](https://www.postgresql.org/docs/current/runtime-config-client.html#GUC-IDLE-IN-TRANSACTION-SESSION-TIMEOUT) on PostgreSQL is either disabled or set to a value longer than the duration of the Fetch task. Otherwise, the connection will be prematurely terminated. To estimate the Fetch duration, you can [perform a dry run](#perform-a-dry-run) and observe the `molt_fetch_overall_duration` metric.
