...
|                         Flag                         |                                                                                                                                                                                                                                                                                                            Description                                                                                                                                                                                                                                                                                                            |
|------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `--source`                                           | (Required) Connection string for the source database. In Oracle migrations, this specifies a PDB or single database, depending on the Oracle architecture. For details, refer to [Source and target databases](#source-and-target-databases).                                                                                                                                                                                                                                                                                                                                                                                     |
| `--source-logminer`                                  | Connection string for an Oracle CDB or single database and LogMiner user, used to query redo logs from the Oracle database during replication from Oracle. For details, refer to [Source and target databases](#source-and-target-databases).                                                                                                                                                                                                                                                                                                                                                                                     |
...
In Oracle migrations, the `--source` connection string specifies an Oracle PDB (in [multi-tenant databases](https://docs.oracle.com/en/database/oracle/oracle-database/21/cncpt/CDBs-and-PDBs.html)) or single database. The `{username}` corresponds to the owner of the tables you will migrate.

#### `--source-logminer`

`--source-logminer` is only required for replication from an Oracle database. It **must** be included along with `--source`:

{% include_cached copy-clipboard.html %}
~~~
--source 'oracle://{username}:{password}@{host}:{port}/{service_name}'
--source-logminer 'oracle://{logminer_user}:{password}@{host}:{port}/{container_service}'
~~~

The `--source-logminer` connection string specifies an Oracle CDB (in [multi-tenant databases](https://docs.oracle.com/en/database/oracle/oracle-database/21/cncpt/CDBs-and-PDBs.html)) or single database. The `{logminer_user}` must have `LOGMINING` and supplemental log privileges on the container database (CDB) or single-tenant database. MOLT Fetch uses this connection to query Oracle redo logs for change replication.

{{site.data.alerts.callout_info}}
The users specified in `--source` and `--source-logminer` are always different users.
{{site.data.alerts.end}}
