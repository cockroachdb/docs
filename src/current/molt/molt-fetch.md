...
| `--bucket-path`                                      | The path within the [cloud storage](#cloud-storage) bucket where intermediate files are written (e.g., `'s3://bucket/path'` or `'gs://bucket/path'`). Only the URL path is used; query parameters (e.g., credentials) are ignored. However, the `AWS_REGION` query parameter in an `s3` URL will be used if you also include the `--import-region` flag.                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `--import-region`                                    | The region of the [cloud storage](#cloud-storage) bucket. This applies only to [Amazon S3 buckets](#cloud-storage) when using [`IMPORT INTO`](#data-movement) for data movement. **Required** when `--bucket-path` specifies an `s3` URL that uses `AWS_REGION`. For example, if the `s3` URL has `AWS_REGION=ap-south-1`, include `--import-region=ap-south-1`.                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
