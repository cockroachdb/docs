...
- If you plan to use continuous replication (using either [`--ongoing-replication`](#load-data-and-replicate-changes) or the [CDC cursor](#cdc-cursor)):
...
| `--ongoing-replication`                       | Enable continuous [replication](#load-data-and-replicate-changes) to begin after the fetch process succeeds (i.e., initial source data is loaded into CockroachDB).                                                                                                                                                                                                                                                                                                                           |
...
| `--replicator-flags`                          | If continuous [replication](#load-data-and-replicate-changes) is enabled with `--ongoing-replication`, specify replication flags ([PostgreSQL](https://github.com/cockroachdb/replicator/wiki/PGLogical#postgresql-logical-replication) or [MySQL](https://github.com/cockroachdb/replicator/wiki/MYLogical#mysqlmariadb-replication)) to override.                                                                                                                                           |
...
Before using this option, the source PostgreSQL or MySQL database **must** be configured for continuous replication, as described in [Setup](#setup).
{{site.data.alerts.end}}
