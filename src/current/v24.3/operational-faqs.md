...
[Drain a node]({% link {{ page.version.version }}/drain-a-node.md %}) to temporarily stop the `cockroach` process on the node. When maintenance is complete and you restart the `cockroach` process, the node will rejoin the cluster.
