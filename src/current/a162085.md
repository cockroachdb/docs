...
advisory_date: <date the advisory is published externally>
docs_area: releases
---
...
Users of affected CockroachDB versions are encouraged to upgrade to v23.2.29, v24.1.26, v24.3.27, v25.2.13, v25.4.5, v26.1.0, or a later version.

### Interim mitigation

If an upgrade cannot be performed immediately, the issue can be mitigated by disabling the ClearRange optimization:

SET CLUSTER SETTING kv.gc.clear\_range\_min\_keys \= 0;
...
SET enable\_inspect\_command \= true;
...
SELECT count(\*) FROM my\_table@my\_table\_pkey WHERE pk \= 1;
SELECT count(\*) FROM my\_table@my\_table\_a\_idx WHERE pk \= 1 AND a \= 2;
COMMIT;
