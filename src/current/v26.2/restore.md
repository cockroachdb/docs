...
<a name="grants"></a>`GRANTS` | N/A | When `RESTORE` runs with `GRANTS`, users in the restore target cluster receive the same grants as their corresponding users in the restoring cluster. If a restore uses `GRANTS` along with `new_db_name`, the new database inherits the privileges in the backed up database.
