...
- {% include_cached new-in.html version="v26.2" %} `kvadmission.store.snapshot_ingest_bandwidth_control.min_rate.enabled` to ensure snapshot ingestion is admitted at a minimum rate when `kvadmission.store.snapshot_ingest_bandwidth_control.enabled` is active and `kvadmission.store.provisioned_bandwidth` is set to a non-zero value. Disabling this setting can allow snapshot ingestion to be starved by other elastic work. This cluster setting is in [Preview]({% link {{ page.version.version }}/cockroachdb-feature-availability.md %}#features-in-preview).
