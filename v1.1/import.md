...
{{site.data.alerts.callout_danger}}<strong>This is an experimental feature</strong>. To enable it, you must <a href="set-cluster-setting.html"><code>SET CLUSTER SETTING experimental.importcsv.enabled = true</code></a>{{site.data.alerts.end}}
...
One of the following directories on the processing node must have enough available storage equal to or greater than the size of the file you're importing:

- If explicitly defined in, your node's `temp` directory
- Otherwise, the node's first-listed/default [`store`](start-a-node.html#store) directory
