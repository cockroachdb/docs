...
To enable transactions that can span your entire cluster (including cross-range and cross-table transactions), CockroachDB optimizes correctness through a somewhat-pessimistic concurrency control system.
...
It's also important to realize that this Transaction Record is also a KV entry, so it benefits from [replication](replication-layer.html), ultimately letting a transaction continue even if a node containing it fails.
...
Though CockroachDB is a distributed, highly available database, if a the gateway node dies during a transaction, the operation will time out and the client will have to re-send the transaction to the cluster.
