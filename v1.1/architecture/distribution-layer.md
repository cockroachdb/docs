...
By using a sorted (rather than hashed) map, CockroachDB enables efficient scans; by ordering data, scanning over ranges of data is both simple and fast.
...
Nodes also cache values of the `meta1` and `meta2` ranges it has accessed before, which optimizes access of that data in the future. Whenever a node discovers that a cached value has become invalid, the cache is updated by performing a regular read on the `meta2` ranges.
...
- **Keys** are the table's indexed columns. This includes your table `PRIMARY KEY` or explicitly indexed columns, such as those identified through `CREATE INDEX`.
- **Values** are the non-indexed/stored columns. For the table's data, this includes all of the non-`PRIMARY KEY` columns. In explicitly created indexes, this includes columns identified in the `STORING`/`COVERING` clause.

### Using the Monolithic Sorted Map

To handle client requests for ranges of data they don't contain, nodes need to route the request to the range's "[Leaseholder](replication-layer.html#leases)." This is a node elected to handle reads and writes for the range, but there is nothing intrinsically special about it.

To find the Leaseholder among all nodes in the cluster, nodes use the monolithic sorted map.
...
To send KV operations between nodes, [`client.Txn`](#client-txn-and-txncoordsender) bundles them into a protobuf known as a `BatchRequest`. Each `BatchRequest` also includes its destination, as well as a pointer to the request's Transaction Record. (On the other side, when a node is replying to a `BatchRequest`, it uses a protobuf––`BatchResponse`.)
