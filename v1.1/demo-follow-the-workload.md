...
This command causes a 100 millisecond delay for all requests on the loopback interface of your local workstation. It will only effect connections from the machine to itself, not to/from the Internet.
...
1. In a new terminal, start `kv`, pointing it at port `26259`, which is the port of the node with the `us-east` locality:
    ...
    This command initiates 32 concurrent read-only workloads for 1 minute but limits each worker to 100 operations per second (since you're running everything on a single machine). While `kv` is running, it will print some stats to the terminal:
    ...
    {{site.data.alerts.callout_info}}The latency numbers printed reflect the 100 millisecond delay caused by the <code>comcast</code> tool. Normally, these numbers would be much lower.{{site.data.alerts.end}}
