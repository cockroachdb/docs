...
Before starting the upgrade, it's important to verify the cluster's health and version, collect debug information, and back up the cluster's data.

1. Run the [`cockroach node status`](view-node-details.html) command with the `--decommission` flag against any node in the cluster.
...
2. Run the [`cockroach node status`](view-node-details.html) command with the `--ranges` flag against any node in the cluster.
...
3. Run the [`cockroach debug zip`](debug-zip.html) command against any node in the cluster to capture your cluster's state. If the upgrade does not go according to plan, the captured details will help you and Cockroach Labs troubleshoot issues.
...
2. Start the [`cockroach sql`](use-the-built-in-sql-client.html) shell against any node in the cluster and execute the following query:
    ...
    {{site.data.alerts.callout_info}}Once this is done, it will no longer be possible to perform a rolling downgrade to 1.0. In the event of a catastrophic failure or corruption due to usage of new features requiring 1.1, the only option is to start a new cluster using the old binary and then restore from one of the backups created prior to finalizing the upgrade.{{site.data.alerts.end}}
