...
`<database>` | A database name to use as current database in the new session; optional.
...
`--database`<br>`-d` | A database name to use as current database in the new session.<br><br>**Env Variable:** `COCKROACH_DATABASE`<br>**Default:** empty string.
