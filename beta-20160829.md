...
- [Mac](https://d3a3ys8r4slo6p.cloudfront.net/cockroach-beta-20160829.darwin-10.9-amd64.tgz)
- [Linux](https://d3a3ys8r4slo6p.cloudfront.net/cockroach-beta-20160829.linux-amd64.tgz)
...
- The [`cockroach zone set`](configure-replication-zones.html) command no longer takes its input directly on the command line. Instead, it accepts a `--file` flag to read from a file, or `--file=-` to read from standard input. [#7953](https://github.com/cockroachdb/cockroach/pull/7953)
...
- [Foreign keys](constraints.html#foreign-keys) can now use a prefix of an index. [#8059](https://github.com/cockroachdb/cockroach/pull/8059)
...
- The `repeat()` and `substr()` [functions](functions-and-operators.html) with very large numeric arguments will no longer crash the server as easily. [#8073](https://github.com/cockroachdb/cockroach/pull/8073), [#8078](https://github.com/cockroachdb/cockroach/pull/8078)
...
- The new [SQL Feature Support](sql-feature-support.html) page explains which standard SQL features are supported by CockroachDB. [#550](https://github.com/cockroachdb/docs/pull/550)
- The new [Features on Develop Branch](develop-branch.html) page tracks features that are available on the `develop` branch but not yet on `master`. [#552](https://github.com/cockroachdb/docs/pull/552) 
- Added docs on [foreign key constraints](constraints.html#foreign-keys). [#528](https://github.com/cockroachdb/docs/pull/528)
- Added docs on the [`TRUNCATE`](truncate.html) statement. [#542](https://github.com/cockroachdb/docs/pull/542)
...
- Fixed errors in the commands for [starting CockroachDB in Docker](start-a-local-cluster-in-docker.html). [#567](https://github.com/cockroachdb/docs/pull/567)
