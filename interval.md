...
SQL Standard | `INTERVAL 'Y-M-D H:M:S'`<br><br>For `Y-M-D`, using one value defines days only, and using two values defines years and months. For `H:M:S`, using one value defines seconds only, and using two values defines hours and minutes. Note that all values can be integers or floats.
