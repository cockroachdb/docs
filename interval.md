...
SQL Standard | `INTERVAL 'Y-M-D H:M:S'`<br><br>Using one value on the left defines days only, whereas using two values on the left defines years and months. Using one value on the right defines seconds only, whereas using two values on the right defines hours and minutes. Note that each side is optional and all values can be integers or floats.
