We recommend starting backups from a time at least 10 seconds in the past using [`AS OF SYSTEM TIME`](../{{site.versions["cloud"]}}/as-of-system-time.html). Read our guidance in the [Performance](../{{site.versions["cloud"]}}/backup.html#performance) section on the [`BACKUP`](../{{site.versions["cloud"]}}/backup.html) page.
...
{{site.data.alerts.callout_info}}
When backing up from a cluster and restoring a database or table that is stored in your `userfile` space to a different cluster, you can run [`cockroach userfile get`](../{{site.versions["cloud"]}}/cockroach-userfile-get.html) to download the backup files to a local machine and [`cockroach userfile upload --url {CONNECTION STRING}`](../{{site.versions["cloud"]}}/cockroach-userfile-upload.html) to upload to the `userfile` of the alternate cluster.
{{site.data.alerts.end}}
...
To restore from the most recent backup, use [`RESTORE FROM LATEST IN ...`](../{{site.versions["cloud"]}}/restore.html#restore-the-most-recent-backup).
...
If you use `cockroach userfile delete {file}`, it will take as long as the [garbage collection](../{{site.versions["cloud"]}}/configure-replication-zones.html#gc-ttlseconds) to be removed from disk.
