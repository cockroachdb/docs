...
{{site.data.alerts.callout_danger}}While setting memory limits is strongly recommended, [setting CPU limits can hurt tail latencies as currently implemented by Kubernetes](https://github.com/kubernetes/kubernetes/issues/51135). We recommend not setting CPU limits on Kubernetes versions 1.9 and below, and only doing so on Kubernetes 1.10 if you use an integer (non-fractional) number of CPUs and set your CPU and memory limits exactly equal to your requests in order for the [Static CPU Management Policy](https://kubernetes.io/docs/tasks/administer-cluster/cpu-management-policies/#static-policy) to kick in.{{site.data.alerts.end}}
