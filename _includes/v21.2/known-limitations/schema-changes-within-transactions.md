...
- `DROP COLUMN` can result in data loss if one of the other schema changes in the transaction fails or is canceled. To work around this, move the `DROP COLUMN` statement to its own transaction. See [tracking issue](https://github.com/cockroachdb/cockroach/issues/46541).
