...
- [`BYTES`](bytes.html) (or its aliases `BYTEA` and `BLOB`) is often used to store machine-readable data. When you stream these types through a changefeed with [`format=avro`](create-changefeed.html#format), CockroachDB does not encode or change the data. However, Avro clients can often include escape sequences so the data is in a printable format, which can interfere with deserialization. A potential solution is to hex-encode `BYTEA` values when initially inserting them into CockroachDB. This will ensure that you can consistently decode the hexadecimal. Note that this process will increase record size.
