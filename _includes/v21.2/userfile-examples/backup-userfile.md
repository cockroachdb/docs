...
{{site.data.alerts.callout_info}}
When backing up from a cluster and restoring a database or table that is stored in your `userfile` space to a different cluster, you can run [`cockroach userfile get`](cockroach-userfile-get.html) to download the backup files to a local machine and [`cockroach userfile upload -r path/to/dir userfile/destination --url {CONNECTION STRING}`](cockroach-userfile-upload.html#upload-a-directory-recursively) to upload to the `userfile` of the alternate cluster.
{{site.data.alerts.end}}
