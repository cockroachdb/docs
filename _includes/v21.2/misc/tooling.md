...
| Go | [pgx](https://github.com/jackc/pgx/releases) | {% remote_include https://raw.githubusercontent.com/cockroachdb/cockroach/master/pkg/cmd/roachtest/tests/pgx.go |var supportedPGXTag = "|"\n\n  %}<br>(use latest version of CockroachDB adapter) | Full | [`crdbpgx`](https://pkg.go.dev/github.com/cockroachdb/cockroach-go/crdb/crdbpgx)<br>(includes client-side transaction retry handling) | [Build a Go App with CockroachDB (pgx)](build-a-go-app-with-cockroachdb.html) |
