...
Encryption is done using [AES-256-GCM](https://en.wikipedia.org/wiki/Galois/Counter_Mode), and GCM is used to both encrypt and authenticate the files. A random per-file [salt](https://en.wikipedia.org/wiki/Salt_(cryptography)) is used to derive per-file [AES](https://en.wikipedia.org/wiki/Advanced_Encryption_Standard) keys from the specified passphrase.
