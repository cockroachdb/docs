...
Because this queueing happens during the read operation, the thrashing that would otherwise occur when multiple concurrently executing transactions attempt a read-modify-write workflow is prevented.  In addition, [transaction retries][retries] are prevented.
