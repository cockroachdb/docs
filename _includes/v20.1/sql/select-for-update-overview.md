...
Because this queueing happens during the read operation, the thrashing that would otherwise occur (when multiple concurrently executing transactions attempt to `SELECT` the same data and then `UPDATE` the results of that selection) is prevented.  By preventing this thrashing, CockroachDB also prevents the [transaction retries][retries] that would otherwise occur.
