...
- Added `payloads_for_span` built-in that takes in a span ID and returns its paylods in `JSONB` format. If the span is not found, or if the span does not have any payloads, the built-in returns an empty JSON object. [#60616][#60616] {% comment %}doc{% endcomment %}
