...
| Core | Support for `WITH GRANT OPTION` privilege | See [Security](#v22-1-0-database-operations). |
...
| Core | Per-replica circuit breakers | When individual ranges become temporarily unavailable, requests to those ranges are refused by a [per-replica "circuit breaker" mechanism](../v22.1/replication-layer.html#per-replica-circuit-breakers) instead of hanging indefinitely. |
