...
- Bucket-count of [hash-sharded index](../v22.1/hash-sharded-indexes.html) is now shown from the `crdb_internal.table_indexes` table. [#78625][#78625]
...
- Fixed a bug where the `LATEST` file that points to the latest full backup in a collection was written to a directory path with the wrong structure. [#78281][#78281]
