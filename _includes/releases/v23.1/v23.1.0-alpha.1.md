...
- CockroachDB now supports using [`DROP CONSTRAINT, ADD CONSTRAINT`](../v22.2/drop-constraint.html) to add a new primary key without moving the old primary key to a secondary index if the primary key name is a reserved SQL keyword. Previously, a `constraint already exists` error was returned. [#90865][#90865]
...
- Index recommendations were added to the **Statement Active Execution Details** page, and the plan gist was addded as the first line of the explain plan. [#91629][#91629]
...
- Added a top-level dropdown in the DB Console which lists tenants the user has logged into. If the cluster is not a multitenant host, the dropdown is not displayed. [#92694][#92694]
...
- CockroachDB now excludes check constraints of hash-shared indexes from being unvalidated when executing `IMPORT INTO`. [#89231][#89231]
...
- During JWT based auth, CockroachDB now infers the algorithm type if it is not specified by the JWKS. This enables support for a wider range of JWKSes. [#89989][#89989]
...
- The GC score triggering a mvcc GC run has been lowered. The GC Score is a ratio computed from mvcc stats and uses the ratio of live objects and estimated garbage age to estimate collectability of old data. The reduced score will trigger garbage collection earlier, lowering the interval between runs 3 times, giving 2 times reduced peak garbage usage at the expense of 30% increase of wasteful data scanning on constantly updated data. [#92118][#92118]
