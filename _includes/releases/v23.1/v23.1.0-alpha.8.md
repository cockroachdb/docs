...
- Normalized CPU usage will now show on [Node Map](../v23.1/ui-cluster-overview-page.html#node-map-enterprise). [#98225][#98225]

<h3 id="v23-1-0-alpha-8-sql-language-changes">SQL language changes</h3>

- Fixed a bug where the check constraint on an `OID` type column panics in the legacy schema changer. [#98800][#98800]
- Added a new `WITH REDACT` option to the following statements: [`SHOW CREATE`](../v23.1/show-create.html), [`SHOW CREATE TABLE`](../v23.1/show-create.html#show-the-create-table-statement-for-a-table), and [`SHOW CREATE VIEW`](../v23.1/show-create.html#show-the-create-view-statement-for-a-view)  which, when used, replaces constants and literals in the printed `CREATE` statement with the redacted marker, `‹×›`. [#98251][#98251]
...
- Added a `sql.pgwire.multiple_active_portals.enabled` setting. This setting is only for a `PREVIEWABLE` feature. With it set to `true`, all non-internal portals with read-only [`SELECT`](../v23.1/selection-queries.html) queries without sub/post queries can be paused and resumed in an interleaving manner, but are executed with a local plan. [#99052][#99052]
- Added stemming and stopword eliminating text search configurations for English, Danish, Dutch, Finnish, French, German, Hungarian, Italian, Norwegian, Portuguese, Russian, Spanish, Swedish, and Turkish. [#97677][#97677]
...
- Fixed the helper message on [`UPDATE`](../v23.1/update.html) to correctly position the optional from the cause. [#99301][#99301]
...
- Some KV server [cluster settings](../v23.1/cluster-settings.html)  are now system only. These settings could previously be written or read by tenants, however writing to these settings have no effect. [#98353][#98353]
...
- New data is not auto fetched every 5m on the [Statement & Transaction Fingerprints](../v23.1/ui-statements-page.html#statement-fingerprint-page) pages. [#98331][#98331]
- Updated the column selector icon to show the gear icon and "Columns". [#98826][#98826]
...
- Timeseries metrics in [DB Concole](../v23.1/ui-overview.html) will now show tenant-specific metrics. For the system tenant, these timeseries will be aggregated across all tenants. For a secondary tenant, only metrics belonging to that particular tenant will be shown. [#98077][#98077]
...
- Data on the [Statement Fingerprint Details](../v23.1/ui-statements-page.html#statement-fingerprints-view) page will now not update automatically every 5mins. [#99298][#99298]
...
- Increased the TestParsingErrorHandling metric to `500ms` to allow the file to re-read before checking the value of the error metrics. [#99050][#99050] 
- Added logic to decide between returning a default test tenant, if one was started, or the server to `testserver`. [#98844][#98844]
- Disabled starting the default test tenant for explicit tenant tests. [#98758][#98758] 
...
- Added a check so percentiles are never greater than the `max_latency` value. [#99294][#99294]
