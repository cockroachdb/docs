...
- Users can grant a new `EXTERNALIOIMPLICITACCESS` system privilege that allows a user to interact with an external storage resource that has [implicit authentication](../v22.2/use-cloud-storage-for-bulk-operations.html#authentication). E.g., `gs`, `s3`, `nodelocal`, etc. Previously, this was an admin only operation. [#87066][#87066]
