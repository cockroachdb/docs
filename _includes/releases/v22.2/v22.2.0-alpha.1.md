...
- Removed the `cockroach quit` command. It has been deprecated since v20.1. To [shut down a node](../v22.2/node-shutdown.html) gracefully, send a `SIGTERM` signal to it. [#82988][#82988]
- Added a cluster version to allow the [Pebble storage engine](../v22.2/architecture/storage-layer.html#pebble) to recombine certain SSTables (specifically, user keys that are split across multiple files in a level of the [log-structured merge-tree](../v22.2/architecture/storage-layer.html#log-structured-merge-trees)). Recombining the split user keys is required for supporting the range keys feature. The migration to recombine the SSTables is expected to be short (split user keys are rare in practice), but will block subsequent migrations until all tables have been recombined. The `storage.marked-for-compaction-files` time series metric can show the progress of the migration. [#84887][#84887]
...
- Changefeed restarts and changefeeds started with the [`cursor`](../v22.2/create-changefeed.html#cursor-option) option are now more efficient by using the catchup scan progress checkpoint. [#77763][#77763]
- Storage and KMS URIs for Google Cloud Storage in [`BACKUP`](../v22.2/backup.html) and [`RESTORE`](../v22.2/restore.html) now accept an `ASSUME_ROLE` parameter, which informs the current service account authenticated by either [implicit or specified credentials](../v22.2/use-cloud-storage-for-bulk-operations.html#authentication) to obtain temporary credentials for the service account specified by the `ASSUME_ROLE` parameter in order to access the resource specified by the URI. [#80417][#80417]
- The [`CREATE CHANGEFEED`](../v22.2/create-changefeed.html) statement now supports general expressions: predicates and projections. Projections allow users to emit specific columnar data, including computed columns. While predicates (i.e., filters) allow users to restrict the data that emits to only those events that match the filter. For example: `CREATE CHANGEFEED INTO 'kafka://' AS SELECT * FROM t WHERE NOT cdc_is_delete()`. [#82562][#82562]
- Added replanning functionality for [changefeeds](../v22.2/change-data-capture-overview.html) when topology changes by adding a new replanning counter metric. This functionality is only supported for {{ site.data.products.serverless }} clusters.. [#83143][#83143]
