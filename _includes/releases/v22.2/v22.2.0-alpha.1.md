...
- Users can now pass [locality-aware backup](../{{site.versions["dev"]}}/take-and-restore-locality-aware-backup.html) URIs to [`SHOW BACKUP`](../{{site.versions["dev"]}}/show-backup.html). This change only affects `SHOW BACKUP` with the new syntax: e.g.,  `SHOW BACKUP FROM LATEST IN (<collectionURI>, <localityURI1>, <localityURI2>)`. Users cannot run `SHOW BACKUP` for locality-aware backups created using the `incremental_location` parameter. [#79121][#79121]
...
- Added the [cluster setting](../{{site.versions["dev"]}}/cluster-settings.html) `sql.metrics.statement_details.index_recommendation_collection.enabled` that can be disabled if index recommendation generation is causing performance issues. [#84282][#84282]
...
- Users can now `CREATE EXTERNAL CONNECTION` to represent an `aws-kms` scheme that represents an [Amazon S3 KMS resource](../{{site.versions["dev"]}}/take-and-restore-encrypted-backups.html). [#86402][#86402]
...
- Fixed a bug that caused internal errors in rare cases when performing `DELETE`s on a table that had [foreign key](../{{site.versions["dev"]}}/foreign-key.html) references to it with the `ON DELETE CASCADE` option. For example, imagine tables `a` and `b` already exist, and `b` has a foreign key `ON DELETE CASCADE` column referencing `a`. If table `c` is added with a foreign key `ON DELETE CASCADE` column referencing table `b` and a `DELETE` statement is performed on table `a` in the same transaction, and internal error could occur. This bug has been present since v21.1.0. [#84219][#84219]
