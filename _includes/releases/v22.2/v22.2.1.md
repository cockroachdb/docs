...
- Changefeeds can now emit files [compressed](../v22.2/create-changefeed.html#compression) with the `zstd` algorithm, which provides good compression, and is much faster than `gzip`. In addition, a new faster implementation of `gzip` is used by default. [#91002][#91002]
...
- Fixed a bug that could potentially cause changefeeds with [`initial_scan_only`](../v22.2/create-changefeed.html#initial_scan) to miss messages. Now, the changefeed ensures that all messages have successfully flushed to the sink prior to completion. [#90277][#90277]
