...
- Introduced the `server.user_login.downgrade_scram_stored_passwords_to_bcrypt.enabled` [cluster setting](../v22.2/cluster-settings.html) , which defaults to `true`. If it is `true` **and** `server.user_login.password_encryption` is `crdb-bcrypt`, then during login, the stored hashed password will be converted from [`SCRAM`](../v22.2/security-reference/scram-authentication.html) to `bcrypt`. Previously, the process to change passwords from `SCRAM` hashing to `bcrypt` hashing was always manual and took a long time. An operator may wish to use this cluster setting if their tools do not support `SCRAM`. [#97628][#97628]
...
- Changefeeds no longer require the `COCKROACH_EXPERIMENTAL_ENABLE_PER_CHANGEFEED_METRICS` environment variable in order to use [`metrics_label`](../v22.2/monitor-and-debug-changefeeds.html#using-changefeed-metrics-labels) option. [#97509][#97509]
...
- Increased the default value of `sql.stats.cleanup.rows_to_delete_per_txn` to 10k, to increase efficiency of the cleanup job for SQL statistics. [#97724][#97724]
...
- Previously, when the SQL API returned a "max size reached" error, the DB Console was only showing the error, but not the data that was also being returned. Now, the Statement Insights, Transaction Insights, and Schema Insights pages also show the data. [#97470][#97470]
- Added the following new metrics to track memory usage of prepared statements in sessions: 
    ...
    - `sql.mem.sql.session.prepared.max-p99.999` [#97654][#97654]
...
- Long chains of [incremental backups](../v22.2/take-full-and-incremental-backups.html) and restore of such chains will now allocate less memory during the un-marshaling of metadata. [#97210][#97210]
