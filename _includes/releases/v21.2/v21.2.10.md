...
- `Nextval`/`setval` are non-transactional except when it is called in the same transaction that the sequence was created in. This change prevents a bug where creating a sequence and calling nextval/setval on it within a transaction caused the query containing nextval to hang. [#79866][#79866]
