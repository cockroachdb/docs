...
2. Create [RBAC resources](https://docs.helm.sh/using_helm/#role-based-access-control) for granting the Helm client access to the Helm server:

    1. Create a `rbac-config.yaml` file defining a role and service account:
...
5. Confirm that three pods are `Running` successfully. Note that they will not be considered `Ready` until after the cluster has been initialized:
