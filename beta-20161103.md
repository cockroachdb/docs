...
- [`SHOW TABLES`](show-tables.html) now adds a `(dropped)` suffix to tables that are being dropped. [#10063](https://github.com/cockroachdb/cockroach/pull/10063)
- [`SHOW CREATE VIEW`](show-create-view.html) now only requires permissions on the view, not the underlying tables. [#10270](https://github.com/cockroachdb/cockroach/pull/10270)

### Command-Line Interface Changes

- When entering a transaction in the [built-in sql shell](use-the-built-in-sql-client.html), the statements are not sent to the server until there is either a `COMMIT`, `ROLLBACK`, or two blank lines. This minimizes the lifetime of the transaction and therefore the risk of conflicts. [#10187](https://github.com/cockroachdb/cockroach/pull/10187)
...
- Clarified and expanded the [`transactions`](transactions.html) overview documentation, and completed docs on transaction-specific statements: [#672](https://github.com/cockroachdb/docs/pull/672)
    - [`BEGIN`](begin-transaction.html)
    - [`COMMIT`](commit-transaction.html)
    - [`SET TRANSACTION`](set-transaction.html)
    - [`SAVEPOINT cockroach_restart`](savepoint.html)
    - [`RELEASE SAVEPOINT cockroach_restart`](release-savepoint.html)
    - [`ROLLBACK`](rollback-transaction.html)
    - [`SHOW`](show-vars.html)
- Added language-specific examples for [`INSERT` statements with `RETURNING`](insert.html#insert-and-return-values). [#813](https://github.com/cockroachdb/docs/pull/813)
- Updated the [SQL Feature Support](sql-feature-support.html) page to reflect support for [views](views.html) (SQL standard) and [interleaved tables](interleave-in-parent.html) (CockroachDB extensions).
