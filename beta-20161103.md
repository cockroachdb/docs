...
- Users (other than `root`) must now be created (with the `CREATE USER` statement or `cockroach user set` command) before being used. [#9794](https://github.com/cockroachdb/cockroach/pull/9794)
...
- Password-based authentication is now supported. Clients can authenticate with either a TLS certificate or a password. [#9794](https://github.com/cockroachdb/cockroach/pull/9794)
...
- `SHOW TABLES` now adds a `(dropped)` suffix to tables that are being dropped. [#10063](https://github.com/cockroachdb/cockroach/pull/10063)
- `SHOW CREATE VIEW` now only requires permissions on the view, not the underlying tables. [#10270](https://github.com/cockroachdb/cockroach/pull/10270)

### Command-Line Interface Changes

- When entering a transaction in the command-line shell, the statements are not sent to the server until there is either a `COMMIT`, `ROLLBACK`, or two blank lines. This minimizes the lifetime of the transaction and therefore the risk of conflicts. [#10187](https://github.com/cockroachdb/cockroach/pull/10187)
