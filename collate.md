...
The `COLLATE` feature lets you lets you sort [`STRING`](string.html) values according to language- and country-specific rules, which are stored in collations (also known as collation keys).
...
However, every time a collated string is accessed, the collation is loaded into memory. This usually doubles the size of the string in memory, which means they perform slightly slower than their non-collated counterparts.
