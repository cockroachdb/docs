...
$ ./cockroach sql --execute='<sql-statement>' --execute='<sql-statement>' <flags>
...
Flag | Description 
-----|------------
`--ca-cert` | The path to the directory containing the [CA certificate](create-security-certificates.html). If the cluster was started with security (i.e., without the `--insecure` flag), this flag is required. 
`--cert` | The path to the directory containing the [client certificate](create-security-certificates.html). If the cluster was started with security (i.e., without the `--insecure` flag), this flag is required.
`--database`<br>`-d` | The database to connect to. The user specified in `--user` must have privileges to access the database. 
`--execute`<br>`-e` | Execute SQL statements directly from the command line, without opening a shell. Set this flag separately for each statement, and wrap each statement in quotes. See the [examples](#execute-sql-statements-from-the-command-line) below. <br><br>If an error occurs in any statement, the command exits with a non-zero status code and further statements are not executed. The results of each SQL statement are printed to the standard output.
...
`--key` | The path to the directory containing the [client key](create-security-certificates.html). If the cluster was started with security (i.e., without the `--insecure` flag), this flag is required.
...
# Using -e:
$ ./cockroach sql -e='CREATE DATABASE roaches' -e='CREATE TABLE roaches.countries (name STRING, code STRING, roach_population INT)' --insecure --user=maxroach --host=roachcluster.com --port=26257 --database=critterdb  
~~~