---
title: Connect a Go App to CockroachDB
summary: Learn how to connect a simple Go application to a CockroachDB cluster using either a low-level client driver or an ORM.
toc: false
asciicast: true
---

This tutorial shows you how to connect a simple Go application to a CockroachDB cluster using either a low-level client driver or an ORM. You can use any PostgreSQL-compatible client drivers or ORMs, but we've tested and can recommend the [Go pg driver](https://godoc.org/github.com/lib/pq) and [GORM ORM](http://jinzhu.me/gorm/), so those are featured here.
...
To install the [Go pg driver](https://godoc.org/github.com/lib/pq), run the following command:
...
For the purpose of this tutorial, you need only one CockroachDB node running:
...
However, if you want to validate that funds were, in fact, transferred from one account to another, you can again use the built-in SQL client:  
...
To validate that the table and rows were created successfully, you can again use the built-in SQL client:  
...
// Showing/hiding content with buttons.
$(document).ready(function(){
